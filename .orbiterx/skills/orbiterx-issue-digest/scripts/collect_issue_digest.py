#!/usr/bin/env python3
"""Collect recent openai/orbiterx issue activity for owner-focused digests."""
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Collect recent GitHub issue activity for a OrbiterX owner digest."
    )
    parser.add_argument(
        "--repo", default="openai/orbiterx", help="OWNER/REPO, default openai/orbiterx"
    )
    parser.add_argument(
        "--labels",
        nargs="+",
...
def clean_title_for_description(title):
    ...
    cleaned = re.sub(
        r"^(orbiterx(?: desktop| app|\.app| cli)?|desktop|windows orbiterx app)\s*[:,-]\s*",
        "",
        cleaned,
        flags=re.IGNORECASE,
    )
