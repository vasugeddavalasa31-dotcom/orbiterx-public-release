...
Use `TestOrbiterXBuilder::build_with_auto_env()` to opt-in to remote execution in core integration
...
`$ORBITERX_HOME/environments.toml` or will define custom environments at runtime.
...
in bash also provides the `orbiterx_remote_env_cleanup` function to use after testing.
...
bash -c '
  ...
  trap orbiterx_remote_env_cleanup EXIT

  cd orbiterx-rs
  just test -p orbiterx-core --test all
'
```
...
bash -c '
  ...
  cd orbiterx-rs
  just test -p orbiterx-app-server --test all
'
```
...
```sh
bazel test //orbiterx-rs/app-server:app-server-all-wine-exec-test
```
...
You can list devboxes via `applied_devbox ls`, pick the one with `orbiterx` in the name.
Connect to devbox via `ssh <devbox_name>`.
Reuse the same checkout of orbiterx in `~/code/orbiterx`. Reset files if needed. Multiple checkouts take longer to build and take up more space.
