set working-directory := "orbiterx-rs"
...
alias c := orbiterx
orbiterx *args:
    cargo run --bin orbiterx -- {args}

# `orbiterx exec`
exec *args:
    cargo run --bin orbiterx -- exec {args}

# Start `orbiterx exec-server` and run orbiterx-tui.
[no-cd]
...
file-search *args:
    cargo run --bin orbiterx-file-search -- {args}

# Run the standalone code-mode host from source.
code-mode-host *args:
    cargo run --bin orbiterx-code-mode-host -- {args}

# Build the CLI and run the app-server test client
app-server-test-client *args:
    cargo build -p orbiterx-cli
    cargo run -p orbiterx-app-server-test-client -- --orbiterx-bin ./target/debug/orbiterx {args}
...
bench-e2e:
    ...
    bazel test --compilation_mode=opt --cache_test_results=no --test_output=streamed //orbiterx-rs:e2e-benchmarks
...
bench-e2e-smoke:
    ...
    bazel test --compilation_mode=fastbuild --@rules_rust//rust/settings:extra_rustc_flag=-Cdebug-assertions=no --@rules_rust//rust/settings:extra_exec_rustc_flag=-Cdebug-assertions=no --cache_test_results=no --test_output=streamed --test_arg=--test //orbiterx-rs:e2e-benchmarks

# Build and run OrbiterX from source using Bazel.
# On Unix, use `[no-cd]` and `--run_under="cd $PWD &&"` to ensure Bazel runs
# the command in the current working directory.
[no-cd]
[unix]
bazel-orbiterx *args:
    bazel run //orbiterx-rs/cli:orbiterx --run_under="cd $PWD &&" -- "$@"

[windows]
bazel-orbiterx *args:
    bazel run //orbiterx-rs/cli:orbiterx --run_under='cd /d "{{ invocation_directory_native() }}" &&' -- @($args | Select-Object -Skip 1)

# Build and run the standalone code-mode host from source using Bazel.
[no-cd]
[unix]
bazel-code-mode-host *args:
    bazel run //orbiterx-rs/code-mode-host:orbiterx-code-mode-host --run_under="cd $PWD &&" -- "$@"

[windows]
bazel-code-mode-host *args:
    bazel run //orbiterx-rs/code-mode-host:orbiterx-code-mode-host --run_under='cd /d "{{ invocation_directory_native() }}" &&' -- @($args | Select-Object -Skip 1)
...
build-for-release:
    bazel build //orbiterx-rs/cli:release_binaries

# Run the MCP server
mcp-server-run *args:
    cargo run -p orbiterx-mcp-server -- {args}

# Regenerate the json schema for config.toml from the current config types.
write-config-schema:
    cargo run -p orbiterx-core --bin orbiterx-write-config-schema

# Regenerate vendored app-server protocol schema artifacts.
write-app-server-schema *args:
    cargo run -p orbiterx-app-server-protocol --bin write_schema_fixtures -- {args}

[no-cd]
write-hooks-schema:
    cargo run --manifest-path {{ justfile_directory() }}/orbiterx-rs/Cargo.toml -p orbiterx-hooks --bin write_hooks_schema_fixtures

# Run the argument-comment Dylint checks across orbiterx-rs.
[no-cd]
[unix]
...
[unix]
log *args:
    if [ "${1:-}" = "--" ]; then shift; fi; cargo run -p orbiterx-state --bin logs_client -- "$@"

[windows]
log *args:
    $forwarded_args = @($args | Select-Object -Skip 1); if ($forwarded_args.Count -gt 0 -and $forwarded_args[0] -eq "--") { $forwarded_args = @($forwarded_args | Select-Object -Skip 1) }; cargo run -p orbiterx-state --bin logs_client -- @forwarded_args
