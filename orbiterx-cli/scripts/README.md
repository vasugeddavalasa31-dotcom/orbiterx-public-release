...
./scripts/stage_npm_packages.py \
  ...
  --package orbiterx \
  --package orbiterx-responses-api-proxy \
  --package orbiterx-sdk
```
...
When `--package orbiterx` is provided, the staging helper builds the lightweight
`@openai/orbiterx` meta package plus all platform-native `@openai/orbiterx` variants
