 ...
 * finished) from message text blocks. These are MODEL context — never chat
 * content — and must not render as messages/cards in the UI.
 */
function stripInternalFragmentsFromBlocks(
  blocks: ContentBlock[]
): ContentBlock[] {
  let changed = false
  ...
  for (const block of blocks) {
    if (block.type === "text") {
      const text = block.text
        .replace(
          /<subagent_notification>[\s\S]*?<\/subagent_notification>/g,
          ""
        )
        .trim()
      if (text === block.text) {
        ...
        changed = true
        if (text.length > 0) next.push({ type: "text", text })
      }
    } else {
      next.push(block)
    }
...
/** Copy of `turn` with internal model-context fragments stripped from its
 *  text blocks (no-op when nothing changed — keeps reference stability). */
function cleanTimelineTurn(turn: MessageTurn): MessageTurn {
  const blocks = stripInternalFragmentsFromBlocks(turn.blocks)
  return blocks === turn.blocks ? turn : { ...turn, blocks }
}
...
export function buildStreamingTurnsFromLiveMessage(
  ...
  const getToolName = (info: ToolCallInfo): string => {
    ...
    if (cached !== undefined) return cached
    // The app-server transport reports command executions with
    // `kind: "command"` and `title` = the raw command string.
    // `inferLiveToolName` would normalize that whole string into garbage
    // (e.g. "python3cprintsumrange10"), so the renderer's bash/terminal
    // detection (isCommandTool) never matches and command cards fall back to
    // a generic tool with no "Ran" prefix / terminal icon. `kind === "command"`
    // is the authoritative signal (the transport only sets it for
    // commandExecution items) → canonical "bash". Engine-classified
    // read/search/list commands are still relabeled to Read/Grep/List by the
    // adapter's `applyCommandActionClassification` afterwards.
    const name =
      info.kind === "command"
        ? "bash"
        : inferLiveToolName({
  ...
  for (const block of content) {
    ...
    switch (block.type) {
      ...
      case "tool_call": {
        ...
        // Chained or unclassified shell commands (`head && echo && tail`,
        // `grep; echo; cat`, a lone `find <dir> -name x`) arrive as one opaque
        // `/bin/zsh -lc "..."` bash card because the engine classifies the
        // whole string as `unknown`. At completion, split them into
        // individually classified Read / Grep / List / Bash cards — official
        // Codex runs reads and searches as separate tool calls and shows one
        // labeled card per op. Commands the engine ALREADY classified cleanly
        // (a single `head` with `meta.commandActions: [read]`) are left to the
        // adapter, which carries the backend's authoritative absolute path.
        // Only at the final state: while streaming, keep the single card so
        // the live terminal output stays visible (this builder re-runs on
        // every batch, so the split takes over the moment it completes).
        const backendActions: any[] = Array.isArray(
          block.info.meta?.commandActions
        )
          ? (block.info.meta as { commandActions: any[] }).commandActions
          : []
        // Skip the split only when the engine classified the command as ONE
        // uniform context-lane kind (pure read / pure search / pure list) —
        // the adapter renders those as authoritative Read/Grep/List cards with
        // absolute paths. A MIXED classification (`ls && cat` →
        // `[listFiles, read]`) can't map to a single card title, so the
        // splitter takes over and renders per-op cards — otherwise the whole
        // chain degrades to one generic bash card.
        const backendKinds = new Set(backendActions.map((a: any) => a.type))
        const isContextLaneKind = (t: string) =>
          t === "read" || t === "search" || t === "listFiles"
        const backendSingleContextKind =
          backendActions.length > 0 &&
          backendKinds.size === 1 &&
          isContextLaneKind([...backendKinds][0])
        // The app-server transport reports command executions with
        // `kind: "command"` and `title` = the raw command string. `inferLiveToolName`
        // falls through to a normalized form of that whole string (e.g.
        // "binzshlcpwdlslafind..."), NOT "bash"/"shell", so the tool name alone
        // cannot gate the split. `kind === "command"` is the authoritative signal
        // (the transport only sets it for commandExecution items); the tool-name
        // check additionally covers other agents that name the shell tool directly.
        const isShellCommandTool =
          block.info.kind === "command" ||
          toolName === "bash" ||
          toolName === "shell" ||
          toolName === "exec_command"
        if (isFinalState && !backendSingleContextKind && isShellCommandTool) {
          ...
          if (segments) {
            segments.forEach((seg, i) => {
              const segToolName = segmentToolName(seg.kind)
              currentBlocks.push({
                type: "tool_use",
                tool_use_id: `${block.info.tool_call_id}::seg${i}`,
                tool_name: segToolName,
              ...
              currentBlocks.push({
                type: "tool_result",
                tool_use_id: `${block.info.tool_call_id}::seg${i}`,
                output_preview: seg.output ?? null,
    ...
    case "APPLY_BACKGROUND_ACTIVITY": {
      ...
      return updateSessionInState(state, action.conversationId, (current) => {
        ...
        for (const turn of action.turns) {
          const cleanBlocks = stripInternalFragmentsFromBlocks(turn.blocks)
          const cleanTurn =
            cleanBlocks === turn.blocks
              ? turn
              : { ...turn, blocks: cleanBlocks }
...
function computeTimelinePrefix(
  ...
  const background: ConversationTimelineTurn[] = session.backgroundTurns.map(
    (entry) => ({
      ...
      turn: cleanTimelineTurn(entry.turn),
      phase: "persisted" as const,
    })
  )
...
export function getConversationIdByExternalIdFromStore(
  ...
  const storeMap =
    useConversationRuntimeStore.getState().conversationIdByExternalId
  return storeMap.get(externalId) ?? convIdFromThreadId(externalId)
}
