...
describe("groupConsecutiveToolCalls", () => {
  it("leaves Codex goal calls and command tools standalone as cards", () => {
  ...
  it("collapses consecutive context tools (read/search/fetch) into one group", () => {
    const out = groupConsecutiveToolCalls([
      poll("read"),
      poll("grep"),
      poll("read"),
    ...
    expect(out.map((p) => p.type)).toEqual(["tool-group"])
    expect(out[0].type === "tool-group" ? out[0].items.length : 0).toBe(3)
  })

  it("keeps command tools out of the context group (own cards)", () => {
    const out = groupConsecutiveToolCalls([
      poll("read"),
      poll("bash"),
      poll("read"),
...
describe("commandActionVirtualToolName", () => {
  const metaOf = (actions: unknown[] | null) =>
    actions ? { commandActions: actions } : null

  it("classifies a cat/head read command as read", () => {
    expect(
      commandActionVirtualToolName(
        metaOf([
          {
  ...
  it("classifies an rg/grep search command as grep", () => {
    expect(
      commandActionVirtualToolName(
        metaOf([
          {
  ...
  it("classifies an ls/find listing as list_files", () => {
    expect(
      commandActionVirtualToolName(
        metaOf([{ type: "listFiles", command: "ls -la", path: null }])
      )
    ).toBe("list_files")
  })

  it("leaves mixed or unknown commands unclassified", () => {
    expect(
      commandActionVirtualToolName(
        metaOf([
          { type: "read", command: "cat a", name: "a", path: "/x/a" },
          { type: "unknown", command: "npm build" },
        ])
      )
    ).toBeNull()
    expect(
      commandActionVirtualToolName(
        metaOf([{ type: "unknown", command: "npm test" }])
      )
    ).toBeNull()
    expect(commandActionVirtualToolName(metaOf(null))).toBeNull()
  })

  it("classifies a compound read chained with echo separators as read", () => {
    // `head -n 20 f && echo "----TAIL----" && tail -n 10 f` parses to
    // [read, unknown(echo), read]. The echo is a label separator, not a
    // second action kind — the command must fold into the "Explored" group
    // as a single Read instead of degrading to a generic bash card.
    expect(
      commandActionVirtualToolName(
        metaOf([
          { type: "read", command: "head -n 20 f", name: "f", path: "/x/f" },
          { type: "unknown", command: "echo ----TAIL----" },
          { type: "read", command: "tail -n 10 f", name: "f", path: "/x/f" },
        ])
      )
    ).toBe("read")
  })

  it("classifies a printf separator between reads as read", () => {
    expect(
      commandActionVirtualToolName(
        metaOf([
          { type: "read", command: "cat a", name: "a", path: "/x/a" },
          { type: "unknown", command: "printf '%s\\n' ---" },
          { type: "read", command: "cat b", name: "b", path: "/x/b" },
        ])
      )
    ).toBe("read")
  })

  it("does not classify an all-echo command (nothing meaningful)", () => {
    expect(
      commandActionVirtualToolName(
        metaOf([{ type: "unknown", command: "echo hello" }])
      )
    ).toBeNull()
  })

  it("classifies command-lane shell calls as context tools for grouping", () => {
    const classifiedRead: AdaptedToolCallPart = {
      type: "tool-call",
      toolCallId: "c1",
      toolName: "shell",
      input: "cat README.md",
    ...
    expect(isContextGroupToolPart(classifiedRead)).toBe(true)
  })
})
...
describe("dropHiddenFeedbackChecks", () => {
  ...
  it("keeps command + context lanes separate once a no-op check is dropped", () => {
    const grouped = groupConsecutiveToolCalls(
      dropHiddenFeedbackChecks([
        poll("exec_command"),
        feedbackCheck(NO_FEEDBACK_OUT),
    ...
    // Without the drop, the standalone check would sit between the command card
    // and the context group. Command stays its own card; read collapses alone.
    expect(grouped.map((p) => p.type)).toEqual(["tool-call", "tool-group"])
  })
