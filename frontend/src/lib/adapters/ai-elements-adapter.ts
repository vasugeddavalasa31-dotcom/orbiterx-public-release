...
function adaptContentBlock(
  ...
  switch (block.type) {
    ...
    case "tool_use":
      return applyCommandActionClassification({
        type: "tool-call",
        toolCallId:
...
/**
 * Merge ALL tool-group parts in an assistant response into a single
 * "Explored" group, placed at the first group's position.
 *
 * `groupConsecutiveToolCalls` intentionally breaks the run whenever a
 * text/reasoning part intervenes, which is why a multi-step answer can render
 * "Explored…" several times (tools → interim text → tools). For the working
 * section we want one group holding every context-lane tool call of the
 * response, in call order, with the interim text and final message flowing
 * around it.
 */
export function mergeAllToolGroups(
  parts: AdaptedContentPart[]
): AdaptedContentPart[] {
  const allItems: AdaptedToolCallPart[] = []
  let firstGroupIndex = -1
  for (let i = 0; i < parts.length; i++) {
    const part = parts[i]
    if (part.type !== "tool-group") continue
    if (firstGroupIndex < 0) firstGroupIndex = i
    allItems.push(...part.items)
  }
  if (firstGroupIndex < 0) return parts
  const merged: AdaptedContentPart = {
    type: "tool-group",
    items: allItems,
    isStreaming: allItems.some(
      (item) =>
        item.state === "input-streaming" || item.state === "input-available"
    ),
  }
...
/**
 * Wire shape of the engine's shell-command classification (`command_actions`
 * on a `commandExecution` item). The backend parses every shell command the
 * model runs (`cat`/`head` → read, `rg`/`grep` → search, `ls`/`find` →
 * listFiles) so the UI can render a Read / Search / List card instead of a
 * generic bash card — the same mechanism the official Codex app uses.
 */
export type CommandActionWire =
...
function commandActionsOf(
  meta: Record<string, unknown> | null | undefined
): CommandActionWire[] | null {
  if (!meta || !Array.isArray(meta.commandActions)) return null
  return meta.commandActions as CommandActionWire[]
}

/**
 * A shell command that exists purely to print a label or separator between
 * real work (e.g. `echo "----TAIL----"` in `head && echo && tail`) parses to a
 * `CommandAction::Unknown`. When such noise actions accompany exactly one
 * meaningful context-lane kind, they should not disqualify the whole command
 * from folding into the "Explored" group — the UI shows a single "Read <path>"
 * row instead of a generic bash card.
 */
function isTrivialEchoAction(action: CommandActionWire): boolean {
...
/**
 * The actions that actually do work, dropping trivial `echo`/`printf`
 * separators. `null` when there is no meta, and empty when every action is a
 * label printer (nothing meaningful to classify).
 */
function meaningfulCommandActions(
  meta: Record<string, unknown> | null | undefined
): CommandActionWire[] | null {
  const actions = commandActionsOf(meta)
  if (!actions || actions.length === 0) return null
  const meaningful = actions.filter((action) => !isTrivialEchoAction(action))
  return meaningful.length === 0 ? null : meaningful
}

/**
 * When a shell command's classification is a single unambiguous
 * read/search/listFiles action (ignoring trivial `echo`/`printf` separators),
 * return the virtual tool name that routes it into the context lane
 * (`read` / `grep` / `list_files`). Returns `null` for mixed or unknown
 * commands, which keep their generic command rendering.
 */
export function commandActionVirtualToolName(
  meta: Record<string, unknown> | null | undefined
): "read" | "grep" | "list_files" | null {
  const meaningful = meaningfulCommandActions(meta)
  if (!meaningful) return null
  const kinds = new Set(meaningful.map((a) => a.type))
  if (kinds.size !== 1) return null
  switch (meaningful[0].type) {
    case "read":
      return "read"
    case "search":
      return "grep"
...
/**
 * Synthesize a model-facing input for a classified command action so the
 * existing title derivation renders "Read <path>" / "Grep <query>" /
 * "Glob <pattern>" instead of the raw shell command. Uses the first
 * MEANINGFUL action — a leading `echo` separator in `head && echo && tail`
 * must not become the synthesized input.
 */
function synthesizedCommandActionInput(
  meta: Record<string, unknown> | null | undefined,
  virtual: "read" | "grep" | "list_files"
): string | null {
  const meaningful = meaningfulCommandActions(meta)
  const action = meaningful?.[0]
  if (!action) return null
  if (virtual === "read" && action.type === "read") {
    return JSON.stringify({ file_path: action.path || action.name })
  }
  if (virtual === "grep" && action.type === "search") {
    return JSON.stringify({ pattern: action.query ?? action.path ?? "" })
  }
  if (virtual === "list_files" && action.type === "listFiles") {
    return JSON.stringify({ pattern: action.path ?? "*" })
  }
  return null
}

/**
 * Relabel a shell command whose engine classification is read/search/list
 * into the matching context-lane tool, with a synthesized input the title
 * derivation understands. The raw command stays available on the part's
 * `meta.commandActions` for anyone who needs the exact invocation.
 */
function applyCommandActionClassification(
  part: AdaptedToolCallPart
): AdaptedToolCallPart {
  const virtual = commandActionVirtualToolName(part.meta)
  if (!virtual) return part
  const synthesized = synthesizedCommandActionInput(part.meta, virtual)
  if (!synthesized) return part
  return { ...part, toolName: virtual, input: synthesized }
}

/**
 * Classify a constructed tool-call part using the engine's shell-command
 * classification (forwarded on `meta.commandActions`). Every tool-call part —
 * whether built inline during `adaptMessageTurn` or via `adaptContentBlock` —
 * must route through this so shell commands the engine classified as
 * read/search/list render as Read/Search/List cards and fold into the
 * "Explored" group instead of generic bash cards.
 */
function classifyToolCallPart(part: AdaptedToolCallPart): AdaptedToolCallPart {
  return applyCommandActionClassification(part)
}

/**
 * Whether a tool-call belongs to the "explore" lane that collapses into a
 * single tool-group (read / search / list / fetch / web). Mirrors opencode's
 * `isContextGroupTool`: only the context-gathering tools fold into the
 * "Explored · Read 3, Search 1" group. Command-lane tools (bash, edit,
 * apply_patch, write) render as their own cards, and agent-like / plan-mode /
 * background / compaction lanes keep their dedicated renderers. Shell
 * commands the engine classified as read/search/list also fold in.
 */
export function isContextGroupToolPart(part: AdaptedToolCallPart): boolean {
  if (
  ...
  const kind = classifyToolKind(part.toolName)
  if (
    kind === "read" ||
    kind === "search" ||
    kind === "list" ||
  ...
  return commandActionVirtualToolName(part.meta) !== null
}

/**
 * Wrap any consecutive run of context-lane tool-call parts into a single
 * tool-group. Command-lane tools (bash / exec_command / edit / apply_patch /
 * write) and the specialized lanes (agent, plan-mode, background-task,
 * context-compaction) break the run and render standalone as their own cards —
 * matching the opencode "shell gets its own card, reads collapse into one
 * Explored group" layout. Text, reasoning, tool-result and any other part type
 * also break the run. Even a single context tool is wrapped, so the renderer
 * can present a uniform collapsed summary across history.
 */
export function groupConsecutiveToolCalls(
  ...
  for (const part of parts) {
    if (part.type === "tool-call" && isContextGroupToolPart(part)) {
      buffer.push(part)
      continue
    }
    flush()
...
/**
 * Split an unclassified shell tool_use (`exec_command` / `bash` / `shell`)
 * into individually classified Read / Grep / List / Bash cards.
 *
 * The DESKTOP CodexParser emits restored commands as raw
 * `ToolUse { tool_name: "exec_command", input_preview: cmd, meta: None }` —
 * no `commandActions` — so a chained `head && echo && tail` would otherwise
 * render as one opaque bash card after reload. The web `thread/read` mapper
 * and the live store split chained commands up front; this adapter hook is
 * the safety net that covers desktop restore (and any other consumer feeding
 * raw shell tools). Commands the engine already classified
 * (`meta.commandActions`) are left alone — `applyCommandActionClassification`
 * carries the authoritative Read/Grep/List path.
 *
 * Returns `null` to keep the single-card rendering (generic commands,
 * pipelines, heredocs — the splitter declines them).
 */
function splitUnclassifiedShellToolCall(args: {
  toolName: string
  input: string | null
  output: string | null
  isError: boolean
  toolCallId: string
  meta: Record<string, unknown> | null
}): AdaptedToolCallPart[] | null {
  const { toolName, input, output, isError, toolCallId, meta } = args
  ...
  if (
    meta &&
    Array.isArray(meta.commandActions) &&
    meta.commandActions.length > 0
  ) {
    return null
  }
  const segments = splitChainedCommandWithOutputs(input ?? "", output)
  if (!segments) return null
  return segments.map((seg, i) =>
    classifyToolCallPart({
      type: "tool-call",
      toolCallId: `${toolCallId}::seg${i}`,
      toolName: segmentToolName(seg.kind),
      input: synthesizeSegmentInput(seg) ?? seg.command,
      state: isError ? "output-error" : "output-available",
      output: seg.output ?? undefined,
      errorText: isError ? seg.output || undefined : undefined,
      meta: null,
    })
  )
}
...
export function adaptMessageTurn(
  ...
  for (let index = 0; index < turn.blocks.length; index++) {
    ...
    if (block.type === "tool_use") {
      ...
      if (matchedResult) {
        matchedResultIds.add(block.tool_use_id!)
        // Desktop-restored shell commands arrive without `commandActions`;
        // split them into Read/Grep/List cards (matches live + web restore).
        if (!isToolStillRunning && !isStreaming) {
          const splitParts = splitUnclassifiedShellToolCall({
            toolName: block.tool_name,
            input: block.input_preview,
            output: matchedResult.output_preview,
            isError: matchedResult.is_error,
            toolCallId,
            meta: block.meta ?? null,
          })
          if (splitParts) {
            adaptedContent.push(...splitParts)
            continue
          }
        }
        ...
        if (positionalResult) {
          ...
          if (!isStreaming) {
            const splitParts = splitUnclassifiedShellToolCall({
              ...
              isError: positionalResult.is_error,
              toolCallId,
              meta: block.meta ?? null,
            })
            if (splitParts) {
  ...
  const groupedContent =
    turn.role === "assistant"
      ? // One "Explored" group per response: every context-lane tool call of
        // the turn folds into a single group (at the first group's position)
        // instead of a separate group per tool-run separated by text/reasoning.
        mergeAllToolGroups(
          groupGoalRuns(
