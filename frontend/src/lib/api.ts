...
import { getOrbiterxToken } from "./transport/web-auth"
...
/** Start a code review of the current changes on the app-server (`review/start`).
 *  `target` defaults to reviewing uncommitted changes inline; delivery can be
 *  `"inline"` (default) or `"detached"`. Resolves to the review turn + thread. */
export async function acpReviewStart(
  connectionId: string,
  target?: unknown,
  delivery?: "inline" | "detached"
): Promise<unknown> {
  return getTransport().call("acp_review_start", {
    connectionId,
...
/** Trigger a session context compaction on the app-server (`thread/compact/start`). */
export async function acpCompactStart(connectionId: string): Promise<void> {
  return getTransport().call("acp_compact_start", { connectionId })
}
...
/** List configured MCP servers + their tool/resource inventory straight from
 *  the app-server (`mcpServerStatus/list`). Returns the raw `{data, nextCursor}`
 *  response. */
export async function acpMcpServerStatus(): Promise<{
  data: Array<{
    name: string
    serverInfo?: unknown
  ...
  return getTransport().call("acp_mcp_server_status", {})
}

/** Ask the app-server to reload MCP servers from config (`config/mcpServer/
 *  reload`). */
export async function acpMcpServerReload(): Promise<unknown> {
  return getTransport().call("acp_mcp_server_reload", {})
}
...
/**
 * Persist the user's gateway API key (their landing-page session token, which
 * is auto-registered in TiDB on login) as the `orbiterx-gateway` provider's
