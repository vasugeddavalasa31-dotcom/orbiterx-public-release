...
const EXACT_TOOL_NAME_ALIASES: Record<string, string> = {
  ...
  writefile: "write",
  editfile: "edit",
  // The app-server transport reports a `fileChange` item with `kind="edit"`
  // and `title="File Change"`; without this alias the card resolves to the
  // literal name and renders as a generic tool (title "File Change", no Edit
  // path, no hidden duplicate) instead of an edit card with an "Edit <path>"
  // title + diff body. `canonicalizeToolName("File Change")` → `file_change`
  // (spaces become underscores); `filechange` covers the separator-stripped
  // form some paths pass.
  file_change: "edit",
  filechange: "edit",
  // Cline
  ...
  // `list_files` keeps its own name so classified directory listings render
  // as "List <dir>" (via deriveToolTitle) instead of being folded into the
  // glob matcher ("Glob <dir>"). Both still classify as the search lane.
