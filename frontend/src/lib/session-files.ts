...
/**
 * The change entries of an edit tool input, across every shape producers use:
 *   - LIVE app-server `fileChange`: `changes` is the raw payload — a TOP-LEVEL
 *     array of `{ path, kind, diff }` (the transport forwards
 *     `JSON.stringify(item.changes)` directly).
 *   - Web `thread/read` restore: `{ changes: [{ path, kind, diff }] }`.
 *   - Other agents: `{ changes: { "path": { old_string, new_string } } }`
 *     (path→value map).
 *
 * Returns `null` when the input carries no usable changes (so callers fall
 * through to the generic edit paths). This is what lets the "Files changed"
 * footer count real `+`/`-` lines for the live fileChange array — previously
 * `parseInputObject` rejected the top-level array, so stats read `+0 -0`.
 */
function extractEditChangeEntries(
  inputPreview: string | null
): unknown[] | null {
  if (!inputPreview) return null
  let parsed: unknown
...
function pickValueByNormalizedPath(
  ...
  // App-server `fileChange` shape: `changes` is an ARRAY of
  // `{ path, kind, diff }` entries (not a path→value map). Match the entry
  // whose `path`/`file_path` equals the target file so its diff stats land on
  // the right file (and aren't lost to `+0 -0`).
  if (Array.isArray(record)) {
    for (const item of record) {
      const entry = asObjectLike(item)
      if (!entry) continue
      const entryPath = firstStringField(entry, ["path", "file_path"])
      if (entryPath && normalizePath(entryPath.trim()) === filePath) {
        return entry
      }
    }
    return undefined
...
function extractFilePaths(inputPreview: string | null): string[] {
  ...
  if (parsed) {
    ...
    if (editChanges) {
      if (Array.isArray(editChanges)) {
        // App-server `fileChange` array shape (`[{ path, kind, diff }]`).
        for (const change of editChanges) {
          const record = asObjectLike(change)
          if (!record) continue
          const entryPath = firstStringField(record, ["path", "file_path"])
          if (entryPath) paths.add(normalizePath(entryPath.trim()))
        }
      } else {
    ...
    // Live app-server `fileChange` is a TOP-LEVEL array (`JSON.stringify
    // (item.changes)`) — `parseInputObject` rejects arrays, so read the
    // entries directly here.
    const changeEntries = extractEditChangeEntries(inputPreview)
    if (changeEntries) {
...
function computeLineDiff(
  ...
  if (op === "edit") {
    // Live app-server `fileChange` is a TOP-LEVEL `[{path,kind,diff}]` array
    // (transport forwards `JSON.stringify(item.changes)`); web restore wraps
    // it as `{ changes: [...] }`. Both are handled by `extractEditChangeEntries`
    // so the footer counts real +/− lines instead of `+0 -0`.
    ...
    // Top-level array shape: match the entry by path directly.
    const changeEntries = extractEditChangeEntries(inputPreview)
    if (changeEntries && normalizedPath) {
      for (const entry of changeEntries) {
        const record = asObjectLike(entry)
        if (!record) continue
        const entryPath = firstStringField(record, ["path", "file_path"])
        if (!entryPath || normalizePath(entryPath.trim()) !== normalizedPath) {
          continue
        }
        const changeValues = collectEditChangeValues(entry)
...
function buildDiffChunk(
  ...
  if (op === "edit") {
    ...
    // Live app-server `fileChange` top-level array (`JSON.stringify
    // (item.changes)`): match the entry by path and render its diff.
    const changeEntries = extractEditChangeEntries(inputPreview)
    if (changeEntries) {
      for (const entry of changeEntries) {
        const record = asObjectLike(entry)
