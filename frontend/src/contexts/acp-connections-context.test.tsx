...
beforeEach(() => {
  ...
  h.updateConversationStatus.mockReset()
  h.updateConversationStatus.mockResolvedValue(undefined)
...
describe("AcpConnectionsProvider liveMessage sink (mirror out of React)", () => {
  ...
  it("marks child-thread live-message mirrors as live even while status is connected (runtime replay-guard bypass)", async () => {
    // A mid-run child-tab attach never observes the prompting status event, so
    // its connection stays "connected" while the delegated run streams. The
    // runtime store rejects non-live mirrors once persisted detail exists, so
    // the mirror must pass isLive=true for child threads — otherwise the
    // sub-agent's live reply never reaches the message list.
    h.acpFindConnectionForConversation.mockResolvedValue(null)
    await mountProvider()
    await act(async () => {
    ...
    const handlers = latestAttachHandlers()
    const calls: Array<{ isLive: boolean }> = []
    h.actions!.registerLiveMessageSink(TAB, (_lm, isLive) =>
      calls.push({ isLive })
    )

    // Status stays connected (the missed turn/started), yet the streamed
    // deltas still apply (guard exemption) and the sink must report live.
    emitAcpEvent(handlers, {
      seq: 1,
      connection_id: "spawned-conn",
      type: "status_changed",
    ...
    const liveCalls = calls.filter((c) => c.isLive)
  ...
  it("strips internal <subagent_notification> fragments from streamed text", async () => {
    const handlers = await connectOwner()
    emitAcpEvent(handlers, {
      seq: 1,
      connection_id: "spawned-conn",
    ...
    emitAcpEvent(handlers, {
      ...
      connection_id: "spawned-conn",
      type: "content_delta",
      text: '<subagent_notification>{"agent_path":"x","status":{"completed":"done"}}</subagent_notification>',
    })
    emitAcpEvent(handlers, {
      seq: 3,
      connection_id: "spawned-conn",
...
describe("out-of-turn wire guard + background activity", () => {
  ...
  it("streams deltas and tool calls for child-thread connections even while status is connected (late-attach exemption)", async () => {
    // Child tab resume: connect with isChildThread=true. A mid-run child tab
    // attach misses the child's `turn/started`, so no prompting status event
    // ever arrives — but the delegated run's live content must still stream.
    h.acpFindConnectionForConversation.mockResolvedValue(null)
    await mountProvider()
    await act(async () => {
      await h.actions!.connect(
    ...
    const handlers = latestAttachHandlers()
    const conn = h.store!.getConnection(TAB)
    expect(conn?.isChildThread).toBe(true)
    ...
    // Without the exemption these deltas would be dropped by the out-of-turn
    // guard (the exact "sub-agent doesn't live stream" bug).
    emitAcpEvent(handlers, {
      seq: 2,
      connection_id: "spawned-conn",
      type: "content_delta",
    ...
    expect(h.store!.getConnection(TAB)?.liveMessage?.content).toEqual([
      { type: "text", text: "child reply streaming" },
    ])

    // Tool calls route into the live message for child threads as well (not
    // the invisible out-of-turn registry).
    emitAcpEvent(handlers, {
      seq: 5,
      connection_id: "spawned-conn",
      type: "tool_call",
    ...
    const afterTool = h.store!.getConnection(TAB)
    expect(
      afterTool?.liveMessage?.content.some(
        (b) => b.type === "tool_call" && b.info.tool_call_id === "call-1"
      )
    ).toBe(true)

    // The child's turn_complete latches completion (so a late-attached viewer
    // can distinguish a finished run from a still-streaming one) and settles
    ...
    const completed = h.store!.getConnection(TAB)
    expect(completed?.turnCompleted).toBe(true)
    expect(completed?.status).toBe("connected")
    // Child threads have no DB row — status persistence is skipped entirely
    // (it would fail with "Conversation not found" on the app-server hash id).
    expect(h.updateConversationStatus).not.toHaveBeenCalled()
  })

  it("keeps the out-of-turn guard for ordinary connections (child flag unset)", async () => {
    const handlers = await mountOwnerConnection()
    expect(h.store!.getConnection(TAB)?.isChildThread).toBe(false)
    emitAcpEvent(handlers, {
      seq: 1,
      connection_id: "spawned-conn",
      type: "content_delta",
  ...
  it("does NOT arm the syncing-results hint for a wire-visible (#870-held) settle", async () => {
    const { resetConversationRuntimeStore } = await import(
