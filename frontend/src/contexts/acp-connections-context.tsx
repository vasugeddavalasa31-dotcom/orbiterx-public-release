...
export interface ConnectionState {
  ...
  /**
   * True when this connection backs a sub-agent child thread — either the
   * delegation-spawned synthetic entry (`isDelegationChild`) or a child
   * session tab opened via the collab card. Child-thread connections exist
   * ONLY while the delegated run is active, so their wire events are the run
   * itself — they must never be treated as out-of-turn background activity,
   * even when the attach happens late and misses the child's prompting status
   * event (mid-run tab open / reload). The out-of-turn guard therefore
   * exempts them so live deltas and tool calls stream regardless of whether
   * `status` ever read "prompting".
   */
  isChildThread: boolean
  /**
   * True once this connection has observed `turn_complete`. Lets a
   * late-attached child viewer distinguish a finished run (adopt/promote the
   * retained reply) from a still-streaming one that merely hasn't surfaced its
   * prompting status.
   */
...
type ConnectRequest = {
  ...
  // True when the connection backs a sub-agent child thread (child tab). Not
  // part of `sameConnectRequest` equality — sessionId already distinguishes a
  // child resume — but carried so a re-fired pending request re-marks it.
  isChildThread?: boolean
}
...
type Action =
  | {
      ...
      isViewer?: boolean
      // Set when the connection backs a sub-agent child thread (child tab).
  ...
  | { type: "TURN_COMPLETED"; contextKey: string }
  | {
      type: "STATUS_CHANGED"
      contextKey: string
      status: ConnectionStatus
...
/**
 * Parse a conversation context key (e.g. `"conv-1-claude_code-42"`) and return
 * its numeric conversationId. Returns `null` for non-conversation keys such as
 * delegation child IDs or `"new-..."` draft tab IDs.
 */
function parseContextKey(
  contextKey: string
): { conversationId: number } | null {
  if (!contextKey.startsWith("conv-")) return null
  const dashIdx = contextKey.lastIndexOf("-")
  if (dashIdx < 0) return null
  const id = Number(contextKey.slice(dashIdx + 1))
  if (!Number.isFinite(id) || id <= 0) return null
  return { conversationId: id }
}

/**
 * Resolve the sidebar conversation row for a live connection's status events.
 *
 * Tried in order:
 * 1. the external-id -> conversation-id mapping (populated by the send/connect
 *    path; keyed by the agent session id on the desktop path and the thread id
 *    on the app-server path);
 * 2. the canonical `conv-<folderId>-<agentType>-<id>` context key (covers
 *    restored/reopened sessions whose mapping entry was never registered);
 * 3. a scan of the workspace store for a row whose `external_id` matches one of
 *    the connection's ids — covers delegation children and any other context
 *    key that isn't `conv-...` (so `parseContextKey` can't resolve).
 *
 * Returns `null` when none of the ids resolve to a sidebar-visible row.
 */
function resolveConversationIdForStatusUpdate(
  contextKey: string,
  externalIds: Array<string | null | undefined>
): number | null {
  // The external-id map can hold the VIRTUAL (negative) draft id: the
  // session-binding effect re-maps `threadId → effectiveConversationId` every
  // time the DB id resolves, clobbering the real-row binding. The sidebar rows
  // are keyed by REAL DB ids, so a virtual id never matches — dereference the
  // session's `dbConversationId` whenever the resolved id is virtual.
  const dereference = (id: number | null): number | null => {
    if (id == null || id > 0) return id
    const session = useConversationRuntimeStore
      .getState()
      .byConversationId.get(id)
    const dbId = session?.dbConversationId
    return dbId != null && dbId > 0 ? dbId : id
  }
  for (const externalId of externalIds) {
    if (!externalId) continue
    const viaMapping = getConversationIdByExternalIdFromStore(externalId)
    if (viaMapping != null) return dereference(viaMapping)
  }
  const viaContextKey = parseContextKey(contextKey)?.conversationId ?? null
  if (viaContextKey != null) return dereference(viaContextKey)
  const conversations = useAppWorkspaceStore.getState().conversations
  for (const externalId of externalIds) {
    if (!externalId) continue
    const viaRow = conversations.find((c) => c.external_id === externalId)
    if (viaRow) return viaRow.id
  }
  return null
}

/**
 * Whether `conversationId` is a real, persisted conversation row in the local
 * store's conversation list (Tauri SQLite rows on desktop; app-server threads
 * mapped by the transport on web). Some ids reach the status handlers without
 * a row — child-thread hash ids, draft ids, or app-server threads the local
 * DB never created — and persisting their status rejects with "{}" on every
 * status flip. Status is local bookkeeping anyway (the app-server thread
 * status is authoritative), so skip the DB write for non-rows.
 */
function isPersistedConversationRow(conversationId: number): boolean {
 ...
 */
function stripInternalFragments(text: string): string {
  return text
...
function applyStreamingAction(
  ...
  // Child-thread connections are exempt: a delegation child / child tab exists
  // ONLY for the duration of its delegated run, so its deltas ARE the run and
  // must stream even when the attach missed the child's prompting status
  // event (mid-run open, reload, stale binding). Without this, the sub-agent's
  // live tool calls + reply never render until the persisted detail is
  // re-fetched (the "open/close then it shows" symptom).
  if (conn.status !== "prompting" && !conn.isChildThread) {
  ...
  if (action.type === "CONTENT_DELTA") {
    if (lastBlock?.type === "text") {
      const text = stripInternalFragments(lastBlock.text + action.text)
      newContent =
        text.length === 0
          ? prev.content.slice(0, -1)
          : [...prev.content.slice(0, -1), { type: "text", text }]
    } else {
      const text = stripInternalFragments(action.text)
      if (text.length === 0) return null
      newContent = [...prev.content, { type: "text", text }]
    }
  } else {
  ...
  switch (action.type) {
    case "CONNECTION_CREATED": {
      ...
      next.set(action.contextKey, {
        ...
        isChildThread: action.isChildThread ?? false,
        turnCompleted: false,
    ...
    case "DELEGATION_CHILD_ATTACH": {
      ...
      next.set(action.contextKey, {
        ...
        isChildThread: true,
        // The child is spawned mid-run by definition; a late attach may miss
        // its `turn/started`, so the prompting edge never fires and the
        // out-of-turn guard would otherwise drop the whole live stream.
        // `isChildThread` exempts the guard; the run's own `turn_complete`
        // marks `turnCompleted` and settles the stream.
        turnCompleted: false,
    ...
    case "HYDRATE_FROM_SNAPSHOT": {
      ...
      // Latched once-per-lifetime like `selectorsReady`: the attach snapshot
      // is the ONLY source of the thread UUID on the app-server path (there is
      // no lifecycle `session_started` notification), and it may land STALE —
      // the snapshot carries the synthetic `event_seq: 1` while real WS events
      // share the transport's GLOBAL seq counter, so a few in-flight events
      // push `lastAppliedSeq` past 1 before `fetchModelOptions` resolves. The
      // stale branch below preserves `current.sessionId`; without this latch a
      // null `sessionId` is never recovered, the detail panel never binds the
      // thread UUID to the sidebar row, and every conversation stays
      // "in_progress" after its turn completes.
      const mergedSessionId = current.sessionId ?? action.patch.sessionId
      ...
      if (action.patch.eventSeq <= current.lastAppliedSeq) {
        if (
          ...
          mergedSessionId === current.sessionId
        ) {
          return state
        }
        const next = new Map(state)
    ...
    case "STATUS_CHANGED": {
      ...
      if (!conn) return state
      const next = new Map(state)
      const updated: ConnectionState = { ...conn, status: action.status }
      if (action.status === "prompting") {
        // A new turn is in flight — clear the completion latch so a late
        // attach of the next run isn't mistaken for a finished one.
        updated.turnCompleted = false
        updated.liveMessage = {
          id: randomUUID(),
          role: "assistant",
          content: [],
    ...
    case "TURN_COMPLETED": {
      const conn = state.get(action.contextKey)
      if (!conn || conn.turnCompleted) return state
      const next = new Map(state)
      next.set(action.contextKey, { ...conn, turnCompleted: true })
      return next
    }
    ...
    case "TOOL_CALL": {
      ...
      // Child-thread connections are exempt (same rationale as
      // `applyStreamingAction`): their tool calls are part of the delegated
      // run and must land in the live message even when the prompting status
      // event was missed.
      if (conn.status !== "prompting" && !conn.isChildThread) {
        const next = new Map(state)
        next.set(action.contextKey, {
...
export interface AcpActionsValue {
  connect(
    ...
    conversationId?: number,
    /**
     * True when this connection backs a sub-agent child thread (a child
     * session tab). Marks the connection so its live stream is exempt from
     * the out-of-turn guard — see `ConnectionState.isChildThread`.
     */
    isChildThread?: boolean
  ): Promise<void>
...
export function AcpConnectionsProvider({ children }: { children: ReactNode }) {
  ...
  const dispatch = useCallback(
    (action: Action) => {
      ...
      const mirrorLiveMessage = (key: string) => {
        ...
        if (!nextConn || nextConn.liveMessage == null) {
          return
        }
        if (nextConn.liveMessage === prev.get(key)?.liveMessage) return
        // Child-thread connections stream their delegated run even when the
        // attach missed the prompting status event (status stays "connected").
        // The runtime store's SET_LIVE_MESSAGE guard treats non-live mirrors
        // as reconnect replays and rejects them once persisted detail exists —
        // so a mid-run child whose detail fetch already returned the partial
        // in-progress turn would never see its live reply. Mark child-thread
        // mirrors as live so the guard lets them through.
        sink(
          nextConn.liveMessage,
          nextConn.status === "prompting" || nextConn.isChildThread
        )
      }
  ...
  const registerLiveMessageSink = useCallback(
    (contextKey: string, sink: LiveMessageSink) => {
      ...
      if (conn?.liveMessage != null) {
        sink(
          conn.liveMessage,
          conn.status === "prompting" || conn.isChildThread
        )
      }
      return () => {
    ...
    (contextKey: string, e: EventEnvelope) => {
      switch (e.type) {
        case "status_changed":
          ...
          try {
            const conn = storeRef.current.connections.get(contextKey)
            const conversationId = resolveConversationIdForStatusUpdate(
              contextKey,
              [conn?.connectionId]
            )
            if (conversationId != null) {
              const mappedStatus =
                e.status === "prompting" ? "in_progress" : "completed"
              useAppWorkspaceStore
                .getState()
                .updateConversationLocal(conversationId, {
                  status: mappedStatus,
                })
              // Only persist status for real, locally-known conversation rows.
              // Child threads, draft ids, and app-server-only threads the local
              // DB never created have no row — persisting them rejects with
              // "{}" on every status flip (and status is bookkeeping anyway).
              if (
                !conn?.isChildThread &&
                isPersistedConversationRow(conversationId)
              ) {
                updateConversationStatus(conversationId, mappedStatus).catch(
                  (err) => {
                    console.error(
                      "[acp-connections] Failed to persist status:",
                      err
                    )
                  }
                )
            ...
            console.error("[acp-connections] Error updating status:", err)
          }
          break
        case "content_delta":
        ...
        case "background_activity": {
          ...
          if (e.settled && e.settled.length > 0) {
            ...
            const title = fn ? `${fn} - OrbiterX` : "OrbiterX"
        ...
        case "session_started":
          ...
          // Directly populate the conversationIdByExternalId mapping
          // so status_changed can resolve connectionId -> conversationId
          {
            const parsedContextKey = parseContextKey(contextKey)
            if (parsedContextKey?.conversationId) {
              useConversationRuntimeStore
                .getState()
                .actions.setExternalId(
                  parsedContextKey.conversationId,
                  e.session_id
                )
            }
          }
        ...
        case "turn_complete": {
          ...
          // Latch completion so a late-attached child viewer can tell a
          // finished run from a still-streaming one (whose prompting status
          // event was consumed before the attach).
          dispatch({ type: "TURN_COMPLETED", contextKey })
          // A sub-agent child TAB attached mid-run never observes the
          // prompting→connected edge (its turn/started was consumed before
          // the attach), so the panel's edge-triggered completeTurn never
          // fires and the accumulated liveMessage would stay "streaming"
          // forever. Promote it here, at the authoritative completion
          // boundary, using the sink-mirrored runtime liveMessage (kept
          // current by the connection dispatch). Delegation-child entries are
          // excluded: their promotion is owned by the sub-agent dialog bridge,
          // which passes the current liveMessage explicitly.
          const completedConn = storeRef.current.connections.get(contextKey)
          if (
            completedConn?.isChildThread &&
            !completedConn.isDelegationChild
          ) {
            try {
              const conversationId = resolveConversationIdForStatusUpdate(
                contextKey,
                [e.session_id, completedConn.connectionId]
              )
              if (conversationId != null) {
                useConversationRuntimeStore
                  .getState()
                  .actions.completeTurn(conversationId)
              }
            } catch (err) {
              console.error(
                "[acp-connections] Failed to promote child-thread turn:",
                err
              )
            }
          }
          // Clear the sidebar's running state on the completion signal itself.
          // The trailing `status_changed: connected` normally does this, but
          // the response can render fully (and the turn be over) while that
          // follow-up event is dropped or deduped between the transport and
          // this reducer — leaving the conversation row stuck on "in_progress".
          // `turn_complete` is the authoritative completion boundary, so sync
          // the conversation status here as well — but only for a clean
          // `end_turn` (cancelled/failed turns must not be marked completed).
          // `e.session_id` is the agent session id on the desktop path and the
          // thread id on the app-server path; the connection id is the fallback
          ...
          if (e.stop_reason === "end_turn") {
            try {
              const conversationId = resolveConversationIdForStatusUpdate(
                contextKey,
                [e.session_id, turnConn?.connectionId]
              )
              if (conversationId != null) {
                useAppWorkspaceStore
                  .getState()
                ...
                // Same as the status_changed path: only persist for real rows.
                if (
                  !turnConn?.isChildThread &&
                  isPersistedConversationRow(conversationId)
                ) {
                  updateConversationStatus(conversationId, "completed").catch(
                    (err) => {
                      console.error(
                        "[acp-connections] Failed to persist status:",
                        err
  ...
  const connect = useCallback(
    async (
      ...
      try {
        ...
        // If a newer connect request superseded ours while acpConnect was
        // in-flight, skip registering THIS connection — the `finally` block
        // will queue a microtask to re-invoke connect() with the newer
        // params, which will create the correct connection. Do NOT
        // acpDisconnect here: the connection we just created is the live
        // backend session the next connect() call will reuse via the
        // "existing connection" path (or the `finally` reconnect will
        // create a fresh one if params differ). Disconnecting it here
        // would cause a visible "connected → disconnected → connected"
        // flicker.
        ...
        // Always register the connection so it's tracked in the store.
        // Even when superseded, the store entry lets the `finally` block's
        // reconnect (or a later connect() call) find and properly tear
        // down this connection instead of leaking it on the server.
        lastActivityRef.current.set(contextKey, Date.now())
        dispatch({
          type: "CONNECTION_CREATED",
          contextKey,
          ...
          isChildThread: isChildThread ?? false,
        })
  ...
  const disconnect = useCallback(
    async (contextKey: string) => {
      // console.warn(`[ACP-TRACE] disconnect() called for ${contextKey}`)
      // console.warn(`[ACP-TRACE] disconnect stack:`, new Error().stack)
      pendingConnectRequestsRef.current.delete(contextKey)
      const conn = storeRef.current.connections.get(contextKey)
      if (!conn) {
        ...
        if (connectingKeysRef.current.has(contextKey)) {
          // console.warn(`[ACP-TRACE] disconnect() — connect() in flight, adding to abandonedKeysRef for ${contextKey}`)
  ...
  const sendPrompt = useCallback(
    async (
      ...
      const conn = storeRef.current.connections.get(contextKey)
      const connectionId = conn?.connectionId || contextKey
      lastActivityRef.current.set(contextKey, Date.now())
      await acpPrompt(
        connectionId,
        blocks,
