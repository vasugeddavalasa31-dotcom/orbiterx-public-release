...
export function AppWorkspaceProvider({ children }: AppWorkspaceProviderProps) {
  useEffect(() => {
    ...
    // `refreshConversations` reads `allFolders` synchronously, so it must
    // run AFTER folders are fetched — otherwise it iterates zero folders
    // and the sidebar shows folder headers with no conversations under them.
    void fetchFolders().then(() => refreshConversations())
  }, [])
