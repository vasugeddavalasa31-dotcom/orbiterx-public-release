...
import { AGENT_LABELS, type AgentType, type FileEditContent } from "@/lib/types"
import {
  expandHomePath,
  findOwningFolder,
  isHomeRelativePath,
...
type FileWorkspaceTabKind = "file" | "diff" | "rich-diff" | "subagent-session"
...
export interface FileWorkspaceTab {
  ...
  stale?: boolean
  // Sub-agent session tab payload (kind "subagent-session") — the live child
  // session rendered in the right-side tab strip, like a diff/file tab.
...
interface WorkspaceActionsValue {
  ...
  /**
   * Open a sub-agent's live session as a right-side workspace tab (the same
   * tab strip that hosts file/diff tabs) so the main conversation stays
   * visible while the sub-agent streams. One tab per child conversation.
   */
  openSubagentSession(args: {
...
export function WorkspaceProvider({ children }: WorkspaceProviderProps) {
  ...
  const resolveTargetFolder = useCallback(
    (explicitFolderId?: number): { id: number; path: string } | null => {
      ...
      // Fallback: resolve relative paths against the ACTIVE conversation tab's
      // folder. Sub-agent session / file-pane contexts may not have an active
      // folder ref, but the conversation tab always carries its workspace.
      const tabState = useTabStore.getState()
      const activeTab = tabState.rawTabs.find(
  ...
  const openFilePreview = useCallback(
    async (
      ...
      if (!absPath) {
        // Clear, actionable error instead of a raw "path.trim" TypeError from
        // joining an empty folder/path.
  ...
  // Open a sub-agent's live session in the right-side workspace tab strip
  // (same panel as file/diff tabs). One tab per child conversation; the tab
  // body (SubAgentSessionBody) fetches + bridges the live stream itself, so
  // the tab is created ready (no content fetch here).
  const openSubagentSession = useCallback(
    (args: {
      ...
      const tabId = `subagent-${args.childConversationId}`
      const title = args.agentType
        ? (AGENT_LABELS[args.agentType] ?? "Sub-agent")
        : "Sub-agent"
      setFileTabs((prev) => {
        ...
        return [
          ...prev,
          {
            ...
            folderId: null,
            title,
            description: args.kickoffTask ?? null,
            path: null,
            language: "markdown",
            content: "",
            loading: false,
            ...
            childConnectionId: args.childConnectionId,
            agentType: args.agentType,
            kickoffTask: args.kickoffTask ?? null,
          },
        ]
      })
      setActiveFileTabId(tabId)
