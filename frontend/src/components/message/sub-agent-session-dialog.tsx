...
import { useChildNickname } from "@/hooks/use-child-nickname"
import {
 ...
 *
 *  **Close / reopen keeps the session.** The runtime session is deliberately
 *  NOT dropped on close: the promoted live reply (`localTurns`) is the exact
 *  rendering the user saw, and rebuilding from the persisted projection on
 *  reopen produces a visibly different transcript (tool-card shapes, reasoning
 *  blocks, message boundaries). Keeping the session makes close+reopen show
 *  the identical view. Reopens always `refetchDetail({ preserveLive: true })`,
 *  so the persisted detail refreshes in the background without replacing the
 *  kept local reply.
 *
 *  **Close-mid-stream / reopen-after-complete.** If the user closes during the
 *  live window and the child later finishes while the body is unmounted, no
 *  bridge runs to promote the retained `liveMessage`. On reopen the child
 *  connection may already be detached — the mount-time promotion below settles
 *  any retained reply directly so it never renders as an eternally-streaming
 *  turn.
 *
...
function useChildLiveBridge(
  ...
  const { setLiveMessage, completeTurn, syncTurnMetadata } =
    useConversationRuntimeActions()
  ...
  useEffect(() => {
    if (liveMessage != null) {
      setLiveMessage(
        childConversationId,
        liveMessage,
        // Child-thread connections stream the delegated run even when the
        // attach missed the prompting status event (status stays "connected").
        // Mark the mirror live so the runtime store's SET_LIVE_MESSAGE guard
        // doesn't reject it as a reconnect replay once persisted detail (the
        // partial in-progress turn) exists.
        connStatus === "prompting" || childConnState?.isChildThread === true
      )
    }
    return () => {
      if (
        connStatusRef.current !== "prompting" &&
        childConnState?.isChildThread !== true
      ) {
  ...
  useEffect(() => {
    ...
    // Mid-run attach: a child connection that missed its `turn/started` stays
    // "connected" while its deltas stream (the child-thread guard exemption).
    // Adopting on the first delta would promote a PARTIAL reply as completed.
    // Only adopt once the connection has actually observed `turn_complete`
    // (the latch is set by the connections context at the authoritative
    // completion boundary).
    if (childConnState?.turnCompleted !== true) return
    adoptedRef.current = true
    setLiveMessage(childConversationId, liveMessage, true)
    completeTurn(childConversationId, liveMessage)
    startMetadataSync()
  }, [
    childConnState?.turnCompleted,
    connStatus,
    liveMessage,
    childConversationId,
    setLiveMessage,
  ...
  // Teardown on close/collapse: cancel any in-flight metadata sync. The runtime
  // session is intentionally KEPT so a reopen renders the identical promoted
  // reply instead of a differently-shaped persisted reconstruction.
  useEffect(() => {
    return () => {
      syncCancelRef.current?.()
  ...
  // Reopen-after-completion: when the body was closed mid-stream and the child
  // finished (and its connection detached) while unmounted, the retained
  // runtime `liveMessage` was never promoted. Promote it on mount so it renders
  // as a settled completed turn, not an eternally-streaming one.
  const retainedSession = useConversationRuntimeStore(
    (s) => s.byConversationId.get(childConversationId) ?? null
  )
  const retainedReplyPromotedRef = useRef(false)
  useEffect(() => {
    if (retainedReplyPromotedRef.current) return
    // A live connection present means the normal bridge paths own the reply.
    if (childConnState !== undefined) return
    if (!retainedSession || retainedSession.liveMessage === null) return
    if (retainedSession.localTurns.length > 0) return
    retainedReplyPromotedRef.current = true
    completeTurn(childConversationId, retainedSession.liveMessage)
    startMetadataSync()
  }, [
    childConversationId,
    childConnState,
...
/**
 * Inline embed of a delegated sub-agent session, expanded directly inside the
 * parent conversation (mirrors the opencode pattern of viewing the sub-agent
 * session in place instead of a modal/tab). Renders the same body as the
 * modal — live stream bridge, permission/ask/plan cards, message list — with
 * a bounded height so the nested list scrolls internally.
 */
export function InlineSubAgentSession({
  childConversationId,
  childConnectionId,
  agentType,
  kickoffTask,
  ...
  return (
    <div className="h-[30rem] min-h-0 overflow-hidden border-t border-border">
      <SubAgentSessionBody
        childConversationId={childConversationId}
        childConnectionId={childConnectionId}
        agentType={agentType}
...
export function SubAgentSessionBody({
  childConversationId,
  childConnectionId,
  agentType,
  kickoffTask,
  ...
  // The sub-agent's friendly name (opencode-style, e.g. "Galileo") — falls back
  // to the agent-type label when the child id is unknown.
  const childNickname = useChildNickname(childConnectionId ?? "")
  const displayName = childConnectionId ? childNickname : null
