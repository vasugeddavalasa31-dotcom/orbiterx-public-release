...
describe("SubAgentSessionDialog", () => {
  ...
  it("bridges conn.liveMessage to setLiveMessage while open and clears the runtime session on close", () => {
    ...
    // Closing the dialog (body unmount) KEEPS the runtime session: the
    // promoted live reply is the exact rendering the user saw, and rebuilding
    // from the persisted projection on reopen renders visibly differently.
    // Reopens refetchDetail({ preserveLive: true }) to refresh persisted state
    // without replacing the kept local reply.
    unmount()
    expect(mockRemoveConversation).not.toHaveBeenCalled()
  })
  ...
  it("does not refetch on the streaming → settled edge — the promoted local reply is kept, never replaced from the lagging DB", () => {
    ...
    mockChildConnection = makeConnState({
      status: "connected",
      liveMessage,
      // The child's turn completed before the dialog reopened — the
      // completion latch distinguishes this settled mount from a mid-run
      // attach whose prompting event was missed.
      turnCompleted: true,
    })
    act(() => {
      notifyStore()
  ...
  it("adopts a retained final reply when reopened onto an already-settled child (reopen-after-completion DB lag)", () => {
    ...
    mockChildConnection = makeConnState({
      status: "connected",
      liveMessage,
      // The turn completed before the dialog reopened — the completion latch
      // distinguishes this settled mount from a mid-run attach.
      turnCompleted: true,
    })
    renderWithIntl(
      <SubAgentSessionDialog
  ...
  it("does not adopt-promote mid-stream for a late-attached child whose prompting status was missed", () => {
    const liveMessage = {
      id: "live-1",
      role: "assistant" as const,
      content: [{ type: "text" as const, text: "partial stream" }],
      startedAt: Date.now(),
    }
    // Late attach: the connection exists (isChildThread) but never observed
    // "prompting" (turn/started was consumed before the attach), so its
    // deltas stream under status "connected". turnCompleted is still false —
    // the run is live, and adopting the partial reply as completed would
    // prematurely settle it.
    mockChildConnection = makeConnState({
      status: "connected",
      liveMessage,
      turnCompleted: false,
  ...
  it("promotes a retained live reply on reopen when the child connection already detached (mid-stream close + completion)", () => {
    const liveMessage = {
      id: "live-1",
      role: "assistant" as const,
      content: [{ type: "text" as const, text: "final retained reply" }],
      startedAt: Date.now(),
    }
    // The body was closed mid-stream; the child finished while unmounted and
    // its connection detached. The kept runtime session still holds the final
    // liveMessage, which no bridge promoted. Reopen must settle it instead of
    // rendering an eternally-streaming turn.
    mockChildConnection = undefined
  ...
  it("forces a fresh refetchDetail on every settled open so an in-flight fetch from a previous (now-closed) dialog can't surface stale state", () => {
    ...
    // resolved. The runtime session is kept (see the bridge teardown), and the
    // in-flight fetch is not cancelled — so the second open's refetchDetail
    // must still fire to refresh the persisted state.
    unmount()
    expect(mockRemoveConversation).not.toHaveBeenCalled()
