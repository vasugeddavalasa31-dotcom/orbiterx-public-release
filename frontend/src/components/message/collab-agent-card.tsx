...
export function CollabAgentCard({ input, errorText, state }: Props) {
  ...
  // First sub-agent's nickname (e.g. "Galileo") — the capsule title shows the
  // friendly name instead of the raw thread id. Kicks a lazy thread/read and
  // re-renders when it resolves.
  const firstThreadId = agents[0]?.threadId ?? null
  const firstNickname = useChildNickname(firstThreadId ?? "")
  const displayName = firstNickname ?? null

  // Click the capsule → the sub-agent's live session opens instantly in the
  // right-side tab strip (same panel as file/diff tabs). No intermediate
  // "Open session" button / inline expand step.
  const openSession = useCallback(
    (threadId: string) => {
      openSubagentSession({
        childConversationId: convIdFromThreadId(threadId),
        childConnectionId: threadId,
        agentType: "codex",
      })
    },
    [openSubagentSession]
  ...
  // Title is the sub-agent's identity: "Sub-agent (Leibniz)" with the
  // backend-minted name. The task text stays in the body (visible on expand)
  // instead of being stuffed into the title. Falls back to the task's first
  // line (while the name is still resolving / unknown), then an op-aware label.
  const title = useMemo(() => {
    if (displayName) {
      return `${t("title")} (${displayName})`
    }
  ...
  return (
    <AgentCapsule
      ...
      statusLabel={title}
      onTriggerClick={
        firstThreadId ? () => openSession(firstThreadId) : undefined
      }
    >
