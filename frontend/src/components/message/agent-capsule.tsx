...
interface AgentCapsuleProps {
  ...
  defaultOpen?: boolean
  /**
   * When set, the capsule renders as a single clickable pill (no expand /
   * collapse, no body) that invokes this handler — used by collab cards that
   * open the sub-agent's live session as a right-side tab on click.
   */
  onTriggerClick?: () => void
  children: ReactNode
}
...
export function AgentCapsule({
  ...
  // Click-to-open mode: a single compact pill that opens the sub-agent session
  // (e.g. as a right-side tab) — no expand/collapse body.
  if (onTriggerClick) {
    return (
      <button
        type="button"
