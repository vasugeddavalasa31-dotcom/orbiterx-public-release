...
// InlineSubAgentSession pulls in MessageListView + useConversationRuntime,
// which would require the full runtime provider tree. Stub it to a sentinel
// exposing the child id so we can assert the "Open conversation" button
// expands it with the right target. The session body's own behavior (live
// bridge, permission rendering) is covered by its dedicated test file.
vi.mock("@/components/message/sub-agent-session-dialog", () => ({
  SubAgentSessionDialog: ({
    ...
    open ? (
      <div
        data-testid="inline-sub-agent-session"
        data-conversation-id={childConversationId}
      />
    ) : null,
  InlineSubAgentSession: ({
    childConversationId,
  }: {
    childConversationId: number
  }) => (
...
describe("DelegatedSubThread", () => {
  ...
  it("renders from broker meta alone (persisted Cursor shape after refresh)", () => {
    ...
    expect(screen.getByText("执行 pnpm build")).toBeInTheDocument()
    fireEvent.click(screen.getByTestId("delegated-sub-thread"))
    expect(workspaceMocks.openSubagentSession).toHaveBeenCalledWith(
      expect.objectContaining({ childConversationId: 321 })
    )
  })
  ...
  it("opens the sub-agent session in the right-side tab when the card is clicked", () => {
    mockedHook.mockReturnValue({
      binding: bindingOf({ status: "running" }),
      detail: null,
      loading: false,
    ...
    renderWithIntl(<DelegatedSubThread parentToolUseId="pt-1" />)
    fireEvent.click(screen.getByTestId("delegated-sub-thread"))
    // The card click opens the child's live session as a right-side tab.
    expect(workspaceMocks.openSubagentSession).toHaveBeenCalledWith(
      expect.objectContaining({ childConversationId: 99 })
    )
  })

  it("does not open anything when the child id is unknown", () => {
    const input = JSON.stringify({
      agent_type: "codex",
    ...
    renderWithIntl(<DelegatedSubThread parentToolUseId="pt-1" input={input} />)
    fireEvent.click(screen.getByTestId("delegated-sub-thread"))
    expect(workspaceMocks.openSubagentSession).not.toHaveBeenCalled()
  })
  ...
  it("recovers the open-conversation link from the broker output (synthetic fallback)", () => {
    ...
    fireEvent.click(screen.getByTestId("delegated-sub-thread"))
    expect(workspaceMocks.openSubagentSession).toHaveBeenCalledWith(
      expect.objectContaining({ childConversationId: 1234 })
    )
  })
  ...
  it("recovers the open-conversation link from a wrapped MCP CallToolResult output", () => {
    ...
    fireEvent.click(screen.getByTestId("delegated-sub-thread"))
    expect(workspaceMocks.openSubagentSession).toHaveBeenCalledWith(
      expect.objectContaining({ childConversationId: 4321 })
    )
  })
  ...
  it("badges 'awaiting approval' when the child has a pending permission (answered in the session tab)", () => {
    mockedHook.mockReturnValue({
      binding: bindingOf({ status: "running" }),
      detail: null,
      loading: false,
    ...
    expect(screen.queryByText("running")).not.toBeInTheDocument()
    // No permission UI on the card — it lives in the opened session tab.
...
describe("DelegatedSubThread (async ack semantics)", () => {
  ...
  it("recognizes a running ack inlined in content[0].text with no structuredContent", () => {
    ...
    fireEvent.click(screen.getByTestId("delegated-sub-thread"))
    expect(workspaceMocks.openSubagentSession).toHaveBeenCalledWith(
      expect.objectContaining({ childConversationId: 4242 })
    )
  })
  ...
  it("recognizes a Codex-wrapped running ack in content[0].text (Wall time prefix, no structuredContent)", () => {
    ...
    fireEvent.click(screen.getByTestId("delegated-sub-thread"))
    expect(workspaceMocks.openSubagentSession).toHaveBeenCalledWith(
      expect.objectContaining({ childConversationId: 909 })
    )
  })

  it("expands the inline sub-agent session when clicking anywhere on the card container", () => {
    const report = JSON.stringify({
      status: "running",
      child_conversation_id: 910,
    })
    const output = JSON.stringify({
      content: [{ type: "text", text: report }],
    })
    const input = JSON.stringify({ agent_type: "codex", task: "click test" })
    renderWithIntl(
      <DelegatedSubThread
        parentToolUseId="delegation-click"
        input={input}
        output={output}
        state="output-available"
      />
    ...
    expect(workspaceMocks.openSubagentSession).toHaveBeenCalledWith(
      expect.objectContaining({ childConversationId: 910 })
    )
  })
})
