...
export function GitChangesTab() {
  ...
  // Aggregate +N/−M across the tracked working-tree changes for the section
  // header. Untracked files carry no numstat counts (git status only), so the
  // totals span the tracked set only.
              ...
              <section className="space-y-1">
                <div className="flex items-center justify-between px-2 py-1 text-[11px] text-muted-foreground">
                  <span className="flex items-center gap-1.5">
                    ...
                    {(trackedTotals.additions > 0 ||
                      trackedTotals.deletions > 0) && (
                      <span className="flex items-center gap-1">
                        <CommitFileAdditions count={trackedTotals.additions} />
                        <CommitFileDeletions count={trackedTotals.deletions} />
                      </span>
                    )}
                  </span>
                  <Button
