...
export function SkillsSettings() {
  ...
  // App-server stack: skills come from the embedded/system registry and the
  // backend has no skill-write RPC, so when every visible skill is read-only
  // the store is effectively read-only — hide the create/edit affordances and
  // show a banner instead of letting save fail with a confusing error.
  const isReadOnlyStore =
    skillItems.length > 0 && skillItems.every((skill) => skill.read_only)
              ...
              <div className="min-h-0 h-full min-w-0 rounded-lg border bg-card flex flex-col overflow-hidden lg:rounded-r-none">
                ...
                <div className="border-t p-2 flex items-center gap-2">
                  ...
                  <Button
                    ...
                    title={isReadOnlyStore ? t("readOnlyStoreHint") : undefined}
                  >
