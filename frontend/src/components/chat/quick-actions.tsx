...
export function QuickActions({ onSelect, agentType }: QuickActionsProps) {
  ...
  const handleTabChange = useCallback((value: string) => {
    // Office Work / Scientific Research are locked (coming soon) — never
    // switch to them; the trigger's tooltip explains why.
    if (value === "office" || value === "research") return
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <TabsTrigger
                value="office"
                disabled
                className="cursor-not-allowed opacity-50 text-muted-foreground data-[state=active]:bg-background data-[state=active]:text-foreground data-[state=active]:shadow-sm data-[state=active]:ring-1 data-[state=active]:ring-border/50"
              >
