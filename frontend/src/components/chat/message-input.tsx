...
interface MessageInputProps {
  ...
  onAddFeedback?: () => void
  /** Start a code review of the current changes (from the "+" menu). When
   *  omitted the entry is hidden. */
  onReview?: () => void
  /** Compact the session context (from the "+" menu → thread/compact/start).
...
export function MessageInput({
  ...
  const handleConfigOptionSelect = useCallback(
    (configId: string, valueId: string) => {
      setLocalConfigOverrides((prev) => ({ ...prev, [configId]: valueId }))
  ...
  const availableConfigOptions = useMemo(() => {
      ...
      configOptions && configOptions.length > 0
        ...
        : [
            ...
            },
            {
              ...
              kind: {
                type: "select" as const,
                // No hardcoded fallback catalog — the picker stays empty until
                // the backend's `model/list` / `session_config_options` arrives.
                current_value: "",
                options: [],
                groups: [],
              },
    ...
    return rawOpts.map((option) => {
      if (option.kind.type !== "select") return option
      const override = localConfigOverrides[option.id]
      if (override && override !== option.kind.current_value) {
        return {
          ...option,
          kind: {
            ...option.kind,
  ...
  }, [configOptions, localConfigOverrides])
  const hasConfigOptions = availableConfigOptions.length > 0
  const hasModes = availableModes.length > 0

  // The composer bar keeps the frequently-toggled selectors (permission mode,
  // model, reasoning effort). Personality and any other config option live in
  // the "+" overflow menu so the input stays clean.
  const INLINE_CONFIG_OPTION_IDS = new Set([
    "permission_mode",
    "model",
    "effort",
  ])
  const inlineConfigOptions = availableConfigOptions.filter((option) =>
    INLINE_CONFIG_OPTION_IDS.has(option.id)
  )
  const overflowConfigOptions = availableConfigOptions.filter(
    (option) => !INLINE_CONFIG_OPTION_IDS.has(option.id)
  )
  ...
  // The mode selector lives in the "+" overflow menu (not the inline bar).
  // `showModeSelector` still gates the collapsed cog panel's mode entry and the
  // mode carried on send; the inline bar only hosts permission/model/effort.
  ...
  const hasInlineSelectors = inlineConfigOptions.length > 0
  ...
  const inlineSelectorItems = (
    <>
      {inlineConfigOptions.length > 0 &&
        inlineConfigOptions.map((option) => {
            ...
            return (
              <ModelOptionPicker
                ...
                onSelect={(configId, valueId) =>
                  handleConfigOptionSelect(configId, valueId)
                }
              />
            )
          }
    ...
    if (hasConfigOptions) {
      for (const option of availableConfigOptions) {
        ...
        result.push({
          ...
          groups,
          onSelect: (value) => handleConfigOptionSelect(option.id, value),
          ...(option.id === "permission_mode" && {
            toneByValue: {
              never: { dot: "bg-rose-500", text: "text-rose-500" },
              full_auto: { dot: "bg-rose-500", text: "text-rose-500" },
              untrusted: { dot: "bg-amber-500", text: "text-amber-500" },
              default: { dot: "bg-amber-500", text: "text-amber-500" },
              "on-request": { dot: "bg-sky-500", text: "text-sky-500" },
              auto: { dot: "bg-sky-500", text: "text-sky-500" },
              granular: { dot: "bg-violet-500", text: "text-violet-500" },
              read_only: { dot: "bg-emerald-500", text: "text-emerald-500" },
            },
          }),
          ...(searchable && {
            search: {
  ...
  const actionButtons = isEditingQueueItem ? (
    <div className="flex items-center gap-1">
      ...
      <Button
        ...
        size="icon"
        className="group flex h-8 w-8 items-center justify-center rounded-full bg-gradient-to-br from-indigo-500 via-violet-500 to-fuchsia-500 text-white shadow-md shadow-violet-500/25 transition-all hover:scale-105 hover:shadow-violet-500/40 active:scale-95 disabled:pointer-events-none disabled:opacity-40"
    ...
    <Button
      onClick={onCancel}
      size="icon"
      className="group flex h-8 w-8 items-center justify-center rounded-full border border-orange-400/40 bg-gradient-to-br from-amber-400/90 to-orange-500/90 text-white shadow-md shadow-orange-500/25 transition-all hover:scale-105 hover:shadow-orange-500/40 active:scale-95 disabled:pointer-events-none disabled:opacity-40"
      title={t("cancel")}
    >
    ...
    <div className="flex items-center">
      <Button
        ...
        size="icon"
        className={cn(
          "group flex h-7 w-7 items-center justify-center rounded-full border transition-all active:scale-95 disabled:pointer-events-none",
      ...
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <Button
            ...
            size="icon"
            className={cn(
              "-ml-1 flex h-7 w-4 items-center justify-center rounded-full border pl-0 transition-all active:scale-95 disabled:pointer-events-none",
              hasSendableContent && !disabled
                ? "border-primary bg-primary text-primary-foreground hover:bg-primary/90"
                : "border-border/70 bg-transparent text-muted-foreground/70"
            )}
                ...
                <div className="flex min-w-0 items-end gap-1">
                  <DropdownMenu onOpenChange={handleAddMenuOpenChange}>
                    ...
                    >
                      ...
                      {overflowConfigOptions.length > 0 && (
                        <DropdownMenuSub>
                          <DropdownMenuSubTrigger>
                            <Smile className="size-4" />
                            {t("personality")}
                          </DropdownMenuSubTrigger>
                          <DropdownMenuSubContent className="min-w-56 p-1">
                            {overflowConfigOptions.map((option) => {
                              if (option.kind.type !== "select") return null
                              return (
                                <DropdownMenuRadioGroup
                                  key={option.id}
                                  ...
                                  onValueChange={(value) =>
                                    handleConfigOptionSelect(option.id, value)
                                  }
                                >
                  ...
                  {hasAnySelector && !hasInlineSelectors && (
                    <div className="flex">
                      <Popover
