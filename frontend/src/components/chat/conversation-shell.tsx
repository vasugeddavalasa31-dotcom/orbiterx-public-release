...
interface ConversationShellProps {
  ...
  /** Start a code review of the current changes (composer "+" menu). */
  onReview?: () => void
  /** Compact the session context (composer "+" menu → thread/compact/start). */
  onCompact?: () => void
  isActive?: boolean
...
export function ConversationShell({
  ...
  return (
    <div className="relative flex h-full min-h-0 flex-col">
      ...
      {/* Permission bar — compact card docked above the composer, width-matched
          to the input like the question/plan cards below. */}
      {pendingPermission && (
