...
export interface SessionSelectorSetting {
  ...
  /** When set, the setting is the permission-mode picker: option rows and the
   *  current-value label get a colored dot + label tone per risk level. */
  toneByValue?: Record<string, { dot: string; text: string }> | null
}
        ...
        {settings.map((setting) => {
          ...
          return (
            ...
            >
              ...
              >
                {setting.toneByValue?.[setting.currentValue] && (
                  <span
                    className={cn(
                      "size-1.5 shrink-0 rounded-full",
                      setting.toneByValue[setting.currentValue].dot
                    )}
                    aria-hidden
                  />
                )}
                <span
                  className={cn(
                    "truncate",
                    setting.toneByValue?.[setting.currentValue]?.text
                  )}
                >
          ...
          {active.groups.map((group, groupIndex) => (
            <div key={group.key} className="flex flex-col gap-0.5">
              ...
              {group.options.map((opt) => {
                ...
                const tone = active.toneByValue?.[opt.value] ?? null
                return (
                  <button
