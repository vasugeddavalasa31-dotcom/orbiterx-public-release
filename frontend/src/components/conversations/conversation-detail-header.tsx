...
interface ConversationDetailHeaderProps {
  ...
  /** When this tab is a SUB-AGENT session, the main (parent) conversation id to
   *  jump back to — renders a "Back to main session" affordance in the header.
   *  Null for a normal conversation tab. */
...
export const ConversationDetailHeader = memo(function ConversationDetailHeader({
  ...
  // Jump back to the MAIN session tab when this is a sub-agent session.
  const handleBackToMain = useCallback(() => {
    if (parentConversationId == null) return
    openTab(folderId, parentConversationId, "codex", true)
  }, [folderId, parentConversationId, openTab])
      ...
      <div className="flex min-w-0 flex-1 items-center gap-1.5">
        {parentConversationId != null && (
          <button
            ...
            title={t("backToMainSession")}
            className="flex h-7 shrink-0 items-center gap-1 rounded px-1.5 text-xs font-medium text-muted-foreground transition-colors hover:bg-muted/60 hover:text-foreground"
          >
