...
export function SidebarConversationList({
  ...
  const loadMoreConversationsForFolder = useAppWorkspaceStore(
    (s) => s.loadMoreConversationsForFolder
  )
  const folderHasMore = useAppWorkspaceStore(
    (s) => s.folderConversationsHasMore
  )
    ...
    if (row.kind === "load-more") {
      return themeWrap(
        ...
        <div className="flex justify-center py-2">
          <button
            onClick={() => loadMoreConversationsForFolder(row.folderId)}
  ...
  const rowKey = (row: SidebarRow): string => {
    ...
    if (row.kind === "load-more") return `load-more-${row.folderId}`
