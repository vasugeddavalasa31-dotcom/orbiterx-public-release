...
const EMPTY_CONTAINER_CHILDREN: ReadonlyMap<number, readonly number[]> =
  new Map()
// No folder has more conversations to load by default (pagination off).
const EMPTY_FOLDER_HAS_MORE: ReadonlyMap<number, boolean> = new Map()
...
export function buildRows(args: {
  ...
  /** Per-folder "has more" map for pagination. When true, a "Load more" row is
   *  added after the last conversation in the folder. Optional. */
  folderHasMore?: ReadonlyMap<number, boolean>
}): SidebarRow[] {
  const {
    pinned,
    pinnedExpanded,
    ...
    folderHasMore = EMPTY_FOLDER_HAS_MORE,
  } = args
  ...
  const pushFolderBody = (folderId: number, baseDepth: number) => {
    ...
    // Add "Load more" row if there are more conversations to load for this folder
    if (folderHasMore.get(folderId)) {
      rows.push({
        kind: "load-more",
        folderId,
      })
