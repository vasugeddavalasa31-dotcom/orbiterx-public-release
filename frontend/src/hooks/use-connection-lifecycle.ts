...
interface UseConnectionLifecycleOptions {
  ...
  /**
   * True when the tab backs a sub-agent child thread (opened via a collab
   * card). Forwarded to `connect()` so the connection is marked as a child
   * thread and its live stream is exempt from the out-of-turn guard.
   */
...
function normalizeErrorMessage(error: unknown): string {
  ...
  // Non-Error rejections (a bare `{}`, `undefined`, a rejected value) have no
  // `.message`; render their shape so the console shows something actionable
...
export function useConnectionLifecycle({
  ...
  // Guard against re-firing the auto-connect effect while a connect is
  // in-flight. Without this, a dependency change during `await acpConnect()`
  // causes a second `connect()` call that queues a `pendingRequest`, which
  // the first call's post-connect guard sees and immediately disconnects
  ...
  useEffect(() => {
    if (!isActive) return
    // Resuming an existing thread (sessionId set — e.g. a sub-agent child tab
    // carrying its thread UUID) doesn't need a workingDir: the transport's
    // resume branch ignores cwd entirely. Only a NEW session requires the
    // folder's path to spawn the agent. Without this, child tabs opened via
    // the collab card (which never set a workingDir) never auto-connect, so
    // the child's live stream never reaches the tab — it shows only the
    // fetched (often empty) detail: "No messages in this conversation."
    if (!workingDir && !sessionId) return
    // If a previous auto-connect is still in-flight (awaiting acpConnect),
    // skip this run to avoid queuing a superseding pendingRequest that
    // would cause the in-flight connect to disconnect its own connection.
    ...
    connConnectRef
      .current(agentType, workingDir, sessionId, conversationId, isChildThread)
      .then(() => {
        if (!cancelled) {
  ...
  const handleFocus = useCallback(() => {
    ...
    if (!status || status === "disconnected" || status === "error") {
      ...
      connConnect(
        ...
        if (!isExpectedConnectError(e)) {
          console.error("[ConnLifecycle] connect:", normalizeErrorMessage(e), e)
        }
      })
    }
  }, [
