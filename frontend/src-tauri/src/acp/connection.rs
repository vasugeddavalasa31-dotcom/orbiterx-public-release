...
fn synthesize_codex_config_options() -> Vec<SessionConfigOptionInfo> {
    vec![SessionConfigOptionInfo {
        id: "model".to_string(),
        name: "Model".to_string(),
        description: Some("OrbiterX Model Selection".to_string()),
        category: Some("model".to_string()),
        kind: SessionConfigKindInfo::Select(SessionConfigSelectInfo {
            current_value: "gpt-5.6-sol".to_string(),
            options: vec![
                SessionConfigSelectOptionInfo {
                ...
                SessionConfigSelectOptionInfo {
                    value: "orbiterx-auto-review".to_string(),
                    name: "OrbiterX Auto Review".to_string(),
                    description: None,
                },
            ],
            groups: vec![],
...
async fn apply_and_emit_session_config_options(
    ...
    if agent_type == AgentType::Codex && initial_config_options.is_empty() {
        let opts = synthesize_codex_config_options();
        emit_session_config_options_info(state, emitter, opts).await;
        return;
    }
