...
/// Read OpenCode's external config file tolerantly. The file is written by the
/// OpenCode CLI (not this app) and is often JSONC-ish — trailing commas,
/// comments — which strict `serde_json` rejects. A missing or unparseable file
/// simply means "no local OpenCode MCP servers"; it must never fail the whole
/// MCP settings page for a user who doesn't even use MCP.
fn read_opencode_config() -> Result<Value, AppCommandError> {
    ...
    match serde_json::from_str::<Value>(&raw) {
        ...
        Err(err) => {
            tracing::warn!(
                "[MCP] {} is not valid JSON ({err}); treating as no local OpenCode MCP servers",
                path.display()
            );
            Ok(json!({}))
        }
