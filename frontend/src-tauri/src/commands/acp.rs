...
pub(crate) fn resolve_system_agent_binary(cmd: &str) -> Option<PathBuf> {
    if cmd == "orbiterx" {
        let debug_bin =
            PathBuf::from("/Volumes/vame/rustorbiterx/orbiterx-rs/target/debug/orbiterx");
        if debug_bin.is_file() {
            return Some(debug_bin);
        }
        let release_bin =
            PathBuf::from("/Volumes/vame/rustorbiterx/orbiterx-rs/target/release/orbiterx");
...
// ─── OrbiterX Login (local callback OAuth flow) ───
// The issuer does not expose device-auth endpoints, so the desktop app uses
// the same browser authorization-code + PKCE flow as the Rust CLI
// (orbiterx-rs/login): a short-lived localhost callback server captures the
// authorization code, the sidecar exchanges it for tokens, and the frontend
// polls for the result. Ports 1455/1457 are the registered localhost redirect
// URIs for the CLI client.

const CODEX_OAUTH_ISSUER: &str = "https://auth.orbiterxai.online";
const CODEX_OAUTH_CLIENT_ID: &str = "app_EMoamEEZ73f0CkXaXp7hrann";
const CODEX_OAUTH_CALLBACK_PORT: u16 = 1455;
const CODEX_OAUTH_CALLBACK_FALLBACK_PORT: u16 = 1457;

/// State for one active browser sign-in, keyed by `device_auth_id` (the port).
struct OauthCallbackSlot {
    state: String,
    redirect_uri: String,
    code_verifier: String,
    error: tokio::sync::Mutex<Option<String>>,
    /// Final result once the callback has exchanged the code (immediately on
    /// browser redirect). Exchanging here — not on the next frontend poll —
    /// consumes the single-use authorization code right away, so it can never
    /// be double-redeemed or expire (OpenAuth codes have a 60s TTL) before a
    /// slow poll picks it up.
    result: tokio::sync::Mutex<Option<CodexDeviceCodePollResult>>,
    /// Signals the local callback server to stop once sign-in resolves.
    shutdown: Arc<tokio::sync::Notify>,
}

/// Registry of active browser sign-ins keyed by the local callback port. A new
/// login replaces the previous attempt; completed attempts are removed.
static OAUTH_CALLBACKS: std::sync::OnceLock<
    std::sync::Mutex<HashMap<String, Arc<OauthCallbackSlot>>>,
> = std::sync::OnceLock::new();

fn oauth_callbacks() -> &'static std::sync::Mutex<HashMap<String, Arc<OauthCallbackSlot>>> {
    OAUTH_CALLBACKS.get_or_init(|| std::sync::Mutex::new(HashMap::new()))
}
...
pub struct CodexDeviceCodeResponse {
    ...
    pub interval: u64,
    /// The exact redirect URI the auth server recorded for this session. Kept
    /// for response-shape compatibility; the Rust side performs the exchange.
...
pub struct CodexDeviceCodePollResult {
    ...
    /// API-key-style token obtained via OAuth token-exchange from the id token.
    /// Optional: the frontend falls back to the access token's `properties`
    /// claim when the exchange is unavailable.
    pub api_key: Option<String>,
}
...
/// Extracts the ChatGPT account id from an id token. Mirrors the login crate:
/// the claim can be top-level (`chatgpt_account_id`) or nested under the
...
pub(crate) async fn codex_request_device_code_core() -> Result<CodexDeviceCodeResponse, AcpError> {
    ...
    // Prefer the registered CLI callback port; fall back to its twin when a
    // previous login attempt left it bound.
    let listener = match tokio::net::TcpListener::bind(("127.0.0.1", CODEX_OAUTH_CALLBACK_PORT))
        .await
    {
        Ok(listener) => listener,
        Err(_) => tokio::net::TcpListener::bind(("127.0.0.1", CODEX_OAUTH_CALLBACK_FALLBACK_PORT))
            .await
            .map_err(|e| AcpError::protocol(format!("bind login callback server failed: {e}")))?,
    };
    let port = listener
        .local_addr()
        .map_err(|e| AcpError::protocol(format!("read callback port failed: {e}")))?
        .port();
    let device_auth_id = port.to_string();
    let redirect_uri = format!("http://localhost:{port}/auth/callback");

    let slot = Arc::new(OauthCallbackSlot {
        state: state.clone(),
    ...
    async fn oauth_callback(
        ...
        if let Some(error_code) = params.get("error") {
            ...
            return (
                ...
                axum::response::Html(
                    "<html><body><h2>Sign-in failed</h2><p>You can close this tab and return to OrbiterX.</p></body></html>",
                ),
            );
        }
        let Some(code) = params.get("code").cloned() else {
            return (
                ...
                axum::response::Html(
                    "<html><body><h2>Sign-in incomplete</h2><p>No authorization code was returned. Please try again.</p></body></html>",
                ),
            );
        };
        // Exchange the single-use code immediately so it is consumed exactly
        // once, right here, instead of waiting for a frontend poll (which could
    ...
    oauth_callbacks()
        .lock()
        .unwrap()
        .insert(device_auth_id.clone(), slot);

    // Authorize URL mirrors the Rust login crate's `build_authorize_url`.
    let params = vec![
        ...
        (
            "orbiterx_cli_simplified_flow".to_string(),
            "true".to_string(),
        ),
        ("state".to_string(), state),
        ("originator".to_string(), "cli".to_string()),
    ];
    let query = params
        .iter()
        ...
        .collect::<Vec<_>>()
        .join("&");
    let auth_url = format!("{CODEX_OAUTH_ISSUER}/oauth/authorize?{query}");

    Ok(CodexDeviceCodeResponse {
        user_code: "ORBITERX".into(),
...
fn pending_poll_result() -> CodexDeviceCodePollResult {
    CodexDeviceCodePollResult {
        status: "pending".into(),
        message: None,
        id_token: None,
        access_token: None,
...
fn error_poll_result(message: impl Into<String>) -> CodexDeviceCodePollResult {
    CodexDeviceCodePollResult {
        status: "error".into(),
...
pub(crate) async fn codex_poll_device_code_core(
    ...
    let Some(slot) = oauth_callbacks()
        .lock()
        .unwrap()
        .get(&device_auth_id)
        .cloned()
    else {
        return Ok(error_poll_result(
            "No active login session. Please try again.",
        ));
    };

    // The browser may have redirected back with an OAuth error.
    if let Some(message) = slot.error.lock().await.take() {
        oauth_callbacks().lock().unwrap().remove(&device_auth_id);
        return Ok(error_poll_result(message));
    }

    // The callback handler exchanges the single-use authorization code
    // immediately on browser redirect and stores the finished result here.
    // The poll only reads it — it never touches the code, so the exchange
    // can't be double-run or lost to the 60s code TTL.
    if let Some(result) = slot.result.lock().await.take() {
        oauth_callbacks().lock().unwrap().remove(&device_auth_id);
        return Ok(result);
    }

    // No callback yet — the frontend polls again.
    Ok(pending_poll_result())
}

/// Exchange the single-use authorization code for tokens (called by the local
/// callback handler the moment the browser redirects back). This is the only
/// place the code is redeemed, so it can never be double-used or expire.
async fn exchange_authorization_code(
    slot: &Arc<OauthCallbackSlot>,
    code: &str,
) -> CodexDeviceCodePollResult {
    ...
    let token_resp = match client
        ...
        .body(format!(
            ...
            urlencoding::encode(&slot.redirect_uri),
            urlencoding::encode(CODEX_OAUTH_CLIENT_ID),
            urlencoding::encode(&slot.code_verifier),
        ))
        .send()
        .await
    {
        Ok(resp) => resp,
        Err(e) => return error_poll_result(format!("token exchange failed: {e}")),
    };

    if !token_resp.status().is_success() {
        let status = token_resp.status();
        let text = token_resp.text().await.unwrap_or_default();
        return error_poll_result(format!("token exchange returned {status}: {text}"));
    }

    let tokens: OAuthTokenResp = match token_resp.json().await {
        Ok(tokens) => tokens,
        Err(e) => return error_poll_result(format!("parse token response failed: {e}")),
    };
    let id_token = tokens
        .id_token
        .unwrap_or_else(|| tokens.access_token.clone());
    let account_id = extract_jwt_account_id(&id_token);

    // Optionally obtain the API-key-style token via OAuth token-exchange
    // (same grant the Rust login crate uses for the CLI). Failure is not fatal:
    // the frontend falls back to the access token's `properties` claim.
    let api_key = exchange_id_token_for_api_key(&client, &id_token).await.ok();
...
/// Exchanges an authenticated ID token for an API-key style access token via
/// the OAuth token-exchange grant (mirrors `obtain_api_key` in orbiterx-rs).
async fn exchange_id_token_for_api_key(
    ...
    let resp = client
        ...
        .send()
        .await
        .map_err(|e| AcpError::protocol(format!("api key exchange failed: {e}")))?;
    ...
    let body: ApiKeyExchangeResp = resp
        .json()
        .await
        .map_err(|e| AcpError::protocol(format!("parse api key response failed: {e}")))?;
