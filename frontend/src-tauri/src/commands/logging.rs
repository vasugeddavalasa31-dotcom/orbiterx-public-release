...
/// Append a frontend log line to `ui.log` in the logs dir.
///
/// The packaged webview's console errors never reach the Rust tracing
/// pipeline, so the frontend forwards `console.error` / warnings here. This
/// makes debugging the installed DMG (where no dev console exists) possible.
#[cfg(feature = "tauri-runtime")]
#[cfg_attr(feature = "tauri-runtime", tauri::command)]
pub fn log_ui(message: String) -> Result<(), AppCommandError> {
    use std::io::Write;
    let dir = crate::paths::codeg_logs_root();
    std::fs::create_dir_all(&dir).map_err(|e| {
        AppCommandError::io_error("Failed to create log directory").with_detail(e.to_string())
    ...
    let mut file = std::fs::OpenOptions::new()
        ...
        .map_err(|e| {
            AppCommandError::io_error("Failed to open ui.log").with_detail(e.to_string())
        })?;
    ...
    writeln!(file, "[{ts}] {message}")
        .map_err(|e| AppCommandError::io_error("Failed to write ui.log").with_detail(e.to_string()))
}
