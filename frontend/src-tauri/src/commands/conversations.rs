...
pub async fn update_conversation_external_id_core(
    conn: &sea_orm::DatabaseConnection,
    conversation_id: i32,
    external_id: String,
) -> Result<(), AppCommandError> {
    conversation_service::update_external_id(conn, conversation_id, external_id)
        .await
        .map_err(AppCommandError::from)
}
...
pub async fn update_conversation_external_id(
    ...
    update_conversation_external_id_core(&db.conn, conversation_id, external_id).await?;
    emit_conversation_upsert(&EventEmitter::Tauri(app), &db.conn, conversation_id).await;
    Ok(())
}
...
mod tests {
    ...
    async fn create_chat_conversation_core_creates_dir_folder_and_conversation() {
        ...
        let rows =
            list_all_conversations_core(&db.conn, None, None, None, None, None, false, None, None)
                .await
                .expect("list");
