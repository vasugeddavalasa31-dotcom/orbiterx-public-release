...
impl CodexParser {
    pub fn new() -> Self {
        let codex_sessions = resolve_codex_home_dir().join("sessions");
        let orbiterx_sessions = resolve_orbiterx_home_dir().join("sessions");
        let mut base_dirs = Vec::new();
        if codex_sessions.exists() {
            base_dirs.push(codex_sessions);
        }
        if orbiterx_sessions.exists() {
            base_dirs.push(orbiterx_sessions);
        }
        Self { base_dirs }
    }
...
pub(crate) fn resolve_orbiterx_home_dir() -> PathBuf {
    resolve_orbiterx_home_dir_from(std::env::var_os("ORBITERX_HOME"), dirs::home_dir())
}

fn resolve_orbiterx_home_dir_from(
    orbiterx_home_env: Option<std::ffi::OsString>,
    home_dir: Option<PathBuf>,
) -> PathBuf {
    orbiterx_home_env
        .filter(|value| !value.is_empty())
        .map(PathBuf::from)
        .unwrap_or_else(|| home_dir.unwrap_or_default().join(".orbiterx"))
}
...
impl CodexParser {
    fn parse_conversation_detail(
        ...
        let mut model: Option<String> = None;
        let mut title: Option<String> = None;
        // Track whether this is an OrbiterX-originated rollout (originator = "orbiterx-frontend").
        // OrbiterX puts user messages in response_item with text-only content,
        // while Codex puts injected/system messages there.
        let mut is_orbiterx_rollout = false;
                ...
                "session_meta" => {
                    if let Some(payload) = value.get("payload") {
                        ...
                        git_branch = payload
                            ...
                            .and_then(|b| b.as_str())
                            .map(|s| s.to_string());
                        // Detect OrbiterX-originated rollouts
                        if let Some(originator) = payload.get("originator").and_then(|s| s.as_str())
                        {
                            if originator == "orbiterx-frontend" {
                                is_orbiterx_rollout = true;
                            }
                        }
                    }
                }
                    ...
                    if let Some(payload) = value.get("payload") {
                        ...
                        match payload_type {
                            ...
                            "message" => {
                                ...
                                if role == "user" {
                                    active_agent_count = 0;
                                    // For OrbiterX rollouts, user messages are in response_item
                                    // with text-only content. For Codex, only image-bearing
                                    // response_items are real user turns.
                                    let blocks = if is_orbiterx_rollout {
                                        extract_response_item_user_blocks(payload)
                                    } else {
...
/// Extract user message blocks from a response_item, handling both image-bearing
/// and text-only messages. Used for OrbiterX-originated rollouts where user
/// messages are in response_item with text-only content.
fn extract_response_item_user_blocks(payload: &serde_json::Value) -> Option<Vec<ContentBlock>> {
    ...
    for item in content {
        ...
        match item_type {
            "input_text" => {
                ...
                // Skip injected system messages that are not real user turns
                let trimmed = text.trim();
                if is_agents_instruction_message(trimmed)
                    || is_environment_context_message(trimmed)
                    || is_codex_internal_context_message(trimmed)
                {
                    continue;
...
fn group_into_turns(messages: Vec<UnifiedMessage>) -> Vec<MessageTurn> {
    ...
    while i < messages.len() {
        ...
        if matches!(msg.role, MessageRole::User) {
            ...
            // The rollout does not persist a per-turn duration; derive one from
            // the group's span (last completed message − first message start) so
            // the "Worked for …" header survives reload instead of showing 0s.
            // The live path carries the backend's real `turn.duration_ms`; this
            // fallback only fills the gap the desktop parser leaves behind.
            if duration_ms.is_none() {
                let derived = completed_at
                    .map(|end| end.signed_duration_since(timestamp).num_milliseconds())
                    .filter(|ms| *ms > 0)
                    .and_then(|ms| u64::try_from(ms).ok());
                if let Some(ms) = derived {
                    duration_ms = Some(ms);
                }
            }
...
mod tests {
    ...
    /// OrbiterX-originated rollouts put user messages in response_item with
    /// text-only content (no input_image). The parser must extract these as
    /// real user turns when the originator is "orbiterx-frontend".
    #[test]
    fn orbiterx_rollout_parses_text_only_user_messages() {
        let lines = vec![
            rollout_line(
                "2026-07-31T03:44:56Z",
                "session_meta",
                serde_json::json!({
                    "id": "orb-1",
                    "cwd": "/tmp/demo",
                    "originator": "orbiterx-frontend"
                }),
            ),
            rollout_line(
                "2026-07-31T03:44:57Z",
                "response_item",
                serde_json::json!({
                    "type": "message",
                    "role": "user",
        ...
        let path = write_temp_rollout("orbiterx", &lines);
        let detail = CodexParser::new()
            .parse_conversation_detail(&path, "orb-1")
            .expect("parse ok");
        ...
        assert_eq!(
            user_turns.len(),
            1,
            "one user turn from text-only response_item"
        );
        ...
        assert_eq!(user_text, Some("who are you"));
