#!/usr/bin/env python3
"""Stage one or more OrbiterX npm packages for release."""
...
BUILD_SCRIPT = REPO_ROOT / "orbiterx-cli" / "scripts" / "build_npm_package.py"
WORKFLOW_NAME = ".github/workflows/rust-release.yml"
GITHUB_REPO = "openai/orbiterx"
BINARY_TARGETS = (
...
_SPEC = importlib.util.spec_from_file_location("orbiterx_build_npm_package", BUILD_SCRIPT)
...
ORBITERX_PLATFORM_PACKAGES = getattr(_BUILD_MODULE, "ORBITERX_PLATFORM_PACKAGES", {})
ORBITERX_PACKAGE_COMPONENT = getattr(
    _BUILD_MODULE, "ORBITERX_PACKAGE_COMPONENT", "orbiterx-package"
)
...
BINARY_COMPONENTS = {
    "orbiterx-responses-api-proxy": BinaryComponent(
        artifact_prefix="orbiterx-responses-api-proxy",
        dest_dir="orbiterx-responses-api-proxy",
        binary_basename="orbiterx-responses-api-proxy",
    ),
}
...
def install_from_workflow_artifacts(
    ...
    if ORBITERX_PACKAGE_COMPONENT in components:
        install_orbiterx_package_archives(artifacts_dir, vendor_dir, BINARY_TARGETS)
...
def select_target_artifacts(
    ...
    needs_target_artifacts = ORBITERX_PACKAGE_COMPONENT in components or any(
...
def install_orbiterx_package_archives(
    artifacts_dir: Path,
    vendor_dir: Path,
    ...
    print(
        "Installing OrbiterX package archives for targets: " + ", ".join(targets),
        flush=True,
    )
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = {
            executor.submit(
                install_single_orbiterx_package_archive,
                artifacts_dir,
                vendor_dir,
                target,
            ): target
...
def install_single_orbiterx_package_archive(
    artifacts_dir: Path,
    vendor_dir: Path,
    target: str,
) -> Path:
    artifact_subdir = artifact_dir_for_target(artifacts_dir, target)
    archive_path = artifact_subdir / f"orbiterx-package-{target}.tar.gz"
...
def tarball_name_for_package(package: str, version: str) -> str:
    if package in ORBITERX_PLATFORM_PACKAGES:
        platform = package.removeprefix("orbiterx-")
        return f"orbiterx-npm-{platform}-{version}.tgz"
