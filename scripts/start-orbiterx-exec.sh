...
remote_path='~/code/orbiterx-sync'
local_exec_server_port="${ORBITERX_REMOTE_EXEC_SERVER_LOCAL_PORT:-8765}"
remote_exec_server_start_timeout_seconds="${ORBITERX_REMOTE_EXEC_SERVER_START_TIMEOUT_SECONDS:-15}"
...
remote_exec_server_log_path="/tmp/orbiterx-exec-server-${sync_instance_id}.log"
remote_exec_server_pid_path="/tmp/orbiterx-exec-server-${sync_instance_id}.pid"
...
remote_repo_root="$HOME/code/orbiterx-sync"
remote_orbiterx_rs="$remote_repo_root/orbiterx-rs"

cd "${remote_orbiterx_rs}"
cargo build -p orbiterx-cli --bin orbiterx

rm -f "${remote_exec_server_log_path}" "${remote_exec_server_pid_path}"
nohup ./target/debug/orbiterx exec-server --listen ws://127.0.0.1:0 \
...
echo "Start orbiterx via: "
printf '  ORBITERX_EXEC_SERVER_URL=ws://127.0.0.1:%s orbiterx -C %q\n' \
