"""Command-line interface for building OrbiterX package directories."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build a canonical OrbiterX package directory and optional archive.",
    ...
    parser.add_argument(
        ...
        type=Path,
        help=(
            "Optional prebuilt orbiterx-code-mode-host executable. If omitted, "
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "scripts/orbiterx_package/orbiterx-zsh."
        ),
    )
    parser.add_argument(
        "--orbiterx-command-runner-bin",
        type=Path,
        help=(
            "Optional prebuilt Windows orbiterx-command-runner.exe executable. "
            "If omitted for Windows targets, orbiterx-command-runner is built "
            "with Cargo."
        ),
    )
    parser.add_argument(
        "--orbiterx-windows-sandbox-setup-bin",
        type=Path,
        help=(
            "Optional prebuilt Windows orbiterx-windows-sandbox-setup.exe "
            "executable. If omitted for Windows targets, "
            "orbiterx-windows-sandbox-setup is built with Cargo."
        ),
    )
    parser.add_argument(
        "--rg-bin",
...
def main() -> int:
    ...
    package_dir = (
        ...
        else Path(tempfile.mkdtemp(prefix="orbiterx-package-")).resolve()
    )
    ...
    source_outputs = build_source_binaries(
        ...
        orbiterx_command_runner_bin=resolve_optional_input_path(
            args.orbiterx_command_runner_bin,
            "prebuilt Windows orbiterx-command-runner.exe executable",
            "--orbiterx-command-runner-bin",
        ),
        orbiterx_windows_sandbox_setup_bin=resolve_optional_input_path(
            args.orbiterx_windows_sandbox_setup_bin,
            "prebuilt Windows orbiterx-windows-sandbox-setup.exe executable",
            "--orbiterx-windows-sandbox-setup-bin",
        ),
    )
    ...
    inputs = PackageInputs(
        ...
        orbiterx_command_runner_bin=source_outputs.orbiterx_command_runner_bin,
        orbiterx_windows_sandbox_setup_bin=source_outputs.orbiterx_windows_sandbox_setup_bin,
    )
    ...
    for archive_output in args.archive_output:
        ...
        print(f"Built OrbiterX package archive at {archive_path}")

    print(f"Built OrbiterX package directory at {package_dir}")
    return 0
