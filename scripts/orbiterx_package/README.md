# OrbiterX package builder

This package contains the implementation behind `scripts/build_orbiterx_package.py`.
...
The builder creates a canonical OrbiterX package directory:
...
├── bin
│   ├── <entrypoint>[.exe]
│   └── orbiterx-code-mode-host[.exe]
├── orbiterx-resources
│   ├── bwrap                             # Linux only
│   ├── zsh/bin/zsh                       # supported Unix targets only
│   ├── orbiterx-command-runner.exe          # Windows only
│   └── orbiterx-windows-sandbox-setup.exe   # Windows only
└── orbiterx-path
    └── rg[.exe]
```
...
host platform. On Linux, that default is a musl target to match OrbiterX release
...
`orbiterx` and `orbiterx-app-server`. The `version` field in `orbiterx-package.json` is
read from `[workspace.package].version` in `orbiterx-rs/Cargo.toml`.
...
- all targets: `orbiterx-code-mode-host`, unless `--code-mode-host-bin` is provided
- Linux targets: `bwrap`, unless `--bwrap-bin` is provided
- Windows targets: `orbiterx-command-runner` and `orbiterx-windows-sandbox-setup`,
...
`--orbiterx-command-runner-bin` plus `--orbiterx-windows-sandbox-setup-bin` for
...
downloads and verifies the matching OrbiterX-built V8 release pair before invoking
...
DotSlash manifest at `scripts/orbiterx_package/rg`. Downloaded archives are cached
under `$TMPDIR/orbiterx-package/<target>-rg` and are reused only after the recorded
...
manifest at `scripts/orbiterx_package/orbiterx-zsh` when the selected target has a
matching prebuilt artifact. Downloaded archives are cached under
`$TMPDIR/orbiterx-package/<target>-zsh` and installed at
`orbiterx-resources/zsh/bin/zsh`. Pass `--zsh-manifest` to use a different
