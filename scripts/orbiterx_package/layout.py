"""Canonical OrbiterX package directory layout."""
...
def build_package_dir(
    ...
    resources_dir = package_dir / "orbiterx-resources"
    path_dir = package_dir / "orbiterx-path"
    bin_dir.mkdir()
    resources_dir.mkdir()
    path_dir.mkdir()
    ...
    copy_executable(
        ...
        bin_dir / f"orbiterx-code-mode-host{spec.exe_suffix}",
        is_windows=spec.is_windows,
    )
    ...
    if inputs.orbiterx_command_runner_bin is not None:
        copy_executable(
            inputs.orbiterx_command_runner_bin,
            resources_dir / "orbiterx-command-runner.exe",
            is_windows=True,
        )

    if inputs.orbiterx_windows_sandbox_setup_bin is not None:
        copy_executable(
            inputs.orbiterx_windows_sandbox_setup_bin,
            resources_dir / "orbiterx-windows-sandbox-setup.exe",
            is_windows=True,
        )
    ...
    metadata = {
        ...
        "resourcesDir": "orbiterx-resources",
        "pathDir": "orbiterx-path",
    }
    write_json(package_dir / "orbiterx-package.json", metadata)
...
def validate_package_dir(
    ...
    *,
    ...
    metadata_path = package_dir / "orbiterx-package.json"
    if not metadata_path.is_file():
        raise RuntimeError("Missing package metadata: orbiterx-package.json")
    ...
    required_files = [
        ...
        Path("bin") / f"orbiterx-code-mode-host{spec.exe_suffix}",
        Path("orbiterx-path") / spec.rg_name,
    ]
    executable_files = list(required_files)

    if include_zsh:
        zsh_path = Path("orbiterx-resources") / ZSH_RESOURCE_PATH
        required_files.append(zsh_path)
        executable_files.append(zsh_path)

    if spec.is_linux:
        required_files.append(Path("orbiterx-resources") / "bwrap")
        executable_files.append(Path("orbiterx-resources") / "bwrap")

    if spec.is_windows:
        required_files.extend(
            [
                Path("orbiterx-resources") / "orbiterx-command-runner.exe",
                Path("orbiterx-resources") / "orbiterx-windows-sandbox-setup.exe",
            ]
        )
