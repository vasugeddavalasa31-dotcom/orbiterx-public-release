...
import { orbiterxExecSpy } from "./orbiterxExecSpy";
...
import { createMockClient, createTestClient } from "./testOrbiterX";
...
describe("OrbiterX", () => {
  ...
  it("passes turn options to exec", async () => {
    ...
    const { args: spawnArgs, restore } = orbiterxExecSpy();
    const { client, cleanup } = createMockClient(url);
  ...
  it("passes OrbiterXOptions config overrides as TOML --config flags", async () => {
    const { url, close } = await startResponsesTestProxy({
      statusCode: 200,
      responseBodies: [
        sse(
  ...
  it("lets thread options override OrbiterXOptions config overrides", async () => {
    const { url, close } = await startResponsesTestProxy({
      statusCode: 200,
      responseBodies: [
        sse(
  ...
  it("forwards images to exec", async () => {
    ...
    const { args: spawnArgs, restore } = orbiterxExecSpy();
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "orbiterx-images-"));
  ...
  it("runs in provided working directory", async () => {
    ...
    const { args: spawnArgs, restore } = orbiterxExecSpy();
    const workingDirectory = fs.mkdtempSync(path.join(os.tmpdir(), "orbiterx-working-dir-"));
    const { client, cleanup } = createTestClient({
      baseUrl: url,
      apiKey: "test",
    });
  ...
  it("sets the orbiterx sdk originator header", async () => {
    const { url, close, requests } = await startResponsesTestProxy({
      statusCode: 200,
      responseBodies: [sse(responseStarted(), assistantMessage("Hi!"), responseCompleted())],
    });
    ...
    try {
      ...
      if (Array.isArray(originatorHeader)) {
        expect(originatorHeader).toContain("orbiterx_sdk_ts");
      } else {
        expect(originatorHeader).toBe("orbiterx_sdk_ts");
      }
    } finally {
      cleanup();
      await close();
...
/**
 * Given a list of args to `orbiterx` and a `key`, collects all `--config`
