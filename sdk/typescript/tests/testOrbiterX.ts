...
import { OrbiterX } from "../src/orbiterx";
import type { OrbiterXConfigObject } from "../src/orbiterxOptions";

export const orbiterxExecPath =
  process.env.ORBITERX_EXEC_PATH ??
...
export function createTestClient(options: CreateTestClientOptions = {}): TestClient {
  ...
  return {
    cleanup: () => {},
    client: new OrbiterX({
      orbiterxPathOverride: orbiterxExecPath,
...
function mergeTestConfig(
  ...
  config: OrbiterXConfigObject | undefined,
): OrbiterXConfigObject | undefined {
  const mergedConfig: OrbiterXConfigObject | undefined =
  ...
  return {
    ...
    // sync does not race temp ORBITERX_HOME cleanup.
    features:
...
function hasExplicitProviderConfig(config: OrbiterXConfigObject | undefined): boolean {
...
function getCurrentEnv(): Record<string, string> {
  ...
  for (const [key, value] of Object.entries(process.env)) {
    if (key === "ORBITERX_INTERNAL_ORIGINATOR_OVERRIDE") {
      continue;
    }
    if (value !== undefined) {
      env[key] = value;
