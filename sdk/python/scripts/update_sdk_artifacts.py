...
SDK_DISTRIBUTION_NAME = "openai-orbiterx"
RUNTIME_DISTRIBUTION_NAME = "openai-orbiterx-cli-bin"
RUNTIME_PACKAGE_ROOT = Path("src") / "orbiterx_cli_bin"
ORBITERX_PACKAGE_METADATA = "orbiterx-package.json"
...
def schema_bundle_path(schema_dir: Path) -> Path:
    ...
    return schema_dir / "orbiterx_app_server_protocol.v2.schemas.json"
...
def runtime_binary_name() -> str:
    return "orbiterx.exe" if _is_windows() else "orbiterx"


def runtime_code_mode_host_name() -> str:
    return "orbiterx-code-mode-host.exe" if _is_windows() else "orbiterx-code-mode-host"
...
def pinned_runtime_version() -> str:
    ...
    return normalize_orbiterx_version(pins[0])


def pinned_runtime_orbiterx_path() -> Path:
    """Return the bundled OrbiterX binary from the installed pinned runtime wheel."""
    ...
    normalized_installed_version = normalize_orbiterx_version(installed_version)
    ...
    try:
        ...
        raise RuntimeError(
            f"Installed {RUNTIME_DISTRIBUTION_NAME} package does not expose bundled_orbiterx_path."
        ) from exc

    orbiterx_path = bundled_orbiterx_path()
    if not orbiterx_path.exists():
        raise RuntimeError(f"Pinned OrbiterX runtime binary not found at {orbiterx_path}.")
    return orbiterx_path


def normalize_orbiterx_version(version: str) -> str:
    ...
    if not re.fullmatch(r"[0-9]+(?:\.[0-9]+)*(?:(?:a|b|rc)[0-9]+)?", normalized):
        raise RuntimeError(f"Could not normalize OrbiterX version {version!r} to a PEP 440 version")
    return normalized
...
def stage_python_sdk_package(staging_dir: Path, sdk_version: str) -> Path:
    package_version = normalize_orbiterx_version(sdk_version)
    _copy_package_tree(sdk_root(), staging_dir)
    sdk_bin_dir = staging_dir / "src" / "orbiterx" / "bin"
...
def stage_python_runtime_package(
    ...
    package_version = normalize_orbiterx_version(orbiterx_version)
    ...
    _extract_orbiterx_package_archive(package_archive, staged_runtime_package_root(staging_dir))
    return staging_dir


def _extract_orbiterx_package_archive(package_archive: Path, runtime_package_root: Path) -> None:
    if not package_archive.name.endswith(".tar.gz"):
        raise RuntimeError(f"Expected a .tar.gz OrbiterX package archive: {package_archive}")
    ...
    _validate_orbiterx_package_layout(runtime_package_root, package_archive)


def _validate_orbiterx_package_layout(package_dir: Path, package_archive: Path) -> None:
    missing_entries = []
    if not (package_dir / ORBITERX_PACKAGE_METADATA).is_file():
        missing_entries.append(ORBITERX_PACKAGE_METADATA)
    for entry in ("bin", "orbiterx-resources", "orbiterx-path"):
    ...
    if missing_entries:
        ...
        raise RuntimeError(f"Missing OrbiterX package layout entries in {package_archive}: {missing}")
...
def generate_schema_from_pinned_runtime(schema_dir: Path) -> Path:
    ...
    orbiterx_path = pinned_runtime_orbiterx_path()
...
def generate_v2_all(schema_dir: Path) -> None:
    ...
    out_path = sdk_root() / "src" / "orbiterx" / "generated" / "v2_all.py"
...
def _notification_specs(schema_dir: Path) -> list[tuple[str, str]]:
    ...
    generated_source = (sdk_root() / "src" / "orbiterx" / "generated" / "v2_all.py").read_text()
...
def generate_notification_registry(schema_dir: Path) -> None:
    ...
    out = sdk_root() / "src" / "orbiterx" / "generated" / "notification_registry.py"
...
def _load_public_fields(
    ...
    if module_name == "orbiterx.generated.v2_all":
...
def _load_generated_v2_all_module() -> types.ModuleType:
    ...
    module_name = "_openai_orbiterx_generated_v2_all_for_artifacts"
    sys.modules.pop(module_name, None)
    module_path = sdk_root() / "src" / "orbiterx" / "generated" / "v2_all.py"
...
def _render_orbiterx_block(
    ...
    lines = [
        ...
        *_kw_signature_lines(thread_start_fields),
        "    ) -> Thread:",
        '        """Create a new OrbiterX conversation thread."""',
...
def _render_async_orbiterx_block(
    thread_start_fields: list[PublicFieldSpec],
    thread_list_fields: list[PublicFieldSpec],
    resume_fields: list[PublicFieldSpec],
    fork_fields: list[PublicFieldSpec],
...
def _render_async_thread_block(
    ...
    lines = [
        ...
        *_model_arg_lines(turn_fields),
        "        )",
        "        turn = await self._orbiterx._client.turn_start(",
        "            self.id,",
        "            wire_input,",
        "            params=params,",
        "        )",
        "        return AsyncTurnHandle(self._orbiterx, self.id, turn.turn.id)",
    ]
    return "\n".join(lines)
...
def generate_public_api_flat_methods() -> None:
    ...
    public_api_path = src_dir / "orbiterx" / "api.py"
    ...
    source = _replace_generated_block(
        source,
        "OrbiterX.flat_methods",
        _render_orbiterx_block(
            thread_start_fields,
            thread_list_fields,
    ...
    source = _replace_generated_block(
        source,
        "AsyncOrbiterX.flat_methods",
        _render_async_orbiterx_block(
            thread_start_fields,
            thread_list_fields,
            thread_resume_fields,
            thread_fork_fields,
...
def generate_types() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="orbiterx-python-schema-") as td:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    stage_runtime_parser.add_argument(
        "package_archive",
        type=Path,
        help="Path to a OrbiterX package .tar.gz archive for this platform.",
    )
    stage_runtime_parser.add_argument(
        "--orbiterx-version",
        required=True,
        help=(
            "OrbiterX release version to write into the staged runtime package. "
...
def run_command(args: argparse.Namespace, ops: CliOps) -> None:
    if args.command == "generate-types":
        ...
        ops.stage_python_sdk_package(
            args.staging_dir,
            normalize_orbiterx_version(args.sdk_version),
        )
    elif args.command == "stage-runtime":
        ops.stage_python_runtime_package(
            args.staging_dir,
            normalize_orbiterx_version(args.orbiterx_version),
