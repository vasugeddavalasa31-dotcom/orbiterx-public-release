...
from .errors import OrbiterXError, InvalidRequestError, TransportClosedError
...
RUNTIME_PKG_NAME = "openai-orbiterx-cli-bin"
...
def _installed_orbiterx_path() -> Path:
    try:
        from orbiterx_cli_bin import bundled_orbiterx_path
    except ImportError as exc:
        raise FileNotFoundError(
            "Unable to locate the pinned OrbiterX runtime. Install the published SDK build "
            f"with its {RUNTIME_PKG_NAME} dependency, or set OrbiterXConfig.orbiterx_bin "
            "explicitly."
        ) from exc

    return bundled_orbiterx_path()


def _installed_orbiterx_path_dirs() -> tuple[Path, ...]:
    try:
        from orbiterx_cli_bin import bundled_path_dir
...
class OrbiterXBinResolverOps:
    installed_orbiterx_path: Callable[[], Path]
    path_exists: Callable[[Path], bool]


def _default_orbiterx_bin_resolver_ops() -> OrbiterXBinResolverOps:
    return OrbiterXBinResolverOps(
        installed_orbiterx_path=_installed_orbiterx_path,
        path_exists=lambda path: path.exists(),
    )


def resolve_orbiterx_bin(config: "OrbiterXConfig", ops: OrbiterXBinResolverOps) -> Path:
    if config.orbiterx_bin is not None:
        orbiterx_bin = Path(config.orbiterx_bin)
        if not ops.path_exists(orbiterx_bin):
            raise FileNotFoundError(
                f"OrbiterX binary not found at {orbiterx_bin}. Set OrbiterXConfig.orbiterx_bin "
                "to a valid binary path."
            )
        return orbiterx_bin

    return ops.installed_orbiterx_path()


def _resolve_orbiterx_bin(config: "OrbiterXConfig") -> Path:
    return resolve_orbiterx_bin(config, _default_orbiterx_bin_resolver_ops())


@dataclass(slots=True)
class OrbiterXConfig:
    """Configuration for launching and identifying the local OrbiterX runtime.

    Most callers can use ``OrbiterX()`` without configuration. Set ``orbiterx_bin``
    only when intentionally using a specific local OrbiterX executable.
    """

    orbiterx_bin: str | None = None
    launch_args_override: tuple[str, ...] | None = None
    config_overrides: tuple[str, ...] = ()
    cwd: str | None = None
    env: dict[str, str] | None = None
    client_name: str = "orbiterx_python_sdk"
    client_title: str = "OrbiterX Python SDK"
    client_version: str = SDK_VERSION
    experimental_api: bool = True


class OrbiterXClient:
    """Synchronous typed JSON-RPC client for `orbiterx app-server` over stdio."""

    def __init__(
        self,
        config: OrbiterXConfig | None = None,
        approval_handler: ApprovalHandler | None = None,
    ) -> None:
        self.config = config or OrbiterXConfig()
    ...
    def __enter__(self) -> "OrbiterXClient":
        self.start()
        return self
    ...
    def start(self) -> None:
        ...
        if self.config.launch_args_override is not None:
            ...
            orbiterx_bin = _resolve_orbiterx_bin(self.config)
            if self.config.orbiterx_bin is None:
                path_dirs = _installed_orbiterx_path_dirs()
            args = [str(orbiterx_bin)]
    ...
    def request(
        ...
        *,
        ...
        if not isinstance(result, dict):
            raise OrbiterXError(f"{method} response must be a JSON object")
    ...
    def _write_message(self, payload: JsonObject) -> None:
        if self._proc is None or self._proc.stdin is None:
            raise TransportClosedError("OrbiterX process is not running")
        with self._lock:
    ...
    def _read_message(self) -> dict[str, JsonValue]:
        ...
        if not line:
            raise TransportClosedError(
                f"OrbiterX process closed stdout. stderr_tail={self._stderr_tail()[:2000]}"
            )

        try:
            message = json.loads(line)
        except json.JSONDecodeError as exc:
            raise OrbiterXError(f"Invalid JSON-RPC line: {line!r}") from exc

        if not isinstance(message, dict):
            raise OrbiterXError(f"Invalid JSON-RPC payload: {message!r}")
        return message


def default_orbiterx_home() -> str:
    return str(Path.home() / ".orbiterx")
