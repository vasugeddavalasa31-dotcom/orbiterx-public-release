...
from .client import OrbiterXClient, OrbiterXConfig
...
class OrbiterX:
    """Synchronous client for creating threads and running OrbiterX turns.

    The client starts its runtime connection during construction. Use it as a
    context manager so resources are closed promptly.
    """

    def __init__(self, config: OrbiterXConfig | None = None) -> None:
        self._client = OrbiterXClient(config=config)
        try:
            self._client.start()
    ...
    def login_api_key(self, api_key: str) -> None:
        """Authenticate OrbiterX with an API key."""
    ...
    def account(self, *, refresh_token: bool = False) -> GetAccountResponse:
        """Read the current OrbiterX account state."""
        return self._client.account_read(GetAccountParams(refresh_token=refresh_token))

    def logout(self) -> None:
        """Clear the current OrbiterX account session."""
        self._client.account_logout()

    # BEGIN GENERATED: OrbiterX.flat_methods
    def thread_start(
        self,
        *,
...
class AsyncOrbiterX:
    ...
    def metadata(self) -> InitializeResponse:
        if self._init is None:
            raise RuntimeError(
                "AsyncOrbiterX is not initialized yet. Prefer `async with AsyncOrbiterX()`; "
    ...
    # BEGIN GENERATED: AsyncOrbiterX.flat_methods
    ...
    # END GENERATED: AsyncOrbiterX.flat_methods
...
class AsyncThread:
    ...
    async def read(self, *, include_turns: bool = False) -> ThreadReadResponse:
        """Read this thread, optionally including its turn history."""
        await self._orbiterx._ensure_initialized()
        return await self._orbiterx._client.thread_read(self.id, include_turns=include_turns)

    async def set_name(self, name: str) -> ThreadSetNameResponse:
        await self._orbiterx._ensure_initialized()
        return await self._orbiterx._client.thread_set_name(self.id, name)

    async def compact(self) -> ThreadCompactStartResponse:
        await self._orbiterx._ensure_initialized()
        return await self._orbiterx._client.thread_compact(self.id)
...
class AsyncTurnHandle:
    ...
    async def steer(self, input: RunInput) -> TurnSteerResponse:
        """Send additional input to this active turn."""
        await self._orbiterx._ensure_initialized()
        return await self._orbiterx._client.turn_steer(
            self.thread_id,
            self.id,
            _to_wire_input(_normalize_run_input(input)),
        )
    ...
    async def interrupt(self) -> TurnInterruptResponse:
        """Request interruption of this active turn."""
        await self._orbiterx._ensure_initialized()
        return await self._orbiterx._client.turn_interrupt(self.thread_id, self.id)

    async def stream(self) -> AsyncIterator[Notification]:
        """Yield only notifications routed to this async turn handle."""
        await self._orbiterx._ensure_initialized()
        self._orbiterx._client.register_turn_notifications(self.id)
        try:
            while True:
                event = await self._orbiterx._client.next_turn_notification(self.id)
                yield event
                if (
                    event.method == "turn/completed"
                    and isinstance(event.payload, TurnCompletedNotification)
            ...
            self._orbiterx._client.unregister_turn_notifications(self.id)
