...
class AsyncOrbiterXClient:
    """Async wrapper around OrbiterXClient using thread offloading."""

    def __init__(self, config: OrbiterXConfig | None = None) -> None:
        """Create the wrapped sync client that owns the transport process."""
        self._sync = OrbiterXClient(config=config)

    async def __aenter__(self) -> "AsyncOrbiterXClient":
        """Start the OrbiterX process when entering an async context."""
        await self.start()
        return self

    async def __aexit__(self, _exc_type, _exc, _tb) -> None:
        """Close the OrbiterX process when leaving an async context."""
        await self.close()
    ...
    async def initialize(self) -> InitializeResponse:
        """Initialize the OrbiterX session."""
        ...
        try:
            ...
            def cleanup_cancelled_start(
                ...
                threading.Thread(
                    ...
                    name="orbiterx-goal-start-cleanup",
                    daemon=True,
                ).start()
