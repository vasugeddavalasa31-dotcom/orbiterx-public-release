...
class OrbiterXError(Exception):
    """Base exception for SDK errors."""


class JsonRpcError(OrbiterXError):
...
class TransportClosedError(OrbiterXError):
    """Raised when the OrbiterX transport closes unexpectedly."""


class OrbiterXRpcError(JsonRpcError):
    """Base typed error for JSON-RPC failures."""


class ParseError(OrbiterXRpcError):
    """Raised when a request or response cannot be parsed."""


class InvalidRequestError(OrbiterXRpcError):
    """Raised when the runtime rejects the request shape."""


class MethodNotFoundError(OrbiterXRpcError):
    """Raised when the requested operation is unavailable."""


class InvalidParamsError(OrbiterXRpcError):
    """Raised when an operation receives invalid parameters."""


class InternalRpcError(OrbiterXRpcError):
    """Raised when the runtime reports an internal RPC failure."""


class ServerBusyError(OrbiterXRpcError):
...
def _is_server_overloaded(data: Any) -> bool:
    ...
    if isinstance(data, dict):
        direct = data.get("orbiterx_error_info") or data.get("orbiterxErrorInfo") or data.get("errorInfo")
...
def map_jsonrpc_error(code: int, message: str, data: Any = None) -> JsonRpcError:
    ...
    if -32099 <= code <= -32000:
        ...
        if _contains_retry_limit_text(message):
            return RetryLimitExceededError(code, message, data)
        return OrbiterXRpcError(code, message, data)
