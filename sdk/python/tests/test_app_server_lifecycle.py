...
def test_thread_list_filters_archived_threads(tmp_path) -> None:
    ...
    with AppServerHarness(tmp_path) as harness:
        ...
        with OrbiterX(config=harness.app_server_config()) as orbiterx:
            active_thread = orbiterx.thread_start()
            archived_thread = orbiterx.thread_start()
            active_thread.run("keep this listed")
            archived_thread.run("archive this")
            orbiterx.thread_archive(archived_thread.id)
            active_list = orbiterx.thread_list(archived=False)
            archived_list = orbiterx.thread_list(archived=True)
    ...
    async def scenario() -> None:
        ...
        with AppServerHarness(tmp_path) as harness:
            ...
            async with AsyncOrbiterX(config=harness.app_server_config()) as orbiterx:
                ...
                resumed = await orbiterx.thread_resume(thread.id)
                forked = await orbiterx.thread_fork(thread.id)
                archive_response = await orbiterx.thread_archive(thread.id)
                unarchived = await orbiterx.thread_unarchive(thread.id)
...
def test_thread_fork_returns_distinct_thread(tmp_path) -> None:
    ...
    with AppServerHarness(tmp_path) as harness:
        ...
        with OrbiterX(config=harness.app_server_config()) as orbiterx:
            ...
            forked = orbiterx.thread_fork(thread.id)
...
def test_archive_unarchive_round_trip_uses_materialized_rollout(tmp_path) -> None:
    ...
    with AppServerHarness(tmp_path) as harness:
        ...
        with OrbiterX(config=harness.app_server_config()) as orbiterx:
            ...
            archived = orbiterx.thread_archive(thread.id)
            unarchived = orbiterx.thread_unarchive(thread.id)
