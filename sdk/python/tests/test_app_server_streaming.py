...
def test_low_level_sync_stream_text_uses_real_turn_routing(tmp_path) -> None:
    """OrbiterXClient.stream_text should stream through a real app-server turn."""
    with AppServerHarness(tmp_path) as harness:
        harness.responses.enqueue_sse(
        ...
        with OrbiterX(config=harness.app_server_config()) as orbiterx:
            thread = orbiterx.thread_start()
            chunks = list(orbiterx._client.stream_text(thread.id, "low-level sync"))  # noqa: SLF001
...
def test_low_level_async_stream_text_allows_parallel_model_list(tmp_path) -> None:
    ...
    async def scenario() -> None:
        ...
        with AppServerHarness(tmp_path) as harness:
            ...
            async with AsyncOrbiterX(config=harness.app_server_config()) as orbiterx:
                thread = await orbiterx.thread_start()
                stream = orbiterx._client.stream_text(  # noqa: SLF001
                    thread.id,
                    "low-level async",
                )
                first = await anext(stream)
                models_task = asyncio.create_task(orbiterx.models())
...
def test_interleaved_sync_turn_streams_route_by_turn_id(tmp_path) -> None:
    ...
    with AppServerHarness(tmp_path) as harness:
        ...
        with OrbiterX(config=harness.app_server_config()) as orbiterx:
            first_thread = orbiterx.thread_start()
            second_thread = orbiterx.thread_start()
...
def test_interleaved_async_turn_streams_route_by_turn_id(tmp_path) -> None:
    ...
    async def scenario() -> None:
        ...
        with AppServerHarness(tmp_path) as harness:
            ...
            async with AsyncOrbiterX(config=harness.app_server_config()) as orbiterx:
                first_thread = await orbiterx.thread_start()
                second_thread = await orbiterx.thread_start()
