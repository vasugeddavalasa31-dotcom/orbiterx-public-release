# OrbiterX Python SDK - API Reference

Public surface of `orbiterx` for OrbiterX workflows.
...
- Version: `orbiterx.__version__`
- Requires Python >= 3.10
- Public OrbiterX protocol value and event types live in `orbiterx.types`

## OrbiterX (sync)

```python
OrbiterX(config: OrbiterXConfig | None = None)
```
...
## AsyncOrbiterX (async parity)

```python
AsyncOrbiterX(config: OrbiterXConfig | None = None)
```
...
`AsyncOrbiterX` initializes lazily. Context entry is the standard path because it
...
When `sandbox=` is omitted, OrbiterX uses its configured default. A sandbox
...
- one `OrbiterX` instance can stream multiple active turns concurrently
...
- `stream()` and `run()` consume only notifications for their own turn ID
- one `AsyncOrbiterX` instance can stream multiple active turns concurrently
...
The SDK wrappers return and accept public OrbiterX protocol models wherever possible:
