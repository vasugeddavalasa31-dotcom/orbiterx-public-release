...
`openai-orbiterx` publishes stable releases. Install the latest one with
`pip install openai-orbiterx`.

## Why does the SDK install a runtime package?

The SDK version tracks the corresponding OrbiterX CLI release. Each SDK release
...
- `OrbiterX` is the sync public API.
- `AsyncOrbiterX` is an async replica of the same public API shape.
- Prefer `async with AsyncOrbiterX()` for async code. It is the standard path for
  explicit startup/shutdown, and `AsyncOrbiterX` initializes lazily on context
  entry or first awaited API use.

If your app is not already async, stay with `OrbiterX`.
...
When `sandbox=` is omitted, OrbiterX uses its configured default. A turn
...
`OrbiterX()` is eager: it starts transport and calls `initialize` in `__init__`.

Common causes:

- installation is incomplete and the pinned `openai-orbiterx-cli-bin` dependency is missing
- local `orbiterx_bin` override points to a missing file
- a custom local OrbiterX executable does not support the SDK operation being used
