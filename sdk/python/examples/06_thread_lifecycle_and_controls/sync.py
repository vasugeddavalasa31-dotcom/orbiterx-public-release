...
with OrbiterX(config=runtime_config()) as orbiterx:
    ...
    reopened = orbiterx.thread_resume(thread.id)
    listing_active = orbiterx.thread_list(limit=20, archived=False)
    reading = reopened.read(include_turns=True)

    _ = reopened.set_name("sdk-lifecycle-demo")
    _ = orbiterx.thread_archive(reopened.id)
    listing_archived = orbiterx.thread_list(limit=20, archived=True)
    unarchived = orbiterx.thread_unarchive(reopened.id)

    resumed = orbiterx.thread_resume(
        unarchived.id,
        model="gpt-5.4",
    ...
    forked = orbiterx.thread_fork(unarchived.id, model="gpt-5.4")
