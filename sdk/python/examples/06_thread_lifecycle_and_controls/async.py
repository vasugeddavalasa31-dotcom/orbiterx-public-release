...
async def main() -> None:
    async with AsyncOrbiterX(config=runtime_config()) as orbiterx:
        ...
        reopened = await orbiterx.thread_resume(thread.id)
        listing_active = await orbiterx.thread_list(limit=20, archived=False)
        reading = await reopened.read(include_turns=True)

        _ = await reopened.set_name("sdk-lifecycle-demo")
        _ = await orbiterx.thread_archive(reopened.id)
        listing_archived = await orbiterx.thread_list(limit=20, archived=True)
        unarchived = await orbiterx.thread_unarchive(reopened.id)

        resumed = await orbiterx.thread_resume(
            unarchived.id,
            model="gpt-5.4",
        ...
        forked = await orbiterx.thread_fork(unarchived.id, model="gpt-5.4")
        forked_result = await (
