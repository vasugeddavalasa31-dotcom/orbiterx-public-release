...
use crate::protocol::OrbiterXErrorInfo;
...
use orbiterx_utils_string::truncate_middle_chars;
use orbiterx_utils_string::truncate_middle_with_token_budget;
...
pub type Result<T> = std::result::Result<T, OrbiterXErr>;
...
pub enum OrbiterXErr {
    ...
    #[error(
        "OrbiterX ran out of room in the model's context window. Start a new thread or clear earlier history before retrying."
    )]
    ...
    /// could not be captured). Analogous to the previous `OrbiterXError::Spawn` variant.
    ...
    QuotaExceeded,
    #[error(
        "To use OrbiterX with your ChatGPT plan, upgrade to Plus: https://chatgpt.com/explore/plus."
    )]
    UsageNotIncluded,
    ...
    #[error("orbiterx-linux-sandbox was required but not provided")]
...
impl From<CancelErr> for OrbiterXErr {
    fn from(_: CancelErr) -> Self {
        OrbiterXErr::TurnAborted
    }
}
...
impl OrbiterXErr {
    pub fn is_retryable(&self) -> bool {
        match self {
            OrbiterXErr::TurnAborted
            | OrbiterXErr::SessionBudgetExceeded
            | OrbiterXErr::Interrupted
            | OrbiterXErr::EnvVar(_)
            | OrbiterXErr::Fatal(_)
            | OrbiterXErr::UsageNotIncluded
            | OrbiterXErr::QuotaExceeded
            | OrbiterXErr::InvalidImageRequest()
            | OrbiterXErr::InvalidRequest(_)
            | OrbiterXErr::RefreshTokenFailed(_)
            | OrbiterXErr::UnsupportedOperation(_)
            | OrbiterXErr::Sandbox(_)
            | OrbiterXErr::LandlockSandboxExecutableNotProvided
            | OrbiterXErr::RetryLimit(_)
            | OrbiterXErr::ContextWindowExceeded
            | OrbiterXErr::ThreadNotFound(_)
            | OrbiterXErr::AgentLimitReached { .. }
            | OrbiterXErr::Spawn
            | OrbiterXErr::SessionConfiguredNotFirstEvent
            | OrbiterXErr::UsageLimitReached(_)
            | OrbiterXErr::ServerOverloaded
            | OrbiterXErr::CyberPolicy { .. } => false,
            OrbiterXErr::Stream(..)
            | OrbiterXErr::Timeout
            | OrbiterXErr::RequestTimeout
            | OrbiterXErr::UnexpectedStatus(_)
            | OrbiterXErr::ResponseStreamFailed(_)
            | OrbiterXErr::ConnectionFailed(_)
            | OrbiterXErr::InternalServerError
            | OrbiterXErr::InternalAgentDied
            | OrbiterXErr::Io(_)
            | OrbiterXErr::Json(_)
            | OrbiterXErr::TokioJoin(_) => true,
            #[cfg(target_os = "linux")]
            OrbiterXErr::LandlockRuleset(_) | OrbiterXErr::LandlockPathFd(_) => false,
        }
    }

    /// Minimal shim so that existing `e.downcast_ref::<OrbiterXErr>()` checks continue to compile
    ...
    pub fn to_orbiterx_protocol_error(&self) -> OrbiterXErrorInfo {
        match self {
            OrbiterXErr::ContextWindowExceeded => OrbiterXErrorInfo::ContextWindowExceeded,
            OrbiterXErr::SessionBudgetExceeded => OrbiterXErrorInfo::SessionBudgetExceeded,
            OrbiterXErr::UsageLimitReached(_)
            | OrbiterXErr::QuotaExceeded
            | OrbiterXErr::UsageNotIncluded => OrbiterXErrorInfo::UsageLimitExceeded,
            OrbiterXErr::ServerOverloaded => OrbiterXErrorInfo::ServerOverloaded,
            OrbiterXErr::CyberPolicy { .. } => OrbiterXErrorInfo::CyberPolicy,
            OrbiterXErr::RetryLimit(_) => OrbiterXErrorInfo::ResponseTooManyFailedAttempts {
                http_status_code: self.http_status_code_value(),
            },
            OrbiterXErr::ConnectionFailed(_) => OrbiterXErrorInfo::HttpConnectionFailed {
                http_status_code: self.http_status_code_value(),
            },
            OrbiterXErr::ResponseStreamFailed(_) => {
                OrbiterXErrorInfo::ResponseStreamConnectionFailed {
                    http_status_code: self.http_status_code_value(),
                }
            }
            OrbiterXErr::RefreshTokenFailed(_) => OrbiterXErrorInfo::Unauthorized,
            OrbiterXErr::SessionConfiguredNotFirstEvent
            | OrbiterXErr::InternalServerError
            | OrbiterXErr::InternalAgentDied => OrbiterXErrorInfo::InternalServerError,
            OrbiterXErr::UnsupportedOperation(_)
            | OrbiterXErr::ThreadNotFound(_)
            | OrbiterXErr::AgentLimitReached { .. } => OrbiterXErrorInfo::BadRequest,
            OrbiterXErr::Sandbox(_) => OrbiterXErrorInfo::SandboxError,
            _ => OrbiterXErrorInfo::Other,
        }
    }
    ...
    pub fn to_error_event(&self, message_prefix: Option<String>) -> ErrorEvent {
        ...
        ErrorEvent {
            message,
            orbiterx_error_info: Some(self.to_orbiterx_protocol_error()),
        }
    }

    pub fn http_status_code_value(&self) -> Option<u16> {
        let http_status_code = match self {
            OrbiterXErr::RetryLimit(err) => Some(err.status),
            OrbiterXErr::UnexpectedStatus(err) => Some(err.status),
            OrbiterXErr::ConnectionFailed(err) => err.source.status(),
            OrbiterXErr::ResponseStreamFailed(err) => err.source.status(),
            _ => None,
        };
...
impl std::fmt::Display for UsageLimitReachedError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        if let Some(limit_name) = self
            ...
            && !limit_name.eq_ignore_ascii_case("orbiterx")
        {
            return write!(
                f,
        ...
        let message = match self.plan_type.as_ref() {
            Some(PlanType::Known(KnownPlan::Plus)) => format!(
                "You've hit your usage limit. Upgrade to Pro (https://chatgpt.com/explore/pro), visit https://chatgpt.com/orbiterx/settings/usage to purchase more credits{}",
                retry_suffix_after_or(self.resets_at.as_ref())
            ),
            Some(PlanType::Known(
                KnownPlan::Team
            ...
            Some(PlanType::Known(KnownPlan::Free)) | Some(PlanType::Known(KnownPlan::Go)) => {
                format!(
                    "You've hit your usage limit. Upgrade to Plus to continue using OrbiterX (https://chatgpt.com/explore/plus),{}",
                    retry_suffix_after_or(self.resets_at.as_ref())
                )
            }
            Some(PlanType::Known(KnownPlan::Pro | KnownPlan::ProLite)) => format!(
                "You've hit your usage limit. Visit https://chatgpt.com/orbiterx/settings/usage to purchase more credits{}",
                retry_suffix_after_or(self.resets_at.as_ref())
            ),
...
pub fn get_error_message_ui(e: &OrbiterXErr) -> String {
    let message = match e {
        OrbiterXErr::Sandbox(SandboxErr::Denied { output, .. }) => {
        ...
        OrbiterXErr::Sandbox(SandboxErr::Timeout { output }) => {
            format!(
