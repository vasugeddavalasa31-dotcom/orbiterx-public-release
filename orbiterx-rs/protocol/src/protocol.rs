//! Defines the protocol for a OrbiterX session between a client and an agent.
//!
...
pub const USER_MESSAGE_BEGIN: &str = "## My request for OrbiterX:";
...
pub struct ConversationStartParams {
    /// Whether OrbiterX response handoffs are managed through explicit client append calls.
    pub client_managed_handoffs: bool,
    /// Whether to route any remaining transcript tail through OrbiterX when the session ends.
    ...
    /// Sends automatic OrbiterX responses as realtime conversation items instead of handoff appends.
    pub orbiterx_responses_as_items: bool,
    /// Optional prefix added to automatic OrbiterX response items when `orbiterx_responses_as_items` is set.
    pub orbiterx_response_item_prefix: Option<String>,
    /// Selects how automatic OrbiterX handoffs are routed in Frameless Bidi sessions.
    /// Realtime V1 and V2 ignore this setting.
    pub orbiterx_response_handoff_mode: OrbiterXResponseHandoffMode,
    /// Overrides the configured realtime model for this session only.
    pub model: Option<String>,
    /// Selects whether the realtime session should produce text or audio output.
    pub output_modality: RealtimeOutputModality,
    /// Whether to append OrbiterX's startup context to the realtime backend prompt.
...
pub enum Op {
    ...
    /// Request OrbiterX to drop the last N user turns from in-memory context.
    ///
    ...
    /// Request to shut down orbiterx instance.
    Shutdown,
...
/// running the command proposed by OrbiterX.
#[derive(
    Debug,
    Clone,
    Copy,
...
/// privileges of the agent (e.g. `.orbiterx`, `.git`, notably `.git/hooks`) under
...
#[serde(rename_all = "camelCase")]
#[ts(rename_all = "camelCase")]
pub enum OrbiterXResponseHandoffMode {
    #[default]
    Thinking,
    Commentary,
    BemTags,
...
/// OrbiterX errors that we expose to clients.
#[derive(Serialize, Deserialize, Clone, Debug, PartialEq, Eq, JsonSchema, TS)]
#[serde(rename_all = "snake_case")]
#[ts(rename_all = "snake_case")]
pub enum OrbiterXErrorInfo {
...
pub struct ErrorEvent {
    pub message: String,
    #[serde(default)]
    pub orbiterx_error_info: Option<OrbiterXErrorInfo>,
}

impl ErrorEvent {
    /// Whether this error should mark the current turn as failed when replaying history.
    pub fn affects_turn_status(&self) -> bool {
        self.orbiterx_error_info
            .as_ref()
            .is_none_or(OrbiterXErrorInfo::affects_turn_status)
    }
}
...
impl SessionSource {
    ...
    pub fn restriction_product(&self) -> Option<Product> {
        match self {
            ...
            | SessionSource::Exec
            | SessionSource::Mcp
            | SessionSource::Unknown => Some(Product::OrbiterX),
...
pub struct TurnContextItem {
    ...
    // Compatibility-only field written with a default value so older OrbiterX
    ...
    pub fn token_budget(&self) -> usize {
        match self {
            TruncationPolicy::Bytes(bytes) => {
                usize::try_from(orbiterx_utils_string::approx_tokens_from_byte_count(*bytes))
                    .unwrap_or(usize::MAX)
            }
    ...
    pub fn byte_budget(&self) -> usize {
        match self {
            ...
            TruncationPolicy::Tokens(tokens) => {
                orbiterx_utils_string::approx_bytes_for_tokens(*tokens)
            }
        }
    }
}
...
mod tests {
    ...
    #[test]
    fn session_source_restriction_product_defaults_non_subagent_sources_to_orbiterx() {
        assert_eq!(
    ...
    fn session_source_restriction_product_maps_custom_sources_to_products() {
        ...
        assert_eq!(
            SessionSource::Custom("orbiterx".to_string()).restriction_product(),
            Some(Product::OrbiterX)
        );
        assert_eq!(
    ...
    fn session_source_matches_product_restriction() {
        ...
        assert!(
            !SessionSource::Custom("chatgpt".to_string())
                .matches_product_restriction(&[Product::OrbiterX])
        );
        assert!(SessionSource::VSCode.matches_product_restriction(&[Product::OrbiterX]));
        assert!(
    ...
    fn restricted_file_system_policy_treats_root_with_carveouts_as_scoped_access() {
        ...
        let canonical_cwd =
            orbiterx_utils_absolute_path::canonicalize_preserving_symlinks(cwd.path())
    ...
    fn restricted_file_system_policy_derives_effective_paths() {
        ...
        std::fs::create_dir_all(cwd.path().join(".orbiterx")).expect("create .orbiterx");
        let canonical_cwd =
            orbiterx_utils_absolute_path::canonicalize_preserving_symlinks(cwd.path())
                .expect("canonicalize cwd");
        let cwd_absolute =
        ...
        let expected_orbiterx =
            AbsolutePathBuf::from_absolute_path(canonical_cwd.join(".orbiterx"))
                .expect("canonical .orbiterx");
        let policy = FileSystemSandboxPolicy::restricted(vec![
            FileSystemSandboxEntry {
                path: FileSystemPath::Special {
        ...
        assert!(
            writable_roots[0]
                .read_only_subpaths
                .iter()
                .any(|path| path.as_path() == expected_orbiterx.as_path())
        );
    }
        ...
        assert_eq!(
            ...
            vec![
                (
                    canonical_cwd,
                    vec![
                        expected_dot_orbiterx.to_path_buf(),
    ...
    fn generic_error_affects_turn_status() {
        let event = ErrorEvent {
            ...
            orbiterx_error_info: Some(OrbiterXErrorInfo::Other),
        };
        assert!(event.affects_turn_status());
    }
