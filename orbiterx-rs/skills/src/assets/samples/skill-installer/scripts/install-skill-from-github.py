#!/usr/bin/env python3
"""Install a skill from a GitHub repo path into $ORBITERX_HOME/skills."""
...
def _orbiterx_home() -> str:
    return os.environ.get("ORBITERX_HOME", os.path.expanduser("~/.orbiterx"))


def _tmp_root() -> str:
    base = os.path.join(tempfile.gettempdir(), "orbiterx")
    os.makedirs(base, exist_ok=True)
    return base


def _request(url: str) -> bytes:
    return github_request(url, "orbiterx-skill-install")
...
def _default_dest() -> str:
    return os.path.join(_orbiterx_home(), "skills")
