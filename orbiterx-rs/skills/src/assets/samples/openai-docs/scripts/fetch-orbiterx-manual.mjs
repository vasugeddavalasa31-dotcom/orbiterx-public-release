...
const DEFAULT_MANUAL_URL = "https://developers.openai.com/orbiterx/orbiterx-manual.md";
const DEFAULT_CACHE_DIR_NAME = "openai-docs-cache";
const CACHE_FILE_NAME = "orbiterx-manual.md";
const OUTLINE_FILE_NAME = "orbiterx-manual.outline.md";
const HASH_HEADER = "x-content-sha256";
const USER_AGENT = "orbiterx-openai-docs";
...
const tempFilePath = (cacheDir, suffix) =>
  path.join(
    cacheDir,
    `.fetch-orbiterx-manual-${process.pid}-${Date.now()}-${Math.random()
      .toString(16)
      .slice(2)}${suffix}`
  );
...
const outlineMarkdown = (outline) => `# OrbiterX Manual Outline\n\n${outline.text}\n`;
...
const fetchOrbiterXManual = async ({
...
const main = async () => {
  ...
  const { outlineText, status } = await fetchOrbiterXManual(args);
...
export { DEFAULT_MANUAL_URL, fetchOrbiterXManual };
