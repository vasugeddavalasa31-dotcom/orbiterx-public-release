...
Set a stable path to the skill CLI (default `ORBITERX_HOME` is `~/.orbiterx`):

```
export ORBITERX_HOME="${ORBITERX_HOME:-$HOME/.orbiterx}"
export IMAGE_GEN="$ORBITERX_HOME/skills/.system/imagegen/scripts/image_gen.py"
```
...
- Network/sandbox notes for fallback CLI mode: `references/orbiterx-network.md`
- Built-in-first transparent image workflow: `SKILL.md` and `$ORBITERX_HOME/skills/.system/imagegen/scripts/remove_chroma_key.py`
