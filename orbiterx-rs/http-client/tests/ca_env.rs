...
//! detailed explanation of what "hermetic" means here lives in `orbiterx_http_client::custom_ca`; these
...
const ORBITERX_CA_CERT_ENV: &str = "ORBITERX_CA_CERTIFICATE";
const PROBE_PROXY_ENV: &str = "ORBITERX_CUSTOM_CA_PROBE_PROXY";
const PROBE_TLS13_ENV: &str = "ORBITERX_CUSTOM_CA_PROBE_TLS13";
const PROBE_URL_ENV: &str = "ORBITERX_CUSTOM_CA_PROBE_URL";
...
fn probe_command() -> Command {
    ...
    cmd.env_remove(ORBITERX_CA_CERT_ENV);
...
fn spawn_tls13_test_server() -> Tls13TestServer {
    orbiterx_utils_rustls_provider::ensure_rustls_crypto_provider();
...
fn generate_tls13_material() -> Tls13Material {
    ...
    ca_distinguished_name.push(DnType::CommonName, "orbiterx test CA");
...
#[test]
fn uses_orbiterx_ca_cert_env() {
    let temp_dir = TempDir::new().expect("tempdir");
    let cert_path = write_cert_file(&temp_dir, "ca.pem", TEST_CERT_1);

    let output = run_probe(&[(ORBITERX_CA_CERT_ENV, cert_path.as_path())]);
...
#[test]
fn prefers_orbiterx_ca_cert_over_ssl_cert_file() {
    let temp_dir = TempDir::new().expect("tempdir");
    let cert_path = write_cert_file(&temp_dir, "ca.pem", TEST_CERT_1);
    let bad_path = write_cert_file(&temp_dir, "bad.pem", "");

    let output = run_probe(&[
        (ORBITERX_CA_CERT_ENV, cert_path.as_path()),
...
fn rejects_empty_pem_file_with_hint() {
    ...
    assert!(stderr.contains("ORBITERX_CA_CERTIFICATE"));
