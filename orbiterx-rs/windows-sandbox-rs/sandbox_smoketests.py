...
# Run a suite of smoke tests against the Windows sandbox via the OrbiterX CLI
...
def _resolve_orbiterx_cmd() -> List[str]:
    """Resolve the OrbiterX CLI to invoke `orbiterx sandbox windows`.

    Prefer local builds (debug first), then fall back to PATH.
    Returns the argv prefix to run OrbiterX.
    """
    ...
    candidates = [
        ws_root / "target" / "debug" / "orbiterx.exe",
        ws_root / "target" / "release" / "orbiterx.exe",
    ]
    if cargo_target:
        cargo_base = Path(cargo_target)
        candidates.extend([
            cargo_base / "debug" / "orbiterx.exe",
            cargo_base / "release" / "orbiterx.exe",
        ])
    ...
    if shutil.which("orbiterx"):
        return ["orbiterx"]

    raise FileNotFoundError(
        "OrbiterX CLI not found. Build it first, e.g.\n"
        "  cargo build -p orbiterx-cli --release\n"
        "or for debug:\n"
        "  cargo build -p orbiterx-cli\n"
    )

ORBITERX_CMD = _resolve_orbiterx_cmd()
print(ORBITERX_CMD)
TIMEOUT_SEC = 20
...
def run_sbx(
    ...
    # Map policy to orbiterx CLI overrides.
    ...
    argv = [*ORBITERX_CMD, "sandbox", "windows", *policy_flags, *overrides, "--", *cmd_argv]
    print(cmd_argv)
...
def main() -> int:
    ...
    if have("curl"):
        with start_loopback_proxy_fixture() as (target_port, proxy_port):
            ...
            rc_direct, _out_direct, err_direct = run_sbx(
                ...
                direct_cmd,
                WS_ROOT,
                env_extra={"ORBITERX_HOME": str(proxy_home)},
            )
    ...
    rc, out, err = run_sbx("workspace-write", ["cmd", "/c", "echo hi > \\\\.\\pipe\\orbiterx_testpipe"], WS_ROOT)
    ...
    # 34. WS: policy tamper (.orbiterx artifacts) denied
    orbiterx_home = Path(os.environ["USERPROFILE"]) / ".orbiterx"
    cap_sid_target = orbiterx_home / "cap_sid"
    rc, out, err = run_sbx(
        "workspace-write",
        ["cmd", "/c", f"echo tamper > \"{cap_sid_target}\""],
        WS_ROOT,
    )
    rc2, out2, err2 = run_sbx("workspace-write", ["cmd", "/c", "echo tamper > .orbiterx\\policy.json"], WS_ROOT)
    add("WS: .orbiterx cap_sid tamper denied", rc != 0, f"rc={rc}, err={err}")
    add("WS: .orbiterx policy tamper denied", rc2 != 0, f"rc={rc2}, err={err2}")
    ...
    if make_symlink(fake_root, Path("C:/")):
        rc, out, err = run_sbx("workspace-write", ["cmd", "/c", "echo owned > orbiterx_escape.txt"], fake_root)
    ...
    rc, out, err = run_sbx(
        ...
        [
            ...
            "-NoProfile",
            "-Command",
            "Start-Process 'https://orbiterx-invalid.local/smoke'",
        ],
        WS_ROOT,
    )
    add(
