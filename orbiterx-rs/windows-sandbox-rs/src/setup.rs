...
pub const OFFLINE_USERNAME: &str = "OrbiterXSandboxOffline";
pub const ONLINE_USERNAME: &str = "OrbiterXSandboxOnline";
const ERROR_CANCELLED: u32 = 1223;
const SECURITY_BUILTIN_DOMAIN_RID: u32 = 0x0000_0020;
const DOMAIN_ALIAS_RID_ADMINS: u32 = 0x0000_0220;
const SETUP_EXE_FILENAME: &str = "orbiterx-windows-sandbox-setup.exe";
...
pub fn sandbox_dir(orbiterx_home: &Path) -> PathBuf {
    orbiterx_home.join(".sandbox")
}

pub fn sandbox_bin_dir(orbiterx_home: &Path) -> PathBuf {
    orbiterx_home.join(".sandbox-bin")
}

pub fn sandbox_secrets_dir(orbiterx_home: &Path) -> PathBuf {
    orbiterx_home.join(".sandbox-secrets")
}

pub fn setup_marker_path(orbiterx_home: &Path) -> PathBuf {
    sandbox_dir(orbiterx_home).join("setup_marker.json")
}

pub fn sandbox_users_path(orbiterx_home: &Path) -> PathBuf {
    sandbox_secrets_dir(orbiterx_home).join("sandbox_users.json")
}
...
pub fn run_setup_refresh_with_extra_read_roots(
    ...
    let mut read_roots = gather_read_roots(command_cwd, &permissions, env_map, orbiterx_home);
...
fn run_setup_refresh_inner(
    ...
    let payload = ElevationPayload {
        ...
        orbiterx_home: request.orbiterx_home.to_path_buf(),
    ...
    run_setup_singleflight(b64.clone(), || {
        run_setup_refresh_payload(&b64, request.orbiterx_home)
    })
}

fn run_setup_refresh_payload(b64: &str, orbiterx_home: &Path) -> Result<()> {
    let exe = find_setup_exe();
    let sbx_dir = sandbox_dir(orbiterx_home);
    let log_path = current_log_file_path(&sbx_dir);
    let cleared_report = match clear_setup_error_report(orbiterx_home) {
        Ok(()) => true,
        Err(err) => {
            log_note(
    ...
    let cwd = std::env::current_dir().unwrap_or_else(|_| orbiterx_home.to_path_buf());
    log_note(
        &format!(
    ...
    if let Err(err) = clear_setup_error_report(orbiterx_home) {
        log_note(
...
fn gather_helper_read_roots(orbiterx_home: &Path) -> Vec<PathBuf> {
    let helper_dir = helper_bin_dir(orbiterx_home);
...
fn gather_full_read_roots_for_permissions(
    ...
    let mut roots = gather_helper_read_roots(orbiterx_home);
    roots.extend(
...
pub(crate) fn effective_write_roots_for_permissions(
    ...
    filter_sensitive_write_roots(write_roots, orbiterx_home)
}
...
struct ElevationPayload {
    ...
    otel: Option<orbiterx_otel::StatsigMetricsSettings>,
    real_user: String,
    mode: SetupMode,
    #[serde(default)]
    refresh_only: bool,
...
const ALLOW_LOCAL_BINDING_ENV_KEY: &str = "ORBITERX_NETWORK_ALLOW_LOCAL_BINDING";
...
fn report_helper_failure(
    ...
    match read_setup_error_report(orbiterx_home) {
...
fn verify_setup_completed(orbiterx_home: &Path) -> Result<()> {
    if sandbox_setup_is_complete(orbiterx_home) {
        Ok(())
    } else {
        Err(failure(
...
fn run_setup_exe(
    ...
    run_setup_singleflight(payload_b64.clone(), || {
        run_setup_exe_payload(&payload_b64, needs_elevation, orbiterx_home)
    })
}
...
fn run_setup_exe_payload(
    ...
    let cleared_report = match clear_setup_error_report(orbiterx_home) {
        ...
        Err(err) => {
            log_note(
                ...
                Some(&sandbox_dir(orbiterx_home)),
            );
            false
        }
    };
    ...
    if !needs_elevation {
        ...
        verify_setup_completed(orbiterx_home)?;
        if let Err(err) = clear_setup_error_report(orbiterx_home) {
            log_note(
                &format!(
...
fn run_elevated_setup_inner(
    ...
    let sbx_dir = sandbox_dir(request.orbiterx_home);
    ...
    let payload = ElevationPayload {
        ...
        allow_local_binding: offline_proxy_settings.allow_local_binding,
        real_user: std::env::var("USERNAME").unwrap_or_else(|_| "Administrators".to_string()),
        otel: orbiterx_otel::global_statsig_metrics_settings(),
        mode: SetupMode::Full,
        refresh_only: false,
    };
    ...
    run_setup_exe(&payload, needs_elevation, request.orbiterx_home)
}

pub fn run_elevated_provisioning_setup(orbiterx_home: &Path, real_user: &str) -> Result<()> {
    let sbx_dir = sandbox_dir(orbiterx_home);
    std::fs::create_dir_all(&sbx_dir).map_err(|err| {
        failure(
            SetupErrorCode::OrchestratorSandboxDirCreateFailed,
    ...
    let payload = ElevationPayload {
        ...
        command_cwd: orbiterx_home.to_path_buf(),
        read_roots: Vec::new(),
        write_roots: Vec::new(),
    ...
    run_setup_exe(&payload, /*needs_elevation*/ false, orbiterx_home)
}
...
fn build_payload_roots(
    ...
    let mut read_roots = if let Some(roots) = overrides.read_roots.as_deref() {
        ...
        let mut read_roots = gather_helper_read_roots(request.orbiterx_home);
...
fn filter_sensitive_write_roots(mut roots: Vec<PathBuf>, orbiterx_home: &Path) -> Vec<PathBuf> {
    // Never grant capability write access to ORBITERX_HOME or anything under ORBITERX_HOME/.sandbox,
    // ORBITERX_HOME/.sandbox-bin, or ORBITERX_HOME/.sandbox-secrets. These locations contain sandbox
    // control/state and helper binaries and must remain tamper-resistant.
    let orbiterx_home_key = canonical_path_key(orbiterx_home);
    let sbx_dir_key = canonical_path_key(&sandbox_dir(orbiterx_home));
    let sbx_dir_prefix = format!("{}/", sbx_dir_key.trim_end_matches('/'));
    let sbx_bin_dir_key = canonical_path_key(&sandbox_bin_dir(orbiterx_home));
    let sbx_bin_dir_prefix = format!("{}/", sbx_bin_dir_key.trim_end_matches('/'));
    let secrets_dir_key = canonical_path_key(&sandbox_secrets_dir(orbiterx_home));
    let secrets_dir_prefix = format!("{}/", secrets_dir_key.trim_end_matches('/'));

    roots.retain(|root| {
        let key = canonical_path_key(root);
        key != orbiterx_home_key
            && key != sbx_dir_key
...
mod tests {
    ...
    fn setup_completion_requires_ready_artifacts() {
        let orbiterx_home = TempDir::new().expect("tempdir");
        let err = verify_setup_completed(orbiterx_home.path())
    ...
    fn report_helper_failure_uses_setup_error_report_when_clear_succeeded() {
        let tmp = TempDir::new().expect("tempdir");
        let orbiterx_home = tmp.path().join("orbiterx-home");
    ...
    fn setup_exe_lookup_checks_package_resource_dir_for_bin_exe() {
        ...
        let exe = bin_dir.join("orbiterx.exe");
        let setup_exe = resources_dir.join("orbiterx-windows-sandbox-setup.exe");
        fs::write(&exe, b"orbiterx").expect("write exe");
    ...
    fn offline_proxy_settings_ignore_proxy_env_when_online_identity_selected() {
        ...
        env.insert(
            "ORBITERX_NETWORK_ALLOW_LOCAL_BINDING".to_string(),
            "1".to_string(),
        );
    ...
    #[test]
    fn expanded_write_roots_still_drop_protected_orbiterx_home() {
        let tmp = TempDir::new().expect("tempdir");
        let user_profile = tmp.path().join("user-profile");
        let orbiterx_home = user_profile.join("OrbiterXHome");
        let documents = user_profile.join("Documents");
        fs::create_dir_all(&orbiterx_home).expect("create orbiterx home");
        fs::create_dir_all(&documents).expect("create documents");
        ...
        let roots = super::filter_sensitive_write_roots(roots, &orbiterx_home);
    ...
    fn gather_read_roots_includes_helper_bin_dir() {
        ...
        let roots = gather_read_roots(&command_cwd, &permissions, &HashMap::new(), &orbiterx_home);
        let expected =
            dunce::canonicalize(helper_bin_dir(&orbiterx_home)).expect("canonical helper dir");
    ...
    fn effective_write_roots_match_payload_filtering_for_overrides() {
        ...
        let sandbox_root = super::sandbox_dir(&orbiterx_home);
        fs::create_dir_all(&orbiterx_home).expect("create orbiterx home");
        fs::create_dir_all(&command_cwd).expect("create workspace");
        ...
        let forbidden_orbiterx_home =
            dunce::canonicalize(&orbiterx_home).expect("canonical orbiterx home");
        ...
        assert!(!effective_write_roots.contains(&forbidden_orbiterx_home));
    ...
    fn payload_deny_write_paths_merge_explicit_and_protected_children() {
        ...
        let extra_orbiterx = extra_write_root.join(".orbiterx");
        let explicit_deny = tmp.path().join("explicit-deny");
        fs::create_dir_all(&command_git).expect("create command .git");
        fs::create_dir_all(&extra_orbiterx).expect("create extra .orbiterx");
        let writable_roots = vec![
        ...
        assert_eq!(
            [
                ...
                dunce::canonicalize(&extra_orbiterx).expect("canonical extra .orbiterx"),
                explicit_deny,
            ]
            .into_iter()
