...
pub(super) fn ensure_orbiterx_app_runtime_paths_readable(
...
fn runtime_paths(local_app_data: Option<PathBuf>, user_profile: Option<PathBuf>) -> Vec<PathBuf> {
    ...
    if let Some(local_app_data) = local_app_data {
        let orbiterx_root = local_app_data.join("OpenAI").join("OrbiterX");
        runtime_paths.extend([orbiterx_root.join("bin"), orbiterx_root.join("runtimes")]);
    }
    // The managed primary runtime is installed outside the LocalAppData runtime roots.
    if let Some(user_profile) = user_profile {
        runtime_paths.push(user_profile.join(".cache").join("orbiterx-runtimes"));
    }
