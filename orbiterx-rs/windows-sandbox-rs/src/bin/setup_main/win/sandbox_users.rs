...
use orbiterx_windows_sandbox::dpapi_protect;
...
pub const SANDBOX_USERS_GROUP: &str = "OrbiterXSandboxUsers";
const SANDBOX_USERS_GROUP_COMMENT: &str = "OrbiterX sandbox internal group (managed)";
...
fn write_secrets(
    ...
    let secrets_dir = sandbox_secrets_dir(orbiterx_home);
...
pub(super) fn prepare_setup_marker(orbiterx_home: &Path, real_user: &str) -> Result<()> {
    let marker_path = sandbox_dir(orbiterx_home).join("setup_marker.json");
