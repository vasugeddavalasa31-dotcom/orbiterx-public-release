...
use anyhow::Context;
use anyhow::Result;
use orbiterx_windows_sandbox::ConsoleMode;
use orbiterx_windows_sandbox::ErrorPayload;
use orbiterx_windows_sandbox::ErrorStage;
use orbiterx_windows_sandbox::ExitPayload;
use orbiterx_windows_sandbox::FramedMessage;
use orbiterx_windows_sandbox::IPC_PROTOCOL_VERSION;
use orbiterx_windows_sandbox::LocalSid;
use orbiterx_windows_sandbox::Message;
use orbiterx_windows_sandbox::OutputPayload;
use orbiterx_windows_sandbox::OutputStream;
use orbiterx_windows_sandbox::PipeSpawnHandles;
use orbiterx_windows_sandbox::ResizePayload;
use orbiterx_windows_sandbox::SpawnReady;
use orbiterx_windows_sandbox::SpawnRequest;
use orbiterx_windows_sandbox::StderrMode;
use orbiterx_windows_sandbox::StdinMode;
use orbiterx_windows_sandbox::WindowsSandboxTokenMode;
use orbiterx_windows_sandbox::allow_null_device;
use orbiterx_windows_sandbox::create_readonly_token_with_caps_and_user_from;
use orbiterx_windows_sandbox::create_workspace_write_token_with_caps_and_user_from;
use orbiterx_windows_sandbox::decode_bytes;
use orbiterx_windows_sandbox::encode_bytes;
use orbiterx_windows_sandbox::get_current_token_for_restriction;
use orbiterx_windows_sandbox::hide_current_user_profile_dir;
use orbiterx_windows_sandbox::log_note;
use orbiterx_windows_sandbox::read_frame;
use orbiterx_windows_sandbox::read_handle_loop;
use orbiterx_windows_sandbox::spawn_process_with_pipes;
use orbiterx_windows_sandbox::to_wide;
use orbiterx_windows_sandbox::token_mode_for_permission_profile;
use orbiterx_windows_sandbox::write_frame;
use std::ffi::OsStr;
use std::fs::File;
...
// Kept in sync with orbiterx_exec_server::ORBITERX_FS_HELPER_ARG1 without introducing
// a dependency cycle.
const FS_HELPER_ARG: &str = "--orbiterx-run-as-fs-helper";
const READ_ACL_MUTEX_NAME: &str = "Local\\OrbiterXSandboxReadAcl";
...
struct IpcSpawnedProcess {
    ...
    conpty_owner: Option<orbiterx_windows_sandbox::ConptyInstance>,
...
fn spawn_ipc_process(req: &SpawnRequest) -> Result<IpcSpawnedProcess> {
    let log_dir = req.orbiterx_home.clone();
    hide_current_user_profile_dir(req.orbiterx_home.as_path());
    ...
    let (pi, stdout_handle, stderr_handle, stdin_handle) = if req.tty {
        let (pi, mut conpty) = orbiterx_windows_sandbox::spawn_conpty_process_as_user(
            h_token.raw(),
            &req.command,
            &effective_cwd,
            &req.env,
