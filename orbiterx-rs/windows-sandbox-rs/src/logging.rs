...
pub fn current_log_file_path_for_orbiterx_home(orbiterx_home: &Path) -> PathBuf {
    current_log_file_path(&crate::sandbox_dir(orbiterx_home))
}
...
mod tests {
    ...
    #[test]
    fn current_log_file_path_for_orbiterx_home_uses_sandbox_dir() {
        let orbiterx_home = Path::new("orbiterx-home");

        assert_eq!(
            current_log_file_path_for_orbiterx_home(orbiterx_home),
            current_log_file_path(&orbiterx_home.join(".sandbox"))
        );
    }
}
