//! Internal `orbiterx.exe --run-as-windows-sandbox` wrapper.
//!
...
pub const ORBITERX_WINDOWS_SANDBOX_ARG1: &str = "--run-as-windows-sandbox";

const COMMAND_CWD_FLAG: &str = "--command-cwd";
const ORBITERX_HOME_FLAG: &str = "--orbiterx-home";
...
pub fn create_windows_sandbox_command_args_for_permission_profile(
    ...
    let mut args = vec![
        ORBITERX_WINDOWS_SANDBOX_ARG1.to_string(),
        ORBITERX_HOME_FLAG.to_string(),
        orbiterx_home.to_string_lossy().into_owned(),
...
async fn run_windows_sandbox_wrapper_request(request: WindowsSandboxWrapperRequest) -> Result<i32> {
    ...
    let spawned =
        crate::spawn_windows_sandbox_session_for_level(crate::WindowsSandboxSessionRequest {
            ...
            orbiterx_home: request.orbiterx_home.as_path(),
...
fn parse_windows_sandbox_wrapper_args(args: Vec<String>) -> Result<WindowsSandboxWrapperRequest> {
    ...
    while let Some(arg) = args.next() {
        match arg.as_str() {
            ORBITERX_HOME_FLAG => {
                orbiterx_home = Some(PathBuf::from(next_flag_value(&mut args, &arg)?))
            }
            COMMAND_CWD_FLAG => {
    ...
    let orbiterx_home =
        orbiterx_home.ok_or_else(|| anyhow!("missing required {ORBITERX_HOME_FLAG}"))?;
    if !orbiterx_home.is_absolute() {
        bail!(
            "{ORBITERX_HOME_FLAG} must be absolute: {}",
