...
pub fn find_runner_exe(orbiterx_home: &Path, log_dir: Option<&Path>) -> PathBuf {
    resolve_helper_for_launch(HelperExecutable::CommandRunner, orbiterx_home, log_dir)
}
...
pub fn pipe_pair() -> (String, String) {
    ...
    let base = format!(r"\\.\pipe\orbiterx-runner-{nonce:x}");
