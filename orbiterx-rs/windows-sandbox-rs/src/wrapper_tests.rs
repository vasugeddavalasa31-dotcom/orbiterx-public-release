...
use super::ORBITERX_HOME_FLAG;
use super::ORBITERX_WINDOWS_SANDBOX_ARG1;
...
fn windows_wrapper_args_round_trip() {
    ...
    let args = create_windows_sandbox_command_args_for_permission_profile(
        vec![
            "orbiterx.exe".to_string(),
            "--orbiterx-run-as-fs-helper".to_string(),
        ],
        &command_cwd,
        ...
        Path::new(r"C:\Users\me\.orbiterx"),
    );

    assert_eq!(args[0], ORBITERX_WINDOWS_SANDBOX_ARG1);
    assert!(args.contains(&ORBITERX_HOME_FLAG.to_string()));
    ...
    assert_eq!(
        parsed.command,
        vec!["orbiterx.exe", "--orbiterx-run-as-fs-helper"]
    );
