...
mod tests {
    ...
    #[test]
    fn denies_orbiterx_and_agents_inside_writable_root() {
        let tmp = TempDir::new().expect("tempdir");
        let command_cwd = tmp.path().join("workspace");
        let orbiterx_dir = command_cwd.join(".orbiterx");
        let agents_dir = command_cwd.join(".agents");
        let _ = fs::create_dir_all(&orbiterx_dir);
        ...
        let expected_deny: HashSet<PathBuf> = [
            dunce::canonicalize(&orbiterx_dir).unwrap(),
