...
use orbiterx_login::AuthEnvTelemetry;
use orbiterx_login::AuthManager;
use orbiterx_login::OrbiterXAuth;
use orbiterx_login::collect_auth_env_telemetry;
use orbiterx_login::default_client::build_default_reqwest_client_for_route_async;
use orbiterx_model_provider_info::ModelProviderInfo;
use orbiterx_models_manager::manager::ModelsEndpointClient;
use orbiterx_models_manager::manager::ModelsEndpointFuture;
...
impl OpenAiModelsEndpoint {
    ...
    async fn auth(&self) -> Option<OrbiterXAuth> {
    ...
    async fn list_models(
        ...
        let _timer = orbiterx_otel::start_global_timer(
            "orbiterx.remote_models.fetch_update.duration_ms",
            &[],
        );
        let auth = self.auth().await;
        let auth_mode = auth.as_ref().map(OrbiterXAuth::auth_mode);
        ...
        let agent_identity_telemetry =
            if let Some(OrbiterXAuth::AgentIdentity(auth)) = auth.as_ref() {
        ...
        .await
        .map_err(|_| OrbiterXErr::Timeout)?
    }

    fn auth_env(&self) -> AuthEnvTelemetry {
        let orbiterx_api_key_env_enabled = self
            .auth_manager
            .as_ref()
            .is_some_and(|auth_manager| auth_manager.orbiterx_api_key_env_enabled());
        collect_auth_env_telemetry(&self.provider_info, orbiterx_api_key_env_enabled)
    }
}
...
impl ModelsEndpointClient for OpenAiModelsEndpoint {
    ...
    fn uses_orbiterx_backend(&self) -> ModelsEndpointFuture<'_, bool> {
        Box::pin(OpenAiModelsEndpoint::uses_orbiterx_backend(self))
    }
...
impl RequestTelemetry for ModelsRequestTelemetry {
    fn on_request(
        ...
        tracing::event!(
            target: "orbiterx_otel.log_only",
            tracing::Level::INFO,
            event.name = "orbiterx.api_request",
            ...
            auth.env_orbiterx_api_key_present = self.auth_env.orbiterx_api_key_env_present,
            auth.env_orbiterx_api_key_enabled = self.auth_env.orbiterx_api_key_env_enabled,
        ...
        tracing::event!(
            target: "orbiterx_otel.trace_safe",
            tracing::Level::INFO,
            event.name = "orbiterx.api_request",
            duration_ms = %duration.as_millis(),
            http.response.status_code = status,
