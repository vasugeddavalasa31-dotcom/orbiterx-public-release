...
use http::HeaderMap;
use orbiterx_api::AuthError;
use orbiterx_api::AuthProvider;
use orbiterx_api::SharedAuthProvider;
use orbiterx_aws_auth::AwsAuthContext;
use orbiterx_aws_auth::AwsAuthError;
use orbiterx_aws_auth::AwsRequestToSign;
use orbiterx_http_client::Request;
use orbiterx_http_client::RequestBody;
use orbiterx_http_client::RequestCompression;
...
pub(super) async fn resolve_auth_method(
    ...
    let context = AwsAuthContext::load(config)
        .await
        .map_err(aws_auth_error_to_orbiterx_error)?;
...
fn aws_auth_error_to_orbiterx_error(error: AwsAuthError) -> OrbiterXErr {
    OrbiterXErr::Fatal(format!("failed to resolve Amazon Bedrock auth: {error}"))
}
...
fn remove_headers_not_preserved_by_bedrock_mantle(headers: &mut HeaderMap) {
    ...
    // richer OrbiterX agent requests fail even though raw Responses requests work.
...
impl AuthProvider for BedrockMantleSigV4AuthProvider {
    ...
    fn apply_auth(&self, request: Request) -> orbiterx_api::AuthProviderFuture<'_> {
