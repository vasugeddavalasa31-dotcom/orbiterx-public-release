...
impl AmazonBedrockModelProvider {
    ...
    fn managed_auth(&self) -> Option<BedrockApiKeyAuth> {
        self.auth_manager
            ...
            .and_then(|auth| match auth {
                OrbiterXAuth::BedrockApiKey(auth) => Some(auth),
                ...
                | OrbiterXAuth::PersonalAccessToken(_) => None,
            })
    }
    ...
    async fn auth(&self) -> Option<OrbiterXAuth> {
        if self.info.has_command_auth() {
            ...
            self.managed_auth().map(OrbiterXAuth::BedrockApiKey)
        }
    }
    ...
    fn account_state(&self) -> ProviderAccountResult {
        Ok(ProviderAccountState {
            account: Some(ProviderAccount::AmazonBedrock {
                uses_orbiterx_managed_credentials: self.managed_auth().is_some(),
            }),
...
mod tests {
    ...
    async fn managed_auth_takes_precedence_over_aws_auth() {
        ...
        let auth_manager =
            AuthManager::from_auth_for_testing(OrbiterXAuth::BedrockApiKey(managed_auth.clone()));
        ...
        assert_eq!(
            provider.auth().await,
            Some(OrbiterXAuth::BedrockApiKey(managed_auth))
        );
        assert_eq!(
            provider.account_state(),
            Ok(ProviderAccountState {
