...
impl AuthProvider for AuthManagerAuthProvider {
    fn add_auth_headers(&self, headers: &mut HeaderMap) {
        let Some(auth) = self
            .auth_manager
            .auth_cached()
            .filter(OrbiterXAuth::uses_orbiterx_backend)
        else {
            return;
        };
...
pub(crate) fn resolve_provider_auth(
    ...
    provider: &ModelProviderInfo,
) -> orbiterx_protocol::error::Result<SharedAuthProvider> {
    if matches!(auth, Some(OrbiterXAuth::BedrockApiKey(_))) {
        return Err(OrbiterXErr::UnsupportedOperation(
...
) -> orbiterx_protocol::error::Result<ResolvedProviderAuth> {
    let ProviderAuthScope {
        agent_identity_policy,
        session_source,
        agent_identity_session_fallback,
    } = scope;
    if let Some(OrbiterXAuth::AgentIdentity(agent_identity_auth)) = auth {
...
fn should_bootstrap_chatgpt_agent_identity(
    ...
    agent_identity_policy == AgentIdentityAuthPolicy::ChatGptAuth
        && matches!(auth, Some(OrbiterXAuth::Chatgpt(_)))
}

fn bearer_auth_for_provider(
    provider: &ModelProviderInfo,
) -> orbiterx_protocol::error::Result<Option<BearerAuthProvider>> {
...
/// Builds request-header auth for a first-party OrbiterX auth snapshot.
pub fn auth_provider_from_auth(auth: &OrbiterXAuth) -> SharedAuthProvider {
    match auth {
        OrbiterXAuth::AgentIdentity(auth) => {
            Arc::new(AgentIdentityAuthProvider { auth: auth.clone() })
        }
        OrbiterXAuth::Headers(auth) => Arc::new(HeaderAuthProvider { auth: auth.clone() }),
        OrbiterXAuth::BedrockApiKey(_) => unreachable!("{BEDROCK_API_KEY_UNSUPPORTED_MESSAGE}"),
        OrbiterXAuth::ApiKey(_)
        | OrbiterXAuth::Chatgpt(_)
        | OrbiterXAuth::ChatgptAuthTokens(_)
        | OrbiterXAuth::PersonalAccessToken(_) => Arc::new(BearerAuthProvider {
...
mod tests {
    ...
    use orbiterx_agent_identity::generate_agent_key_material;
    use orbiterx_login::AuthCredentialsStoreMode;
    use orbiterx_login::AuthKeyringBackendKind;
    use orbiterx_login::auth::AgentIdentityAuthRecord;
    use orbiterx_login::auth::BedrockApiKeyAuth;
    use orbiterx_login::auth::login_with_chatgpt_auth_tokens;
    use orbiterx_model_provider_info::WireApi;
    use orbiterx_model_provider_info::create_oss_provider_with_base_url;
    ...
    static NEXT_ORBITERX_HOME_ID: AtomicUsize = AtomicUsize::new(0);
    ...
    fn test_orbiterx_home() -> PathBuf {
        let id = NEXT_ORBITERX_HOME_ID.fetch_add(1, Ordering::Relaxed);
        let path = std::env::temp_dir().join(format!(
            "orbiterx-model-provider-agent-identity-{pid}-{id}",
            pid = std::process::id()
        ));
        let _ = std::fs::remove_dir_all(&path);
        std::fs::create_dir_all(&path).expect("create temp orbiterx home");
        path
    }

    fn write_chatgpt_auth_json(orbiterx_home: &Path) {
        let auth_json = json!({
            "tokens": {
    ...
    async fn chatgpt_auth_manager(
    ...
    ) -> (PathBuf, Arc<AuthManager>, OrbiterXAuth) {
        let orbiterx_home = test_orbiterx_home();
        write_chatgpt_auth_json(&orbiterx_home);
        ...
        (orbiterx_home, auth_manager, auth)
    }
    ...
    fn header_auth_adds_predefined_headers() {
        ...
        let auth = OrbiterXAuth::Headers(AuthHeaders::new(expected.clone()));
    ...
    fn openai_provider_rejects_bedrock_api_key_auth() {
        ...
        let auth = OrbiterXAuth::BedrockApiKey(BedrockApiKeyAuth {
            api_key: "bedrock-api-key-test".to_string(),
            region: "us-east-1".to_string(),
        });

        match resolve_provider_auth(Some(&auth), &provider) {
            Err(OrbiterXErr::UnsupportedOperation(message)) => {
    ...
    async fn first_party_run_scope_uses_agent_assertion_and_exposes_telemetry() {
        let auth = OrbiterXAuth::AgentIdentity(
    ...
    async fn agent_identity_auth_provider_preserves_account_routing_headers() {
        ...
        let provider = auth_provider_from_auth(&OrbiterXAuth::AgentIdentity(auth));
    ...
    async fn chatgpt_bootstrap_unavailable_uses_session_bearer_fallback() {
        ...
        .await;
        let (_orbiterx_home, auth_manager, auth) = chatgpt_auth_manager(server.uri()).await;
        let provider = ModelProviderInfo::create_openai_provider(/*base_url*/ None);
