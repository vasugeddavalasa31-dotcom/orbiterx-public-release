...
use orbiterx_apply_patch::ORBITERX_CORE_APPLY_PATCH_ARG1;
use orbiterx_exec_server::ORBITERX_FS_HELPER_ARG1;
...
use orbiterx_windows_sandbox::ORBITERX_WINDOWS_SANDBOX_ARG1;
#[cfg(unix)]
...
#[cfg(unix)]
const EXECVE_WRAPPER_ARG0: &str = "orbiterx-execve-wrapper";
const LOCK_FILENAME: &str = ".lock";
const TOKIO_WORKER_STACK_SIZE_BYTES: usize = 16 * 1024 * 1024;

#[derive(Clone, Debug, Default, Eq, PartialEq)]
pub struct Arg0DispatchPaths {
    /// Stable path to the current OrbiterX executable for child re-execs.
    ///
    ...
    /// instead of the real OrbiterX CLI.
...
pub fn arg0_dispatch() -> Option<Arg0PathEntryGuard> {
    ...
    if exe_name == EXECVE_WRAPPER_ARG0 {
        ...
        let exit_code = runtime
            .block_on(orbiterx_shell_escalation::run_shell_escalation_execve_wrapper(file, argv));
        match exit_code {
    ...
    if exe_name == ORBITERX_LINUX_SANDBOX_ARG0 {
        // Safety: [`run_main`] never returns.
        orbiterx_linux_sandbox::run_main();
    } else if exe_name == APPLY_PATCH_ARG0 || exe_name == MISSPELLED_APPLY_PATCH_ARG0 {
        orbiterx_apply_patch::main();
    }

    let argv1 = args.next().unwrap_or_default();
    if argv1 == ORBITERX_FS_HELPER_ARG1 {
        orbiterx_exec_server::run_fs_helper_main();
    }
    #[cfg(target_os = "windows")]
    if argv1 == ORBITERX_WINDOWS_SANDBOX_ARG1 {
        orbiterx_windows_sandbox::run_windows_sandbox_wrapper_main();
    }
    if argv1 == ORBITERX_CORE_APPLY_PATCH_ARG1 {
        let patch_arg = args.next().and_then(|s| s.to_str().map(str::to_owned));
        let exit_code = match patch_arg {
            Some(patch_arg) => {
                let mut stdout = std::io::stdout();
                let mut stderr = std::io::stderr();
                let cwd = match orbiterx_utils_absolute_path::AbsolutePathBuf::current_dir() {
                    Ok(cwd) => cwd,
                    Err(_) => std::process::exit(1),
                };
                let runtime = match tokio::runtime::Builder::new_current_thread()
                ...
                let cwd = cwd.into();
                match runtime.block_on(orbiterx_apply_patch::apply_patch(
                    &patch_arg,
                    &cwd,
                    &mut stdout,
                    &mut stderr,
                    orbiterx_exec_server::LOCAL_FS.as_ref(),
                    /*sandbox*/ None,
                )) {
                    Ok(_) => 0,
                    Err(_) => 1,
            ...
            None => {
                eprintln!(
                    "Error: {ORBITERX_CORE_APPLY_PATCH_ARG1} requires a UTF-8 PATCH argument."
                );
                1
            }
        };
    ...
    let (path_entry_guard, updated_path_env_var) = prepare_path_env_var_with_aliases(
        ...
        std::env::var_os("PATH"),
        prepare_path_entry_for_orbiterx_aliases,
    );
...
fn prepare_path_env_var_with_aliases(
    ...
    match prepare_aliases(path_for_aliases) {
        ...
        Err(err) => {
            // It is possible that OrbiterX will proceed successfully even if
...
/// While we want to deploy the OrbiterX CLI as a single executable for simplicity,
...
/// `orbiterx-linux-sandbox` we *directly* execute
/// [`orbiterx_linux_sandbox::run_main`] (which never returns). Otherwise we:
///
/// 1.  Load `.env` values from `~/.orbiterx/.env` before creating any threads.
...
///     `orbiterx-linux-sandbox` helper path (falling back to the current
    ...
    Fut: Future<Output = anyhow::Result<()>>,
{
    let paths = Arg0DispatchPaths {
        orbiterx_self_exe: current_exe.clone(),
        orbiterx_linux_sandbox_exe: if cfg!(target_os = "linux") {
...
fn linux_sandbox_exe_path(
    ...
    // Prefer the `orbiterx-linux-sandbox` alias when available so callers can
    // re-exec through a path whose basename still triggers arg0 dispatch on
    // bubblewrap builds that do not support `--argv0`.
    path_entry_guard
        .and_then(|path_entry| path_entry.paths().orbiterx_linux_sandbox_exe.clone())
        .or(current_exe)
}
...
const ILLEGAL_ENV_VAR_PREFIX: &str = "ORBITERX_";

/// Load env vars from ~/.orbiterx/.env.
///
/// Security: Do not allow `.env` files to create or modify any variables
/// with names starting with `ORBITERX_`.
fn load_dotenv() {
    if let Ok(orbiterx_home) = find_orbiterx_home()
        && let Ok(iter) = dotenvy::from_path_iter(orbiterx_home.join(".env"))
    {
        set_filtered(iter);
    }
}

/// Helper to set vars from a dotenvy iterator while filtering out `ORBITERX_` keys.
...
///   with the hidden `--orbiterx-run-as-apply-patch` flag.
///
...
/// user to install a separate executable, simplifying the deployment of OrbiterX
/// CLI.
...
fn prepare_path_entry_for_orbiterx_aliases(
    existing_path: Option<OsString>,
    ...
    #[cfg(not(debug_assertions))]
    {
        ...
        if orbiterx_home.starts_with(&temp_root) {
            return Err(std::io::Error::new(
                std::io::ErrorKind::InvalidInput,
                format!(
                    "Refusing to create helper binaries under temporary dir {temp_root:?} (orbiterx_home: {orbiterx_home:?})"
                ),
            ));
        }
    }
    ...
    // Use a ORBITERX_HOME-scoped temp root to avoid cluttering the top-level directory.
    let temp_root = orbiterx_home.join("tmp").join("arg0");
    ...
    for filename in &[
        ...
        #[cfg(windows)]
        {
            ...
            std::fs::write(
                ...
                format!(
                    r#"@echo off
"{exe}" {ORBITERX_CORE_APPLY_PATCH_ARG1} %*
"#,
                ),
            )?;
        }
    ...
    let paths = Arg0DispatchPaths {
        orbiterx_self_exe: std::env::current_exe().ok(),
        orbiterx_linux_sandbox_exe: {
            #[cfg(target_os = "linux")]
            {
                Some(path.join(ORBITERX_LINUX_SANDBOX_ARG0))
            }
...
mod tests {
    ...
    fn package_path_test_fixture() -> anyhow::Result<PackagePathTestFixture> {
        ...
        let path_dir = package_dir.join("orbiterx-path");
        ...
        let install_context = InstallContext {
            ...
            package_layout: Some(OrbiterXPackageLayout {
    ...
    #[test]
    fn linux_sandbox_exe_path_prefers_orbiterx_linux_sandbox_alias() -> std::io::Result<()> {
        let temp_dir = TempDir::new()?;
        let lock_file = create_lock(temp_dir.path())?;
        let alias_path = temp_dir.path().join("orbiterx-linux-sandbox");
        let path_entry = Arg0PathEntryGuard::new(
            temp_dir,
            lock_file,
            Arg0DispatchPaths {
                orbiterx_self_exe: Some(PathBuf::from("/usr/bin/orbiterx")),
                orbiterx_linux_sandbox_exe: Some(alias_path.clone()),
                main_execve_wrapper_exe: None,
            },
        );

        assert_eq!(
            linux_sandbox_exe_path(Some(&path_entry), Some(PathBuf::from("/usr/bin/orbiterx"))),
            Some(alias_path),
        );
        Ok(())
    }
    ...
    fn run_main_with_arg0_guard_keeps_aliases_alive_until_main_returns() -> anyhow::Result<()> {
        let temp_dir = TempDir::new()?;
        let alias_path = temp_dir.path().join("orbiterx-helper-alias");
        ...
        super::build_runtime()?.block_on(run_main_with_arg0_guard(
            ...
            Some(PathBuf::from("/usr/bin/orbiterx")),
            |paths| async move {
