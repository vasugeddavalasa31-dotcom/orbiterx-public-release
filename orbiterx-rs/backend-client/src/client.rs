...
use crate::types::OrbiterXWorkspaceMessagesResponse;
...
use orbiterx_http_client::build_reqwest_client_with_custom_ca;
use orbiterx_http_client::with_chatgpt_cloudflare_cookie_store;
...
use orbiterx_protocol::protocol::RateLimitReachedType;
use orbiterx_protocol::protocol::RateLimitSnapshot;
use orbiterx_protocol::protocol::RateLimitWindow;
use orbiterx_protocol::protocol::SpendControlLimitSnapshot;
...
impl Client {
    pub fn new(base_url: impl Into<String>) -> Result<Self> {
        ...
        Ok(Self {
            base_url,
            http,
            auth_provider: orbiterx_model_provider::unauthenticated_auth_provider(),
            user_agent: None,
    ...
    pub fn from_auth(base_url: impl Into<String>, auth: &OrbiterXAuth) -> Result<Self> {
        Ok(Self::new(base_url)?
            .with_user_agent(get_orbiterx_user_agent())
            .with_auth_provider(orbiterx_model_provider::auth_provider_from_auth(auth)))
    }
    ...
    fn headers(&self) -> HeaderMap {
        ...
        if let Some(ua) = &self.user_agent {
            ...
            h.insert(USER_AGENT, HeaderValue::from_static("orbiterx-cli"));
        }
    ...
    pub async fn get_rate_limits(&self) -> Result<RateLimitSnapshot> {
        ...
        let preferred = snapshots
            .iter()
            .find(|snapshot| snapshot.limit_id.as_deref() == Some("orbiterx"))
            .cloned();
    ...
    pub async fn get_accounts_check(&self) -> Result<AccountsCheckResponse> {
        let url = match self.path_style {
            PathStyle::OrbiterXApi => format!("{}/api/orbiterx/accounts/check", self.base_url),
    ...
    fn token_usage_profile_url(&self) -> String {
        match self.path_style {
            PathStyle::OrbiterXApi => format!("{}/api/orbiterx/profiles/me", self.base_url),
    ...
    pub async fn list_tasks(
        ...
        let url = match self.path_style {
            PathStyle::OrbiterXApi => format!("{}/api/orbiterx/tasks/list", self.base_url),
    ...
    pub async fn get_task_details_with_body(
        ...
        let url = match self.path_style {
            PathStyle::OrbiterXApi => format!("{}/api/orbiterx/tasks/{}", self.base_url, task_id),
    ...
    pub async fn list_sibling_turns(
        ...
        let url = match self.path_style {
            PathStyle::OrbiterXApi => format!(
                "{}/api/orbiterx/tasks/{}/turns/{}/sibling_turns",
    ...
    /// Fetch the selected cloud-managed config bundle from orbiterx-backend.
    ///
    /// `GET /api/orbiterx/config/bundle` (OrbiterX API style) or
    ...
    pub async fn get_config_bundle(
        ...
        let url = match self.path_style {
            PathStyle::OrbiterXApi => format!("{}/api/orbiterx/config/bundle", self.base_url),
    ...
    pub async fn list_workspace_messages(
        &self,
    ) -> std::result::Result<OrbiterXWorkspaceMessagesResponse, RequestError> {
        ...
        let (body, ct) = self.exec_request_detailed(req, "GET", &url).await?;
        self.decode_json::<OrbiterXWorkspaceMessagesResponse>(&url, &ct, &body)
            .map_err(RequestError::from)
    }
    ...
    pub async fn create_task(&self, request_body: serde_json::Value) -> Result<String> {
        let url = match self.path_style {
            PathStyle::OrbiterXApi => format!("{}/api/orbiterx/tasks", self.base_url),
    ...
    fn make_rate_limit_snapshot(
        ...
        spend_control: Option<orbiterx_backend_openapi_models::models::SpendControlStatusDetails>,
    ...
    fn send_add_credits_nudge_email_url(&self) -> String {
        match self.path_style {
            PathStyle::OrbiterXApi => format!(
                "{}/api/orbiterx/accounts/send_add_credits_nudge_email",
                self.base_url
            ),
            PathStyle::ChatGptApi => {
                format!(
    ...
    fn workspace_messages_url(&self) -> String {
        match self.path_style {
            PathStyle::OrbiterXApi => format!("{}/api/orbiterx/workspace-messages", self.base_url),
...
mod tests {
    use super::*;
    use orbiterx_backend_openapi_models::models::AdditionalRateLimitDetails;
    use orbiterx_backend_openapi_models::models::RateLimitReachedKind;
    use orbiterx_backend_openapi_models::models::RateLimitReachedType as BackendRateLimitReachedType;
    ...
    fn usage_payload_maps_primary_and_additional_rate_limits() {
        let payload = RateLimitStatusPayload {
            ...
            additional_rate_limits: Some(Some(vec![AdditionalRateLimitDetails {
                limit_name: "orbiterx_other".to_string(),
                metered_feature: "orbiterx_other".to_string(),
                rate_limit: Some(Some(Box::new(crate::types::RateLimitStatusDetails {
                    primary_window: Some(Some(Box::new(crate::types::RateLimitWindowSnapshot {
                        used_percent: 70,
            ...
            spend_control: Some(Some(Box::new(
                orbiterx_backend_openapi_models::models::SpendControlStatusDetails {
                    reached: false,
        ...
        assert_eq!(snapshots[0].limit_id.as_deref(), Some("orbiterx"));
        ...
        assert_eq!(snapshots[1].limit_id.as_deref(), Some("orbiterx_other"));
        assert_eq!(snapshots[1].limit_name.as_deref(), Some("orbiterx_other"));
        assert_eq!(
    ...
    fn preferred_snapshot_selection_matches_get_rate_limits_behavior() {
        ...
        assert_eq!(preferred.limit_id.as_deref(), Some("orbiterx"));
    }
    ...
    fn add_credits_nudge_email_uses_expected_paths_and_bodies() {
        let orbiterx_client = test_client("https://example.test", PathStyle::OrbiterXApi);
        assert_eq!(
            orbiterx_client.send_add_credits_nudge_email_url(),
            "https://example.test/api/orbiterx/accounts/send_add_credits_nudge_email"
        );
    ...
    fn token_usage_profile_uses_expected_paths() {
        let orbiterx_client = test_client("https://example.test", PathStyle::OrbiterXApi);
        assert_eq!(
            orbiterx_client.token_usage_profile_url(),
            "https://example.test/api/orbiterx/profiles/me"
        );
    ...
    fn workspace_messages_uses_expected_paths() {
        let orbiterx_client = test_client("https://example.test", PathStyle::OrbiterXApi);
        assert_eq!(
            orbiterx_client.workspace_messages_url(),
            "https://example.test/api/orbiterx/workspace-messages"
        );
