...
fn rate_limit_reset_contract_uses_expected_paths_and_payloads() {
    assert_eq!(
        test_client("https://example.test", PathStyle::OrbiterXApi).rate_limit_status_url(),
        "https://example.test/api/orbiterx/usage"
    );
    assert_eq!(
        test_client("https://example.test", PathStyle::OrbiterXApi).rate_limit_reset_credits_url(),
        "https://example.test/api/orbiterx/rate-limit-reset-credits"
    );
    assert_eq!(
        test_client("https://example.test", PathStyle::OrbiterXApi)
            .consume_rate_limit_reset_credit_url(),
        "https://example.test/api/orbiterx/rate-limit-reset-credits/consume"
    );
    assert_eq!(
        ...
        RateLimitResetCreditsDetails {
            credits: vec![
                RateLimitResetCreditDetails {
                    ...
                    reset_type: "orbiterx_rate_limits".to_string(),
