...
use crate::landlock::ORBITERX_LINUX_SANDBOX_ARG0;
...
pub struct SandboxTransformRequest<'a> {
    ...
    pub orbiterx_linux_sandbox_exe: Option<&'a Path>,
...
pub struct SandboxDirectSpawnTransformRequest<'a> {
    ...
    pub windows_sandbox_proxy_settings_mode:
        orbiterx_windows_sandbox::WindowsSandboxProxySettingsMode,
}
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::MissingLinuxSandboxExecutable => {
                write!(f, "missing orbiterx-linux-sandbox executable path")
            }
...
impl SandboxManager {
    ...
    pub fn transform(
        ...
        let (argv, arg0_override, pending_sandboxed_request) = match sandbox {
            ...
            SandboxType::LinuxSeccomp => {
                let pending = pending_sandboxed_request?;
                let exe = orbiterx_linux_sandbox_exe
    ...
    pub fn transform_for_direct_spawn(
        ...
        #[cfg(target_os = "windows")]
        {
            let orbiterx_home = orbiterx_utils_home_dir::find_orbiterx_home()
                .map_err(|err| SandboxTransformError::WindowsSandboxPreparation(err.to_string()))?;
            self.transform_for_direct_spawn_with_orbiterx_home(request, orbiterx_home.as_path())
        }
    ...
    #[cfg(target_os = "windows")]
    fn transform_for_direct_spawn_with_orbiterx_home(
        &self,
        request: SandboxDirectSpawnTransformRequest<'_>,
        orbiterx_home: &Path,
    ) -> Result<SandboxExecRequest, SandboxTransformError> {
...
fn wrap_windows_sandbox_exec_request_for_direct_spawn(
    ...
    orbiterx_home: &Path,
    proxy_settings_mode: orbiterx_windows_sandbox::WindowsSandboxProxySettingsMode,
    ...
    let helper = orbiterx_windows_sandbox::resolve_exe_for_launch(source.as_path(), orbiterx_home);
    *program = helper.to_string_lossy().into_owned();
    ...
    let mut wrapper_args =
        orbiterx_windows_sandbox::create_windows_sandbox_command_args_for_permission_profile(
            inner_command,
            &native_cwd,
            workspace_roots,
            &request.env,
...
fn linux_sandbox_arg0_override(exe: &Path) -> String {
    if exe.file_name().and_then(|name| name.to_str()) == Some(ORBITERX_LINUX_SANDBOX_ARG0) {
