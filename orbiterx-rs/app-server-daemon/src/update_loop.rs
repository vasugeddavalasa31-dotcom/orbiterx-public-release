...
#[cfg(unix)]
use crate::managed_install::resolved_managed_orbiterx_bin;
...
async fn update_once(
    ...
    let managed_orbiterx_bin = resolved_managed_orbiterx_bin(&daemon.managed_orbiterx_bin).await?;
    let managed_identity = executable_identity(&managed_orbiterx_bin).await?;
    ...
    loop {
        ...
        match daemon
            .try_restart_if_running(restart_mode, updater_refresh_mode, &managed_orbiterx_bin)
            .await?
        {
...
#[cfg(unix)]
pub(crate) fn reexec_managed_updater(managed_orbiterx_bin: &std::path::Path) -> Result<()> {
    let err = StdCommand::new(managed_orbiterx_bin)
        .args(["app-server", "daemon", "pid-update-loop"])
        .exec();
    Err(err).with_context(|| {
        format!(
            "failed to replace updater with managed OrbiterX binary {}",
            managed_orbiterx_bin.display()
        )
    })
}
...
async fn install_latest_standalone() -> Result<()> {
    let script = reqwest::get("https://chatgpt.com/orbiterx/install.sh")
        .await
        .context("failed to fetch standalone OrbiterX updater")?
        .error_for_status()
        .context("standalone OrbiterX updater request failed")?
        .bytes()
        .await
        .context("failed to read standalone OrbiterX updater")?;
    ...
    let mut child = Command::new("/bin/sh")
        ...
        .stderr(Stdio::null())
        .spawn()
        .context("failed to invoke standalone OrbiterX updater")?;
    let mut stdin = child
        .stdin
        .take()
        .context("standalone OrbiterX updater stdin was unavailable")?;
    stdin
        .write_all(&script)
        .await
        .context("failed to pass standalone OrbiterX updater to shell")?;
    drop(stdin);
    let status = child
        .wait()
        .await
        .context("failed to wait for standalone OrbiterX updater")?;

    if status.success() {
        Ok(())
    } else {
        anyhow::bail!("standalone OrbiterX updater exited with status {status}")
    }
}
