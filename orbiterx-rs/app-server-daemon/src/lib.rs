...
use managed_install::managed_orbiterx_bin;
#[cfg(unix)]
use managed_install::managed_orbiterx_version;
use orbiterx_app_server_protocol::RemoteControlConnectionStatus;
use orbiterx_app_server_protocol::RemoteControlPairingStartResponse;
use orbiterx_app_server_transport::app_server_control_socket_path;
...
pub struct LifecycleOutput {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub pid: Option<u32>,
    pub managed_orbiterx_path: PathBuf,
    pub managed_orbiterx_version: Option<String>,
...
fn ensure_supported_platform() -> Result<()> {
    Err(anyhow!(
        "orbiterx app-server daemon lifecycle is only supported on Unix platforms"
    ))
}
...
struct Daemon {
    ...
    managed_orbiterx_bin: PathBuf,
}

impl Daemon {
    fn from_environment() -> Result<Self> {
        let orbiterx_home = find_orbiterx_home().context("failed to resolve ORBITERX_HOME")?;
        let socket_path = app_server_control_socket_path(orbiterx_home.as_path())?
            .as_path()
            .to_path_buf();
        let state_dir = orbiterx_home.as_path().join(STATE_DIR_NAME);
        Ok(Self {
            socket_path,
            ...
            managed_orbiterx_bin: managed_orbiterx_bin(orbiterx_home.as_path()),
        })
    }
    ...
    async fn start(&self) -> Result<LifecycleOutput> {
        ...
        self.ensure_managed_orbiterx_bin()?;
    ...
    async fn restart(&self) -> Result<LifecycleOutput> {
        ...
        {
            return Err(anyhow!(
                "app server is running but is not managed by orbiterx app-server daemon"
            ));
        }
    ...
    pub(crate) async fn try_restart_if_running(
        ...
        let outcome = if let Some(backend) = self.running_backend_instance(&settings).await? {
            ...
            let managed_version = if info.is_some() {
                Some(managed_orbiterx_version(managed_orbiterx_bin).await?)
            } else {
                None
            };
            match restart_decision(mode, info.as_ref(), managed_version.as_deref()) {
                ...
                RestartDecision::Restart => {
                    backend.stop().await?;
                    let _ = self
                        .start_managed_backend_with_bin(&settings, managed_orbiterx_bin)
                        .await?;
        ...
        if should_reexec_updater(updater_refresh_mode, outcome) {
            crate::update_loop::reexec_managed_updater(managed_orbiterx_bin)?;
        }
    ...
    async fn append_daemon_app_server_context(&self, context: &mut String) {
        let managed_orbiterx_version = self
            .managed_orbiterx_version_best_effort()
            .await
            .unwrap_or_else(|| "unknown".to_string());
        context.push_str(&format!(
            "\n\nDaemon used app-server:\n  path: {}\n  version: {managed_orbiterx_version}",
            self.managed_orbiterx_bin.display()
        ));
    }
    ...
    async fn bootstrap_locked(&self, options: BootstrapOptions) -> Result<BootstrapOutput> {
        ...
        let info = self.wait_until_ready().await?;
        let managed_orbiterx_version = self.managed_orbiterx_version_best_effort().await;
        Ok(BootstrapOutput {
            status: BootstrapStatus::Bootstrapped,
            backend: BackendKind::Pid,
            auto_update_enabled: true,
            remote_control_enabled: settings.remote_control_enabled,
            managed_orbiterx_path: self.managed_orbiterx_bin.clone(),
            managed_orbiterx_version,
    ...
    async fn start_managed_backend(&self, settings: &DaemonSettings) -> Result<Option<u32>> {
        self.start_managed_backend_with_bin(settings, &self.managed_orbiterx_bin)
            .await
    }
    ...
    async fn start_managed_backend_with_bin(
        ...
        let backend =
            backend::pid_backend(self.backend_paths_with_bin(settings, managed_orbiterx_bin));
        backend.start().await
    }
    ...
    fn ensure_managed_orbiterx_bin(&self) -> Result<()> {
        if self.managed_orbiterx_bin.is_file() {
            return Ok(());
        }

        let managed_orbiterx_path = self.managed_orbiterx_bin.display();
        Err(anyhow!(
            "managed standalone OrbiterX install not found at {managed_orbiterx_path}\n\n\
             This command requires the standalone install managed by the OrbiterX installer, because \
    ...
    #[cfg(unix)]
    async fn managed_orbiterx_version_best_effort(&self) -> Option<String> {
        managed_orbiterx_version(&self.managed_orbiterx_bin)
            .await
            .ok()
    }
    ...
    fn backend_paths(&self, settings: &DaemonSettings) -> BackendPaths {
        self.backend_paths_with_bin(settings, &self.managed_orbiterx_bin)
    }
    ...
    fn backend_paths_with_bin(
        ...
        BackendPaths {
            orbiterx_bin: managed_orbiterx_bin.to_path_buf(),
...
mod tests {
    ...
    fn remote_control_start_output_serializes_inner_output_without_tag() {
        let lifecycle_output = LifecycleOutput {
            ...
            pid: None,
            managed_orbiterx_path: "orbiterx".into(),
            managed_orbiterx_version: Some("1.2.3".to_string()),
            socket_path: "orbiterx.sock".into(),
        ...
        assert_eq!(
            ...
            serde_json::json!({
                ...
                "backend": "pid",
                "managedOrbiterXPath": "orbiterx",
                "managedOrbiterXVersion": "1.2.3",
                "socketPath": "orbiterx.sock",
                "cliVersion": "1.2.3",
    ...
    async fn not_ready_context_reports_daemon_app_server_before_stderr() {
        ...
        let daemon = Daemon {
            ...
            managed_orbiterx_bin: temp_dir.path().join("missing-orbiterx"),
        };
        ...
        assert_eq!(
            ...
            format!(
                ...
                daemon.managed_orbiterx_bin.display(),
                stderr_log.display()
            )
        );
    }
