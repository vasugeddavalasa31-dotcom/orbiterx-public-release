# orbiterx-app-server-daemon

> `orbiterx-app-server-daemon` is experimental and its lifecycle contract may
> change while the remote-management flow is still being developed.

`orbiterx-app-server-daemon` backs the machine-readable `orbiterx app-server`
lifecycle commands used by remote clients such as the desktop and mobile apps.
It is intended for OrbiterX instances launched over SSH, including fresh developer
...
```sh
orbiterx app-server daemon start
orbiterx app-server daemon restart
orbiterx app-server daemon enable-remote-control
orbiterx app-server daemon disable-remote-control
orbiterx app-server daemon stop
orbiterx app-server daemon version
orbiterx app-server daemon bootstrap --remote-control
```
...
$HOME/.orbiterx/packages/standalone/current/orbiterx app-server daemon bootstrap --remote-control
```

`bootstrap` requires the standalone managed install. It records the daemon
settings under `ORBITERX_HOME/app-server-daemon/`, starts app-server as a
pidfile-backed detached process, and launches a detached updater loop.

## Installation and update cases

The daemon assumes OrbiterX is installed through `install.sh` and always launches
the standalone managed binary under `ORBITERX_HOME`.

| Situation | What starts | Does this daemon fetch new binaries? | Does a running app-server eventually move to a newer binary on its own? |
| --- | --- | --- | --- |
| `install.sh` has run, but only `start` is used | `start` uses `ORBITERX_HOME/packages/standalone/current/orbiterx` | No | No. The managed path is used when starting or restarting, but no updater is installed. |
| `install.sh` has run, then `bootstrap` is used | The pidfile backend uses `ORBITERX_HOME/packages/standalone/current/orbiterx` | Yes. Bootstrap launches a detached updater loop that runs `install.sh` hourly. | Yes, while that updater process is alive and app-server is already running. After a successful fetch, the updater restarts app-server with the refreshed binary and only then replaces its own process image. |
...
Top-level `orbiterx remote-control` bootstraps with `--remote-control` when the
...
All mutating lifecycle commands are serialized per `ORBITERX_HOME`, so a concurrent
...
The daemon stores its local state under `ORBITERX_HOME/app-server-daemon/`:
