...
use orbiterx_state::BackfillState;
use orbiterx_state::BackfillStats;
use orbiterx_state::BackfillStatus;
use orbiterx_state::DB_ERROR_METRIC;
use orbiterx_state::DB_METRIC_BACKFILL;
use orbiterx_state::DB_METRIC_BACKFILL_DURATION_MS;
use orbiterx_state::ExtractionOutcome;
use orbiterx_state::ThreadMetadataBuilder;
use orbiterx_state::apply_rollout_item;
use std::path::Path;
...
pub(crate) async fn backfill_sessions_with_lease(
    ...
    let metric_client = orbiterx_otel::global();
    ...
    let sessions_root = orbiterx_home.join(SESSIONS_SUBDIR);
    let archived_root = orbiterx_home.join(ARCHIVED_SESSIONS_SUBDIR);
        ...
        match collect_rollout_paths(&root).await {
            Ok(paths) => {
                rollout_paths.extend(paths.into_iter().map(|path| BackfillRolloutPath {
                    watermark: backfill_watermark_for_path(orbiterx_home, &path),
                    path,
                    archived,
                }));
            }
...
fn backfill_watermark_for_path(orbiterx_home: &Path, path: &Path) -> String {
    path.strip_prefix(orbiterx_home)
        .unwrap_or(path)
        .to_string_lossy()
        .replace('\\', "/")
}
