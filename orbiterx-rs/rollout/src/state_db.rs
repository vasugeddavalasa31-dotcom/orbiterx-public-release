...
pub use orbiterx_state::LogEntry;
...
pub type StateDbHandle = Arc<orbiterx_state::StateRuntime>;
...
async fn try_init_with_roots_inner(
    ...
    let runtime =
        orbiterx_state::StateRuntime::init(sqlite_home.clone(), default_model_provider_id.clone())
            .await
            .with_context(|| {
                format!(
    ...
    .await;
    orbiterx_state::record_backfill_gate(
        /*telemetry*/ None,
...
async fn wait_for_backfill_gate(
    ...
    loop {
        ...
        if backfill_state.status == orbiterx_state::BackfillStatus::Complete {
            return Ok(());
        }
        ...
        if let Some(backfill_lease_seconds) = backfill_lease_seconds {
            ...
            metadata::backfill_sessions(runtime, orbiterx_home, default_model_provider_id).await;
        }
        let backfill_state = runtime.get_backfill_state().await.map_err(|err| {
            anyhow::anyhow!(
...
pub async fn get_state_db(config: &impl RolloutConfigView) -> Option<StateDbHandle> {
    let state_path = orbiterx_state::state_db_path(config.sqlite_home());
    ...
    let runtime = match orbiterx_state::StateRuntime::init(
...
pub fn sqlite_telemetry_recorder(
    ...
    originator: &str,
) -> orbiterx_state::DbTelemetryHandle {
...
async fn require_backfill_complete(
    ...
    match runtime.get_backfill_state().await {
        Ok(state) if state.status == orbiterx_state::BackfillStatus::Complete => Some(runtime),
        Ok(state) => {
            warn!(
...
fn cursor_to_anchor(cursor: Option<&Cursor>) -> Option<orbiterx_state::Anchor> {
    let cursor = cursor?;
...
pub async fn list_thread_ids_db(
    context: Option<&orbiterx_state::StateRuntime>,
    orbiterx_home: &Path,
    page_size: usize,
    ...
    let ctx = context?;
    if ctx.orbiterx_home() != orbiterx_home {
        warn!(
            "state db orbiterx_home mismatch: expected {}, got {}",
            ctx.orbiterx_home().display(),
            orbiterx_home.display()
        );
    }
    ...
    match ctx
        .list_thread_ids(
            ...
            anchor.as_ref(),
            match sort_key {
                ThreadSortKey::CreatedAt => orbiterx_state::SortKey::CreatedAt,
                ThreadSortKey::UpdatedAt => orbiterx_state::SortKey::UpdatedAt,
                ThreadSortKey::RecencyAt => orbiterx_state::SortKey::RecencyAt,
            },
...
pub async fn list_threads_db(
    ...
    relation_filter: Option<orbiterx_state::ThreadRelationFilter>,
    archived: bool,
    search_term: Option<&str>,
) -> Option<orbiterx_state::ThreadsPage> {
    let ctx = context?;
    if ctx.orbiterx_home() != orbiterx_home {
        warn!(
            "state db orbiterx_home mismatch: expected {}, got {}",
    ...
    let filters = orbiterx_state::ThreadFilterOptions {
        ...
        sort_direction: match sort_direction {
            SortDirection::Asc => orbiterx_state::SortDirection::Asc,
            SortDirection::Desc => orbiterx_state::SortDirection::Desc,
        },
        search_term,
    };
