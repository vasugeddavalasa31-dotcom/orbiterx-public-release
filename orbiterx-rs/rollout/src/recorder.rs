//! Persist OrbiterX session rollouts (.jsonl) so sessions can be replayed or inspected later.
...
use orbiterx_protocol::protocol::GitInfo as ProtocolGitInfo;
...
use orbiterx_protocol::protocol::SessionContextWindow;
...
///
/// ```ignore
/// $ jq -C . ~/.orbiterx/sessions/rollout-2025-05-07T17-24-21-5973b6c0-94b8-487b-a530-2aeb6098ae0e.jsonl
/// $ fx ~/.orbiterx/sessions/rollout-2025-05-07T17-24-21-5973b6c0-94b8-487b-a530-2aeb6098ae0e.jsonl
/// ```
#[derive(Clone)]
...
impl RolloutRecorder {
    /// List threads (rollout files) under the provided OrbiterX home directory.
    #[allow(clippy::too_many_arguments)]
    ...
    async fn list_threads_with_db_fallback(
        ...
        let orbiterx_home = config.orbiterx_home();
        ...
        if state_db_ctx.is_none() {
            ...
            orbiterx_state::record_fallback(
                "list_threads",
                "db_unavailable",
...
async fn list_threads_from_files_desc(
    ...
    if let Some(search_term) = search_term {
        ...
        loop {
            ...
            filter_thread_items_by_search_term(orbiterx_home, &mut page.items, Some(search_term))
                .await?;
...
async fn list_threads_from_files_desc_unfiltered(
    ...
    if archived {
        let root = orbiterx_home.join(ARCHIVED_SESSIONS_SUBDIR);
        get_threads_in_root(
            root,
            page_size,
            cursor,
...
async fn list_threads_from_files_asc(
    ...
    filter_thread_items_by_search_term(orbiterx_home, &mut all_items, search_term).await?;
...
async fn filter_thread_items_by_search_term(
    ...
    let thread_ids = items
        ...
        .filter_map(|item| item.thread_id)
        .collect::<HashSet<_>>();
    let thread_names = find_thread_names_by_ids(orbiterx_home, &thread_ids).await?;
    items.retain(|item| {
        item.thread_id
...
fn precompute_log_file_info(
    ...
    // Resolve ~/.orbiterx/sessions/YYYY/MM/DD path.
    let timestamp = OffsetDateTime::now_local()
        .map_err(|e| IoError::other(format!("failed to get local time: {e}")))?;
    let mut dir = config.orbiterx_home().to_path_buf();
...
impl From<orbiterx_state::ThreadsPage> for ThreadsPage {
    fn from(db_page: orbiterx_state::ThreadsPage) -> Self {
        let orbiterx_state::ThreadsPage {
            items,
            parent_thread_ids,
            next_anchor,
            num_scanned_rows,
...
fn thread_item_from_state_metadata(
    item: orbiterx_state::ThreadMetadata,
    parent_thread_id: Option<ThreadId>,
) -> ThreadItem {
    ThreadItem {
...
async fn select_resume_path_from_db_page(
    page: &orbiterx_state::ThreadsPage,
    filter_cwd: Option<&Path>,
    default_provider: &str,
) -> Option<PathBuf> {
    match filter_cwd {
