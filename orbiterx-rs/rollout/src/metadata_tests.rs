...
use orbiterx_state::BackfillStatus;
...
async fn backfill_sessions_resumes_from_watermark_and_marks_complete() {
    let dir = tempdir().expect("tempdir");
    let orbiterx_home = dir.path().to_path_buf();
    ...
    let runtime =
        orbiterx_state::StateRuntime::init(orbiterx_home.clone(), "test-provider".to_string())
            .await
            .expect("initialize runtime");
    let first_watermark =
        backfill_watermark_for_path(orbiterx_home.as_path(), first_path.as_path());
    ...
    backfill_sessions(runtime.as_ref(), orbiterx_home.as_path(), "test-provider").await;
...
async fn backfill_sessions_normalizes_cwd_before_upsert() {
    ...
    let orbiterx_home = dir.path().to_path_buf();
    let thread_uuid = Uuid::new_v4();
    let session_cwd = orbiterx_home.join(".");
...
fn write_rollout_in_sessions_with_cwd(
    ...
    let sessions_dir = orbiterx_home.join("sessions");
