...
pub async fn append_thread_name(
    ...
    append_session_index_entry(orbiterx_home, &entry).await
}
...
pub async fn append_session_index_entry(
    ...
    let path = session_index_path(orbiterx_home);
...
fn session_index_path(orbiterx_home: &Path) -> PathBuf {
    orbiterx_home.join(SESSION_INDEX_FILE)
}
