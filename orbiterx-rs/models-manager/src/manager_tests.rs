...
use orbiterx_login::ExternalAuth;
use orbiterx_login::ExternalAuthRefreshContext;
use orbiterx_login::OrbiterXAuth;
use orbiterx_login::TokenData;
...
impl ExternalAuth for TestExternalApiKeyAuth {
    fn resolve(&self) -> orbiterx_login::ExternalAuthFuture<'_, OrbiterXAuth> {
        Box::pin(async { Ok(OrbiterXAuth::from_api_key("test-external-api-key")) })
    }

    fn refresh(
        &self,
        _context: ExternalAuthRefreshContext,
    ) -> orbiterx_login::ExternalAuthFuture<'_, OrbiterXAuth> {
        Box::pin(async { Ok(OrbiterXAuth::from_api_key("test-external-api-key")) })
    }
}
...
impl ModelsEndpointClient for TestModelsEndpoint {
    ...
    fn uses_orbiterx_backend(&self) -> ModelsEndpointFuture<'_, bool> {
        Box::pin(async { self.uses_orbiterx_backend })
    }
...
fn openai_manager_for_tests(
    orbiterx_home: std::path::PathBuf,
...
fn openai_manager_for_tests_with_auth(
    ...
    OpenAiModelsManager::new(orbiterx_home, endpoint_client, auth_manager)
}
...
async fn chatgpt_auth_tokens_for_tests(orbiterx_home: &Path) -> OrbiterXAuth {
    let auth_dot_json = orbiterx_login::AuthDotJson {
        auth_mode: Some(AuthMode::ChatgptAuthTokens),
        openai_api_key: None,
        tokens: Some(TokenData {
            id_token: orbiterx_login::token_data::parse_chatgpt_jwt_claims(
...
c2ln",
    ...
    std::fs::create_dir_all(orbiterx_home).expect("orbiterx home should be created");
    std::fs::write(
...
async fn dynamic_manager_preserves_requested_model_when_fallback_is_allowed() {
    let orbiterx_home = tempdir().expect("temp dir");
    let endpoint = TestModelsEndpoint::new(Vec::new());
    let manager = openai_manager_for_tests(orbiterx_home.path().to_path_buf(), endpoint.clone());
    let requested_model = Some("unsupported".to_string());
...
async fn get_model_info_matches_hyphenated_provider_namespace_suffix() {
    ...
    let namespaced_model = "openai-orbiterx/gpt-image".to_string();
...
async fn refresh_available_models_uses_cached_remote_only_catalog_for_chatgpt_auth() {
    ...
    let fetch_manager =
        openai_manager_for_tests(orbiterx_home.path().to_path_buf(), fetch_endpoint.clone());
    ...
    let cache_manager =
        openai_manager_for_tests(orbiterx_home.path().to_path_buf(), cache_endpoint.clone());
...
async fn get_model_info_uses_fallback_for_bundled_models_when_chatgpt_remote_is_authoritative() {
    ...
    let manager = openai_manager_for_tests(orbiterx_home.path().to_path_buf(), endpoint);
...
async fn refresh_available_models_drops_removed_remote_models() {
    ...
    let mut manager =
        openai_manager_for_tests(orbiterx_home.path().to_path_buf(), endpoint.clone());
    manager
        .cache_manager
        .as_mut()
        .expect("cached model manager")
...
impl TestAuthAwareModelsEndpoint {
    ...
    async fn uses_orbiterx_backend(&self) -> bool {
...
impl ModelsEndpointClient for TestAuthAwareModelsEndpoint {
    ...
    fn uses_orbiterx_backend(&self) -> ModelsEndpointFuture<'_, bool> {
        Box::pin(TestAuthAwareModelsEndpoint::uses_orbiterx_backend(self))
    }
...
async fn refresh_available_models_fetches_with_chatgpt_auth_tokens() {
    ...
    let auth = chatgpt_auth_tokens_for_tests(orbiterx_home.path()).await;
    let manager = openai_manager_for_tests_with_auth(
        orbiterx_home.path().to_path_buf(),
        endpoint.clone(),
