...
use orbiterx_cloud_config::cloud_config_bundle_loader;
...
use orbiterx_utils_json_to_toml::json_to_toml;
...
/// Shared app-server entry point for loading effective OrbiterX configuration.
#[derive(Clone)]
...
impl ConfigManager {
    ...
    pub(crate) fn orbiterx_home(&self) -> &Path {
        self.orbiterx_home.as_path()
    }

    pub(crate) fn user_config_path(&self) -> std::io::Result<AbsolutePathBuf> {
        self.loader_overrides.user_config_path(self.orbiterx_home())
    }
    ...
    pub(crate) fn replace_cloud_config_bundle_loader(
        ...
        let loader =
            cloud_config_bundle_loader(auth_manager, chatgpt_base_url, self.orbiterx_home.clone());
    ...
    fn current_thread_config_loader(&self) -> Arc<dyn ThreadConfigLoader> {
        self.thread_config_loader
            ...
            .unwrap_or_else(|_| Arc::new(orbiterx_config::NoopThreadConfigLoader))
    }
    ...
    pub(crate) async fn load_default_config(&self) -> std::io::Result<Config> {
            ...
            || self.loader_overrides.user_config_profile.is_some()
        {
            let user_config_path = self
                .loader_overrides
                .user_config_path(self.orbiterx_home())?;
    ...
    pub(crate) async fn load_with_cli_overrides(
        ...
        let mut config = orbiterx_core::config::ConfigBuilder::default()
            .orbiterx_home(self.orbiterx_home.clone())
    ...
    fn apply_arg0_paths(&self, config: &mut Config) {
        config.orbiterx_self_exe = self.arg0_paths.orbiterx_self_exe.clone();
        config.orbiterx_linux_sandbox_exe = self.arg0_paths.orbiterx_linux_sandbox_exe.clone();
    ...
    pub(crate) fn new_for_tests(
        ...
        Self::new(
            ...
            Arc::new(orbiterx_config::NoopThreadConfigLoader),
        )
    }

    #[cfg(test)]
    pub(crate) fn without_managed_config_for_tests(orbiterx_home: PathBuf) -> Self {
        Self::new_for_tests(
            orbiterx_home,
            Vec::new(),
