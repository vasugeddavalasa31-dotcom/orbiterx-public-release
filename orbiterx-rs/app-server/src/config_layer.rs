use orbiterx_app_server_protocol::ConfigLayer as ApiConfigLayer;
use orbiterx_app_server_protocol::ConfigLayerMetadata as ApiConfigLayerMetadata;
use orbiterx_app_server_protocol::ConfigLayerSource as ApiConfigLayerSource;
use orbiterx_config::ConfigLayer;
use orbiterx_config::ConfigLayerMetadata;
use orbiterx_config::ConfigLayerSource;

/// Converts a config-layer source owned by `orbiterx-config` into the app-server wire type owned by
/// `orbiterx-app-server-protocol`.
///
...
/// Converts config-layer metadata owned by `orbiterx-config` into the app-server wire type owned by
/// `orbiterx-app-server-protocol`.
///
/// The types stay separate so app-server protocol ownership does not leak into the config domain
/// crate. Because this crate owns neither type, Rust's orphan rules require an explicit conversion
...
/// Converts a config layer owned by `orbiterx-config` into the app-server wire type owned by
/// `orbiterx-app-server-protocol`.
///
/// The types stay separate so app-server protocol ownership does not leak into the config domain
/// crate. Because this crate owns neither type, Rust's orphan rules require an explicit conversion
