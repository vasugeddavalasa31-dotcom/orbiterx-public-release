...
use orbiterx_app_server_protocol::Config as ApiConfig;
...
use orbiterx_config::merge_toml_values;
use orbiterx_core::config::deserialize_config_toml_with_base;
use orbiterx_core::config::edit::ConfigEdit;
use orbiterx_core::config::edit::ConfigEditsBuilder;
use orbiterx_core::config::validate_feature_requirements_for_config_toml;
use orbiterx_core::path_utils;
use orbiterx_core::path_utils::SymlinkWritePaths;
use orbiterx_core::path_utils::resolve_symlink_write_paths;
use orbiterx_core::path_utils::write_atomically;
...
impl ConfigManager {
    ...
    async fn apply_edits(
        ...
        let user_config_toml =
            deserialize_config_toml_with_base(user_config.clone(), self.orbiterx_home()).map_err(
                |err| {
                    ConfigManagerError::write(
                        ConfigWriteErrorCode::ConfigValidationError,
                        format!("Invalid configuration: {err}"),
    ...
    /// include any in-repo .orbiterx/ folders because there is no cwd/project root
