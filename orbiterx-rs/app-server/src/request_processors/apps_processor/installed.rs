...
use orbiterx_connectors::ConnectorRuntimeTool;
use orbiterx_connectors::connector_runtime_context_key;
use orbiterx_connectors::connector_tool_is_synthetic;
use orbiterx_connectors::installed_connector_runtime;
...
use orbiterx_mcp::host_owned_orbiterx_apps_enabled;
...
const APPS_INSTALLED_RESPONSE_BYTES_METRIC: &str = "orbiterx.apps.installed.response_bytes";
const APPS_INSTALLED_CONNECTOR_COUNT_METRIC: &str = "orbiterx.apps.installed.connector_count";
const APPS_INSTALLED_TOOL_COUNT_METRIC: &str = "orbiterx.apps.installed.tool_count";
const APPS_SNAPSHOT_AGE_METRIC: &str = "orbiterx.apps.snapshot.age_ms";
    ...
    pub(crate) async fn apps_installed(
        ...
        let result = async {
            ...
            let apps_enabled = config
                .features
                .apps_enabled_for_auth(auth.as_ref().is_some_and(OrbiterXAuth::uses_orbiterx_backend));
            ...
            let previous_snapshot = mcp_manager
                ...
                .current_snapshot(config.orbiterx_home.to_path_buf(), cache_key.clone());
            let snapshot = if force_refresh && runtime_enabled {
                let refresh_result = async {
                    anyhow::ensure!(
                        !mcp_servers.is_empty(),
                        "host-owned MCP server '{ORBITERX_APPS_MCP_SERVER_NAME}' is not enabled"
                    );
                    ...
                    let orbiterx_apps_auth_manager =
                        host_owned_orbiterx_apps_enabled(&mcp_config, auth.as_ref())
                    ...
                    let result = if connection_manager
                        .wait_for_server_ready(ORBITERX_APPS_MCP_SERVER_NAME, startup_timeout)
                        .await
                    {
                        mcp_manager
                            .orbiterx_apps_tools_cache()
                        ...
                        Err(anyhow::anyhow!(
                            "failed to refresh tools for MCP server '{ORBITERX_APPS_MCP_SERVER_NAME}'"
                        ))
                    };
