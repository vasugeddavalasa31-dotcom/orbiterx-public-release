...
use orbiterx_app_server_protocol::ComputerUseRequirements;
...
use orbiterx_app_server_protocol::ConfigRequirements;
use orbiterx_app_server_protocol::ConfigRequirementsReadResponse;
use orbiterx_app_server_protocol::ConfigValueWriteParams;
use orbiterx_app_server_protocol::ConfigWriteErrorCode;
use orbiterx_app_server_protocol::ConfigWriteResponse;
use orbiterx_app_server_protocol::ConfiguredHookHandler;
use orbiterx_app_server_protocol::ConfiguredHookMatcherGroup;
use orbiterx_app_server_protocol::ExperimentalFeatureEnablementSetParams;
use orbiterx_app_server_protocol::ExperimentalFeatureEnablementSetResponse;
use orbiterx_app_server_protocol::JSONRPCErrorError;
use orbiterx_app_server_protocol::ManagedHooksRequirements;
use orbiterx_app_server_protocol::ModelProviderCapabilitiesReadResponse;
use orbiterx_app_server_protocol::ModelsRequirements;
use orbiterx_app_server_protocol::NetworkDomainPermission;
use orbiterx_app_server_protocol::NetworkRequirements;
use orbiterx_app_server_protocol::NetworkUnixSocketPermission;
...
use orbiterx_config::HookEventsToml;
use orbiterx_config::HookHandlerConfig as CoreHookHandlerConfig;
use orbiterx_config::ManagedHooksRequirementsToml;
use orbiterx_config::MatcherGroup as CoreMatcherGroup;
use orbiterx_config::ResidencyRequirement as CoreResidencyRequirement;
use orbiterx_config::SandboxModeRequirement as CoreSandboxModeRequirement;
use orbiterx_core::ThreadManager;
use orbiterx_features::canonical_feature_for_key;
use orbiterx_features::feature_for_key;
...
impl ConfigRequestProcessor {
    ...
    ) -> Result<orbiterx_core::config::Config, JSONRPCErrorError> {
        self.config_manager
    ...
    async fn write_value(
        ...
        let pending_changes = orbiterx_core_plugins::toggles::collect_plugin_enabled_candidates(
...
fn map_requirements_toml_to_api(requirements: ConfigRequirementsToml) -> ConfigRequirements {
    ConfigRequirements {
        allowed_approval_policies: requirements.allowed_approval_policies.map(|policies| {
            policies
                .into_iter()
                .map(orbiterx_app_server_protocol::AskForApproval::from)
                .collect()
        }),
        allowed_approvals_reviewers: requirements.allowed_approvals_reviewers.map(|reviewers| {
            reviewers
                .into_iter()
                .map(orbiterx_app_server_protocol::ApprovalsReviewer::from)
                .collect()
        }),
                ...
                .map(|implementations| {
                    implementations
                        ...
                        .map(|implementation| match implementation {
                            orbiterx_config::types::WindowsSandboxModeToml::Elevated => {
                                WindowsSandboxSetupMode::Elevated
                            }
                            orbiterx_config::types::WindowsSandboxModeToml::Unelevated => {
...
fn map_computer_use_requirements_to_api(
    computer_use: orbiterx_config::ComputerUseRequirementsToml,
...
) -> orbiterx_app_server_protocol::ResidencyRequirement {
    match residency {
        CoreResidencyRequirement::Us => orbiterx_app_server_protocol::ResidencyRequirement::Us,
    }
}

fn map_network_requirements_to_api(
    network: orbiterx_config::NetworkRequirementsToml,
) -> NetworkRequirements {
    let allowed_domains = network
        .domains
        .as_ref()
        .and_then(orbiterx_config::NetworkDomainPermissionsToml::allowed_domains);
    let denied_domains = network
        .domains
        .as_ref()
        .and_then(orbiterx_config::NetworkDomainPermissionsToml::denied_domains);
    let allow_unix_sockets = network
        .unix_sockets
        .as_ref()
        .map(orbiterx_config::NetworkUnixSocketPermissionsToml::allow_unix_sockets)
...
fn map_network_domain_permission_to_api(
    permission: orbiterx_config::NetworkDomainPermissionToml,
) -> NetworkDomainPermission {
    match permission {
        orbiterx_config::NetworkDomainPermissionToml::Allow => NetworkDomainPermission::Allow,
        orbiterx_config::NetworkDomainPermissionToml::Deny => NetworkDomainPermission::Deny,
    }
}

fn map_network_unix_socket_permission_to_api(
    permission: orbiterx_config::NetworkUnixSocketPermissionToml,
) -> NetworkUnixSocketPermission {
    match permission {
        orbiterx_config::NetworkUnixSocketPermissionToml::Allow => {
            NetworkUnixSocketPermission::Allow
        }
        orbiterx_config::NetworkUnixSocketPermissionToml::Deny => NetworkUnixSocketPermission::Deny,
    }
}
...
mod tests {
    ...
    use orbiterx_app_server_protocol::WindowsSandboxSetupMode;
    use orbiterx_config::ComputerUseRequirementsToml;
    use orbiterx_config::ConfigRequirementsToml;
    use orbiterx_config::ModelsRequirementsToml;
    use orbiterx_config::NewThreadModelDefaultsToml;
    use orbiterx_config::WindowsRequirementsToml;
