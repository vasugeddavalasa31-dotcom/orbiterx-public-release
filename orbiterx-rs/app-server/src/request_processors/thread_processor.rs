...
use orbiterx_app_server_protocol::SelectedCapabilityRoot;
...
const ORBITERX_TUI_CLIENT_NAME: &str = "orbiterx-tui";
...
fn collect_resume_override_mismatches(
    ...
    if let Some(requested_review_policy) = request.approvals_reviewer.as_ref() {
        let active_review_policy: orbiterx_app_server_protocol::ApprovalsReviewer =
    ...
    if let Some(requested_sandbox) = request.sandbox.as_ref() {
        ...
        let sandbox_matches = matches!(
            ...
            (
                ...
                orbiterx_protocol::protocol::SandboxPolicy::DangerFullAccess
            ) | (
                SandboxMode::DangerFullAccess,
...
fn validate_dynamic_tools(tools: &[DynamicToolSpec]) -> Result<(), String> {
    ...
    fn validate_dynamic_tool<'a>(
        ...
        if let Err(err) = orbiterx_tools::parse_tool_input_schema(&tool.input_schema) {
            return Err(format!(
...
impl ThreadRequestProcessor {
    ...
    pub(crate) async fn thread_rollback(
        ...
        if app_server_client_name != Some(ORBITERX_TUI_CLIENT_NAME) {
    ...
    async fn load_thread(
        &self,
        thread_id: &str,
    ) -> Result<(ThreadId, Arc<OrbiterXThread>), JSONRPCErrorError> {
    ...
    fn listener_task_context(&self) -> ListenerTaskContext {
        ListenerTaskContext {
            ...
            orbiterx_home: self.config.orbiterx_home.to_path_buf(),
    ...
    async fn thread_start_inner(
        ...
        if matches!(
            history_mode,
            Some(orbiterx_app_server_protocol::ThreadHistoryMode::Paginated)
    ...
    async fn request_trace_context(
        &self,
        request_id: &ConnectionRequestId,
    ) -> Option<orbiterx_protocol::protocol::W3cTraceContext> {
    ...
    async fn thread_start_task(
        ...
        session_start_source: Option<orbiterx_app_server_protocol::ThreadStartSource>,
            ...
            && (requested_permissions_trust_project || effective_permissions_trust_project)
        {
            ...
            let cli_overrides_for_reload = if let Err(err) =
                orbiterx_core::config::set_project_trust_level(
                    &listener_task_context.orbiterx_home,
        ...
        if let Ok(Some(err)) =
            orbiterx_core::check_execpolicy_for_warnings(&config.config_layer_stack).await
        {
        ...
        let NewThread {
            ...
            .start_thread_with_options(StartThreadOptions {
                ...
                initial_history: match session_start_source
                    .unwrap_or(orbiterx_app_server_protocol::ThreadStartSource::Startup)
                {
                    orbiterx_app_server_protocol::ThreadStartSource::Startup => InitialHistory::New,
                    orbiterx_app_server_protocol::ThreadStartSource::Clear => {
            ...
            .map_err(|err| match err {
                ...
                OrbiterXErr::UnsupportedOperation(message) => method_not_found(message),
    ...
    fn build_thread_config_overrides(
        ...
        approval_policy: Option<orbiterx_app_server_protocol::AskForApproval>,
        approvals_reviewer: Option<orbiterx_app_server_protocol::ApprovalsReviewer>,
        ...
        ConfigOverrides {
            ...
            approval_policy: approval_policy
                .map(orbiterx_app_server_protocol::AskForApproval::to_core),
            approvals_reviewer: approvals_reviewer
                .map(orbiterx_app_server_protocol::ApprovalsReviewer::to_core),
            sandbox_mode: sandbox.map(SandboxMode::to_core),
            orbiterx_linux_sandbox_exe: self.arg0_paths.orbiterx_linux_sandbox_exe.clone(),
    ...
    ) -> Result<(ThreadSetNameResponse, Option<ThreadNameUpdatedNotification>), JSONRPCErrorError>
    {
        ...
        let thread_id = ThreadId::from_string(&thread_id)
            .map_err(|err| invalid_request(format!("invalid thread id: {err}")))?;
        let Some(name) = orbiterx_core::util::normalize_thread_name(&name) else {
    ...
    async fn memory_reset_response_inner(&self) -> Result<MemoryResetResponse, JSONRPCErrorError> {
        ...
        clear_memory_roots_contents(&self.config.orbiterx_home)
            .await
            .map_err(|err| {
                internal_error(format!(
                    "failed to clear memory directories under {}: {err}",
                    self.config.orbiterx_home.display()
                ))
            })?;
    ...
    async fn thread_resume_inner(
            ...
            .await
        {
            Ok(NewThread {
                ...
                let instruction_sources = orbiterx_thread.legacy_instruction_sources().await;
                ...
                thread.thread_source = orbiterx_thread
                    .config_snapshot()
                    .await
                    .thread_source
                    .map(Into::into);
                ...
                let config_snapshot = orbiterx_thread.config_snapshot().await;
                ...
                self.thread_goal_processor
                    .emit_resume_goal_snapshot_and_continue(thread_id, orbiterx_thread.as_ref())
                    .await;
            }
            Err(err) => {
    ...
    async fn thread_fork_inner(
        ...
        let source_thread_name = source_thread
            .name
            .as_deref()
            .and_then(orbiterx_core::util::normalize_thread_name);
        ...
        let NewThread {
            ...
            .await
            .map_err(|err| match err {
                OrbiterXErr::Io(_) | OrbiterXErr::Json(_) => {
...
) -> Result<orbiterx_app_server_protocol::TurnsPage, JSONRPCErrorError> {
    build_thread_turns_page_response(
        items,
        loaded_status,
        has_live_running_thread,
...
fn deserialize_stored_thread_item(
    item: orbiterx_thread_store::StoredThreadItem,
...
fn stored_turn_to_api_turn(
    ...
    let error = turn.error.map(|error| TurnError {
        ...
        orbiterx_error_info: error.orbiterx_error_info,
...
pub(super) fn core_thread_write_error(operation: &str, err: OrbiterXErr) -> JSONRPCErrorError {
    match err {
...
) -> (Thread, Option<orbiterx_thread_store::StoredThreadHistory>) {
...
fn summary_from_state_db_metadata(
    ...
    cli_version: String,
    source: String,
    _thread_source: Option<orbiterx_protocol::protocol::ThreadSource>,
    ...
    let source = serde_json::from_str(&source)
        ...
        .unwrap_or(orbiterx_protocol::protocol::SessionSource::Unknown);
...
fn preview_from_rollout_items(items: &[RolloutItem]) -> String {
    items
        .iter()
        .find_map(|item| match item {
            RolloutItem::ResponseItem(item) => match orbiterx_core::parse_turn_item(item) {
                Some(orbiterx_protocol::items::TurnItem::UserMessage(user)) => Some(user.message()),
                _ => None,
            },
            _ => None,
        })
...
fn requested_permissions_trust_project(overrides: &ConfigOverrides, cwd: &Path) -> bool {
    if matches!(
        ...
        Some(
            orbiterx_protocol::config_types::SandboxMode::WorkspaceWrite
                | orbiterx_protocol::config_types::SandboxMode::DangerFullAccess
        )
    ) {
        return true;
    }
...
fn permission_profile_trusts_project(
    profile: &orbiterx_protocol::models::PermissionProfile,
    cwd: &Path,
) -> bool {
    match profile {
        orbiterx_protocol::models::PermissionProfile::Disabled
        | orbiterx_protocol::models::PermissionProfile::External { .. } => true,
        orbiterx_protocol::models::PermissionProfile::Managed { .. } => profile
...
fn build_thread_from_snapshot(
    thread_id: ThreadId,
    session_id: String,
    multi_agent_version: Option<orbiterx_protocol::protocol::MultiAgentVersion>,
    config_snapshot: &ThreadConfigSnapshot,
    path: Option<PathBuf>,
) -> Thread {
