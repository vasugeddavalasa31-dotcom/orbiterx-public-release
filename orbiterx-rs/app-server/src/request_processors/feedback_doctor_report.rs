...
//! the configured OrbiterX executable as a subprocess, accepts only valid JSON from
//! `orbiterx doctor --json`, derives a small set of Sentry tags, and otherwise
...
use orbiterx_feedback::FeedbackAttachment;
...
pub(crate) struct DoctorFeedbackReport {
    /// JSON support report to upload as `orbiterx-doctor-report.json`.
    pub(crate) attachment: FeedbackAttachment,
    /// Low-cardinality Sentry tags derived from the report status and check ids.
    pub(crate) tags: BTreeMap<String, String>,
}

/// Runs `orbiterx doctor --json` and returns a best-effort feedback attachment.
///
/// Failure to spawn OrbiterX, finish before the timeout, or parse JSON means the
