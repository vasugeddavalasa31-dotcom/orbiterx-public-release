...
async fn build_refresh_config(
    ...
    let mcp_servers = orbiterx_mcp::configured_mcp_servers(&mcp_config);
...
mod tests {
    ...
    use orbiterx_config::ThreadConfigContext;
    use orbiterx_config::ThreadConfigLoadError;
    use orbiterx_config::ThreadConfigLoadErrorCode;
    ...
    use orbiterx_extension_api::NoopExtensionEventSink;
    ...
    async fn refresh_config_uses_latest_auth_keyring_backend() -> anyhow::Result<()> {
        ...
        std::fs::write(
            temp_dir.path().join(orbiterx_config::CONFIG_TOML_FILE),
    ...
    async fn refresh_test_state() -> anyhow::Result<(
        ...
        let executor_skill_provider: Arc<dyn orbiterx_skills_extension::SkillProvider> = Arc::new(
            orbiterx_skills_extension::ExecutorSkillProvider::new_with_restriction_product(
        ...
        let thread_manager = Arc::new_cyclic(|thread_manager| {
            ThreadManager::new(
                &good_config,
                auth_manager.clone(),
                orbiterx_core::build_models_manager(&good_config, auth_manager.clone()),
                ...
                thread_extensions(
                    ...
                    ThreadExtensionDependencies {
                        ...
                        goal_service: Arc::new(orbiterx_goal_extension::GoalService::new()),
                ...
                Arc::new(OrbiterXHomeUserInstructionsProvider::new(
                    good_config.orbiterx_home.clone(),
                )),
                /*analytics_events_client*/ None,
                Arc::clone(&thread_store),
                orbiterx_core::local_agent_graph_store_from_state_db(Some(&state_db)),
    ...
    impl ThreadConfigLoader for CountingThreadConfigLoader {
        fn load(
            &self,
            context: ThreadConfigContext,
        ) -> orbiterx_config::ThreadConfigLoaderFuture<'_, Vec<ThreadConfigSource>> {
