...
async fn initialize_uses_client_info_name_as_originator() -> Result<()> {
    ...
    let expected_orbiterx_home = AbsolutePathBuf::try_from(orbiterx_home.path().canonicalize()?)?;
    ...
    let InitializeResponse {
        user_agent,
        orbiterx_home: response_orbiterx_home,
        platform_family,
        platform_os,
    } = to_response::<InitializeResponse>(response)?;

    assert!(user_agent.starts_with("orbiterx_vscode/"));
    assert_eq!(response_orbiterx_home, expected_orbiterx_home);
...
async fn initialize_probe_does_not_override_originator() -> Result<()> {
    ...
    let message = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp.initialize_with_client_info(ClientInfo {
            name: "orbiterx_app_server_daemon".to_string(),
            title: Some("OrbiterX App Server Daemon".to_string()),
            version: "0.1.0".to_string(),
        }),
    )
    .await??;
    ...
    assert!(user_agent.starts_with("orbiterx_cli_rs/"));
    Ok(())
}

#[tokio::test]
async fn initialize_orbiterx_backend_does_not_override_originator() -> Result<()> {
    let responses = Vec::new();
    let server = create_mock_responses_server_sequence_unchecked(responses).await;
    let orbiterx_home = TempDir::new()?;
    create_config_toml(orbiterx_home.path(), &server.uri(), "never")?;
    ...
    let message = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp.initialize_with_client_info(ClientInfo {
            name: "orbiterx-backend".to_string(),
            title: Some("OrbiterX Backend".to_string()),
            version: "0.1.0".to_string(),
        }),
    )
    .await??;
...
async fn initialize_respects_originator_override_env_var() -> Result<()> {
    ...
    let mut mcp = TestAppServer::builder()
        ...
        .with_env_overrides(&[(
            ...
            Some("orbiterx_originator_via_env_var"),
        )])
        .build()
        .await?;
    ...
    assert!(user_agent.starts_with("orbiterx_originator_via_env_var/"));
    assert_eq!(response_orbiterx_home, expected_orbiterx_home);
    assert_eq!(platform_family, std::env::consts::FAMILY);
    assert_eq!(platform_os, std::env::consts::OS);
    Ok(())
...
async fn initialize_rejects_invalid_client_name() -> Result<()> {
    ...
    let mut mcp = TestAppServer::builder()
        .with_orbiterx_home(orbiterx_home.path())
        .without_auto_env()
        .with_env_overrides(&[("ORBITERX_INTERNAL_ORIGINATOR_OVERRIDE", None)])
        .build()
        .await?;
...
async fn turn_start_notify_payload_includes_initialize_client_name() -> Result<()> {
    ...
    let server = create_mock_responses_server_sequence_unchecked(responses).await;
    let orbiterx_home = TempDir::new()?;
    let notify_file = orbiterx_home.path().join("notify.json");
    let notify_capture = cargo_bin("orbiterx-app-server-test-notify-capture")?;
...
fn create_config_toml(
    ...
    create_config_toml_with_extra(orbiterx_home, server_uri, approval_policy, "")
}
