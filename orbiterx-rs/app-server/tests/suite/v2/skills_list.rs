...
use orbiterx_app_server_protocol::SkillsChangedNotification;
...
fn write_cached_remote_plugin_with_skill(
    ...
    let plugin_root = orbiterx_home.join("plugins/cache/openai-curated-remote/linear/local");
    std::fs::create_dir_all(plugin_root.join(".orbiterx-plugin"))?;
    std::fs::write(
        plugin_root.join(".orbiterx-plugin/plugin.json"),
        r#"{"name":"linear"}"#,
...
fn write_cached_local_curated_plugin_with_skill(orbiterx_home: &std::path::Path) -> Result<()> {
    let plugin_root = orbiterx_home.join("plugins/cache/openai-curated/google-calendar/local");
    std::fs::create_dir_all(plugin_root.join(".orbiterx-plugin"))?;
    std::fs::write(
        plugin_root.join(".orbiterx-plugin/plugin.json"),
...
async fn runtime_remote_plugin_toggle_updates_local_curated_plugin_skills() -> Result<()> {
    ...
    write_cached_local_curated_plugin_with_skill(orbiterx_home.path())?;
    std::fs::write(
...
async fn skills_list_loads_remote_installed_plugin_skills_from_cache() -> Result<()> {
    ...
    let expected_skill_path =
        std::fs::canonicalize(write_cached_remote_plugin_with_skill(orbiterx_home.path())?)?;
...
#[tokio::test]
async fn skills_list_excludes_plugin_skills_when_workspace_orbiterx_plugins_disabled() -> Result<()>
{
    let orbiterx_home = TempDir::new()?;
    let repo_root = TempDir::new()?;
    let server = MockServer::start().await;
    write_skill(&orbiterx_home, "home-skill")?;
    ...
    assert!(
        ...
        "plugin skills should be hidden when workspace OrbiterX plugins are disabled"
    );
    Ok(())
}
...
async fn skills_list_skips_cwd_roots_when_environment_disabled() -> Result<()> {
    ...
    let cwd = TempDir::new()?;
    write_skill(&orbiterx_home, "home-skill")?;
    let repo_skill_dir = cwd.path().join(".orbiterx/skills/repo-skill");
...
async fn skills_list_accepts_relative_cwds() -> Result<()> {
    ...
    std::fs::create_dir_all(orbiterx_home.path().join(&relative_cwd))?;
...
async fn skills_list_uses_cached_result_until_force_reload() -> Result<()> {
    ...
    let skill_dir = cwd.path().join(".orbiterx/skills/late-extra-skill");
    std::fs::create_dir_all(&skill_dir)?;
    std::fs::write(
        skill_dir.join("SKILL.md"),
...
async fn skills_changed_notification_is_emitted_after_skill_change() -> Result<()> {
    ...
    write_skill(&orbiterx_home, "demo")?;
