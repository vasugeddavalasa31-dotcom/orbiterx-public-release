...
use orbiterx_app_server_protocol::ExternalAgentConfigImportCompletedNotification;
use orbiterx_app_server_protocol::ExternalAgentConfigImportHistoriesReadResponse;
use orbiterx_app_server_protocol::ExternalAgentConfigImportProgressNotification;
use orbiterx_app_server_protocol::ExternalAgentConfigImportResponse;
use orbiterx_app_server_protocol::ExternalAgentConfigMigrationItemType;
use orbiterx_app_server_protocol::ExternalAgentImportedConnectorCandidate;
use orbiterx_app_server_protocol::ExternalAgentImportedConnectorSource;
...
fn external_agent_home(orbiterx_home: &Path) -> PathBuf {
    orbiterx_home.join(concat!(".", "cla", "ude"))
}
...
fn secondary_external_agent_home(orbiterx_home: &Path) -> PathBuf {
    orbiterx_home.join(concat!(".", "cur", "sor"))
}
...
-> Result<()> {
    ...
    let source_home = external_agent_home(orbiterx_home.path());
    std::fs::create_dir_all(&source_home)?;
    std::fs::write(source_home.join("CLAUDE.md"), "project instructions")?;
    let home_dir = orbiterx_home.path().display().to_string();
...
async fn external_agent_config_migration_source_drives_detect_and_import() -> Result<()> {
    let orbiterx_home = TempDir::new()?;
    let source_home = secondary_external_agent_home(orbiterx_home.path());
    std::fs::create_dir_all(&source_home)?;
    ...
    assert!(
        std::fs::read_to_string(orbiterx_home.path().join("config.toml"))?
...
async fn external_agent_config_import_source_remains_attribution_only() -> Result<()> {
    ...
    assert_eq!(completed.item_type_results[0].failures, Vec::new());
    assert_eq!(
        std::fs::read_to_string(orbiterx_home.path().join("AGENTS.md"))?,
        "OrbiterX guidance"
    );
...
async fn external_agent_config_secondary_source_imports_session_and_plugin_end_to_end() -> Result<()>
{
    let orbiterx_home = TempDir::new()?;
    let source_home = secondary_external_agent_home(orbiterx_home.path());
    let project_root = orbiterx_home.path().join("workspace with.dots_and-dashes");
    ...
    let configured_marketplace_root = orbiterx_home.path().join("configured-marketplace");
    ...
    std::fs::create_dir_all(configured_plugin_root.join(".orbiterx-plugin"))?;
    std::fs::write(
  ...
  "plugins": [{
    ...
    std::fs::write(
        configured_plugin_root.join(".orbiterx-plugin/plugin.json"),
...
"#,
    ...
    assert_eq!(
        marketplace
            .path
            .as_ref()
            .map(orbiterx_config::AbsolutePathBuf::as_path),
...
-> Result<()> {
    ...
    let mut mcp = TestAppServer::builder()
        ...
        .with_env_overrides(&[
            ...
            ("ORBITERX_SQLITE_HOME", Some(sqlite_home_dir.as_str())),
        ])
        .build()
        .await?;
    timeout(DEFAULT_TIMEOUT, mcp.initialize()).await??;
...
async fn external_agent_config_detects_and_imports_project_memory_files() -> Result<()> {
    ...
    let project_cwd = orbiterx_home.path().join("project-a");
    std::fs::create_dir_all(&source_memory)?;
    ...
    let expected_resources_root = orbiterx_home
        .path()
    ...
    let memory_diff = orbiterx_git_utils::diff_since_latest_init(&memory_root).await?;
    ...
    orbiterx_memories_write::workspace::reset_memory_workspace_baseline(&memory_root).await?;
    ...
    assert_eq!(
        ...
        vec![
            (
                orbiterx_git_utils::GitBaselineChangeStatus::Deleted,
                "extensions/external_agent_import/resources/project-a/MEMORY.md",
            ),
            (
                orbiterx_git_utils::GitBaselineChangeStatus::Deleted,
...
async fn external_agent_config_import_reports_failed_sync_import_in_completion() -> Result<()> {
    ...
    std::fs::write(orbiterx_home.path().join("config.toml"), "invalid = [")?;
    let home_dir = orbiterx_home.path().display().to_string();
    let analytics_capture_file = orbiterx_home.path().join("analytics-events.jsonl");
    ...
    let mut mcp = TestAppServer::builder()
        ...
        .with_env_overrides(&[
            ("HOME", Some(home_dir.as_str())),
            (
                "ORBITERX_ANALYTICS_EVENTS_CAPTURE_FILE",
    ...
    let events = timeout(DEFAULT_TIMEOUT, async {
        loop {
            ...
            if captured_events.iter().any(|event| {
                event["event_type"] == "orbiterx_onboarding_external_agent_import_complete"
    ...
    let event = events
        .iter()
        .find(|event| {
            event["event_type"] == "orbiterx_onboarding_external_agent_import_failure"
...
async fn external_agent_config_import_completed_tracks_analytics_event() -> Result<()> {
    ...
    let orbiterx_home = TempDir::new()?;
    write_analytics_config(orbiterx_home.path(), &analytics_server.uri())?;
    write_chatgpt_auth(
        orbiterx_home.path(),
        ChatGptAuthFixture::new("chatgpt-token")
            .account_id("account-123")
    ...
    let missing_session_path =
        external_agent_home(orbiterx_home.path()).join("projects/repo/missing.jsonl");
    let project_root = orbiterx_home.path().join("repo");
    let home_dir = orbiterx_home.path().display().to_string();
    let mut mcp = TestAppServer::builder()
        .with_orbiterx_home(orbiterx_home.path())
        .without_auto_env()
    ...
    let event = wait_for_analytics_event(
        &analytics_server,
        DEFAULT_TIMEOUT,
        "orbiterx_onboarding_external_agent_import_complete",
    )
    .await?;
    let event_params = &event["event_params"];
    ...
    let event = wait_for_analytics_event(
        &analytics_server,
        DEFAULT_TIMEOUT,
        "orbiterx_onboarding_external_agent_import_failure",
    )
    .await?;
    let event_params = &event["event_params"];
    assert_eq!(event_params["import_id"], serde_json::json!(import_id));
...
async fn external_agent_config_import_reinstalls_plugins_from_known_marketplaces() -> Result<()> {
  ...
  "plugins": [
    ...
    assert_eq!(
        ...
        Some(vec![orbiterx_app_server_protocol::PluginsMigration {
    ...
    let event = wait_for_analytics_event(
        &analytics_server,
        DEFAULT_TIMEOUT,
        "orbiterx_plugin_install_failed",
    )
    .await?;
    let event_params = &event["event_params"];
...
async fn external_agent_config_import_creates_session_rollouts() -> Result<()> {
    ...
    let session_dir = external_agent_home(orbiterx_home.path()).join("projects/repo");
    let session_path = session_dir.join("session.jsonl");
    let manifest_dir = connector_metadata_root(orbiterx_home.path())
...
async fn external_agent_config_import_does_not_initialize_required_mcp() -> Result<()> {
    ...
    let orbiterx_home = TempDir::new()?;
    create_config_toml(orbiterx_home.path(), &server.uri())?;
    let mut config = std::fs::read_to_string(orbiterx_home.path().join("config.toml"))?;
    config.push_str(
        r#"
...
"#,
    ...
    std::fs::write(orbiterx_home.path().join("config.toml"), config)?;
    let project_root = orbiterx_home.path().join("repo");
    let recent_timestamp = chrono::Utc::now().to_rfc3339_opts(chrono::SecondsFormat::Secs, true);
    let session_dir = external_agent_home(orbiterx_home.path()).join("projects/repo");
    let session_path = session_dir.join("session.jsonl");
...
fn write_analytics_config(orbiterx_home: &std::path::Path, base_url: &str) -> std::io::Result<()> {
    std::fs::write(
        orbiterx_home.join("config.toml"),
