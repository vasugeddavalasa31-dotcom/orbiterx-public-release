...
async fn turn_start_shell_zsh_fork_executes_command_v2() -> Result<()> {
    ...
    let mut mcp = create_zsh_test_mcp_process(&orbiterx_home, &workspace, &zsh_path).await?;
    ...
    let turn_id = mcp
        .send_turn_start_request(TurnStartParams {
            ...
            model: Some("mock-model".to_string()),
            effort: Some(orbiterx_protocol::openai_models::ReasoningEffort::Medium),
            summary: Some(orbiterx_protocol::config_types::ReasoningSummary::Auto),
            ..Default::default()
        })
        .await?;
    ...
    assert!(
        command.starts_with(
            &command_packaged_zsh_path(&orbiterx_home)
                .display()
                .to_string()
        )
    );
...
async fn turn_start_shell_zsh_fork_subcommand_decline_marks_parent_declined_v2() -> Result<()> {
    ...
    let turn_id = mcp
        .send_turn_start_request(TurnStartParams {
            ...
            cwd: Some(workspace.clone()),
            approval_policy: Some(orbiterx_app_server_protocol::AskForApproval::UnlessTrusted),
...
async fn create_zsh_test_mcp_process(
    ...
    let app_server = create_test_package_app_server(orbiterx_home, zsh_path)?;
...
fn create_test_package_app_server(orbiterx_home: &Path, zsh_path: &Path) -> Result<PathBuf> {
    let package_dir = orbiterx_home.join("test-package");
    let bin_dir = package_dir.join("bin");
    let package_zsh_path = packaged_zsh_path(orbiterx_home);
    ...
    std::fs::write(package_dir.join("orbiterx-package.json"), "{}")?;

    let app_server = bin_dir.join("orbiterx-app-server");
    copy_with_permissions(
        &orbiterx_utils_cargo_bin::cargo_bin("orbiterx-app-server")?,
        &app_server,
    )?;
...
fn packaged_zsh_path(orbiterx_home: &Path) -> PathBuf {
    orbiterx_home
        .join("test-package")
...
fn command_packaged_zsh_path(orbiterx_home: &Path) -> PathBuf {
    let path = packaged_zsh_path(orbiterx_home);
...
fn find_test_zsh_path() -> Result<Option<std::path::PathBuf>> {
    let repo_root = orbiterx_utils_cargo_bin::repo_root()?;
    let dotslash_zsh = repo_root.join("orbiterx-rs/app-server/tests/suite/zsh");
