...
use orbiterx_app_server::AppServerRuntimeOptions;
use orbiterx_app_server::AppServerTransport;
use orbiterx_app_server::AppServerWebsocketAuthSettings;
use orbiterx_app_server::PluginStartupTasks;
use orbiterx_app_server::RemoteControlStartupMode;
use orbiterx_app_server::run_main_with_transport_options;
use orbiterx_app_server_protocol::JSONRPCError;
use orbiterx_app_server_protocol::JSONRPCResponse;
use orbiterx_app_server_protocol::RemoteControlClient;
use orbiterx_app_server_protocol::RemoteControlClientsListOrder;
use orbiterx_app_server_protocol::RemoteControlClientsListParams;
use orbiterx_app_server_protocol::RemoteControlClientsListResponse;
use orbiterx_app_server_protocol::RemoteControlClientsRevokeParams;
use orbiterx_app_server_protocol::RemoteControlClientsRevokeResponse;
use orbiterx_app_server_protocol::RemoteControlConnectionStatus;
use orbiterx_app_server_protocol::RemoteControlDisableResponse;
use orbiterx_app_server_protocol::RemoteControlEnableResponse;
...
use orbiterx_app_server_protocol::RemoteControlPairingStatusResponse;
use orbiterx_app_server_protocol::RemoteControlStatusChangedNotification;
use orbiterx_app_server_protocol::RemoteControlStatusReadResponse;
...
use orbiterx_state::RemoteControlEnrollmentRecord;
...
async fn managed_requirements_reject_all_remote_control_rpcs() -> Result<()> {
    ...
    std::fs::write(
        orbiterx_home.path().join("requirements.toml"),
...
async fn explicit_remote_control_startup_fails_when_disabled_by_requirements() -> Result<()> {
    ...
    let managed_config_path = orbiterx_home.path().join("managed_config.toml");
    let socket_path = orbiterx_home.path().join("app-server.sock");
    let transport =
        AppServerTransport::from_listen_url(&format!("unix://{}", socket_path.display()))?;
    let _orbiterx_home_guard = EnvVarGuard::set("ORBITERX_HOME", orbiterx_home.path().as_os_str());
...
async fn listen_off_honors_persisted_remote_control_enable() -> Result<()> {
    let orbiterx_home = TempDir::new()?;
    let listener = configured_remote_control_listener(orbiterx_home.path()).await?;
...
async fn remote_control_disable_returns_disabled_status() -> Result<()> {
    let orbiterx_home = TempDir::new()?;
    let _listener = configured_remote_control_listener(orbiterx_home.path()).await?;
    let mut mcp = TestAppServer::builder()
        .with_orbiterx_home(orbiterx_home.path())
        .without_auto_env()
        .build()
...
async fn remote_control_enable_returns_connecting_status() -> Result<()> {
    let orbiterx_home = TempDir::new()?;
    let mut backend = BlockingRemoteControlBackend::start(orbiterx_home.path()).await?;
    let mut mcp = TestAppServer::builder()
        .with_orbiterx_home(orbiterx_home.path())
        .without_auto_env()
        .build()
...
async fn remote_control_pairing_start_returns_pairing_artifacts() -> Result<()> {
    let orbiterx_home = TempDir::new()?;
    let mut backend = PairingRemoteControlBackend::start(orbiterx_home.path()).await?;
    let mut mcp = TestAppServer::builder()
        .with_orbiterx_home(orbiterx_home.path())
        .without_auto_env()
        .build()
...
async fn remote_control_client_management_works_while_disabled() -> Result<()> {
    let orbiterx_home = TempDir::new()?;
    let mut backend = ClientManagementRemoteControlBackend::start(orbiterx_home.path()).await?;
    let mut mcp = TestAppServer::builder()
        .with_orbiterx_home(orbiterx_home.path())
        .without_auto_env()
        .build()
...
impl ClientManagementRemoteControlBackend {
    async fn start(orbiterx_home: &std::path::Path) -> Result<Self> {
        let listener = configured_remote_control_listener(orbiterx_home).await?;
