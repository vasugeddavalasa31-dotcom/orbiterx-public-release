...
async fn plugin_install_writes_remote_plugin_to_cloud_and_cache() -> Result<()> {
    ...
    .await;
    configure_remote_plugin_test(orbiterx_home.path(), &server)?;
    ...
    mount_remote_plugin_install_after_cache_write(
        &server,
        REMOTE_PLUGIN_ID,
        installed_path.join(".orbiterx-plugin/plugin.json"),
    )
    .await;
...
async fn plugin_install_uses_remote_apps_needing_auth_response() -> Result<()> {
    ...
    .await;
    configure_remote_plugin_with_apps_test(orbiterx_home.path(), &server)?;
    mount_remote_plugin_detail_with_app_manifest(
        &server,
        REMOTE_PLUGIN_ID,
        "1.2.3",
...
#[tokio::test]
async fn plugin_install_rejects_when_workspace_orbiterx_plugins_disabled() -> Result<()> {
    let orbiterx_home = TempDir::new()?;
    ...
    assert!(
        err.error
            .message
            .contains("OrbiterX plugins are disabled for this workspace")
    );
    Ok(())
}
...
async fn plugin_install_returns_invalid_request_for_missing_marketplace_file() -> Result<()> {
    ...
    let request_id = mcp
        .send_plugin_install_request(PluginInstallParams {
            marketplace_path: Some(AbsolutePathBuf::try_from(
                orbiterx_home.path().join("missing-marketplace.json"),
            )?),
            remote_marketplace_name: None,
...
async fn plugin_install_skips_mcp_oauth_for_chatgpt_dual_surface_plugin() -> Result<()> {
    ...
    let orbiterx_home = TempDir::new()?;
    write_connectors_config(orbiterx_home.path(), &apps_server_url)?;
    write_chatgpt_auth(
        orbiterx_home.path(),
        ChatGptAuthFixture::new("chatgpt-token")
            .account_id("account-123")
...
async fn start_apps_server(
    ...
    let router = Router::new()
        ...
        .with_state(state)
        .nest_service("/api/orbiterx/ps/mcp", mcp_service);
...
async fn mount_backend_analytics_events(server: &MockServer) {
    Mock::given(method("POST"))
        .and(path("/backend-api/orbiterx/analytics-events/events"))
            ...
            if let Some(request) = requests.iter().find(|request| {
                request.method == "POST"
                    && request
                        .url
                        .path()
                        .ends_with("/orbiterx/analytics-events/events")
            }) {
...
fn configure_remote_plugin_test(
    ...
    write_remote_plugin_catalog_config(orbiterx_home, &format!("{}/backend-api/", server.uri()))?;
    write_chatgpt_auth(
        orbiterx_home,
        ChatGptAuthFixture::new("chatgpt-token")
            .account_id("account-123")
