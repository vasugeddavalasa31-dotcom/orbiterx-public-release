...
//! the temporary `orbiterx_home` must not contain rollout session files or sqlite
...
use orbiterx_app_server::in_process::InProcessClientHandle;
use orbiterx_app_server::in_process::InProcessServerEvent;
...
use orbiterx_thread_store::CreateThreadParams as StoreCreateThreadParams;
...
async fn thread_start_rejects_paginated_history_without_list_support() -> Result<()> {
    ...
    create_config_toml_with_thread_store(orbiterx_home.path(), &server.uri(), &store_id)?;
...
async fn thread_delete_with_non_local_thread_store_does_not_create_local_persistence() -> Result<()>
{
    ...
    let orbiterx_home = TempDir::new()?;
    let store_id = Uuid::new_v4().to_string();
    // Plugin startup warmups may create `.tmp` under orbiterx_home. Disable them
    ...
    let mut client = start_in_process_server(orbiterx_home.path()).await?;
    ...
    assert_no_local_persistence_artifacts(orbiterx_home.path())?;
...
async fn start_in_process_server(orbiterx_home: &Path) -> Result<InProcessClientHandle> {
    let loader_overrides = LoaderOverrides::without_managed_config_for_tests();
    let config = Arc::new(
        ConfigBuilder::default()
...
fn assert_no_local_persistence_artifacts(orbiterx_home: &Path) -> Result<()> {
    ...
    // the isolated test orbiterx_home.
    assert!(
        !orbiterx_home.join("sessions").exists(),
        "non-local thread persistence should not create local rollout sessions"
    );
    assert!(
        !orbiterx_home.join("archived_sessions").exists(),
        "non-local thread persistence should not create archived rollout sessions"
    );
    assert!(
        !orbiterx_state::state_db_path(orbiterx_home).exists(),
        "non-local thread persistence should not create local thread sqlite"
    );

    let sqlite_artifacts = std::fs::read_dir(orbiterx_home)?
    ...
    let mut entries = orbiterx_home_entries(orbiterx_home)?;
    // Bazel test runs may initialize shell snapshot storage under orbiterx_home.
    ...
    assert_eq!(
        ...
        "non-local thread persistence should not create unexpected files in orbiterx_home"
    );

    Ok(())
}

fn orbiterx_home_entries(orbiterx_home: &Path) -> Result<BTreeSet<String>> {
    Ok(std::fs::read_dir(orbiterx_home)?
        .filter_map(|entry| {
            let entry = entry.ok()?;
