...
use orbiterx_app_server::in_process;
...
use orbiterx_protocol::protocol::SessionSource as ProtocolSessionSource;
...
use orbiterx_thread_store::AppendThreadItemsParams;
...
async fn thread_turns_list_can_page_backward_and_forward() -> Result<()> {
    ...
    let rollout_path = rollout_path(orbiterx_home.path(), filename_ts, &conversation_id);
...
async fn thread_search_occurrences_reads_paginated_projection() -> Result<()> {
    ...
    let orbiterx_home = TempDir::new()?;
    create_config_toml(orbiterx_home.path(), &server.uri())?;
    let thread_id = orbiterx_protocol::ThreadId::default();
    ...
    let store = LocalThreadStore::new(
        LocalThreadStoreConfig {
            ...
            sqlite_home: orbiterx_home.path().to_path_buf(),
    ...
    store
        .create_thread(CreateThreadParams {
            ...
            history_mode: orbiterx_protocol::protocol::ThreadHistoryMode::Paginated,
            ...
            items: vec![
                ...
                paginated_completed_item(
                    ...
                    CoreTurnItem::UserMessage(UserMessageItem {
                        ...
                        client_id: None,
                        content: vec![
                            orbiterx_protocol::user_input::UserInput::Text {
                                text: "Nee".to_string(),
...
async fn thread_turns_list_reads_store_history_without_rollout_path() -> Result<()> {
    let orbiterx_home = TempDir::new()?;
    let thread_id =
        orbiterx_protocol::ThreadId::from_string("00000000-0000-4000-8000-000000000123")?;
    let store_id = Uuid::new_v4().to_string();
    create_config_toml_with_thread_store(orbiterx_home.path(), &store_id)?;
    ...
    let client = in_process::start(InProcessStartArgs {
        ...
        thread_config_loader: Arc::new(orbiterx_config::NoopThreadConfigLoader),
        ...
        initialize: InitializeParams {
            client_info: ClientInfo {
                name: "orbiterx-app-server-tests".to_string(),
                title: None,
...
async fn thread_read_loaded_include_turns_reads_store_history_without_rollout_path() -> Result<()> {
    ...
    let thread_id = orbiterx_protocol::ThreadId::from_string(&thread.id)?;
    store
        .append_items(AppendThreadItemsParams {
            thread_id,
...
async fn thread_list_includes_store_thread_without_rollout_path() -> Result<()> {
    let orbiterx_home = TempDir::new()?;
    let thread_id =
        orbiterx_protocol::ThreadId::from_string("00000000-0000-4000-8000-000000000124")?;
    let store_id = Uuid::new_v4().to_string();
    create_config_toml_with_thread_store(orbiterx_home.path(), &store_id)?;
    let store = InMemoryThreadStore::for_id(store_id.clone());
    let _in_memory_store = InMemoryThreadStoreId { store_id };
...
async fn thread_resume_initial_turns_page_matches_requested_turns_list_page() -> Result<()> {
    ...
    assert_eq!(
        initial_turns_page,
        Some(orbiterx_app_server_protocol::TurnsPage::from(expected_page))
    );
...
) -> anyhow::Result<orbiterx_app_server_protocol::Turn> {
    let read_id = mcp
        .send_thread_turns_list_request(ThreadTurnsListParams {
            thread_id: thread_id.to_string(),
            cursor: None,
...
async fn read_turns_page(
    mcp: &mut TestAppServer,
    thread_id: orbiterx_protocol::ThreadId,
...
fn turn_user_texts(turns: &[orbiterx_app_server_protocol::Turn]) -> Vec<&str> {
    turns
        .iter()
...
fn turn_agent_texts(turns: &[orbiterx_app_server_protocol::Turn]) -> Vec<&str> {
    turns
        .iter()
...
fn create_config_toml(orbiterx_home: &Path, server_uri: &str) -> std::io::Result<()> {
    let config_toml = orbiterx_home.join("config.toml");
    std::fs::write(
        config_toml,
        format!(
