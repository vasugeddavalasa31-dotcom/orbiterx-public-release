...
use orbiterx_app_server_protocol::AppConfig;
use orbiterx_app_server_protocol::AppToolApproval;
use orbiterx_app_server_protocol::ApprovalsReviewer;
use orbiterx_app_server_protocol::AppsConfig;
use orbiterx_app_server_protocol::AppsDefaultConfig;
use orbiterx_app_server_protocol::AskForApproval;
use orbiterx_app_server_protocol::ConfigBatchWriteParams;
use orbiterx_app_server_protocol::ConfigEdit;
use orbiterx_app_server_protocol::ConfigLayerSource;
...
use orbiterx_app_server_protocol::ForcedChatgptWorkspaceIds;
...
use orbiterx_app_server_protocol::ToolsV2;
...
use orbiterx_protocol::config_types::WebSearchLocation;
...
fn write_config(orbiterx_home: &TempDir, contents: &str) -> Result<()> {
    Ok(std::fs::write(
...
"#,
    ...
    let orbiterx_home_path = orbiterx_home.path().canonicalize()?;
    let user_file = AbsolutePathBuf::try_from(orbiterx_home_path.join("config.toml"))?;
...
"#,
    ...
    assert_eq!(desktop.get("selected-avatar-id"), Some(&json!("orbiterx")));
    assert_eq!(
...
async fn config_read_includes_project_layers_for_cwd() -> Result<()> {
    let orbiterx_home = TempDir::new()?;
    write_config(&orbiterx_home, r#"model = "gpt-user""#)?;
...
"#,
    ...
    set_project_trust_level(orbiterx_home.path(), workspace.path(), TrustLevel::Trusted)?;
    ...
    assert_eq!(
        ...
        ConfigLayerSource::Project {
            dot_orbiterx_folder: project_config
        }
    );
...
async fn config_value_write_replaces_value() -> Result<()> {
    ...
    let orbiterx_home = temp_dir.path().canonicalize()?;
    write_config(
        &temp_dir,
        r#"
model = "gpt-old"
"#,
    ...
    let expected_file_path =
        AbsolutePathBuf::resolve_path_against_base("config.toml", orbiterx_home);
...
async fn config_batch_write_applies_multiple_edits() -> Result<()> {
    ...
    let orbiterx_home = tmp_dir.path().canonicalize()?;
    ...
    let batch_id = mcp
        .send_config_batch_write_request(ConfigBatchWriteParams {
            file_path: Some(orbiterx_home.join("config.toml").display().to_string()),
            edits: vec![
                ConfigEdit {
...
"#,
    ...
    let config: toml::Value =
        toml::from_str(&std::fs::read_to_string(orbiterx_home.join("config.toml"))?)?;
    assert_eq!(
...
fn assert_layers_user_then_optional_system(
    layers: &[orbiterx_app_server_protocol::ConfigLayer],
