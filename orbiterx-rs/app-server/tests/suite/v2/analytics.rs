...
use orbiterx_config::types::OtelHttpProtocol;
...
fn set_metrics_exporter(config: &mut orbiterx_core::config::Config) {
...
pub(crate) async fn wait_for_goal_event(
    ...
    wait_for_matching_analytics_event(server, read_timeout, |event| {
        event["event_type"] == "orbiterx_goal_event"
...
pub(crate) fn thread_initialized_event(payload: &Value) -> Result<&Value> {
    ...
    events
        ...
        .ok_or_else(|| anyhow::anyhow!("orbiterx_thread_initialized event should be present"))
}
