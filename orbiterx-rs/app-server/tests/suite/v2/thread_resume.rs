...
use orbiterx_app_server_protocol::ApprovalsReviewer;
...
use orbiterx_app_server_protocol::JSONRPCError;
...
use orbiterx_core::ARCHIVED_SESSIONS_SUBDIR;
use orbiterx_login::REFRESH_TOKEN_URL_OVERRIDE_ENV_VAR;
...
use orbiterx_protocol::user_input::ByteRange;
use orbiterx_protocol::user_input::TextElement;
use orbiterx_rollout::append_rollout_item_to_path;
use orbiterx_rollout::read_session_meta_line;
...
const ORBITERX_5_2_INSTRUCTIONS_TEMPLATE_DEFAULT: &str = "You are OrbiterX, a coding agent based on GPT-5. You and the user share the same workspace and collaborate to achieve the user's goals.";

#[tokio::test]
async fn thread_resume_paginated_uses_model_context_without_history() -> Result<()> {
    let server = create_mock_responses_server_repeating_assistant("Done").await;
    let orbiterx_home = TempDir::new()?;
    create_config_toml(orbiterx_home.path(), &server.uri())?;
...
-> Result<()> {
    ...
    let thread_id = {
        ...
        let update_id = mcp
            .send_thread_settings_update_request(ThreadSettingsUpdateParams {
                thread_id: thread.id.clone(),
                model: Some("gpt-5.2-orbiterx".to_string()),
    ...
    assert_eq!(model, "gpt-5.2-orbiterx");
    ...
    let update_id = mcp
        .send_thread_settings_update_request(ThreadSettingsUpdateParams {
            ...
            collaboration_mode: Some(CollaborationMode {
                ...
                settings: Settings {
                    model: "gpt-5.2-orbiterx".to_string(),
...
async fn goal_first_live_thread_appears_in_state_db_thread_list() -> Result<()> {
    let server = create_mock_responses_server_repeating_assistant("Done").await;
    let orbiterx_home = TempDir::new()?;
    let orbiterx_home_path = normalized_existing_path(orbiterx_home.path())?;
    create_config_toml(&orbiterx_home_path, &server.uri())?;
    let config_path = orbiterx_home_path.join("config.toml");
    let config = std::fs::read_to_string(&config_path)?;
    std::fs::write(
        &config_path,
        config.replace("personality = true\n", "personality = true\ngoals = true\n"),
    ...
    let sqlite_home = orbiterx_home_path
        .as_path()
        .to_str()
        .expect("test orbiterx home should be utf-8");
    let mut mcp = TestAppServer::builder()
        .with_orbiterx_home(&orbiterx_home_path)
        .without_managed_config()
        .with_env_overrides(&[("ORBITERX_SQLITE_HOME", Some(sqlite_home))])
        .build()
        .await?;
    timeout(DEFAULT_READ_TIMEOUT, mcp.initialize()).await??;
...
async fn thread_resume_tracks_thread_initialized_analytics() -> Result<()> {
    ...
    let orbiterx_home = TempDir::new()?;
    create_config_toml_with_chatgpt_base_url(orbiterx_home.path(), &server.uri(), &server.uri())?;
    mount_analytics_capture(&server, orbiterx_home.path()).await?;
...
async fn thread_resume_running_thread_tracks_thread_originator_in_analytics() -> Result<()> {
    ...
    let start_id = mcp
        .send_thread_start_request_with_auto_env(ThreadStartParams {
            ...
            service_name: Some("orbiterx_work_desktop".to_string()),
            ..Default::default()
        })
        .await?;
    let start_resp: JSONRPCResponse = timeout(
    ...
    let event = wait_for_matching_analytics_event(&server, DEFAULT_READ_TIMEOUT, |event| {
        event["event_type"] == "orbiterx_thread_initialized"
...
fn set_session_meta_on_fake_rollout(
    ...
    let path = rollout_path(orbiterx_home, filename_ts, thread_id);
...
async fn thread_resume_redacts_payloads_for_chatgpt_remote_clients() -> Result<()> {
    for client_name in [
        "orbiterx_chatgpt_android_remote",
        "orbiterx_chatgpt_ios_remote",
    ] {
...
fn append_resume_redaction_history(
    ...
    let rollout_file_path = rollout_path(orbiterx_home, filename_ts, conversation_id);
...
async fn thread_resume_rejects_archived_session_by_id() -> Result<()> {
    ...
    let active_rollout_path = rollout_path(orbiterx_home.path(), filename_ts, &conversation_id);
    let archived_dir = orbiterx_home.path().join(ARCHIVED_SESSIONS_SUBDIR);
    ...
    assert!(
        message.contains(&format!("session {conversation_id} is archived"))
            && message.contains(&format!(
                "orbiterx unarchive {conversation_id}` to unarchive it first"
            )),
...
async fn thread_goal_set_edits_objective_without_resetting_usage() -> Result<()> {
    ...
    let state_db =
        StateRuntime::init(orbiterx_home.path().to_path_buf(), "mock_provider".into()).await?;
    ...
    state_db
        ...
        .account_thread_goal_usage(
            ...
            /*token_delta*/ 50,
            orbiterx_state::GoalAccountingMode::ActiveOnly,
...
async fn thread_goal_lifecycle_emits_analytics_and_clear_deletes_goal() -> Result<()> {
    ...
    .await??;
    let get: orbiterx_app_server_protocol::ThreadGoalGetResponse = to_response(get_resp)?;
...
async fn thread_resume_token_usage_replay_ignores_stale_interrupted_tail_turn() -> Result<()> {
    ...
    let rollout_file_path = rollout_path(orbiterx_home.path(), filename_ts, &conversation_id);
    let persisted_rollout = std::fs::read_to_string(&rollout_file_path)?;
...
async fn thread_resume_prefers_persisted_git_metadata_for_local_threads() -> Result<()> {
    let server = create_mock_responses_server_repeating_assistant("Done").await;
    let orbiterx_home = TempDir::new()?;
    let config_toml = orbiterx_home.path().join("config.toml");
    std::fs::write(
        &config_toml,
        format!(
            r#"
...
"#,
    ...
    let repo_path = orbiterx_home.path().join("repo");
    ...
    let rollout_path = rollout_path(orbiterx_home.path(), "2025-01-05T12-00-00", &thread_id);
...
async fn thread_resume_defers_updated_at_until_turn_start() -> Result<()> {
    let server = create_mock_responses_server_repeating_assistant("Done").await;
    let orbiterx_home = TempDir::new()?;
    let rollout = setup_rollout_fixture(orbiterx_home.path(), &server.uri()).await?;
    ...
    let resume_id = mcp
        .send_thread_resume_request(ThreadResumeParams {
            ...
            cwd: Some(orbiterx_home.path().to_string_lossy().to_string()),
            ..Default::default()
        })
        .await?;
    let resume_resp: JSONRPCResponse = timeout(
    ...
    assert_eq!(
        cwd,
        AbsolutePathBuf::from_absolute_path(orbiterx_home.path())?
    );
...
async fn thread_resume_replays_pending_file_change_request_approval() -> Result<()> {
    ...
    let tmp = TempDir::new()?;
    let orbiterx_home = tmp.path().join("orbiterx_home");
    std::fs::create_dir(&orbiterx_home)?;
...
"#;
    ...
    let server = create_mock_responses_server_sequence_unchecked(responses).await;
    create_config_toml(&orbiterx_home, &server.uri())?;

    let mut primary = TestAppServer::builder()
        .with_orbiterx_home(&orbiterx_home)
        .build()
        .await?;
    timeout(DEFAULT_READ_TIMEOUT, primary.initialize()).await??;
    ...
    let expected_file_change = ThreadItem::FileChange {
        ...
        changes: vec![orbiterx_app_server_protocol::FileUpdateChange {
...
async fn thread_resume_with_overrides_defers_updated_at_until_turn_start() -> Result<()> {
    ...
    let RestartedThreadFixture {
        ...
        rollout_file_path,
        updated_at,
    } = start_materialized_thread_and_restart(orbiterx_home.path(), "materialize").await?;
...
async fn thread_resume_fails_when_required_mcp_server_fails_to_initialize() -> Result<()> {
    ...
    let orbiterx_home = TempDir::new()?;
    let rollout = setup_rollout_fixture(orbiterx_home.path(), &server.uri()).await?;
    create_config_toml_with_required_broken_mcp(orbiterx_home.path(), &server.uri())?;
...
async fn thread_resume_supports_history_and_overrides() -> Result<()> {
    ...
    let RestartedThreadFixture {
        mut mcp, thread_id, ..
    } = start_materialized_thread_and_restart(orbiterx_home.path(), "seed history").await?;
...
async fn thread_resume_accepts_personality_override() -> Result<()> {
    ...
    assert!(
        instructions_text.contains(ORBITERX_5_2_INSTRUCTIONS_TEMPLATE_DEFAULT),
...
fn create_config_toml(orbiterx_home: &std::path::Path, server_uri: &str) -> std::io::Result<()> {
...
command = "orbiterx-definitely-not-a-real-binary"
required = true
"#
        ),
    )
...
async fn setup_rollout_fixture(orbiterx_home: &Path, server_uri: &str) -> Result<RolloutFixture> {
    create_config_toml(orbiterx_home, server_uri)?;
