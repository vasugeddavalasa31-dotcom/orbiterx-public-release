...
async fn installed_apps_force_refresh_only_refreshes_tools_snapshot() -> Result<()> {
    ...
    let orbiterx_home = configured_orbiterx_home(fixture.base_url())?;
    let mut app_server = start_app_server(orbiterx_home.path()).await?;
...
async fn start_app_server(orbiterx_home: &Path) -> Result<TestAppServer> {
...
fn configured_orbiterx_home(base_url: &str) -> Result<TempDir> {
