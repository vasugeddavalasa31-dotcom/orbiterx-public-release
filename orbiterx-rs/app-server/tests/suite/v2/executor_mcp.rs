...
async fn selected_executor_plugin_exposes_its_mcps_only_to_that_thread() -> Result<()> {
    ...
    let orbiterx_bin = toml::Value::String(
        orbiterx_utils_cargo_bin::cargo_bin("orbiterx")?
            .to_string_lossy()
            .into_owned(),
    );
...
"#
    ...
    std::fs::create_dir_all(plugin.path().join(".orbiterx-plugin"))?;
    std::fs::write(
        plugin.path().join(".orbiterx-plugin/plugin.json"),
