...
async fn get_account_rate_limits_returns_snapshot() -> Result<()> {
    ...
    write_chatgpt_base_url(orbiterx_home.path(), &server_url)?;
    ...
    let response_body = json!({
        ...
        "additional_rate_limits": [
            {
                "limit_name": "orbiterx_other",
                "metered_feature": "orbiterx_other",
                "rate_limit": {
                    "allowed": true,
                    "limit_reached": false,
                    "primary_window": {
    ...
    Mock::given(method("GET"))
        ...
        .await;
    Mock::given(method("GET"))
        .and(path("/api/orbiterx/rate-limit-reset-credits"))
        .and(header("authorization", "Bearer chatgpt-token"))
        .and(header("chatgpt-account-id", "account-123"))
        .respond_with(ResponseTemplate::new(200).set_body_json(json!({
            "credits": [
                {
                    "id": "credit-1",
                    "reset_type": "orbiterx_rate_limits",
                    "status": "available",
        ...
        rate_limit_reset_credits: Some(RateLimitResetCreditsSummary {
            ...
            credits: Some(vec![
                RateLimitResetCredit {
                    ...
                    reset_type: RateLimitResetType::OrbiterXRateLimits,
...
async fn get_account_rate_limits_preserves_count_when_reset_credit_details_fail() -> Result<()> {
    ...
    let server = MockServer::start().await;
    write_chatgpt_base_url(orbiterx_home.path(), &server.uri())?;
...
async fn send_add_credits_nudge_email_posts_expected_body() -> Result<()> {
    ...
    Mock::given(method("POST"))
        .and(path("/api/orbiterx/accounts/send_add_credits_nudge_email"))
        .and(header("authorization", "Bearer chatgpt-token"))
        .and(header("chatgpt-account-id", "account-123"))
...
fn write_chatgpt_base_url(orbiterx_home: &Path, base_url: &str) -> std::io::Result<()> {
