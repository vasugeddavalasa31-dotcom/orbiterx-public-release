...
use orbiterx_app_server::INPUT_TOO_LARGE_ERROR_CODE;
use orbiterx_app_server::INVALID_PARAMS_ERROR_CODE;
use orbiterx_app_server_protocol::AdditionalContextEntry;
use orbiterx_app_server_protocol::AdditionalContextKind;
use orbiterx_app_server_protocol::ByteRange;
...
use orbiterx_app_server_protocol::FileChangePatchUpdatedNotification;
...
use orbiterx_app_server_protocol::TextElement;
...
use orbiterx_core::test_support::all_model_presets;
...
use orbiterx_protocol::protocol::MULTI_AGENT_MODE_OPEN_TAG;
...
const TEST_ORIGINATOR: &str = "orbiterx_vscode";
...
async fn run_local_image_turn(detail: Option<ImageDetail>) -> Result<Vec<Value>> {
    // Two OrbiterX turns hit the mock model (session start + turn/start).
    let responses = vec![
    ...
    let image_path = orbiterx_home.path().join("image.png");
...
async fn turn_start_sends_originator_header() -> Result<()> {
    ...
    timeout(
        ...
        mcp.initialize_with_client_info(ClientInfo {
            ...
            title: Some("OrbiterX VS Code Extension".to_string()),
...
async fn turn_start_emits_thread_scoped_warning_notification_for_trimmed_skills() -> Result<()> {
    ...
    write_models_cache(orbiterx_home.path())?;
    let cache_path = orbiterx_home.path().join("models_cache.json");
    ...
    write_test_skill(orbiterx_home.path(), "alpha-skill")?;
    write_test_skill(orbiterx_home.path(), "beta-skill")?;

    let isolated_home = orbiterx_home.path().to_string_lossy();
    let mut mcp = TestAppServer::builder()
        .with_orbiterx_home(orbiterx_home.path())
        .with_env_overrides(&[
...
async fn turn_start_emits_raw_response_completed_with_upstream_usage() -> Result<()> {
    ...
    .await??;
    let notification: orbiterx_app_server_protocol::ServerNotification = notification.try_into()?;
    let orbiterx_app_server_protocol::ServerNotification::RawResponseCompleted(notification) =
        notification
    else {
...
async fn turn_start_tracks_thread_originator_in_analytics() -> Result<()> {
    ...
    let event =
        wait_for_analytics_event(&server, DEFAULT_READ_TIMEOUT, "orbiterx_turn_event").await?;
    ...
    let turn_req = mcp
        .send_turn_start_request(TurnStartParams {
            ...
            environments: Some(vec![TurnEnvironmentParams {
                ...
                cwd: orbiterx_utils_absolute_path::AbsolutePathBuf::try_from(
...
async fn turn_start_emits_notifications_and_accepts_model_override() -> Result<()> {
    ...
    // Three OrbiterX turns hit the mock model (session start + two turn/start calls).
    let responses = vec![
        create_final_assistant_message_sse_response("Done")?,
        create_final_assistant_message_sse_response("Done")?,
        create_final_assistant_message_sse_response("Done")?,
    ...
    assert_eq!(
        started.turn.status,
        orbiterx_app_server_protocol::TurnStatus::InProgress
    );
    assert_eq!(started.turn.id, turn.id);
...
async fn turn_start_accepts_personality_override_v2() -> Result<()> {
    ...
    let thread_req = mcp
        .send_thread_start_request_with_auto_env(ThreadStartParams {
            model: Some("exp-orbiterx-personality".to_string()),
            ..Default::default()
        })
        .await?;
    let thread_resp: JSONRPCResponse = timeout(
...
async fn turn_start_exec_approval_toggle_v2() -> Result<()> {
    ...
    let tmp = TempDir::new()?;
    let orbiterx_home = tmp.path().to_path_buf();
    ...
    let mut mcp = TestAppServer::builder()
        .with_orbiterx_home(orbiterx_home.as_path())
        .build()
        .await?;
    timeout(DEFAULT_READ_TIMEOUT, mcp.initialize()).await??;
    ...
    let second_turn_id = mcp
        .send_turn_start_request(TurnStartParams {
            ...
            approval_policy: Some(orbiterx_app_server_protocol::AskForApproval::Never),
            sandbox_policy: Some(orbiterx_app_server_protocol::SandboxPolicy::DangerFullAccess),
            model: Some("mock-model".to_string()),
...
async fn turn_start_explicit_local_environment_updates_legacy_cwd_between_turns() -> Result<()> {
    ...
    let first_turn = mcp
        .send_turn_start_request(TurnStartParams {
            ...
            sandbox_policy: Some(
                orbiterx_app_server_protocol::SandboxPolicy::WorkspaceWrite {
...
async fn turn_start_permission_profile_rebinds_runtime_workspace_roots_between_turns() -> Result<()>
{
    ...
    let old_root = orbiterx_utils_absolute_path::AbsolutePathBuf::from_absolute_path(old_root)?;
    let new_root = orbiterx_utils_absolute_path::AbsolutePathBuf::from_absolute_path(new_root)?;
...
async fn turn_start_resolves_sticky_thread_local_environment_and_turn_overrides() -> Result<()> {
    ...
    let server = create_mock_responses_server_repeating_assistant("done").await;
    create_config_toml(&orbiterx_home, &server.uri(), "never", &BTreeMap::default())?;
    std::fs::write(
        orbiterx_home.join("environments.toml"),
        r#"
[[environments]]
id = "remote"
url = "ws://127.0.0.1:1"
...
"#;
    ...
    pretty_assertions::assert_eq!(
        started_changes,
        vec![orbiterx_app_server_protocol::FileUpdateChange {
...
async fn turn_start_streams_apply_patch_change_updates_v2() -> Result<()> {
    ...
    write_models_cache(&orbiterx_home)?;
    let cache_path = orbiterx_home.join("models_cache.json");
    let mut cache: serde_json::Value =
        serde_json::from_str(&std::fs::read_to_string(&cache_path)?)?;
    let models = cache["models"]
        .as_array_mut()
...
async fn turn_start_emits_spawn_agent_item_with_effective_role_model_metadata_v2() -> Result<()> {
    ...
    std::fs::write(
        orbiterx_home.path().join("custom-role.toml"),
...
async fn turn_start_with_elevated_override_does_not_persist_project_trust() -> Result<()> {
    ...
    let config_toml = std::fs::read_to_string(orbiterx_home.path().join("config.toml"))?;
...
fn write_test_skill(orbiterx_home: &Path, name: &str) -> std::io::Result<()> {
    let skill_dir = orbiterx_home.join("skills").join(name);
