...
use orbiterx_app_server_protocol::AppBranding;
...
use orbiterx_app_server_protocol::AppReview;
use orbiterx_app_server_protocol::AppScreenshot;
...
#[tokio::test]
async fn list_apps_returns_empty_when_workspace_orbiterx_plugins_disabled() -> Result<()> {
    let connectors = vec![AppInfo {
        id: "beta".to_string(),
        name: "Beta".to_string(),
        description: Some("Beta connector".to_string()),
...
async fn list_apps_includes_plugin_apps_for_chatgpt_auth() -> Result<()> {
    ...
    let orbiterx_home = TempDir::new()?;
    write_connectors_and_plugins_config(orbiterx_home.path(), &server_url)?;
    write_plugin_app_fixture(orbiterx_home.path(), "sample", "connector_sample")?;
    write_chatgpt_auth(
        orbiterx_home.path(),
        ChatGptAuthFixture::new("chatgpt-token")
            .account_id("account-123")
...
fn write_plugin_app_fixture(orbiterx_home: &Path, plugin_name: &str, app_id: &str) -> Result<()> {
