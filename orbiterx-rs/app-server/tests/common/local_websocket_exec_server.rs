...
impl LocalWebsocketExecServer {
    pub(crate) async fn start(orbiterx_home: &Path, exec_server_program: &Path) -> Result<Self> {
        let mut command = Command::new(exec_server_program);
        command.stdin(Stdio::null());
        command.stdout(Stdio::piped());
        command.stderr(Stdio::inherit());
        command.current_dir(orbiterx_home);
        command.env("ORBITERX_HOME", orbiterx_home);
        #[cfg(target_os = "linux")]
        command.env(
            ORBITERX_LINUX_SANDBOX_EXE_ENV_VAR,
            core_test_support::find_orbiterx_linux_sandbox_exe()
