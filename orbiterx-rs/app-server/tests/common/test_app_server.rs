...
use core_test_support::test_orbiterx::TestEnv;
use core_test_support::test_orbiterx::test_env;
...
use orbiterx_app_server_protocol::ConfigReadParams;
use orbiterx_app_server_protocol::ConfigValueWriteParams;
...
use orbiterx_app_server_protocol::FsCopyParams;
use orbiterx_app_server_protocol::FsCreateDirectoryParams;
use orbiterx_app_server_protocol::FsGetMetadataParams;
use orbiterx_app_server_protocol::FsReadDirectoryParams;
use orbiterx_app_server_protocol::FsReadFileParams;
use orbiterx_app_server_protocol::FsRemoveParams;
use orbiterx_app_server_protocol::FsUnwatchParams;
use orbiterx_app_server_protocol::FsWatchParams;
use orbiterx_app_server_protocol::FsWriteFileParams;
...
use orbiterx_app_server_protocol::ModelProviderCapabilitiesReadParams;
...
use orbiterx_app_server_protocol::ProcessKillParams;
use orbiterx_app_server_protocol::ProcessResizePtyParams;
use orbiterx_app_server_protocol::ProcessSpawnParams;
use orbiterx_app_server_protocol::ProcessWriteStdinParams;
use orbiterx_app_server_protocol::RemoteControlClientsListParams;
use orbiterx_app_server_protocol::RemoteControlClientsRevokeParams;
use orbiterx_app_server_protocol::RemoteControlPairingStartParams;
use orbiterx_app_server_protocol::RemoteControlPairingStatusParams;
...
use orbiterx_app_server_protocol::ThreadRealtimeListVoicesParams;
...
use orbiterx_exec_server::ORBITERX_EXEC_SERVER_NOISE_AUTH_TOKEN_ENV_VAR;
use orbiterx_exec_server::ORBITERX_EXEC_SERVER_NOISE_CHATGPT_ACCOUNT_ID_ENV_VAR;
use orbiterx_exec_server::ORBITERX_EXEC_SERVER_NOISE_ENVIRONMENT_ID_ENV_VAR;
use orbiterx_exec_server::ORBITERX_EXEC_SERVER_NOISE_REGISTRY_URL_ENV_VAR;
use orbiterx_exec_server::ORBITERX_EXEC_SERVER_URL_ENV_VAR;
...
pub struct TestAppServer {
    ...
    // removing an owned ORBITERX_HOME that may still be its cwd on Windows.
    _delayed_exec_server: Option<(LocalWebsocketExecServer, WebsocketDelayInterposer)>,
    _owned_orbiterx_home: Option<TempDir>,
}

pub const DEFAULT_CLIENT_NAME: &str = "orbiterx-app-server-tests";
pub const DISABLE_PLUGIN_STARTUP_TASKS_ARG: &str = "--disable-plugin-startup-tasks-for-tests";
const DISABLE_MANAGED_CONFIG_ENV_VAR: &str = "ORBITERX_APP_SERVER_DISABLE_MANAGED_CONFIG";
const CODE_MODE_HOST_PATH_ENV_VAR: &str = "ORBITERX_CODE_MODE_HOST_PATH";

impl TestAppServer {
    /// Starts building a server with a temporary ORBITERX_HOME and the standard
    ...
    /// Returns the effective ORBITERX_HOME used by the child app-server.
    pub fn orbiterx_home(&self) -> &Path {
        &self.orbiterx_home
    }
    ...
    async fn new_with_program_env_and_args(
        ...
        cmd.env(
            ...
            orbiterx_home.join("managed_config.toml"),
        );
        cmd.env_remove(ORBITERX_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR);
        cmd.args(args);
        ...
        let mut process = cmd
            .kill_on_drop(true)
            .spawn()
            .context("orbiterx-mcp-server proc should start")?;
        let stdin = process
            .stdin
            .take()
    ...
    pub async fn send_experimental_feature_enablement_set_request(
        &mut self,
        params: orbiterx_app_server_protocol::ExperimentalFeatureEnablementSetParams,
    ) -> anyhow::Result<i64> {
        let params = Some(serde_json::to_value(params)?);
...
impl TestAppServerBuilder {
    /// Uses this existing ORBITERX_HOME instead of a temporary one.
    pub fn with_orbiterx_home(mut self, orbiterx_home: &Path) -> Self {
        self.orbiterx_home = Some(orbiterx_home.to_path_buf());
        self
    }
    ...
    /// Builds a server with a temporary ORBITERX_HOME and automatic environment
    /// by default.
    pub async fn build(self) -> anyhow::Result<TestAppServer> {
        ...
        let (orbiterx_home, owned_orbiterx_home) = match orbiterx_home {
            Some(orbiterx_home) => (orbiterx_home, None),
            None => {
                let owned_orbiterx_home = TempDir::new()?;
                (
                    owned_orbiterx_home.path().to_path_buf(),
                    Some(owned_orbiterx_home),
                )
            }
        };
        let (auto_env, delayed_exec_server) = match environment {
            TestAppServerEnvironment::Auto => {
                let environments_toml = orbiterx_home.join("environments.toml");
                ensure!(
                ...
                let (auto_env, delayed_exec_server) = match exec_server_delay {
                    Some(added_delay) => {
                        ...
                        let exec_server_program =
                            orbiterx_utils_cargo_bin::cargo_bin("exec-server")
                        ...
                        let local_websocket_exec_server =
                            LocalWebsocketExecServer::start(&orbiterx_home, &exec_server_program)
                                .await?;
                ...
                let mut auto_env_overrides = vec![
                    (
                        ORBITERX_EXEC_SERVER_URL_ENV_VAR.to_string(),
                        auto_env.exec_server_url().map(str::to_string),
                    ),
                    (
                        ORBITERX_EXEC_SERVER_NOISE_REGISTRY_URL_ENV_VAR.to_string(),
                        None,
                    ),
                    (
                        ORBITERX_EXEC_SERVER_NOISE_ENVIRONMENT_ID_ENV_VAR.to_string(),
                        None,
                    ),
                    (
                        ORBITERX_EXEC_SERVER_NOISE_AUTH_TOKEN_ENV_VAR.to_string(),
                        None,
                    ),
                    (
                        ORBITERX_EXEC_SERVER_NOISE_CHATGPT_ACCOUNT_ID_ENV_VAR.to_string(),
                        None,
                    ),
                ];
        ...
        if !env_overrides
            ...
            && let Ok(code_mode_host_program) =
                orbiterx_utils_cargo_bin::cargo_bin("orbiterx-code-mode-host")
        {
        ...
        let program = match program {
            ...
            None => orbiterx_utils_cargo_bin::cargo_bin("orbiterx-app-server")
                .context("should find binary for orbiterx-app-server")?,
        };
        ...
        app_server._owned_orbiterx_home = owned_orbiterx_home;
...
impl Drop for TestAppServer {
    fn drop(&mut self) {
        // These tests spawn a `orbiterx-app-server` child process.
        //
