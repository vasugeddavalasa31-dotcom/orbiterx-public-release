...
pub enum Command {
    /// Submit a new OrbiterX Cloud task without launching the TUI.
    Exec(ExecCommand),
    /// Show the status of a OrbiterX Cloud task.
    Status(StatusCommand),
    /// List OrbiterX Cloud tasks.
    List(ListCommand),
    /// Apply the diff for a OrbiterX Cloud task locally.
    Apply(ApplyCommand),
    /// Show the unified diff for a OrbiterX Cloud task.
    Diff(DiffCommand),
}

#[derive(Debug, Args)]
pub struct ExecCommand {
    /// Task prompt to run in OrbiterX Cloud.
    #[arg(value_name = "QUERY")]
    pub query: Option<String>,

    /// Target environment identifier (see `orbiterx cloud` to browse).
    ...
    /// Git branch to run in OrbiterX Cloud (defaults to current branch).
...
pub struct StatusCommand {
    /// OrbiterX Cloud task identifier to inspect.
...
pub struct ApplyCommand {
    /// OrbiterX Cloud task identifier to apply.
    #[arg(value_name = "TASK_ID")]
    pub task_id: String,
...
pub struct DiffCommand {
    /// OrbiterX Cloud task identifier to display.
    #[arg(value_name = "TASK_ID")]
    pub task_id: String,
