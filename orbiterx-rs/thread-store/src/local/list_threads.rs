...
use orbiterx_rollout::RolloutConfig;
use orbiterx_rollout::RolloutRecorder;
use orbiterx_rollout::find_thread_names_by_ids;
use orbiterx_rollout::parse_cursor;
...
pub(super) async fn list_threads(
    ...
    let sort_key = match params.sort_key {
        ThreadSortKey::CreatedAt => orbiterx_rollout::ThreadSortKey::CreatedAt,
        ThreadSortKey::UpdatedAt => orbiterx_rollout::ThreadSortKey::UpdatedAt,
        ThreadSortKey::RecencyAt => orbiterx_rollout::ThreadSortKey::RecencyAt,
    };
    let sort_direction = match params.sort_direction {
        SortDirection::Asc => orbiterx_rollout::SortDirection::Asc,
        SortDirection::Desc => orbiterx_rollout::SortDirection::Desc,
    };
    let state_db = store.state_db().await;
    let rollout_config = RolloutConfig {
        orbiterx_home: store.config.orbiterx_home.clone(),
        sqlite_home: store.config.sqlite_home.clone(),
        cwd: store.config.orbiterx_home.clone(),
    ...
    if names.len() < thread_ids.len()
        && let Ok(legacy_names) =
            find_thread_names_by_ids(store.config.orbiterx_home.as_path(), &thread_ids).await
    {
...
pub(super) async fn list_rollout_threads(
    state_db: Option<orbiterx_rollout::StateDbHandle>,
    config: &RolloutConfig,
    default_model_provider_id: &str,
    params: &ListThreadsParams,
    cursor: Option<&orbiterx_rollout::Cursor>,
    sort_key: orbiterx_rollout::ThreadSortKey,
    sort_direction: orbiterx_rollout::SortDirection,
) -> ThreadStoreResult<orbiterx_rollout::ThreadsPage> {
    if let Some(relation_filter) = params.relation_filter {
        let relation_filter = match relation_filter {
            ThreadRelationFilter::DirectChildrenOf(parent_thread_id) => {
                orbiterx_state::ThreadRelationFilter::DirectChildrenOf(parent_thread_id)
            }
            ThreadRelationFilter::DescendantsOf(ancestor_thread_id) => {
                orbiterx_state::ThreadRelationFilter::DescendantsOf(ancestor_thread_id)
            }
        };
        let page = orbiterx_rollout::state_db::list_threads_db(
            state_db.as_deref(),
