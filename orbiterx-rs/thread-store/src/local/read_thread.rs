...
use orbiterx_rollout::RolloutRecorder;
use orbiterx_rollout::find_archived_thread_path_by_id_str;
use orbiterx_rollout::find_thread_name_by_id;
use orbiterx_rollout::find_thread_path_by_id_str;
use orbiterx_rollout::read_session_meta_line;
use orbiterx_rollout::read_thread_item_from_rollout;
        ...
        && (params.include_archived
            || (metadata.archived_at.is_none()
                && !rollout_path_is_archived(
                    store.config.orbiterx_home.as_path(),
...
async fn sqlite_rollout_path_can_load_history_for_thread(
    ...
    if orbiterx_rollout::existing_rollout_path(path)
        .await
        .is_none()
    {
        return false;
...
async fn resolve_requested_rollout_path(
    ...
    let path = if rollout_path.is_relative() {
        store.config.orbiterx_home.join(rollout_path)
    } else {
        rollout_path
    };
    ...
    let Some(path) = orbiterx_rollout::existing_rollout_path(path.as_path()).await else {
        return Err(ThreadStoreError::InvalidRequest {
            message: format!(
...
pub(super) async fn resolve_rollout_path(
    ...
    if let Ok(path) = live_writer::rollout_path(store, thread_id).await
        && orbiterx_rollout::existing_rollout_path(path.as_path())
            .await
            .is_some()
        && (include_archived
            || !rollout_path_is_archived(store.config.orbiterx_home.as_path(), &path))
    {
        return Ok(Some(path));
    }
...
async fn read_thread_from_rollout_path(
    ...
    let archived = rollout_path_is_archived(store.config.orbiterx_home.as_path(), path.as_path());
    ...
    thread.rollout_path = Some(orbiterx_rollout::plain_rollout_path(path.as_path()));
    ...
    if let Ok(Some(title)) =
        find_thread_name_by_id(store.config.orbiterx_home.as_path(), &thread.thread_id).await
    {
...
pub(super) async fn load_history_items(
    path: &std::path::Path,
) -> ThreadStoreResult<Vec<orbiterx_protocol::protocol::RolloutItem>> {
...
async fn stored_thread_from_sqlite_metadata(
    ...
    let name = match distinct_thread_metadata_title(&metadata) {
        ...
        None => find_thread_name_by_id(store.config.orbiterx_home.as_path(), &metadata.id)
            .await
            .ok()
            .flatten()
    ...
    let session_meta = match read_required_session_meta_line(metadata.rollout_path.as_path()).await
    {
        ...
        Err(_)
            if orbiterx_rollout::existing_rollout_path(metadata.rollout_path.as_path())
                .await
                .is_none() =>
        {
            None
    ...
    let rollout_path = orbiterx_rollout::plain_rollout_path(metadata.rollout_path.as_path());
...
fn stored_thread_from_meta_line(
    ...
    let rollout_path = orbiterx_rollout::plain_rollout_path(path.as_path());
    StoredThread {
...
mod tests {
    ...
    async fn read_thread_returns_rollout_path_summary() {
        ...
        let relative_path = active_path
            ...
            .expect("path should be under orbiterx home")
            .to_path_buf();
    ...
    async fn read_thread_by_rollout_path_prefers_sqlite_git_info() {
        ...
        let active_path =
            write_session_file(home.path(), "2025-01-03T12-00-00", uuid).expect("session file");
        let runtime = orbiterx_state::StateRuntime::init(
    ...
    async fn read_thread_uses_legacy_thread_name_when_sqlite_title_is_missing() {
        ...
        let thread_id = ThreadId::from_string(&uuid.to_string()).expect("valid thread id");
        write_session_file(home.path(), "2025-01-03T12-00-00", uuid).expect("session file");
        orbiterx_rollout::append_thread_name(home.path(), thread_id, "Legacy title")
            .await
