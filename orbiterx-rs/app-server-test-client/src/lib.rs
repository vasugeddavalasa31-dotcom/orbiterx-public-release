...
use orbiterx_app_server_protocol::JSONRPCMessage;
use orbiterx_app_server_protocol::JSONRPCNotification;
use orbiterx_app_server_protocol::JSONRPCRequest;
...
use orbiterx_otel::OtelProvider;
use orbiterx_otel::current_span_w3c_trace_context;
use orbiterx_protocol::dynamic_tools::normalize_dynamic_tool_specs;
...
const OTEL_SERVICE_NAME: &str = "orbiterx-app-server-test-client";
const TRACE_DISABLED_MESSAGE: &str =
    "Not enabled - enable tracing in $ORBITERX_HOME/config.toml to get a trace URL!";

/// Minimal launcher that initializes the OrbiterX app-server and logs the handshake.
#[derive(Parser)]
#[command(author = "OrbiterX", version, about = "Bootstrap OrbiterX app-server", long_about = None)]
struct Cli {
    /// Path to the `orbiterx` CLI binary. When set, requests use stdio by
    /// spawning `orbiterx app-server` as a child process.
    #[arg(long, env = "ORBITERX_BIN", global = true)]
    orbiterx_bin: Option<PathBuf>,

    /// Existing websocket server URL to connect to.
    ///
    /// If neither `--orbiterx-bin` nor `--url` is provided, defaults to
    /// `ws://127.0.0.1:4222`.
    #[arg(long, env = "ORBITERX_APP_SERVER_URL", global = true)]
    url: Option<String>,

    /// Forwarded to the `orbiterx` CLI as `--config key=value`. Repeatable.
    ///
    /// Example:
...
#[derive(Subcommand)]
enum CliCommand {
    /// Start `orbiterx app-server` on a websocket endpoint in the background.
    ///
    /// Logs are written to:
    ///   `/tmp/orbiterx-app-server-test-client/`
    Serve {
        /// WebSocket listen URL passed to `orbiterx app-server --listen`.
    ...
    /// Send a user message through the OrbiterX app-server.
    SendMessage {
        /// User message to send to OrbiterX.
        user_message: String,
    },
    ...
    TestLogin {
        ...
        device_code: bool,
        /// Use a OrbiterX-managed Amazon Bedrock API key.
    ...
    TestLogout,
    /// Fetch the current account rate limits from the OrbiterX app-server.
    GetAccountRateLimits,
    /// List the available models from the OrbiterX app-server.
    #[command(name = "model-list")]
    ModelList,
    /// List stored threads from the OrbiterX app-server.
    ...
    LiveElicitationTimeoutPause {
        ...
        #[arg(long, env = "ORBITERX_E2E_MODEL", default_value = "gpt-5")]
        model: String,
...
pub async fn run() -> Result<()> {
    ...
    match command {
        CliCommand::Serve { listen, kill } => {
            ...
            let orbiterx_bin = orbiterx_bin.unwrap_or_else(|| PathBuf::from("orbiterx"));
            serve(&orbiterx_bin, &config_overrides, &listen, kill)
        }
        CliCommand::SendMessage { user_message } => {
            ensure_dynamic_tools_unused(&dynamic_tools, "send-message")?;
            let endpoint = resolve_endpoint(orbiterx_bin, url)?;
        ...
        CliCommand::ThreadIncrementElicitation { thread_id } => {
            ...
            let url =
                resolve_shared_websocket_url(orbiterx_bin, url, "thread-increment-elicitation")?;
        ...
        CliCommand::ThreadDecrementElicitation { thread_id } => {
            ...
            let url =
                resolve_shared_websocket_url(orbiterx_bin, url, "thread-decrement-elicitation")?;
        ...
        CliCommand::PluginAnalyticsSmoke {
            ...
            if url.is_some() {
                bail!("plugin-analytics-smoke requires --orbiterx-bin and does not support --url");
            }
            let orbiterx_bin =
                orbiterx_bin.context("plugin-analytics-smoke requires --orbiterx-bin")?;
            plugin_analytics_smoke::run(&orbiterx_bin, &config_overrides, &plugin_id, capture_file)
        }
        CliCommand::PluginAnalyticsMutationSmoke {
            ...
            if url.is_some() {
                bail!(
                    "plugin-analytics-mutation-smoke requires --orbiterx-bin and does not support --url"
                );
            }
            let orbiterx_bin =
                orbiterx_bin.context("plugin-analytics-mutation-smoke requires --orbiterx-bin")?;
        ...
        CliCommand::PluginRemoteUninstall {
            ...
            if url.is_some() {
                bail!("plugin-remote-uninstall requires --orbiterx-bin and does not support --url");
            }
            let orbiterx_bin =
                orbiterx_bin.context("plugin-remote-uninstall requires --orbiterx-bin")?;
...
fn resolve_endpoint(orbiterx_bin: Option<PathBuf>, url: Option<String>) -> Result<Endpoint> {
    if orbiterx_bin.is_some() && url.is_some() {
        bail!("--orbiterx-bin and --url are mutually exclusive");
    }
    if let Some(orbiterx_bin) = orbiterx_bin {
        return Ok(Endpoint::SpawnOrbiterX(orbiterx_bin));
    }
    if let Some(url) = url {
...
fn resolve_shared_websocket_url(
    ...
    if orbiterx_bin.is_some() {
        bail!(
            "{command} requires --url or an already-running websocket app-server; --orbiterx-bin would spawn a private stdio app-server instead"
        );
    }
...
impl BackgroundAppServer {
    fn spawn(orbiterx_bin: &Path, config_overrides: &[String]) -> Result<Self> {
        ...
        let mut cmd = Command::new(orbiterx_bin);
        if let Some(orbiterx_bin_parent) = orbiterx_bin.parent() {
            let mut path = OsString::from(orbiterx_bin_parent.as_os_str());
        ...
        let process = cmd
            ...
            .spawn()
            .with_context(|| format!("failed to start `{}` app-server", orbiterx_bin.display()))?;
...
fn serve(orbiterx_bin: &Path, config_overrides: &[String], listen: &str, kill: bool) -> Result<()> {
    let runtime_dir = PathBuf::from("/tmp/orbiterx-app-server-test-client");
    ...
    let mut cmdline = format!(
        "tail -f /dev/null | RUST_BACKTRACE=full RUST_LOG=warn,orbiterx_=trace {}",
        shell_quote(&orbiterx_bin.display().to_string())
    );
    for override_kv in config_overrides {
    ...
    println!("started orbiterx app-server");
...
pub async fn send_message_v2(
    ...
    let endpoint = Endpoint::SpawnOrbiterX(orbiterx_bin.to_path_buf());
    send_message_v2_endpoint(
        &endpoint,
        config_overrides,
        user_message,
            ...
            TestLoginMode::AmazonBedrock { api_key, region } => {
                ...
                let login_response: LoginAccountResponse = client.send_request(
                    ClientRequest::LoginAccount {
                        ...
                        params: orbiterx_app_server_protocol::LoginAccountParams::AmazonBedrock {
                            api_key,
                            region,
                        },
                    },
...
async fn with_client<T>(
    ...
    endpoint: &Endpoint,
    config_overrides: &[String],
    f: impl FnOnce(&mut OrbiterXClient) -> Result<T>,
) -> Result<T> {
    ...
    let result = command_span.in_scope(|| {
        let mut client = OrbiterXClient::connect(endpoint, config_overrides)?;
        f(&mut client)
    });
...
fn thread_increment_elicitation(url: &str, thread_id: String) -> Result<()> {
    ...
    let mut client = OrbiterXClient::connect(&endpoint, &[])?;
...
fn live_elicitation_timeout_pause(
    ...
    let websocket_url = match (orbiterx_bin, url) {
        (Some(_), Some(_)) => bail!("--orbiterx-bin and --url are mutually exclusive"),
        (Some(orbiterx_bin), None) => {
            let server = BackgroundAppServer::spawn(&orbiterx_bin, config_overrides)?;
    ...
    let app_server_test_client_bin = std::env::current_exe()
        .context("failed to resolve orbiterx-app-server-test-client binary path")?;
...
impl OrbiterXClient {
    fn connect(endpoint: &Endpoint, config_overrides: &[String]) -> Result<Self> {
        match endpoint {
            Endpoint::SpawnOrbiterX(orbiterx_bin) => {
                Self::spawn_stdio(orbiterx_bin, config_overrides)
            }
            Endpoint::ConnectWs(url) => Self::connect_websocket(url),
        }
    }

    fn spawn_stdio(orbiterx_bin: &Path, config_overrides: &[String]) -> Result<Self> {
        Self::spawn_stdio_with_env(orbiterx_bin, config_overrides, &[])
    }
    ...
    fn spawn_stdio_with_env(
        ...
        let orbiterx_bin_display = orbiterx_bin.display();
        let mut cmd = Command::new(orbiterx_bin);
        if let Some(orbiterx_bin_parent) = orbiterx_bin.parent() {
            let mut path = OsString::from(orbiterx_bin_parent.as_os_str());
            if let Some(existing_path) = std::env::var_os("PATH") {
        ...
        let mut orbiterx_app_server = cmd
            .arg("app-server")
            .stdin(Stdio::piped())
            .stdout(Stdio::piped())
            .stderr(Stdio::inherit())
            .spawn()
            .with_context(|| format!("failed to start `{orbiterx_bin_display}` app-server"))?;

        let stdin = orbiterx_app_server
            .stdin
            .take()
            .context("orbiterx app-server stdin unavailable")?;
        let stdout = orbiterx_app_server
            .stdout
            .take()
            .context("orbiterx app-server stdout unavailable")?;

        Ok(Self {
            transport: ClientTransport::Stdio {
                child: orbiterx_app_server,
                stdin: Some(stdin),
                ...
                Err(err) => {
                    if Instant::now() >= deadline {
                        return Err(err).with_context(|| {
                            format!(
                                "failed to connect to websocket app-server at `{url}`; if no server is running, start one with `orbiterx-app-server-test-client serve --listen {url}`"
                            )
                        });
                    }
        ...
        let request = ClientRequest::Initialize {
            ...
            params: InitializeParams {
                client_info: ClientInfo {
                    name: "orbiterx-toy-app-server".to_string(),
                    title: Some("OrbiterX Toy App Server".to_string()),
    ...
    fn login_account_chatgpt(&mut self) -> Result<LoginAccountResponse> {
        ...
        let request = ClientRequest::LoginAccount {
            request_id: request_id.clone(),
            params: orbiterx_app_server_protocol::LoginAccountParams::Chatgpt {
                app_brand: None,
                orbiterx_streamlined_login: false,
    ...
    fn login_account_chatgpt_device_code(&mut self) -> Result<LoginAccountResponse> {
        ...
        let request = ClientRequest::LoginAccount {
            request_id: request_id.clone(),
            params: orbiterx_app_server_protocol::LoginAccountParams::ChatgptDeviceCode,
        };
        ...
        match &mut self.transport {
            ClientTransport::Stdio { stdin, .. } => {
                if let Some(stdin) = stdin.as_mut() {
                    ...
                    stdin
                        .flush()
                        .context("failed to flush payload to orbiterx app-server")?;
                    return Ok(());
                }
                bail!("orbiterx app-server stdin closed")
            }
        ...
        match &mut self.transport {
            ClientTransport::Stdio { stdout, .. } => {
                ...
                let bytes = stdout
                    ...
                    .context("failed to read from orbiterx app-server")?;
                if bytes == 0 {
                    bail!("orbiterx app-server closed stdout");
                }
                Ok(response_line)
            }
...
impl TestClientTracing {
    async fn initialize(config_overrides: &[String]) -> Result<Self> {
        ...
        let otel_provider = orbiterx_core::otel_init::build_provider(
            &config,
...
impl Drop for OrbiterXClient {
    fn drop(&mut self) {
        ...
        if let Ok(Some(status)) = child.try_wait() {
            println!("[orbiterx app-server exited: {status}]");
            return;
        }
