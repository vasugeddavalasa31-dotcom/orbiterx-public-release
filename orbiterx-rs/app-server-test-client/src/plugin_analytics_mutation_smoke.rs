...
pub(super) fn run(
    ...
    let capture_path = capture_file.unwrap_or_else(|| {
        std::env::temp_dir().join(format!(
            "orbiterx-plugin-analytics-mutation-{}.jsonl",
            process::id()
        ))
    });
    prepare_capture_file(&capture_path)?;
    let mut client = spawn_client(orbiterx_bin, config_overrides, &capture_path)?;
    ...
    match (sequence_result, restoration) {
        ...
        (sequence_result, RestorationStatus::Unknown(cleanup_err)) => {
            ...
            print_recovery_command(orbiterx_bin, config_overrides, remote_plugin_id);
            Err(cleanup_err)
        }
    }
}
...
pub(super) fn run_cleanup(
    ...
    let mut client = OrbiterXClient::spawn_stdio(orbiterx_bin, &overrides)?;
    client.initialize()?;
    ...
    match restore_uninstalled_state(&mut client, remote_plugin_id) {
        ...
        RestorationStatus::Dirty(err) => {
            print_dirty_recovery(orbiterx_bin, config_overrides, remote_plugin_id, &err);
            Err(err)
        }
...
fn spawn_client(
    ...
    OrbiterXClient::spawn_stdio_with_env(orbiterx_bin, &overrides, &environment)
}
...
fn uninstall_remote_plugin(client: &mut OrbiterXClient, remote_plugin_id: &str) -> Result<()> {
    let request_id = client.request_id();
...
fn print_recovery_command(
    ...
    let test_client = std::env::current_exe()
        ...
        .unwrap_or_else(|_| "orbiterx-app-server-test-client".to_string());
