# orbiterx-network-proxy

`orbiterx-network-proxy` is OrbiterX's local network policy enforcement proxy. It runs:
...
`orbiterx-network-proxy` reads from OrbiterX's merged `config.toml` (via `orbiterx-core` config loading).
...
# bundle env vars pointing at immutable public files under $ORBITERX_HOME/proxy/ so common HTTPS
...
```bash
cargo run -p orbiterx-network-proxy --
```
...
`orbiterx-network-proxy` can be embedded as a library with a thin API:

```rust
use orbiterx_network_proxy::{NetworkProxy, NetworkDecision, NetworkPolicyRequest};
...
When `orbiterx-network-proxy` is embedded in managed OrbiterX runtime, policy decisions emit structured
OTEL-compatible events with `target=orbiterx_otel.network_proxy`.

Event name:

- `orbiterx.network_proxy.policy_decision`
...
This section documents the protections implemented by `orbiterx-network-proxy`, and the boundaries of
