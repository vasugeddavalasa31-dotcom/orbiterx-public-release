...
fn managed_ca_dir() -> Result<PathBuf> {
    let orbiterx_home =
        find_orbiterx_home().context("failed to resolve ORBITERX_HOME for managed MITM CA")?;
    Ok(orbiterx_home.join(MANAGED_MITM_CA_DIR).to_path_buf())
}
...
/// Returns whether `path` points at a current OrbiterX-generated MITM CA bundle.
