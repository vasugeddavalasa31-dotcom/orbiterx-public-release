...
use orbiterx_config::types::AppConfig;
use orbiterx_config::types::AppToolConfig;
use orbiterx_config::types::AppToolsConfig;
use orbiterx_config::types::ApprovalsReviewer;
use orbiterx_config::types::AppsConfigToml;
...
use orbiterx_rollout_trace::ToolDispatchInvocation;
use orbiterx_rollout_trace::ToolDispatchPayload;
use orbiterx_rollout_trace::ToolDispatchRequester;
...
fn write_sample_plugin_mcp(orbiterx_home: &std::path::Path) {
    let plugin_root = orbiterx_home.join("plugins/cache/test/sample/local");
    ...
    let dispatch_trace = session
        ...
        .start_tool_dispatch_trace(|| {
            Some(ToolDispatchInvocation {
                ...
                orbiterx_turn_id: turn_context.sub_id.clone(),
...
fn install_mcp_permission_request_hook(
    ...
    std::fs::create_dir_all(&turn_context.config.orbiterx_home)
        .expect("create orbiterx home for MCP permission hook");
    let script = format!(
        r#"import json
...
"#,
    ...
    std::fs::write(
        turn_context.config.orbiterx_home.join("hooks.json"),
        serde_json::json!({
            "hooks": {
                "PermissionRequest": [{
                    "matcher": matcher,
    ...
    let hook_list = orbiterx_hooks::list_hooks(HooksConfig {
    ...
    let trusted_config_layer_stack = trusted_config_layer_stack(
        ...
        &turn_context.config.orbiterx_home,
        hook_list.hooks,
    );
...
fn attach_trace_bundle(
    ...
    let rollout_thread_trace =
        orbiterx_rollout_trace::ThreadTraceContext::start_root_in_root_for_test(
            root,
    ...
    rollout_thread_trace.record_orbiterx_turn_started(turn_context.sub_id.as_str());
...
#[test]
fn openai_file_params_are_only_honored_for_orbiterx_apps() {
    let params = HashMap::from([("file".to_string(), Vec::new())]);

    assert_eq!(
        openai_file_input_optional_fields_for_server(ORBITERX_APPS_MCP_SERVER_NAME, &params),
        Some(params.clone())
    );
    assert_eq!(
...
async fn mcp_tool_call_span_records_error_type_and_error_code() {
    ...
    let span = mcp_tool_call_span(
        ...
        &turn_context,
        McpToolCallSpanFields {
            server_name: ORBITERX_APPS_MCP_SERVER_NAME,
            tool_name: "calendar_search",
            call_id: "call-123",
            server_origin: Some("https://chatgpt.com/api/orbiterx/ps/mcp"),
            connector_id: Some("calendar"),
            connector_name: Some("Calendar"),
        },
    );
    ...
    assert!(
        logs.contains("error.type=\"tool_result\"")
            && logs.contains("orbiterx.mcp.error.code=\"RATE_LIMITED\""),
...
async fn mcp_result_telemetry_records_allowlisted_span_fields() {
    ...
    assert!(
        logs.contains("orbiterx.mcp.target.id=\"com.apple.reminders\"")
            && logs.contains("orbiterx.mcp.server_user_flow.triggered=false"),
...
async fn mcp_result_telemetry_ignores_invalid_and_missing_values() {
    ...
    .await;
    assert!(
        !invalid_logs.contains("orbiterx.mcp.target.id=")
            && !invalid_logs.contains("orbiterx.mcp.server_user_flow.triggered="),
    ...
    .await;
    assert!(
        !missing_logs.contains("orbiterx.mcp.target.id=")
            && !missing_logs.contains("orbiterx.mcp.server_user_flow.triggered="),
    ...
    assert!(
        !no_meta_logs.contains("orbiterx.mcp.target.id=")
            && !no_meta_logs.contains("orbiterx.mcp.server_user_flow.triggered="),
...
async fn mcp_result_telemetry_truncates_long_target_id() {
    ...
    assert!(
        logs.contains(&format!("orbiterx.mcp.target.id=\"{truncated}\"")) && !logs.contains("tail"),
...
#[test]
fn orbiterx_apps_tool_question_uses_fallback_app_label() {
    let question = build_mcp_tool_approval_question(
        "q".to_string(),
        ORBITERX_APPS_MCP_SERVER_NAME,
        "run_action",
...
#[test]
fn trusted_orbiterx_apps_tool_question_offers_always_allow() {
    let question = build_mcp_tool_approval_question(
        "q".to_string(),
        ORBITERX_APPS_MCP_SERVER_NAME,
        "run_action",
...
#[test]
fn orbiterx_apps_tool_question_without_elicitation_omits_always_allow() {
    let session_key = McpToolApprovalKey {
        server: ORBITERX_APPS_MCP_SERVER_NAME.to_string(),
...
#[test]
fn orbiterx_apps_connectors_support_persistent_approval() {
    let invocation = McpInvocation {
        server: ORBITERX_APPS_MCP_SERVER_NAME.to_string(),
...
async fn mcp_tool_call_request_meta_includes_turn_metadata_for_custom_server() {
    ...
    let turn_metadata = meta
        .get(crate::X_ORBITERX_TURN_METADATA_HEADER)
    ...
    assert_eq!(
        meta,
        serde_json::json!({
            crate::X_ORBITERX_TURN_METADATA_HEADER: expected_turn_metadata,
        })
    );
}
...
#[test]
fn mcp_tool_call_item_metadata_only_trusts_orbiterx_apps_identity() {
    let mut metadata = approval_metadata(
    ...
    metadata.orbiterx_apps_meta = Some(
        serde_json::json!({
            "resource_uri": "/asdk_app_0123456789abcdef0123456789abcdef/link_fedcba9876543210fedcba9876543210/create_event",
        })
        .as_object()
        .cloned()
        .expect("_orbiterx_apps metadata should be an object"),
    );

    assert_eq!(
        McpToolCallItemMetadata::from_tool_metadata(ORBITERX_APPS_MCP_SERVER_NAME, Some(&metadata),),
...
#[tokio::test]
async fn orbiterx_apps_tool_call_request_meta_includes_turn_metadata_and_orbiterx_apps_meta() {
    let (_, turn_context) = make_session_and_context().await;
    let expected_turn_metadata = turn_context
        .turn_metadata_state
        .current_meta_value_for_mcp_request(mcp_turn_metadata_context(&turn_context))
    ...
    assert_eq!(
        ...
        Some(serde_json::json!({
            crate::X_ORBITERX_TURN_METADATA_HEADER: expected_turn_metadata,
            MCP_TOOL_ORBITERX_APPS_META_KEY: {
...
#[tokio::test]
async fn orbiterx_apps_tool_call_request_meta_includes_call_id_without_existing_orbiterx_apps_meta()
{
    let (_, turn_context) = make_session_and_context().await;
    let expected_turn_metadata = turn_context
        .turn_metadata_state
...
fn orbiterx_apps_auth_failure_result() -> CallToolResult {
    CallToolResult {
        content: vec![serde_json::json!({
            "type": "text",
...
fn orbiterx_apps_auth_failure_metadata() -> McpToolApprovalMetadata {
    approval_metadata(
...
async fn host_owned_orbiterx_apps_manager(
    session: &Session,
    turn_context: &TurnContext,
) -> Arc<orbiterx_mcp::McpConnectionManager> {
    ...
    let mcp_servers = HashMap::from([(
        ...
        orbiterx_mcp::EffectiveMcpServer::configured(
            orbiterx_mcp::orbiterx_apps_mcp_server_config(
                "https://chatgpt.com",
    ...
    let manager = orbiterx_mcp::McpConnectionManager::new(
        &mcp_servers,
        ...
        orbiterx_mcp::McpRuntimeContext::new(
        ...
        turn_context.config.orbiterx_home.to_path_buf(),
        session.services.mcp_manager.orbiterx_apps_tools_cache(),
        session.services.mcp_manager.tool_catalog_cache(),
        orbiterx_connectors::connector_runtime_context_key(auth.as_ref()),
        turn_context.config.prefix_mcp_tool_names(),
        rmcp::model::ElicitationCapability::default(),
        /*supports_openai_form_elicitation*/ false,
        orbiterx_mcp::ToolPluginProvenance::default(),
        auth.as_ref(),
        /*orbiterx_apps_auth_manager*/ None,
        /*elicitation_reviewer*/ None,
        /*elicitation_lifecycle*/ None,
        orbiterx_mcp::ElicitationRequestRouter::default(),
    )
    .await;
    Arc::new(manager)
}
...
#[tokio::test]
async fn orbiterx_apps_auth_elicitation_feature_disabled_returns_original_result() {
    ...
    let manager = host_owned_orbiterx_apps_manager(&session, &turn_context).await;
    let result = orbiterx_apps_auth_failure_result();
    let metadata = orbiterx_apps_auth_failure_metadata();

    let returned = maybe_request_orbiterx_apps_auth_elicitation(
        &session,
        &turn_context,
        manager.as_ref(),
        "call_123",
...
#[tokio::test]
async fn orbiterx_apps_auth_elicitation_non_host_owned_server_returns_original_result() {
    let (session, mut turn_context, rx_event) = make_session_and_context_with_rx().await;
    let mut features = Features::with_defaults();
...
#[tokio::test]
async fn orbiterx_apps_auth_elicitation_disallowed_by_policy_returns_original_result() {
    let (session, mut turn_context, rx_event) = make_session_and_context_with_rx().await;
    let manager = host_owned_orbiterx_apps_manager(&session, &turn_context).await;
    let mut features = Features::with_defaults();
    features.enable(Feature::AuthElicitation);
...
#[tokio::test]
async fn orbiterx_apps_auth_elicitation_granular_mcp_disabled_returns_original_result() {
    let (session, mut turn_context, rx_event) = make_session_and_context_with_rx().await;
    let manager = host_owned_orbiterx_apps_manager(&session, &turn_context).await;
    let mut features = Features::with_defaults();
    features.enable(Feature::AuthElicitation);
...
#[tokio::test]
async fn orbiterx_apps_auth_elicitation_enabled_by_default_requests_elicitation() {
    let (session, turn_context, rx_event) = make_session_and_context_with_rx().await;
    let manager = host_owned_orbiterx_apps_manager(&session, &turn_context).await;
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    let request_task = tokio::spawn({
        ...
        async move {
            maybe_request_orbiterx_apps_auth_elicitation(
                &session,
                &turn_context,
                manager.as_ref(),
                "call_123",
    ...
    assert_eq!(request.server_name, ORBITERX_APPS_MCP_SERVER_NAME);
    assert_eq!(
        request.id,
        orbiterx_protocol::mcp::RequestId::String("orbiterx_apps_auth_call_123".to_string())
    );
    assert!(matches!(
        request.request,
        orbiterx_protocol::approvals::ElicitationRequest::Url { .. }
    ));

    session
        .resolve_elicitation(
            ORBITERX_APPS_MCP_SERVER_NAME.to_string(),
            rmcp::model::RequestId::String("orbiterx_apps_auth_call_123".into()),
            ElicitationResponse {
...
async fn guardian_review_decision_maps_to_mcp_tool_decision() {
    ...
    session.services.guardian_rejections.lock().await.insert(
        ...
        crate::guardian::GuardianRejection {
            ...
            source: orbiterx_protocol::protocol::GuardianAssessmentDecisionSource::Agent,
        },
    );
...
#[test]
fn approval_elicitation_meta_includes_connector_source_for_orbiterx_apps() {
    assert_eq!(
        build_mcp_tool_approval_elicitation_meta(
            ORBITERX_APPS_MCP_SERVER_NAME,
            Some(&approval_metadata(
...
#[tokio::test]
async fn persist_orbiterx_app_tool_approval_writes_tool_override() {
    let tmp = tempdir().expect("tempdir");
    let config = ConfigBuilder::default()
        .orbiterx_home(tmp.path().to_path_buf())
        .build()
        .await
        .expect("load config");

    persist_orbiterx_app_tool_approval(&config, "calendar", "calendar/list_events")
        .await
...
async fn custom_mcp_tool_approval_mode_uses_plugin_mcp_policy() {
    ...
    write_sample_plugin_mcp(orbiterx_home.as_path());
    std::fs::write(
...
async fn maybe_persist_mcp_tool_approval_reloads_session_config_for_custom_server() {
    ...
    let config = ConfigBuilder::without_managed_config_for_tests()
        .orbiterx_home(orbiterx_home.clone().to_path_buf())
        .build()
        .await
        .expect("load config");
    turn_context.config = Arc::new(config);
...
"#,
    ...
    let contents =
        std::fs::read_to_string(orbiterx_home.join(CONFIG_TOML_FILE)).expect("read config");
    let parsed: ConfigToml = toml::from_str(&contents).expect("parse config");
    let tool = parsed
        .plugins
        .get("sample@test")
...
async fn maybe_persist_mcp_tool_approval_writes_project_config_for_project_server() {
    ...
    let project_orbiterx_dir = project_dir.path().join(".orbiterx");
    std::fs::create_dir_all(&project_orbiterx_dir).expect("create project .orbiterx dir");
    std::fs::write(
        project_orbiterx_dir.join(CONFIG_TOML_FILE),
        "[mcp_servers.docs]\ncommand = \"docs-server\"\n",
    )
    ...
    ConfigEditsBuilder::new(&orbiterx_home)
        .set_project_trust_level(
            project_dir.path(),
            orbiterx_protocol::config_types::TrustLevel::Trusted,
        )
        .apply()
        .await
    ...
    let contents = std::fs::read_to_string(project_orbiterx_dir.join(CONFIG_TOML_FILE))
...
async fn approve_mode_skips_guardian_in_every_permission_mode() {
    ...
    for approval_policy in [
        ...
        turn_context.auth_manager = Some(crate::test_support::auth_manager_from_auth(
            orbiterx_login::OrbiterXAuth::create_dummy_chatgpt_auth_for_testing(),
        ));
        turn_context
            .approval_policy
            .set(approval_policy)
