...
pub(crate) struct AgentControl {
    ...
    /// `ThreadManagerState -> OrbiterXThread -> Session -> SessionServices -> ThreadManagerState`.
...
impl AgentControl {
    ...
    pub(crate) async fn interrupt_agent(&self, agent_id: ThreadId) -> OrbiterXResult<String> {
        let state = self.upgrade()?;
        self.handle_thread_request_result(
            agent_id,
            &state,
    ...
    async fn handle_thread_request_result(
        ...
        agent_id: ThreadId,
        state: &Arc<ThreadManagerState>,
        result: OrbiterXResult<String>,
    ) -> OrbiterXResult<String> {
        if matches!(result, Err(OrbiterXErr::InternalAgentDied)) {
    ...
    pub(crate) fn ensure_agent_known(&self, agent_id: ThreadId) -> OrbiterXResult<AgentMetadata> {
        self.state
            .agent_metadata_for_thread(agent_id)
            .ok_or(OrbiterXErr::ThreadNotFound(agent_id))
    }
    ...
    pub(crate) async fn resolve_agent_reference(
        ...
        Err(OrbiterXErr::UnsupportedOperation(format!(
    ...
    pub(crate) async fn subscribe_status(
        &self,
        agent_id: ThreadId,
    ) -> OrbiterXResult<watch::Receiver<AgentStatus>> {
        let state = self.upgrade()?;
    ...
    ) -> OrbiterXResult<Vec<ListedAgent>> {
        let state = self.upgrade()?;
    ...
    fn prepare_thread_spawn(
        ...
        agent_role: Option<String>,
        preferred_agent_nickname: Option<String>,
    ) -> OrbiterXResult<(SessionSource, AgentMetadata)> {
        if depth == 1 {
    ...
    fn upgrade(&self) -> OrbiterXResult<Arc<ThreadManagerState>> {
        self.manager
            .upgrade()
            .ok_or_else(|| OrbiterXErr::UnsupportedOperation("thread manager dropped".to_string()))
    }
    ...
    async fn open_thread_spawn_children(
        &self,
        parent_thread_id: ThreadId,
    ) -> OrbiterXResult<Vec<(ThreadId, AgentMetadata)>> {
    ...
    async fn live_thread_spawn_children(
        &self,
    ) -> OrbiterXResult<HashMap<ThreadId, Vec<(ThreadId, AgentMetadata)>>> {
        let state = self.upgrade()?;
    ...
    async fn persist_thread_spawn_edge_for_source(
        &self,
        child_thread: &crate::OrbiterXThread,
        child_thread_id: ThreadId,
        session_source: Option<&SessionSource>,
    ) {
        ...
        if let Err(err) = agent_graph_store
            .upsert_thread_spawn_edge(
                parent_thread_id,
                child_thread_id,
                orbiterx_agent_graph_store::ThreadSpawnEdgeStatus::Open,
            )
            .await
        {
