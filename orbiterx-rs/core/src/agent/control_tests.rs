...
use orbiterx_protocol::protocol::ThreadSettingsAppliedEvent;
use orbiterx_protocol::protocol::ThreadSettingsSnapshot;
...
use orbiterx_thread_store::ArchiveThreadParams;
...
impl AgentControlHarness {
    ...
    async fn new_with_config(home: TempDir, config: Config) -> Self {
        ...
        let manager = ThreadManager::with_models_provider_home_and_state_for_tests(
            ...
            std::sync::Arc::new(orbiterx_exec_server::EnvironmentManager::default_for_tests()),
            state_db.clone(),
        );
    ...
    async fn start_thread(&self) -> (ThreadId, Arc<OrbiterXThread>) {
        let new_thread = self
            .manager
    ...
    async fn start_paginated_thread(&self) -> (ThreadId, Arc<OrbiterXThread>) {
        let new_thread = self
            .manager
...
async fn persisted_originator(thread: &OrbiterXThread) -> String {
...
async fn wait_for_subagent_notification(parent_thread: &Arc<OrbiterXThread>) -> bool {
    let wait = async {
        loop {
...
async fn persist_thread_for_tree_resume(thread: &Arc<OrbiterXThread>, message: &str) {
    thread
...
async fn assert_thread_not_loaded(manager: &ThreadManager, thread_id: ThreadId) {
    match manager.get_thread(thread_id).await {
        Err(OrbiterXErr::ThreadNotFound(id)) => assert_eq!(id, thread_id),
...
async fn send_input_errors_when_thread_missing() {
    ...
    assert_matches!(err, OrbiterXErr::ThreadNotFound(id) if id == thread_id);
}
...
async fn ensure_v2_agent_loaded_reloads_registered_unloaded_agent() {
    ...
    match harness.manager.get_thread(spawned_agent.thread_id).await {
        Err(OrbiterXErr::ThreadNotFound(id)) => assert_eq!(id, spawned_agent.thread_id),
        Err(err) => panic!("expected ThreadNotFound, got {err:?}"),
...
async fn resume_agent_from_rollout_does_not_reopen_v2_descendants() {
    ...
    let resumed_manager = ThreadManager::with_models_provider_home_and_state_for_tests(
        ...
        harness.config.orbiterx_home.to_path_buf(),
        std::sync::Arc::new(orbiterx_exec_server::EnvironmentManager::default_for_tests()),
...
async fn spawn_agent_without_fork_from_paginated_parent_stays_fresh_and_paginated() {
    ...
    child_thread
        ...
        .await
        .expect("child rollout should flush");
    let meta = orbiterx_rollout::read_session_meta_line(
        &child_thread
            .rollout_path()
            .expect("child rollout should exist"),
    )
...
async fn spawn_agent_respects_legacy_max_threads_alias() {
    ...
    let OrbiterXErr::AgentLimitReached {
        max_threads: seen_max_threads,
    } = err
    else {
        panic!("expected OrbiterXErr::AgentLimitReached");
    };
...
async fn spawn_agent_limit_shared_across_clones() {
    ...
    let OrbiterXErr::AgentLimitReached { max_threads } = err else {
        panic!("expected OrbiterXErr::AgentLimitReached");
    };
...
async fn spawn_thread_subagents_persist_parent_originator_across_new_and_truncated_fork() {
    ...
    assert_eq!(parent_originator, "orbiterx_work_desktop");
...
async fn resume_thread_subagent_restores_stored_metadata() {
    ...
    let auth_manager = AuthManager::from_auth_for_testing(OrbiterXAuth::from_api_key("dummy"));
