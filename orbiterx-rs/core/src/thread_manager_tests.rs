...
use orbiterx_protocol::capabilities::CapabilityRootLocation;
use orbiterx_protocol::capabilities::SelectedCapabilityRoot;
...
impl orbiterx_agent_graph_store::AgentGraphStore for FakeAgentGraphStore {
    fn upsert_thread_spawn_edge(
        &self,
        _parent_thread_id: ThreadId,
        _child_thread_id: ThreadId,
        _status: orbiterx_agent_graph_store::ThreadSpawnEdgeStatus,
    ) -> orbiterx_agent_graph_store::AgentGraphStoreFuture<'_, ()> {
    ...
    fn list_thread_spawn_children(
        &self,
        _parent_thread_id: ThreadId,
        _status_filter: Option<orbiterx_agent_graph_store::ThreadSpawnEdgeStatus>,
    ) -> orbiterx_agent_graph_store::AgentGraphStoreFuture<'_, Vec<ThreadId>> {
    ...
    fn list_thread_spawn_descendants(
        &self,
        root_thread_id: ThreadId,
        status_filter: Option<orbiterx_agent_graph_store::ThreadSpawnEdgeStatus>,
    ) -> orbiterx_agent_graph_store::AgentGraphStoreFuture<'_, Vec<ThreadId>> {
...
fn effective_originator_prefers_thread_scoped_sources_before_env_originator() {
    for (metrics_service_name, persisted_originator, inherited_originator, expected_originator) in [
        (
            Some("orbiterx_work_desktop"),
            Some("persisted_originator"),
            Some("inherited_originator"),
            "orbiterx_work_desktop",
        ),
        (
            Some("orbiterx_work_web"),
            Some("persisted_originator"),
            Some("inherited_originator"),
            "orbiterx_work_web",
        ),
        (
            Some("orbiterx_work_mobile"),
            Some("persisted_originator"),
            Some("inherited_originator"),
            "orbiterx_work_mobile",
        ),
        (
            Some("orbiterx_work_cca"),
            Some("persisted_originator"),
            Some("inherited_originator"),
            "orbiterx_work_cca",
        ),
...
async fn shutdown_all_threads_bounded_submits_shutdown_to_every_thread() {
    ...
    config.orbiterx_home = temp_dir.path().join("orbiterx-home").abs();
    config.cwd = config.orbiterx_home.abs();
    std::fs::create_dir_all(&config.orbiterx_home).expect("create orbiterx home");
...
async fn start_thread_seeds_extension_data_for_mcp_and_lifecycle_contributors() {
    ...
    impl orbiterx_extension_api::ThreadLifecycleContributor<Config> for InitialDataRecorder {
    ...
    impl orbiterx_extension_api::McpServerContributor<Config> for InitialDataRecorder {
        ...
        > {
            Box::pin(async move {
                ...
                let mut server = orbiterx_mcp::orbiterx_apps_mcp_server_config(
                ...
                vec![
                    orbiterx_extension_api::McpServerContribution::SelectedPlugin {
    ...
    let mut extensions = orbiterx_extension_api::ExtensionRegistryBuilder::new();
    ...
    let selected_root_init = |id: &str, environment_id: &str| {
        let mut init = orbiterx_extension_api::ExtensionDataInit::new();
    ...
    let first_thread = manager
        .start_thread_with_options(StartThreadOptions {
            ...
            thread_source: None,
            dynamic_tools: Vec::new(),
            metrics_service_name: Some("orbiterx_work_desktop".to_string()),
            parent_trace: None,
            environments: Vec::new(),
    ...
    let selected_servers = |config: &orbiterx_mcp::McpConfig| {
        orbiterx_mcp::configured_mcp_servers(config)
            .into_iter()
    ...
    let orbiterx_apps_server = orbiterx_mcp::configured_mcp_servers(&first_resolved.config)
        .remove(orbiterx_mcp::ORBITERX_APPS_MCP_SERVER_NAME)
        .expect("OrbiterX Apps server should be configured");
    let orbiterx_apps_headers = match orbiterx_apps_server.transport {
        orbiterx_config::McpServerTransportConfig::StreamableHttp { http_headers, .. } => {
            http_headers
        }
        orbiterx_config::McpServerTransportConfig::Stdio { .. } => {
            panic!("OrbiterX Apps server should use streamable HTTP")
        }
    };
    assert_eq!(
        orbiterx_apps_headers
            .expect("OrbiterX Apps headers should be configured")
            .get("originator"),
...
#[tokio::test]
async fn explicit_installation_id_skips_orbiterx_home_file() {
    let temp_dir = tempdir().expect("tempdir");
    let mut config = test_config().await;
    config.orbiterx_home = temp_dir.path().join("orbiterx-home").abs();
    config.cwd = config.orbiterx_home.abs();
    ...
    assert!(!config.orbiterx_home.join(INSTALLATION_ID_FILENAME).exists());
...
async fn injected_models_manager_controls_refresh_policy() {
    ...
    assert!(!config.orbiterx_home.join("models_cache.json").exists());
}
