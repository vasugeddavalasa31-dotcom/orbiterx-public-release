...
use orbiterx_tools::TOOL_SEARCH_TOOL_NAME;
use orbiterx_tools::ToolCall as ExtensionToolCall;
use orbiterx_tools::ToolEnvironmentMode;
...
use orbiterx_tools::can_request_original_image_detail;
use orbiterx_tools::collect_code_mode_exec_prompt_tool_definitions;
use orbiterx_tools::collect_request_plugin_install_entries;
use orbiterx_tools::default_namespace_description;
use orbiterx_tools::request_user_input_available_modes;
use orbiterx_tools::shell_command_backend_for_features;
use orbiterx_tools::shell_type_for_model_and_features;
...
fn spec_for_model_request(
    ...
    if matches!(tool_mode, ToolMode::CodeMode | ToolMode::CodeModeOnly)
        ...
        && orbiterx_code_mode::is_code_mode_nested_tool(spec.name())
    {
        orbiterx_tools::augment_tool_spec_for_code_mode(spec)
    } else {
        spec
    }
}
    ...
    (turn_context
        ...
        || (turn_context.provider.info().requires_openai_auth
            && turn_context
                .auth_manager
                .as_deref()
                .is_some_and(AuthManager::current_auth_uses_orbiterx_backend)))
...
fn is_hidden_by_code_mode_only(
    ...
    tool_mode == ToolMode::CodeModeOnly
        && exposure != ToolExposure::DirectModelOnly
        && orbiterx_code_mode::is_code_mode_nested_tool(
            &orbiterx_tools::code_mode_name_for_tool_name(tool_name),
        )
}
...
fn code_mode_namespace_descriptions(
    specs: &[ToolSpec],
) -> BTreeMap<String, orbiterx_code_mode::ToolNamespaceDescription> {
    ...
    for spec in specs {
        ...
        let entry = namespace_descriptions
            ...
            .or_insert_with(|| orbiterx_code_mode::ToolNamespaceDescription {
...
fn append_extension_tool_executors(
    ...
    let tool_mode = effective_tool_mode(turn_context);
    if matches!(tool_mode, ToolMode::CodeMode | ToolMode::CodeModeOnly) {
        reserved_tool_names.insert(ToolName::plain(orbiterx_code_mode::PUBLIC_TOOL_NAME));
        reserved_tool_names.insert(ToolName::plain(orbiterx_code_mode::WAIT_TOOL_NAME));
    }
    if search_tool_enabled(turn_context)
        && planned_tools
            .runtimes()
...
fn compare_code_mode_tools(
    left: &orbiterx_code_mode::ToolDefinition,
    right: &orbiterx_code_mode::ToolDefinition,
    namespace_descriptions: &BTreeMap<String, orbiterx_code_mode::ToolNamespaceDescription>,
) -> std::cmp::Ordering {
...
fn code_mode_namespace_name<'a>(
    tool: &orbiterx_code_mode::ToolDefinition,
    namespace_descriptions: &'a BTreeMap<String, orbiterx_code_mode::ToolNamespaceDescription>,
) -> Option<&'a str> {
    tool.tool_name
        .namespace
        .as_ref()
