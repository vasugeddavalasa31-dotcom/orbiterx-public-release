...
*/
use crate::exec_env::ORBITERX_PERMISSION_PROFILE_ENV_VAR;
use crate::exec_env::ORBITERX_THREAD_ID_ENV_VAR;
...
use orbiterx_network_proxy::is_managed_mitm_ca_trust_bundle_path;
...
pub(crate) fn is_managed_proxy_env_var(key: &str, value: &str) -> bool {
    ...
    #[cfg(target_os = "macos")]
    {
        key == PROXY_GIT_SSH_COMMAND_ENV_KEY
            && value.starts_with(ORBITERX_PROXY_GIT_SSH_COMMAND_MARKER)
    }
...
/// PATH entries owned by OrbiterX runtime setup.
///
...
/// runtime-only vars like `ORBITERX_THREAD_ID` without pretending they came from
/// the explicit override policy.
///
/// `runtime_path_prepends` contains OrbiterX-owned PATH entries already applied to
...
pub(crate) fn maybe_wrap_shell_lc_with_snapshot(
    ...
    for key in [
        ORBITERX_THREAD_ID_ENV_VAR,
        ORBITERX_PERMISSION_PROFILE_ENV_VAR,
    ] {
    ...
    let (override_captures, override_exports) =
        build_override_exports(&override_env, &[ORBITERX_PERMISSION_PROFILE_ENV_VAR]);
...
fn build_override_exports(
    ...
    build_override_exports_for_keys("__ORBITERX_SNAPSHOT_OVERRIDE", &keys)
}
...
fn build_proxy_env_exports() -> (String, String) {
    ...
    let (captures, restores) =
        build_override_exports_for_keys("__ORBITERX_SNAPSHOT_PROXY_OVERRIDE", &keys);
    let key = PROXY_ACTIVE_ENV_KEY;
    let proxy_blocks = (
        format!("{captures}\n__ORBITERX_SNAPSHOT_PROXY_ENV_SET=\"${{{key}+x}}\""),
        format!(
            "if [ -n \"$__ORBITERX_SNAPSHOT_PROXY_ENV_SET\" ] || [ -n \"${{{key}+x}}\" ]; then\n{restores}\nfi"
        ),
    );
    let git_blocks = build_orbiterx_proxy_git_ssh_command_exports();
    (
...
#[cfg(target_os = "macos")]
fn build_orbiterx_proxy_git_ssh_command_exports() -> (String, String) {
    let key = PROXY_GIT_SSH_COMMAND_ENV_KEY;
    let marker_pattern = format!("{}\\ *", ORBITERX_PROXY_GIT_SSH_COMMAND_MARKER.trim_end());
    (
        format!(
            "__ORBITERX_SNAPSHOT_PROXY_GIT_SSH_COMMAND_SET=\"${{{key}+x}}\"\n__ORBITERX_SNAPSHOT_PROXY_GIT_SSH_COMMAND=\"${{{key}-}}\"\ncase \"$__ORBITERX_SNAPSHOT_PROXY_GIT_SSH_COMMAND\" in\n  {marker_pattern}) __ORBITERX_SNAPSHOT_PROXY_GIT_SSH_COMMAND_LIVE_MARKED=1 ;;\n  *) __ORBITERX_SNAPSHOT_PROXY_GIT_SSH_COMMAND_LIVE_MARKED= ;;\nesac"
        ),
        format!(
            "case \"${{{key}-}}\" in\n  {marker_pattern}) __ORBITERX_SNAPSHOT_PROXY_GIT_SSH_COMMAND_AFTER_MARKED=1 ;;\n  *) __ORBITERX_SNAPSHOT_PROXY_GIT_SSH_COMMAND_AFTER_MARKED= ;;\nesac\nif [ -n \"$__ORBITERX_SNAPSHOT_PROXY_GIT_SSH_COMMAND_LIVE_MARKED\" ]; then\n  if [ -z \"${{{key}+x}}\" ] || [ -n \"$__ORBITERX_SNAPSHOT_PROXY_GIT_SSH_COMMAND_AFTER_MARKED\" ]; then\n    export {key}=\"$__ORBITERX_SNAPSHOT_PROXY_GIT_SSH_COMMAND\"\n  fi\nelif [ -n \"$__ORBITERX_SNAPSHOT_PROXY_GIT_SSH_COMMAND_AFTER_MARKED\" ]; then\n  if [ -n \"$__ORBITERX_SNAPSHOT_PROXY_GIT_SSH_COMMAND_SET\" ]; then\n    export {key}=\"$__ORBITERX_SNAPSHOT_PROXY_GIT_SSH_COMMAND\"\n  else\n    unset {key}\n  fi\nfi"
        ),
    )
}
