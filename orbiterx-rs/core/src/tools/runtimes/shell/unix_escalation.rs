...
use orbiterx_execpolicy::MatchOptions;
...
use orbiterx_protocol::protocol::NetworkPolicyRuleAction;
...
use orbiterx_shell_command::bash::parse_shell_lc_plain_commands;
use orbiterx_shell_command::bash::parse_shell_lc_single_command_prefix;
use orbiterx_shell_escalation::EscalateServer;
use orbiterx_shell_escalation::EscalationDecision;
use orbiterx_shell_escalation::EscalationExecution;
use orbiterx_shell_escalation::EscalationPermissions;
use orbiterx_shell_escalation::EscalationPolicy;
use orbiterx_shell_escalation::EscalationPolicyFuture;
use orbiterx_shell_escalation::EscalationSession;
use orbiterx_shell_escalation::ExecParams;
use orbiterx_shell_escalation::ExecResult;
use orbiterx_shell_escalation::PreparedExec;
use orbiterx_shell_escalation::ResolvedPermissionProfile;
use orbiterx_shell_escalation::ShellCommandExecutor;
use orbiterx_shell_escalation::ShellCommandExecutorFuture;
use orbiterx_shell_escalation::Stopwatch;
...
pub(super) async fn try_run_zsh_fork(
    ...
    let command_executor = CoreShellCommandExecutor {
        ...
        sandbox_policy_cwd,
        windows_sandbox_workspace_roots,
        orbiterx_linux_sandbox_exe: ctx.turn.config.orbiterx_linux_sandbox_exe.clone(),
        ...
        Ok(stopwatch
            .pause_for(async move {
                ...
                let permission_request = PermissionRequestPayload::bash(
                    orbiterx_shell_command::parse_command::shlex_join(&command),
                    /*description*/ None,
                );
...
struct CoreShellCommandExecutor {
    ...
    network: Option<orbiterx_network_proxy::NetworkProxy>,
...
impl CoreShellCommandExecutor {
    async fn run(
        ...
        for var in ["ORBITERX_ESCALATE_SOCKET", "EXEC_WRAPPER"] {
    ...
    fn prepare_sandboxed_exec(
        ...
        let exec_request = sandbox_manager.transform(SandboxTransformRequest {
            ...
            orbiterx_linux_sandbox_exe: self.orbiterx_linux_sandbox_exe.as_deref(),
...
fn map_exec_result(
    ...
    if result.timed_out {
        return Err(ToolError::OrbiterX(OrbiterXErr::Sandbox(
            SandboxErr::Timeout {
