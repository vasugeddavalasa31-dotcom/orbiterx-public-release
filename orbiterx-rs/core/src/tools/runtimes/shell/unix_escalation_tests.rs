...
use orbiterx_execpolicy::PolicyParser;
...
use orbiterx_sandboxing::policy_transforms::effective_permission_profile;
use orbiterx_shell_escalation::EscalationExecution;
use orbiterx_shell_escalation::EscalationPermissions;
use orbiterx_shell_escalation::ExecResult;
use orbiterx_shell_escalation::ResolvedPermissionProfile;
...
fn extract_shell_script_supports_wrapped_command_prefixes() {
    assert_eq!(
        extract_shell_script(&[
            "/usr/bin/env".into(),
            "ORBITERX_EXECVE_WRAPPER=1".into(),
            "/bin/zsh".into(),
            "-lc".into(),
            "echo hello".into()
        ])
...
async fn preapproved_additional_permissions_escalate_intercepted_exec() -> anyhow::Result<()> {
    ...
    let provider = CoreShellActionProvider {
        policy: Arc::new(RwLock::new(orbiterx_execpolicy::Policy::empty())),
        ...
        stopwatch: orbiterx_shell_escalation::Stopwatch::new(Duration::from_secs(1)),
    };

    let action = orbiterx_shell_escalation::EscalationPolicy::determine_action(
        &provider,
    ...
    let expected = orbiterx_shell_escalation::EscalationDecision::Escalate(
        EscalationExecution::Permissions(EscalationPermissions::ResolvedPermissionProfile(
            ResolvedPermissionProfile { permission_profile },
        )),
    );
...
async fn execve_permission_request_hook_short_circuits_prompt() -> anyhow::Result<()> {
    ...
    std::fs::create_dir_all(&turn_context.config.orbiterx_home)
        .context("recreate orbiterx home for hook fixtures")?;
    ...
    let config_toml_path = turn_context
        .config
        .orbiterx_home
        .join(orbiterx_config::CONFIG_TOML_FILE);
    ...
    let expected_hook_command = orbiterx_shell_command::parse_command::shlex_join(&[
        "/usr/bin/touch".to_string(),
        target_str,
    ]);
    let provider = CoreShellActionProvider {
        policy: std::sync::Arc::new(RwLock::new(orbiterx_execpolicy::Policy::empty())),
    ...
    let action = tokio::time::timeout(
        ...
        orbiterx_shell_escalation::EscalationPolicy::determine_action(
            &provider,
    ...
    assert!(matches!(
        action,
        orbiterx_shell_escalation::EscalationDecision::Escalate(
            orbiterx_shell_escalation::EscalationExecution::Unsandboxed
        )
    ));
...
"#
    ...
    assert_eq!(action, orbiterx_shell_escalation::EscalationDecision::Run);
    Ok(())
}
...
async fn denied_reads_keep_granular_sandbox_rejection_for_escalation() -> anyhow::Result<()> {
    ...
    assert_eq!(
        action,
        orbiterx_shell_escalation::EscalationDecision::Deny {
