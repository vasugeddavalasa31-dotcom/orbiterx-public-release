...
use orbiterx_tools::ConversationHistory;
use orbiterx_tools::ExtensionTurnItem;
use orbiterx_tools::ToolCall as ExtensionToolCall;
use orbiterx_tools::ToolEnvironment;
use orbiterx_tools::ToolName;
use orbiterx_tools::ToolSearchInfo;
use orbiterx_tools::ToolSpec;
use orbiterx_tools::TurnItemEmissionFuture;
use orbiterx_tools::TurnItemEmitter;
...
pub(crate) struct ExtensionToolAdapter(Arc<dyn orbiterx_tools::ToolExecutor<ExtensionToolCall>>);

impl ExtensionToolAdapter {
    pub(crate) fn new(executor: Arc<dyn orbiterx_tools::ToolExecutor<ExtensionToolCall>>) -> Self {
        Self(executor)
    }
}
...
async fn to_extension_call(invocation: &ToolInvocation) -> ExtensionToolCall {
    ...
    let orbiterx_turn_metadata = invocation
        .turn
        .turn_metadata_state
...
mod tests {
    ...
    use orbiterx_extension_items::web_search::WebSearchItem;
    use orbiterx_protocol::items::TurnItem;
    ...
    impl orbiterx_extension_api::ToolExecutor<orbiterx_tools::ToolCall> for StubExtensionExecutor {
        fn tool_name(&self) -> orbiterx_tools::ToolName {
            orbiterx_tools::ToolName::plain("extension_echo")
        }

        fn spec(&self) -> orbiterx_tools::ToolSpec {
            orbiterx_tools::ToolSpec::Function(orbiterx_tools::ResponsesApiTool {
                name: "extension_echo".to_string(),
                description: "Echoes arguments.".to_string(),
                strict: true,
                parameters: orbiterx_tools::parse_tool_input_schema(&json!({
                    "type": "object",
                    "properties": {
                        "message": { "type": "string" },
                    },
        ...
        fn handle(
            &self,
            _call: orbiterx_tools::ToolCall,
        ) -> orbiterx_tools::ToolExecutorFuture<'_> {
            Box::pin(async {
                Ok(
                    Box::new(orbiterx_tools::JsonToolOutput::new(json!({ "ok": true })))
                        as Box<dyn orbiterx_tools::ToolOutput>,
                )
            })
        }
    }
    ...
    struct CapturingExtensionExecutor {
        captured_call: Arc<Mutex<Option<orbiterx_tools::ToolCall>>>,
    }

    impl orbiterx_extension_api::ToolExecutor<orbiterx_tools::ToolCall> for CapturingExtensionExecutor {
        fn tool_name(&self) -> orbiterx_tools::ToolName {
            orbiterx_tools::ToolName::plain("extension_echo")
        }
        ...
        fn handle(&self, call: orbiterx_tools::ToolCall) -> orbiterx_tools::ToolExecutorFuture<'_> {
    ...
    impl CapturingExtensionExecutor {
        async fn handle_call(
            &self,
            call: orbiterx_tools::ToolCall,
        ) -> Result<Box<dyn orbiterx_tools::ToolOutput>, orbiterx_tools::FunctionCallError>
        {
    ...
    async fn exposes_generic_hook_payloads() {
        ...
        let invocation = ToolInvocation {
            ...
            tool_name: orbiterx_tools::ToolName::plain("extension_echo"),
            source: ToolCallSource::Direct,
            payload: ToolPayload::Function {
                arguments: json!({ "message": "hello" }).to_string(),
            },
        };
        let output = orbiterx_tools::JsonToolOutput::new(json!({ "ok": true }));
    ...
    async fn image_generation_publication_preserves_extension_saved_path() {
        ...
        orbiterx_tools::TurnItemEmitter::emit_started(
            &emitter,
            ExtensionTurnItem {
        ...
        .await;
        orbiterx_tools::TurnItemEmitter::emit_completed(
            &emitter,
            ExtensionTurnItem {
