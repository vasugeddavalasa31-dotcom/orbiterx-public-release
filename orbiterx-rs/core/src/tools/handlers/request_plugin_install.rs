...
use orbiterx_analytics::PluginInstallRequestSource;
use orbiterx_analytics::PluginInstallRequested;
use orbiterx_analytics::PluginInstallRequestedPlugin;
...
use orbiterx_tools::DiscoverableToolAction;
use orbiterx_tools::DiscoverableToolType;
use orbiterx_tools::LIST_AVAILABLE_PLUGINS_TO_INSTALL_TOOL_NAME;
use orbiterx_tools::REQUEST_PLUGIN_INSTALL_PERSIST_ALWAYS_VALUE;
use orbiterx_tools::REQUEST_PLUGIN_INSTALL_PERSIST_KEY;
use orbiterx_tools::REQUEST_PLUGIN_INSTALL_TOOL_NAME;
use orbiterx_tools::RequestPluginInstallArgs;
use orbiterx_tools::RequestPluginInstallResult;
use orbiterx_tools::ToolName;
use orbiterx_tools::ToolSpec;
use orbiterx_tools::all_requested_connectors_picked_up;
use orbiterx_tools::build_request_plugin_install_elicitation_request;
use orbiterx_tools::filter_request_plugin_install_discoverable_tools_for_client;
use orbiterx_tools::verified_connector_install_completed;
...
impl RequestPluginInstallHandler {
    async fn handle_call(
        ...
        if (requested_tool_type == Some(DiscoverableToolType::Plugin)
            ...
            && turn.app_server_client_name.as_deref() == Some("orbiterx-tui")
        {
            return Err(FunctionCallError::RespondToModel(
                "plugin install requests are not available in orbiterx-tui yet".to_string(),
            ));
        }
...
async fn maybe_persist_disabled_install_request(
    ...
    if let Err(err) = persist_disabled_install_request(&turn.config.orbiterx_home, tool).await {
        warn!(
            error = %err,
            tool_id = tool.id(),
...
async fn persist_disabled_install_request(
    orbiterx_home: &orbiterx_utils_absolute_path::AbsolutePathBuf,
    tool: &DiscoverableTool,
) -> anyhow::Result<()> {
...
async fn verify_request_plugin_install_completed(
    session: &crate::session::session::Session,
    turn: &crate::session::turn_context::TurnContext,
    manager: &orbiterx_mcp::McpConnectionManager,
    tool: &DiscoverableTool,
    auth: Option<&orbiterx_login::OrbiterXAuth>,
) -> bool {
    match tool {
...
async fn refresh_missing_requested_connectors(
    ...
    match manager.hard_refresh_orbiterx_apps_tools_cache().await {
        Ok(mcp_tools) => {
            let accessible_connectors = connectors::with_app_enabled_state(
                connectors::accessible_connectors_from_mcp_tools(&mcp_tools),
                &turn.config,
        ...
        Err(err) => {
            warn!(
                "failed to refresh orbiterx apps tools cache after plugin install request for {tool_id}: {err:#}"
            );
            None
        }
    }
