...
pub(crate) fn collab_spawn_error(err: OrbiterXErr) -> FunctionCallError {
    match err {
        OrbiterXErr::UnsupportedOperation(message) if message == "thread manager dropped" => {
            FunctionCallError::RespondToModel("collab manager unavailable".to_string())
        }
        OrbiterXErr::UnsupportedOperation(message) => FunctionCallError::RespondToModel(message),
        err => FunctionCallError::RespondToModel(format!("collab spawn failed: {err}")),
    }
}

pub(crate) fn collab_agent_error(agent_id: ThreadId, err: OrbiterXErr) -> FunctionCallError {
    match err {
        OrbiterXErr::ThreadNotFound(id) => {
