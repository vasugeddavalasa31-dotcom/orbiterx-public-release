...
use orbiterx_code_mode::CellId;
use orbiterx_code_mode::CodeModeNestedToolCall;
use orbiterx_code_mode::CodeModeSession;
use orbiterx_code_mode::CodeModeSessionProvider;
use orbiterx_code_mode::CodeModeToolKind;
use orbiterx_code_mode::RuntimeResponse;
...
use orbiterx_utils_output_truncation::formatted_truncate_text_content_items_with_policy;
use orbiterx_utils_output_truncation::truncate_function_output_items_with_policy;
...
pub(crate) const PUBLIC_TOOL_NAME: &str = orbiterx_code_mode::PUBLIC_TOOL_NAME;
pub(crate) const WAIT_TOOL_NAME: &str = orbiterx_code_mode::WAIT_TOOL_NAME;
pub(crate) const DEFAULT_WAIT_YIELD_TIME_MS: u64 = orbiterx_code_mode::DEFAULT_WAIT_YIELD_TIME_MS;
...
impl CodeModeService {
    ...
    pub(crate) async fn execute(
        &self,
        request: orbiterx_code_mode::ExecuteRequest,
    ) -> Result<orbiterx_code_mode::StartedCell, String> {
        self.session().await?.execute(request).await
    }

    pub(crate) async fn wait(
        &self,
        request: orbiterx_code_mode::WaitRequest,
    ) -> Result<orbiterx_code_mode::WaitOutcome, String> {
    ...
    pub(crate) fn mark_cell_ready_for_dispatch(&self, cell_id: &orbiterx_code_mode::CellId) {
...
mod tests {
    ...
    use crate::tools::context::ToolPayload;
    use orbiterx_code_mode::CodeModeToolKind;
    use orbiterx_code_mode::ExecuteRequest;
    use orbiterx_code_mode::FunctionCallOutputContentItem as CodeModeOutputContentItem;
    ...
    async fn missing_process_host_falls_back_to_in_process_session() {
        let service = CodeModeService::new(Arc::new(
            ProcessOwnedCodeModeSessionProvider::with_host_program(
                "orbiterx-code-mode-host-does-not-exist".into(),
            ),
        ));
        ...
        assert_eq!(
            response,
            RuntimeResponse::Result {
                cell_id: orbiterx_code_mode::CellId::new("1".to_string()),
