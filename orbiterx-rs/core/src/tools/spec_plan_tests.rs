...
use orbiterx_protocol::openai_models::ApplyPatchToolType;
use orbiterx_protocol::openai_models::ConfigShellToolType;
use orbiterx_protocol::openai_models::InputModality;
use orbiterx_protocol::openai_models::ToolMode;
use orbiterx_protocol::openai_models::WebSearchToolType;
...
use orbiterx_tools::ToolCall as ExtensionToolCall;
use orbiterx_tools::ToolExecutor;
use orbiterx_tools::ToolExposure;
use orbiterx_tools::ToolName;
use orbiterx_tools::ToolOutput;
...
fn zsh_fork_config_for_spec_plan_tests() -> orbiterx_tools::ZshForkConfig {
    let placeholder_exe = orbiterx_utils_absolute_path::AbsolutePathBuf::try_from(
...
impl ToolExecutor<ExtensionToolCall> for TestNamespaceExtensionTool {
    ...
    fn spec(&self) -> ToolSpec {
        ToolSpec::Namespace(orbiterx_tools::ResponsesApiNamespace {
            ...
            tools: vec![ResponsesApiNamespaceTool::Function(ResponsesApiTool {
                ...
                strict: false,
                defer_loading: None,
                parameters: orbiterx_tools::JsonSchema::default(),
                output_schema: None,
            })],
        })
    }

    fn handle(&self, _call: ExtensionToolCall) -> orbiterx_tools::ToolExecutorFuture<'_> {
        Box::pin(async {
            Ok(Box::new(orbiterx_tools::JsonToolOutput::new(json!({}))) as Box<dyn ToolOutput>)
        })
    }
}
...
impl ToolExecutor<ExtensionToolCall> for DeferredExtensionTool {
    ...
    fn spec(&self) -> ToolSpec {
        ToolSpec::Function(ResponsesApiTool {
            ...
            strict: true,
            defer_loading: None,
            parameters: orbiterx_tools::JsonSchema::object(
                BTreeMap::from([(
                    "message".to_string(),
                    orbiterx_tools::JsonSchema::string(/*description*/ None),
                )]),
...
fn dynamic_tool(namespace: Option<&str>, name: &str, defer_loading: bool) -> DynamicToolSpec {
    let function = orbiterx_protocol::dynamic_tools::DynamicToolFunctionSpec {
        name: name.to_string(),
    ...
    match namespace {
        Some(namespace) => {
            DynamicToolSpec::Namespace(orbiterx_protocol::dynamic_tools::DynamicToolNamespaceSpec {
                name: namespace.to_string(),
                description: format!("{namespace} dynamic tools"),
                tools: vec![
                    orbiterx_protocol::dynamic_tools::DynamicToolNamespaceTool::Function(function),
                ],
            })
        }
...
async fn request_user_input_stays_direct_in_code_mode_only() {
    ...
    plan.assert_visible_contains(&[
        "request_user_input",
        orbiterx_code_mode::PUBLIC_TOOL_NAME,
        orbiterx_code_mode::WAIT_TOOL_NAME,
    ]);
    ...
    let ToolSpec::Freeform(exec) = plan.visible_spec(orbiterx_code_mode::PUBLIC_TOOL_NAME) else {
...
async fn shell_zsh_fork_stays_standalone_until_unified_exec_composition_is_enabled() {
    ...
    if orbiterx_utils_pty::conpty_supported() {
...
async fn zsh_fork_unified_exec_hides_shell_parameter() {
    ...
    let plan = probe(|turn| {
        ...
        turn.unified_exec_shell_mode =
            orbiterx_tools::UnifiedExecShellMode::ZshFork(zsh_fork_config_for_spec_plan_tests());
    })
    .await;
        ...
        turn.environments
            ...
            .push(TurnEnvironmentState::Ready(
                crate::session::turn_context::TurnEnvironment::new(
                    "remote".to_string(),
                    Arc::new(
                        orbiterx_exec_server::Environment::create_for_tests(Some(
...
async fn code_mode_only_exposes_code_executor_and_hides_nested_tools() {
    ...
    assert_eq!(
        plain.namespace_function_names("orbiterx_app"),
        &["lookup".to_string()]
    );
    ...
    assert_eq!(
        code_mode_only.namespace_function_names("orbiterx_app"),
