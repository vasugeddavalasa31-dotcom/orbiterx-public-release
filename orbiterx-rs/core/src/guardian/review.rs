...
use orbiterx_analytics::GuardianReviewAnalyticsResult;
use orbiterx_analytics::GuardianReviewDecision;
use orbiterx_analytics::GuardianReviewFailureReason;
use orbiterx_analytics::GuardianReviewTerminalStatus;
use orbiterx_analytics::GuardianReviewTrackContext;
use orbiterx_analytics::GuardianReviewedAction;
...
use orbiterx_protocol::protocol::GuardianAssessmentDecisionSource;
...
pub(super) enum GuardianReviewError {
    ...
    Session {
        message: String,
        error_info: Option<OrbiterXErrorInfo>,
    },
    Parse {
        message: String,
    },
...
impl GuardianReviewError {
    ...
    fn session_with_error_info(err: anyhow::Error, error_info: OrbiterXErrorInfo) -> Self {
        Self::Session {
            message: err.to_string(),
...
pub(crate) fn is_guardian_reviewer_source(
    session_source: &orbiterx_protocol::protocol::SessionSource,
) -> bool {
    matches!(
        session_source,
        orbiterx_protocol::protocol::SessionSource::SubAgent(SubAgentSource::Other(label))
    ...
    let (guardian_model, guardian_reasoning_effort) = if let Some(preset) = review_model {
        let reasoning_effort = preferred_reasoning_effort(
            preset.supported_reasoning_efforts.iter().any(|effort| {
                effort.effort == orbiterx_protocol::openai_models::ReasoningEffort::Low
            }),
        ...
        let reasoning_effort = preferred_reasoning_effort(
            turn.model_info
                ...
                .iter()
                .any(|preset| {
                    preset.effort == orbiterx_protocol::openai_models::ReasoningEffort::Low
                }),
            ...
            GuardianReviewError::Session {
                error_info: Some(
                    OrbiterXErrorInfo::ServerOverloaded
                        | OrbiterXErrorInfo::HttpConnectionFailed { .. }
                        | OrbiterXErrorInfo::ResponseStreamConnectionFailed { .. }
                        | OrbiterXErrorInfo::InternalServerError
                        | OrbiterXErrorInfo::ResponseStreamDisconnected { .. }
                ),
                ..
            } | GuardianReviewError::Parse { .. }
        )
