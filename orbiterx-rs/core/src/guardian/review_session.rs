...
use orbiterx_analytics::GuardianReviewSessionAnalyticsParams;
use orbiterx_analytics::GuardianReviewSessionKind;
...
use orbiterx_protocol::openai_models::ModelMessages;
...
use crate::orbiterx_delegate::run_orbiterx_thread_interactive;
...
impl GuardianReviewSessionReuseKey {
    fn from_spawn_config(
        ...
        Self {
            ...
            orbiterx_linux_sandbox_exe: spawn_config.orbiterx_linux_sandbox_exe.clone(),
                                ...
                                && let Some(error) = last_error
                            {
                                return (
                                    GuardianReviewSessionOutcome::SessionFailed {
                                        ...
                                        error_info: error.orbiterx_error_info,
                                    },
                                    true,
                                    true,
                                );
...
pub(crate) fn build_guardian_review_session_config(
    ...
    live_network_config: Option<orbiterx_network_proxy::NetworkProxyConfig>,
    active_model: &str,
...
mod tests {
    use super::*;
    use orbiterx_protocol::openai_models::AutoReviewMessages;
    ...
    async fn test_review_params() -> GuardianReviewSessionParams {
        ...
        GuardianReviewSessionParams {
            ...
            model,
            reasoning_effort,
            guardian_default_review_model_id: "orbiterx-auto-review".to_string(),
    ...
    async fn guardian_review_session_config_disables_hooks() {
        ...
        assert!(!guardian_config.features.enabled(Feature::OrbiterXHooks));
    }
    ...
    async fn wait_for_guardian_review_preserves_structured_session_error() {
        ...
        tx_event
            .send(Event {
                ...
                msg: EventMsg::Error(ErrorEvent {
                    ...
                    orbiterx_error_info: Some(OrbiterXErrorInfo::ServerOverloaded),
                }),
            })
            .await
        ...
        assert_eq!(error_info, Some(OrbiterXErrorInfo::ServerOverloaded));
        assert!(keep_review_session);
        assert!(capture_token_usage);
    }
