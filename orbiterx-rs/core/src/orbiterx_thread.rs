...
use orbiterx_exec_server::SelectedCapabilityRootsStatus;
...
use orbiterx_thread_store::ThreadStoreResult;
...
/// Explains why `OrbiterXThread::try_start_turn_if_idle` rejected an automatic
/// idle turn.
...
pub struct OrbiterXThreadSettingsOverrides {
...
/// (formerly called a conversation) in OrbiterX.
impl OrbiterXThread {
    pub(crate) fn new(
        session: Arc<Session>,
        io: SessionIo,
    ...
    pub async fn submit(&self, op: Op) -> OrbiterXResult<String> {
    ...
    pub async fn shutdown_and_wait(&self) -> OrbiterXResult<()> {
    ...
    pub(crate) async fn emit_thread_resume_lifecycle(&self) {
        ...
        {
            contributor
                .on_thread_resume(orbiterx_extension_api::ThreadResumeInput {
    ...
    /// callers that only hold a `OrbiterXThread`.
    ...
    pub async fn preview_thread_settings_overrides(
        &self,
        overrides: OrbiterXThreadSettingsOverrides,
    ...
    async fn thread_settings_update(
        ...
        let OrbiterXThreadSettingsOverrides {
            environments,
    ...
    pub async fn submit_with_id(&self, sub: Submission) -> OrbiterXResult<()> {
        self.io.submit_with_id(sub).await
    }

    pub async fn next_event(&self) -> OrbiterXResult<Event> {
    ...
    pub async fn inject_response_items(&self, items: Vec<ResponseItem>) -> OrbiterXResult<()> {
        if items.is_empty() {
    ...
    pub async fn increment_out_of_band_elicitation_count(&self) -> OrbiterXResult<i64> {
        let mut elicitations = self.out_of_band_elicitations.lock().await;
        let incremented = elicitations.count.checked_add(1).ok_or_else(|| {
            OrbiterXErr::Fatal("out-of-band elicitation count overflowed".to_string())
        })?;
    ...
    pub async fn decrement_out_of_band_elicitation_count(&self) -> OrbiterXResult<i64> {
        let mut elicitations = self.out_of_band_elicitations.lock().await;
        if elicitations.count == 0 {
            return Err(OrbiterXErr::InvalidRequest(
