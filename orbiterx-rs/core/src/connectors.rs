...
pub use orbiterx_connectors::AppBranding;
pub use orbiterx_connectors::AppInfo;
pub use orbiterx_connectors::AppMetadata;
use orbiterx_connectors::ConnectorDirectoryCacheContext;
use orbiterx_connectors::ConnectorDirectoryCacheKey;
use orbiterx_connectors::app_is_enabled;
use orbiterx_connectors::apps_config_from_layer_stack;
...
use orbiterx_mcp::ToolInfo;
use orbiterx_mcp::ToolPluginProvenance;
use orbiterx_mcp::effective_mcp_servers;
use orbiterx_mcp::tool_plugin_provenance;
...
pub(crate) async fn list_tool_suggest_discoverable_tools_with_auth(
    ...
    let directory_connectors = orbiterx_connectors::merge::merge_plugin_connectors(
    ...
    let discoverable_connectors =
        orbiterx_connectors::filter::filter_tool_suggest_discoverable_connectors(
...
pub async fn list_cached_accessible_connectors_from_mcp_tools(
    ...
    if !config.features.apps_enabled_for_auth(
        auth.as_ref()
            .is_some_and(OrbiterXAuth::uses_orbiterx_backend),
    ) {
        return Some(Vec::new());
    }
...
pub async fn list_accessible_connectors_from_mcp_tools_with_options_and_status(
    ...
    let environment_manager = EnvironmentManager::from_orbiterx_home(
...
pub async fn list_accessible_connectors_from_mcp_tools_with_mcp_manager(
    ...
    mcp_servers.retain(|name, _| name == ORBITERX_APPS_MCP_SERVER_NAME);
    ...
    let orbiterx_apps_auth_manager =
        orbiterx_mcp::host_owned_orbiterx_apps_enabled(&mcp_config, auth.as_ref())
    ...
    let mcp_connection_manager = McpConnectionManager::new(
        ...
        mcp_manager.orbiterx_apps_tools_cache(),
    ...
    let refreshed_tools = if force_refetch {
        match mcp_connection_manager
            .hard_refresh_orbiterx_apps_tools_cache()
            .await
        {
            Ok(tools) => Some(tools),
            Err(err) => {
                warn!(
                    "failed to force-refresh tools for MCP server '{ORBITERX_APPS_MCP_SERVER_NAME}', using cached/startup tools: {err:#}"
                );
                None
            }
        }
    ...
    let orbiterx_apps_ready = if refreshed_tools_succeeded {
        true
    } else if let Some(cfg) = mcp_servers.get(ORBITERX_APPS_MCP_SERVER_NAME) {
        let immediate_ready = mcp_connection_manager
            .wait_for_server_ready(ORBITERX_APPS_MCP_SERVER_NAME, Duration::ZERO)
            .await;
        if immediate_ready {
            true
            ...
            let ready = mcp_connection_manager
                .wait_for_server_ready(ORBITERX_APPS_MCP_SERVER_NAME, timeout)
                .await;
    ...
    if orbiterx_apps_ready || !accessible_connectors.is_empty() {
        write_cached_accessible_connectors(cache_key, &accessible_connectors);
    }
...
fn accessible_connectors_cache_key(
    ...
    let account_id = auth.and_then(OrbiterXAuth::get_account_id);
    let chatgpt_user_id = auth.and_then(OrbiterXAuth::get_chatgpt_user_id);
    let is_workspace_account = auth.is_some_and(OrbiterXAuth::is_workspace_account);
    AccessibleConnectorsCacheKey {
...
fn write_cached_accessible_connectors(
    ...
    *cache_guard = Some(CachedAccessibleConnectors {
        key: cache_key,
        expires_at: Instant::now() + orbiterx_connectors::CONNECTORS_CACHE_TTL,
...
async fn cached_directory_connectors_for_tool_suggest_with_auth(
    ...
    orbiterx_connectors::cached_directory_connectors(&cache_context).unwrap_or_default()
}
...
fn collect_accessible_connectors_from_mcp_tools<'a>(
    ...
    let tools = mcp_tools.filter_map(|tool| {
        if tool.server_name != ORBITERX_APPS_MCP_SERVER_NAME {
            return None;
        }
        let connector_id = tool.connector_id.as_deref()?;
        Some(orbiterx_connectors::accessible::AccessibleConnectorTool {
    ...
    orbiterx_connectors::accessible::collect_accessible_connectors(tools)
}
...
pub(crate) fn mcp_approvals_reviewer(
    ...
    let app_reviewer = if server_name == ORBITERX_APPS_MCP_SERVER_NAME {
