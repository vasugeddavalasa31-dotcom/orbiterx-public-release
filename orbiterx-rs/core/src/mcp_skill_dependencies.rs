...
use orbiterx_config::ConfigEditsBuilder;
use orbiterx_config::McpServerConfig;
use orbiterx_config::McpServerTransportConfig;
use orbiterx_config::load_global_mcp_servers;
use orbiterx_login::default_client::is_first_party_originator;
...
pub(crate) async fn maybe_prompt_and_install_mcp_dependencies(
    ...
    if mentioned_skills.is_empty()
        || !config
            .features
            .enabled(orbiterx_features::Feature::SkillMcpDependencyInstall)
    {
        return;
    }
...
pub(crate) async fn maybe_install_mcp_dependencies(
    ...
    let mut servers = match load_global_mcp_servers(&orbiterx_home).await {
        Ok(servers) => servers,
        Err(err) => {
    ...
    if let Err(err) = ConfigEditsBuilder::new(&orbiterx_home)
