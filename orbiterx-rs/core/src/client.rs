...
//!
//! `ModelClient` is intended to live for the lifetime of a OrbiterX session and holds the stable
...
//! per-turn state such as the `x-orbiterx-turn-state` token used for sticky routing.
//!
...
use orbiterx_api::AgentIdentityTelemetry;
use orbiterx_api::ApiError;
use orbiterx_api::AuthProvider;
use orbiterx_api::CompactClient as ApiCompactClient;
use orbiterx_api::CompactionInput as ApiCompactionInput;
use orbiterx_api::Compression;
use orbiterx_api::MemoriesClient as ApiMemoriesClient;
use orbiterx_api::MemorySummarizeInput as ApiMemorySummarizeInput;
use orbiterx_api::MemorySummarizeOutput as ApiMemorySummarizeOutput;
use orbiterx_api::Provider as ApiProvider;
use orbiterx_api::RawMemory as ApiRawMemory;
use orbiterx_api::RealtimeCallClient as ApiRealtimeCallClient;
use orbiterx_api::RealtimeSessionConfig as ApiRealtimeSessionConfig;
use orbiterx_api::Reasoning;
use orbiterx_api::ReasoningContext;
use orbiterx_api::RequestTelemetry;
use orbiterx_api::ReqwestTransport;
use orbiterx_api::ResponseCreateWsRequest;
use orbiterx_api::ResponsesApiRequest;
use orbiterx_api::ResponsesClient as ApiResponsesClient;
use orbiterx_api::ResponsesOptions as ApiResponsesOptions;
use orbiterx_api::ResponsesWebsocketClient as ApiWebSocketResponsesClient;
use orbiterx_api::ResponsesWebsocketConnection as ApiWebSocketConnection;
use orbiterx_api::ResponsesWsRequest;
use orbiterx_api::SharedAuthProvider;
use orbiterx_api::SseTelemetry;
use orbiterx_api::StreamOptions;
use orbiterx_api::TransportError;
use orbiterx_api::WebsocketTelemetry;
use orbiterx_api::auth_header_telemetry;
use orbiterx_api::build_session_headers;
use orbiterx_api::create_text_param_for_request;
use orbiterx_api::response_create_client_metadata;
use orbiterx_http_client::ClientRouteClass;
use orbiterx_http_client::HttpClientFactory;
use orbiterx_login::AuthManager;
use orbiterx_login::OrbiterXAuth;
use orbiterx_login::RefreshTokenError;
use orbiterx_login::UnauthorizedRecovery;
use orbiterx_login::default_client::add_originator_header;
use orbiterx_login::default_client::build_default_reqwest_client_for_route;
...
use orbiterx_protocol::config_types::Verbosity as VerbosityConfig;
...
use orbiterx_protocol::protocol::InternalSessionSource;
use orbiterx_protocol::protocol::SessionSource;
use orbiterx_protocol::protocol::W3cTraceContext;
use orbiterx_rollout_trace::CompactionTraceContext;
use orbiterx_rollout_trace::InferenceTraceAttempt;
use orbiterx_rollout_trace::InferenceTraceContext;
use orbiterx_tools::create_tools_json_for_responses_api;
...
use orbiterx_feedback::FeedbackRequestTags;
use orbiterx_feedback::emit_feedback_request_tags_with_auth_env;
use orbiterx_login::auth::AgentIdentityAuthPolicy;
use orbiterx_login::auth_env_telemetry::AuthEnvTelemetry;
use orbiterx_login::auth_env_telemetry::collect_auth_env_telemetry;
use orbiterx_model_provider::AgentIdentitySessionFallback;
use orbiterx_model_provider::ProviderAuthScope;
use orbiterx_model_provider::SharedModelProvider;
use orbiterx_model_provider::create_model_provider;
#[cfg(test)]
use orbiterx_model_provider_info::DEFAULT_WEBSOCKET_CONNECT_TIMEOUT_MS;
...
use orbiterx_response_debug_context::extract_response_debug_context;
use orbiterx_response_debug_context::extract_response_debug_context_from_api_error;
use orbiterx_response_debug_context::telemetry_api_error_message;
use orbiterx_response_debug_context::telemetry_transport_error_message;

pub const OPENAI_BETA_HEADER: &str = "OpenAI-Beta";
pub const X_ORBITERX_INSTALLATION_ID_HEADER: &str = "x-orbiterx-installation-id";
pub const X_ORBITERX_TURN_STATE_HEADER: &str = "x-orbiterx-turn-state";
pub const X_ORBITERX_TURN_METADATA_HEADER: &str = "x-orbiterx-turn-metadata";
pub const X_ORBITERX_PARENT_THREAD_ID_HEADER: &str = "x-orbiterx-parent-thread-id";
pub const X_ORBITERX_WINDOW_ID_HEADER: &str = "x-orbiterx-window-id";
...
const X_ORBITERX_WS_STREAM_REQUEST_START_MS_CLIENT_METADATA_KEY: &str =
    "x-orbiterx-ws-stream-request-start-ms";
const WS_REQUEST_HEADER_RESPONSES_LITE_CLIENT_METADATA_KEY: &str =
    "ws_request_header_x_openai_internal_orbiterx_responses_lite";
const RESPONSES_WEBSOCKETS_V2_BETA_HEADER_VALUE: &str = "responses_websockets=2026-02-06";
const X_OPENAI_INTERNAL_ORBITERX_RESPONSES_LITE_HEADER: &str =
    "x-openai-internal-orbiterx-responses-lite";
...
///
/// This holds configuration and state that should be shared across turns within a OrbiterX session
...
/// - The `x-orbiterx-turn-state` sticky-routing token, which must be replayed for all requests within
///   the same turn.
///
/// Create a fresh `ModelClientSession` for each OrbiterX turn. Reusing it across turns would replay
...
pub struct ModelClientSession {
    ...
    /// on turn start via the `x-orbiterx-turn-state` response header. Once set, this value
    /// should be sent back to the server in the `x-orbiterx-turn-state` request header for
...
impl ModelClient {
    ...
    ///
    /// All arguments are expected to be stable for the lifetime of a OrbiterX session. Per-turn values
    ...
    pub fn new(
        ...
        let orbiterx_api_key_env_enabled = model_provider
            .auth_manager()
            .as_ref()
            .is_some_and(|manager| manager.orbiterx_api_key_env_enabled());
        let auth_env_telemetry =
            collect_auth_env_telemetry(model_provider.info(), orbiterx_api_key_env_enabled);
    ...
    fn prompt_cache_key(&self, responses_metadata: &OrbiterXResponsesMetadata) -> String {
    ...
    pub(crate) fn force_http_fallback(
        ...
        if activated {
            ...
            session_telemetry.counter(
                "orbiterx.transport.fallback_to_http",
                /*inc*/ 1,
    ...
    pub(crate) async fn compact_conversation_history(
        ...
        responses_metadata: &OrbiterXResponsesMetadata,
        ...
        let request_telemetry = Self::build_request_telemetry(
            ...
            AuthRequestTelemetryContext::new(
                client_setup.auth.as_ref().map(OrbiterXAuth::auth_mode),
        ...
        if let Ok(header_value) = HeaderValue::from_str(&responses_metadata.installation_id) {
            extra_headers.insert(X_ORBITERX_INSTALLATION_ID_HEADER, header_value);
        }
    ...
    fn build_responses_request(
        &self,
        provider: &orbiterx_api::Provider,
        prompt: &Prompt,
        model_info: &ModelInfo,
        effort: Option<ReasoningEffortConfig>,
        summary: ReasoningSummaryConfig,
        ...
        .then_some(StreamOptions {
            reasoning_summary_delivery: orbiterx_api::ReasoningSummaryDelivery::SequentialCutoff,
        });
    ...
    async fn connect_websocket(
        &self,
        session_telemetry: &SessionTelemetry,
        api_provider: orbiterx_api::Provider,
        api_auth: SharedAuthProvider,
        responses_metadata: &OrbiterXResponsesMetadata,
        auth_context: AuthRequestTelemetryContext,
        request_route_telemetry: RequestRouteTelemetry,
        ...
        let result = match tokio::time::timeout(
            ...
            ApiWebSocketResponsesClient::new(api_provider, api_auth).connect(
                &self.http_client_factory,
                headers,
                orbiterx_login::default_client::default_headers(),
                /*turn_state*/ None,
...
impl ModelClientSession {
    ...
    fn responses_request_compression(&self, auth: Option<&OrbiterXAuth>) -> Compression {
        if self.client.state.enable_request_compression
            && auth.is_some_and(OrbiterXAuth::uses_orbiterx_backend)
    ...
    async fn stream_responses_websocket(
        ...
        loop {
            ...
            if let Some(turn_state) = self.turn_state.get() {
                client_metadata
                    .insert(X_ORBITERX_TURN_STATE_HEADER.to_string(), turn_state.clone());
            }
    ...
    pub async fn stream(
        ...
        match wire_api {
            ...
            WireApi::AnthropicMessages => {
                self.stream_anthropic_messages_api(prompt, model_info, session_telemetry)
                    .await
            }
        }
    }
    ...
    async fn stream_anthropic_messages_api(
        ...
        let request_url = format!("{}/v1/messages", base_url.trim_end_matches('/'));

        let http_client = build_default_reqwest_client_for_route(
            &self.client.http_client_factory,
            &request_url,
            ClientRouteClass::Api,
        )
        .map_err(|e| {
            orbiterx_protocol::error::OrbiterXErr::Fatal(format!(
        ...
        client_setup.api_auth.add_auth_headers(&mut auth_headers);
        ...
        if !anthropic_tools.is_empty() {
            request_body
                .as_object_mut()
                .unwrap()
                .insert("tools".to_string(), serde_json::json!(anthropic_tools));
        }
        if let Some(sys) = system_prompt {
            request_body
                .as_object_mut()
                .unwrap()
                .insert("system".to_string(), serde_json::json!(sys));
        }
        ...
        let response = req_builder.send().await.map_err(|e| {
            orbiterx_protocol::error::OrbiterXErr::Fatal(format!("Anthropic request failed: {e}"))
        })?;

        if !response.status().is_success() {
            let status = response.status();
            let body_text = response.text().await.unwrap_or_default();
            return Err(orbiterx_protocol::error::OrbiterXErr::Fatal(format!(
                "Anthropic error status {status}: {body_text}"
            )));
        }

        let (tx_event, rx_event) = tokio::sync::mpsc::channel::<Result<ResponseEvent>>(1600);
        ...
        tokio::spawn(async move {
            ...
            while let Some(event_res) = event_stream.next().await {
                let event = match event_res {
                    ...
                    Err(e) => {
                        let _ = tx_event
                            .send(Err(orbiterx_protocol::error::OrbiterXErr::Fatal(format!(
                                "stream error: {e}"
                            ))))
                            .await;
                        return;
                ...
                if event.event == "message_start" {
                    ...
                    if let Ok(block_start) = serde_json::from_str::<BlockStartEvent>(&event.data) {
                        let state = match block_start.content_block {
                            ...
                            BlockStart::ToolUse { id, name } => BlockState {
                                is_tool: true,
                                text: String::new(),
                                tool_id: id,
                                tool_name: name,
                    ...
                    if let Ok(delta_ev) = serde_json::from_str::<DeltaEvent>(&event.data) {
                        if let Some(Some(state)) = blocks.get_mut(delta_ev.index) {
                    ...
                    if let Ok(stop_ev) = serde_json::from_str::<StopEvent>(&event.data) {
                        if let Some(opt_state) = blocks.get_mut(stop_ev.index) {
                            if let Some(state) = opt_state.take() {
                                if state.is_tool {
                                    let (namespace, name) = match state.tool_name.split_once("__") {
                                        Some((ns, name)) => {
                                            (Some(ns.to_string()), name.to_string())
                                        }
                                        None => (None, state.tool_name.clone()),
                                    };
                                    let item = ResponseItem::FunctionCall {
                                        id: Some(orbiterx_protocol::ResponseItemId::from_server(
                                            format!("item_{}", state.tool_id),
                                        )),
                                        name,
                                        namespace,
                                        arguments: state.tool_input_json,
                                    ...
                                    let item = ResponseItem::Message {
                                        id: Some(orbiterx_protocol::ResponseItemId::from_server(
                                            format!("item_text_{}", stop_ev.index),
                                        )),
    ...
    /// Permanently disables WebSockets for this OrbiterX session and resets WebSocket state.
    ///
...
fn stamp_ws_stream_request_start_ms(request: &mut ResponsesWsRequest) {
    ...
    payload
        ...
        .insert(
            X_ORBITERX_WS_STREAM_REQUEST_START_MS_CLIENT_METADATA_KEY.to_string(),
...
///
/// These headers implement OrbiterX-specific conventions:
///
/// - `x-orbiterx-beta-features`: comma-separated beta feature keys enabled for the session.
/// - `x-orbiterx-turn-state`: sticky routing token captured earlier in the turn.
fn build_responses_headers(
    ...
    {
        headers.insert("x-orbiterx-beta-features", header_value);
    }
    if let Some(turn_state) = turn_state
        && let Some(state) = turn_state.get()
        && let Ok(header_value) = HeaderValue::from_str(state)
    {
        headers.insert(X_ORBITERX_TURN_STATE_HEADER, header_value);
    }
    headers
}
...
fn add_responses_lite_header(headers: &mut ApiHeaderMap, use_responses_lite: bool) {
    if use_responses_lite {
        headers.insert(
            X_OPENAI_INTERNAL_ORBITERX_RESPONSES_LITE_HEADER,
            HeaderValue::from_static("true"),
        );
    }
}
...
fn map_response_stream(
    api_stream: orbiterx_api::ResponseStream,
    session_telemetry: SessionTelemetry,
    inference_trace_attempt: InferenceTraceAttempt,
    provider: SharedModelProvider,
) -> (ResponseStream, oneshot::Receiver<LastResponse>) {
    let orbiterx_api::ResponseStream {
        rx_event,
        upstream_request_id,
    } = api_stream;
    let api_stream = orbiterx_api::ResponseStream {
        rx_event,
...
/// the mapped `OrbiterXErr` is returned to the caller.
...
struct WebsocketConnectParams<'a> {
    ...
    api_provider: orbiterx_api::Provider,
    api_auth: SharedAuthProvider,
    responses_metadata: &'a OrbiterXResponsesMetadata,
    auth_context: AuthRequestTelemetryContext,
    request_route_telemetry: RequestRouteTelemetry,
}
        ...
        && recovery.has_next()
    {
        ...
        return match recovery.next().await {
            ...
            Err(RefreshTokenError::Permanent(failed)) => {
                ...
                Err(OrbiterXErr::RefreshTokenFailed(failed))
            }
