...
/// Start an interactive sub-OrbiterX thread and return its runtime and IO channels.
///
...
pub(crate) async fn run_orbiterx_thread_interactive(
    config: Config,
...
) -> Result<(Arc<Session>, SessionIo), OrbiterXErr> {
...
#[allow(clippy::too_many_arguments)]
pub(crate) async fn run_orbiterx_thread_one_shot(
    config: Config,
    auth_manager: Arc<AuthManager>,
    models_manager: SharedModelsManager,
    input: Vec<UserInput>,
    ...
    let (session, io) = Box::pin(run_orbiterx_thread_interactive(
        config,
        auth_manager,
        models_manager,
        parent_session,
    ...
    let guardian_decision = if routes_approval_to_guardian(parent_ctx) {
        ...
        let patch = changes
            ...
            .map(|(path, change)| match change {
                orbiterx_protocol::protocol::FileChange::Add { content } => {
                    format!("*** Add File: {}\n{}", path.display(), content)
                }
                orbiterx_protocol::protocol::FileChange::Delete { content } => {
                    format!("*** Delete File: {}\n{}", path.display(), content)
                }
                orbiterx_protocol::protocol::FileChange::Update {
                    unified_diff,
                    move_path,
                } => {
...
async fn maybe_auto_review_mcp_request_user_input(
    ...
    Some(RequestUserInputResponse {
        answers: HashMap::from([(
            question.id.clone(),
            orbiterx_protocol::request_user_input::RequestUserInputAnswer {
...
) -> orbiterx_protocol::protocol::ReviewDecision
where
    F: core::future::Future<Output = orbiterx_protocol::protocol::ReviewDecision>,
{
    tokio::select! {
        biased;
        _ = cancel_token.cancelled() => {
            ...
            parent_session
                .notify_approval(approval_id, orbiterx_protocol::protocol::ReviewDecision::Abort)
                .await;
            orbiterx_protocol::protocol::ReviewDecision::Abort
        }
        decision = fut => {
            decision
        }
...
#[cfg(test)]
#[path = "orbiterx_delegate_tests.rs"]
mod tests;
