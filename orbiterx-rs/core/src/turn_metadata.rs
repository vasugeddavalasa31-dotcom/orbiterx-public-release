...
use orbiterx_git_utils::get_git_remote_urls_assume_git_repo;
use orbiterx_git_utils::get_git_repo_root;
use orbiterx_git_utils::get_has_changes;
use orbiterx_git_utils::get_head_commit_hash;
...
pub async fn detached_memory_responses_metadata(
    ...
    OrbiterXResponsesMetadata {
        request_kind: Some(OrbiterXResponsesRequestKind::Memory),
        subagent_header: subagent_header_value(session_source),
        sandbox: sandbox.map(ToString::to_string),
        workspaces: memory_workspaces(cwd).await,
        ..OrbiterXResponsesMetadata::new(installation_id, session_id, thread_id, window_id)
    }
}
...
impl TurnMetadataState {
    ...
    fn responses_metadata_template(&self) -> OrbiterXResponsesMetadata {
        OrbiterXResponsesMetadata {
