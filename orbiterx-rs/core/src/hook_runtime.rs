...
use orbiterx_analytics::HookRunFact;
use orbiterx_analytics::build_track_events_context;
use orbiterx_hooks::PermissionRequestDecision;
use orbiterx_hooks::PermissionRequestOutcome;
use orbiterx_hooks::PermissionRequestRequest;
use orbiterx_hooks::PostToolUseOutcome;
use orbiterx_hooks::PostToolUseRequest;
use orbiterx_hooks::PreToolUseOutcome;
use orbiterx_hooks::PreToolUseRequest;
use orbiterx_hooks::SessionStartOutcome;
use orbiterx_hooks::StartHookTarget;
use orbiterx_hooks::StopHookTarget;
use orbiterx_hooks::StopOutcome;
use orbiterx_hooks::SubagentHookContext;
use orbiterx_hooks::UserPromptSubmitOutcome;
use orbiterx_hooks::UserPromptSubmitRequest;
use orbiterx_otel::HOOK_RUN_DURATION_METRIC;
use orbiterx_otel::HOOK_RUN_METRIC;
...
use orbiterx_protocol::protocol::HookCompletedEvent;
use orbiterx_protocol::protocol::HookEventName;
use orbiterx_protocol::protocol::HookRunStatus;
use orbiterx_protocol::protocol::HookRunSummary;
use orbiterx_protocol::protocol::HookSource;
use orbiterx_protocol::protocol::HookStartedEvent;
...
pub(crate) async fn run_turn_stop_hooks(
    ...
    let request = orbiterx_hooks::StopRequest {
        session_id: sess.session_id().into(),
        turn_id: turn_context.sub_id.clone(),
        #[allow(deprecated)]
        cwd: turn_context.cwd.clone(),
...
pub(crate) async fn run_session_end_hooks(sess: &Arc<Session>) {
    ...
    let request = orbiterx_hooks::SessionEndRequest {
        session_id: sess.session_id().into(),
        turn_id: turn_context.sub_id.clone(),
        #[allow(deprecated)]
        cwd: turn_context.cwd.clone(),
...
pub(crate) async fn run_pre_compact_hooks(
    ...
    let request = orbiterx_hooks::PreCompactRequest {
        session_id: sess.session_id().into(),
        turn_id: turn_context.sub_id.clone(),
        subagent: thread_spawn_subagent_hook_context(sess, turn_context),
        #[allow(deprecated)]
...
pub(crate) async fn run_post_compact_hooks(
    ...
    let request = orbiterx_hooks::PostCompactRequest {
        session_id: sess.session_id().into(),
        turn_id: turn_context.sub_id.clone(),
        subagent: thread_spawn_subagent_hook_context(sess, turn_context),
        #[allow(deprecated)]
...
pub(crate) async fn run_legacy_after_agent_hook(
    ...
    for hook_outcome in hooks
        .dispatch(orbiterx_hooks::HookPayload {
            session_id: sess.session_id().into(),
            #[allow(deprecated)]
            cwd: turn_context.cwd.clone(),
            client: turn_context.app_server_client_name.clone(),
            triggered_at: chrono::Utc::now(),
            hook_event: orbiterx_hooks::HookEvent::AfterAgent {
                event: orbiterx_hooks::HookEventAfterAgent {
        ...
        .await
    {
        ...
        let (error, should_abort) = match hook_outcome.result {
            orbiterx_hooks::HookResult::Success => continue,
            orbiterx_hooks::HookResult::FailedContinue(error) => (error, false),
            orbiterx_hooks::HookResult::FailedAbort(error) => (error, true),
        };
    ...
    let event = EventMsg::Error(orbiterx_protocol::protocol::ErrorEvent {
        message,
...
fn hook_run_analytics_payload(
    ...
    completed: &HookCompletedEvent,
) -> (orbiterx_analytics::TrackEventsContext, HookRunFact) {
    (
...
mod tests {
    ...
    use orbiterx_protocol::protocol::HookEventName;
    use orbiterx_protocol::protocol::HookExecutionMode;
    use orbiterx_protocol::protocol::HookHandlerType;
    use orbiterx_protocol::protocol::HookRunStatus;
    use orbiterx_protocol::protocol::HookScope;
    use orbiterx_protocol::protocol::HookSource;
        ...
        assert_eq!(
            messages
                ...
                .map(|message| match message {
                    orbiterx_protocol::models::ResponseItem::Message { role, content, .. } => {
                        let text = content
                            .iter()
                            .map(|item| match item {
