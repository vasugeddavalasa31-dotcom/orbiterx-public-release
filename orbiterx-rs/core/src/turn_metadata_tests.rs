...
fn test_responses_metadata_json(
    ...
    window_id: &str,
    request_kind: OrbiterXResponsesRequestKind,
) -> String {
    state
...
fn test_turn_responses_metadata_json(state: &TurnMetadataState, window_id: &str) -> String {
    test_responses_metadata_json(state, window_id, OrbiterXResponsesRequestKind::Turn)
}
...
fn test_compaction_responses_metadata_json(
    ...
    test_responses_metadata_json(
        state,
        window_id,
        OrbiterXResponsesRequestKind::Compaction(compaction),
    )
}
...
fn turn_metadata_state_merges_client_metadata_without_replacing_reserved_fields() {
    ...
    state.set_responsesapi_client_metadata(HashMap::from([
        ...
        (
            "x-orbiterx-installation-id".to_string(),
            "client-supplied".to_string(),
        ),
        (
            "x-orbiterx-parent-thread-id".to_string(),
            "client-supplied".to_string(),
        ),
        (
    ...
    assert!(json.get("x-orbiterx-installation-id").is_none());
    assert!(json.get("x-orbiterx-parent-thread-id").is_none());
