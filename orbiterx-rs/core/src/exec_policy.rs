...
use orbiterx_execpolicy::AmendError;
use orbiterx_execpolicy::Decision;
use orbiterx_execpolicy::Error as ExecPolicyRuleError;
...
use orbiterx_execpolicy::blocking_append_allow_prefix_rule;
use orbiterx_execpolicy::blocking_append_network_rule;
...
use orbiterx_shell_command::is_dangerous_command::DangerousCommandMatch;
use orbiterx_shell_command::is_dangerous_command::dangerous_command_match;
use orbiterx_shell_command::is_safe_command::is_known_safe_command;
use thiserror::Error;
use tokio::fs;
...
pub(crate) fn child_uses_parent_exec_policy(parent_config: &Config, child_config: &Config) -> bool {
    fn exec_policy_config_folders(config: &Config) -> Vec<AbsolutePathBuf> {
        config
            ...
            .into_iter()
            .filter_map(orbiterx_config::ConfigLayerEntry::config_folder)
            .collect()
    }
...
pub enum ExecPolicyError {
    ...
    ParsePolicy {
        path: String,
        source: orbiterx_execpolicy::Error,
    },
}
...
impl ExecPolicyManager {
    ...
    pub(crate) async fn append_amendment_and_update(
        ...
        let policy_path = default_policy_path(orbiterx_home);
        spawn_blocking({
...
fn exec_policy_message_for_display(source: &orbiterx_execpolicy::Error) -> String {
...
fn dangerous_command_match_for_origin(
    ...
    match command_origin {
        ...
        ExecPolicyCommandOrigin::PowerShell => {
            orbiterx_shell_command::is_dangerous_command::dangerous_powershell_words_match(command)
        }
    }
}
...
pub(crate) fn render_decision_for_unmatched_command(
    ...
    let is_known_safe = match command_origin {
        ...
        #[cfg(windows)]
        ExecPolicyCommandOrigin::PowerShell => {
            orbiterx_shell_command::is_safe_command::is_safe_powershell_words(command)
        }
    };
...
fn default_policy_path(orbiterx_home: &Path) -> PathBuf {
    orbiterx_home.join(RULES_DIR_NAME).join(DEFAULT_POLICY_FILE)
}
...
fn commands_for_exec_policy(command: &[String]) -> ExecPolicyCommands {
    ...
    #[cfg(windows)]
    {
        if let Some(commands) =
            orbiterx_shell_command::powershell::parse_powershell_command_into_plain_commands(
                command,
            )
            && !commands.is_empty()
        {
...
/// - Note: we only use this amendment when the command fails to run in sandbox and orbiterx prompts the user to run outside the sandbox
