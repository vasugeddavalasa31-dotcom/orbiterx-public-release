...
use orbiterx_config::CloudConfigBundleLoader;
use orbiterx_config::ConfigLayerSource;
use orbiterx_config::ConfigLayerStack;
use orbiterx_config::ConfigLayerStackOrdering;
use orbiterx_config::ConfigRequirements;
use orbiterx_config::ConfigRequirementsToml;
use orbiterx_config::ConstrainedWithSource;
use orbiterx_config::FeatureRequirementsToml;
use orbiterx_config::McpServerRequirement;
use orbiterx_config::PluginRequirementsToml;
use orbiterx_config::ProfileV2Name;
use orbiterx_config::ResidencyRequirement;
use orbiterx_config::SandboxModeRequirement;
use orbiterx_config::Sourced;
use orbiterx_config::ThreadConfigLoader;
use orbiterx_config::config_toml::ConfigLockfileToml;
use orbiterx_config::config_toml::ConfigToml;
use orbiterx_config::config_toml::DEFAULT_PROJECT_DOC_MAX_BYTES;
use orbiterx_config::config_toml::ProjectConfig;
use orbiterx_config::config_toml::RealtimeAudioConfig;
use orbiterx_config::config_toml::RealtimeConfig;
use orbiterx_config::config_toml::ThreadStoreToml;
use orbiterx_config::config_toml::validate_model_providers;
use orbiterx_config::loader::load_config_layers_state;
use orbiterx_config::loader::project_trust_key;
use orbiterx_config::permissions_toml::PermissionsToml;
use orbiterx_config::sandbox_mode_requirement_for_permission_profile;
use orbiterx_config::types::ApprovalsReviewer;
use orbiterx_config::types::AuthCredentialsStoreMode;
use orbiterx_config::types::AuthKeyringBackendKind;
use orbiterx_config::types::History;
use orbiterx_config::types::McpServerConfig;
use orbiterx_config::types::McpServerDisabledReason;
use orbiterx_config::types::MemoriesConfig;
use orbiterx_config::types::ModelAvailabilityNuxConfig;
use orbiterx_config::types::Notice;
use orbiterx_config::types::OAuthCredentialsStoreMode;
use orbiterx_config::types::ResumeCwdMode;
use orbiterx_config::types::SessionPickerViewMode;
use orbiterx_config::types::ToolSuggestConfig;
use orbiterx_config::types::ToolSuggestDisabledTool;
use orbiterx_config::types::ToolSuggestDiscoverable;
use orbiterx_config::types::TuiKeymap;
use orbiterx_config::types::TuiNotificationSettings;
use orbiterx_config::types::TuiPetAnchor;
use orbiterx_config::types::UriBasedFileOpener;
use orbiterx_config::types::WindowsSandboxModeToml;
use orbiterx_core_plugins::PluginLoadOutcome;
use orbiterx_core_plugins::PluginsConfigInput;
use orbiterx_exec_server::ExecutorFileSystem;
use orbiterx_exec_server::LOCAL_FS;
use orbiterx_features::CodeModeConfigToml;
use orbiterx_features::CurrentTimeReminderConfigToml;
use orbiterx_features::CurrentTimeReminderDeliveryMode;
use orbiterx_features::CurrentTimeSource;
use orbiterx_features::Feature;
use orbiterx_features::FeatureConfigSource;
use orbiterx_features::FeatureOverrides;
use orbiterx_features::FeatureToml;
use orbiterx_features::Features;
use orbiterx_features::FeaturesToml;
use orbiterx_features::MultiAgentV2ConfigToml;
use orbiterx_features::NetworkProxyConfigToml;
use orbiterx_features::TokenBudgetConfigToml;
...
use orbiterx_install_context::InstallContext;
use orbiterx_login::AuthManagerConfig;
use orbiterx_login::AuthRouteConfig;
use orbiterx_mcp::McpConfig;
use orbiterx_mcp::McpPluginAttribution;
use orbiterx_mcp::McpServerRegistration;
use orbiterx_mcp::ResolvedMcpCatalog;
use orbiterx_memories_read::memory_root;
use orbiterx_model_provider_info::LEGACY_OLLAMA_CHAT_PROVIDER_ID;
use orbiterx_model_provider_info::ModelProviderInfo;
use orbiterx_model_provider_info::OLLAMA_CHAT_PROVIDER_REMOVED_ERROR;
use orbiterx_model_provider_info::built_in_model_providers;
use orbiterx_model_provider_info::merge_configured_model_providers;
use orbiterx_models_manager::ModelsManagerConfig;
use orbiterx_protocol::config_types::AltScreenMode;
use orbiterx_protocol::config_types::AutoCompactTokenLimitScope;
...
use orbiterx_protocol::config_types::SandboxMode;
use orbiterx_protocol::config_types::ServiceTier;
use orbiterx_protocol::config_types::ShellEnvironmentPolicy;
use orbiterx_protocol::config_types::TrustLevel;
use orbiterx_protocol::config_types::Verbosity;
use orbiterx_protocol::config_types::WebSearchConfig;
use orbiterx_protocol::config_types::WebSearchMode;
use orbiterx_protocol::config_types::WindowsSandboxLevel;
use orbiterx_protocol::models::ActivePermissionProfile;
...
pub use orbiterx_thread_store::ExtraConfig;
use orbiterx_utils_absolute_path::AbsolutePathBuf;
use orbiterx_utils_absolute_path::AbsolutePathBufGuard;
...
use crate::config::permissions::get_readable_roots_required_for_orbiterx_runtime;
...
pub use orbiterx_config::ConfigLoadOptions;
pub use orbiterx_config::Constrained;
pub use orbiterx_config::ConstraintError;
pub use orbiterx_config::ConstraintResult;
pub use orbiterx_config::LoaderOverrides;
pub use orbiterx_network_proxy::NetworkProxyAuditMetadata;
use orbiterx_sandboxing::compatibility_sandbox_policy_for_permission_profile;
pub use orbiterx_sandboxing::system_bwrap_warning;
...
fn resolve_sqlite_home_env(resolved_cwd: &Path) -> Option<PathBuf> {
    let raw = std::env::var(orbiterx_state::SQLITE_HOME_ENV).ok()?;
...
pub(crate) async fn test_config() -> Config {
    ...
    Config::load_from_base_config_with_overrides(
        ...
        AbsolutePathBuf::from_absolute_path(orbiterx_home.path()).expect("temp dir should resolve"),
    )
    .await
...
impl Permissions {
    ...
    /// selection. Internal OrbiterX-only writable roots are intentionally excluded.
...
// when OrbiterX is still responsible for the network policy. `Disabled` means no
...
fn build_network_proxy_spec(
    ...
    network_requirements: Option<Sourced<orbiterx_config::NetworkConstraints>>,
    permission_profile: &PermissionProfile,
...
pub struct Config {
    ...
    /// Optional external notifier command. When set, OrbiterX will spawn this
    /// program after each completed *turn* (i.e. when the agent finishes
    /// processing a user submission). The value must be the full command
    /// broken into argv tokens **without** the trailing JSON argument - OrbiterX
    /// appends one extra argument containing a JSON payload describing the
    /// event.
    ///
    /// Example `~/.orbiterx/config.toml` snippet:
    ///
    /// ```toml
    /// notify = ["notify-send", "OrbiterX"]
    /// ```
    ///
    /// which will be invoked as:
    ///
    /// ```shell
    /// notify-send OrbiterX '{"type":"agent-turn-complete","turn-id":"12345"}'
    /// ```
    ///
    ...
    /// file (default): Use a file in the OrbiterX home directory.
    /// keyring: Use an OS-specific keyring service.
    /// auto: Use the OS-specific keyring service if available, otherwise use a file.
    pub cli_auth_credentials_store_mode: AuthCredentialsStoreMode,

    /// Definition for MCP servers that OrbiterX can reach out to for tool calls.
    pub mcp_servers: Constrained<HashMap<String, McpServerConfig>>,

    /// Preferred store for MCP OAuth credentials.
    /// keyring: Use an OS-specific keyring service.
    ///          Credentials stored in the keyring will only be readable by OrbiterX unless the user explicitly grants access via OS-level keyring access.
    ///          https://github.com/openai/orbiterx/blob/main/orbiterx-rs/rmcp-client/src/oauth.rs#L2
    /// file: ORBITERX_HOME/.credentials.json
    ///       This file will be readable to OrbiterX and other applications running as the same user.
    /// auto (default): keyring if available, otherwise file.
    pub mcp_oauth_credentials_store_mode: OAuthCredentialsStoreMode,

    /// Optional fixed port to use for the local HTTP callback server used during MCP OAuth login.
    ///
    /// When unset, OrbiterX will bind to an ephemeral port chosen by the OS.
    ...
    /// Directory containing all OrbiterX state (defaults to `~/.orbiterx` but can be
    /// overridden by the `ORBITERX_HOME` environment variable).
    pub orbiterx_home: AbsolutePathBuf,

    /// Directory where OrbiterX stores the SQLite state DB.
    pub sqlite_home: PathBuf,

    /// Directory where OrbiterX writes log files (defaults to `$ORBITERX_HOME/log`).
    pub log_dir: PathBuf,

    /// Directory where OrbiterX writes effective session config lock files.
    pub config_lock_export_dir: Option<AbsolutePathBuf>,

    /// Whether config lock replay ignores OrbiterX version drift between the
    /// lock metadata and the regenerated lock.
    pub config_lock_allow_orbiterx_version_mismatch: bool,
    ...
    /// Settings that govern if and what will be written to `~/.orbiterx/history.jsonl`.
    pub history: History,
    ...
    /// Path to the current OrbiterX executable. This cannot be set in the config
    /// file: it must be set in code via [`ConfigOverrides`].
    pub orbiterx_self_exe: Option<PathBuf>,

    /// Path to the `orbiterx-linux-sandbox` executable. This must be set if
    /// [`orbiterx_sandboxing::SandboxType::LinuxSeccomp`] is used. Note that this
    /// cannot be set in the config file: it must be set in code via
    /// [`ConfigOverrides`].
    ///
    /// When this program is invoked, arg0 will be set to `orbiterx-linux-sandbox`.
    pub orbiterx_linux_sandbox_exe: Option<PathBuf>,

    /// Path to the `orbiterx-execve-wrapper` executable used for shell
    ...
    /// Whether OrbiterX-owned clients should respect host system proxy settings.
    ...
    /// When `true`, checks for OrbiterX updates on startup and surfaces update prompts.
    /// Set to `false` only if your OrbiterX updates are centrally managed.
    /// Defaults to `true`.
    ...
    /// When `false`, disables analytics across OrbiterX product surfaces in this machine.
    /// Voluntarily left as Optional because the default value might depend on the client.
    pub analytics_enabled: Option<bool>,

    /// When `false`, disables feedback collection across OrbiterX product surfaces.
    /// Defaults to `true`.
    ...
    pub otel: orbiterx_config::types::OtelConfig,
}
...
impl AuthManagerConfig for Config {
    fn orbiterx_home(&self) -> PathBuf {
        self.orbiterx_home.to_path_buf()
    }
...
impl ConfigBuilder {
    pub fn orbiterx_home(mut self, orbiterx_home: PathBuf) -> Self {
        self.orbiterx_home = Some(orbiterx_home);
        self
    }
    ...
    async fn build_inner(self) -> std::io::Result<Config> {
        ...
        let orbiterx_home = match orbiterx_home {
            Some(orbiterx_home) => AbsolutePathBuf::from_absolute_path(orbiterx_home)?,
            None => find_orbiterx_home()?,
        };
        ...
        let config_layer_stack = load_config_layers_state(
            ...
            thread_config_loader
                .as_deref()
                .unwrap_or(&orbiterx_config::NoopThreadConfigLoader),
        )
        .await?;
        ...
        let config_toml: ConfigToml = match merged_toml.try_into() {
            ...
            Err(err) => {
                if let Some(config_error) = orbiterx_config::first_layer_config_error::<ConfigToml>(
                    &config_layer_stack,
                    orbiterx_config::CONFIG_TOML_FILE,
                )
                .await
                {
                    return Err(orbiterx_config::io_error_from_config_error(
        ...
        {
            let allow_orbiterx_version_mismatch = config_lock_settings
                .and_then(|config_lock| config_lock.allow_orbiterx_version_mismatch)
                .unwrap_or(false);
            ...
            config.config_lock_allow_orbiterx_version_mismatch = allow_orbiterx_version_mismatch;
...
impl Config {
    ...
    pub async fn to_mcp_config(
        &self,
        plugins_manager: &orbiterx_core_plugins::PluginsManager,
    ) -> McpConfig {
    ...
    pub(crate) fn to_mcp_config_with_loaded_plugins(
        ...
        McpConfig {
            ...
            orbiterx_home: self.orbiterx_home.to_path_buf(),
            ...
            orbiterx_linux_sandbox_exe: self.orbiterx_linux_sandbox_exe.clone(),
            ...
            connector_snapshot:
                orbiterx_connectors::ConnectorSnapshot::from_plugin_capability_summaries(
    ...
    pub async fn rebuild_preserving_session_layers(
        ...
        Self::load_config_with_layer_stack(
            ...
            refreshed_config.orbiterx_home.clone(),
            config_layer_stack,
        )
        .await
    }
    ...
    pub async fn load_default_with_cli_overrides(
        ...
        Self::load_default_with_cli_overrides_for_orbiterx_home(
            orbiterx_home.to_path_buf(),
            cli_overrides,
        )
        .await
    }

    /// Load a default configuration for a specific OrbiterX home without reading
    /// user, project, or system config layers.
    pub async fn load_default_with_cli_overrides_for_orbiterx_home(
        ...
        let cli_layer = orbiterx_config::build_cli_overrides_layer(&cli_overrides);
        orbiterx_config::merge_toml_values(&mut merged, &cli_layer);
        let orbiterx_home = AbsolutePathBuf::from_absolute_path_checked(orbiterx_home)?;
        let config_toml = deserialize_config_toml_with_base(merged, &orbiterx_home)?;
        Self::load_config_with_layer_stack(
            LOCAL_FS.as_ref(),
            config_toml,
            ConfigOverrides::default(),
    ...
    /// ignores user settings. For example, the `orbiterx exec` subcommand is
    /// designed to use [AskForApproval::Never] exclusively.
    ///
    /// Further, [ConfigOverrides] contains some options that are not supported
    /// in [ConfigToml], such as `cwd`, `orbiterx_self_exe`, `orbiterx_linux_sandbox_exe`, and
...
pub async fn load_config_as_toml_with_cli_and_load_options(
    ...
    load_config_toml_with_layer_stack(orbiterx_home, cwd, cli_overrides, options)
        .await
...
pub async fn load_config_toml_with_layer_stack(
    ...
    let config_layer_stack = load_config_layers_state(
        ...
        &cli_overrides,
        options,
        &orbiterx_config::NoopThreadConfigLoader,
    )
    .await?;

    let merged_toml = config_layer_stack.effective_config();
    let cfg = deserialize_config_toml_with_base(merged_toml, orbiterx_home).map_err(|e| {
...
pub async fn load_global_mcp_servers(
    ...
    // MCP servers defined in in-repo .orbiterx/ folders.
...
/// Patch `ORBITERX_HOME/config.toml` project state to set trust level.
/// Use with caution.
...
pub fn set_default_oss_provider(orbiterx_home: &Path, provider: &str) -> std::io::Result<()> {
    orbiterx_config::config_toml::validate_oss_provider(provider)?;
    use toml_edit::value;
...
fn apply_managed_filesystem_constraints(
    ...
    filesystem_constraints: &orbiterx_config::FilesystemConstraints,
) {
    for deny_read in &filesystem_constraints.deny_read {
        let deny_entry = if deny_read.contains_glob() {
            orbiterx_protocol::permissions::FileSystemSandboxEntry {
                path: orbiterx_protocol::permissions::FileSystemPath::GlobPattern {
                    pattern: deny_read.as_str().to_string(),
                },
                access: orbiterx_protocol::permissions::FileSystemAccessMode::Deny,
            }
        } else {
            ...
            orbiterx_protocol::permissions::FileSystemSandboxEntry {
                path: orbiterx_protocol::permissions::FileSystemPath::Path { path },
                access: orbiterx_protocol::permissions::FileSystemAccessMode::Deny,
            }
        };
...
fn resolve_orchestrator_feature_enabled(
    feature: Option<&orbiterx_config::config_toml::OrchestratorFeatureToml>,
) -> bool {
...
impl Config {
    ...
    pub(crate) async fn load_config_with_layer_stack(
        ...
        let memories_root = memory_root(&orbiterx_home);
        ...
        let forced_chatgpt_workspace_id = cfg
            ...
            .clone()
            .map(orbiterx_config::config_toml::ForcedChatgptWorkspaceIds::into_vec)
            .map(|values| {
                values
                    .into_iter()
        ...
        let log_dir = cfg
            ...
            .unwrap_or_else(|| orbiterx_home.join("log").to_path_buf());
        let sqlite_home = cfg
            .sqlite_home
            .as_ref()
            .map(AbsolutePathBuf::to_path_buf)
            .or_else(|| resolve_sqlite_home_env(&resolved_cwd))
            .unwrap_or_else(|| orbiterx_home.to_path_buf());
            ...
            && constrained_approval_policy.value() == AskForApproval::Never
        {
            return Err(std::io::Error::new(
                std::io::ErrorKind::InvalidInput,
                "`approval_policy = \"never\"` cannot be used because requirements do not allow `sandbox_mode = \"danger-full-access\"`; OrbiterX would fall back to read-only permissions with approvals disabled. Choose an `approval_policy` based on what you need, such as `on-request`, or choose an allowed sandbox mode.",
            ));
        }
        if permission_profile_was_constrained {
        ...
        let mut helper_readable_roots = get_readable_roots_required_for_orbiterx_runtime(
            &orbiterx_home,
            zsh_path.as_ref(),
        ...
        let config = Self {
            ...
            config_lock_allow_orbiterx_version_mismatch: cfg
                .debug
                .as_ref()
                .and_then(|debug| debug.config_lockfile.as_ref())
                .and_then(|config_lock| config_lock.allow_orbiterx_version_mismatch)
...
pub fn find_orbiterx_home() -> std::io::Result<AbsolutePathBuf> {
    orbiterx_utils_home_dir::find_orbiterx_home()
}

/// Returns the path to the folder where OrbiterX logs are stored. Does not verify
