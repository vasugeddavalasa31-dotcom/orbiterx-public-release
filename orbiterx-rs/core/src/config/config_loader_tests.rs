...
use orbiterx_config::ConfigError;
use orbiterx_config::ConfigLayerEntry;
use orbiterx_config::ConfigLayerSource;
use orbiterx_config::ConfigLayerStackOrdering;
use orbiterx_config::ConfigLoadError;
use orbiterx_config::ConfigLoadOptions;
use orbiterx_config::ConfigRequirements;
use orbiterx_config::ConfigRequirementsToml;
use orbiterx_config::ConfigRequirementsWithSources;
use orbiterx_config::FilesystemDenyReadPattern;
use orbiterx_config::LoaderOverrides;
use orbiterx_config::RequirementSource;
use orbiterx_config::RequirementsLayerEntry;
use orbiterx_config::SessionThreadConfig;
use orbiterx_config::StaticThreadConfigLoader;
use orbiterx_config::ThreadConfigSource;
use orbiterx_config::compose_requirements;
use orbiterx_config::config_error_from_ignored_toml_fields;
use orbiterx_config::config_error_from_toml;
...
use orbiterx_config::loader::load_requirements_toml;
...
async fn write_project_hook_config(
    dot_orbiterx_folder: &Path,
    foo: Option<&str>,
    command: &str,
) -> std::io::Result<()> {
    tokio::fs::create_dir_all(dot_orbiterx_folder).await?;
    let foo = foo
        .map(|value| format!("foo = \"{value}\"\n\n"))
        .unwrap_or_default();
    tokio::fs::write(
        dot_orbiterx_folder.join(CONFIG_TOML_FILE),
        format!(
            r#"{foo}[hooks]
...
async fn cli_overrides_resolve_relative_paths_against_cwd() -> std::io::Result<()> {
    let orbiterx_home = tempdir().expect("tempdir");
...
invalid = ["#,
    ...
    let user_layer = layers
        ...
        .expect("expected a user layer even when ORBITERX_HOME/config.toml is ignored");
    assert_eq!(
        user_layer.config,
...
async fn returns_config_error_for_schema_error_in_user_config() {
    ...
    let config_error = config_error_from_io(&err);
    let _guard = orbiterx_utils_absolute_path::AbsolutePathBufGuard::new(tmp.path());
    let expected_config_error =
        orbiterx_config::config_error_from_typed_toml::<ConfigToml>(&config_path, contents)
            .expect("schema error");
    assert_eq!(config_error, &expected_config_error);
}
...
collaboration_modes = "true""#;
    ...
    let _guard = orbiterx_utils_absolute_path::AbsolutePathBufGuard::new(tmp.path());
    let error = orbiterx_config::config_error_from_typed_toml::<ConfigToml>(&config_path, contents)
        .expect("schema error");
...
async fn returns_empty_when_all_layers_missing() {
    ...
    let user_layer = layers
        .get_active_user_layer()
        .expect("expected a user layer even when ORBITERX_HOME/config.toml does not exist");
...
"#,
    ...
    assert_eq!(
        config_requirements_toml
            ...
            .as_deref()
            .cloned(),
        Some(vec![orbiterx_config::WebSearchModeRequirement::Cached])
    );
    assert_eq!(
        config_requirements_toml
        ...
        Some(orbiterx_config::FeatureRequirementsToml {
    ...
    assert_eq!(
        ...
        Some(orbiterx_config::ResidencyRequirement::Us)
    );
    assert_eq!(
        config_requirements
            .feature_requirements
    ...
    assert_eq!(
        ...
        Err(ConstraintError::InvalidValue {
            ...
            requirement_source: RequirementSource::MdmManagedPreferences {
                domain: "com.openai.orbiterx".to_string(),
...
async fn load_config_layers_includes_cloud_config_bundle() -> anyhow::Result<()> {
    let tmp = tempdir()?;
    let orbiterx_home = tmp.path().join("home");
    tokio::fs::create_dir_all(&orbiterx_home).await?;
...
"#,
    ...
    assert_eq!(
        ...
        vec![
            ...
            ConfigLayerSource::User {
                file: AbsolutePathBuf::from_absolute_path(orbiterx_home.join(CONFIG_TOML_FILE))?,
                profile: None,
            },
        ]
    );
...
#[tokio::test]
async fn load_config_layers_resolves_relative_bundle_requirements_paths_against_orbiterx_home()
-> anyhow::Result<()> {
    let tmp = tempdir()?;
    let orbiterx_home = tmp.path().join("home");
    tokio::fs::create_dir_all(&orbiterx_home).await?;
...
"#;
    ...
    assert_eq!(
        ...
        Some(vec![
            ...
            .expect("bundle requirements path should resolve against orbiterx_home")
        ])
    );
...
async fn strict_config_rejects_unknown_cloud_config_key() {
    let tmp = tempdir().expect("tempdir");
    let orbiterx_home = tmp.path().join("home");
    tokio::fs::create_dir_all(&orbiterx_home)
        .await
        .expect("create orbiterx home");
...
async fn load_config_layers_applies_matching_remote_sandbox_config() -> anyhow::Result<()> {
    ...
    assert_eq!(
        ...
        Some(vec![
            orbiterx_config::SandboxModeRequirement::ReadOnly,
            orbiterx_config::SandboxModeRequirement::WorkspaceWrite,
        ])
    );
    assert!(
        layers
        ...
        ConfigLoadOptions {
            cloud_config_bundle: CloudConfigBundleLoader::new(async {
                Err(CloudConfigBundleLoadError::new(
                    orbiterx_config::CloudConfigBundleLoadErrorCode::RequestFailed,
                    /*status_code*/ None,
...
async fn project_layers_prefer_closest_cwd() -> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(nested.join(".orbiterx")).await?;
    tokio::fs::create_dir_all(project_root.join(".orbiterx")).await?;
    tokio::fs::write(project_root.join(".git"), "gitdir: here").await?;

    tokio::fs::write(
        project_root.join(".orbiterx").join(CONFIG_TOML_FILE),
        r#"foo = "root"
"#,
    )
    .await?;
    tokio::fs::write(
        nested.join(".orbiterx").join(CONFIG_TOML_FILE),
        r#"foo = "child"
"#,
    )
    .await?;
    ...
    assert_eq!(
        ...
        nested.join(".orbiterx").as_path()
    );
    assert_eq!(
        project_layers[1].as_path(),
        project_root.join(".orbiterx").as_path()
    );
...
-> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(worktree_root.join(".orbiterx")).await?;
    tokio::fs::create_dir_all(worktree_child.join(".orbiterx")).await?;
    write_linked_worktree_pointer(&repo_root, &worktree_root).await?;
    write_project_hook_config(
        &repo_root.join(".orbiterx"),
        Some("repo-root"),
        "echo repo root hook",
    )
    .await?;
    write_project_hook_config(
        &repo_child.join(".orbiterx"),
        Some("repo-child"),
        "echo repo child hook",
    )
    .await?;
    write_project_hook_config(
        &worktree_root.join(".orbiterx"),
        Some("worktree-root"),
        "echo worktree root hook",
    )
    .await?;
    write_project_hook_config(
        &worktree_child.join(".orbiterx"),
        Some("worktree-child"),
...
async fn nested_project_root_markers_do_not_redirect_regular_repo_hooks() -> std::io::Result<()> {
    ...
    .await?;
    write_project_hook_config(
        &project_root.join(".orbiterx"),
        /*foo*/ None,
        "echo project root hook",
    )
    .await?;
    write_project_hook_config(
        &nested.join(".orbiterx"),
        /*foo*/ None,
        "echo nested hook",
    )
    .await?;
...
#[tokio::test]
async fn project_paths_resolve_relative_to_dot_orbiterx_and_override_in_order()
-> std::io::Result<()> {
    let tmp = tempdir()?;
    let project_root = tmp.path().join("project");
    let nested = project_root.join("child");
...
"#;
    ...
    .await?;
    tokio::fs::write(nested.join(".orbiterx").join(CONFIG_TOML_FILE), nested_cfg).await?;
    tokio::fs::write(
        project_root.join(".orbiterx").join("root.txt"),
        "root instructions",
    )
    .await?;
    tokio::fs::write(
        nested.join(".orbiterx").join("child.txt"),
        "child instructions",
    )
    .await?;
...
async fn cli_override_model_instructions_file_sets_base_instructions() -> std::io::Result<()> {
    ...
    let orbiterx_home = tmp.path().join("home");
    tokio::fs::create_dir_all(&orbiterx_home).await?;
    tokio::fs::write(orbiterx_home.join(CONFIG_TOML_FILE), "").await?;
...
#[tokio::test]
async fn project_layer_is_added_when_dot_orbiterx_exists_without_config_toml() -> std::io::Result<()>
{
    let tmp = tempdir()?;
    let project_root = tmp.path().join("project");
    let nested = project_root.join("child");
    ...
    let expected_project_layer = ConfigLayerEntry::new(
        ConfigLayerSource::Project {
            dot_orbiterx_folder: AbsolutePathBuf::from_absolute_path(
                project_root.join(".orbiterx"),
            )?,
        },
        TomlValue::Table(toml::map::Map::new()),
...
#[tokio::test]
async fn orbiterx_home_is_not_loaded_as_project_layer_from_home_dir() -> std::io::Result<()> {
    let tmp = tempdir()?;
    let home_dir = tmp.path().join("home");
    let orbiterx_home = home_dir.join(".orbiterx");
    tokio::fs::create_dir_all(&orbiterx_home).await?;
    tokio::fs::write(
        orbiterx_home.join(CONFIG_TOML_FILE),
        r#"foo = "user"
...
#[tokio::test]
async fn orbiterx_home_within_project_tree_is_not_double_loaded() -> std::io::Result<()> {
    let tmp = tempdir()?;
    let project_root = tmp.path().join("project");
    let nested = project_root.join("child");
    let project_dot_orbiterx = project_root.join(".orbiterx");
    let nested_dot_orbiterx = nested.join(".orbiterx");

    tokio::fs::create_dir_all(&nested_dot_orbiterx).await?;
    tokio::fs::create_dir_all(project_root.join(".git")).await?;
    tokio::fs::write(
        nested_dot_orbiterx.join(CONFIG_TOML_FILE),
        r#"foo = "child"
"#,
    )
    .await?;

    tokio::fs::create_dir_all(&project_dot_orbiterx).await?;
    make_config_for_test(
        &project_dot_orbiterx,
        &project_root,
        TrustLevel::Trusted,
        /*project_root_markers*/ None,
    )
    .await?;
    let user_config_path = project_dot_orbiterx.join(CONFIG_TOML_FILE);
...
"#,
    ...
    let expected_project_layer = ConfigLayerEntry::new(
        ConfigLayerSource::Project {
            dot_orbiterx_folder: AbsolutePathBuf::from_absolute_path(&nested_dot_orbiterx)?,
        },
        child_config,
    );
    assert_eq!(vec![&expected_project_layer], project_layers);
...
"#,
    ...
    let orbiterx_home_untrusted = tmp.path().join("home_untrusted");
    tokio::fs::create_dir_all(&orbiterx_home_untrusted).await?;
    make_config_for_test(
        &orbiterx_home_untrusted,
        &project_root,
        TrustLevel::Untrusted,
        /*project_root_markers*/ None,
    )
    .await?;
    let untrusted_config_path = orbiterx_home_untrusted.join(CONFIG_TOML_FILE);
...
{untrusted_config_contents}"#
    ...
    let orbiterx_home_unknown = tmp.path().join("home_unknown");
    tokio::fs::create_dir_all(&orbiterx_home_unknown).await?;
    tokio::fs::write(
        orbiterx_home_unknown.join(CONFIG_TOML_FILE),
        r#"foo = "user"
"#,
    )
    .await?;
...
async fn project_layer_ignores_unsupported_config_keys() -> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(&dot_orbiterx).await?;
    ...
    // `.orbiterx` folder. The malformed profile value below would fail typed path
    // resolution if `profiles` were not stripped before that pass runs.
    tokio::fs::write(
        dot_orbiterx.join(CONFIG_TOML_FILE),
        r#"
model = "project-model"
...
"#,
    ...
    let expected_startup_warnings = vec![format!(
        ...
        dot_orbiterx.join(CONFIG_TOML_FILE).display(),
    ...
    // still use the project-local `.orbiterx` folder as its relative-path base.
    assert_eq!(
...
async fn invalid_project_config_ignored_when_untrusted_or_unknown() -> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(nested.join(".orbiterx")).await?;
    tokio::fs::write(project_root.join(".git"), "gitdir: here").await?;
    tokio::fs::write(nested.join(".orbiterx").join(CONFIG_TOML_FILE), "foo =").await?;
    ...
    for (name, trust_level) in cases {
        let orbiterx_home = tmp.path().join(format!("home_{name}"));
        tokio::fs::create_dir_all(&orbiterx_home).await?;
...
-> std::io::Result<()> {
    ...
    for (name, trust_level, expect_disabled) in cases {
        let orbiterx_home = tmp.path().join(format!("home_no_config_{name}"));
        tokio::fs::create_dir_all(&orbiterx_home).await?;
        if let Some(trust_level) = trust_level {
            make_config_for_test(
                &orbiterx_home,
...
mod requirements_exec_policy_tests {
    ...
    use orbiterx_config::ConfigRequirementsWithSources;
    use orbiterx_config::RequirementSource;
    use orbiterx_config::RequirementsExecPolicyDecisionToml;
    use orbiterx_config::RequirementsExecPolicyParseError;
    use orbiterx_config::RequirementsExecPolicyPatternTokenToml;
    use orbiterx_config::RequirementsExecPolicyPrefixRuleToml;
    use orbiterx_config::RequirementsExecPolicyToml;
    use orbiterx_execpolicy::Decision;
    use orbiterx_execpolicy::Evaluation;
    use orbiterx_execpolicy::RuleMatch;
    use orbiterx_utils_absolute_path::AbsolutePathBuf;
    use pretty_assertions::assert_eq;
    use std::path::Path;
    use tempfile::tempdir;
    ...
    fn config_stack_for_dot_orbiterx_folder_with_requirements(
        dot_orbiterx_folder: &Path,
        requirements: ConfigRequirements,
    ) -> ConfigLayerStack {
        let dot_orbiterx_folder = AbsolutePathBuf::from_absolute_path(dot_orbiterx_folder)
            .expect("absolute dot_orbiterx_folder");
...
"#;
    ...
    async fn loads_requirements_exec_policy_without_rules_files() -> anyhow::Result<()> {
        ...
        let config_stack =
            config_stack_for_dot_orbiterx_folder_with_requirements(temp_dir.path(), requirements);
