...
pub fn apply_blocking(orbiterx_home: &Path, edits: &[ConfigEdit]) -> anyhow::Result<()> {
...
///
pub async fn apply(orbiterx_home: &Path, edits: Vec<ConfigEdit>) -> anyhow::Result<()> {
...
impl ConfigEditsBuilder {
    pub fn new(orbiterx_home: &Path) -> Self {
        Self::for_config_path(&orbiterx_home.join(CONFIG_TOML_FILE))
    }
    ...
    pub fn for_config(config: &crate::config::Config) -> Self {
        let config_path = config
            ...
            .map(orbiterx_utils_absolute_path::AbsolutePathBuf::to_path_buf)
            .unwrap_or_else(|| config.orbiterx_home.join(CONFIG_TOML_FILE).to_path_buf());
