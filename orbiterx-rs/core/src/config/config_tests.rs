...
use orbiterx_config::ConfigLayerEntry;
use orbiterx_config::ConfigLayerSource;
use orbiterx_config::ConfigLayerStack;
use orbiterx_config::McpServerCommandMatcher;
use orbiterx_config::McpServerIdentity;
use orbiterx_config::McpServerRequirement;
use orbiterx_config::McpServerValueMatcher;
use orbiterx_config::ProfileV2Name;
use orbiterx_config::RequirementSource;
use orbiterx_config::Sourced;
use orbiterx_config::config_toml::AgentRoleToml;
use orbiterx_config::config_toml::AgentsToml;
use orbiterx_config::config_toml::AutoReviewToml;
use orbiterx_config::config_toml::ConfigToml;
use orbiterx_config::config_toml::ExperimentalRequestUserInput;
use orbiterx_config::config_toml::ProjectConfig;
use orbiterx_config::config_toml::RealtimeConfig;
use orbiterx_config::config_toml::RealtimeToml;
use orbiterx_config::config_toml::RealtimeTransport;
use orbiterx_config::config_toml::RealtimeWsMode;
use orbiterx_config::config_toml::RealtimeWsVersion;
use orbiterx_config::config_toml::ToolsToml;
...
use orbiterx_config::permissions_toml::NetworkDomainPermissionToml;
use orbiterx_config::permissions_toml::NetworkDomainPermissionsToml;
use orbiterx_config::permissions_toml::NetworkMitmActionToml;
use orbiterx_config::permissions_toml::NetworkMitmHookToml;
use orbiterx_config::permissions_toml::NetworkMitmToml;
...
use orbiterx_config::permissions_toml::WorkspaceRootsToml;
use orbiterx_config::types::AppToolApproval;
use orbiterx_config::types::ApprovalsReviewer;
use orbiterx_config::types::BundledSkillsConfig;
use orbiterx_config::types::FeedbackConfigToml;
use orbiterx_config::types::HistoryPersistence;
use orbiterx_config::types::McpServerEnvVar;
use orbiterx_config::types::McpServerOAuthConfig;
use orbiterx_config::types::McpServerToolConfig;
use orbiterx_config::types::McpServerTransportConfig;
use orbiterx_config::types::MemoriesConfig;
use orbiterx_config::types::MemoriesToml;
use orbiterx_config::types::ModelAvailabilityNuxConfig;
use orbiterx_config::types::Notice;
use orbiterx_config::types::NotificationCondition;
use orbiterx_config::types::NotificationMethod;
use orbiterx_config::types::Notifications;
use orbiterx_config::types::OtelConfigToml;
use orbiterx_config::types::OtelExporterKind;
use orbiterx_config::types::ResumeCwdMode;
use orbiterx_config::types::SandboxWorkspaceWrite;
use orbiterx_config::types::SessionPickerViewMode;
use orbiterx_config::types::SkillsConfig;
use orbiterx_config::types::ToolSuggestDisabledTool;
use orbiterx_config::types::ToolSuggestDiscoverableType;
use orbiterx_config::types::Tui;
...
use orbiterx_config::types::WindowsToml;
...
use orbiterx_model_provider_info::LMSTUDIO_OSS_PROVIDER_ID;
use orbiterx_model_provider_info::OLLAMA_OSS_PROVIDER_ID;
use orbiterx_model_provider_info::WireApi;
use orbiterx_models_manager::bundled_models_response;
use orbiterx_network_proxy::NetworkMode;
use orbiterx_protocol::config_types::ModelProviderAuthInfo;
...
use orbiterx_protocol::models::BUILT_IN_PERMISSION_PROFILE_DANGER_FULL_ACCESS;
use orbiterx_protocol::models::BUILT_IN_PERMISSION_PROFILE_READ_ONLY;
use orbiterx_protocol::models::BUILT_IN_PERMISSION_PROFILE_WORKSPACE;
...
use orbiterx_protocol::protocol::NetworkAccess;
...
use orbiterx_config::test_support::CloudConfigBundleFixture;
...
fn stdio_mcp_with_args(command: &str, args: &[&str]) -> McpServerConfig {
    McpServerConfig {
        ...
        environment_id: orbiterx_config::DEFAULT_MCP_SERVER_ENVIRONMENT_ID.to_string(),
        enabled: true,
        required: false,
...
[features.code_mode]
enabled = true
excluded_tool_namespaces = ["mcp__orbiterx_apps", "multi_agent_v1"]
...
async fn network_proxy_cli_overrides_merge_toggle_with_proxy_config() -> std::io::Result<()> {
    ...
    let cwd = TempDir::new()?;
    std::fs::write(
        orbiterx_home.path().join(CONFIG_TOML_FILE),
        r#"
...
"#,
    ...
    assert_eq!(
        ...
        orbiterx_http_client::OutboundProxyPolicy::RespectSystemProxy
    );
    Ok(())
}
...
-> std::io::Result<()> {
    ...
    requirements.network = Some(Sourced::new(
        orbiterx_config::NetworkConstraints {
            enabled: Some(true),
            ..Default::default()
        },
...
-> std::io::Result<()> {
    ...
    let memories_root = orbiterx_home.path().join("memories").abs();
    assert!(
        !config
            .permissions
            .file_system_sandbox_policy()
...
-> std::io::Result<()> {
    ...
    let orbiterx_home = temp_dir.path().join("orbiterx-home");
    ...
    for root in [&cwd, &runtime_root, &profile_root] {
        ...
        std::fs::create_dir_all(root.join(".orbiterx"))?;
    }
                        ...
                        filesystem: Some(FilesystemPermissionsToml {
                            ...
                            entries: BTreeMap::from([(
                                ...
                                FilesystemPermissionToml::Scoped(BTreeMap::from([
                                    ...
                                    (".orbiterx".to_string(), FileSystemAccessMode::Read),
                                ])),
                            )]),
                        }),
                        network: None,
    ...
    for root in [cwd_abs, runtime_root_abs, profile_root_abs.clone()] {
        ...
        assert!(
            !policy.can_write_path_with_cwd(&root.join(".orbiterx"), cwd.as_path()),
            "expected .orbiterx carveout under {root:?}, policy: {policy:?}"
        );
    }
    assert_eq!(
...
async fn empty_config_defaults_to_builtin_profile_for_trusted_project() -> std::io::Result<()> {
    ...
    if cfg!(target_os = "windows") {
        ...
        assert!(
            !policy.can_write_path_with_cwd(&cwd.path().join(".orbiterx"), cwd.path()),
...
-> std::io::Result<()> {
    ...
    let cwd = TempDir::new()?;
    let extra_root = TempDir::new()?;
    for subpath in [".git", ".agents", ".orbiterx"] {
...
async fn permissions_profiles_allow_unknown_special_paths() -> std::io::Result<()> {
    ...
    assert!(
        config.startup_warnings.iter().any(|warning| warning.contains(
            "Configured filesystem path `:future_special_path` is not recognized by this version of OrbiterX and will be ignored."
        )),
        "{:?}",
...
-> std::io::Result<()> {
    ...
    assert!(
        config.startup_warnings.iter().any(|warning| warning.contains(
            "Configured filesystem path `:future_special_path` with nested entry `docs` is not recognized by this version of OrbiterX and will be ignored."
        )),
        "{:?}",
        config.startup_warnings
    );
...
async fn permissions_profiles_allow_missing_filesystem_with_warning() -> std::io::Result<()> {
    ...
    assert!(
        config.startup_warnings.iter().any(|warning| warning.contains(
            "Permissions profile `dev` does not define any recognized filesystem entries for this version of OrbiterX."
        )),
        "{:?}",
        config.startup_warnings
    );
...
fn filter_plugin_mcp_servers_by_allowlist_enforces_plugin_and_identity_rules() {
    ...
    let requirements = Sourced::new(
        BTreeMap::from([(
            ...
            orbiterx_config::PluginRequirementsToml {
...
async fn rebuild_preserving_session_layers_refreshes_requirements() -> std::io::Result<()> {
    let orbiterx_home = TempDir::new()?;
    let user_file =
        AbsolutePathBuf::resolve_path_against_base(CONFIG_TOML_FILE, orbiterx_home.path());
    let project_dot_orbiterx =
        AbsolutePathBuf::resolve_path_against_base("project/.orbiterx", orbiterx_home.path());
    ...
    let requirements_toml = orbiterx_config::ConfigRequirementsToml {
        mcp_servers: Some(mcp_requirements.clone()),
        ..Default::default()
    };
    let requirements = orbiterx_config::ConfigRequirements {
        ...
        vec![
            ...
            ConfigLayerEntry::new(
                ConfigLayerSource::Project {
                    dot_orbiterx_folder: project_dot_orbiterx.clone(),
                },
                toml::toml! {
    ...
    let refreshed_config = Config::load_config_with_layer_stack(
        ...
        refreshed_toml,
        ConfigOverrides {
            cwd: Some(orbiterx_home.path().to_path_buf()),
            ..Default::default()
        },
        orbiterx_home.abs(),
    ...
    let thread_layer_stack = ConfigLayerStack::new(
        vec![
            ...
            ConfigLayerEntry::new(
                ConfigLayerSource::Project {
                    dot_orbiterx_folder: project_dot_orbiterx,
                },
                toml::toml! {
                    [mcp_servers.fresh_project]
...
-> anyhow::Result<()> {
    let orbiterx_home = TempDir::new()?;
    let plugin_root = orbiterx_home
        .path()
        .join("plugins/cache")
        .join("test/sample/local");
    std::fs::create_dir_all(plugin_root.join(".orbiterx-plugin"))?;
    std::fs::write(
  ...
  "mcpServers": {
    ...
    let config = thread_config
        .rebuild_preserving_session_layers(&refreshed_config)
        .await?;
    let plugins_manager = PluginsManager::new(orbiterx_home.path().to_path_buf());
...
"#,
    ...
    assert_eq!(
        ...
        Some((
            orbiterx_mcp::McpServerSource::SelectedPlugin(McpPluginAttribution::new(
                "selected-root".to_string(),
                "Selected Plugin".to_string(),
            )),
            selected,
...
async fn default_zsh_path_sets_runtime_zsh_path() -> std::io::Result<()> {
    let orbiterx_home = TempDir::new()?;
    let default_zsh_path = orbiterx_home.path().join("packaged-zsh");
...
#[tokio::test]
async fn sqlite_home_defaults_to_orbiterx_home_for_workspace_write() -> std::io::Result<()> {
    let orbiterx_home = TempDir::new()?;
    let config = Config::load_from_base_config_with_overrides(
        ConfigToml::default(),
        ConfigOverrides {
    ...
    assert_eq!(config.sqlite_home, orbiterx_home.path().to_path_buf());
...
-> std::io::Result<()> {
    let orbiterx_home = TempDir::new()?;
    let memories_root = orbiterx_home.path().join("memories");
    let writable_root = orbiterx_home.path().join("writable").abs();
    let config = Config::load_from_base_config_with_overrides(
        ConfigToml {
            sandbox_workspace_write: Some(SandboxWorkspaceWrite {
...
"#,
    ...
    let project_config_dir = workspace.path().join(".orbiterx");
...
async fn managed_config_overrides_oauth_store_mode() -> anyhow::Result<()> {
    let orbiterx_home = TempDir::new()?;
    let managed_path = orbiterx_home.path().join("managed_config.toml");
    let config_path = orbiterx_home.path().join(CONFIG_TOML_FILE);
    ...
    let cwd = orbiterx_home.path().abs();
...
async fn load_global_mcp_servers_returns_empty_if_missing() -> anyhow::Result<()> {
    ...
    let servers = load_global_mcp_servers(orbiterx_home.path()).await?;
...
async fn replace_mcp_servers_round_trips_entries() -> anyhow::Result<()> {
    let orbiterx_home = TempDir::new()?;
    let expected_cwd = LegacyAppPathString::from_path(orbiterx_home.path());
    ...
    let loaded = load_global_mcp_servers(orbiterx_home.path()).await?;
...
"#,
    ...
    let err = load_global_mcp_servers(orbiterx_home.path())
        .await
...
async fn replace_mcp_servers_serializes_cwd() -> anyhow::Result<()> {
    ...
    let cwd_path = PathBuf::from("/tmp/orbiterx-mcp");
    ...
    let serialized = std::fs::read_to_string(&config_path)?;
    assert!(
        serialized.contains(r#"cwd = "/tmp/orbiterx-mcp""#),
...
async fn replace_mcp_servers_streamable_http_serializes_oauth_resource() -> anyhow::Result<()> {
    ...
    let servers = BTreeMap::from([(
        ...
        McpServerConfig {
            ...
            oauth: Some(McpServerOAuthConfig {
                client_id: Some("eci-prd-pub-orbiterx-123".to_string()),
            }),
    ...
    assert!(serialized.contains(r#"client_id = "eci-prd-pub-orbiterx-123""#));
    ...
    assert_eq!(docs.oauth_client_id(), Some("eci-prd-pub-orbiterx-123"));
...
async fn set_model_updates_defaults() -> anyhow::Result<()> {
    ...
    ConfigEditsBuilder::new(orbiterx_home.path())
        .set_model(Some("gpt-5.4"), Some(ReasoningEffort::High))
        .apply()
        .await?;

    let serialized = tokio::fs::read_to_string(orbiterx_home.path().join(CONFIG_TOML_FILE)).await?;
...
async fn for_config_writes_selected_user_config_file() -> anyhow::Result<()> {
    let orbiterx_home = TempDir::new()?;
    let base_config = orbiterx_home.path().join(CONFIG_TOML_FILE);
    let selected_config = orbiterx_home.path().join("work.config.toml");
...
fn profile_v2_config_path_resolves_validated_names() -> anyhow::Result<()> {
    ...
    assert_eq!(
        resolve_profile_v2_config_path(orbiterx_home.path(), &profile_name),
        orbiterx_home.path().join("work.config.toml").abs()
    );
    Ok(())
}
...
impl PrecedenceTestFixture {
    ...
    fn orbiterx_home(&self) -> AbsolutePathBuf {
        self.orbiterx_home.abs()
    }
}
...
async fn loads_compact_prompt_from_file() -> std::io::Result<()> {
    let orbiterx_home = TempDir::new()?;
    let workspace = orbiterx_home.path().join("workspace");
    ...
    let config =
        Config::load_from_base_config_with_overrides(cfg, overrides, orbiterx_home.abs()).await?;
...
async fn load_config_uses_requirements_guardian_policy_config() -> std::io::Result<()> {
    ...
    let config_layer_stack = ConfigLayerStack::new(
        Vec::new(),
        Default::default(),
        orbiterx_config::ConfigRequirementsToml {
...
async fn load_config_rejects_missing_agent_role_config_file() -> std::io::Result<()> {
    let orbiterx_home = TempDir::new()?;
    let missing_path = orbiterx_home.path().join("agents").join("researcher.toml");
    let cfg = ConfigToml {
...
async fn agent_role_relative_config_file_resolves_against_config_toml() -> std::io::Result<()> {
    let orbiterx_home = TempDir::new()?;
    let role_config_path = orbiterx_home.path().join("agents").join("researcher.toml");
...
"#,
    ...
    let config_layer_stack = orbiterx_config::ConfigLayerStack::new(
        vec![orbiterx_config::ConfigLayerEntry::new(
            ConfigLayerSource::User {
                file: orbiterx_home.path().join(CONFIG_TOML_FILE).abs(),
                profile: None,
            },
            layer_config,
        )],
        Default::default(),
        orbiterx_config::ConfigRequirementsToml::default(),
    )
    .map_err(std::io::Error::other)?;
...
"#
    ...
    let standalone_agents_dir = repo_root.path().join(".orbiterx").join("agents");
...
async fn loads_legacy_split_agent_roles_from_config_toml() -> std::io::Result<()> {
    let orbiterx_home = TempDir::new()?;
    let researcher_path = orbiterx_home.path().join("agents").join("researcher.toml");
    let reviewer_path = orbiterx_home.path().join("agents").join("reviewer.toml");
    tokio::fs::create_dir_all(
        researcher_path
            .parent()
            .expect("role config should have a parent directory"),
...
"#
    ...
    let home_agents_dir = orbiterx_home.path().join("agents");
...
async fn model_catalog_json_loads_from_path() -> std::io::Result<()> {
    let orbiterx_home = TempDir::new()?;
    let catalog_path = orbiterx_home.path().join("catalog.json");
...
"#;
    ...
    let orbiterx_home_temp_dir = TempDir::new().unwrap();

    Ok(PrecedenceTestFixture {
        cwd: cwd_temp_dir,
        orbiterx_home: orbiterx_home_temp_dir,
        cfg,
    })
}
...
async fn trace_exporter_defaults_to_none_when_log_exporter_is_set() -> std::io::Result<()> {
    ...
    cfg.otel = Some(OtelConfigToml {
        exporter: Some(OtelExporterKind::OtlpHttp {
            ...
            headers: HashMap::new(),
            protocol: orbiterx_config::types::OtelHttpProtocol::Binary,
            tls: None,
        }),
...
async fn config_toml_priority_service_tier_uses_priority_request_value() -> std::io::Result<()> {
    ...
    let orbiterx_home = fixture.orbiterx_home();
...
async fn test_requirements_web_search_mode_allowlist_does_not_warn_when_unset() -> anyhow::Result<()>
{
    ...
    let requirements_toml = orbiterx_config::ConfigRequirementsToml {
        ...
        allowed_web_search_modes: Some(vec![orbiterx_config::WebSearchModeRequirement::Cached]),
    ...
    let requirement_source = orbiterx_config::RequirementSource::Unknown;
    ...
    let requirements = orbiterx_config::ConfigRequirements {
        web_search_mode: orbiterx_config::ConstrainedWithSource::new(
            constrained,
    ...
    let config_layer_stack =
        orbiterx_config::ConfigLayerStack::new(Vec::new(), requirements, requirements_toml)
...
projects = { "/Users/mbolin/code/orbiterx4" = { trust_level = "trusted", foo = "bar" } , "/Users/mbolin/code/orbiterx3" = { trust_level = "trusted" } }
model = "foo""#;
    let mut doc = initial.parse::<DocumentMut>()?;

    // Approve a new directory
    let new_project = Path::new("/Users/mbolin/code/orbiterx2");
...
[projects."/Users/mbolin/code/orbiterx4"]
trust_level = "trusted"
foo = "bar"

[projects."/Users/mbolin/code/orbiterx3"]
trust_level = "trusted"
...
fn test_set_default_oss_provider() -> std::io::Result<()> {
    let temp_dir = TempDir::new()?;
    let orbiterx_home = temp_dir.path();
    let config_path = orbiterx_home.join(CONFIG_TOML_FILE);

    // Test setting valid provider on empty config
    set_default_oss_provider(orbiterx_home, OLLAMA_OSS_PROVIDER_ID)?;
    let content = std::fs::read_to_string(&config_path)?;
    assert!(content.contains("oss_provider = \"ollama\""));

    // Test updating existing config
    std::fs::write(&config_path, "model = \"gpt-4\"\n")?;
    set_default_oss_provider(orbiterx_home, LMSTUDIO_OSS_PROVIDER_ID)?;
    let content = std::fs::read_to_string(&config_path)?;
    ...
    let result = set_default_oss_provider(orbiterx_home, "invalid_provider");
...
fn test_set_default_oss_provider_rejects_legacy_ollama_chat_provider() -> std::io::Result<()> {
    ...
    let result = set_default_oss_provider(orbiterx_home, LEGACY_OLLAMA_CHAT_PROVIDER_ID);
    assert!(result.is_err());
    let error = result.unwrap_err();
    assert_eq!(error.kind(), std::io::ErrorKind::InvalidInput);
    assert!(
...
"#,
    ...
    assert_eq!(
        ...
        Some(orbiterx_config::types::WindowsSandboxModeToml::Elevated)
    );
    assert!(
...
async fn permission_profile_override_preserves_split_write_roots() -> std::io::Result<()> {
    let orbiterx_home = TempDir::new()?;
    let cwd = orbiterx_home.path().join("workspace");
    let outside_root = orbiterx_home.path().join("outside-write");
...
export_dir = "locks"
allow_orbiterx_version_mismatch = true
...
"#,
    ...
    assert!(config.config_lock_allow_orbiterx_version_mismatch);
...
async fn debug_config_lockfile_load_path_loads_lock_from_nested_table() -> std::io::Result<()> {
    let orbiterx_home = TempDir::new()?;
    let lock_path = orbiterx_home.path().join("session.config.lock.toml");
    std::fs::write(
        &lock_path,
        format!(
            r#"version = {}
orbiterx_version = "older-version"
...
-> std::io::Result<()> {
    let orbiterx_home = TempDir::new()?;
    let selected_config = orbiterx_home.path().join("default.config.toml");
    std::fs::write(
        &selected_config,
        r#"approvals_reviewer = "user"
"#,
