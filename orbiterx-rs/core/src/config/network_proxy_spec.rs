...
use orbiterx_network_proxy::BlockedRequestObserver;
...
use orbiterx_network_proxy::NetworkDecision;
use orbiterx_network_proxy::NetworkPolicyDecider;
...
use orbiterx_network_proxy::host_and_port_from_network_addr;
use orbiterx_network_proxy::normalize_host;
use orbiterx_network_proxy::validate_policy_against_constraints;
    ...
    for host in hosts {
        if incoming.insert(host.clone()) {
            config.upsert_domain_permission(
                host,
                if allow {
                    orbiterx_network_proxy::NetworkDomainPermission::Allow
                } else {
                    orbiterx_network_proxy::NetworkDomainPermission::Deny
                },
                normalize_host,
            );
        }
