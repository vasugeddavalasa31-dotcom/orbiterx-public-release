...
async fn restricted_read_implicitly_allows_helper_executables() -> std::io::Result<()> {
    ...
    let orbiterx_home = temp_dir.path().join(".orbiterx");
    let zsh_path = temp_dir.path().join("runtime").join("zsh");
    let arg0_root = orbiterx_home.join("tmp").join("arg0");
    let allowed_arg0_dir = arg0_root.join("orbiterx-arg0-session");
    let sibling_arg0_dir = arg0_root.join("orbiterx-arg0-other-session");
    let execve_wrapper = allowed_arg0_dir.join("orbiterx-execve-wrapper");
    ...
    let config = Config::load_from_base_config_with_overrides(
        ...
        AbsolutePathBuf::from_absolute_path(&orbiterx_home)?,
    )
    .await?;
...
fn network_toml_overlays_unix_socket_permissions_by_path() {
    ...
    assert_eq!(
        config.unix_sockets,
        Some(orbiterx_network_proxy::NetworkUnixSocketPermissions {
            entries: BTreeMap::from([
                (
                    "/tmp/base.sock".to_string(),
...
fn profile_network_proxy_config_keeps_proxy_disabled_for_proxy_policy() {
    ...
    assert_eq!(
        config.domains,
        Some(orbiterx_network_proxy::NetworkDomainPermissions {
            entries: vec![orbiterx_network_proxy::NetworkDomainPermissionEntry {
                pattern: "openai.com".to_string(),
                permission: orbiterx_network_proxy::NetworkDomainPermission::Allow,
            }],
        })
    );
}
