...
use crate::orbiterx_thread::BackgroundTerminalInfo;
...
const NETWORK_ACCESS_DENIED_MESSAGE: &str =
    "Network access was denied by the OrbiterX sandbox network proxy.";
...
) -> orbiterx_exec_server::ExecEnvPolicy {
    let mut exclude = policy
        .exclude
        .iter()
        .map(std::string::ToString::to_string)
        .collect::<Vec<_>>();
    exclude.push(ORBITERX_PERMISSION_PROFILE_ENV_VAR.to_string());
    let mut r#set = policy.r#set.clone();
    r#set.retain(|key, _| !key.eq_ignore_ascii_case(ORBITERX_PERMISSION_PROFILE_ENV_VAR));
    orbiterx_exec_server::ExecEnvPolicy {
...
fn env_overlay_for_exec_server(
    ...
    request_env
        .iter()
        .filter(|(key, value)| {
            key.as_str() == ORBITERX_PERMISSION_PROFILE_ENV_VAR
...
fn exec_server_env_for_request(
    ...
    Option<orbiterx_exec_server::ExecEnvPolicy>,
    HashMap<String, String>,
) {
...
fn exec_server_params_for_request(
    ...
    request: &ExecRequest,
    tty: bool,
) -> orbiterx_exec_server::ExecParams {
...
fn network_approval_error_message(err: ToolError) -> String {
    match err {
        ...
        ToolError::OrbiterX(err) => err.to_string(),
    }
}
...
impl UnifiedExecProcessManager {
    ...
    pub(crate) async fn open_session_with_exec_env(
        ...
        network_proxy_launch: Option<orbiterx_network_proxy::RemoteNetworkProxyLaunchConfig>,
        ...
        .map_err(|err| match err {
            UnifiedExecError::SandboxDenied { output, .. } => {
                ToolError::OrbiterX(OrbiterXErr::Sandbox(SandboxErr::Denied {
    ...
    pub(crate) async fn open_session_with_prepared_exec_env(
        ...
        if request.sandbox == orbiterx_sandboxing::SandboxType::WindowsRestrictedToken {
            ...
            let orbiterx_home = crate::config::find_orbiterx_home().map_err(|err| {
            ...
            let spawned = match request.windows_sandbox_level {
                orbiterx_protocol::config_types::WindowsSandboxLevel::Elevated => {
                    orbiterx_windows_sandbox::spawn_windows_sandbox_session_elevated_for_permission_profile(
                ...
                orbiterx_protocol::config_types::WindowsSandboxLevel::RestrictedToken
                | orbiterx_protocol::config_types::WindowsSandboxLevel::Disabled => {
                    orbiterx_windows_sandbox::spawn_windows_sandbox_session_legacy(
                        &request.permission_profile,
                        request.windows_sandbox_workspace_roots.as_slice(),
                        orbiterx_home.as_ref(),
                        request.command.clone(),
        ...
        let spawn_result = if tty {
            orbiterx_utils_pty::pty::spawn_process_with_inherited_fds(
                program,
                args,
                native_cwd.as_path(),
                &request.env,
                &request.arg0,
                orbiterx_utils_pty::TerminalSize::default(),
                &inherited_fds,
            )
            .await
        } else {
            orbiterx_utils_pty::pipe::spawn_process_no_stdin_with_inherited_fds(
                program,
                args,
                native_cwd.as_path(),
                &request.env,
    ...
    pub(super) async fn open_session_with_sandbox(
        ...
        env.insert(
            ORBITERX_THREAD_ID_ENV_VAR.to_string(),
