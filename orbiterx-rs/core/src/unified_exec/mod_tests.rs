...
use core_test_support::test_orbiterx::test_env as remote_test_env;
use orbiterx_exec_server::ExecProcess;
use orbiterx_exec_server::ExecProcessEventReceiver;
use orbiterx_exec_server::ExecProcessFuture;
use orbiterx_exec_server::ProcessId;
use orbiterx_exec_server::ProcessSignal;
use orbiterx_exec_server::ReadResponse;
use orbiterx_exec_server::StartedExecProcess;
use orbiterx_exec_server::WriteResponse;
use orbiterx_exec_server::WriteStatus;
...
impl BlockingTerminateExecProcess {
    async fn read(&self) -> Result<ReadResponse, orbiterx_exec_server::ExecServerError> {
        Ok(ReadResponse {
            chunks: Vec::new(),
            next_seq: 1,
            exited: false,
    ...
    async fn write(&self) -> Result<WriteResponse, orbiterx_exec_server::ExecServerError> {
        Ok(WriteResponse {
            status: WriteStatus::Accepted,
        })
    }

    async fn terminate(&self) -> Result<(), orbiterx_exec_server::ExecServerError> {
...
async fn unified_exec_persists_across_requests() -> anyhow::Result<()> {
    ...
    write_stdin(
        &session,
        process_id,
        "export ORBITERX_INTERACTIVE_SHELL_VAR=orbiterx\n",
        /*yield_time_ms*/ 2_500,
    )
    .await?;
    ...
    let out_2 = write_stdin(
        &session,
        process_id,
        "echo $ORBITERX_INTERACTIVE_SHELL_VAR\n",
        /*yield_time_ms*/ 2_500,
    )
    .await?;
    assert!(
...
async fn unified_exec_timeouts() -> anyhow::Result<()> {
    ...
    write_stdin(
        &session,
        process_id,
        format!("export ORBITERX_INTERACTIVE_SHELL_VAR={TEST_VAR_VALUE}\n").as_str(),
        /*yield_time_ms*/ 2_500,
    )
    .await?;
    ...
    let out_2 = write_stdin(
        &session,
        process_id,
        "sleep 5 && echo $ORBITERX_INTERACTIVE_SHELL_VAR\n",
        /*yield_time_ms*/ 10,
    )
    .await?;
    assert!(
...
async fn completed_pipe_commands_preserve_exit_code() -> anyhow::Result<()> {
    ...
    let environment = orbiterx_exec_server::Environment::default_for_tests();
