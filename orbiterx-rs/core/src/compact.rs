...
use orbiterx_analytics::CompactionStatus;
use orbiterx_analytics::CompactionStrategy;
use orbiterx_analytics::CompactionTrigger;
use orbiterx_analytics::OrbiterXCompactionEvent;
use orbiterx_analytics::now_unix_seconds;
use orbiterx_protocol::error::OrbiterXErr;
use orbiterx_protocol::error::Result as OrbiterXResult;
use orbiterx_protocol::items::ContextCompactionItem;
...
use orbiterx_utils_output_truncation::approx_token_count;
use orbiterx_utils_output_truncation::truncate_text;
use tracing::error;

use orbiterx_model_provider_info::ModelProviderInfo;

pub use orbiterx_prompts::SUMMARIZATION_PROMPT;
pub use orbiterx_prompts::SUMMARY_PREFIX;
...
async fn run_compact_task_inner(
    ...
    match pre_compact_outcome {
        ...
        PreCompactHookOutcome::Stopped => {
            let error = OrbiterXErr::TurnAborted;
            attempt
                .track(
                    sess.as_ref(),
    ...
    let orbiterx_error = result.as_ref().err();
    if result.is_ok() {
...
async fn run_compact_task_inner_impl(
    ...
    let responses_metadata = turn_context.turn_metadata_state.to_responses_metadata(
        ...
        window_id,
        OrbiterXResponsesRequestKind::Compaction(compaction_metadata),
    );
    ...
    loop {
        ...
        match attempt_result {
            ...
            Err(err @ (OrbiterXErr::Interrupted | OrbiterXErr::TurnAborted)) => {
                return Err(err);
            }
            Err(e @ OrbiterXErr::SessionBudgetExceeded) => {
                sess.track_turn_orbiterx_error(turn_context.as_ref(), &e);
                let event = EventMsg::Error(e.to_error_event(/*message_prefix*/ None));
                sess.send_event(&turn_context, event).await;
                return Err(e);
            }
            Err(e @ OrbiterXErr::ContextWindowExceeded) => {
                if turn_input_len > 1 {
...
impl CompactionAnalyticsAttempt {
    ...
    pub(crate) async fn track(
        ...
        orbiterx_error: Option<&OrbiterXErr>,
        ...
        sess.services
            ...
            .track_compaction(OrbiterXCompactionEvent {
                ...
                status,
                orbiterx_error_kind: orbiterx_error.map(Into::into),
                orbiterx_error_http_status_code: orbiterx_error
                    .and_then(OrbiterXErr::http_status_code_value),
                active_context_tokens_before,
...
pub(crate) fn compaction_status_from_result<T>(result: &OrbiterXResult<T>) -> CompactionStatus {
    match result {
        Ok(_) => CompactionStatus::Completed,
        Err(OrbiterXErr::Interrupted | OrbiterXErr::TurnAborted) => CompactionStatus::Interrupted,
