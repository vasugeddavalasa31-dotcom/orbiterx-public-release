...
command transformation live in the orbiterx-sandboxing crate; this module keeps
...
#[cfg(target_os = "macos")]
use crate::spawn::ORBITERX_SANDBOX_ENV_VAR;
use crate::spawn::ORBITERX_SANDBOX_NETWORK_DISABLED_ENV_VAR;
...
pub use orbiterx_protocol::models::SandboxPermissions;
...
pub(crate) struct ExecServerEnvConfig {
    pub(crate) policy: orbiterx_exec_server::ExecEnvPolicy,
...
impl ExecRequest {
    ...
    pub(crate) fn from_sandbox_exec_request(
        ...
        if !network_sandbox_policy.is_enabled() {
            env.insert(
                ORBITERX_SANDBOX_NETWORK_DISABLED_ENV_VAR.to_string(),
                "1".to_string(),
            );
        }
        #[cfg(target_os = "macos")]
        if sandbox == SandboxType::MacosSeatbelt {
            env.insert(ORBITERX_SANDBOX_ENV_VAR.to_string(), "seatbelt".to_string());
        }
        Self {
            command,
            cwd,
...
) -> orbiterx_protocol::error::Result<ExecToolCallOutput> {
