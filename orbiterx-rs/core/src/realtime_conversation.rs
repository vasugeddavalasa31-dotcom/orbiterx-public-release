...
use orbiterx_api::RealtimeAudioFrame;
use orbiterx_api::RealtimeContextAppendChannel;
use orbiterx_api::RealtimeEvent;
use orbiterx_api::RealtimeEventParser;
use orbiterx_api::RealtimeSessionConfig;
use orbiterx_api::RealtimeSessionMode;
use orbiterx_api::RealtimeWebsocketClient;
use orbiterx_api::RealtimeWebsocketEvents;
use orbiterx_api::RealtimeWebsocketWriter;
use orbiterx_api::map_api_error;
...
use orbiterx_login::read_openai_api_key_from_env;
...
use orbiterx_protocol::protocol::OrbiterXResponseHandoffMode;
use orbiterx_protocol::protocol::RealtimeConversationClosedEvent;
use orbiterx_protocol::protocol::RealtimeConversationRealtimeEvent;
use orbiterx_protocol::protocol::RealtimeConversationSdpEvent;
use orbiterx_protocol::protocol::RealtimeConversationStartedEvent;
use orbiterx_protocol::protocol::RealtimeHandoffRequested;
use orbiterx_protocol::protocol::RealtimeOutputModality;
use orbiterx_protocol::protocol::RealtimeTranscriptEntry;
use orbiterx_protocol::protocol::RealtimeVoice;
use orbiterx_protocol::protocol::RealtimeVoicesList;
use orbiterx_utils_output_truncation::approx_bytes_for_tokens;
use orbiterx_utils_string::take_bytes_at_char_boundary;
use serde_json::json;
...
const STANDALONE_HANDOFF_ID: &str = "orbiterx";
...
struct RealtimeHandoffState {
    ...
    orbiterx_responses_as_items: bool,
    orbiterx_response_item_prefix: Option<String>,
    orbiterx_response_handoff_mode: OrbiterXResponseHandoffMode,
...
impl RealtimeHandoffState {
    fn new(
        ...
        Self {
            ...
            orbiterx_responses_as_items,
            orbiterx_response_item_prefix,
    ...
    fn streams_handoff_append(&self) -> bool {
        self.event_parser == RealtimeEventParser::FramelessBidi
            ...
            && !self.orbiterx_responses_as_items
    }

    fn routes_handoff_by_bem(&self) -> bool {
        self.event_parser == RealtimeEventParser::FramelessBidi
            && self.orbiterx_response_handoff_mode == OrbiterXResponseHandoffMode::BemTags
    }
}
...
impl RealtimeConversationManager {
    ...
    async fn start(&self, start: RealtimeStart) -> OrbiterXResult<RealtimeStartOutput> {
        let previous_state = {
    ...
    async fn start_inner(&self, start: RealtimeStart) -> OrbiterXResult<RealtimeStartOutput> {
        let RealtimeStart {
            api_provider,
            extra_headers,
            client_managed_handoffs,
    ...
    pub(crate) async fn audio_in(&self, frame: RealtimeAudioFrame) -> OrbiterXResult<()> {
        let sender = {
        ...
        match sender.try_send(frame) {
            ...
            Err(TrySendError::Closed(_)) => Err(OrbiterXErr::InvalidRequest(
                "conversation is not running".to_string(),
            )),
        }
    }

    pub(crate) async fn text_in(&self, mut params: ConversationTextParams) -> OrbiterXResult<()> {
        let sender = {
            let guard = self.state.lock().await;
            guard
                .as_ref()
        ...
        sender
            .send(params)
            .await
            .map_err(|_| OrbiterXErr::InvalidRequest("conversation is not running".to_string()))?;
        Ok(())
    }
        ...
        let output = match active_handoff {
            Some(handoff_id) => {
                ...
                *handoff.last_output.lock().await = Some(RealtimeHandoffOutput {
                ...
                if handoff.orbiterx_responses_as_items {
                    RealtimeOutbound::ConversationItem {
                        text: realtime_backend_item(
                            output_text,
                            handoff.orbiterx_response_item_prefix.as_deref(),
                        ),
                        phase,
                    }
    ...
    pub(crate) async fn append_speech(&self, text: String) -> OrbiterXResult<()> {
    ...
    pub(crate) async fn handoff_complete(&self) -> OrbiterXResult<()> {
        let handoff = {
            let guard = self.state.lock().await;
            guard.as_ref().map(|state| state.handoff.clone())
        };
        ...
        let output = if handoff.orbiterx_responses_as_items {
    ...
    pub(crate) async fn shutdown(&self) -> OrbiterXResult<()> {
        let state = {
            let mut guard = self.state.lock().await;
            guard.take()
        };
...
async fn prepare_realtime_start(
    sess: &Arc<Session>,
    params: ConversationStartParams,
) -> OrbiterXResult<PreparedRealtimeConversationStart> {
    ...
    Ok(PreparedRealtimeConversationStart {
        ...
        orbiterx_responses_as_items: params.orbiterx_responses_as_items,
        orbiterx_response_item_prefix: params.orbiterx_response_item_prefix,
        orbiterx_response_handoff_mode: params.orbiterx_response_handoff_mode,
        realtime_call_api_provider,
...
) -> OrbiterXResult<RealtimeSessionConfig> {
    let config = sess.get_config().await;
...
fn validate_realtime_voice(version: RealtimeWsVersion, voice: RealtimeVoice) -> OrbiterXResult<()> {
    let voices = RealtimeVoicesList::builtin();
...
pub(crate) async fn handle_audio(
    ...
    if let Err(err) = sess.conversation.audio_in(params.frame).await {
        ...
        if sess.conversation.running_state().await.is_some() {
            ...
            send_conversation_error(sess, sub_id, err.to_string(), OrbiterXErrorInfo::BadRequest)
                .await;
        }
    }
}
...
fn realtime_api_key(
    ...
    if let Some(api_key) = auth.and_then(OrbiterXAuth::api_key) {
    ...
    Err(OrbiterXErr::InvalidRequest(
...
fn realtime_request_headers(
    ...
    event_parser: RealtimeEventParser,
    originator: &str,
) -> OrbiterXResult<Option<HeaderMap>> {
    ...
    if let Some(api_key) = api_key {
        let auth_value = HeaderValue::from_str(&format!("Bearer {api_key}")).map_err(|err| {
            OrbiterXErr::InvalidRequest(format!("invalid realtime api key header: {err}"))
        })?;
...
fn v3_output_writer(
    writer: &RealtimeWebsocketWriter,
    phase: Option<&MessagePhase>,
    handoff_mode: OrbiterXResponseHandoffMode,
) -> RealtimeWebsocketWriter {
    let channel = match handoff_mode {
        OrbiterXResponseHandoffMode::Thinking => None,
        OrbiterXResponseHandoffMode::Commentary => Some(RealtimeContextAppendChannel::Commentary),
        OrbiterXResponseHandoffMode::BemTags => match phase {
        ...
        RealtimeEventParser::FramelessBidi => match handoff_output {
            RealtimeOutbound::StandaloneHandoff { text, phase } => {
                v3_output_writer(
                    writer,
                    phase.as_ref(),
                    handoff_state.orbiterx_response_handoff_mode,
                )
                .send_standalone_handoff(STANDALONE_HANDOFF_ID.to_string(), text)
                .await
            }
...
async fn send_conversation_error(
    ...
    sub_id: String,
    message: String,
    orbiterx_error_info: OrbiterXErrorInfo,
) {
    sess.send_event_raw(Event {
        id: sub_id,
