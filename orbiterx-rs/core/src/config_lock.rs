...
pub(crate) struct ConfigLockReplayOptions {
    pub allow_orbiterx_version_mismatch: bool,
}
...
pub(crate) fn validate_config_lock_replay(
    ...
    if !options.allow_orbiterx_version_mismatch
        && expected_lock.orbiterx_version != actual_lock.orbiterx_version
    {
        return Err(config_lock_error(format!(
            "config lock OrbiterX version mismatch: lock was generated by {}, current version is {}; set debug.config_lockfile.allow_orbiterx_version_mismatch=true to ignore this",
            expected_lock.orbiterx_version, actual_lock.orbiterx_version
        )));
    }
...
fn config_lock_for_comparison(
    ...
    if options.allow_orbiterx_version_mismatch {
        lockfile.orbiterx_version.clear();
    }
    lockfile
}
