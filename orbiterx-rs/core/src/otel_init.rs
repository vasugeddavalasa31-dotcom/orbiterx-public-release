...
use orbiterx_config::types::OtelExporterKind as Kind;
use orbiterx_config::types::OtelHttpProtocol as Protocol;
use orbiterx_features::Feature;
use orbiterx_login::default_client::originator;
use orbiterx_otel::OtelExporter;
use orbiterx_otel::OtelHttpProtocol;
use orbiterx_otel::OtelProvider;
use orbiterx_otel::OtelSettings;
use orbiterx_otel::OtelTlsConfig as OtelTlsSettings;
use std::error::Error;
...
/// Filter predicate for exporting only OrbiterX-owned events via OTEL.
/// Keeps events that originated from orbiterx_otel module
pub fn orbiterx_export_filter(meta: &tracing::Metadata<'_>) -> bool {
    meta.target().starts_with("orbiterx_otel")
}
...
pub fn record_process_start(otel: Option<&OtelProvider>, originator: &str) {
    ...
    let _ = orbiterx_otel::record_process_start_once(metrics, originator);
}
...
pub fn install_sqlite_telemetry(otel: Option<&OtelProvider>, originator: &str) {
    ...
    let telemetry = orbiterx_rollout::sqlite_telemetry_recorder(metrics.clone(), originator);
