...
use orbiterx_analytics::AppInvocation;
use orbiterx_analytics::InvocationType;
use orbiterx_analytics::build_track_events_context;
...
use orbiterx_connectors::AppToolPolicy;
use orbiterx_connectors::AppToolPolicyEvaluator;
use orbiterx_connectors::AppToolPolicyInput;
use orbiterx_features::Feature;
use orbiterx_hooks::PermissionRequestDecision;
use orbiterx_mcp::MCP_TOOL_ORBITERX_APPS_META_KEY;
use orbiterx_mcp::McpConnectionManager;
use orbiterx_mcp::McpPermissionPromptAutoApproveContext;
use orbiterx_mcp::ORBITERX_APPS_MCP_SERVER_NAME;
use orbiterx_mcp::SandboxState;
use orbiterx_mcp::auth_elicitation_completed_result;
use orbiterx_mcp::build_auth_elicitation_plan;
use orbiterx_mcp::mcp_permission_prompt_is_auto_approved;
use orbiterx_mcp::tool_is_model_visible;
use orbiterx_protocol::approvals::ElicitationRequest;
use orbiterx_protocol::items::McpToolCallError;
use orbiterx_protocol::items::McpToolCallItem;
use orbiterx_protocol::items::McpToolCallStatus;
use orbiterx_protocol::items::TurnItem;
use orbiterx_protocol::mcp::CallToolResult;
use orbiterx_protocol::mcp_approval_meta::APPROVAL_KIND_KEY as MCP_TOOL_APPROVAL_KIND_KEY;
use orbiterx_protocol::mcp_approval_meta::APPROVAL_KIND_MCP_TOOL_CALL as MCP_TOOL_APPROVAL_KIND_MCP_TOOL_CALL;
use orbiterx_protocol::mcp_approval_meta::CONNECTOR_DESCRIPTION_KEY as MCP_TOOL_APPROVAL_CONNECTOR_DESCRIPTION_KEY;
use orbiterx_protocol::mcp_approval_meta::CONNECTOR_ID_KEY as MCP_TOOL_APPROVAL_CONNECTOR_ID_KEY;
use orbiterx_protocol::mcp_approval_meta::CONNECTOR_NAME_KEY as MCP_TOOL_APPROVAL_CONNECTOR_NAME_KEY;
use orbiterx_protocol::mcp_approval_meta::PERSIST_ALWAYS as MCP_TOOL_APPROVAL_PERSIST_ALWAYS;
use orbiterx_protocol::mcp_approval_meta::PERSIST_KEY as MCP_TOOL_APPROVAL_PERSIST_KEY;
use orbiterx_protocol::mcp_approval_meta::PERSIST_SESSION as MCP_TOOL_APPROVAL_PERSIST_SESSION;
use orbiterx_protocol::mcp_approval_meta::SOURCE_CONNECTOR as MCP_TOOL_APPROVAL_SOURCE_CONNECTOR;
use orbiterx_protocol::mcp_approval_meta::SOURCE_KEY as MCP_TOOL_APPROVAL_SOURCE_KEY;
use orbiterx_protocol::mcp_approval_meta::TOOL_DESCRIPTION_KEY as MCP_TOOL_APPROVAL_TOOL_DESCRIPTION_KEY;
use orbiterx_protocol::mcp_approval_meta::TOOL_PARAMS_DISPLAY_KEY as MCP_TOOL_APPROVAL_TOOL_PARAMS_DISPLAY_KEY;
use orbiterx_protocol::mcp_approval_meta::TOOL_PARAMS_KEY as MCP_TOOL_APPROVAL_TOOL_PARAMS_KEY;
use orbiterx_protocol::mcp_approval_meta::TOOL_TITLE_KEY as MCP_TOOL_APPROVAL_TOOL_TITLE_KEY;
...
use orbiterx_protocol::request_user_input::RequestUserInputAnswer;
use orbiterx_protocol::request_user_input::RequestUserInputArgs;
use orbiterx_protocol::request_user_input::RequestUserInputQuestion;
use orbiterx_protocol::request_user_input::RequestUserInputQuestionOption;
...
use orbiterx_utils_output_truncation::TruncationPolicy;
use orbiterx_utils_output_truncation::truncate_text;
...
const MCP_RESULT_TELEMETRY_META_KEY: &str = "orbiterx/telemetry";
...
const MCP_RESULT_TELEMETRY_TARGET_ID_SPAN_ATTR: &str = "orbiterx.mcp.target.id";
const MCP_RESULT_TELEMETRY_SERVER_USER_FLOW_SPAN_ATTR: &str =
    "orbiterx.mcp.server_user_flow.triggered";
...
pub(crate) async fn handle_mcp_tool_call(
    ...
    let app_tool_policy = if server == ORBITERX_APPS_MCP_SERVER_NAME {
    ...
    let approval_mode = if server == ORBITERX_APPS_MCP_SERVER_NAME {
    ...
    if server == ORBITERX_APPS_MCP_SERVER_NAME && !app_tool_policy.enabled {
        let result = notify_mcp_tool_call_skip(
            sess.as_ref(),
            turn_context.as_ref(),
            &call_id,
...
impl McpToolCallItemMetadata {
    fn from_tool_metadata(server: &str, metadata: Option<&McpToolApprovalMetadata>) -> Self {
        let trusted_mcp_app_metadata = if server == ORBITERX_APPS_MCP_SERVER_NAME {
            metadata
        } else {
            None
        };
        Self {
            ...
            action_name: trusted_mcp_app_metadata
                .and_then(|metadata| metadata.orbiterx_apps_meta.as_ref())
...
async fn handle_approved_mcp_tool_call(
    ...
    .await;
    maybe_track_orbiterx_app_used(sess, turn_context, manager, &server, &tool_name).await;
...
fn mcp_tool_call_span(
    ...
    let span = tracing::info_span!(
        ...
        server.port = Empty,
        orbiterx.mcp.target.id = Empty,
        orbiterx.mcp.server_user_flow.triggered = Empty,
        error.type = Empty,
        orbiterx.mcp.error.code = Empty,
    );
...
async fn execute_mcp_tool_call(
    ...
    Ok(maybe_request_orbiterx_apps_auth_elicitation(
        sess,
        turn_context,
        manager,
        call_id,
...
async fn maybe_request_orbiterx_apps_auth_elicitation(
    sess: &Session,
    turn_context: &TurnContext,
    ...
    if !manager.is_host_owned_orbiterx_apps_server(server) {
        return result;
    }
    ...
    let install_url = connector_id.map(|connector_id| {
        orbiterx_connectors::metadata::connector_install_url(
    ...
    let response = sess
        .request_mcp_server_elicitation(
            turn_context,
            ORBITERX_APPS_MCP_SERVER_NAME.to_string(),
            request_id,
            request,
        )
        .await
    ...
    refresh_orbiterx_apps_after_connector_auth(sess, turn_context, manager).await;
    auth_elicitation_completed_result(&plan.auth_failure, result.meta)
}

async fn refresh_orbiterx_apps_after_connector_auth(
    sess: &Session,
    turn_context: &TurnContext,
    manager: &McpConnectionManager,
) {
    let mcp_tools_result = manager.hard_refresh_orbiterx_apps_tools_cache().await;
    ...
    match mcp_tools_result {
        ...
        Err(err) => {
            tracing::warn!("failed to refresh OrbiterX Apps tools after connector auth: {err:#}");
        }
    }
}
...
async fn augment_mcp_tool_request_meta_with_sandbox_state(
    ...
    let server_environment_id = manager
        ...
        .unwrap_or(orbiterx_config::DEFAULT_MCP_SERVER_ENVIRONMENT_ID);
    ...
    let sandbox_state = serde_json::to_value(SandboxState {
        permission_profile,
        orbiterx_linux_sandbox_exe: step_context.mcp.config().orbiterx_linux_sandbox_exe.clone(),
        sandbox_cwd,
    ...
    match meta.as_mut() {
        Some(serde_json::Value::Object(map)) => {
            map.insert(
                orbiterx_mcp::MCP_SANDBOX_STATE_META_CAPABILITY.to_string(),
                sandbox_state,
            );
        }
        Some(_) => {}
...
fn sandbox_cwd_for_mcp_server(step_context: &StepContext, environment_id: &str) -> Option<PathUri> {
    ...
    if environment_id == orbiterx_config::DEFAULT_MCP_SERVER_ENVIRONMENT_ID {
        #[allow(deprecated)]
...
async fn maybe_track_orbiterx_app_used(
    sess: &Session,
    turn_context: &TurnContext,
    manager: &McpConnectionManager,
    server: &str,
    ...
    if server != ORBITERX_APPS_MCP_SERVER_NAME {
        return;
    }
...
pub(crate) struct McpToolApprovalMetadata {
    ...
    mcp_app_resource_uri: Option<String>,
    orbiterx_apps_meta: Option<serde_json::Map<String, serde_json::Value>>,
...
async fn custom_mcp_tool_approval_mode(
    ...
    let user_configured_mode = turn_context
        ...
        .cloned()
        .and_then(|value| {
            HashMap::<String, orbiterx_config::types::McpServerConfig>::deserialize(value).ok()
        })
        .and_then(|servers| {
...
fn build_mcp_tool_call_request_meta(
    ...
    {
        request_meta.insert(
            crate::X_ORBITERX_TURN_METADATA_HEADER.to_string(),
            turn_metadata,
        );
    }

    if server == ORBITERX_APPS_MCP_SERVER_NAME {
        let mut orbiterx_apps_meta = metadata
            .and_then(|metadata| metadata.orbiterx_apps_meta.clone())
            .unwrap_or_default();
        orbiterx_apps_meta.insert(
            "call_id".to_string(),
            serde_json::Value::String(call_id.to_string()),
        );
        request_meta.insert(
            MCP_TOOL_ORBITERX_APPS_META_KEY.to_string(),
            serde_json::Value::Object(orbiterx_apps_meta),
        );
    }
...
pub(crate) const MCP_TOOL_APPROVAL_DECLINE_SYNTHETIC: &str = "__orbiterx_mcp_decline__";
...
fn session_mcp_tool_approval_key(
    ...
    if invocation.server == ORBITERX_APPS_MCP_SERVER_NAME && connector_id.is_none() {
        return None;
    }
...
pub(crate) fn build_guardian_mcp_tool_review_request(
    ...
    GuardianApprovalRequest::McpToolCall {
        ...
        connected_account_email: (invocation.server == ORBITERX_APPS_MCP_SERVER_NAME)
...
pub(crate) async fn lookup_mcp_tool_metadata(
    ...
    let connector_description = if server == ORBITERX_APPS_MCP_SERVER_NAME {
    ...
    let orbiterx_apps_meta = tool_info
        .tool
        .meta
        .as_ref()
        .and_then(|meta| meta.get(MCP_TOOL_ORBITERX_APPS_META_KEY))
        .and_then(serde_json::Value::as_object)
        .cloned();
    let connected_account_email = if server == ORBITERX_APPS_MCP_SERVER_NAME {
        orbiterx_apps_meta
            .as_ref()
...
fn openai_file_input_optional_fields_for_server(
    ...
    (server == ORBITERX_APPS_MCP_SERVER_NAME)
...
async fn maybe_persist_mcp_tool_approval(
    ...
    let persist_result = if key.server == ORBITERX_APPS_MCP_SERVER_NAME {
        let Some(connector_id) = key.connector_id.clone() else {
            remember_mcp_tool_approval(sess, key).await;
            return;
        };
        persist_orbiterx_app_tool_approval(&turn_context.config, &connector_id, &tool_name).await
    } else {
...
async fn persist_orbiterx_app_tool_approval(
    config: &Config,
    connector_id: &str,
    tool_name: &str,
) -> anyhow::Result<()> {
...
fn user_mcp_server_is_configured(config: &Config, server: &str) -> anyhow::Result<bool> {
    ...
    let servers =
        HashMap::<String, orbiterx_config::types::McpServerConfig>::deserialize(mcp_servers_toml)?;
    ...
    config
        ...
        .find_map(|layer| {
            ...
            let servers = layer
                ...
                .cloned()
                .and_then(|value| {
                    HashMap::<String, orbiterx_config::types::McpServerConfig>::deserialize(value)
                        .ok()
                })?;
