...
pub(crate) struct SessionState {
    ...
    pub(crate) pending_session_start_sources: VecDeque<orbiterx_hooks::SessionStartSource>,
...
impl SessionState {
    ...
    pub(crate) fn queue_pending_session_start_source(
        &mut self,
        value: orbiterx_hooks::SessionStartSource,
    ) {
...
// missing values as the default `"orbiterx"` bucket.
fn merge_rate_limit_fields(
    previous: Option<&RateLimitSnapshot>,
    mut snapshot: RateLimitSnapshot,
) -> RateLimitSnapshot {
    if snapshot.limit_id.is_none() {
        snapshot.limit_id = Some("orbiterx".to_string());
    }
