...
pub use orbiterx_protocol::shell_environment::ORBITERX_THREAD_ID_ENV_VAR;
...
pub const ORBITERX_PERMISSION_PROFILE_ENV_VAR: &str = "ORBITERX_PERMISSION_PROFILE";
...
///
/// `ORBITERX_THREAD_ID` is injected when a thread id is provided, even when
/// `include_only` is set.
pub fn create_env(
...
pub(crate) fn inject_permission_profile_env(
    ...
    if cfg!(windows) {
        env.retain(|key, _| !key.eq_ignore_ascii_case(ORBITERX_PERMISSION_PROFILE_ENV_VAR));
    } else {
        env.remove(ORBITERX_PERMISSION_PROFILE_ENV_VAR);
    }
