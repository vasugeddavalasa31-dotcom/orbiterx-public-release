...
use futures::Future;
use orbiterx_memories_read::citations::parse_memory_citation;
use orbiterx_memories_read::citations::thread_ids_from_memory_citation;
use orbiterx_protocol::error::OrbiterXErr;
use orbiterx_protocol::error::Result;
use orbiterx_protocol::memory_citation::MemoryCitation;
...
use orbiterx_utils_stream_parser::strip_proposed_plan_blocks;
use tracing::debug;
...
fn strip_hidden_assistant_markup_and_parse_memory_citation(
    ...
    String,
    Option<orbiterx_protocol::memory_citation::MemoryCitation>,
) {
        ...
        && role == "assistant"
    {
        let combined = content
            ...
            .filter_map(|ci| match ci {
                orbiterx_protocol::models::ContentItem::OutputText { text } => Some(text.as_str()),
                _ => None,
            })
            .collect::<String>();
            ...
            TurnItem::AgentMessage(agent_message) => {
                let combined = agent_message
                    ...
                    .map(|entry| match entry {
                        orbiterx_protocol::items::AgentMessageContent::Text { text } => {
                            text.as_str()
                        }
                    })
                    .collect::<String>();
...
pub(crate) async fn handle_output_item_done(
    ...
    match ToolRouter::build_tool_call(item.clone()) {
        ...
        Err(FunctionCallError::Fatal(message)) => {
            return Err(OrbiterXErr::Fatal(message));
        }
    }
...
pub(crate) async fn finalize_turn_item(
    ...
    if let TurnItem::AgentMessage(agent_message) = &mut *turn_item {
        ...
        agent_message.content =
            vec![orbiterx_protocol::items::AgentMessageContent::Text { text: stripped }];
