use crate::OrbiterXAppsToolsCache;
...
use orbiterx_agent_graph_store::AgentGraphStore;
use orbiterx_agent_graph_store::LocalAgentGraphStore;
...
use orbiterx_code_mode::CodeModeSessionProvider;
use orbiterx_code_mode::InProcessCodeModeSessionProvider;
use orbiterx_code_mode::ProcessOwnedCodeModeSessionProvider;
...
use orbiterx_login::default_client::ORBITERX_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR;
...
use orbiterx_model_provider_info::OPENAI_PROVIDER_ID;
...
use orbiterx_thread_store::InMemoryThreadStore;
use orbiterx_thread_store::LoadThreadHistoryParams;
use orbiterx_thread_store::LocalThreadStore;
use orbiterx_thread_store::LocalThreadStoreConfig;
use orbiterx_thread_store::ReadThreadByRolloutPathParams;
use orbiterx_thread_store::ReadThreadParams;
use orbiterx_thread_store::StoredModelContext;
use orbiterx_thread_store::StoredThread;
use orbiterx_thread_store::ThreadMetadataPatch;
use orbiterx_thread_store::ThreadStore;
use orbiterx_thread_store::ThreadStoreError;
use orbiterx_thread_store::UpdateThreadMetadataParams;
...
struct TempOrbiterXHomeGuard {
    path: PathBuf,
}

impl Drop for TempOrbiterXHomeGuard {
    fn drop(&mut self) {
        let _ = std::fs::remove_dir_all(&self.path);
    }
}

/// Represents a newly created OrbiterX thread (formerly called a conversation), including the first event
...
pub struct ThreadManager {
    ...
    _test_orbiterx_home_guard: Option<TempOrbiterXHomeGuard>,
}
...
pub struct StartThreadOptions {
    ...
    pub dynamic_tools: Vec<orbiterx_protocol::dynamic_tools::DynamicToolSpec>,
...
pub(crate) struct ThreadManagerState {
    threads: Arc<RwLock<HashMap<ThreadId, Arc<OrbiterXThread>>>>,
...
pub fn thread_store_from_config(
    ...
    match &config.experimental_thread_store {
        ThreadStoreConfig::Local => {
            ...
            {
                orbiterx_rollout::spawn_rollout_compression_worker(
                    config.orbiterx_home.to_path_buf(),
                );
            }
...
impl ThreadManager {
    ...
    pub fn new(
        ...
        auth_manager: Arc<AuthManager>,
        models_manager: SharedModelsManager,
        orbiterx_apps_tools_cache: OrbiterXAppsToolsCache,
        session_source: SessionSource,
        environment_manager: Arc<EnvironmentManager>,
        extensions: Arc<ExtensionRegistry<Config>>,
        user_instructions_provider: Arc<dyn UserInstructionsProvider>,
        ...
        let orbiterx_home = config.orbiterx_home.clone();
        ...
        Self {
            ...
            _test_orbiterx_home_guard: None,
        }
    }
    ...
    /// Construct with a dummy AuthManager containing the provided OrbiterXAuth.
    ...
    pub(crate) fn with_models_provider_for_tests(
        ...
        let orbiterx_home = std::env::temp_dir().join(format!(
            "orbiterx-thread-manager-test-{}",
            uuid::Uuid::new_v4()
        ));
        std::fs::create_dir_all(&orbiterx_home)
            .unwrap_or_else(|err| panic!("temp orbiterx home dir create failed: {err}"));
        ...
        manager._test_orbiterx_home_guard = Some(TempOrbiterXHomeGuard {
            path: orbiterx_home,
        });
        manager
    }

    /// Construct with a dummy AuthManager containing the provided OrbiterXAuth and orbiterx home.
    /// Used for integration tests: should not be used by ordinary business logic.
    ...
    pub(crate) fn with_models_provider_home_and_state_for_tests(
        ...
        let skills_orbiterx_home = match AbsolutePathBuf::from_absolute_path_checked(&orbiterx_home)
        {
            Ok(orbiterx_home) => orbiterx_home,
            Err(err) => panic!("test orbiterx_home should be absolute: {err}"),
        };
        let (thread_created_tx, _) = broadcast::channel(THREAD_CREATED_CHANNEL_CAPACITY);
        ...
        let thread_store: Arc<dyn ThreadStore> = Arc::new(LocalThreadStore::new(
            LocalThreadStoreConfig {
                ...
                sqlite_home: orbiterx_home.clone(),
        ...
        Self {
            state: Arc::new(ThreadManagerState {
                ...
                models_manager: create_model_provider(provider, Some(auth_manager.clone()))
                    .models_manager(orbiterx_home, /*config_model_catalog*/ None),
                environment_manager,
                skills_service,
                plugins_manager,
                mcp_manager,
    ...
    pub fn validate_environment_selections(
        ...
        for environment in environments {
            if !environment_ids.insert(environment.environment_id.as_str()) {
                return Err(OrbiterXErr::InvalidRequest(format!(
            ...
            self.state
                ...
                .ok_or_else(|| {
                    OrbiterXErr::InvalidRequest(format!(
    ...
    pub async fn list_models(
        ...
        http_client_factory: orbiterx_http_client::HttpClientFactory,
    ) -> Vec<ModelPreset> {
        self.state
            .models_manager
    ...
    pub async fn get_thread(&self, thread_id: ThreadId) -> OrbiterXResult<Arc<OrbiterXThread>> {
        self.state.get_thread(thread_id).await
    }

    /// Updates metadata for loaded and cold threads through one entrypoint.
    ///
    /// Loaded threads route through `OrbiterXThread`/`LiveThread`, so metadata changes stay ordered
        ...
        self.state
            ...
            .map_err(|err| match err {
                ThreadStoreError::ThreadNotFound { thread_id } => {
                    OrbiterXErr::ThreadNotFound(thread_id)
                }
    ...
    pub async fn list_agent_subtree_thread_ids(
        ...
        if let Some(agent_graph_store) = self.state.agent_graph_store() {
            for descendant_id in agent_graph_store
                ...
                .await
                .map_err(|err| {
                    OrbiterXErr::Fatal(format!("failed to load thread-spawn descendants: {err}"))
                })?
            {
    ...
    pub async fn start_thread(&self, config: Config) -> OrbiterXResult<NewThread> {
    ...
    pub async fn start_thread_with_tools(
        &self,
        config: Config,
        dynamic_tools: Vec<orbiterx_protocol::dynamic_tools::DynamicToolSpec>,
    ) -> OrbiterXResult<NewThread> {
    ...
    /// as `Arc<OrbiterXThread>`, it is possible that other references to it exist elsewhere.
    /// Returns the thread if the thread was found and removed.
    pub async fn remove_thread(&self, thread_id: &ThreadId) -> Option<Arc<OrbiterXThread>> {
...
impl ThreadManagerState {
    ...
    pub(crate) async fn get_thread(
        &self,
        thread_id: ThreadId,
    ) -> OrbiterXResult<Arc<OrbiterXThread>> {
        let threads = self.threads.read().await;
        match threads.get(&thread_id) {
            Some(thread) if !thread.session_source.is_internal() => Ok(thread.clone()),
            Some(_) | None => Err(OrbiterXErr::ThreadNotFound(thread_id)),
        }
    }
    ...
    pub(crate) async fn read_stored_thread(
        ...
        self.thread_store
            ...
            .map_err(|err| match err {
                ...
                err => OrbiterXErr::Fatal(format!("failed to read stored thread {thread_id}: {err}")),
            })
    }
    ...
    ) -> OrbiterXResult<StoredModelContext> {
        let thread_id = params.thread_id;
        self.thread_store
    ...
    pub(crate) async fn send_op(&self, thread_id: ThreadId, op: Op) -> OrbiterXResult<String> {
    ...
    pub(crate) async fn remove_thread(&self, thread_id: &ThreadId) -> Option<Arc<OrbiterXThread>> {
    ...
    async fn effective_originator(
        ...
        let env_originator = std::env::var(ORBITERX_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR)
            .is_ok()
    ...
    async fn finalize_thread_spawn(
        ...
        let session_configured = match event {
            ...
            _ => {
                return Err(OrbiterXErr::SessionConfiguredNotFirstEvent);
            }
        };

        {
            ...
            if let std::collections::hash_map::Entry::Vacant(e) = threads.entry(thread_id) {
                let thread = Arc::new(OrbiterXThread::new(
                    session,
                    io,
        ...
        Err(OrbiterXErr::InvalidRequest(format!(
    ...
    async fn parent_rollout_thread_trace_for_source(
        ...
        session_source: &SessionSource,
        initial_history: &InitialHistory,
    ) -> orbiterx_rollout_trace::ThreadTraceContext {
        ...
        let SessionSource::SubAgent(SubAgentSource::ThreadSpawn {
            ...
            return orbiterx_rollout_trace::ThreadTraceContext::disabled();
        };
        ...
        self.get_thread(*parent_thread_id)
            ...
            .unwrap_or_else(orbiterx_rollout_trace::ThreadTraceContext::disabled)
    }
}
...
fn thread_store_rollout_read_error(err: ThreadStoreError) -> OrbiterXErr {
    match err {
        ThreadStoreError::ThreadNotFound { thread_id } => OrbiterXErr::ThreadNotFound(thread_id),
        ThreadStoreError::InvalidRequest { message } => OrbiterXErr::InvalidRequest(message),
        err => OrbiterXErr::Fatal(format!("failed to read thread by rollout path: {err}")),
    }
}

fn thread_store_metadata_update_error(thread_id: ThreadId, err: ThreadStoreError) -> OrbiterXErr {
    match err {
        ThreadStoreError::ThreadNotFound { thread_id } => OrbiterXErr::ThreadNotFound(thread_id),
        ThreadStoreError::InvalidRequest { message } => OrbiterXErr::InvalidRequest(message),
        ThreadStoreError::Unsupported { operation } => OrbiterXErr::UnsupportedOperation(format!(
