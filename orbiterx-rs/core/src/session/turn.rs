...
use orbiterx_analytics::CompactionPhase;
use orbiterx_analytics::CompactionReason;
use orbiterx_analytics::InvocationType;
use orbiterx_analytics::TurnResolvedConfigFact;
...
use orbiterx_core_skills::injection::InjectedHostSkillPrompts;
use orbiterx_extension_api::TurnInputContext;
use orbiterx_extension_api::TurnInputEnvironment;
use orbiterx_features::Feature;
use orbiterx_git_utils::get_git_repo_root_with_fs;
...
use orbiterx_protocol::items::PlanItem;
...
use orbiterx_protocol::protocol::AgentMessageContentDeltaEvent;
use orbiterx_protocol::protocol::AgentReasoningSectionBreakEvent;
...
use orbiterx_protocol::protocol::PlanDeltaEvent;
use orbiterx_protocol::protocol::RawResponseCompletedEvent;
use orbiterx_protocol::protocol::ReasoningContentDeltaEvent;
use orbiterx_protocol::protocol::ReasoningRawContentDeltaEvent;
use orbiterx_protocol::protocol::SafetyBufferingEvent;
...
use orbiterx_tools::filter_request_plugin_install_discoverable_tools_for_client;
use orbiterx_utils_stream_parser::AssistantTextChunk;
use orbiterx_utils_stream_parser::AssistantTextStreamParser;
use orbiterx_utils_stream_parser::ProposedPlanSegment;
use orbiterx_utils_stream_parser::extract_proposed_plan_text;
use orbiterx_utils_stream_parser::strip_citations;
...
pub(crate) async fn run_turn(
    ...
    turn_extension_data: Arc<orbiterx_extension_api::ExtensionData>,
    input: Vec<TurnInput>,
    prewarmed_client_session: Option<ModelClientSession>,
    cancellation_token: CancellationToken,
) -> OrbiterXResult<Option<String>> {
    ...
    if let Err(err) = run_pre_sampling_compact(&sess, &turn_context, &mut client_session).await {
        if matches!(err, OrbiterXErr::TurnAborted) {
            return Err(err);
        }
        let error = err.to_orbiterx_protocol_error();
    ...
    // Although from the perspective of orbiterx.rs, TurnDiffTracker has the lifecycle of a Task which contains
    ...
    loop {
        ...
        let sampling_request_result: OrbiterXResult<_> = async {
            ...
            let responses_metadata = turn_context.turn_metadata_state.to_responses_metadata(
                ...
                window_id,
                OrbiterXResponsesRequestKind::Turn,
            );
        ...
        match sampling_request_result {
            ...
            Err(err @ OrbiterXErr::TurnAborted) => {
                return Err(err);
            }
            Err(orbiterx_error @ OrbiterXErr::InvalidImageRequest()) => {
                {
                ...
                sess.track_turn_orbiterx_error(turn_context.as_ref(), &orbiterx_error);
                let error = OrbiterXErrorInfo::BadRequest;
                sess.emit_turn_error_lifecycle(turn_context.as_ref(), error.clone())
                    .await;
                let event = EventMsg::Error(ErrorEvent {
                    message: "Invalid image in your last message. Please remove it and try again."
                        .to_string(),
                    orbiterx_error_info: Some(error),
                });
            ...
            Err(e) => {
                info!("Turn error: {e:#}");
                let error = e.to_orbiterx_protocol_error();
                sess.emit_turn_error_lifecycle(turn_context.as_ref(), error.clone())
                    .await;
                sess.track_turn_orbiterx_error(turn_context.as_ref(), &e);
...
async fn build_skills_and_plugins(
    ...
    let available_connectors = if turn_context.apps_enabled() {
        let connectors = orbiterx_connectors::merge::merge_plugin_connectors_with_accessible(
            connector_snapshot
                .connector_ids()
                .iter()
...
///
/// Returns `None` when the active authentication does not use the OrbiterX backend, the provider is
...
async fn capture_current_model_fallback_step_context(
    ...
    let uses_orbiterx_backend = turn_context
        .auth_manager
        .as_deref()
        .is_some_and(orbiterx_login::AuthManager::current_auth_uses_orbiterx_backend);
    if !uses_orbiterx_backend
...
pub(super) fn collect_explicit_app_ids_from_skill_items(
    ...
    for connector in connectors {
        let slug = orbiterx_connectors::metadata::connector_mention_slug(connector);
...
async fn run_sampling_request(
    sess: Arc<Session>,
    step_context: Arc<StepContext>,
    turn_store: Arc<orbiterx_extension_api::ExtensionData>,
    turn_diff_tracker: SharedTurnDiffTracker,
    client_session: &mut ModelClientSession,
    responses_metadata: &OrbiterXResponsesMetadata,
    input: Vec<ResponseItem>,
    cancellation_token: CancellationToken,
) -> OrbiterXResult<(SamplingRequestResult, Vec<ResponseItem>)> {
    ...
    loop {
        ...
        .await
        {
            ...
            Err(OrbiterXErr::ContextWindowExceeded) => {
                sess.set_total_tokens_full(&turn_context).await;
                return Err(OrbiterXErr::ContextWindowExceeded);
            }
            Err(OrbiterXErr::UsageLimitReached(e)) => {
                let rate_limits = e.rate_limits.clone();
                if let Some(rate_limits) = rate_limits {
                    sess.update_rate_limits(&turn_context, *rate_limits).await;
                }
                return Err(OrbiterXErr::UsageLimitReached(e));
            }
            Err(err) => err,
        };
...
pub(super) fn agent_message_text(item: &orbiterx_protocol::items::AgentMessageItem) -> String {
    item.content
        .iter()
        .map(|entry| match entry {
            orbiterx_protocol::items::AgentMessageContent::Text { text } => text.as_str(),
        })
        .collect()
}
...
async fn emit_agent_message_in_plan_mode(
    sess: &Session,
    turn_context: &TurnContext,
    agent_message: orbiterx_protocol::items::AgentMessageItem,
    state: &mut PlanModeStreamState,
) {
        ...
        .contains(&agent_message_id)
    {
        let start_item = state
            ...
            .unwrap_or_else(|| {
                TurnItem::AgentMessage(orbiterx_protocol::items::AgentMessageItem {
...
async fn handle_assistant_item_done_in_plan_mode(
    sess: &Session,
    turn_context: &TurnContext,
    turn_store: &orbiterx_extension_api::ExtensionData,
    item: &ResponseItem,
    state: &mut PlanModeStreamState,
    previously_active_item: Option<&TurnItem>,
...
async fn drain_in_flight(
    in_flight: &mut FuturesOrdered<BoxFuture<'static, OrbiterXResult<ResponseInputItem>>>,
    sess: Arc<Session>,
    turn_context: Arc<TurnContext>,
) -> OrbiterXResult<()> {
...
async fn try_run_sampling_request(
    ...
    prompt: &Prompt,
    cancellation_token: CancellationToken,
) -> OrbiterXResult<SamplingRequestResult> {
    feedback_tags!(
        model = turn_context.model_info.slug.clone(),
        approval_policy = turn_context.approval_policy.value(),
    ...
    let mut stream = client_session
        ...
        .or_cancel(&cancellation_token)
        .await??;
    let mut in_flight: FuturesOrdered<BoxFuture<'static, OrbiterXResult<ResponseInputItem>>> =
        FuturesOrdered::new();
    ...
    let outcome: OrbiterXResult<SamplingRequestResult> = loop {
        let handle_responses = trace_span!(
            ...
            from = field::Empty,
            orbiterx.request.reasoning_effort = %reasoning_effort,
            gen_ai.usage.input_tokens = field::Empty,
            gen_ai.usage.cache_read.input_tokens = field::Empty,
            gen_ai.usage.cache_write.input_tokens = field::Empty,
            gen_ai.usage.output_tokens = field::Empty,
            orbiterx.usage.reasoning_output_tokens = field::Empty,
            orbiterx.usage.total_tokens = field::Empty,
        );
        ...
        {
            Ok(event) => event,
            Err(orbiterx_async_utils::CancelErr::Cancelled) => break Err(OrbiterXErr::TurnAborted),
        };
                        ...
                        && let Some(raw_text) = raw_assistant_output_text_from_item(&item)
                    {
                        ...
                        if let TurnItem::AgentMessage(agent_message) = &mut turn_item {
                            agent_message.content =
                                vec![orbiterx_protocol::items::AgentMessageContent::Text {
                                    text: if plan_mode {
                                        String::new()
                                    } else {
    ...
    if cancellation_token.is_cancelled() {
        return Err(OrbiterXErr::TurnAborted);
    }
