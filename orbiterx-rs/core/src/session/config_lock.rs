...
use orbiterx_config::config_toml::OrchestratorFeatureToml;
use orbiterx_config::config_toml::OrchestratorToml;
...
use orbiterx_features::RolloutBudgetConfigToml;
...
pub(crate) async fn validate_config_lock_if_configured(
    ...
    let options = ConfigLockReplayOptions {
        allow_orbiterx_version_mismatch: config.config_lock_allow_orbiterx_version_mismatch,
    };
...
/// OrbiterX actually ran with, not only the user-authored TOML inputs.
...
mod tests {
    ...
    async fn lock_contains_prompts_and_materializes_features() {
        ...
        for spec in orbiterx_features::FEATURES {
            assert_eq!(
        ...
        assert_eq!(
            ...
            Some(FeatureToml::Config(CurrentTimeReminderConfigToml {
                ...
                clock_source: Some(orbiterx_features::CurrentTimeSource::System),
                delivery_mode: Some(
                    orbiterx_features::CurrentTimeReminderDeliveryMode::AnyInference
                ),
    ...
    #[tokio::test]
    async fn lock_validation_rejects_orbiterx_version_mismatch_by_default() {
        let sc = crate::session::tests::make_session_configuration_for_tests().await;
        let mut expected = sc.to_config_lockfile_toml().expect("lock should serialize");
        expected.orbiterx_version = "older-version".to_string();
        let actual = sc.to_config_lockfile_toml().expect("lock should serialize");
        ...
        let message = error.to_string();
        assert!(
            message.contains("config lock OrbiterX version mismatch"),
            "{message}"
        );
        assert!(
            message.contains("debug.config_lockfile.allow_orbiterx_version_mismatch=true"),
            "{message}"
        );
    }

    #[tokio::test]
    async fn lock_validation_can_ignore_orbiterx_version_mismatch() {
        let sc = crate::session::tests::make_session_configuration_for_tests().await;
        let mut expected = sc.to_config_lockfile_toml().expect("lock should serialize");
        expected.orbiterx_version = "older-version".to_string();
        let actual = sc.to_config_lockfile_toml().expect("lock should serialize");
