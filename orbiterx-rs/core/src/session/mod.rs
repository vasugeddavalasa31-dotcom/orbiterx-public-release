...
use orbiterx_analytics::SubAgentThreadStartedInput;
use orbiterx_analytics::TurnOrbiterXErrorFact;
use orbiterx_config::types::AuthKeyringBackendKind;
use orbiterx_config::types::OAuthCredentialsStoreMode;
use orbiterx_connectors::connector_runtime_context_key;
use orbiterx_core_skills::injection::HostSkillsCatalogInWorldState;
use orbiterx_exec_server::Environment;
...
use orbiterx_extension_api::PromptSlot;
use orbiterx_extension_api::TurnContextContributionInput;
use orbiterx_features::FEATURES;
use orbiterx_features::Feature;
use orbiterx_features::unstable_features_warning_event;
use orbiterx_hooks::Hooks;
use orbiterx_hooks::HooksConfig;
use orbiterx_login::AuthManager;
use orbiterx_login::OrbiterXAuth;
use orbiterx_login::auth_env_telemetry::collect_auth_env_telemetry;
use orbiterx_mcp::McpConnectionManager;
use orbiterx_mcp::McpResourceClient;
use orbiterx_mcp::McpRuntime;
...
use orbiterx_network_proxy::NetworkProxyAuditMetadata;
use orbiterx_network_proxy::normalize_host;
use orbiterx_otel::current_span_trace_id;
use orbiterx_otel::current_span_w3c_trace_context;
use orbiterx_otel::set_parent_from_w3c_trace_context;
use orbiterx_protocol::SessionId;
use orbiterx_protocol::ThreadId;
use orbiterx_protocol::approvals::ElicitationRequest;
use orbiterx_protocol::approvals::ElicitationRequestEvent;
use orbiterx_protocol::approvals::ExecPolicyAmendment;
use orbiterx_protocol::approvals::NetworkPolicyAmendment;
use orbiterx_protocol::approvals::NetworkPolicyRuleAction;
...
use orbiterx_protocol::dynamic_tools::DynamicToolResponse;
use orbiterx_protocol::dynamic_tools::DynamicToolSpec;
use orbiterx_protocol::items::EnteredReviewModeItem;
use orbiterx_protocol::items::TurnItem;
use orbiterx_protocol::items::UserMessageItem;
use orbiterx_protocol::models::ActivePermissionProfile;
use orbiterx_protocol::models::AdditionalPermissionProfile;
...
use orbiterx_protocol::models::format_allow_prefixes;
...
use orbiterx_protocol::protocol::AdditionalContextEntry;
use orbiterx_protocol::protocol::FileChange;
use orbiterx_protocol::protocol::HasLegacyEvent;
...
use orbiterx_protocol::protocol::RawResponseItemEvent;
...
use orbiterx_protocol::protocol::TurnContextItem;
use orbiterx_protocol::protocol::TurnContextNetworkItem;
...
use orbiterx_protocol::protocol::WorldStateItem;
...
use orbiterx_protocol::request_permissions::RequestPermissionsArgs;
use orbiterx_protocol::request_permissions::RequestPermissionsEvent;
use orbiterx_protocol::request_permissions::RequestPermissionsResponse;
use orbiterx_protocol::request_user_input::RequestUserInputArgs;
use orbiterx_protocol::request_user_input::RequestUserInputResponse;
use orbiterx_rmcp_client::ElicitationResponse;
use orbiterx_rollout::state_db;
use orbiterx_rollout_trace::AgentResultTracePayload;
use orbiterx_rollout_trace::ThreadStartedTraceMetadata;
use orbiterx_rollout_trace::ThreadTraceContext;
use orbiterx_sandboxing::policy_transforms::intersect_permission_profiles;
use orbiterx_shell_command::parse_command::parse_command;
use orbiterx_terminal_detection::user_agent;
use orbiterx_thread_store::CreateThreadParams;
use orbiterx_thread_store::LiveThread;
use orbiterx_thread_store::LiveThreadInitGuard;
use orbiterx_thread_store::LocalThreadStore;
use orbiterx_thread_store::ReadThreadParams;
use orbiterx_thread_store::ResumeThreadParams;
use orbiterx_thread_store::ThreadPersistenceMetadata;
...
use crate::orbiterx_thread::ThreadConfigSnapshot;
...
#[cfg(test)]
use orbiterx_protocol::exec_output::StreamOutput;
    ...
    fn to_error_event(&self) -> ErrorEvent {
        match self {
            Self::NoActiveTurn(_) => ErrorEvent {
                ...
                orbiterx_error_info: Some(OrbiterXErrorInfo::BadRequest),
            },
...
use orbiterx_core_plugins::RecommendedPluginCandidatesInput;
use orbiterx_git_utils::get_git_repo_root;
use orbiterx_mcp::McpConfig;
use orbiterx_mcp::effective_mcp_servers;
use orbiterx_otel::SessionTelemetry;
use orbiterx_otel::THREAD_STARTED_METRIC;
...
use orbiterx_protocol::config_types::ReasoningSummary as ReasoningSummaryConfig;
use orbiterx_protocol::config_types::WindowsSandboxLevel;
use orbiterx_protocol::models::LocalImagePreparation;
use orbiterx_protocol::models::ResponseInputItem;
use orbiterx_protocol::models::ResponseItem;
use orbiterx_protocol::openai_models::ReasoningEffort as ReasoningEffortConfig;
use orbiterx_protocol::protocol::ApplyPatchApprovalRequestEvent;
...
use orbiterx_protocol::protocol::ErrorEvent;
...
use orbiterx_protocol::protocol::McpServerRefreshConfig;
use orbiterx_protocol::protocol::ModelRerouteEvent;
use orbiterx_protocol::protocol::ModelRerouteReason;
use orbiterx_protocol::protocol::ModelVerification;
use orbiterx_protocol::protocol::ModelVerificationEvent;
use orbiterx_protocol::protocol::NetworkApprovalContext;
...
use orbiterx_protocol::protocol::RequestUserInputEvent;
...
use orbiterx_protocol::protocol::SessionNetworkProxyRuntime;
use orbiterx_protocol::protocol::StreamErrorEvent;
use orbiterx_protocol::protocol::Submission;
use orbiterx_protocol::protocol::ThreadMemoryMode;
...
use orbiterx_protocol::protocol::TurnModerationMetadataEvent;
use orbiterx_protocol::protocol::WarningEvent;
use orbiterx_protocol::user_input::UserInput;
use orbiterx_tools::UnifiedExecShellMode;
use orbiterx_utils_absolute_path::AbsolutePathBuf;
#[cfg(test)]
use orbiterx_utils_stream_parser::ProposedPlanSegment;
...
pub(crate) struct SessionSpawnArgs {
    ...
    pub(crate) code_mode_session_provider: Arc<dyn orbiterx_code_mode::CodeModeSessionProvider>,
    pub(crate) extensions: Arc<orbiterx_extension_api::ExtensionRegistry<crate::config::Config>>,
...
const CYBER_SAFETY_URL: &str = "https://developers.openai.com/orbiterx/concepts/cyber-safety";

impl Session {
    /// Spawn and initialize a new session.
    pub(crate) async fn spawn(args: SessionSpawnArgs) -> OrbiterXResult<(Arc<Self>, SessionIo)> {
        let parent_trace = match args.parent_trace {
            Some(trace) => {
                if orbiterx_otel::context_from_w3c_trace_context(&trace).is_some() {
                    Some(trace)
                } else {
    ...
    async fn spawn_internal(args: SessionSpawnArgs) -> OrbiterXResult<(Arc<Self>, SessionIo)> {
        ...
        let exec_policy = if crate::guardian::is_guardian_reviewer_source(&session_source) {
            ...
            Arc::new(
                ExecPolicyManager::load(&config.config_layer_stack)
                    .await
                    .map_err(|err| OrbiterXErr::Fatal(format!("failed to load rules: {err}")))?,
            )
        };

        let config = Arc::new(config);
        let refresh_strategy = if session_source.is_non_root_agent() {
            orbiterx_models_manager::manager::RefreshStrategy::Offline
        } else {
            orbiterx_models_manager::manager::RefreshStrategy::OnlineIfUncached
        };
        ...
        .map_err(|e| {
            ...
            map_session_init_error(&e, &config.orbiterx_home)
        })?;
...
impl SessionIo {
    ...
    pub(crate) async fn submit(&self, op: Op) -> OrbiterXResult<String> {
    ...
    pub(crate) async fn submit_with_id(&self, mut sub: Submission) -> OrbiterXResult<()> {
        ...
        self.tx_sub
            .send(sub)
            .await
            .map_err(|_| OrbiterXErr::InternalAgentDied)?;
        Ok(())
    }

    pub(crate) async fn shutdown_and_wait(&self) -> OrbiterXResult<()> {
        let session_loop_termination = self.session_loop_termination.clone();
        match self.submit(Op::Shutdown).await {
            Ok(_) => {}
            Err(OrbiterXErr::InternalAgentDied) => {}
    ...
    pub(crate) async fn next_event(&self) -> OrbiterXResult<Event> {
        let event = self
            .rx_event
            .recv()
            .await
...
fn session_permission_profile_state_from_config(
    config: &Config,
) -> OrbiterXResult<PermissionProfileState> {
...
impl Session {
    ...
    /// Builds the `x-orbiterx-beta-features` header value for this session.
    ///
    ...
    async fn start_managed_network_proxy(
        ...
        exec_policy: &orbiterx_execpolicy::Policy,
        permission_profile: &PermissionProfile,
        network_policy_decider: Option<Arc<dyn orbiterx_network_proxy::NetworkPolicyDecider>>,
        blocked_request_observer: Option<Arc<dyn orbiterx_network_proxy::BlockedRequestObserver>>,
    ...
    #[cfg(test)]
    pub(crate) async fn orbiterx_home(&self) -> AbsolutePathBuf {
        let state = self.state.lock().await;
        state.session_configuration.orbiterx_home().clone()
    }
                    ...
                    .filter(|model| *model != curr)
                {
                    ...
                    self.send_event(
                        ...
                        EventMsg::Warning(WarningEvent {
                         ...
                         Consider switching back to `{prev}` as it may affect OrbiterX performance."
                            ),
                        }),
                    )
                    .await;
    ...
    pub(crate) async fn user_instructions(
        &self,
    ) -> Option<orbiterx_extension_api::UserInstructions> {
    ...
    /// Record a terminal OrbiterXErr before the app-server completion notification is reduced.
    pub(crate) fn track_turn_orbiterx_error(
        &self,
        turn_context: &TurnContext,
        error: &OrbiterXErr,
    ) {
        self.services
            .analytics_events_client
            .track_turn_orbiterx_error(TurnOrbiterXErrorFact::from_orbiterx_err(
    ...
    pub(crate) async fn send_event(&self, turn_context: &TurnContext, msg: EventMsg) {
        ...
        if let EventMsg::Error(error) = &legacy_source
            && error
                .orbiterx_error_info
                .as_ref()
                .is_some_and(OrbiterXErrorInfo::affects_turn_status)
        {
            turn_context
                .terminal_error
                .lock()
        ...
        self.services
            ...
            .record_orbiterx_turn_event(&turn_context.sub_id, &legacy_source);
        self.services
            .rollout_thread_trace
    ...
    async fn forward_child_completion_to_parent(
        ...
        child_agent_path: &orbiterx_protocol::AgentPath,
        status: AgentStatus,
    ) {
        let Some(parent_agent_path) = child_agent_path
            .as_str()
            .rsplit_once('/')
            .and_then(|(parent, _)| orbiterx_protocol::AgentPath::try_from(parent).ok())
        else {
            return;
        };
    ...
    pub(crate) async fn send_event_raw_without_materializing_rollout(&self, event: Event) {
        let persist = match self.current_rollout_path().await {
            Ok(Some(path)) => orbiterx_rollout::existing_rollout_path(&path)
                .await
                .is_some(),
            Ok(None) => true,
            Err(err) => {
    ...
    pub(crate) async fn persist_execpolicy_amendment(
        ...
        self.services
            .exec_policy
            .append_amendment_and_update(&orbiterx_home, amendment)
            .await?;
    ...
    pub(crate) async fn request_permissions_for_environment(
        ...
        let Ok(native_environment_cwd) = environment.cwd.to_abs_path() else {
            warn!(
                cwd = %environment.cwd,
                "request_permissions requires a cwd native to the OrbiterX host"
            );
            return Some(RequestPermissionsResponse {
                permissions: RequestPermissionProfile::default(),
                scope: PermissionGrantScope::Turn,
        ...
        if crate::guardian::routes_approval_to_guardian(turn_context.as_ref()) {
            ...
            let review_rx = crate::guardian::spawn_approval_request_review(
                ...
                request,
                /*retry_reason*/ None,
                orbiterx_analytics::GuardianApprovalRequestSource::MainTurn,
    ...
    async fn maybe_warn_on_server_model_mismatch(
        ...
        let warning_message = format!(
            "Your account was flagged for potentially high-risk cyber activity and this request was routed to gpt-5.2 as a fallback. To regain access to gpt-5.3-orbiterx, apply for trusted access: {CYBER_VERIFY_URL} or learn more: {CYBER_SAFETY_URL}"
        );
    ...
    pub(crate) async fn replace_compacted_history(
        ...
        {
            let mut state = self.state.lock().await;
            state.queue_pending_session_start_source(orbiterx_hooks::SessionStartSource::Compact);
        }
    }
    ...
    pub(crate) async fn notify_stream_error(
        ...
        orbiterx_error: OrbiterXErr,
    ) {
        let additional_details = orbiterx_error.to_string();
        let orbiterx_error_info = OrbiterXErrorInfo::ResponseStreamDisconnected {
            http_status_code: orbiterx_error.http_status_code_value(),
        };
    ...
    pub(crate) async fn take_pending_session_start_source(
        &self,
    ) -> Option<orbiterx_hooks::SessionStartSource> {
        let mut state = self.state.lock().await;
...
async fn build_hooks_for_config(
    ...
    Hooks::new(HooksConfig {
        ...
        feature_enabled: config.features.enabled(Feature::OrbiterXHooks),
