...
use crate::orbiterx_thread::TryStartTurnIfIdleRejectionReason;
...
use orbiterx_config::ConfigLayerStackOrdering;
use orbiterx_config::DEFAULT_MCP_SERVER_ENVIRONMENT_ID;
use orbiterx_config::LoaderOverrides;
use orbiterx_config::NetworkConstraints;
use orbiterx_config::NetworkDomainPermissionToml;
use orbiterx_config::NetworkDomainPermissionsToml;
use orbiterx_config::RequirementSource;
use orbiterx_config::Sourced;
use orbiterx_config::loader::project_trust_key;
use orbiterx_config::types::McpServerConfig;
use orbiterx_config::types::McpServerTransportConfig;
use orbiterx_config::types::ToolSuggestDisabledTool;
use orbiterx_core_skills::HostSkillsSnapshot;
...
use orbiterx_protocol::ResponseItemId;
use orbiterx_protocol::SessionId;
use orbiterx_protocol::ThreadId;
use orbiterx_protocol::config_types::SERVICE_TIER_DEFAULT_REQUEST_VALUE;
use orbiterx_protocol::config_types::ServiceTier;
use orbiterx_protocol::config_types::TrustLevel;
use orbiterx_protocol::exec_output::ExecToolCallOutput;
use orbiterx_protocol::models::AgentMessageInputContent;
use orbiterx_protocol::models::FileSystemPermissions;
use orbiterx_protocol::models::FunctionCallOutputBody;
use orbiterx_protocol::models::FunctionCallOutputContentItem;
use orbiterx_protocol::models::FunctionCallOutputPayload;
use orbiterx_protocol::models::ImageDetail;
use orbiterx_protocol::models::PermissionProfile;
use orbiterx_protocol::models::SandboxEnforcement;
use orbiterx_protocol::openai_models::ModelServiceTier;
use orbiterx_protocol::permissions::FileSystemAccessMode;
use orbiterx_protocol::permissions::FileSystemPath;
use orbiterx_protocol::permissions::FileSystemSandboxEntry;
use orbiterx_protocol::permissions::FileSystemSandboxPolicy;
use orbiterx_protocol::permissions::FileSystemSpecialPath;
use orbiterx_protocol::protocol::NonSteerableTurnKind;
use orbiterx_protocol::protocol::SandboxPolicy;
use orbiterx_protocol::protocol::ThreadHistoryMode;
use orbiterx_protocol::protocol::TurnEnvironmentSelections;
use orbiterx_protocol::request_permissions::PermissionGrantScope;
use orbiterx_protocol::request_permissions::RequestPermissionProfile;
use orbiterx_utils_path_uri::PathUri;
use tracing::Span;
...
use core_test_support::test_orbiterx::local;
...
use orbiterx_config::config_toml::ConfigToml;
use orbiterx_config::config_toml::ProjectConfig;
use orbiterx_config::permissions_toml::FilesystemPermissionToml;
use orbiterx_config::permissions_toml::FilesystemPermissionsToml;
use orbiterx_config::permissions_toml::NetworkToml;
use orbiterx_config::permissions_toml::PermissionProfileToml;
use orbiterx_config::permissions_toml::PermissionsToml;
use orbiterx_execpolicy::Decision;
use orbiterx_execpolicy::NetworkRuleProtocol;
use orbiterx_execpolicy::Policy;
use orbiterx_network_proxy::NetworkProxyConfig;
use orbiterx_otel::MetricsClient;
use orbiterx_otel::MetricsConfig;
use orbiterx_otel::THREAD_SKILLS_DESCRIPTION_TRUNCATED_CHARS_METRIC;
use orbiterx_otel::THREAD_SKILLS_ENABLED_TOTAL_METRIC;
use orbiterx_otel::THREAD_SKILLS_KEPT_TOTAL_METRIC;
use orbiterx_otel::THREAD_SKILLS_TRUNCATED_METRIC;
use orbiterx_otel::TelemetryAuthMode;
use orbiterx_protocol::config_types::CollaborationMode;
use orbiterx_protocol::config_types::ModeKind;
use orbiterx_protocol::config_types::Settings;
use orbiterx_protocol::items::HookPromptFragment;
use orbiterx_protocol::items::build_hook_prompt_message;
use orbiterx_protocol::models::BaseInstructions;
use orbiterx_protocol::models::ContentItem;
use orbiterx_protocol::models::InternalChatMessageMetadataPassthrough;
use orbiterx_protocol::models::ResponseItem;
use orbiterx_protocol::protocol::AskForApproval;
use orbiterx_protocol::protocol::CompactedItem;
use orbiterx_protocol::protocol::ConversationAudioParams;
use orbiterx_protocol::protocol::CreditsSnapshot;
use orbiterx_protocol::protocol::GranularApprovalConfig;
...
use orbiterx_protocol::protocol::NetworkApprovalProtocol;
use orbiterx_protocol::protocol::OrbiterXErrorInfo;
use orbiterx_protocol::protocol::RateLimitSnapshot;
use orbiterx_protocol::protocol::RateLimitWindow;
use orbiterx_protocol::protocol::RealtimeAudioFrame;
use orbiterx_protocol::protocol::RealtimeConversationListVoicesResponseEvent;
use orbiterx_protocol::protocol::RealtimeVoice;
...
use orbiterx_protocol::protocol::SessionMeta;
use orbiterx_protocol::protocol::SessionMetaLine;
use orbiterx_protocol::protocol::SkillScope;
use orbiterx_protocol::protocol::Submission;
use orbiterx_protocol::protocol::ThreadRolledBackEvent;
use orbiterx_protocol::protocol::ThreadSettingsOverrides;
use orbiterx_protocol::protocol::TokenCountEvent;
use orbiterx_protocol::protocol::TokenUsage;
use orbiterx_protocol::protocol::TokenUsageInfo;
use orbiterx_protocol::protocol::TurnAbortedEvent;
use orbiterx_protocol::protocol::TurnCompleteEvent;
use orbiterx_protocol::protocol::TurnStartedEvent;
use orbiterx_protocol::protocol::UserMessageEvent;
use orbiterx_protocol::protocol::W3cTraceContext;
use orbiterx_rmcp_client::ElicitationAction;
use std::path::Path;
...
use orbiterx_protocol::mcp::CallToolResult as McpCallToolResult;
...
fn test_session_telemetry_without_metadata() -> SessionTelemetry {
    ...
    let metrics = MetricsClient::new(
        MetricsConfig::in_memory("test", "orbiterx-core", env!("CARGO_PKG_VERSION"), exporter)
            .with_runtime_reader(),
    )
...
async fn regular_turn_emits_turn_started_with_trace_id_without_waiting_for_startup_prewarm() {
    let _trace_test_context = install_test_tracing("orbiterx-core-tests");
...
fn test_model_client_session() -> crate::client::ModelClientSession {
    ...
    crate::client::ModelClient::new(
        ...
        orbiterx_protocol::protocol::SessionSource::Exec,
        "test_originator".to_string(),
...
fn write_project_hooks(dot_orbiterx: &Path) -> std::io::Result<()> {
    std::fs::create_dir_all(dot_orbiterx)?;
    std::fs::write(
        dot_orbiterx.join("hooks.json"),
        r#"{
  "hooks": {
    "SessionStart": [
      {
...
async fn write_project_trust_config(
    ...
    tokio::fs::write(
        orbiterx_home.join(orbiterx_config::CONFIG_TOML_FILE),
...
async fn preview_session_start_hooks(
...
) -> std::io::Result<Vec<orbiterx_protocol::protocol::HookRunSummary>> {
    ...
    Ok(
        hooks.preview_session_start(&orbiterx_hooks::SessionStartRequest {
            session_id: ThreadId::new(),
            cwd: config.cwd.clone(),
            transcript_path: None,
            model: "gpt-5.2".to_string(),
            permission_mode: "default".to_string(),
            target: orbiterx_hooks::StartHookTarget::SessionStart {
                source: orbiterx_hooks::SessionStartSource::Startup,
            },
        }),
    )
}
...
async fn managed_network_proxy_decider_survives_full_access_start() -> anyhow::Result<()> {
    ...
    let network_policy_decider: Arc<dyn orbiterx_network_proxy::NetworkPolicyDecider> = Arc::new({
        let decider_calls = Arc::clone(&decider_calls);
        move |_request| {
            decider_calls.fetch_add(1, std::sync::atomic::Ordering::SeqCst);
            async { orbiterx_network_proxy::NetworkDecision::ask("not_allowed") }
        }
    });
...
async fn danger_full_access_tool_attempts_do_not_enforce_managed_network() -> anyhow::Result<()> {
    ...
    impl crate::tools::sandboxing::Approvable<()> for ProbeToolRuntime {
        ...
        fn approval_action(
            ...
            Ok(crate::tools::sandboxing::ApprovalAction::Shell {
                ...
                environment_id: orbiterx_exec_server::LOCAL_ENVIRONMENT_ID.to_string(),
                command: Vec::new(),
                #[allow(deprecated)]
    ...
    impl crate::tools::sandboxing::Sandboxable for ProbeToolRuntime {
        fn sandbox_preference(&self) -> orbiterx_sandboxing::SandboxablePreference {
            orbiterx_sandboxing::SandboxablePreference::Auto
        }
    }
    ...
    let session = make_session_with_config(move |config| {
        ...
        requirements_toml.network = Some(orbiterx_config::NetworkRequirementsToml {
            enabled: Some(true),
            ..Default::default()
        });
    ...
    let tool_ctx = crate::tools::sandboxing::ToolCtx {
        ...
        tool_name: orbiterx_tools::ToolName::plain("probe"),
    };
...
async fn reload_user_config_layer_updates_effective_apps_config() {
    let (session, _turn_context) = make_session_and_context().await;
    let orbiterx_home = session.orbiterx_home().await;
    std::fs::create_dir_all(&orbiterx_home).expect("create orbiterx home");
    let config_toml_path = orbiterx_home.join(CONFIG_TOML_FILE);
    std::fs::write(
        &config_toml_path,
    ...
    let apps_toml = config
        ...
        .cloned()
        .expect("apps table");
    let apps = orbiterx_config::types::AppsConfigToml::deserialize(apps_toml)
...
async fn reload_user_config_layer_updates_base_and_selected_profile_layers() {
    ...
    let orbiterx_home = session.orbiterx_home().await;
    std::fs::create_dir_all(&orbiterx_home).expect("create orbiterx home");
    let base_config_path = orbiterx_home.join(CONFIG_TOML_FILE);
    let profile_config_path = orbiterx_home.join("work.config.toml");
    std::fs::write(
        &base_config_path,
    ...
    assert_eq!(
        config
            ...
            .map(orbiterx_utils_absolute_path::AbsolutePathBuf::as_path),
...
async fn reload_user_config_layer_refreshes_hooks() -> anyhow::Result<()> {
    let session = make_session_with_config(|config| {
        config
            ...
            .expect("enable OrbiterX hooks");
    })
    .await?;
    let orbiterx_home = session.orbiterx_home().await;
    std::fs::create_dir_all(&orbiterx_home)?;
    let config_toml_path = orbiterx_home.join(CONFIG_TOML_FILE);
    let user_config: orbiterx_config::TomlValue = serde_json::from_value(serde_json::json!({
        "hooks": {
            "SessionStart": [{
                "hooks": [{
                    "type": "command",
    ...
    let request = orbiterx_hooks::SessionStartRequest {
    ...
    let config = session.get_config().await;
    let hook_list = orbiterx_hooks::list_hooks(orbiterx_hooks::HooksConfig {
        feature_enabled: true,
    ...
    assert_eq!(
        ...
        orbiterx_protocol::protocol::HookTrustStatus::Untrusted
    );

    let trusted_user_config: orbiterx_config::TomlValue =
...
async fn refresh_runtime_config_refreshes_hooks() -> anyhow::Result<()> {
    ...
    struct NormalizedHookIdentity {
        ...
        #[serde(flatten)]
        group: orbiterx_config::MatcherGroup,
    }
    let trusted_hash = {
        let identity = NormalizedHookIdentity {
            ...
            group: orbiterx_config::MatcherGroup {
                matcher: None,
                hooks: vec![orbiterx_config::HookHandlerConfig::Command {
        ...
        let identity = orbiterx_config::TomlValue::try_from(identity)?;
        orbiterx_config::version_for_toml(&identity)
    };
...
async fn record_inter_agent_communication_preserves_item_id_in_rollout_and_resume() {
    let (mut session, turn_context, _rx) = make_session_and_context_with_auth_and_config_and_rx(
        OrbiterXAuth::from_api_key("Test API Key"),
        Vec::new(),
        |config| {
...
async fn record_token_usage_info_notifies_extension_contributors() {
    ...
    impl orbiterx_extension_api::TokenUsageContributor for TokenUsageRecorder {
        fn on_token_usage<'a>(
            &'a self,
            session_store: &'a orbiterx_extension_api::ExtensionData,
            thread_store: &'a orbiterx_extension_api::ExtensionData,
            turn_store: &'a orbiterx_extension_api::ExtensionData,
            token_usage: &'a TokenUsageInfo,
        ) -> orbiterx_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
                self.records
                    .lock()
    ...
    let mut builder =
        orbiterx_extension_api::ExtensionRegistryBuilder::<crate::config::Config>::new();
...
async fn turn_start_lifecycle_exposes_turn_metadata_and_token_baseline() {
    ...
    impl orbiterx_extension_api::TurnLifecycleContributor for TurnStartRecorder {
        fn on_turn_start<'a>(
            &'a self,
            input: orbiterx_extension_api::TurnStartInput<'a>,
        ) -> orbiterx_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
                self.records
                    .lock()
...
async fn turn_error_lifecycle_exposes_error_and_stores() {
    ...
    impl orbiterx_extension_api::TurnLifecycleContributor for TurnErrorRecorder {
        fn on_turn_error<'a>(
            &'a self,
            input: orbiterx_extension_api::TurnErrorInput<'a>,
        ) -> orbiterx_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
                self.records
                    .lock()
    ...
    let expected = RecordedTurnError {
        ...
        turn_level_id: turn_context.sub_id.clone(),
        turn_id: turn_context.sub_id.clone(),
        error: OrbiterXErrorInfo::UsageLimitExceeded,
        saw_session_store: true,
        saw_thread_store: true,
    };

    session
        .emit_turn_error_lifecycle(&turn_context, OrbiterXErrorInfo::UsageLimitExceeded)
        .await;
...
async fn config_change_contributor_observes_effective_config_changes() {
    ...
    impl orbiterx_extension_api::ConfigContributor<crate::config::Config> for ConfigRecorder {
        fn on_config_changed(
            &self,
            session_store: &orbiterx_extension_api::ExtensionData,
            thread_store: &orbiterx_extension_api::ExtensionData,
...
async fn session_configured_reports_permission_profile_for_external_sandbox() -> anyhow::Result<()>
{
    ...
    let sandbox_policy = SandboxPolicy::ExternalSandbox {
        network_access: orbiterx_protocol::protocol::NetworkAccess::Restricted,
    };
...
async fn fork_startup_context_then_first_turn_diff_snapshot() -> anyhow::Result<()> {
    ...
    let mut builder = test_orbiterx().with_config(|config| {
        config.permissions.approval_policy =
            orbiterx_config::Constrained::allow_any(AskForApproval::OnRequest);
    });
    ...
    fork_config.permissions.approval_policy =
        orbiterx_config::Constrained::allow_any(AskForApproval::UnlessTrusted);
    let forked = initial
        .thread_manager
        .fork_thread(
            usize::MAX,
    ...
    settings.bind(|| {
        insta::assert_snapshot!(
            "orbiterx_core__orbiterx_tests__fork_startup_context_then_first_turn_diff",
            snapshot
        );
    });
...
async fn record_initial_history_forked_hydrates_previous_turn_settings() {
    ...
    let previous_context_item = TurnContextItem {
        ...
        summary: orbiterx_protocol::config_types::ReasoningSummary::Auto,
    };
    ...
    let rollout_items = vec![
        RolloutItem::EventMsg(EventMsg::TurnStarted(
            orbiterx_protocol::protocol::TurnStartedEvent {
        ...
        RolloutItem::EventMsg(EventMsg::UserMessage(
            orbiterx_protocol::protocol::UserMessageEvent {
                client_id: None,
        ...
        RolloutItem::EventMsg(EventMsg::TurnComplete(
            orbiterx_protocol::protocol::TurnCompleteEvent {
                turn_id,
...
async fn thread_rollback_fails_without_persisted_thread_history() {
    ...
    assert_eq!(
        error_event.orbiterx_error_info,
        Some(OrbiterXErrorInfo::ThreadRollbackFailed)
    );
...
async fn set_rate_limits_retains_previous_credits() {
    let orbiterx_home = tempfile::tempdir().expect("create temp dir");
    let config = build_test_config(orbiterx_home.path()).await;
    ...
    let initial = RateLimitSnapshot {
        ...
        plan_type: Some(orbiterx_protocol::account::PlanType::Plus),
        rate_limit_reached_type: None,
    };
    state.set_rate_limits(initial.clone());

    let update = RateLimitSnapshot {
        limit_id: Some("orbiterx_other".to_string()),
        limit_name: Some("orbiterx_other".to_string()),
        primary: Some(RateLimitWindow {
            used_percent: 40.0,
...
async fn set_rate_limits_updates_plan_type_when_present() {
    ...
    let update = RateLimitSnapshot {
        ...
        individual_limit: None,
        spend_control_reached: None,
        plan_type: Some(orbiterx_protocol::account::PlanType::Pro),
        rate_limit_reached_type: None,
    };
    state.set_rate_limits(update.clone());
    ...
    assert_eq!(
        state.latest_rate_limits,
        Some(RateLimitSnapshot {
            limit_id: Some("orbiterx".to_string()),
            limit_name: None,
    ...
    loop {
        ...
        match evt.msg {
            EventMsg::Error(payload)
                if payload.orbiterx_error_info == Some(OrbiterXErrorInfo::ThreadRollbackFailed) =>
            {
                return payload;
            }
            _ => continue,
...
async fn build_test_config(orbiterx_home: &Path) -> Config {
...
async fn emit_subagent_session_started_includes_fork_lineage_and_originator() {
    ...
    Mock::given(method("POST"))
        .and(path("/orbiterx/analytics-events/events"))
        .respond_with(ResponseTemplate::new(200))
        .mount(&server)
        .await;

    let auth_manager =
        AuthManager::from_auth_for_testing(OrbiterXAuth::create_dummy_chatgpt_auth_for_testing());
    ...
    emit_subagent_session_started(
        ...
        AppServerClientMetadata {
            client_name: Some("orbiterx-tui".to_string()),
            ...
            if let Some(requests) = server.received_requests().await {
                for request in requests {
                    ...
                    if let Some(event) = payload["events"].as_array().and_then(|events| {
                        events
                            .iter()
                            .find(|event| event["event_type"] == "orbiterx_thread_initialized")
                    }) {
...
async fn session_configuration_apply_permission_profile_preserves_existing_deny_read_entries() {
    ...
    let permission_profile = orbiterx_protocol::models::PermissionProfile::from_runtime_permissions(
...
async fn session_configuration_apply_permission_profile_accepts_direct_write_roots() {
    ...
    let external_write_path = AbsolutePathBuf::from_absolute_path(
        orbiterx_utils_absolute_path::canonicalize_preserving_symlinks(external_write_dir.path())
...
async fn active_profile_update_rebuilds_network_proxy_config() -> std::io::Result<()> {
    let orbiterx_home = tempfile::tempdir().expect("create orbiterx home");
    let cwd = tempfile::tempdir().expect("create cwd");
    ...
    std::fs::write(
        orbiterx_home.path().join(orbiterx_config::CONFIG_TOML_FILE),
...
async fn new_default_turn_uses_config_aware_skills_for_role_overrides() {
    ...
    let orbiterx_home = parent_config.orbiterx_home.clone();
    let skill_dir = orbiterx_home.join("skills").join("demo");
    ...
    let skill_fs = session
        ...
        .unwrap_or_else(|| std::sync::Arc::clone(&orbiterx_exec_server::LOCAL_FS));
    ...
    let role_path = orbiterx_home.join("skills-role.toml");
    std::fs::write(
        &role_path,
        format!(
...
async fn session_new_fails_when_zsh_fork_enabled_without_packaged_zsh() {
    let orbiterx_home = tempfile::tempdir().expect("create temp dir");
    let mut config = build_test_config(orbiterx_home.path()).await;
    config
        .features
    ...
    let auth_manager =
        AuthManager::from_auth_for_testing(OrbiterXAuth::from_api_key("Test API Key"));
    ...
    let plugins_manager = Arc::new(PluginsManager::new(config.orbiterx_home.to_path_buf()));
    ...
    let result = Session::new(
        ...
        plugins_manager,
        mcp_manager,
        Arc::new(orbiterx_code_mode::InProcessCodeModeSessionProvider),
        Arc::new(orbiterx_extension_api::ExtensionRegistryBuilder::new().build()),
        orbiterx_extension_api::ExtensionDataInit::default(),
        /*supports_openai_form_elicitation*/ false,
        AgentControl::default(),
        environment_manager,
        /*inherited_environments*/ None,
        /*analytics_events_client*/ None,
        Arc::new(orbiterx_thread_store::LocalThreadStore::new(
            orbiterx_thread_store::LocalThreadStoreConfig::from_config(config.as_ref()),
            /*state_db*/ None,
        )),
        orbiterx_rollout_trace::ThreadTraceContext::disabled(),
        /*attestation_provider*/ None,
...
pub(crate) async fn make_session_and_context() -> (Session, TurnContext) {
    ...
    let mcp_runtime = Arc::new(orbiterx_mcp::McpRuntime::new(
    ...
    let services = SessionServices {
        ...
        rollout_thread_trace: orbiterx_rollout_trace::ThreadTraceContext::disabled(),
        ...
        plugins_manager,
        mcp_manager,
        extensions: Arc::new(orbiterx_extension_api::ExtensionRegistryBuilder::new().build()),
        session_extension_data: orbiterx_extension_api::ExtensionData::new(
            agent_control.session_id().to_string(),
        ),
        thread_extension_data: orbiterx_extension_api::ExtensionData::new(thread_id.to_string()),
        selected_capability_roots: Vec::new(),
        mcp_thread_init: orbiterx_extension_api::ExtensionDataInit::default(),
        ...
        state_db: None,
        live_thread: None,
        thread_store: Arc::new(orbiterx_thread_store::LocalThreadStore::new(
            orbiterx_thread_store::LocalThreadStoreConfig::from_config(config.as_ref()),
            /*state_db*/ None,
        )),
        attestation_provider: None,
...
async fn load_latest_config_for_session(session: &Session) -> Config {
    let config = session.get_config().await;
    ConfigBuilder::default()
        .orbiterx_home(config.orbiterx_home.to_path_buf())
...
async fn make_session_with_history_source_and_agent_control_and_rx(
    ...
    let session = Session::new(
        ...
        Arc::new(orbiterx_thread_store::LocalThreadStore::new(
            orbiterx_thread_store::LocalThreadStoreConfig::from_config(config.as_ref()),
            Some(
                orbiterx_state::StateRuntime::init(
    ...
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    session
        .notify_request_permissions_response(
            "missing",
            orbiterx_protocol::request_permissions::RequestPermissionsResponse {
                permissions: RequestPermissionProfile {
                    network: Some(orbiterx_protocol::models::NetworkPermissions {
                        enabled: Some(true),
                    }),
    ...
    assert_eq!(
        session
            .granted_turn_permissions(orbiterx_exec_server::LOCAL_ENVIRONMENT_ID)
            .await,
        None
    );
}
    ...
    *session.active_turn.lock().await = Some(current_active_turn);
    ...
    session
        .record_granted_request_permissions_for_turn(
            ...
            orbiterx_exec_server::LOCAL_ENVIRONMENT_ID,
    ...
    assert_eq!(
        originating_turn_state
            .lock()
            .await
            .granted_permissions(orbiterx_exec_server::LOCAL_ENVIRONMENT_ID),
...
async fn request_permissions_emits_event_when_granular_policy_allows_requests() {
    ...
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    let expected_response = orbiterx_protocol::request_permissions::RequestPermissionsResponse {
        permissions: RequestPermissionProfile {
            network: Some(orbiterx_protocol::models::NetworkPermissions {
                enabled: Some(true),
            }),
        ...
        async move {
            ...
            session
                .request_permissions_for_environment(
                    &turn_context,
                    call_id,
                    orbiterx_protocol::request_permissions::RequestPermissionsArgs {
                        environment_id: None,
    ...
    assert_eq!(
        ...
        Some(orbiterx_exec_server::LOCAL_ENVIRONMENT_ID)
    );
    #[allow(deprecated)]
    let turn_cwd = turn_context.cwd.clone();
        ...
        async move {
            handler
                .handle(ToolInvocation {
                    ...
                    tracker,
                    call_id,
                    tool_name: orbiterx_tools::ToolName::plain("request_permissions"),
...
async fn request_permissions_response_materializes_session_cwd_grants_before_recording() {
    ...
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    assert_eq!(
        session
            .granted_session_permissions(orbiterx_exec_server::LOCAL_ENVIRONMENT_ID)
            .await,
...
async fn user_turn_updates_approvals_reviewer() {
    ...
    handlers::user_input_or_turn(
        ...
        Op::UserInput {
            ...
            thread_settings: orbiterx_protocol::protocol::ThreadSettingsOverrides {
                ...
                approvals_reviewer: Some(orbiterx_config::types::ApprovalsReviewer::AutoReview),
    ...
    assert_eq!(
        ...
        orbiterx_config::types::ApprovalsReviewer::AutoReview
    );
}
...
async fn primary_environment_uses_first_turn_environment() {
    ...
    let second_cwd_uri = orbiterx_utils_path_uri::PathUri::from_abs_path(&second_cwd);
    turn_context
        .environments
        .environments
...
async fn spawn_task_turn_span_inherits_dispatch_trace_context() {
    ...
    let submission_context =
        orbiterx_otel::context_from_w3c_trace_context(&submission_trace).expect("submission");
    let task_context =
        orbiterx_otel::context_from_w3c_trace_context(&task_trace).expect("task trace");
...
async fn shutdown_complete_does_not_append_to_thread_store_after_shutdown() {
    let (mut session, _turn_context) = make_session_and_context().await;
    let store = Arc::new(orbiterx_thread_store::InMemoryThreadStore::default());
    let thread_store: Arc<dyn orbiterx_thread_store::ThreadStore> = store.clone();
    let config = session.get_config().await;
    let live_thread = LiveThread::create(
    ...
    assert_eq!(
        orbiterx_thread_store::InMemoryThreadStoreCalls {
            create_thread: 1,
            shutdown_thread: 1,
            ..Default::default()
        },
...
async fn submission_loop_channel_close_runs_full_thread_teardown() {
    ...
    impl orbiterx_extension_api::ThreadLifecycleContributor<crate::config::Config>
        for ThreadStopRecorder
    {
        fn on_thread_stop<'a>(
            &'a self,
            input: orbiterx_extension_api::ThreadStopInput<'a>,
        ) -> orbiterx_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
                assert_eq!(
...
async fn submission_loop_channel_close_aborts_active_turn_before_thread_stop_lifecycle() {
    ...
    impl orbiterx_extension_api::TurnLifecycleContributor for LifecycleRecorder {
        fn on_turn_abort<'a>(
            &'a self,
            input: orbiterx_extension_api::TurnAbortInput<'a>,
        ) -> orbiterx_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
                assert_eq!(
                    self.expected_thread_id.to_string(),
...
{
    let (tx_event, rx_event) = async_channel::unbounded();
    let mut config = build_test_config(orbiterx_home).await;
...
async fn refresh_mcp_servers_keeps_the_previous_runtime_alive() {
    ...
    assert!(Arc::ptr_eq(&step_context.mcp, &old_runtime));
    assert_eq!(
        orbiterx_mcp::configured_mcp_servers(new_runtime.config()),
...
impl orbiterx_exec_server::NoiseRendezvousConnectProvider for PendingNoiseConnectProvider {
    fn connect_bundle(
        &self,
        _: orbiterx_exec_server::NoiseChannelPublicKey,
    ) -> futures::future::BoxFuture<
        '_,
        Result<
            orbiterx_exec_server::NoiseRendezvousConnectBundle,
            orbiterx_exec_server::ExecServerError,
        >,
    > {
...
async fn deferred_environment_roots_refresh_plugin_availability() {
    ...
    impl orbiterx_extension_api::McpServerContributor<Config> for ReadyPluginContributor {
        ...
        fn contribute<'a>(
            &'a self,
            context: orbiterx_extension_api::McpServerContributionContext<'a, Config>,
        ) -> orbiterx_extension_api::ExtensionFuture<
            'a,
            Vec<orbiterx_extension_api::McpServerContribution>,
        > {
            Box::pin(async move {
                ...
                available
                    .then(
                        || orbiterx_extension_api::McpServerContribution::SelectedPluginPackage {
    ...
    let (mut session, turn_context) = make_session_and_context().await;
    let mut registry = orbiterx_extension_api::ExtensionRegistryBuilder::new();
    ...
    session.services.mcp_manager = Arc::new(McpManager::new_with_extensions(
        ...
        registry,
        crate::OrbiterXAppsToolsCache::default(),
    ));
    let session = Arc::new(session);
    session
    ...
    let selected_root = orbiterx_protocol::capabilities::SelectedCapabilityRoot {
        id: "skill-only".to_string(),
        location: orbiterx_protocol::capabilities::CapabilityRootLocation::Environment {
    ...
    registration
        .complete(Ok(orbiterx_exec_server::EnvironmentReadyInfo {
...
async fn conflicting_ready_environment_root_ids_keep_first_location() {
    ...
    let selected_root = |environment_id: &str, path: &str| {
        orbiterx_protocol::capabilities::SelectedCapabilityRoot {
    ...
    for selected_root in &selected_roots {
        let orbiterx_protocol::capabilities::CapabilityRootLocation::Environment {
            environment_id,
            ..
...
impl orbiterx_extension_api::ContextContributor for PromptExtensionTestContributor {
    fn contribute_thread_context<'a>(
        &'a self,
        _session_store: &'a orbiterx_extension_api::ExtensionData,
        thread_store: &'a orbiterx_extension_api::ExtensionData,
    ) -> std::pin::Pin<
        Box<
            dyn std::future::Future<Output = Vec<orbiterx_extension_api::PromptFragment>>
                + Send
                + 'a,
        >,
    > {
        Box::pin(async move {
            thread_store
                ...
                .is_some()
                .then(|| {
                    orbiterx_extension_api::PromptFragment::developer_policy(
...
-> Arc<orbiterx_extension_api::ExtensionRegistry<crate::config::Config>> {
    let mut builder = orbiterx_extension_api::ExtensionRegistryBuilder::new();
    builder.prompt_contributor(Arc::new(PromptExtensionTestContributor));
    Arc::new(builder.build())
}

impl orbiterx_extension_api::ContextContributor for TurnContextExtensionTestContributor {
    fn contribute_turn_context<'a>(
        &'a self,
        input: orbiterx_extension_api::TurnContextContributionInput<'a>,
    ) -> std::pin::Pin<
        Box<
            dyn std::future::Future<Output = Vec<orbiterx_extension_api::PromptFragment>>
                + Send
...
async fn attach_in_memory_thread_store(
    session: &mut Session,
) -> Arc<orbiterx_thread_store::InMemoryThreadStore> {
    let store = Arc::new(orbiterx_thread_store::InMemoryThreadStore::default());
    let thread_store: Arc<dyn orbiterx_thread_store::ThreadStore> = store.clone();
    let config = session.get_config().await;
    let live_thread = LiveThread::create(
...
async fn wait_for_flush_count(
    store: &orbiterx_thread_store::InMemoryThreadStore,
...
async fn guardian_auto_review_emits_thread_idle_after_interrupt() {
        ...
        for ThreadIdleRecorder
    {
        fn on_thread_idle<'a>(
            &'a self,
            _input: orbiterx_extension_api::ThreadIdleInput<'a>,
        ) -> orbiterx_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
...
async fn task_finish_emits_turn_item_lifecycle_for_leftover_pending_user_input() {
    ...
    let text_element = orbiterx_protocol::user_input::TextElement::new(
        orbiterx_protocol::user_input::ByteRange { start: 5, end: 12 },
...
async fn task_finish_emits_thread_idle_lifecycle_after_active_turn_clears() {
        ...
        for ThreadIdleRecorder
    {
        fn on_thread_idle<'a>(
            &'a self,
            input: orbiterx_extension_api::ThreadIdleInput<'a>,
        ) -> orbiterx_extension_api::ExtensionFuture<'a, ()> {
            Box::pin(async move {
                assert_eq!(
                    self.expected_thread_id.to_string(),
...
async fn tool_calls_reopen_mailbox_delivery_for_current_turn() {
    ...
    let mut ctx = HandleOutputCtx {
        ...
        turn_store: Arc::new(orbiterx_extension_api::ExtensionData::new(
            tc.sub_id.clone(),
        )),
...
async fn rejects_escalated_permissions_when_policy_not_on_request() {
    ...
    use crate::turn_diff_tracker::TurnDiffTracker;
    use orbiterx_protocol::protocol::AskForApproval;
    use orbiterx_tools::ShellCommandBackendConfig;
    ...
    let resp = handler
        .handle(ToolInvocation {
            ...
            call_id,
            tool_name: orbiterx_tools::ToolName::plain(tool_name),
...
async fn unified_exec_rejects_escalated_permissions_when_policy_not_on_request() {
    ...
    let resp = handler
        .handle(ToolInvocation {
            ...
            tool_name: orbiterx_tools::ToolName::plain("exec_command"),
            source: crate::tools::context::ToolCallSource::Direct,
            payload: ToolPayload::Function {
                arguments: serde_json::json!({
                    "cmd": "echo hi",
...
async fn session_start_hooks_only_load_from_trusted_project_layers() -> std::io::Result<()> {
    ...
    let orbiterx_home = temp.path().join("home");
    let project_root = temp.path().join("project");
    let nested = project_root.join("nested");
    let root_dot_orbiterx = project_root.join(".orbiterx");
    let nested_dot_orbiterx = nested.join(".orbiterx");

    std::fs::create_dir_all(&orbiterx_home)?;
    std::fs::create_dir_all(&nested_dot_orbiterx)?;
    std::fs::write(project_root.join(".git"), "gitdir: here")?;
    write_project_hooks(&root_dot_orbiterx)?;
    write_project_hooks(&nested_dot_orbiterx)?;
    write_project_trust_config(&orbiterx_home, &[(&nested, TrustLevel::Trusted)]).await?;

    let config = ConfigBuilder::default()
        .orbiterx_home(orbiterx_home)
    ...
    let expected_source_path = orbiterx_utils_absolute_path::AbsolutePathBuf::from_absolute_path(
        nested_dot_orbiterx.join("hooks.json"),
    )?;
    assert_eq!(
        hook_list
            .hooks
...
async fn session_start_hooks_require_project_trust_without_config_toml() -> std::io::Result<()> {
    ...
    let project_root = temp.path().join("project");
    let nested = project_root.join("nested");
    let dot_orbiterx = project_root.join(".orbiterx");
    std::fs::create_dir_all(&nested)?;
    std::fs::write(project_root.join(".git"), "gitdir: here")?;
    write_project_hooks(&dot_orbiterx)?;
    ...
    for (name, trust_entries, expected_hooks) in cases {
        let orbiterx_home = temp.path().join(format!("home_{name}"));
        std::fs::create_dir_all(&orbiterx_home)?;
        write_project_trust_config(&orbiterx_home, &trust_entries).await?;
