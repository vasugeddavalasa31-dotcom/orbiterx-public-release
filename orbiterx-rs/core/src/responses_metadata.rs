...
use orbiterx_analytics::CompactionStrategy;
use orbiterx_analytics::CompactionTrigger;
...
use orbiterx_utils_string::to_ascii_json_string;
use serde::Serialize;
use serde_json::Value;

use crate::client::X_OPENAI_SUBAGENT_HEADER;
use crate::client::X_ORBITERX_INSTALLATION_ID_HEADER;
use crate::client::X_ORBITERX_PARENT_THREAD_ID_HEADER;
use crate::client::X_ORBITERX_TURN_METADATA_HEADER;
use crate::client::X_ORBITERX_WINDOW_ID_HEADER;
...
const RESERVED_METADATA_KEYS: &[&str] = &[
    ...
    X_ORBITERX_TURN_METADATA_HEADER,
...
pub(crate) enum OrbiterXResponsesRequestKind {
    Turn,
    Prewarm,
...
impl OrbiterXResponsesRequestKind {
    fn metadata(self) -> (&'static str, Option<CompactionTurnMetadata>) {
        match self {
            OrbiterXResponsesRequestKind::Turn => ("turn", None),
            OrbiterXResponsesRequestKind::Prewarm => ("prewarm", None),
            OrbiterXResponsesRequestKind::Compaction(metadata) => ("compaction", Some(metadata)),
            OrbiterXResponsesRequestKind::Memory => ("memory", None),
        }
    }

    fn has_turn_identity(self) -> bool {
        !matches!(self, OrbiterXResponsesRequestKind::Memory)
    }
}
...
/// Caller-owned snapshot of OrbiterX metadata sent to ResponsesAPI.
///
/// The full OrbiterX turn metadata blob is transported canonically as
/// `client_metadata["x-orbiterx-turn-metadata"]`. Flat `client_metadata` keys and direct HTTP/ws
/// headers are generated compatibility projections of this snapshot, not separate sources of
/// truth.
#[derive(Clone, Debug)]
pub struct OrbiterXResponsesMetadata {
    pub(crate) installation_id: String,
    pub(crate) session_id: String,
    pub(crate) thread_id: String,
    pub(crate) turn_id: Option<String>,
    pub(crate) window_id: String,
    pub(crate) request_kind: Option<OrbiterXResponsesRequestKind>,
...
impl OrbiterXResponsesMetadata {
    pub(crate) fn new(
    ...
    pub(crate) fn client_metadata(&self) -> HashMap<String, String> {
        ...
        if let Some(parent_thread_id) = self.parent_thread_id {
            client_metadata.insert(
                X_ORBITERX_PARENT_THREAD_ID_HEADER.to_string(),
        ...
        {
            client_metadata.insert(
                X_ORBITERX_TURN_METADATA_HEADER.to_string(),
                turn_metadata_json,
            );
        }
        client_metadata
    ...
    pub(crate) fn compatibility_headers(&self) -> ApiHeaderMap {
        ...
        insert_header(&mut headers, X_ORBITERX_WINDOW_ID_HEADER, &self.window_id);
        // Direct x-orbiterx-turn-metadata is compatibility output. New per-request consumers should
        // prefer client_metadata["x-orbiterx-turn-metadata"], which is rendered from this same object.
        if self.has_turn_metadata()
            && let Some(turn_metadata_json) = self.turn_metadata_json()
        {
            insert_header(
    ...
    fn turn_metadata_payload(&self) -> OrbiterXTurnMetadataPayload<'_> {
        ...
        let has_turn_identity =
            request_kind.is_none_or(OrbiterXResponsesRequestKind::has_turn_identity);
        let has_request_identity =
            request_kind.is_some_and(OrbiterXResponsesRequestKind::has_turn_identity);
        OrbiterXTurnMetadataPayload {
            ...
            compaction,
            // responsesapi_client_metadata enriches the OrbiterX turn metadata blob, not literal
            // top-level Responses client_metadata. Reserved OrbiterX-owned keys are filtered when
...
#[derive(Serialize)]
struct OrbiterXTurnMetadataPayload<'a> {
    #[serde(default, skip_serializing_if = "Option::is_none")]
