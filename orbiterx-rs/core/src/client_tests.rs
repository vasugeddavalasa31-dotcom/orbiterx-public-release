...
use super::X_ORBITERX_INSTALLATION_ID_HEADER;
use super::X_ORBITERX_PARENT_THREAD_ID_HEADER;
use super::X_ORBITERX_TURN_METADATA_HEADER;
use super::X_ORBITERX_WINDOW_ID_HEADER;
use crate::AttestationContext;
use crate::AttestationProvider;
use crate::GenerateAttestationFuture;
use crate::responses_metadata::OrbiterXResponsesMetadata;
use crate::test_support::TestOrbiterXResponsesRequestKind;
...
use orbiterx_model_provider::BearerAuthProvider;
use orbiterx_model_provider::SharedModelProvider;
use orbiterx_model_provider::create_model_provider;
use orbiterx_model_provider_info::CHATGPT_ORBITERX_BASE_URL;
...
use orbiterx_rollout_trace::ExecutionStatus;
...
use orbiterx_rollout_trace::RolloutTrace;
...
async fn compact_uses_bearer_after_agent_identity_session_fallback() -> anyhow::Result<()> {
    ...
    let auth_manager = chatgpt_auth_manager(&orbiterx_home, server.uri()).await;
...
fn write_chatgpt_auth_json(orbiterx_home: &std::path::Path) {
    let auth_json = json!({
        "tokens": {
...
async fn chatgpt_auth_manager(
    ...
    write_chatgpt_auth_json(orbiterx_home.path());
...
fn started_inference_attempt(temp: &TempDir) -> anyhow::Result<InferenceTraceAttempt> {
    ...
    writer.append(RawTraceEventPayload::OrbiterXTurnStarted {
...
fn output_message(id: &str, text: &str) -> ResponseItem {
    ResponseItem::Message {
        id: Some(orbiterx_protocol::ResponseItemId::with_suffix("msg", id)),
...
fn build_ws_client_metadata_includes_window_lineage_and_turn_metadata() {
    ...
    for (client_key, metadata_key, expected) in [
        ...
        (
            X_ORBITERX_WINDOW_ID_HEADER,
            "window_id",
            expected_window_id.as_str(),
        ),
        (
            X_ORBITERX_PARENT_THREAD_ID_HEADER,
            "parent_thread_id",
...
async fn bedrock_unauthorized_error_uses_provider_mapping() {
    ...
    assert_eq!(
        error.to_string(),
        format!(
            "Amazon Bedrock rejected the request because its AWS signature has expired. Refresh your AWS credentials and retry. If `AWS_BEARER_TOKEN_BEDROCK` is set, update or unset it, then restart OrbiterX, url: {url}"
        )
    );
}
...
fn model_client_with_counting_attestation(
    ...
    let (auth_manager, provider) = if include_attestation {
        (
            ...
            ModelProviderInfo::create_openai_provider(Some(CHATGPT_ORBITERX_BASE_URL.to_string())),
        )
    } else {
        (
            None,
...
#[tokio::test]
async fn websocket_handshake_includes_attestation_for_chatgpt_orbiterx_responses() {
...
#[tokio::test]
async fn non_chatgpt_orbiterx_endpoints_omit_attestation_generation() {
    let (model_client, attestation_calls) =
