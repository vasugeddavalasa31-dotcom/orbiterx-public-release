...
use core_test_support::test_orbiterx::TestOrbiterXBuilder;
use core_test_support::test_orbiterx::TestOrbiterXHarness;
use core_test_support::test_orbiterx::test_orbiterx as base_test_orbiterx;
use core_test_support::test_path_buf;
use core_test_support::wait_for_event;
use core_test_support::wait_for_event_match;
use core_test_support::wait_for_event_with_timeout;
use orbiterx_core::compact::SUMMARY_PREFIX;
use orbiterx_features::Feature;
use orbiterx_login::OrbiterXAuth;
use orbiterx_login::auth::AgentIdentityAuth;
use orbiterx_login::auth::AgentIdentityAuthRecord;
use orbiterx_protocol::account::PlanType as AccountPlanType;
use orbiterx_protocol::config_types::ServiceTier;
use orbiterx_protocol::dynamic_tools::DynamicToolFunctionSpec;
use orbiterx_protocol::dynamic_tools::DynamicToolNamespaceSpec;
use orbiterx_protocol::dynamic_tools::DynamicToolNamespaceTool;
...
use orbiterx_protocol::protocol::RealtimeConversationRealtimeEvent;
use orbiterx_protocol::protocol::RealtimeEvent;
use orbiterx_protocol::protocol::RealtimeOutputModality;
...
const TURN_STATE_HEADER: &str = "x-orbiterx-turn-state";
...
fn test_orbiterx() -> TestOrbiterXBuilder {
    base_test_orbiterx().with_config(|config| {
        let _ = config.features.disable(Feature::RemoteCompactionV2);
    })
}

fn remote_realtime_test_orbiterx_builder(
    realtime_server: &responses::WebSocketTestServer,
) -> TestOrbiterXBuilder {
    let realtime_base_url = realtime_server.uri().to_string();
    test_orbiterx()
        .with_auth(OrbiterXAuth::from_api_key("dummy"))
...
async fn start_realtime_conversation(orbiterx: &orbiterx_core::OrbiterXThread) -> Result<()> {
    orbiterx
        .submit(Op::RealtimeConversationStart(ConversationStartParams {
            client_managed_handoffs: false,
            flush_transcript_tail_on_session_end: false,
            orbiterx_responses_as_items: false,
            orbiterx_response_item_prefix: None,
            orbiterx_response_handoff_mode:
                orbiterx_protocol::protocol::OrbiterXResponseHandoffMode::Thinking,
            model: None,
    ...
    wait_for_event_match(orbiterx, |msg| match msg {
...
async fn close_realtime_conversation(orbiterx: &orbiterx_core::OrbiterXThread) -> Result<()> {
    orbiterx.submit(Op::RealtimeConversationClose).await?;
    wait_for_event_match(orbiterx, |msg| match msg {
...
async fn wait_for_turn_complete(orbiterx: &orbiterx_core::OrbiterXThread) {
...
async fn remote_compact_replaces_history_for_followups() -> Result<()> {
    ...
    let harness = TestOrbiterXHarness::with_builder(
        test_orbiterx().with_auth(OrbiterXAuth::create_dummy_chatgpt_auth_for_testing()),
    )
    .await?;
    let orbiterx = harness.test().orbiterx.clone();
    ...
    orbiterx
        ...
        .await?;
    wait_for_turn_complete(&orbiterx).await;

    orbiterx.submit(Op::Compact).await?;
    wait_for_turn_complete(&orbiterx).await;
    ...
    assert_eq!(
        ...
        compact_request.header("x-orbiterx-window-id").as_deref()
    );
    assert_eq!(
    ...
    assert_eq!(
        ...
        follow_up_request.header("x-orbiterx-window-id").as_deref()
    );
    assert_ne!(
...
async fn remote_compact_uses_agent_identity_assertion() -> Result<()> {
    ...
    let harness = TestOrbiterXHarness::with_builder(
        test_orbiterx().with_auth(OrbiterXAuth::AgentIdentity(
...
async fn assert_remote_manual_compact_request_parity(
    ...
    let mut builder = test_orbiterx().with_auth(auth);
    ...
    let harness = TestOrbiterXHarness::with_builder(builder).await?;
    let orbiterx = harness.test().orbiterx.clone();
    let image_url =
...
async fn remote_compact_v2_reuses_compaction_trigger_for_followups() -> Result<()> {
    ...
    assert!(
        compact_request
            .header("x-orbiterx-beta-features")
            .as_deref()
    ...
    assert_eq!(
        compact_request.body_json()["client_metadata"]["x-orbiterx-window-id"].as_str(),
        compact_metadata["window_id"].as_str()
    );
    assert_eq!(
        compact_metadata["compaction"],
...
async fn remote_compact_filters_deferred_dynamic_tools() -> Result<()> {
    ...
    let dynamic_tools = vec![DynamicToolSpec::Namespace(DynamicToolNamespaceSpec {
        name: "orbiterx_app".to_string(),
        description: "OrbiterX app tools.".to_string(),
        tools: vec![
    ...
    let new_thread = test
        ...
        .await?;
    test.orbiterx = new_thread.thread;
    ...
    assert_eq!(
        namespace_child_tool_names(&first_response_body, "orbiterx_app"),
        vec![visible_tool.to_string()]
    );
    assert_eq!(
        namespace_child_tool_names(&compact_body, "orbiterx_app"),
        vec![visible_tool.to_string()]
    );
...
async fn remote_compact_runs_automatically() -> Result<()> {
    ...
    let message = wait_for_event_match(&orbiterx, |event| match event {
...
async fn auto_remote_compact_failure_stops_agent_loop() -> Result<()> {
    ...
    let error_message = wait_for_event_match(&orbiterx, |event| match event {
...
async fn remote_compact_trim_estimate_uses_session_base_instructions() -> Result<()> {
    ...
    let baseline_harness = TestOrbiterXHarness::with_builder(
        test_orbiterx()
            .with_auth(OrbiterXAuth::create_dummy_chatgpt_auth_for_testing())
            .with_config(|config| {
    ...
    .await?;
    let baseline_orbiterx = baseline_harness.test().orbiterx.clone();
    ...
    baseline_orbiterx
        ...
        .await?;
    wait_for_event(&baseline_orbiterx, |event| {
        matches!(event, EventMsg::TurnComplete(_))
    })
    .await;
    ...
    baseline_orbiterx.submit(Op::Compact).await?;
    wait_for_event(&baseline_orbiterx, |event| {
        matches!(event, EventMsg::TurnComplete(_))
    })
    .await;
    ...
    let override_harness = TestOrbiterXHarness::with_builder(
        test_orbiterx()
            .with_auth(OrbiterXAuth::create_dummy_chatgpt_auth_for_testing())
            .with_config({
    ...
    .await?;
    let override_orbiterx = override_harness.test().orbiterx.clone();
    ...
    override_orbiterx
        ...
        .await?;
    wait_for_event(&override_orbiterx, |event| {
        matches!(event, EventMsg::TurnComplete(_))
    })
    .await;
    ...
    override_orbiterx.submit(Op::Compact).await?;
    wait_for_event(&override_orbiterx, |event| {
        matches!(event, EventMsg::TurnComplete(_))
    })
    .await;
...
async fn remote_manual_compact_emits_context_compaction_items() -> Result<()> {
    ...
    {
        let event = orbiterx.next_event().await.unwrap();
        match event.msg {
...
async fn remote_compact_persists_replacement_history_in_rollout() -> Result<()> {
    ...
    orbiterx.submit(Op::Shutdown).await?;
    wait_for_event(&orbiterx, |ev| matches!(ev, EventMsg::ShutdownComplete)).await;
...
async fn remote_compact_and_resume_refresh_stale_developer_instructions() -> Result<()> {
    ...
    initial.orbiterx.submit(Op::Compact).await?;
    wait_for_event(&initial.orbiterx, |ev| {
        matches!(ev, EventMsg::TurnComplete(_))
    })
    .await;
    ...
    resumed
        ...
        .await?;
    wait_for_event(&resumed.orbiterx, |ev| {
        matches!(ev, EventMsg::TurnComplete(_))
    })
    .await;
...
async fn remote_compact_refreshes_stale_developer_instructions_without_resume() -> Result<()> {
    ...
    test.orbiterx
        ...
        .await?;
    wait_for_event(&test.orbiterx, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
...
async fn snapshot_request_shape_remote_pre_turn_compaction_restates_realtime_start() -> Result<()> {
    ...
    let mut builder =
        remote_realtime_test_orbiterx_builder(&realtime_server).with_config(|config| {
    ...
    start_realtime_conversation(test.orbiterx.as_ref()).await?;
    ...
    close_realtime_conversation(test.orbiterx.as_ref()).await?;
...
async fn remote_request_uses_custom_experimental_realtime_start_instructions() -> Result<()> {
    ...
    let mut builder = remote_realtime_test_orbiterx_builder(&realtime_server).with_config({
...
async fn active_realtime_does_not_diff_changed_start_instructions_after_resume() -> Result<()> {
    ...
    let mut initial_builder = remote_realtime_test_orbiterx_builder(&initial_realtime_server)
        .with_config({
    ...
    start_realtime_conversation(initial.orbiterx.as_ref()).await?;
    initial.submit_turn("USER_ONE").await?;
    close_realtime_conversation(initial.orbiterx.as_ref()).await?;
    initial.orbiterx.submit(Op::Shutdown).await?;
    wait_for_event(&initial.orbiterx, |ev| {
        matches!(ev, EventMsg::ShutdownComplete)
    })
    ...
    let mut resume_builder = remote_realtime_test_orbiterx_builder(&resumed_realtime_server)
        .with_config({
    ...
    start_realtime_conversation(resumed.orbiterx.as_ref()).await?;
    ...
    close_realtime_conversation(resumed.orbiterx.as_ref()).await?;
...
async fn snapshot_request_shape_remote_manual_compact_restates_realtime_start() -> Result<()> {
    ...
    let server = wiremock::MockServer::start().await;
    let realtime_server = start_remote_realtime_server().await;
    let mut builder = remote_realtime_test_orbiterx_builder(&realtime_server);
    let test = builder.build(&server).await?;
    ...
    for user in ["USER_ONE", "USER_TWO", "USER_THREE"] {
        if user == "USER_THREE" {
            core_test_support::submit_thread_settings(
                &orbiterx,
                orbiterx_protocol::protocol::ThreadSettingsOverrides {
...
async fn remote_mid_turn_compact_v2_sends_turn_state_over_websocket() -> Result<()> {
    ...
    test.orbiterx
        ...
        .await?;
    wait_for_turn_complete(&test.orbiterx).await;
