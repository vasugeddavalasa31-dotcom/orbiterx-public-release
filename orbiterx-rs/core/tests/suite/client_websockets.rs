...
use orbiterx_api::WS_REQUEST_HEADER_TRACEPARENT_CLIENT_METADATA_KEY;
use orbiterx_api::WS_REQUEST_HEADER_TRACESTATE_CLIENT_METADATA_KEY;
use orbiterx_core::ModelClient;
use orbiterx_core::ModelClientSession;
use orbiterx_core::OrbiterXResponsesMetadata;
use orbiterx_core::Prompt;
use orbiterx_core::ResponseEvent;
use orbiterx_core::X_RESPONSESAPI_INCLUDE_TIMING_METRICS_HEADER;
...
use orbiterx_rollout_trace::ConversationPart;
use orbiterx_rollout_trace::InferenceTraceContext;
use orbiterx_rollout_trace::RawTraceEventPayload;
use orbiterx_rollout_trace::TraceWriter;
...
fn turn_metadata(
    ...
    responses_metadata(harness, turn_id, TestOrbiterXResponsesRequestKind::Turn)
}
...
fn prewarm_metadata(
    ...
    responses_metadata(harness, turn_id, TestOrbiterXResponsesRequestKind::Prewarm)
}

fn websocket_connection_metadata(harness: &WebsocketTestHarness) -> OrbiterXResponsesMetadata {
    responses_metadata(
        harness,
        /*turn_id*/ None,
        TestOrbiterXResponsesRequestKind::WebsocketConnection,
    )
}
...
async fn responses_websocket_streams_request() {
    ...
    assert_eq!(
        ...
        Some(orbiterx_login::default_client::get_orbiterx_user_agent())
    );
    assert_eq!(
        body["client_metadata"]["x-orbiterx-installation-id"].as_str(),
        Some(TEST_INSTALLATION_ID)
    );
    let stream_request_start_ms = body["client_metadata"]
        [X_ORBITERX_WS_STREAM_REQUEST_START_MS_CLIENT_METADATA_KEY]
        .as_str()
...
async fn responses_websocket_preconnect_reuses_connection() {
    ...
    assert_eq!(
        server.single_handshake().header("x-orbiterx-window-id"),
...
async fn responses_websocket_request_prewarm_reuses_connection() {
    ...
    let warmup_turn_metadata: serde_json::Value = serde_json::from_str(
        warmup["client_metadata"]["x-orbiterx-turn-metadata"]
            .as_str()
...
async fn responses_websocket_request_prewarm_traces_logical_request() {
    ...
    writer
        .append(RawTraceEventPayload::OrbiterXTurnStarted {
            orbiterx_turn_id: "turn-1".to_string(),
            thread_id: harness.thread_id.to_string(),
        })
        .expect("turn started");
...
async fn responses_websocket_preconnect_runs_when_only_v2_feature_enabled() {
    ...
    assert_eq!(
        server.single_handshake().header("x-orbiterx-turn-metadata"),
        None
    );
...
async fn responses_websocket_usage_limit_error_emits_rate_limit_event() {
    ...
    let usage_limit_error = json!({
        ...
        "headers": {
            "x-orbiterx-primary-used-percent": "100.0",
            "x-orbiterx-secondary-used-percent": "87.5",
            "x-orbiterx-primary-over-secondary-limit-percent": "95.0",
            "x-orbiterx-primary-window-minutes": "15",
            "x-orbiterx-secondary-window-minutes": "60",
            "x-orbiterx-credits-has-credits": "true",
            "x-orbiterx-credits-unlimited": "false",
            "x-orbiterx-credits-balance": "",
            "x-orbiterx-rate-limit-reached-type": "workspace_member_usage_limit_reached"
        }
    });
    ...
    let test = builder
        ...
        .await
        .expect("build websocket orbiterx");
    ...
    let token_event =
        wait_for_event(&test.orbiterx, |msg| matches!(msg, EventMsg::TokenCount(_))).await;
    ...
    let error_event = wait_for_event(&test.orbiterx, |msg| matches!(msg, EventMsg::Error(_))).await;
...
async fn responses_websocket_forwards_turn_metadata_on_initial_and_incremental_create() {
    ...
    let first_metadata: serde_json::Value = serde_json::from_str(
        first["client_metadata"]["x-orbiterx-turn-metadata"]
            .as_str()
    ...
    let second_metadata: serde_json::Value = serde_json::from_str(
        second["client_metadata"]["x-orbiterx-turn-metadata"]
            .as_str()
...
async fn responses_websocket_sends_canonical_turn_metadata() {
    ...
    let turn_metadata: serde_json::Value = serde_json::from_str(
        body["client_metadata"]["x-orbiterx-turn-metadata"]
            .as_str()
...
async fn websocket_harness_with_provider_options(
    ...
    let model_info = orbiterx_core::test_support::construct_model_info_offline(MODEL, &config);
