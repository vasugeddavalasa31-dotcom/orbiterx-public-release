...
async fn guardian_session_prewarms_and_is_reused_for_first_review() -> Result<()> {
    ...
    let mut review_model = orbiterx_models_manager::bundled_models_response()?
        .models
        .into_iter()
        .find(|model| model.slug == "orbiterx-auto-review")
...
printf '%s\n' "${@: -1}" >> "${payload_path}""#,
    ...
    assert!(
        !notify_payload_raw.contains(
            "The following is the OrbiterX agent history whose request action you are assessing."
        ),
