...
use orbiterx_mcp::ElicitationRequestRouter;
...
use orbiterx_mcp::OrbiterXAppsToolsCache;
...
async fn hosted_plugin_runtime_ps_mcp_tool_calls_use_current_auth_manager_token() -> Result<()> {
    ...
    let expected_auth = OrbiterXAuth::from_external_chatgpt_tokens(
        "header.e30.first",
        "test-account",
    ...
    let mut hosted_plugin_runtime_config = orbiterx_mcp::hosted_plugin_runtime_mcp_server_config(
    ...
    *bearer_token_env_var = None;
    ...
    let manager = McpConnectionManager::new(
        ...
        McpRuntimeContext::new(
            Arc::new(orbiterx_exec_server::EnvironmentManager::without_environments()),
            home.path().to_path_buf(),
        ),
        home.path().to_path_buf(),
        OrbiterXAppsToolsCache::default(),
        McpToolCatalogCache::default(),
        orbiterx_mcp::orbiterx_apps_tools_cache_key(Some(&expected_auth)),
    ...
    auth_manager
        .set_external_auth(Arc::new(StaticExternalAuth(
            OrbiterXAuth::from_external_chatgpt_tokens(
                "header.e30.reloaded",
                "test-account",
                /*chatgpt_plan_type*/ None,
            )?,
    ...
    // so this tool call only sees the new token if the OrbiterX Apps provider
    ...
    let tool_call_request = requests
        .iter()
        .find(|request| {
            request.url.path() == "/api/orbiterx/ps/mcp"
        ...
        .expect("OrbiterX Apps should receive a tool call");
    assert_eq!(
        tool_call_request
            .headers
            .get("authorization")
