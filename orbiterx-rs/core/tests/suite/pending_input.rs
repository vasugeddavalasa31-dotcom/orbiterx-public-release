...
use orbiterx_core::config::CurrentTimeReminderConfig;
use orbiterx_extension_items::ExtensionItem;
use orbiterx_extension_items::sleep::SleepItem;
...
async fn build_orbiterx(server: &StreamingSseServer) -> Arc<OrbiterXThread> {
    test_orbiterx()
        .with_model("gpt-5.4")
        .build_with_streaming_server(server)
        .await
        .expect("build streaming OrbiterX test session")
        .orbiterx
}

async fn submit_user_input(orbiterx: &OrbiterXThread, text: &str) {
    orbiterx
        .submit(Op::UserInput {
...
async fn submit_danger_full_access_user_turn(test: &TestOrbiterX, text: &str) {
...
async fn steer_user_input(orbiterx: &OrbiterXThread, text: &str) {
    orbiterx
        .steer_input(
            vec![UserInput::Text {
                text: text.to_string(),
...
async fn submit_queue_only_agent_mail(orbiterx: &OrbiterXThread, text: &str) {
    orbiterx
...
async fn wait_for_reasoning_item_started(orbiterx: &OrbiterXThread) {
    wait_for_event(orbiterx, |event| {
        matches!(
            event,
...
async fn wait_for_agent_message(orbiterx: &OrbiterXThread, text: &str) {
...
async fn wait_for_turn_complete(orbiterx: &OrbiterXThread) {
    wait_for_event(orbiterx, |event| matches!(event, EventMsg::TurnComplete(_))).await;
}

async fn wait_for_sleep_item_started(orbiterx: &OrbiterXThread, call_id: &str, duration_ms: u64) {
    let event = wait_for_event(orbiterx, |event| {
        matches!(
            event,
...
async fn wait_for_sleep_item_completed(orbiterx: &OrbiterXThread, call_id: &str, duration_ms: u64) {
    let event = wait_for_event(orbiterx, |event| {
        matches!(
            event,
...
async fn steer_interrupts_wait_agent_and_is_sent_in_follow_up_request() {
    ...
    let orbiterx = test_orbiterx()
        ...
        .build_with_streaming_server(&server)
        .await
        .expect("build OrbiterX test session")
        .orbiterx;

    submit_user_input(&orbiterx, INITIAL_PROMPT).await;
    wait_for_event(&orbiterx, |event| {
        matches!(event, EventMsg::CollabWaitingBegin(_))
    })
    .await;

    steer_user_input(&orbiterx, STEER_PROMPT).await;
...
async fn any_new_input_interrupts_sleep() {
    ...
    submit_user_input(&orbiterx, INITIAL_PROMPT).await;
    wait_for_sleep_item_started(&orbiterx, FIRST_SLEEP_CALL_ID, SLEEP_DURATION_MS).await;

    steer_user_input(&orbiterx, STEER_PROMPT).await;
    wait_for_sleep_item_completed(&orbiterx, FIRST_SLEEP_CALL_ID, SLEEP_DURATION_MS).await;
    wait_for_sleep_item_started(&orbiterx, SECOND_SLEEP_CALL_ID, SLEEP_DURATION_MS).await;

    submit_queue_only_agent_mail(&orbiterx, "new mailbox input").await;
    wait_for_sleep_item_completed(&orbiterx, SECOND_SLEEP_CALL_ID, SLEEP_DURATION_MS).await;
    wait_for_turn_complete(&orbiterx).await;
    ...
    let rollout_path = orbiterx.rollout_path().expect("rollout path");
...
async fn queued_inter_agent_mail_triggers_follow_up_after_reasoning_item() {
    ...
    let orbiterx = build_orbiterx(&server).await;

    submit_user_input(&orbiterx, "first prompt").await;

    wait_for_reasoning_item_started(&orbiterx).await;

    submit_queue_only_agent_mail(&orbiterx, "queued child update").await;
...
async fn queued_inter_agent_mail_triggers_follow_up_after_commentary_message_item() {
    ...
    wait_for_agent_message(&orbiterx, "first answer").await;
...
async fn queued_inter_agent_mail_does_not_restart_after_final_answer() {
    ...
    submit_user_input(&orbiterx, "second prompt").await;
    wait_for_turn_complete(&orbiterx).await;
...
async fn user_input_does_not_preempt_after_reasoning_item() {
    ...
    steer_user_input(&orbiterx, "second prompt").await;
...
async fn steered_user_input_waits_for_model_continuation_after_mid_turn_compact() {
    ...
    wait_for_agent_message(&orbiterx, "resumed old task").await;
    wait_for_turn_complete(&orbiterx).await;
...
async fn steered_user_input_follows_compact_when_only_the_steer_needs_follow_up() {
    ...
    wait_for_agent_message(&orbiterx, "processed steered prompt").await;
    wait_for_turn_complete(&orbiterx).await;
...
async fn steered_user_input_waits_when_tool_output_triggers_compact_before_next_request() {
    ...
    wait_for_event(&orbiterx, |event| matches!(event, EventMsg::TurnStarted(_))).await;
    steer_user_input(&orbiterx, "second prompt").await;
    let _ = gate_first_completed_tx.send(());
