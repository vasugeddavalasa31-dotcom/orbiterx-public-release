...
use orbiterx_config::types::Personality;
...
fn read_only_user_turn(test: &TestOrbiterX, items: Vec<UserInput>, model: String) -> Op {
...
async fn model_change_appends_model_instructions_developer_message() -> Result<()> {
    ...
    let mut builder = test_orbiterx().with_model("gpt-5.2");
...
async fn model_and_personality_change_only_appends_model_instructions() -> Result<()> {
    ...
    let test = builder.build(&server).await?;
    let next_model = "exp-orbiterx-personality";
...
async fn service_tier_change_is_applied_on_next_http_turn() -> Result<()> {
    ...
    let test = test_orbiterx().build(&server).await?;
...
async fn unsupported_configured_service_tier_warns_at_session_start() -> Result<()> {
    ...
    let warning = wait_for_event(&test.orbiterx, |event| {
        matches!(
            event,
...
async fn model_switch_to_smaller_model_updates_token_context_window() -> Result<()> {
    ...
    let large_window_event = wait_for_event(&test.orbiterx, |event| {
        matches!(
            event,
    ...
    let smaller_turn_started_event = wait_for_event(&test.orbiterx, |event| {
        matches!(
            event,
    ...
    let smaller_window_event = wait_for_event(&test.orbiterx, |event| {
        matches!(
            event,
            EventMsg::TokenCount(token_count)
                if token_count
