...
async fn orbiterx_apps_auth_failure_requests_elicitation_by_default() -> Result<()> {
    skip_if_no_network!(Ok(()));

    let server = start_mock_server().await;
    let apps_server = AppsTestServer::mount_searchable(&server).await?;
    Mock::given(method("POST"))
        .and(path_regex("^/api/orbiterx/apps/?$"))
...
"#,
    ...
    assert_eq!(
        request.id,
        orbiterx_protocol::mcp::RequestId::String(format!("orbiterx_apps_auth_{call_id}"))
    );
    assert_eq!(
        request.request,
        ElicitationRequest::Url {
            ...
            elicitation_id: format!("orbiterx_apps_auth_{call_id}"),
        }
    );
