...
use chrono::Utc;
use orbiterx_core::RolloutRecorder;
use orbiterx_core::RolloutRecorderParams;
...
fn write_minimal_rollout_with_id_in_subdir(
    ...
    let sessions = orbiterx_home.join(subdir).join("2024/01/01");
...
/// provided conversation id in the SessionMeta line. Returns the absolute path.
fn write_minimal_rollout_with_id(orbiterx_home: &Path, id: Uuid) -> PathBuf {
    write_minimal_rollout_with_id_in_subdir(orbiterx_home, "sessions", id)
}
...
async fn upsert_thread_metadata(
    ...
    let runtime = StateRuntime::init(orbiterx_home.to_path_buf(), "test-provider".to_string())
        .await
        .unwrap();
    runtime
...
#[tokio::test]
async fn find_handles_gitignore_covering_orbiterx_home_directory() {
    let repo = TempDir::new().unwrap();
    let orbiterx_home = repo.path().join(".orbiterx");
    std::fs::create_dir_all(&orbiterx_home).unwrap();
    std::fs::write(repo.path().join(".gitignore"), ".orbiterx/**\n").unwrap();
    let id = Uuid::new_v4();
    let expected = write_minimal_rollout_with_id(&orbiterx_home, id);

    let found =
        find_thread_path_by_id_str(&orbiterx_home, &id.to_string(), /*state_db_ctx*/ None)
            .await
            .unwrap();
