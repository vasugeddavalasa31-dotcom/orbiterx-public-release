...
async fn window_id_advances_after_compact_persists_on_resume_and_resets_on_fork() -> Result<()> {
    ...
    let initial_thread = Arc::clone(&initial.orbiterx);
    ...
    let resumed = builder
        ...
        .await?;
    submit_user_turn(&resumed.orbiterx, "after resume").await?;
    shutdown_thread(&resumed.orbiterx).await?;
...
async fn submit_user_turn(orbiterx: &Arc<OrbiterXThread>, text: &str) -> Result<()> {
    orbiterx
        .submit(Op::UserInput {
...
async fn submit_compact_turn(orbiterx: &Arc<OrbiterXThread>) -> Result<()> {
    orbiterx.submit(Op::Compact).await?;
    let warning_event =
        wait_for_event(orbiterx, |event| matches!(event, EventMsg::Warning(_))).await;
...
async fn shutdown_thread(orbiterx: &Arc<OrbiterXThread>) -> Result<()> {
...
fn window_id_parts(request: &ResponsesRequest) -> (String, u64) {
    let window_id = request
        ...
        .expect("missing x-orbiterx-window-id header");
