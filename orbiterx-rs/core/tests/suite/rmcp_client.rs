...
use orbiterx_config::types::McpServerAuth;
...
use orbiterx_exec_server::HttpRedirectPolicy;
use orbiterx_exec_server::HttpRequestParams;
use orbiterx_login::OrbiterXAuth;
use orbiterx_mcp::MCP_SANDBOX_STATE_META_CAPABILITY;
...
use orbiterx_protocol::openai_models::ConfigShellToolType;
...
use orbiterx_protocol::openai_models::TruncationPolicyConfig;
...
use orbiterx_utils_cargo_bin::cargo_bin;
...
fn auto_approved_user_turn(fixture: &TestOrbiterX, text: impl Into<String>) -> Op {
...
pub(super) fn remote_aware_environment_id() -> String {
    if is_remote_test_environment() {
        ...
        orbiterx_config::DEFAULT_MCP_SERVER_ENVIRONMENT_ID.to_string()
    }
}
...
fn unique_remote_path(binary_name: &str) -> anyhow::Result<String> {
    ...
    Ok(format!(
        "/tmp/orbiterx-remote-env/{binary_name}-{}-{unique_suffix}",
        std::process::id()
    ))
}
...
async fn call_structured_tool(
    ...
    wait_for_event(&fixture.orbiterx, |ev| {
        matches!(ev, EventMsg::McpToolCallBegin(_))
    })
    .await;
    let end_event = wait_for_event(&fixture.orbiterx, |ev| {
...
async fn openai_form_capability_updates_for_loaded_thread() -> anyhow::Result<()> {
    ...
    let fixture = test_orbiterx()
        ...
        .build(&server)
        .await?;
    wait_for_mcp_server(&fixture.orbiterx, server_name).await?;
...
async fn stdio_server_round_trip() -> anyhow::Result<()> {
    ...
    let begin_event = wait_for_event(&fixture.orbiterx, |ev| {
        matches!(ev, EventMsg::McpToolCallBegin(_))
    })
    .await;
...
async fn shutdown_cancels_startup_prewarm_waiting_for_mcp_startup() -> anyhow::Result<()> {
    ...
    tokio::time::timeout(Duration::from_secs(2), fixture.orbiterx.shutdown_and_wait())
        .await
...
async fn stdio_mcp_tool_call_includes_sandbox_state_meta() -> anyhow::Result<()> {
    ...
    assert_eq!(
        ...
        SandboxState {
            ...
            orbiterx_linux_sandbox_exe: fixture.config.orbiterx_linux_sandbox_exe.clone(),
...
async fn stdio_mcp_parallel_tool_calls_default_false_runs_serially() -> anyhow::Result<()> {
    ...
    while call_events.len() < 4 {
        let event = wait_for_event(&fixture.orbiterx, |ev| {
            matches!(
                ev,
...
async fn stdio_image_responses_are_sanitized_for_text_only_model() -> anyhow::Result<()> {
        ...
        ModelsResponse {
            models: vec![ModelInfo {
                ...
                supported_reasoning_levels: vec![ReasoningEffortPreset {
                    effort: orbiterx_protocol::openai_models::ReasoningEffort::Medium,
    ...
    fixture
        ...
        .list_models(
            ...
            orbiterx_core::test_support::default_http_client_factory(),
        )
        .await;
...
impl StreamableHttpTestServer {
    /// Returns the MCP endpoint URL that OrbiterX should connect to.
    fn url(&self) -> &str {
        &self.server_url
    }
...
/// What this tests: OrbiterX can discover and call a Streamable HTTP MCP tool in
...
async fn streamable_http_tool_call_round_trip() -> anyhow::Result<()> {
    ...
    // Phase 1: script the model responses so OrbiterX will call the MCP echo tool
    ...
    // Phase 3: configure OrbiterX with the Streamable HTTP MCP server and build a
    ...
    // Phase 5: assert OrbiterX begins the expected tool invocation.
    let begin_event = wait_for_event(&fixture.orbiterx, |ev| {
        matches!(ev, EventMsg::McpToolCallBegin(_))
    })
    .await;
...
async fn streamable_http_configured_auth_precedes_chatgpt_auth() -> anyhow::Result<()> {
    ...
    let configured_auth_fixture = test_orbiterx()
        .with_auth(OrbiterXAuth::create_dummy_chatgpt_auth_for_testing())
        .with_config(move |config| {
            insert_mcp_server(
                config,
    ...
    wait_for_mcp_server(&configured_auth_fixture.orbiterx, "configured_auth").await?;
...
async fn streamable_http_chatgpt_auth_is_not_sent_to_configured_origin() -> anyhow::Result<()> {
    ...
    let untrusted_mcp_url = format!("{}/api/orbiterx/apps", untrusted_apps.chatgpt_base_url);
    ...
    wait_for_mcp_server(&fixture.orbiterx, "untrusted_origin").await?;
    let observed_requests = untrusted_server
        .received_requests()
        .await
        .expect("mock server should capture MCP startup requests")
        .into_iter()
        .filter(|request| request.url.path() == "/api/orbiterx/apps")
        .filter_map(|request| {
...
async fn configured_chatgpt_base_url_does_not_grant_mcp_chatgpt_auth() -> anyhow::Result<()> {
    ...
    let fixture = test_orbiterx()
        .with_auth(OrbiterXAuth::create_dummy_chatgpt_auth_for_testing())
        .with_pre_build_hook(move |orbiterx_home| {
            fs::write(
...
/// This test writes to a fallback credentials file in ORBITERX_HOME.
/// Ideally, we wouldn't need to serialize the test but it's much more cumbersome to wire ORBITERX_HOME through the code.
#[test]
#[serial(orbiterx_home)]
...
async fn streamable_http_with_oauth_round_trip_impl() -> anyhow::Result<()> {
    ...
    // Phase 1: script the model responses so OrbiterX will call the OAuth-backed
    ...
    // Phase 3: seed an isolated ORBITERX_HOME with fallback OAuth tokens for this
    // server so the test does not share credentials with other suite cases.
    let temp_home = Arc::new(tempdir()?);
    let _orbiterx_home_guard = EnvVarGuard::set("ORBITERX_HOME", temp_home.path().as_os_str());
    ...
    // Phase 4: configure OrbiterX with the OAuth-backed Streamable HTTP MCP
    ...
    // Phase 7: assert OrbiterX begins the expected tool invocation.
    let begin_event = wait_for_event(&fixture.orbiterx, |ev| {
        matches!(ev, EventMsg::McpToolCallBegin(_))
    })
    .await;
...
async fn start_remote_streamable_http_test_server(
    ...
    // remote HTTP before handing the URL to the OrbiterX fixture.
