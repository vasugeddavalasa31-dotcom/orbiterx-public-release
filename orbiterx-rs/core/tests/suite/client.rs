...
use core_test_support::TestOrbiterXResponsesRequestKind;
...
use orbiterx_core::ModelClient;
use orbiterx_core::NewThread;
use orbiterx_core::Prompt;
use orbiterx_core::ResponseEvent;
use orbiterx_core::ThreadManager;
use orbiterx_core::resolve_installation_id;
use orbiterx_core::thread_store_from_config;
use orbiterx_extension_api::empty_extension_registry;
use orbiterx_features::Feature;
use orbiterx_login::AuthKeyringBackendKind;
...
use orbiterx_model_provider_info::AMAZON_BEDROCK_PROVIDER_ID;
...
use orbiterx_protocol::models::LocalShellAction;
use orbiterx_protocol::models::LocalShellExecAction;
use orbiterx_protocol::models::LocalShellStatus;
use orbiterx_protocol::models::MessagePhase;
use orbiterx_protocol::models::ReasoningItemContent;
use orbiterx_protocol::models::ReasoningItemReasoningSummary;
use orbiterx_protocol::models::ResponseItem;
use orbiterx_protocol::models::WebSearchAction;
use orbiterx_protocol::openai_models::InputModality;
...
use orbiterx_protocol::protocol::RolloutLine;
...
fn test_turn_responses_metadata(
    _client: &ModelClient,
    thread_id: ThreadId,
) -> orbiterx_core::OrbiterXResponsesMetadata {
...
fn assert_orbiterx_client_metadata(
    ...
    assert_eq!(
        client_metadata["x-orbiterx-installation-id"].as_str(),
        Some(installation_id)
    );
    assert_eq!(client_metadata["session_id"].as_str(), Some(session_id));
    assert_eq!(client_metadata["thread_id"].as_str(), Some(thread_id));
    let turn_metadata_str = client_metadata["x-orbiterx-turn-metadata"]
        .as_str()
        .expect("missing x-orbiterx-turn-metadata client metadata");
    let turn_metadata = serde_json::from_str::<serde_json::Value>(turn_metadata_str)
        .expect("invalid x-orbiterx-turn-metadata json");
    assert_eq!(
    ...
    assert_eq!(
        client_metadata["x-orbiterx-window-id"].as_str(),
...
async fn openai_stateless_responses_requests_preserve_item_turn_metadata_across_turns() {
    ...
    .await;
    let test = test_orbiterx().build(&server).await.unwrap();
...
async fn response_item_ids_persist_across_resume_and_preserve_server_ids() -> anyhow::Result<()> {
    ...
    initial.orbiterx.submit(Op::Shutdown).await?;
    wait_for_event(&initial.orbiterx, |event| {
...
async fn synthetic_call_output_id_is_stable_across_resumes() -> anyhow::Result<()> {
    ...
    .await;
    let orbiterx_home = Arc::new(TempDir::new()?);
    let mut builder = test_orbiterx().with_config(|config| {
        let _ = config.features.enable(Feature::ItemIds);
    });
    let first = builder
        .resume(&server, Arc::clone(&orbiterx_home), session_path.clone())
        .await?;

    first.submit_turn("first resume").await?;
    first.orbiterx.submit(Op::Shutdown).await?;
    wait_for_event(&first.orbiterx, |event| {
        matches!(event, EventMsg::ShutdownComplete)
    })
    .await;
    assert!(
    ...
    let second = builder.resume(&server, orbiterx_home, session_path).await?;
...
async fn response_item_ids_are_sent_for_all_remote_v2_compaction_requests() -> anyhow::Result<()> {
    ...
    let test = test_orbiterx()
        .with_auth(OrbiterXAuth::create_dummy_chatgpt_auth_for_testing())
        .with_config(|config| {
            let _ = config.features.enable(Feature::ItemIds);
    ...
    test.orbiterx.submit(Op::Compact).await?;
    wait_for_event(&test.orbiterx, |event| {
        matches!(event, EventMsg::TurnComplete(_))
    })
    .await;
...
/// Writes an `auth.json` into the provided `orbiterx_home` with the specified parameters.
...
fn write_auth_json(
    ...
    std::fs::write(
        orbiterx_home.path().join("auth.json"),
...
"#,
    ...
    fn auth(&self) -> ModelProviderAuthInfo {
        ModelProviderAuthInfo {
            ...
            cwd: orbiterx_utils_absolute_path::AbsolutePathBuf::try_from(self.tempdir.path())
...
async fn resume_includes_initial_messages_and_sends_prior_items() {
    ...
    let prior_user = orbiterx_protocol::models::ResponseItem::Message {
        id: None,
        role: "user".to_string(),
        content: vec![orbiterx_protocol::models::ContentItem::InputText {
    ...
    let prior_system = orbiterx_protocol::models::ResponseItem::Message {
        id: None,
        role: "system".to_string(),
        content: vec![orbiterx_protocol::models::ContentItem::OutputText {
    ...
    let prior_item = orbiterx_protocol::models::ResponseItem::Message {
        id: None,
    ...
    // Configure OrbiterX to resume from our file
    let orbiterx_home = Arc::new(TempDir::new().unwrap());
    let mut builder = test_orbiterx()
        .with_home(orbiterx_home.clone())
        .with_pre_build_hook(|home| {
            std::fs::write(home.join("AGENTS.md"), "be nice").expect("write global instructions");
        });
    let test = builder
        .resume(&server, orbiterx_home, session_path.clone())
        .await
        .expect("resume conversation");
    let orbiterx = test.orbiterx.clone();
    ...
    orbiterx
        ...
        .await
        .unwrap();
    wait_for_event(&orbiterx, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
...
async fn resume_replays_legacy_js_repl_image_rollout_shapes() {
    ...
    let orbiterx_home = Arc::new(TempDir::new().unwrap());
    let mut builder = test_orbiterx().with_model("gpt-5.4");
    let test = builder
        .resume(&server, orbiterx_home, session_path.clone())
        .await
        .expect("resume conversation");
...
async fn includes_session_id_thread_id_and_model_headers_in_request() {
    ...
    let mut builder = test_orbiterx().with_auth(OrbiterXAuth::from_api_key("Test API Key"));
    let test = builder
        .build(&server)
        .await
    ...
    let request_body = request.body_json();
    let installation_id =
        std::fs::read_to_string(test.orbiterx_home_path().join(INSTALLATION_ID_FILENAME))
    ...
    assert_orbiterx_client_metadata(
        &request_body,
...
async fn send_request_with_provider(provider: ModelProviderInfo) {
    ...
    let mut config = load_default_config_for_test(&orbiterx_home).await;
    config.model_provider_id = provider.name.clone();
    config.model_provider = provider.clone();
    let effort = config.model_reasoning_effort.clone();
    let summary = config.model_reasoning_summary;
    let model = orbiterx_core::test_support::get_model_offline(config.model.as_deref());
    config.model = Some(model.clone());
    let config = Arc::new(config);
    let model_info =
        orbiterx_core::test_support::construct_model_info_offline(model.as_str(), &config);
    ...
    let client = ModelClient::new(
        Some(AuthManager::from_auth_for_testing(
            OrbiterXAuth::from_api_key("unused-api-key"),
        )),
    ...
    let mut stream = client_session
        .stream(
            ...
            /*service_tier*/ None,
            &responses_metadata,
            &orbiterx_rollout_trace::InferenceTraceContext::disabled(),
        )
        .await
...
async fn includes_base_instructions_override_in_request() {
    ...
    let mut builder = test_orbiterx()
        .with_auth(OrbiterXAuth::from_api_key("Test API Key"))
        .with_config(|config| {
            config.base_instructions = Some("test instructions".to_string());
        });
    let orbiterx = builder
        .build(&server)
        .await
        .expect("create new conversation")
        .orbiterx;
...
async fn chatgpt_auth_sends_correct_request() {
    ...
    let mut model_provider =
        built_in_model_providers(/* openai_base_url */ /*openai_base_url*/ None)["openai"].clone();
    model_provider.base_url = Some(format!("{}/api/orbiterx", server.uri()));
    model_provider.supports_websockets = false;
    let mut builder = test_orbiterx()
        .with_auth(create_dummy_orbiterx_auth())
        .with_config(move |config| {
    ...
    let request = resp_mock.single_request();
    assert_eq!(request.path(), "/api/orbiterx/responses");
    let request_authorization = request
        .header("authorization")
        .expect("authorization header");
    let request_originator = request.header("originator").expect("originator header");
...
async fn prefers_apikey_when_config_prefers_apikey_even_with_chatgpt_tokens() {
    ...
    let auth = OrbiterXAuth::from_auth_storage(
        orbiterx_home.path(),
    ...
    .await
    .expect("Failed to load OrbiterXAuth")
    .expect("No OrbiterXAuth found in orbiterx_home");
    let auth_manager = orbiterx_core::test_support::auth_manager_from_auth(auth);
    let installation_id = resolve_installation_id(&config.orbiterx_home)
        .await
        .expect("resolve installation id");
    let thread_manager = ThreadManager::new(
        &config,
        auth_manager.clone(),
        orbiterx_core::build_models_manager(&config, auth_manager),
        orbiterx_core::OrbiterXAppsToolsCache::default(),
        SessionSource::Exec,
        Arc::new(orbiterx_exec_server::EnvironmentManager::default_for_tests()),
        empty_extension_registry(),
        Arc::new(orbiterx_core::test_support::EmptyUserInstructionsProvider),
...
async fn omits_apps_guidance_when_orchestrator_mcp_is_disabled() {
    ...
    assert!(
        !request.body_contains_text("mcp__orbiterx_apps"),
        "did not expect orbiterx_apps MCP tools when orchestrator MCP is disabled, got {:?}",
    ...
    assert!(
        ...
        "did not expect orbiterx_apps resource calls: {resource_methods:?}"
    );
}
...
async fn skills_append_to_developer_message() {
    ...
    let orbiterx_home = Arc::new(TempDir::new().unwrap());
    let skill_dir = orbiterx_home.path().join("skills/demo");
    ...
    let orbiterx_home_path = orbiterx_home.path().to_path_buf();
    let mut builder = test_orbiterx()
        .with_home(orbiterx_home.clone())
        .with_auth(OrbiterXAuth::from_api_key("Test API Key"))
        .with_config(move |config| {
            config.cwd = orbiterx_home_path.abs();
        });
    let orbiterx = builder
        .build(&server)
        .await
    ...
    let _orbiterx_home_guard = orbiterx_home;
}
...
async fn skills_use_aliases_in_developer_message_under_budget_pressure() {
    ...
    let orbiterx_home_parent = TempDir::new().unwrap();
    let long_home_parent = orbiterx_home_parent
        .path()
        .join("orbiterx-home-with-long-shared-prefix-for-skill-alias-budget-test");
    std::fs::create_dir_all(&long_home_parent).expect("create long home parent");
    let orbiterx_home = Arc::new(TempDir::new_in(long_home_parent).unwrap());
    let skill_root = orbiterx_home.path().join("skills");
    for index in 0..12 {
    ...
    let mut builder = test_orbiterx()
        ...
        .with_config(move |config| {
            config.cwd = orbiterx_home_path.abs();
            let user_config_path = orbiterx_home_path.join("config.toml").abs();
    ...
    let _orbiterx_home_guard = orbiterx_home;
    let _orbiterx_home_parent_guard = orbiterx_home_parent;
}
...
async fn includes_configured_max_effort_in_request() -> anyhow::Result<()> {
    ...
    .await;
    let TestOrbiterX { orbiterx, .. } = test_orbiterx()
        .with_model("gpt-5.4")
        .with_config(|config| {
...
async fn includes_no_effort_in_request() -> anyhow::Result<()> {
    ...
    .await;
    let TestOrbiterX { orbiterx, .. } =
        test_orbiterx().with_model("gpt-5.4").build(&server).await?;
...
async fn azure_responses_request_includes_store_and_prefixed_item_ids() {
    ...
    let model_info =
        orbiterx_core::test_support::construct_model_info_offline(model.as_str(), &config);
    let thread_id = ThreadId::new();
    let auth_manager = orbiterx_core::test_support::auth_manager_from_auth(
...
async fn token_count_includes_rate_limits_snapshot() {
    ...
    let response = ResponseTemplate::new(200)
        .insert_header("content-type", "text/event-stream")
        .insert_header("x-orbiterx-primary-used-percent", "12.5")
        .insert_header("x-orbiterx-secondary-used-percent", "40.0")
        .insert_header("x-orbiterx-primary-window-minutes", "10")
        .insert_header("x-orbiterx-secondary-window-minutes", "60")
        .insert_header("x-orbiterx-primary-reset-at", "1704069000")
        .insert_header("x-orbiterx-secondary-reset-at", "1704074400")
    ...
    let mut builder = test_orbiterx()
        .with_auth(OrbiterXAuth::from_api_key("test"))
        .with_config(move |config| {
            config.model_provider = provider;
        });
    let orbiterx = builder
    ...
    wait_for_event(&orbiterx, |msg| matches!(msg, EventMsg::TurnComplete(_))).await;
}
...
async fn usage_limit_error_emits_rate_limit_event() -> anyhow::Result<()> {
    ...
    let response = ResponseTemplate::new(429)
        .insert_header("x-orbiterx-primary-used-percent", "100.0")
        .insert_header("x-orbiterx-secondary-used-percent", "87.5")
        .insert_header("x-orbiterx-primary-over-secondary-limit-percent", "95.0")
        .insert_header("x-orbiterx-primary-window-minutes", "15")
        .insert_header("x-orbiterx-secondary-window-minutes", "60")
        .insert_header("x-orbiterx-credits-has-credits", "true")
        .insert_header("x-orbiterx-credits-unlimited", "false")
        .insert_header("x-orbiterx-credits-balance", "")
        .insert_header(
            "x-orbiterx-rate-limit-reached-type",
    ...
    let mut builder = test_orbiterx();
    let orbiterx_fixture = builder.build(&server).await?;
    let orbiterx = orbiterx_fixture.orbiterx.clone();
    ...
    let token_event = wait_for_event(&orbiterx, |msg| matches!(msg, EventMsg::TokenCount(_))).await;
    ...
    let error_event = wait_for_event(&orbiterx, |msg| matches!(msg, EventMsg::Error(_))).await;
...
async fn context_window_error_sets_total_tokens_to_model_window() -> anyhow::Result<()> {
    ...
    let token_event = wait_for_event(&orbiterx, |event| {
        matches!(
            event,
    ...
    let error_event = wait_for_event(&orbiterx, |ev| matches!(ev, EventMsg::Error(_))).await;
    let expected_context_window_message = OrbiterXErr::ContextWindowExceeded.to_string();
    assert!(
        matches!(
            error_event,
...
fn create_dummy_orbiterx_auth() -> OrbiterXAuth {
...
async fn history_dedupes_streamed_and_final_messages_across_turns() {
    // Skip under OrbiterX sandbox network restrictions (mirrors other tests).
    skip_if_no_network!();
