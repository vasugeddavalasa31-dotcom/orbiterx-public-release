...
async fn request_body_is_zstd_compressed_for_orbiterx_backend_when_enabled() -> anyhow::Result<()> {
    ...
    let base_url = format!("{}/backend-api/orbiterx/v1", server.uri());
    ...
    let orbiterx = builder.build(&server).await?.orbiterx;
