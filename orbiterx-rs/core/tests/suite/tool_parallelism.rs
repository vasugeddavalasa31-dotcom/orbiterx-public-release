...
async fn run_turn(test: &TestOrbiterX, prompt: &str) -> anyhow::Result<()> {
...
async fn run_turn_and_measure(test: &TestOrbiterX, prompt: &str) -> anyhow::Result<Duration> {
    let start = Instant::now();
    run_turn(test, prompt).await?;
    Ok(start.elapsed())
}

async fn build_orbiterx_with_test_tool(
    server: &wiremock::MockServer,
) -> anyhow::Result<TestOrbiterX> {
    let mut builder = test_orbiterx().with_model("test-gpt-5.1-orbiterx");
...
async fn read_file_tools_run_in_parallel() -> anyhow::Result<()> {
    ...
    let test = build_orbiterx_with_test_tool(&server).await?;
