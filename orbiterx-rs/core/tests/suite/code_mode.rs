...
use orbiterx_protocol::dynamic_tools::DynamicToolCallOutputContentItem;
...
use orbiterx_web_search_extension::install as install_web_search_extension;
...
async fn run_code_mode_turn(
    ...
    prompt: &str,
    code: &str,
) -> Result<(TestOrbiterX, ResponseMock)> {
...
async fn run_code_mode_turn_with_builder(
    ...
    prompt: &str,
    code: &str,
    mut builder: TestOrbiterXBuilder,
) -> Result<(TestOrbiterX, ResponseMock)> {
...
async fn missing_process_host_falls_back_to_in_process_code_mode() -> Result<()> {
    ...
    let builder = test_orbiterx()
        ...
        .with_code_mode_host_program("orbiterx-code-mode-host-does-not-exist".into())
        .with_config(|config| {
            config
                .features
...
"#,
    ...
    let auth = OrbiterXAuth::from_api_key("dummy");
    ...
    assert_eq!(
        search_body["model"],
        serde_json::json!("test-gpt-5.1-orbiterx")
    );
    assert_eq!(
...
async fn run_code_mode_turn_with_rmcp(
    ...
    run_code_mode_turn_with_rmcp_model(server, prompt, code, "test-gpt-5.1-orbiterx").await
}
...
async fn run_code_mode_turn_with_rmcp_config(
    ...
    let test = builder.build(server).await?;
    wait_for_mcp_server(&test.orbiterx, "rmcp").await?;
...
const tool = ALL_TOOLS.find(
  ({ name }) => name === "mcp__orbiterx_apps__calendar_timezone_option_99"
);
if (!tool) {
...
"#,
        visible_tool_name = "mcp__orbiterx_apps__calendar_timezone_option_99",
...
async fn code_mode_can_call_hidden_dynamic_tools() -> Result<()> {
    ...
    let code = r#"
const tool = ALL_TOOLS.find(({ name }) => name === "orbiterx_app__hidden_dynamic_tool");
const out = await tools.orbiterx_app__hidden_dynamic_tool({ city: "Paris" });
text(
  JSON.stringify({
    name: tool?.name ?? null,
...
"#;
    ...
    test.orbiterx
        .submit(Op::UserInput {
            ...
            thread_settings: orbiterx_protocol::protocol::ThreadSettingsOverrides {
                environments: Some(orbiterx_protocol::protocol::TurnEnvironmentSelections::new(
                    cwd,
                    Vec::new(),
                )),
    ...
    let turn_id = wait_for_event_match(&test.orbiterx, |event| match event {
    ...
    .await;
    assert_eq!(request.namespace.as_deref(), Some("orbiterx_app"));
    ...
    test.orbiterx
        ...
        .await?;
    wait_for_event(&test.orbiterx, |event| match event {
    ...
    assert_eq!(
        parsed.get("name"),
        Some(&Value::String(
            "orbiterx_app__hidden_dynamic_tool".to_string()
        ))
    );
    assert_eq!(
        parsed.get("out"),
    ...
    assert!(
        parsed
            ...
            .is_some_and(|description| {
                description.contains("OrbiterX app tools.")
                    && description.contains("A hidden dynamic tool.")
                    && description.contains("declare const tools:")
                    && description.contains("orbiterx_app__hidden_dynamic_tool(args:")
            })
    );
