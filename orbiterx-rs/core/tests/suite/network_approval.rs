...
use orbiterx_protocol::approvals::NetworkApprovalContext;
...
const NETWORK_TEST_HOST: &str = "orbiterx-network-test.invalid";
const NETWORK_TEST_TARGET: &str = "http://orbiterx-network-test.invalid:80";
...
async fn cancelled_guardian_network_review_fails_closed_without_rewriting_turn_state() -> Result<()>
{
    ...
    test.orbiterx.submit(Op::Interrupt).await?;
...
async fn user_network_approval_once_session_and_denial_semantics() -> Result<()> {
    ...
    assert_eq!(
        approval.call_id,
        "network#local#http#orbiterx-network-test.invalid#80"
    );
    ...
    assert_eq!(
        ...
        Some("orbiterx-network-test.invalid is not in the allowed_domains")
    );
    test.orbiterx
...
async fn allowing_network_policy_amendment_persists_context_and_bypasses_prompt() -> Result<()> {
    ...
    assert!(policy.contains(
        r#"network_rule(host="orbiterx-network-test.invalid", protocol="http", decision="allow""#
    ));
    assert!(first_responses.requests().iter().any(|request| {
        request.body_contains_text(
            "Allowed network rule saved in execpolicy (allowlist): orbiterx-network-test.invalid",
        )
    }));
...
async fn ambiguous_unattributed_network_request_is_not_assigned_to_active_calls() -> Result<()> {
    ...
    .await;
    test.orbiterx.submit(Op::CleanBackgroundTerminals).await?;
    tokio::time::timeout(Duration::from_secs(10), async {
        loop {
            if test.orbiterx.list_background_terminals().await.is_empty() {
                break;
            }
...
async fn managed_network_unified_exec_test(server: &wiremock::MockServer) -> Result<TestOrbiterX> {
...
async fn wait_for_completion_without_network_prompt(test: &TestOrbiterX) {
...
async fn wait_for_turn_complete(test: &TestOrbiterX) {
    wait_for_event(&test.orbiterx, |event| {
