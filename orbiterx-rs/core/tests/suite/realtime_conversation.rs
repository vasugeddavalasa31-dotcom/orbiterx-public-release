...
use orbiterx_core::test_support::auth_manager_from_auth;
...
use orbiterx_protocol::protocol::RealtimeConversationVersion;
use orbiterx_protocol::protocol::RealtimeEvent;
use orbiterx_protocol::protocol::RealtimeNoopRequested;
...
const STARTUP_CONTEXT_HEADER: &str = "Startup context from OrbiterX.";
const STARTUP_CONTEXT_OPEN_TAG: &str = "<startup_context>";
const STARTUP_CONTEXT_CLOSE_TAG: &str = "</startup_context>";
const REALTIME_BACKEND_PROMPT: &str = orbiterx_prompts::BACKEND_PROMPT;
...
const REALTIME_CONVERSATION_TEST_SUBPROCESS_ENV_VAR: &str =
    "ORBITERX_REALTIME_CONVERSATION_TEST_SUBPROCESS";
...
fn run_realtime_conversation_test_in_subprocess(
    ...
    for &key in orbiterx_network_proxy::PROXY_ENV_KEYS {
...
async fn seed_recent_thread(
    ...
    let db = test.orbiterx.state_db().context("state db enabled")?;
    ...
    let mut metadata_builder = orbiterx_state::ThreadMetadataBuilder::new(
        thread_id,
        rollout_path,
        updated_at,
        SessionSource::Cli,
...
async fn conversation_start_audio_text_close_round_trip() -> Result<()> {
    ...
    let started = wait_for_event_match(&test.orbiterx, |msg| match msg {
    ...
    let session_updated = wait_for_event_match(&test.orbiterx, |msg| match msg {
    ...
    let audio_out = wait_for_event_match(&test.orbiterx, |msg| match msg {
        EventMsg::RealtimeConversationRealtime(RealtimeConversationRealtimeEvent {
    ...
    test.orbiterx.submit(Op::RealtimeConversationClose).await?;
    let closed = wait_for_event_match(&test.orbiterx, |msg| match msg {
...
async fn conversation_webrtc_start_posts_generated_session() -> Result<()> {
    ...
    let created = wait_for_event_match(&test.orbiterx, |msg| match msg {
    ...
    assert_eq!(
        request
            ...
            .get("content-type")
            .and_then(|value| value.to_str().ok()),
        Some("multipart/form-data; boundary=orbiterx-realtime-call-boundary")
    );
    ...
    assert_eq!(
        body,
        format!(
            "--orbiterx-realtime-call-boundary\r\n\
...
async fn conversation_webrtc_default_v1_rejects_explicit_v2_voice() -> Result<()> {
    ...
    let error = wait_for_event_match(&test.orbiterx, |msg| match msg {
        EventMsg::RealtimeConversationRealtime(RealtimeConversationRealtimeEvent {
...
async fn conversation_webrtc_close_while_sideband_connecting_drops_pending_join() -> Result<()> {
    ...
    let sdp = wait_for_event_match(&test.orbiterx, |msg| match msg {
    ...
    let stale_event = timeout(Duration::from_millis(700), async {
        wait_for_event_match(&test.orbiterx, |msg| match msg {
            EventMsg::RealtimeConversationRealtime(RealtimeConversationRealtimeEvent {
                payload: RealtimeEvent::Error(message),
...
async fn conversation_webrtc_sideband_connect_failure_closes_with_error() -> Result<()> {
    ...
    let err = wait_for_event_match(&test.orbiterx, |msg| match msg {
        EventMsg::RealtimeConversationRealtime(RealtimeConversationRealtimeEvent {
            payload: RealtimeEvent::Error(message),
        }) => Some(message.clone()),
        _ => None,
...
async fn conversation_start_uses_openai_env_key_fallback_with_chatgpt_auth() -> Result<()> {
    ...
    test.orbiterx.submit(Op::RealtimeConversationClose).await?;
    let _closed = wait_for_event_match(&test.orbiterx, |msg| match msg {
        EventMsg::RealtimeConversationClosed(closed) => Some(closed.clone()),
        _ => None,
    })
    .await;
...
async fn conversation_audio_before_start_emits_error() -> Result<()> {
    ...
    .await;
    assert_eq!(err.orbiterx_error_info, Some(OrbiterXErrorInfo::BadRequest));
    assert_eq!(err.message, "conversation is not running");
...
async fn conversation_second_start_replaces_runtime() -> Result<()> {
    ...
    test.orbiterx
        ...
        .await?;
    let _ = wait_for_event_match(&test.orbiterx, |msg| match msg {
        EventMsg::RealtimeConversationRealtime(RealtimeConversationRealtimeEvent {
            payload: RealtimeEvent::AudioOut(frame),
...
async fn conversation_uses_explicit_start_voice() -> Result<()> {
    ...
    .await;
    let test = test_orbiterx().build_with_websocket_server(&server).await?;
    assert!(
        server
            .wait_for_handshakes(/*expected*/ 1, Duration::from_secs(2))
            .await
...
async fn conversation_startup_context_current_thread_selects_many_turns_by_budget() -> Result<()> {
    ...
    let resumed_thread = test
        ...
        .resume_thread_with_history(
            ...
            auth_manager_from_auth(OrbiterXAuth::from_api_key("dummy")),
            /*parent_trace*/ None,
            /*supports_openai_form_elicitation*/ false,
        )
        .await?;
    let orbiterx = resumed_thread.thread;
    ...
    orbiterx.shutdown_and_wait().await?;
    realtime_server.shutdown().await;
    Ok(())
}
...
async fn conversation_startup_context_falls_back_to_workspace_map() -> Result<()> {
    ...
    let test = builder.build_with_websocket_server(&startup_server).await?;
    fs::create_dir_all(test.workspace_path("orbiterx-rs/core"))?;
    ...
    assert!(startup_context.contains("orbiterx-rs/"));
...
async fn conversation_user_text_turn_is_not_sent_to_realtime() -> Result<()> {
    ...
    let turn_complete = wait_for_event_match(&test.orbiterx, |event| match event {
...
async fn conversation_flushes_assistant_deltas_every_200ms_for_v3_handoff() -> Result<()> {
    ...
    test.orbiterx
        .submit(Op::RealtimeConversationStart(ConversationStartParams {
            ...
            orbiterx_response_item_prefix: None,
            orbiterx_response_handoff_mode:
                orbiterx_protocol::protocol::OrbiterXResponseHandoffMode::BemTags,
            model: None,
            output_modality: RealtimeOutputModality::Audio,
            include_startup_context: true,
            prompt: Some(Some("backend prompt".to_string())),
...
async fn inbound_handoff_request_starts_turn() -> Result<()> {
    ...
    let turn_id = loop {
        let event = test.orbiterx.next_event().await?;
...
async fn delegated_turn_user_role_echo_does_not_redelegate_and_still_forwards_audio() -> Result<()>
{
    ...
    let mut builder = test_orbiterx().with_model("gpt-5.4").with_config({
        let realtime_base_url = realtime_server.uri().to_string();
        move |config| {
            config.experimental_realtime_ws_base_url = Some(realtime_base_url);
            config.realtime.version = RealtimeWsVersion::V1;
