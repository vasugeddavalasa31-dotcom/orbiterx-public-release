...
async fn granular_complex_forced_rm_requests_approval_when_allowed() -> Result<()> {
    ...
    let approval_event = wait_for_event(&test.orbiterx, |event| {
        matches!(
            event,
...
async fn execpolicy_blocks_shell_invocation() -> Result<()> {
    let mut builder = test_orbiterx().with_config(|config| {
        let policy_path = config.orbiterx_home.join("rules").join("policy.rules");
        fs::create_dir_all(
            policy_path
                .parent()
    ...
    let EventMsg::ExecCommandEnd(end) = wait_for_event(&test.orbiterx, |event| {
...
"#,
        ...
        .with_config(|config| {
            config
                ...
                .enable(Feature::UnifiedExec)
                .expect("test config should allow feature update");
            let policy_path = config.orbiterx_home.join("rules").join("broken.rules");
            fs::create_dir_all(
                policy_path
                    .parent()
                    .expect("policy directory must have a parent"),
