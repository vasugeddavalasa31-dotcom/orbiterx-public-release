...
const FIXED_CWD: &str = "/tmp/orbiterx_remote_compaction_parity_workspace";
...
impl AuthCase {
    fn build(self) -> OrbiterXAuth {
        match self {
            AuthCase::ChatGpt => OrbiterXAuth::create_dummy_chatgpt_auth_for_testing(),
            AuthCase::ApiKey => OrbiterXAuth::from_api_key("dummy"),
        }
    }
}
...
async fn run_manual_session(
    ...
    for (idx, step) in scenario.steps.iter().enumerate() {
        submit_user_input(&orbiterx, user_input_for_step(scenario.name, idx, *step)).await?;
    }
...
async fn run_manual_hook_session(mode: Mode) -> Result<Value> {
    ...
    let home = harness.test().orbiterx_home_path();
...
async fn build_auto_harness(mode: Mode) -> Result<TestOrbiterXHarness> {
    build_harness_inner(
        mode,
        RunSettings::default(),
        /*hooks*/ false,
...
async fn build_harness_inner(
    ...
    TestOrbiterXHarness::with_builder(builder.with_config(move |config| {
        config.cwd = orbiterx_utils_absolute_path::AbsolutePathBuf::from_absolute_path(
...
fn rollout_path(harness: &TestOrbiterXHarness) -> PathBuf {
    harness
        .test()
        .session_configured
        .rollout_path
...
async fn capture_from_requests(
    mode: Mode,
    orbiterx: &orbiterx_core::OrbiterXThread,
    rollout_path: &Path,
...
fn normalize_tmp_prefix_before_marker(text: &mut String, marker: &str) {
    ...
    while let Some(relative_marker_index) = text[search_start..].find(marker) {
        ...
        if let Some(start_index) = start {
            text.replace_range(start_index..marker_index, "<ORBITERX_HOME>");
            search_start = start_index + "<ORBITERX_HOME>".len() + marker.len();
        } else {
...
fn normalize_string_rewrites_linux_temp_skill_paths() {
    ...
    assert_eq!(
        text,
        "file: <ORBITERX_HOME>/skills/.system/imagegen/SKILL.md and \
         <ORBITERX_HOME>/skills/custom/SKILL.md"
    );
}
