...
async fn user_turn_includes_skill_instructions() -> Result<()> {
    ...
    let mut builder = test_orbiterx().with_workspace_setup(move |cwd, fs| async move {
    ...
    core_test_support::wait_for_event(test.orbiterx.as_ref(), |event| {
        matches!(
            event,
            orbiterx_protocol::protocol::EventMsg::TurnComplete(_)
        )
    })
    .await;
