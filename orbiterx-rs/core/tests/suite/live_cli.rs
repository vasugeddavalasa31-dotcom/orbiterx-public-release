...
//! `just test -p orbiterx-core --test all --run-ignored only live_cli` provided they set a valid
//! `OPENAI_API_KEY`.
...
fn run_live(prompt: &str) -> (assert_cmd::assert::Assert, TempDir) {
    ...
    let orbiterx_home = home.path().join(".orbiterx");
    ...
    let mut cmd = Command::new(orbiterx_utils_cargo_bin::cargo_bin("orbiterx-rs").unwrap());
    ...
    let mut child = cmd.spawn().expect("failed to spawn orbiterx-rs");
