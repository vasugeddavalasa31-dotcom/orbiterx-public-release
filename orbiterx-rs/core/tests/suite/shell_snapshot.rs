...
const POLICY_PATH_FOR_TEST: &str = "/orbiterx/policy/path";
const SNAPSHOT_PATH_FOR_TEST: &str = "/orbiterx/snapshot/path";
const SNAPSHOT_MARKER_VAR: &str = "ORBITERX_SNAPSHOT_POLICY_MARKER";
...
async fn wait_for_snapshot(orbiterx_home: &Path) -> Result<PathBuf> {
    let snapshot_dir = orbiterx_home.join("shell_snapshots");
...
async fn run_snapshot_command_with_options(
    ...
    let builder = test_orbiterx().with_config(move |config| {
    ...
    let orbiterx_home = test.home.path().to_path_buf();
    ...
    let begin = wait_for_event_match(&orbiterx, |ev| match ev {
        EventMsg::ExecCommandBegin(ev) if ev.call_id == call_id => Some(ev.clone()),
        _ => None,
    })
    .await;
    let snapshot_path = wait_for_snapshot(&orbiterx_home).await?;
    let snapshot_content = fs::read_to_string(&snapshot_path).await?;

    let end = wait_for_event_match(&orbiterx, |ev| match ev {
...
async fn run_tool_turn_on_harness(
    ...
    wait_for_event_match(&orbiterx, |ev| match ev {
        EventMsg::ExecCommandBegin(ev) if ev.call_id == call_id => Some(ev.clone()),
        _ => None,
    })
    .await;
...
async fn linux_unified_exec_uses_shell_snapshot() -> Result<()> {
    ...
    assert!(run.snapshot_path.starts_with(&run.orbiterx_home));
...
async fn shell_command_snapshot_preserves_shell_environment_policy_set() -> Result<()> {
    ...
    let harness = TestOrbiterXHarness::with_builder(builder).await?;
    let orbiterx_home = harness.test().home.path().to_path_buf();
    ...
    assert!(snapshot_path.starts_with(orbiterx_home));
...
async fn shell_command_snapshot_still_intercepts_apply_patch() -> Result<()> {
    ...
    wait_for_event(&orbiterx, |ev| match ev {
...
async fn shell_snapshot_deleted_after_shutdown_with_skills() -> Result<()> {
    ...
    let orbiterx_home = home.path().to_path_buf();
