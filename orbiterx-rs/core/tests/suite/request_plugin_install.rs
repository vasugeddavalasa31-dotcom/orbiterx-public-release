...
use orbiterx_protocol::approvals::ElicitationAction;
...
const CALENDAR_NAMESPACE: &str = "mcp__orbiterx_apps__calendar";
...
async fn start_install_turn(test: &TestOrbiterX, prompt: &str) -> Result<ElicitationRequestEvent> {
    ...
    Ok(wait_for_event_match(&test.orbiterx, |event| match event {
        ...
        if let Some(event) = requests
            ...
            .find_map(|request| {
                ...
                payload["events"].as_array().and_then(|events| {
                    events
                        .iter()
                        .find(|event| event["event_type"] == "orbiterx_plugin_install_requested")
                        .cloned()
                })
            })
        {
    ...
    assert_eq!(
        ...
        json!({
            ...
            "event_params": {
                ...
                "turn_id": turn_id,
                "model_slug": "gpt-5.4",
                "product_client_id": orbiterx_login::default_client::originator().value,
            }
        })
    );
