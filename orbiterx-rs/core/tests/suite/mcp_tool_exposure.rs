...
async fn session_resource_client_follows_published_mcp_runtime() -> Result<()> {
    ...
    let test = core_test_support::test_orbiterx::test_orbiterx()
...
async fn code_mode_only_exposes_direct_model_only_mcp_namespaces() -> Result<()> {
    ...
    assert!(
        exec_description.is_some_and(|description| {
            !description.contains("mcp__orbiterx_apps__calendar_create_event(args:")
        }),
...
async fn apps_guidance_appears_after_background_recovery_within_a_turn() -> Result<()> {
    ...
    let test = builder.build(&server).await?;
    let mcp_runtime = test.orbiterx.current_mcp_runtime().await;
...
-> Result<()> {
    ...
    let test = builder.build(&server).await?;
    wait_for_mcp_server(&test.orbiterx, ORBITERX_APPS_MCP_SERVER_NAME).await?;
    ...
    tokio::fs::remove_dir_all(test.orbiterx_home_path().join("cache/orbiterx_apps_tools")).await?;
    startup_control.fail_next_initialize_attempts(/*attempts*/ 1);
    let runtime_mcp_config = test.orbiterx.runtime_mcp_config(&test.config).await;
    let refresh_config = McpServerRefreshConfig {
        mcp_servers: serde_json::to_value(orbiterx_mcp::configured_mcp_servers(
            &runtime_mcp_config,
        ))?,
        mcp_oauth_credentials_store_mode: serde_json::to_value(
