...
//! request payload that OrbiterX would send to the model and assert that the
...
fn network_disabled() -> bool {
    std::env::var(ORBITERX_SANDBOX_NETWORK_DISABLED_ENV_VAR).is_ok()
}
...
async fn start_test_conversation(
    ...
    (test.home, test.config, test.thread_manager, test.orbiterx)
}

async fn user_turn(conversation: &Arc<OrbiterXThread>, text: &str) {
    conversation
        .submit(Op::UserInput {
...
async fn compact_conversation(conversation: &Arc<OrbiterXThread>) {
    conversation
        .submit(Op::Compact)
        .await
...
fn fetch_conversation_path(conversation: &Arc<OrbiterXThread>) -> std::path::PathBuf {
    conversation.rollout_path().expect("rollout path")
}

async fn shutdown_conversation(conversation: &Arc<OrbiterXThread>) {
    conversation
        .shutdown_and_wait()
        .await
...
async fn resume_conversation(
    ...
    let auth_manager = orbiterx_core::test_support::auth_manager_from_auth(
        orbiterx_login::OrbiterXAuth::from_api_key("dummy"),
    );
