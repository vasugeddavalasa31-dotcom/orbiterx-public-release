...
use orbiterx_core::REVIEW_PROMPT;
...
use orbiterx_protocol::protocol::ENVIRONMENT_CONTEXT_OPEN_TAG;
...
use orbiterx_protocol::protocol::ReviewCodeLocation;
use orbiterx_protocol::protocol::ReviewFinding;
use orbiterx_protocol::protocol::ReviewLineRange;
use orbiterx_protocol::protocol::ReviewOutputEvent;
...
use orbiterx_protocol::review_format::render_review_output_text;
...
async fn review_op_emits_lifecycle_and_review_output() {
    // Skip under OrbiterX sandbox network restrictions.
    skip_if_no_network!();
    ...
    let orbiterx = new_conversation_for_server(&server, orbiterx_home.clone(), |_| {}).await;
    ...
    let entered_started = wait_for_event(&orbiterx, |ev| {
        matches!(
            ev,
    ...
    let entered_completed = wait_for_event(&orbiterx, |ev| {
        matches!(
            ev,
    ...
    let entered =
        wait_for_event(&orbiterx, |ev| matches!(ev, EventMsg::EnteredReviewMode(_))).await;
    match entered {
    ...
    let exited_started = wait_for_event(&orbiterx, |ev| {
        matches!(
            ev,
            EventMsg::ItemStarted(event)
    ...
    let exited_completed = wait_for_event(&orbiterx, |ev| {
        matches!(
            ev,
            EventMsg::ItemCompleted(event)
                if matches!(event.item, TurnItem::ExitedReviewMode(_))
    ...
    let closed = wait_for_event(&orbiterx, |ev| matches!(ev, EventMsg::ExitedReviewMode(_))).await;
    ...
    let _complete = wait_for_event(&orbiterx, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;

    let path = orbiterx.rollout_path().expect("rollout path");
...
async fn cancelled_review_does_not_forward_delegate_mcp_startup() {
    ...
    tokio::time::timeout(Duration::from_secs(5), async {
        loop {
            match orbiterx.next_event().await.expect("review event").msg {
    ...
    orbiterx.submit(Op::Interrupt).await.unwrap();
...
async fn review_uses_custom_review_model_from_config() {
    ...
    let orbiterx = new_conversation_for_server(&server, orbiterx_home.clone(), |cfg| {
    ...
    let _entered =
        wait_for_event(&orbiterx, |ev| matches!(ev, EventMsg::EnteredReviewMode(_))).await;
    let _closed = wait_for_event(&orbiterx, |ev| {
        matches!(
            ev,
...
async fn review_input_isolated_from_parent_history() {
    ...
    let session_file = orbiterx_home.path().join("resume.jsonl");
    {
        ...
        let user = orbiterx_protocol::models::ResponseItem::Message {
            id: None,
            role: "user".to_string(),
        ...
        let assistant = orbiterx_protocol::models::ResponseItem::Message {
            id: None,
    ...
    let orbiterx = resume_conversation_for_server(
        &server,
        orbiterx_home.clone(),
        session_file.clone(),
        |_| {},
...
async fn review_uses_overridden_cwd_for_base_branch_merge_base() {
    ...
    let orbiterx = new_conversation_for_server(&server, orbiterx_home.clone(), move |config| {
    ...
    F: FnOnce(&mut Config) + Send + 'static,
{
    ...
    builder
        .resume(server, orbiterx_home, resume_path)
        .await
