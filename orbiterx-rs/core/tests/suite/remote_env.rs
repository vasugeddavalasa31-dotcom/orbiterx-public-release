...
use orbiterx_exec_server::NoiseChannelPublicKey;
use orbiterx_exec_server::NoiseRendezvousConnectBundle;
use orbiterx_exec_server::NoiseRendezvousConnectProvider;
use orbiterx_exec_server::REMOTE_ENVIRONMENT_ID;
...
use orbiterx_protocol::protocol::ENVIRONMENTS_INSTRUCTIONS_OPEN_TAG;
...
async fn unified_exec_test(server: &wiremock::MockServer) -> Result<TestOrbiterX> {
...
async fn submit_turn_with_approval_and_environments(
    ...
    let turn_environment_selections = orbiterx_protocol::protocol::TurnEnvironmentSelections::new(
...
async fn expect_patch_approval(
    ...
    let event = wait_for_event(&test.orbiterx, |event| {
        matches!(
            event,
...
async fn wait_for_completion_without_patch_approval(test: &TestOrbiterX) {
    let event = wait_for_event(&test.orbiterx, |event| {
        matches!(
            event,
            EventMsg::ApplyPatchApprovalRequest(_) | EventMsg::TurnComplete(_)
...
async fn remote_test_env_exposes_target_shell_to_model() -> Result<()> {
    ...
    .await;
    let test = test_orbiterx().build_with_auto_env(&server).await?;
...
async fn explicit_remote_shell_runs_in_remote_cwd() -> Result<()> {
    ...
    let (shell, command) = match test_target_os() {
        TestTargetOs::Linux => (
            "bash",
            r#"case "$PWD" in /tmp/orbiterx-core-test-cwd-*) ;; *) echo "unexpected cwd: $PWD" >&2; exit 1 ;; esac"#,
        ),
        TestTargetOs::Windows => (
            "powershell",
            r#"$cwd = (Get-Location).Path; if ($cwd -notlike 'C:\orbiterx-core-test-cwd-*') { Write-Error "unexpected cwd: $cwd"; exit 1 }"#,
        ),
        TestTargetOs::MacOs => unreachable!("remote test targets do not run macOS"),
    };
...
async fn settings_update_does_not_retarget_active_turn_environment() -> Result<()> {
    ...
    test.orbiterx
        ...
        .await?;
    let request = wait_for_event_match(&test.orbiterx, |event| match event {
...
async fn deferred_executor_loads_agents_md_when_environment_becomes_ready() -> Result<()> {
    ...
    assert_eq!(test.orbiterx.instruction_sources().await, vec![agents_path]);
...
async fn deferred_executor_compaction_preserves_then_updates_environment_once() -> Result<()> {
    ...
    test.orbiterx.ensure_rollout_materialized().await;
    test.orbiterx.flush_rollout().await?;
    let rollout_path = test.orbiterx.rollout_path().context("rollout path")?;
...
async fn remote_exec_materializes_target_roots_before_sandbox_selection() -> Result<()> {
    ...
    let local_cwd = TempDir::new()?;
    let remote_cwd = PathBuf::from(format!(
        "/tmp/orbiterx-remote-target-roots-{}",
        SystemTime::now().duration_since(UNIX_EPOCH)?.as_millis()
    ))
    .abs();
...
async fn remote_request_permissions_grant_unblocks_later_remote_exec() -> Result<()> {
    ...
    let local_cwd = TempDir::new()?;
    let remote_cwd = PathBuf::from(format!(
        "/tmp/orbiterx-remote-request-permissions-{}",
        SystemTime::now().duration_since(UNIX_EPOCH)?.as_millis()
    ))
    .abs();
...
async fn apply_patch_freeform_routes_to_selected_remote_environment() -> Result<()> {
    ...
    let remote_cwd = PathBuf::from(format!(
        "/tmp/orbiterx-remote-apply-patch-freeform-{}",
        SystemTime::now().duration_since(UNIX_EPOCH)?.as_millis()
    ))
    .abs();
    let remote_cwd_uri = PathUri::from_host_native_path(&remote_cwd)?;
...
async fn apply_patch_approvals_are_remembered_per_environment() -> Result<()> {
    ...
    let local_cwd = TempDir::new()?;
    let remote_cwd = PathBuf::from(format!(
        "/tmp/orbiterx-remote-apply-patch-approval-cwd-{}",
        SystemTime::now().duration_since(UNIX_EPOCH)?.as_millis()
    ))
    .abs();
    let remote_cwd_uri = PathUri::from_host_native_path(&remote_cwd)?;
    ...
    let target_path = PathBuf::from(format!(
        "/tmp/orbiterx-apply-patch-approval-scope-{}.txt",
        SystemTime::now().duration_since(UNIX_EPOCH)?.as_millis()
    ))
    .abs();
...
async fn apply_patch_intercepted_exec_command_routes_to_selected_remote_environment() -> Result<()>
{
    ...
    let remote_cwd = PathBuf::from(format!(
        "/tmp/orbiterx-remote-apply-patch-exec-{}",
        SystemTime::now().duration_since(UNIX_EPOCH)?.as_millis()
    ))
    .abs();
    let remote_cwd_uri = PathUri::from_host_native_path(&remote_cwd)?;
...
async fn remote_test_env_remove_removes_symlink_not_target() -> Result<()> {
    ...
    let root = PathBuf::from(format!(
        "/tmp/orbiterx-remote-remove-link-{}",
        std::process::id()
    ));
    let allowed_dir = root.join("allowed");
