...
const SAMPLE_PLUGIN_APP_NAMESPACE: &str = "mcp__orbiterx_apps__google_calendar";
...
async fn build_analytics_plugin_test_orbiterx(
    server: &MockServer,
...
async fn build_apps_enabled_plugin_test_orbiterx(
    server: &MockServer,
    orbiterx_home: Arc<TempDir>,
...
async fn capability_sections_render_in_developer_message_in_order() -> Result<()> {
    ...
    write_plugin_skill_plugin(orbiterx_home.as_ref());
    write_plugin_app_plugin(orbiterx_home.as_ref());
    let test_orbiterx = build_apps_enabled_plugin_test_orbiterx(
        &server,
        Arc::clone(&orbiterx_home),
        apps_server.chatgpt_base_url,
    )
    .await?;
    let orbiterx = Arc::clone(&test_orbiterx.orbiterx);

    orbiterx
        .submit(Op::UserInput {
            items: vec![orbiterx_protocol::user_input::UserInput::Text {
                text: "hello".into(),
...
async fn explicit_plugin_mentions_use_apps_for_chatgpt_dual_surface_plugins() -> Result<()> {
    ...
    write_plugin_skill_plugin(orbiterx_home.as_ref());
    write_plugin_mcp_plugin(orbiterx_home.as_ref(), &rmcp_test_server_bin);
    write_plugin_app_plugin(orbiterx_home.as_ref());
    ...
    .await?;
    let orbiterx = Arc::clone(&test_orbiterx.orbiterx);
    wait_for_mcp_server(&orbiterx, ORBITERX_APPS_MCP_SERVER_NAME).await?;

    orbiterx
        .submit(Op::UserInput {
            items: vec![orbiterx_protocol::user_input::UserInput::Mention {
                name: "sample".into(),
...
async fn explicit_plugin_mentions_keep_non_conflicting_mcp_for_chatgpt_auth() -> Result<()> {
    ...
    write_plugin_skill_plugin(orbiterx_home.as_ref());
    write_plugin_mcp_plugin(orbiterx_home.as_ref(), &rmcp_test_server_bin);
    write_plugin_app_plugin_with_name(orbiterx_home.as_ref(), "sample_app");
    ...
    .await?;
    let orbiterx = Arc::clone(&test_orbiterx.orbiterx);
    wait_for_mcp_server(&orbiterx, "sample").await?;
...
async fn explicit_plugin_mentions_track_plugin_used_analytics() -> Result<()> {
    ...
    let orbiterx_home = Arc::new(TempDir::new()?);
    write_plugin_skill_plugin(orbiterx_home.as_ref());
    let test_orbiterx = build_analytics_plugin_test_orbiterx(&server, orbiterx_home).await?;
    let orbiterx = Arc::clone(&test_orbiterx.orbiterx);
    ...
    let plugin_event = loop {
        ...
        if let Some(event) = requests
            .into_iter()
            .filter(|request| request.url.path() == "/orbiterx/analytics-events/events")
            .find_map(|request| {
                let payload: serde_json::Value = serde_json::from_slice(&request.body).ok()?;
                payload["events"].as_array().and_then(|events| {
                    events
                        .iter()
                        .find(|event| event["event_type"] == "orbiterx_plugin_used")
                        .cloned()
                })
            })
        {
    ...
    assert_eq!(
        ...
        serde_json::json!(orbiterx_login::default_client::originator().value)
    );
