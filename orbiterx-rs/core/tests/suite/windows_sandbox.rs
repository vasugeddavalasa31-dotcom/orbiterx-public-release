...
enum TestOrbiterXHome {
    Persistent(PathBuf),
    Temporary(TempDir),
}

impl TestOrbiterXHome {
    fn path(&self) -> &Path {
        match self {
...
fn orbiterx_home_for_windows_sandbox_test(name: &str) -> anyhow::Result<TestOrbiterXHome> {
    if let Some(test_tmpdir) = std::env::var_os("TEST_TMPDIR") {
        // The elevated backend provisions machine-local sandbox users. Bazel
        // retries run in the same Windows VM, so keep ORBITERX_HOME stable within
        // the test temp root and let setup reconcile its persisted ACL state.
        let orbiterx_home = PathBuf::from(test_tmpdir).join(name);
        std::fs::create_dir_all(&orbiterx_home).with_context(|| {
            format!(
                "create stable test ORBITERX_HOME {}",
                orbiterx_home.display()
            )
        })?;
        return Ok(TestOrbiterXHome::Persistent(orbiterx_home));
    }

    Ok(TestOrbiterXHome::Temporary(TempDir::new()?))
}
...
fn stage_windows_sandbox_helpers() -> anyhow::Result<()> {
    ...
    let resources_dir = test_exe_dir.join("orbiterx-resources");
    ...
    for helper_name in ["orbiterx-windows-sandbox-setup", "orbiterx-command-runner"] {
        let helper = orbiterx_utils_cargo_bin::cargo_bin(helper_name)?;
...
async fn windows_restricted_token_rejects_exact_and_glob_deny_read_policy() -> anyhow::Result<()> {
    let orbiterx_home =
        orbiterx_home_for_windows_sandbox_test("windows-restricted-token-deny-read-orbiterx-home")?;
...
async fn windows_elevated_enforces_deny_read_and_protects_setup_marker() -> anyhow::Result<()> {
    let orbiterx_home =
        orbiterx_home_for_windows_sandbox_test("windows-elevated-deny-read-orbiterx-home")?;
    let _orbiterx_home_guard = EnvVarGuard::set("ORBITERX_HOME", orbiterx_home.path().as_os_str());
    ...
    let public = cwd.join("public.txt");
    let setup_marker = orbiterx_home
        .path()
        .join(".sandbox")
    ...
    assert!(
        sandbox_setup_is_complete(orbiterx_home.path()),
