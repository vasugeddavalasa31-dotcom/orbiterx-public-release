...
use orbiterx_core::TimeProvider;
use orbiterx_core::config::Config;
use orbiterx_core::resolve_installation_id;
use orbiterx_core::shell::Shell;
use orbiterx_core::shell::get_shell_by_model_provided_path;
use orbiterx_core::thread_store_from_config;
use orbiterx_exec_server::CreateDirectoryOptions;
use orbiterx_exec_server::ExecutorFileSystem;
use orbiterx_exec_server::RemoveOptions;
...
use orbiterx_protocol::protocol::RealtimeConversationVersion as RealtimeWsVersion;
...
type WorkspaceSetup = dyn FnOnce(AbsolutePathBuf, Arc<dyn ExecutorFileSystem>) -> BoxFuture<'static, Result<()>>
    + Send;
const TEST_MODEL_WITH_EXPERIMENTAL_TOOLS: &str = "test-gpt-5.1-orbiterx";
const REMOTE_EXEC_SERVER_URL_ENV_VAR: &str = "ORBITERX_TEST_REMOTE_EXEC_SERVER_URL";
...
#[derive(Debug)]
pub struct TestEnv {
    environment: orbiterx_exec_server::Environment,
...
impl TestEnv {
    ...
    pub async fn local_with_exec_server_url(exec_server_url: Option<String>) -> Result<Self> {
        ...
        let selection = match exec_server_url {
            Some(_) => TurnEnvironmentSelection {
                environment_id: orbiterx_exec_server::REMOTE_ENVIRONMENT_ID.to_string(),
                cwd: PathUri::from_abs_path(&cwd),
                workspace_roots: vec![PathUri::from_abs_path(&cwd)],
            },
        ...
        let environment =
            orbiterx_exec_server::Environment::create_for_tests(exec_server_url.clone())?;
        Ok(Self {
            environment,
            exec_server_url,
            cwd,
    ...
    pub fn environment(&self) -> &orbiterx_exec_server::Environment {
        &self.environment
    }
...
pub async fn test_env() -> Result<TestEnv> {
    match test_environment() {
        remote_env @ (TestEnvironment::Docker { .. } | TestEnvironment::WineExec) => {
            ...
            let environment =
                orbiterx_exec_server::Environment::create_for_tests(Some(websocket_url.clone()))?;
            let cwd = remote_env
...
pub struct TestOrbiterXBuilder {
...
impl TestOrbiterXBuilder {
    ...
    pub fn with_auth(mut self, auth: OrbiterXAuth) -> Self {
        self.auth = auth;
        self
    }
    ...
    pub async fn build(&mut self, server: &wiremock::MockServer) -> anyhow::Result<TestOrbiterX> {
    ...
    ///
    /// With no remote test configuration, or with `ORBITERX_TEST_ENVIRONMENT=local`, this uses a
    /// temporary local environment just like [`Self::build`]. `ORBITERX_TEST_ENVIRONMENT=docker` or
    /// `ORBITERX_TEST_ENVIRONMENT=wine-exec` selects the remote exec server configured by
    /// `ORBITERX_TEST_REMOTE_EXEC_SERVER_URL`; the legacy `ORBITERX_TEST_REMOTE_ENV` Docker-container
    ...
    async fn build_with_home_and_base_url(
        ...
        #[cfg(target_os = "linux")]
        let orbiterx_linux_sandbox_exe = Some(
            crate::find_orbiterx_linux_sandbox_exe()
                .context("should find binary for orbiterx-linux-sandbox")?,
        );
        #[cfg(not(target_os = "linux"))]
        let orbiterx_linux_sandbox_exe = None;
        let local_runtime_paths = orbiterx_exec_server::ExecServerRuntimePaths::new(
            std::env::current_exe()?,
            orbiterx_linux_sandbox_exe,
        )?;
        let environment_manager = Arc::new(if include_local_environment {
            orbiterx_exec_server::EnvironmentManager::create_for_tests_with_local(
                exec_server_url,
                local_runtime_paths,
            )
            .await
        } else {
            orbiterx_exec_server::EnvironmentManager::create_for_tests(
                exec_server_url,
    ...
    async fn build_from_config(
        ...
        resume_from: Option<PathBuf>,
        test_env: TestEnv,
        environment_manager: Arc<orbiterx_exec_server::EnvironmentManager>,
    ) -> anyhow::Result<TestOrbiterX> {
        let auth = self.auth.clone();
        let state_db = orbiterx_core::init_state_db(&config).await;
        let thread_store = thread_store_from_config(&config, state_db.clone());
        let installation_id = resolve_installation_id(&config.orbiterx_home).await?;
        let user_instructions_provider =
            self.user_instructions_provider.clone().unwrap_or_else(|| {
                Arc::new(OrbiterXHomeUserInstructionsProvider::new(
                    config.orbiterx_home.clone(),
                ))
            });
        let auth_manager = orbiterx_core::test_support::auth_manager_from_auth(auth.clone());
        let thread_manager = ThreadManager::new(
            ...
            thread_store,
            orbiterx_core::local_agent_graph_store_from_state_db(state_db.as_ref()),
            installation_id,
        ...
        let code_mode_host_program = self
            ...
            .take()
            .or_else(|| orbiterx_utils_cargo_bin::cargo_bin("orbiterx-code-mode-host").ok());
        let thread_manager = if config.features.enabled(Feature::CodeModeHost)
            && let Some(code_mode_host_program) = code_mode_host_program
        {
            orbiterx_core::test_support::with_code_mode_host_program(
                thread_manager,
                code_mode_host_program,
            )
        } else {
        ...
        let new_conversation = match (resume_from, user_shell_override) {
            (Some(path), Some(user_shell_override)) => {
                ...
                Box::pin(
                    orbiterx_core::test_support::resume_thread_from_rollout_with_user_shell_override(
            ...
            (None, Some(user_shell_override)) => {
                Box::pin(
                    orbiterx_core::test_support::start_thread_with_user_shell_override(
                        thread_manager.as_ref(),
                        config.clone(),
                        user_shell_override,
                        self.supports_openai_form_elicitation,
        ...
        Ok(TestOrbiterX {
            ...
            cwd,
            config,
            orbiterx: new_conversation.thread,
    ...
    async fn prepare_config(
        ...
        if let Ok(path) = orbiterx_utils_cargo_bin::cargo_bin("orbiterx") {
            config.orbiterx_self_exe = Some(path);
        } else if let Ok(path) = orbiterx_utils_cargo_bin::cargo_bin("orbiterx-exec") {
            // `orbiterx-exec` also supports `--orbiterx-run-as-apply-patch`, so use it
        ...
        {
            let orbiterx = bin_dir.join("orbiterx");
            let orbiterx_exec = bin_dir.join("orbiterx-exec");
            if orbiterx.is_file() {
                config.orbiterx_self_exe = Some(orbiterx);
            } else if orbiterx_exec.is_file() {
                config.orbiterx_self_exe = Some(orbiterx_exec);
            }
        }
...
pub struct TestOrbiterX {
    pub home: Arc<TempDir>,
    pub cwd: Arc<TempDir>,
    pub orbiterx: Arc<OrbiterXThread>,
...
impl TestOrbiterX {
    ...
    pub fn orbiterx_home_path(&self) -> &Path {
        self.config.orbiterx_home.as_path()
    }
    ...
    async fn submit_turn_with_context(
        ...
        let turn_id = wait_for_event_match(&self.orbiterx, |event| match event {
...
pub struct TestOrbiterXHarness {
    server: MockServer,
    test: TestOrbiterX,
}

impl TestOrbiterXHarness {
    pub async fn new() -> Result<Self> {
        Self::with_builder(test_orbiterx()).await
    }

    pub async fn with_config(mutator: impl FnOnce(&mut Config) + Send + 'static) -> Result<Self> {
        Self::with_builder(test_orbiterx().with_config(mutator)).await
    }

    pub async fn with_builder(mut builder: TestOrbiterXBuilder) -> Result<Self> {
        let server = start_mock_server().await;
        let test = builder.build(&server).await?;
        Ok(Self { server, test })
    }

    pub async fn with_auto_env_builder(mut builder: TestOrbiterXBuilder) -> Result<Self> {
        let server = start_mock_server().await;
...
pub fn test_orbiterx() -> TestOrbiterXBuilder {
    TestOrbiterXBuilder {
        ...
        auth: OrbiterXAuth::from_api_key("dummy"),
        pre_build_hooks: vec![],
