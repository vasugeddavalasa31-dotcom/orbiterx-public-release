...
use crate::test_orbiterx::TestOrbiterX;
...
pub fn zsh_fork_runtime(test_name: &str) -> Result<Option<ZshForkRuntime>> {
    ...
    let Ok(main_execve_wrapper_exe) =
        orbiterx_utils_cargo_bin::cargo_bin("orbiterx-execve-wrapper")
    else {
        eprintln!("skipping {test_name}: unable to resolve `orbiterx-execve-wrapper` binary");
        return Ok(None);
    };
...
pub fn zsh_fork_test_builder(
    ...
    test_orbiterx().with_config(move |config| {
