...
use orbiterx_utils_cargo_bin::CargoBinError;
...
pub use orbiterx_core::test_support::TestOrbiterXResponsesRequestKind;
pub use orbiterx_core::test_support::responses_metadata;
use orbiterx_utils_absolute_path::AbsolutePathBuf;
pub use orbiterx_utils_absolute_path::test_support::PathBufExt;
pub use orbiterx_utils_absolute_path::test_support::PathExt;
...
pub mod test_orbiterx;
pub mod test_orbiterx_exec;
pub mod tracing;
pub mod zsh_fork;
...
fn enable_deterministic_unified_exec_process_ids_for_tests() {
    orbiterx_core::test_support::set_thread_manager_test_mode(/*enabled*/ true);
    orbiterx_core::test_support::set_deterministic_process_ids(/*enabled*/ true);
}

#[ctor]
fn configure_arg0_dispatch_for_test_binaries() {
    let _ = TEST_ARG0_PATH_ENTRY.get_or_init(orbiterx_arg0::arg0_dispatch);
}
...
fn configure_insta_workspace_root_for_snapshot_tests() {
    ...
    let workspace_root = orbiterx_utils_cargo_bin::repo_root()
        .ok()
        .map(|root| root.join("orbiterx-rs"));
...
pub fn test_tmp_path() -> AbsolutePathBuf {
    test_absolute_path_with_windows("/tmp", Some(r"C:\Users\orbiterx\AppData\Local\Temp"))
}
...
/// avoids clobbering a developer’s real `~/.orbiterx`.
pub async fn load_default_config_for_test(orbiterx_home: &TempDir) -> Config {
...
fn default_test_overrides() -> ConfigOverrides {
    ConfigOverrides {
        orbiterx_linux_sandbox_exe: Some(
            find_orbiterx_linux_sandbox_exe()
                .expect("should find binary for orbiterx-linux-sandbox"),
        ),
...
#[cfg(target_os = "linux")]
pub fn find_orbiterx_linux_sandbox_exe() -> Result<PathBuf, CargoBinError> {
    ...
    orbiterx_utils_cargo_bin::cargo_bin("orbiterx-linux-sandbox")
}

pub async fn wait_for_event<F>(
    orbiterx: &OrbiterXThread,
    predicate: F,
) -> orbiterx_protocol::protocol::EventMsg
where
    F: FnMut(&orbiterx_protocol::protocol::EventMsg) -> bool,
{
    use tokio::time::Duration;
    wait_for_event_with_timeout(orbiterx, predicate, Duration::from_secs(1)).await
}
...
pub async fn submit_thread_settings(
    ...
    let submission_id = orbiterx
        ...
        .await?;
    loop {
        let ev = timeout(Duration::from_secs(10), orbiterx.next_event())
            .await
...
pub async fn wait_for_event_match<T, F>(orbiterx: &OrbiterXThread, matcher: F) -> T
where
    F: Fn(&orbiterx_protocol::protocol::EventMsg) -> Option<T>,
{
    let ev = wait_for_event(orbiterx, |ev| matcher(ev).is_some()).await;
...
pub fn sandbox_env_var() -> &'static str {
    orbiterx_core::spawn::ORBITERX_SANDBOX_ENV_VAR
}

pub fn sandbox_network_env_var() -> &'static str {
    orbiterx_core::spawn::ORBITERX_SANDBOX_NETWORK_DISABLED_ENV_VAR
}

pub fn format_with_current_shell(command: &str) -> Vec<String> {
    orbiterx_core::shell::default_user_shell()
...
pub fn stdio_server_bin() -> Result<String, CargoBinError> {
    orbiterx_utils_cargo_bin::cargo_bin("test_stdio_server")
...
#[macro_export]
macro_rules! orbiterx_linux_sandbox_exe_or_skip {
    () => {{
        #[cfg(target_os = "linux")]
        {
            match $crate::find_orbiterx_linux_sandbox_exe() {
                Ok(path) => Some(path),
                Err(err) => {
                    eprintln!("orbiterx-linux-sandbox binary not available, skipping test: {err}");
                    return;
                }
            }
        }
