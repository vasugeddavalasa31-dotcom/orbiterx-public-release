...
async fn responses_stream_includes_subagent_header_on_review() {
    ...
    assert_eq!(
        request.header("x-orbiterx-window-id").as_deref(),
        Some(expected_window_id.as_str())
    );
    assert_eq!(request.header("x-orbiterx-parent-thread-id"), None);
    assert_eq!(
        request.body_json()["client_metadata"]["x-orbiterx-installation-id"].as_str(),
        Some(TEST_INSTALLATION_ID)
    );
    assert_eq!(
        request.body_json()["client_metadata"]["x-orbiterx-window-id"].as_str(),
        Some(expected_window_id.as_str())
    );
    assert_eq!(request.header("x-orbiterx-sandbox"), None);
}
...
async fn responses_respects_model_info_overrides_from_config() {
    ...
    let thread_id = ThreadId::new();
    let auth_mode = orbiterx_core::test_support::auth_manager_from_auth(
...
async fn responses_stream_includes_turn_metadata_header_for_git_workspace_e2e() {
    ...
    let initial_header = first_request
        ...
        .expect("x-orbiterx-turn-metadata header should be present");
    let initial_parsed: serde_json::Value = serde_json::from_str(&initial_header)
        .expect("x-orbiterx-turn-metadata should be valid JSON");
    ...
    assert!(
        ...
        "turn_id should not be empty in x-orbiterx-turn-metadata"
    );
    ...
    run_git(&[
        ...
        "add",
        "origin",
        "https://github.com/openai/orbiterx.git",
    ]);
