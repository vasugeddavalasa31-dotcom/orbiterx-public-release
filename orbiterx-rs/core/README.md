...
This crate implements the business logic for OrbiterX. It is designed to be used by the various OrbiterX UIs written in Rust.

## Wine-exec integration tests

On x86-64 Linux, run the shared suite against the Windows exec server with
`bazel test //orbiterx-rs/core:core-all-wine-exec-test`.
...
Note that `orbiterx-core` makes some assumptions about certain helper utilities being available in the environment. Currently, this support matrix is:
...
pointer file), the resolved `gitdir:` target, and `.orbiterx` read-only.
...
Expects the binary containing `orbiterx-core` to run the equivalent of `orbiterx sandbox` when `arg0` is `orbiterx-linux-sandbox`. See the `orbiterx-arg0` crate for details.
...
`bwrap` is missing, it falls back to the bundled `orbiterx-resources/bwrap`
binary shipped with OrbiterX and OrbiterX surfaces a startup warning through its
normal notification path instead of printing directly from the sandbox helper.
OrbiterX also surfaces a startup warning when bubblewrap cannot create user
...
namespaces, so OrbiterX rejects sandboxed shell commands that would enter the
...
Expects the binary containing `orbiterx-core` to simulate the virtual
`apply_patch` CLI when `arg1` is `--orbiterx-run-as-apply-patch`. See the
`orbiterx-arg0` crate for details.
