...
/// Owns the currently published MCP connection set for one OrbiterX thread.
///
...
impl McpRuntimeContext {
    ...
    pub(crate) fn resolve_server_environment(
        &self,
        server_name: &str,
        config: &orbiterx_config::McpServerConfig,
        ...
        if config.is_local_environment() {
            return match config.transport {
                orbiterx_config::McpServerTransportConfig::Stdio { .. } => Err(format!(
                    "local stdio MCP server `{server_name}` requires a local environment"
                )),
                orbiterx_config::McpServerTransportConfig::StreamableHttp { .. } => Ok(None),
            };
        }
