...
//! Session orchestration stays in `orbiterx-core`.

use orbiterx_protocol::mcp::CallToolResult;
use serde::Serialize;

pub const MCP_TOOL_ORBITERX_APPS_META_KEY: &str = "_orbiterx_apps";
...
pub struct OrbiterXAppsConnectorAuthFailure {
...
pub struct OrbiterXAppsAuthElicitation {
...
#[derive(Debug, Clone, PartialEq)]
pub struct OrbiterXAppsAuthElicitationPlan {
    pub auth_failure: OrbiterXAppsConnectorAuthFailure,
    pub elicitation: OrbiterXAppsAuthElicitation,
}

#[derive(Serialize)]
struct OrbiterXAppsConnectorAuthFailureMeta<'a> {
...
pub fn connector_auth_failure_from_tool_result(
...
) -> Option<OrbiterXAppsConnectorAuthFailure> {
    ...
    Some(OrbiterXAppsConnectorAuthFailure {
...
pub fn build_auth_elicitation_plan(
    ...
    connector_name: Option<&str>,
    install_url: Option<String>,
) -> Option<OrbiterXAppsAuthElicitationPlan> {
    let auth_failure =
    ...
    Some(OrbiterXAppsAuthElicitationPlan {
        auth_failure,
        elicitation,
    })
}
...
pub fn build_auth_elicitation(
    call_id: &str,
    auth_failure: &OrbiterXAppsConnectorAuthFailure,
) -> OrbiterXAppsAuthElicitation {
    OrbiterXAppsAuthElicitation {
        meta: serde_json::json!({
            MCP_TOOL_ORBITERX_APPS_META_KEY: {
                CONNECTOR_AUTH_FAILURE_META_KEY: OrbiterXAppsConnectorAuthFailureMeta {
...
pub fn auth_elicitation_id(call_id: &str) -> String {
    format!("orbiterx_apps_auth_{call_id}")
}
...
fn auth_elicitation_message(auth_failure: &OrbiterXAppsConnectorAuthFailure) -> String {
    match auth_failure.auth_reason.as_deref() {
        ...
        Some("missing_link") => format!(
            "Sign in to {} on ChatGPT to use it in OrbiterX.",
            auth_failure.connector_name
        ),
        _ => format!(
