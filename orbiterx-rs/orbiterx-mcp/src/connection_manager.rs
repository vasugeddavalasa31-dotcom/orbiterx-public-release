//! Aggregates MCP server connections for OrbiterX.
//!
...
use crate::rmcp_client::ORBITERX_APPS_REFRESH_DURATION_METRIC;
...
use orbiterx_config::McpServerAuth;
...
use orbiterx_connectors::ConnectorRuntimeContextKey;
use orbiterx_connectors::ConnectorRuntimeFetchSource;
use orbiterx_connectors::ConnectorRuntimeManager;
...
use orbiterx_protocol::protocol::McpStartupCompleteEvent;
use orbiterx_protocol::protocol::McpStartupFailure;
use orbiterx_protocol::protocol::McpStartupFailureReason;
...
use orbiterx_rmcp_client::McpAuthState;
use orbiterx_rmcp_client::McpLoginRequirement;
use orbiterx_rmcp_client::determine_streamable_http_auth_status_from_credentials;
...
impl McpConnectionManager {
    ...
    pub async fn new(
        ...
        orbiterx_apps_tools_cache: ConnectorRuntimeManager<ToolInfo>,
        tool_catalog_cache: McpToolCatalogCache,
        orbiterx_apps_tools_cache_key: ConnectorRuntimeContextKey,
        prefix_mcp_tool_names: bool,
        client_elicitation_capability: ElicitationCapability,
        supports_openai_form_elicitation: bool,
        tool_plugin_provenance: ToolPluginProvenance,
        auth: Option<&OrbiterXAuth>,
        orbiterx_apps_auth_manager: Option<Arc<AuthManager>>,
        ...
        let static_chatgpt_auth_provider = auth
            .filter(|auth| auth.uses_orbiterx_backend())
            .map(orbiterx_model_provider::auth_provider_from_auth);
        let orbiterx_apps_auth_provider = orbiterx_apps_auth_manager.and_then(|auth_manager| {
            auth.filter(|auth| auth.uses_orbiterx_backend())
                .map(|auth| {
                    orbiterx_model_provider::auth_provider_from_auth_manager(auth_manager, auth)
                })
        });
            ...
            .filter(|(_, server)| server.enabled())
        {
            ...
            // For built-in OrbiterX Apps, `ORBITERX_CONNECTORS_TOKEN` is a debug
            ...
            let shares_orbiterx_apps_tools_cache =
                should_share_orbiterx_apps_tools_cache(&server_name, uses_env_bearer_token);
            let orbiterx_apps_tools_cache_context = shares_orbiterx_apps_tools_cache.then(|| {
                orbiterx_apps_tools_cache
                    .context(orbiterx_home.clone(), orbiterx_apps_tools_cache_key.clone())
            });
            // The reserved OrbiterX Apps registration follows the shared
            ...
            let chatgpt_auth_provider = if server_name == ORBITERX_APPS_MCP_SERVER_NAME {
                orbiterx_apps_auth_provider
                    .clone()
                    .or_else(|| static_chatgpt_auth_provider.clone())
            } else {
                static_chatgpt_auth_provider.clone()
            };
            // If OrbiterX Apps has an env bearer token, that is its auth path. Do
            // not also attach the ambient OrbiterXAuth provider.
            let runtime_auth_provider =
                if server_name == ORBITERX_APPS_MCP_SERVER_NAME && uses_env_bearer_token {
                    None
                } else {
                    chatgpt_auth_provider_for_server(&server, chatgpt_auth_provider)
                };
            let tool_catalog_cache_context = if server_name == ORBITERX_APPS_MCP_SERVER_NAME {
                None
    ...
    pub fn tool_approval_mode(
        ...
        server_name: &str,
        tool_name: &str,
    ) -> orbiterx_config::AppToolApproval {
        self.server_metadata
            .get(server_name)
    ...
    pub fn is_host_owned_orbiterx_apps_server(&self, server_name: &str) -> bool {
    ...
    /// Force-refresh orbiterx apps tools by bypassing the in-process cache.
    ///
    ...
    pub async fn hard_refresh_orbiterx_apps_tools_cache(&self) -> Result<Vec<ToolInfo>> {
        let refresh_start = Instant::now();
        let managed_client = self
            .clients
            .get(ORBITERX_APPS_MCP_SERVER_NAME)
            .ok_or_else(|| anyhow!("unknown MCP server '{ORBITERX_APPS_MCP_SERVER_NAME}'"))?
            .client()
            .await
        ...
        let tools = list_tools_for_client_uncached(
            ...
            /*orbiterx_apps_refresh_trigger*/ "explicit",
        ...
        .await
        .with_context(|| {
            format!("failed to refresh tools for MCP server '{ORBITERX_APPS_MCP_SERVER_NAME}'")
        })?;

        let tools =
            match (
                managed_client.orbiterx_apps_tools_cache_context.as_ref(),
                fetch_ticket,
            ) {
    ...
    /// OrbiterX Apps metadata may come from its existing cache; regular MCP server information is
...
fn should_share_orbiterx_apps_tools_cache(server_name: &str, uses_env_bearer_token: bool) -> bool {
    server_name == ORBITERX_APPS_MCP_SERVER_NAME && !uses_env_bearer_token
}
...
fn mcp_init_error_display(
    ...
    {
        format!(
            "GitHub MCP does not support OAuth. Log in by adding a personal access token (https://github.com/settings/personal-access-tokens) to your environment and config.toml:\n[mcp_servers.{server_name}]\nbearer_token_env_var = ORBITERX_GITHUB_PERSONAL_ACCESS_TOKEN"
        )
    } else if is_mcp_client_auth_required_error(err) {
        format!(
            "The {server_name} MCP server is not logged in. Run `orbiterx mcp login {server_name}`."
        )
