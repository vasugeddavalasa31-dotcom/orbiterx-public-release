...
//! and exposing cached OrbiterX Apps tools while a client is still connecting.
...
use crate::mcp::ORBITERX_APPS_MCP_SERVER_NAME;
use crate::mcp::ToolPluginProvenance;
use crate::openai_docs_source_attribution::maybe_with_openai_docs_source_attribution;
use crate::orbiterx_apps::normalize_orbiterx_apps_callable_name;
use crate::orbiterx_apps::normalize_orbiterx_apps_callable_namespace;
use crate::orbiterx_apps::normalize_orbiterx_apps_tool_title;
use crate::orbiterx_apps::prepare_openai_file_params_for_model;
...
use orbiterx_async_utils::CancelErr;
use orbiterx_async_utils::OrCancelExt;
...
use orbiterx_connectors::ConnectorRuntimeContext;
use orbiterx_connectors::ConnectorRuntimeFetchSource;
use orbiterx_exec_server::Environment;
use orbiterx_exec_server::HttpClient;
use orbiterx_exec_server::ReqwestHttpClient;
use orbiterx_protocol::mcp::McpServerInfo;
use orbiterx_protocol::protocol::Event;
use orbiterx_protocol::protocol::EventMsg;
use orbiterx_protocol::protocol::McpStartupStatus;
use orbiterx_protocol::protocol::McpStartupUpdateEvent;
use orbiterx_rmcp_client::ExecutorStdioServerLauncher;
use orbiterx_rmcp_client::LocalStdioServerLauncher;
use orbiterx_rmcp_client::RmcpClient;
use orbiterx_rmcp_client::StdioServerLauncher;
use orbiterx_rmcp_client::ToolWithConnectorId;
use orbiterx_rmcp_client::is_authentication_required_error;
...
/// MCP server capability indicating that OrbiterX should include [`SandboxState`]
/// in tool-call request `_meta` under this key.
pub const MCP_SANDBOX_STATE_META_CAPABILITY: &str = "orbiterx/sandbox-state-meta";
...
const MCP_TOOL_CATALOG_CACHE_CAPABILITY: &str = "orbiterx/tool-catalog-cache";
const MCP_TOOL_CATALOG_CACHEABLE_PROPERTY: &str = "cacheable";
pub const OPENAI_FORM_CAPABILITY: &str = "openai/form";

pub(crate) const MCP_TOOLS_LIST_DURATION_METRIC: &str = "orbiterx.mcp.tools.list.duration_ms";
pub(crate) const MCP_TOOLS_FETCH_UNCACHED_DURATION_METRIC: &str =
    "orbiterx.mcp.tools.fetch_uncached.duration_ms";
pub(crate) const ORBITERX_APPS_REFRESH_DURATION_METRIC: &str = "orbiterx.apps.refresh.duration_ms";
pub(crate) const DEFAULT_STARTUP_TIMEOUT: Duration = Duration::from_secs(30);
pub(crate) const DEFAULT_TOOL_TIMEOUT: Duration = Duration::from_secs(300);

pub(crate) const ORBITERX_APPS_RECONNECT_INITIAL_BACKOFF: Duration = Duration::from_secs(1);
const ORBITERX_APPS_RECONNECT_MAX_BACKOFF: Duration = Duration::from_secs(30);
...
pub(crate) struct ManagedClient {
    ...
    pub(crate) orbiterx_apps_tools_cache_context: Option<ConnectorRuntimeContext<ToolInfo>>,
}
...
impl ManagedClient {
    pub(crate) fn listed_tools(&self) -> Vec<ToolInfo> {
        ...
        if let Some(tools) = self
            .orbiterx_apps_tools_cache_context
            .as_ref()
        ...
        if self.orbiterx_apps_tools_cache_context.is_some() {
            emit_duration(
                MCP_TOOLS_LIST_DURATION_METRIC,
                total_start.elapsed(),
                &[("cache", "miss")],
...
#[derive(Default)]
struct OrbiterXAppsStartupReconnectState {
...
#[derive(Clone)]
struct OrbiterXAppsStartupStatusContext {
    submit_id: String,
    server_name: String,
    tx_event: Sender<Event>,
}

pub(crate) struct OrbiterXAppsStartupReconnect {
    factory: Arc<dyn Fn() -> ManagedClientFuture + Send + Sync>,
    state: StdMutex<OrbiterXAppsStartupReconnectState>,
    startup_status_context: Option<OrbiterXAppsStartupStatusContext>,
}

impl OrbiterXAppsStartupReconnect {
    pub(crate) fn new(factory: Arc<dyn Fn() -> ManagedClientFuture + Send + Sync>) -> Self {
        Self {
            factory,
            state: StdMutex::new(OrbiterXAppsStartupReconnectState::default()),
    ...
    fn with_startup_status_context(
        ...
        self.startup_status_context = Some(OrbiterXAppsStartupStatusContext {
            submit_id,
            server_name,
            tx_event,
        });
            ...
            let recovered = {
                ...
                match result {
                    ...
                    Err(error) => {
                        ...
                        let retry_after =
                            orbiterx_apps_reconnect_backoff(state.consecutive_failures);
...
fn orbiterx_apps_reconnect_backoff(consecutive_failures: u32) -> Duration {
    let exponent = consecutive_failures.saturating_sub(1).min(5);
    ORBITERX_APPS_RECONNECT_INITIAL_BACKOFF
        .saturating_mul(1 << exponent)
        .min(ORBITERX_APPS_RECONNECT_MAX_BACKOFF)
}
...
struct ManagedClientStartup {
    ...
    orbiterx_apps_tools_cache_context: Option<ConnectorRuntimeContext<ToolInfo>>,
...
impl ManagedClientStartup {
    fn start(&self) -> ManagedClientFuture {
        ...
        let is_orbiterx_apps_mcp_server = server_name == ORBITERX_APPS_MCP_SERVER_NAME;
        ...
        async move {
            let refresh_start = is_orbiterx_apps_mcp_server.then(Instant::now);
            ...
            {
                emit_duration(
                    ORBITERX_APPS_REFRESH_DURATION_METRIC,
...
pub(crate) struct AsyncManagedClient {
    ...
    pub(crate) is_orbiterx_apps_mcp_server: bool,
    ...
    pub(crate) startup_reconnect: Option<Arc<OrbiterXAppsStartupReconnect>>,
...
impl AsyncManagedClient {
    ...
    pub(crate) fn new(
        ...
        let tool_filter = server
            ...
            .map(ToolFilter::from_config)
            .unwrap_or_default();
        let cached_server_info = if is_orbiterx_apps_mcp_server {
            orbiterx_apps_tools_cache_context
                .as_ref()
        ...
        let startup = Arc::new(ManagedClientStartup {
            ...
            tx_event,
            elicitation_requests,
            orbiterx_apps_tools_cache_context: orbiterx_apps_tools_cache_context.clone(),
        ...
        let startup_reconnect = is_orbiterx_apps_mcp_server.then(|| {
            let startup = Arc::clone(&startup);
            Arc::new(
                OrbiterXAppsStartupReconnect::new(Arc::new(move || startup.start()))
        ...
        if orbiterx_apps_tools_cache_context
            .as_ref()
    ...
    pub(crate) fn has_cached_tools(&self) -> bool {
        self.orbiterx_apps_tools_cache_context
            .as_ref()
            .is_some_and(ConnectorRuntimeContext::has_current_tools)
            || self
                .tool_catalog_cache_context
    ...
    pub(crate) fn prepare_tools(&self, tools: Vec<ToolInfo>) -> Vec<ToolInfo> {
        if self.is_orbiterx_apps_mcp_server {
            prepare_orbiterx_apps_tools_for_model(tools, &self.tool_plugin_provenance)
        } else {
    ...
    pub(crate) async fn listed_tools(&self) -> Option<Vec<ToolInfo>> {
            ...
            && let Some(startup_tools) = self.cached_tools()
        {
            ...
            match self.client().await {
                ...
                Err(_) if self.is_orbiterx_apps_mcp_server => self.cached_tools(),
                Err(_) => None,
            }
        }?;
...
pub(crate) async fn list_tools_for_client_uncached(
    server_name: &str,
    is_orbiterx_apps_mcp_server: bool,
    orbiterx_apps_refresh_trigger: &'static str,
    ...
    let tools = resp
        ...
        .collect();
    if is_orbiterx_apps_mcp_server {
        emit_duration(
            MCP_TOOLS_FETCH_UNCACHED_DURATION_METRIC,
            fetch_start.elapsed(),
            &[("trigger", orbiterx_apps_refresh_trigger)],
        );
    } else {
        emit_duration(
            MCP_TOOLS_FETCH_UNCACHED_DURATION_METRIC,
...
/// Presents declared OrbiterX Apps file parameters to the model as local-path inputs and adds plugin
/// names to each tool. Plugin membership is resolved by connector ID, falling back to the MCP
/// server when absent.
fn prepare_orbiterx_apps_tools_for_model(
...
fn tool_info_from_listed_tool(
    ...
    if is_orbiterx_apps_mcp_server {
        orbiterx_apps_tool_info_from_listed_tool(server_name, server_instructions, tool)
    } else {
        regular_mcp_tool_info_from_listed_tool(server_name, server_instructions, tool)
    }
}

/// Converts a OrbiterX Apps tool by preserving connector fields, removing connector prefixes from
/// model-visible names and titles, and using the connector description for its tool namespace.
fn orbiterx_apps_tool_info_from_listed_tool(
    server_name: &str,
    server_instructions: Option<&str>,
    tool: ToolWithConnectorId,
) -> ToolInfo {
    ...
    let callable_name = normalize_orbiterx_apps_callable_name(
        &tool_def.name,
        connector_id.as_deref(),
        connector_name.as_deref(),
    );
    let callable_namespace =
        normalize_orbiterx_apps_callable_namespace(server_name, connector_name.as_deref());
    if let Some(title) = tool_def.title.as_deref() {
        let normalized_title = normalize_orbiterx_apps_tool_title(connector_name.as_deref(), title);
...
async fn start_server_task(
    ...
    let fetch_ticket = orbiterx_apps_tools_cache_context
        .as_ref()
        .map(|cache_context| cache_context.begin_fetch(ConnectorRuntimeFetchSource::Startup));
    let tools = list_tools_for_client_uncached(
        &server_name,
        is_orbiterx_apps_mcp_server,
        /*orbiterx_apps_refresh_trigger*/ "initial",
        &client,
        startup_timeout,
    ...
    let tools = match (orbiterx_apps_tools_cache_context.as_ref(), fetch_ticket) {
        (Some(cache_context), Some(fetch_ticket)) => {
            cache_context.publish_if_newest_accepted(fetch_ticket, &server_info, tools)
        }
        (None, None) => tools,
        _ => unreachable!("OrbiterX Apps fetch ticket requires cache context"),
    };
    let has_shared_tool_catalog =
        is_orbiterx_apps_mcp_server || tool_catalog_cache_context.is_some();
...
fn mcp_initialize_request_params(
    ...
    InitializeRequestParams::new(
        capabilities,
        Implementation::new("orbiterx-mcp-client", env!("CARGO_PKG_VERSION"))
            .with_title("OrbiterX"),
    )
...
async fn make_rmcp_client(
    ...
    match transport {
        McpServerTransportConfig::Stdio {
            ...
            let cwd = cwd.map(orbiterx_utils_path_uri::LegacyAppPathString::into_string);
...
mod tests {
    ...
    #[test]
    fn orbiterx_apps_connector_metadata_is_preserved() {
        ...
        let tool_info = tool_info_from_listed_tool(
            ...
            /*is_orbiterx_apps_mcp_server*/ true,
        ...
        let expected = ToolInfo {
            server_name: ORBITERX_APPS_MCP_SERVER_NAME.to_string(),
            supports_parallel_tool_calls: false,
            server_origin: None,
            callable_name: "capture_file_upload".to_string(),
            callable_namespace: "orbiterx_apps__gmail".to_string(),
