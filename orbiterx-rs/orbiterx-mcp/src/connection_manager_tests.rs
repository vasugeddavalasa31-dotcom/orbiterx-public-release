...
use crate::rmcp_client::ORBITERX_APPS_RECONNECT_INITIAL_BACKOFF;
use crate::rmcp_client::OrbiterXAppsStartupReconnect;
...
use orbiterx_config::McpServerEnvVar;
...
use orbiterx_protocol::ToolName;
...
use orbiterx_rmcp_client::InProcessTransportFactory;
...
async fn create_test_managed_client(tools: Vec<ToolInfo>) -> ManagedClient {
    ManagedClient {
        ...
        orbiterx_apps_tools_cache_context: None,
    }
}
...
async fn create_ready_async_managed_client(tools: Vec<ToolInfo>) -> AsyncManagedClient {
    AsyncManagedClient {
        ...
        .boxed()
        .shared(),
        is_orbiterx_apps_mcp_server: false,
...
fn create_test_manager_with_failed_apps_startup(
    ...
    manager.clients.insert(
        ...
        AsyncManagedClient {
            ...
            cached_server_info: None,
            orbiterx_apps_tools_cache_context: Some(cache_context),
            tool_catalog_cache_context: None,
            tool_filter: ToolFilter::default(),
            startup_complete: Arc::new(std::sync::atomic::AtomicBool::new(true)),
            startup_reconnect: Some(Arc::new(OrbiterXAppsStartupReconnect::new(
                reconnect_factory,
            ))),
            tool_plugin_provenance: Arc::new(ToolPluginProvenance::default()),
            cancel_token: CancellationToken::new(),
...
async fn disabled_permissions_auto_accept_elicitation_with_empty_form_schema() {
    ...
    let response = sender(
        ...
        orbiterx_rmcp_client::Elicitation::Mcp(
...
async fn shared_elicitation_router_targets_the_exact_pending_request() {
    ...
    let elicitation = orbiterx_rmcp_client::Elicitation::Mcp(
        CreateElicitationRequestParams::FormElicitationParams {
            meta: None,
    ...
    let (
        orbiterx_protocol::mcp::RequestId::String(request_a_id),
        orbiterx_protocol::mcp::RequestId::String(request_b_id),
    ) = (request_a.id, request_b.id)
    else {
        panic!("expected OrbiterX-owned string request IDs");
    };
...
#[test]
fn orbiterx_apps_env_bearer_token_bypasses_shared_tools_cache() {
    assert!(!should_share_orbiterx_apps_tools_cache(
...
#[tokio::test]
async fn list_all_tools_uses_shared_orbiterx_apps_cache_while_client_is_pending() {
    let orbiterx_home = tempdir().expect("tempdir");
    let cache_context = create_orbiterx_apps_tools_cache_context(
        orbiterx_home.path().to_path_buf(),
        Some("account-one"),
    ...
    let tool = tools
        ...
        .find(|tool| {
            tool.canonical_tool_name()
                == ToolName::namespaced("mcp__orbiterx_apps", "calendar_create_event")
        })
        .expect("tool from shared cache");
    assert_eq!(tool.server_name, ORBITERX_APPS_MCP_SERVER_NAME);
...
async fn tool_catalog_cache_sanitizes_tools_and_tracks_environment_generation() {
    ...
    let cache_context = |environment: &Arc<orbiterx_exec_server::Environment>| {
        cache
            .context(
                "docs",
                &config,
...
async fn list_available_server_infos_uses_cache_while_client_is_pending() {
    ...
    assert_eq!(
        server_infos.get(ORBITERX_APPS_MCP_SERVER_NAME),
        Some(&server_info)
    );
}
...
#[tokio::test]
async fn list_all_tools_does_not_block_when_shared_orbiterx_apps_cache_is_empty() {
    let orbiterx_home = tempdir().expect("tempdir");
    let cache_context = create_orbiterx_apps_tools_cache_context(
        orbiterx_home.path().to_path_buf(),
        Some("account-one"),
...
#[tokio::test]
async fn list_all_tools_uses_shared_orbiterx_apps_cache_when_client_startup_fails() {
    let orbiterx_home = tempdir().expect("tempdir");
    let cache_context = create_orbiterx_apps_tools_cache_context(
        orbiterx_home.path().to_path_buf(),
        Some("account-one"),
...
#[tokio::test]
async fn list_all_tools_reconnects_failed_orbiterx_apps_startup_and_reuses_client() {
...
async fn later_tool_list_retries_after_failed_reconnect_and_keeps_cached_tools() {
    ...
    tokio::time::advance(ORBITERX_APPS_RECONNECT_INITIAL_BACKOFF).await;
...
#[tokio::test]
async fn tool_lists_do_not_block_and_share_orbiterx_apps_startup_reconnect() {
    let recovered_client = create_test_managed_client(vec![create_test_tool(
        ORBITERX_APPS_MCP_SERVER_NAME,
        "drive_search",
    )])
...
fn server_metadata_preserves_tool_approval_policy() {
    let mut config = crate::orbiterx_apps_mcp_server_config(
        "https://docs.example",
...
#[test]
fn host_owned_orbiterx_apps_requires_server_metadata() {
    let approval_policy = Constrained::allow_any(AskForApproval::OnRequest);
    let permission_profile = Constrained::allow_any(PermissionProfile::default());
    ...
    assert!(!manager.is_host_owned_orbiterx_apps_server(ORBITERX_APPS_MCP_SERVER_NAME));
}

#[test]
fn host_owned_orbiterx_apps_matches_reserved_name_with_server_metadata() {
    let approval_policy = Constrained::allow_any(AskForApproval::OnRequest);
    let permission_profile = Constrained::allow_any(PermissionProfile::default());
    let mut manager = McpConnectionManager::new_uninitialized(
        &approval_policy,
    ...
    let server = EffectiveMcpServer::configured(crate::orbiterx_apps_mcp_server_config(
        "https://chatgpt.com",
        /*apps_mcp_product_sku*/ None,
        /*originator*/ None,
    ));
    ...
    assert!(manager.is_host_owned_orbiterx_apps_server(ORBITERX_APPS_MCP_SERVER_NAME));
    assert!(!manager.is_host_owned_orbiterx_apps_server("docs"));
}
