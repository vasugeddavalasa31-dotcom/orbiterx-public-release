...
fn test_mcp_config(orbiterx_home: PathBuf) -> McpConfig {
    McpConfig {
        ...
        connector_snapshot: orbiterx_connectors::ConnectorSnapshot::default(),
    }
}
...
fn tool_plugin_provenance_collects_app_and_mcp_sources() {
    ...
    catalog.register(McpServerRegistration::from_plugin(
        ...
        /*plugin_order*/ 0,
        orbiterx_apps_mcp_server_config(
            "https://alpha.example",
            /*apps_mcp_product_sku*/ None,
            /*originator*/ None,
        ),
...
#[test]
fn orbiterx_apps_mcp_url_for_base_url_keeps_existing_paths() {
    assert_eq!(
        orbiterx_apps_mcp_url_for_base_url("https://chatgpt.com/backend-api"),
        "https://chatgpt.com/backend-api/wham/apps"
    );
    assert_eq!(
        orbiterx_apps_mcp_url_for_base_url("https://chat.openai.com"),
        "https://chat.openai.com/backend-api/wham/apps"
    );
    assert_eq!(
        orbiterx_apps_mcp_url_for_base_url("http://localhost:8080/api/orbiterx"),
        "http://localhost:8080/api/orbiterx/apps"
    );
    assert_eq!(
        orbiterx_apps_mcp_url_for_base_url("http://localhost:8080"),
        "http://localhost:8080/api/orbiterx/apps"
    );
}

#[test]
fn orbiterx_apps_server_config_uses_legacy_orbiterx_apps_path() {
    let config = orbiterx_apps_mcp_server_config(
        "https://chatgpt.com",
        /*apps_mcp_product_sku*/ None,
        /*originator*/ None,
    );
    let url = match &config.transport {
        ...
        _ => panic!("expected streamable http transport for orbiterx apps"),
    };
...
#[test]
fn orbiterx_apps_server_config_forwards_thread_originator_header() {
    let config = orbiterx_apps_mcp_server_config(
        "https://chatgpt.com",
        /*apps_mcp_product_sku*/ None,
        ...
        McpServerTransportConfig::StreamableHttp {
            ...
            assert_eq!(
                ...
                &Some(HashMap::from([
                    ...
                    ("X-OpenAI-Product-Sku".to_string(), "orbiterx".to_string()),
                ]))
            );
...
#[test]
fn orbiterx_apps_server_config_sets_product_sku_header() {
    for (configured_product_sku, expected_product_sku) in [(None, "orbiterx"), (Some("tpp"), "tpp")]
    {
        let config = orbiterx_apps_mcp_server_config(
            "https://chatgpt.com",
...
#[test]
fn orbiterx_apps_server_config_forwards_originator_and_configured_product_sku_headers() {
    let config = orbiterx_apps_mcp_server_config(
        "https://chatgpt.com",
        Some("tpp"),
        Some("thread_originator"),
...
async fn effective_mcp_servers_preserve_runtime_servers() {
    ...
    let mut config = test_mcp_config(orbiterx_home.path().to_path_buf());
    ...
    let orbiterx_apps = effective
        .get(ORBITERX_APPS_MCP_SERVER_NAME)
        .expect("orbiterx apps server should exist");
    ...
    let docs = docs
        .configured_config()
        .expect("configured server should retain transport");
    let orbiterx_apps = orbiterx_apps
        .configured_config()
        .expect("orbiterx apps should use configured transport");
