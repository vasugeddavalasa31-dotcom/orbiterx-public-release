...
use orbiterx_connectors::ConnectorSnapshot;
use orbiterx_connectors::connector_runtime_context_key;
use orbiterx_login::OrbiterXAuth;
use orbiterx_model_provider::CHATGPT_ORBITERX_BASE_URL;
use orbiterx_protocol::mcp::McpServerInfo;
use orbiterx_protocol::mcp::Resource;
use orbiterx_protocol::mcp::ResourceTemplate;
use orbiterx_protocol::mcp::Tool;
use orbiterx_protocol::models::PermissionProfile;
use orbiterx_protocol::protocol::AskForApproval;
use orbiterx_protocol::protocol::McpAuthStatus;
...
pub const ORBITERX_APPS_MCP_SERVER_NAME: &str = "orbiterx_apps";
const DEFAULT_ORBITERX_APPS_MCP_PRODUCT_SKU: &str = "orbiterx";
const MCP_TOOL_NAME_PREFIX: &str = "mcp";
const MCP_TOOL_NAME_DELIMITER: &str = "__";
const ORBITERX_CONNECTORS_TOKEN_ENV_VAR: &str = "ORBITERX_CONNECTORS_TOKEN";
...
/// MCP runtime settings derived from `orbiterx_core::config::Config`.
///
/// This struct should contain only long-lived configuration values that the
/// `orbiterx-mcp` crate needs to construct server transports, enforce MCP
...
pub struct McpConfig {
    ...
    /// OrbiterX home directory used for MCP OAuth state and app-tool cache files.
    ...
    /// Optional path to `orbiterx-linux-sandbox` for sandboxed MCP tool execution.
...
pub fn host_owned_orbiterx_apps_enabled(config: &McpConfig, auth: Option<&OrbiterXAuth>) -> bool {
    config.apps_enabled && auth.is_some_and(OrbiterXAuth::uses_orbiterx_backend)
}
...
pub fn effective_mcp_servers_from_configured(
    ...
    let chatgpt_origin = url::Url::parse(CHATGPT_ORBITERX_BASE_URL)
        .ok()
        .map(|url| url.origin());
    let mut servers = configured_servers
        ...
        .collect::<HashMap<_, _>>();
    if !host_owned_orbiterx_apps_enabled(config, auth) {
        servers.remove(ORBITERX_APPS_MCP_SERVER_NAME);
    }
    servers
}
...
fn orbiterx_apps_mcp_bearer_token_env_var() -> Option<String> {
    match env::var(ORBITERX_CONNECTORS_TOKEN_ENV_VAR) {
        Ok(value) if !value.trim().is_empty() => {
            Some(ORBITERX_CONNECTORS_TOKEN_ENV_VAR.to_string())
        }
        Ok(_) => None,
        Err(env::VarError::NotPresent) => None,
        Err(env::VarError::NotUnicode(_)) => Some(ORBITERX_CONNECTORS_TOKEN_ENV_VAR.to_string()),
    }
}

fn normalize_orbiterx_apps_base_url(base_url: &str) -> String {
...
fn orbiterx_apps_mcp_url_for_base_url(base_url: &str) -> String {
    let base_url = normalize_orbiterx_apps_base_url(base_url);
    let (base_url, default_path) = if base_url.contains("/backend-api") {
        (base_url, "wham/apps")
    } else if base_url.contains("/api/orbiterx") {
        (base_url, "apps")
    } else {
        (format!("{base_url}/api/orbiterx"), "apps")
    };
    format!("{base_url}/{default_path}")
}

pub fn orbiterx_apps_mcp_server_config(
    chatgpt_base_url: &str,
    apps_mcp_product_sku: Option<&str>,
    originator: Option<&str>,
) -> McpServerConfig {
    mcp_server_config_for_url(
        orbiterx_apps_mcp_url_for_base_url(chatgpt_base_url),
...
pub fn hosted_plugin_runtime_mcp_server_config(
    ...
    let base_url = normalize_orbiterx_apps_base_url(chatgpt_base_url);
    let base_url = if base_url.contains("/backend-api") || base_url.contains("/api/orbiterx") {
        base_url
    } else {
        format!("{base_url}/api/orbiterx")
    };
    mcp_server_config_for_url(
...
fn mcp_server_config_for_url(
    ...
    let product_sku = apps_mcp_product_sku.unwrap_or(DEFAULT_ORBITERX_APPS_MCP_PRODUCT_SKU);
    ...
    McpServerConfig {
        transport: McpServerTransportConfig::StreamableHttp {
            url,
            bearer_token_env_var: orbiterx_apps_mcp_bearer_token_env_var(),
