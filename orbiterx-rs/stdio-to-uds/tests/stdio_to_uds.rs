...
async fn pipes_stdin_and_stdout_through_socket() -> anyhow::Result<()> {
    ...
    let child_task = tokio::task::spawn_blocking(move || -> anyhow::Result<ChildOutput> {
        ...
        let mut child = Command::new(orbiterx_utils_cargo_bin::cargo_bin(
            "orbiterx-stdio-to-uds",
        )?)
        .arg(&socket_path)
        ...
        .stderr(Stdio::piped())
        .spawn()
        .context("failed to spawn orbiterx-stdio-to-uds")?;
        ...
        let status = loop {
            ...
            if Instant::now() >= deadline {
                ...
                anyhow::bail!(
                    "orbiterx-stdio-to-uds did not exit in time; server events: {:?}; stderr: {}",
                    server_events,
    ...
    assert!(
        ...
        "orbiterx-stdio-to-uds exited with {status}; server events: {:?}; stderr: {}",
