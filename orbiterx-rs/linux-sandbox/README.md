# orbiterx-linux-sandbox

This crate is responsible for producing:

- a `orbiterx-linux-sandbox` standalone executable for Linux that is bundled with the Node.js version of the OrbiterX CLI
- a lib crate that exposes the business logic of the executable as `run_main()` so that
  - the `orbiterx-exec` CLI can check if its arg0 is `orbiterx-linux-sandbox` and, if so, execute as if it were `orbiterx-linux-sandbox`
  - this should also be true of the `orbiterx` multitool CLI

On Linux, OrbiterX prefers the first `bwrap` found on `PATH`
...
the helper falls back to the bundled `orbiterx-resources/bwrap` binary shipped
with OrbiterX.
OrbiterX also surfaces a startup warning when `bwrap` is missing so users know it
is falling back to the bundled helper. OrbiterX surfaces the same startup warning
...
it cannot create the required user namespaces, so OrbiterX rejects sandboxed shell
...
- If `bwrap` is missing, the helper falls back to the bundled
  `orbiterx-resources/bwrap` path.
- If `bwrap` is missing, OrbiterX also surfaces a startup warning instead of
  printing directly from the sandbox helper.
- If bubblewrap cannot create user namespaces, OrbiterX surfaces a startup warning
  instead of waiting for a runtime sandbox failure.
- WSL2 uses the normal Linux bubblewrap path.
- WSL1 is not supported for bubblewrap sandboxing; OrbiterX rejects sandboxed
...
- When bubblewrap is active, protected subpaths under writable roots (for
  example `.git`,
  resolved `gitdir:`, and `.orbiterx`) are re-applied as read-only via `--ro-bind`.
...
**Notes**
- The CLI surface is `orbiterx sandbox`; the host OS selects the sandbox backend.
