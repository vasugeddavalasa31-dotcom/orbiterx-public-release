...
//! - sensitive subpaths such as `.git`, `.agents`, and `.orbiterx` remain
...
use orbiterx_protocol::permissions::is_protected_metadata_name;
use orbiterx_protocol::protocol::FileSystemAccessMode;
use orbiterx_protocol::protocol::FileSystemPath;
use orbiterx_protocol::protocol::FileSystemSandboxPolicy;
use orbiterx_protocol::protocol::FileSystemSpecialPath;
use orbiterx_protocol::protocol::WritableRoot;
    ...
    let missing_auto_metadata_read_only_project_root_subpaths: HashSet<PathBuf> =
        file_system_sandbox_policy
            ...
            .filter_map(|entry| {
                ...
                // protection consistently for `.git`, `.agents`, and `.orbiterx`.
                ...
                if project_subpath != Path::new(".git")
                    ...
                    && project_subpath != Path::new(".orbiterx")
                {
                    return None;
                }
...
mod tests {
    ...
    use orbiterx_protocol::protocol::FileSystemAccessMode;
    use orbiterx_protocol::protocol::FileSystemPath;
    use orbiterx_protocol::protocol::FileSystemSandboxEntry;
    use orbiterx_protocol::protocol::FileSystemSandboxPolicy;
    use orbiterx_protocol::protocol::FileSystemSpecialPath;
    use orbiterx_utils_absolute_path::AbsolutePathBuf;
    ...
    fn writable_roots_under_symlinked_ancestors_bind_real_target() {
        ...
        let real_orbiterx = temp_dir.path().join("real-orbiterx");
        let logical_orbiterx = logical_home.join(".orbiterx");
        let real_memories = real_orbiterx.join("memories");
        let logical_memories = logical_orbiterx.join("memories");
        ...
        std::os::unix::fs::symlink(&real_orbiterx, &logical_orbiterx)
            .expect("create symlinked orbiterx home");
    ...
    fn missing_read_only_subpath_uses_empty_file_bind_data() {
        ...
        assert_empty_directory_mounted_read_only(&args.args, &workspace.join(".orbiterx"));
    ...
    fn missing_project_root_metadata_carveouts_use_metadata_path_masks() {
        ...
        let dot_orbiterx = path_to_string(&temp_dir.path().join(".orbiterx"));
        ...
        assert_empty_directory_mounted_read_only(&args.args, Path::new(&dot_orbiterx));
        ...
        assert!(synthetic_targets.contains(&PathBuf::from(&dot_orbiterx)));
        assert_eq!(
            protected_create_target_paths(&args),
            Vec::<PathBuf>::new(),
    ...
    fn mounts_dev_before_writable_dev_binds() {
        ...
        assert_eq!(
            ...
            vec![
                PathBuf::from("/.git"),
                PathBuf::from("/.agents"),
                PathBuf::from("/.orbiterx"),
                PathBuf::from("/dev/.git"),
                PathBuf::from("/dev/.agents"),
                PathBuf::from("/dev/.orbiterx"),
            ]
        );
        assert_eq!(
            args.args,
