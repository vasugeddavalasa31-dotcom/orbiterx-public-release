...
use orbiterx_core::exec::process_exec_tool_call;
...
fn orbiterx_linux_sandbox_exe() -> PathBuf {
    let sandbox_program = PathBuf::from(env!("CARGO_BIN_EXE_orbiterx-linux-sandbox"));
...
async fn run_cmd_output(
    ...
    timeout_ms: u64,
) -> orbiterx_protocol::exec_output::ExecToolCallOutput {
...
async fn run_cmd_result_with_writable_roots(
    ...
    network_access: bool,
) -> Result<orbiterx_protocol::exec_output::ExecToolCallOutput> {
...
async fn run_cmd_result_with_permission_profile_for_cwd(
    ...
    let orbiterx_linux_sandbox_exe = Some(orbiterx_linux_sandbox_exe());
...
fn is_bwrap_unavailable_output(
    output: &orbiterx_protocol::exec_output::ExecToolCallOutput,
) -> bool {
...
async fn should_skip_bwrap_tests() -> bool {
    ...
    .await
    {
        ...
        Err(OrbiterXErr::Sandbox(SandboxErr::Denied { output, .. })) => {
        ...
        Err(OrbiterXErr::Sandbox(SandboxErr::Timeout { .. })) => true,
        Err(err) => panic!("bwrap availability probe failed unexpectedly: {err:?}"),
    }
}

fn expect_denied(
    result: Result<orbiterx_protocol::exec_output::ExecToolCallOutput>,
    context: &str,
) -> orbiterx_protocol::exec_output::ExecToolCallOutput {
    match result {
        Ok(output) => {
        ...
        Err(OrbiterXErr::Sandbox(SandboxErr::Denied { output, .. })) => *output,
...
async fn assert_network_blocked(cmd: &[&str]) {
    ...
    let orbiterx_linux_sandbox_exe: Option<PathBuf> = Some(orbiterx_linux_sandbox_exe());
...
#[tokio::test]
async fn sandbox_blocks_git_and_orbiterx_writes_inside_writable_root() {
    if should_skip_bwrap_tests().await {
        eprintln!("skipping bwrap test: bwrap sandbox prerequisites are unavailable");
        return;
    }
    ...
    let dot_orbiterx = tmpdir.path().join(".orbiterx");
    std::fs::create_dir_all(&dot_git).expect("create .git");
    std::fs::create_dir_all(&dot_orbiterx).expect("create .orbiterx");

    let git_target = dot_git.join("config");
    let orbiterx_target = dot_orbiterx.join("config.toml");
    ...
    let orbiterx_output = expect_denied(
        run_cmd_result_with_writable_roots(
            &[
                "bash",
                "-lc",
                &format!("echo denied > {}", orbiterx_target.to_string_lossy()),
            ],
            &[tmpdir.path().to_path_buf()],
            LONG_TIMEOUT_MS,
            /*use_legacy_landlock*/ false,
        ...
        .await,
        ".orbiterx write should be denied under bubblewrap",
    );
    assert_ne!(git_output.exit_code, 0);
    assert_ne!(orbiterx_output.exit_code, 0);
}

#[tokio::test]
async fn sandbox_blocks_orbiterx_symlink_replacement_attack() {
    if should_skip_bwrap_tests().await {
        eprintln!("skipping bwrap test: bwrap sandbox prerequisites are unavailable");
        return;
    }
    ...
    let tmpdir = tempfile::tempdir().expect("tempdir");
    let decoy = tmpdir.path().join("decoy-orbiterx");
    std::fs::create_dir_all(&decoy).expect("create decoy dir");

    let dot_orbiterx = tmpdir.path().join(".orbiterx");
    symlink(&decoy, &dot_orbiterx).expect("create .orbiterx symlink");
    ...
    let orbiterx_output = expect_denied(
        ...
        .await,
        ".orbiterx symlink replacement should be denied",
    );
    assert_ne!(orbiterx_output.exit_code, 0);
}

#[tokio::test]
async fn sandbox_reports_orbiterx_symlink_build_failure_without_panicking() {
    if should_skip_bwrap_tests().await {
        eprintln!("skipping bwrap test: bwrap sandbox prerequisites are unavailable");
        return;
    }
    ...
    {
        Err(OrbiterXErr::Sandbox(SandboxErr::Denied { output, .. })) => *output,
        result => panic!(".orbiterx symlink build failure should deny: {result:?}"),
    };
...
if grep -E '(^|[[:space:]])\.(git|orbiterx|agents)(/|$)' status.before; then
  cat status.before
  exit 21
fi
"#,
    ...
    let mkdir_orbiterx_output = expect_denied(
        run_cmd_result_with_cwd_and_writable_roots(
            &["mkdir", ".orbiterx"],
            &subdir,
            std::slice::from_ref(&subdir),
        ...
        .await,
        "child .orbiterx directory creation should be denied",
    );
    assert_ne!(mkdir_orbiterx_output.exit_code, 0);
    assert!(!subdir.join(".orbiterx").exists());
...
async fn sandbox_blocks_explicit_split_policy_carveouts_under_bwrap() {
    ...
    let sandbox_helper_dir = orbiterx_linux_sandbox_exe()
        .parent()
