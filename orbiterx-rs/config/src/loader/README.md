# `orbiterx-config` loader

This module is the canonical place to **load and describe OrbiterX configuration layers** (user config, CLI/session overrides, cloud-managed config, managed config, and MDM-managed preferences) and to produce:
...
Exported from `orbiterx_config::loader`:

- `load_config_layers_state(fs, orbiterx_home, cwd_opt, cli_overrides, options, thread_config_loader) -> ConfigLayerStack`
- `ConfigLayerStack`
