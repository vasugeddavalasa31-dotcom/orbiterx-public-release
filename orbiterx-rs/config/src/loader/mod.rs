...
use orbiterx_file_system::ExecutorFileSystem;
...
#[cfg(unix)]
const SYSTEM_CONFIG_TOML_FILE_UNIX: &str = "/etc/orbiterx/config.toml";
...
///
/// - system    `/etc/orbiterx/requirements.toml` (Unix) or
///   `%ProgramData%\OpenAI\OrbiterX\requirements.toml` (Windows)
...
///
/// - admin:    managed preferences (*)
/// - system    `/etc/orbiterx/config.toml` (Unix) or
///   `%ProgramData%\OpenAI\OrbiterX\config.toml` (Windows)
/// - cloud     enterprise-managed cloud config bundle fragments
/// - user      `${ORBITERX_HOME}/config.toml`
/// - profile   `${ORBITERX_HOME}/<name>.config.toml`, when selected
/// - cwd       `${PWD}/config.toml` (loaded but disabled when the directory is untrusted)
/// - tree      parent directories up to root looking for `./.orbiterx/config.toml` (loaded but disabled when untrusted)
/// - repo      `$(git rev-parse --show-toplevel)/.orbiterx/config.toml` (loaded but disabled when untrusted)
/// - runtime   e.g., --config flags, model selector in UI
///
/// (*) Only available on macOS via managed device profiles.
///
/// See https://developers.openai.com/orbiterx/security for details.
///
...
pub async fn load_config_layers_state(
    ...
    if !ignore_managed_requirements {
        if let Some(bundle) = cloud_config_bundle.get().await.map_err(io::Error::other)? {
            let cloud_config_base_dir = AbsolutePathBuf::from_absolute_path(orbiterx_home)?;
    ...
    let loaded_config_layers =
        layer_io::load_config_layers_internal(fs, orbiterx_home, overrides.clone(), strict_config)
            .await?;
    if !ignore_managed_requirements {
    ...
    // contain overrides.
    let active_user_file = overrides.user_config_path(orbiterx_home)?;
    let base_user_file =
        AbsolutePathBuf::resolve_path_against_base(CONFIG_TOML_FILE, orbiterx_home);
        ...
        && let Some(base_user_config) = base_user_layer.config.as_table()
    {
        ...
        if legacy_profile_is_selected || legacy_profile_table_exists {
            return Err(io::Error::new(
                ...
                format!(
                    "--profile `{active_user_profile}` cannot be used while {} contains legacy `profile = \"{active_user_profile}\"` or `[profiles.{active_user_profile}]` config; move those settings into {} and remove the legacy profile selector/table. See https://developers.openai.com/orbiterx/config-advanced#profiles for more information.",
    ...
    if let Some(config) = managed_config_from_mdm {
        ...
        let raw_toml_base_dir = AbsolutePathBuf::from_absolute_path(orbiterx_home)?;
...
fn system_requirements_toml_file() -> io::Result<AbsolutePathBuf> {
    AbsolutePathBuf::from_absolute_path(Path::new("/etc/orbiterx/requirements.toml"))
}
...
#[cfg(windows)]
fn windows_orbiterx_system_dir() -> PathBuf {
    ...
    program_data.join("OpenAI").join("OrbiterX")
}

#[cfg(windows)]
fn windows_system_requirements_toml_file() -> io::Result<AbsolutePathBuf> {
    let requirements_toml_file = windows_orbiterx_system_dir().join("requirements.toml");
    AbsolutePathBuf::try_from(requirements_toml_file)
}

#[cfg(windows)]
fn windows_system_config_toml_file() -> io::Result<AbsolutePathBuf> {
    let config_toml_file = windows_orbiterx_system_dir().join("config.toml");
...
fn legacy_requirements_to_toml_value(legacy: LegacyManagedConfigToml) -> io::Result<TomlValue> {
    ...
    if let Some(sandbox_mode) = sandbox_mode {
        ...
        // Allowing read-only is a requirement for OrbiterX to function correctly.
...
impl ProjectTrustContext {
    ...
    fn root_checkout_hooks_folder_for_dir(&self, dir: &AbsolutePathBuf) -> Option<AbsolutePathBuf> {
        ...
        Some(repo_root.join(relative_dir).join(".orbiterx"))
    }
}

fn project_layer_entry(
    dot_orbiterx_folder: &AbsolutePathBuf,
    config: TomlValue,
    disabled_reason: Option<String>,
    hooks_config_folder_override: Option<AbsolutePathBuf>,
) -> ConfigLayerEntry {
    let source = ConfigLayerSource::Project {
        dot_orbiterx_folder: dot_orbiterx_folder.clone(),
    };
...
fn project_ignored_config_keys_warning(
    ...
    let config_path = dot_orbiterx_folder.join(CONFIG_TOML_FILE);
...
async fn load_project_layers(
    ...
    let orbiterx_home_abs = AbsolutePathBuf::from_absolute_path(orbiterx_home)?;
    let orbiterx_home_normalized = normalize_path(orbiterx_home_abs.as_path())
        .unwrap_or_else(|_| orbiterx_home_abs.to_path_buf());
    let mut dirs = cwd
        .ancestors()
        .scan(false, |done, a| {
            if *done {
    ...
    for dir in dirs {
        let dot_orbiterx_abs = dir.join(".orbiterx");
        let dot_orbiterx_uri = PathUri::from_abs_path(&dot_orbiterx_abs);
        if !fs
            .get_metadata(&dot_orbiterx_uri, /*sandbox*/ None)
            .await
        ...
        let dot_orbiterx_normalized = normalize_path(dot_orbiterx_abs.as_path())
            .unwrap_or_else(|_| dot_orbiterx_abs.to_path_buf());
        if dot_orbiterx_abs == orbiterx_home_abs
            || dot_orbiterx_normalized == orbiterx_home_normalized
        {
            continue;
        }
        let config_file = dot_orbiterx_abs.join(CONFIG_TOML_FILE);
            ...
            Ok(contents) => {
                ...
                if disabled_reason.is_none() && strict_config {
                    validate_config_toml_strictly(
                        ...
                        &contents,
                        &config,
                        dot_orbiterx_abs.as_path(),
                    )?;
                }
                let ignored_project_config_keys = sanitize_project_config(&mut config);
                let config =
                    resolve_relative_paths_in_config_toml(config, dot_orbiterx_abs.as_path())?;
...
/// from a file like `/etc/orbiterx/managed_config.toml` that has the same
    ...
    fn windows_system_config_toml_file_uses_expected_suffix() {
        ...
        assert!(
            windows_system_config_toml_file()
                .expect("config.toml path")
                .as_path()
                .ends_with(Path::new("OpenAI").join("OrbiterX").join("config.toml"))
        );
    }
}
