//! Schema-heavy configuration TOML types used by OrbiterX.
...
use orbiterx_protocol::config_types::WebSearchToolConfig;
...
use orbiterx_utils_path::normalize_for_path_comparison;
...
/// Base config deserialized from ~/.orbiterx/config.toml.
...
pub struct ConfigToml {
    ...
    /// sanctioned by OrbiterX will likely degrade model performance.
    ...
    /// file: Use a file in the OrbiterX home directory.
    ...
    /// Defaults to `$ORBITERX_SQLITE_HOME` when set. Otherwise uses `$ORBITERX_HOME`.
    pub sqlite_home: Option<AbsolutePathBuf>,

    /// Directory where OrbiterX writes log files. Setting this value explicitly
    /// also enables the TUI text log in this directory.
    /// Defaults to `$ORBITERX_HOME/log`.
    ...
    /// Optional product SKU forwarded on host-owned OrbiterX Apps MCP requests.
    ...
    /// directories for `.orbiterx` folders. Defaults to [".git"] when unset.
    #[serde(default)]
...
pub struct DebugConfigLockToml {
    ...
    /// Allow replaying a lock generated by a different OrbiterX version.
    pub allow_orbiterx_version_mismatch: Option<bool>,
...
pub use orbiterx_protocol::protocol::RealtimeConversationVersion as RealtimeWsVersion;
pub use orbiterx_protocol::protocol::RealtimeVoice;
