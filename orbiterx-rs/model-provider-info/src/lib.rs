//! Registry of model providers supported by OrbiterX.
//!
//! Providers can be defined in two places:
//!   1. Built-in defaults compiled into the binary so OrbiterX works out-of-the-box.
//!   2. User-defined entries inside `~/.orbiterx/config.toml` under the `model_providers`
...
use orbiterx_api::RetryConfig as ApiRetryConfig;
...
use orbiterx_protocol::error::EnvVarError;
...
const AMAZON_BEDROCK_MANTLE_CLIENT_AGENT_VALUE: &str = "orbiterx";
const CHAT_WIRE_API_REMOVED_ERROR: &str = "`wire_api = \"chat\"` is no longer supported.\nHow to fix: set `wire_api = \"responses\"` in your provider config.\nMore info: https://github.com/openai/orbiterx/discussions/7782";
...
pub enum WireApi {
    ...
    #[default]
    Responses,
    /// The standard Messages API exposed by Anthropic.
    AnthropicMessages,
}
...
impl ModelProviderInfo {
    ...
    fn build_header_map(&self) -> OrbiterXResult<HeaderMap> {
    ...
    pub fn to_api_provider(&self, auth_mode: Option<AuthMode>) -> OrbiterXResult<ApiProvider> {
    ...
    pub fn api_key(&self) -> OrbiterXResult<Option<String>> {
        match &self.env_key {
            Some(env_key) => {
                let api_key = std::env::var(env_key)
                    ...
                    .ok_or_else(|| {
                        OrbiterXErr::EnvVar(EnvVarError {
                            var: env_key.clone(),
...
pub fn built_in_model_providers(
    ...
    // providers are bundled with OrbiterX CLI, so we only include the OpenAI and
...
pub fn create_oss_provider(default_provider_port: u16, wire_api: WireApi) -> ModelProviderInfo {
    // These ORBITERX_OSS_ environment variables are experimental: we may
    // switch to reading values from config.toml instead.
    let default_orbiterx_oss_base_url = format!(
        "http://localhost:{orbiterx_oss_port}/v1",
        orbiterx_oss_port = std::env::var("ORBITERX_OSS_PORT")
            .ok()
    ...
    let orbiterx_oss_base_url = std::env::var("ORBITERX_OSS_BASE_URL")
        .ok()
        .filter(|v| !v.trim().is_empty())
        .unwrap_or(default_orbiterx_oss_base_url);
    create_oss_provider_with_base_url(&orbiterx_oss_base_url, wire_api)
}
