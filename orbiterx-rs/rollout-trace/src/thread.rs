...
use crate::protocol_event::orbiterx_turn_trace_event;
...
pub const ORBITERX_ROLLOUT_TRACE_ROOT_ENV: &str = "ORBITERX_ROLLOUT_TRACE_ROOT";
...
impl ThreadTraceContext {
    ...
    /// Starts a root thread trace from `ORBITERX_ROLLOUT_TRACE_ROOT`, or disables tracing.
    ///
    /// Trace startup is best-effort. A tracing failure must not make the OrbiterX
    ...
    pub fn start_root_or_disabled(metadata: ThreadStartedTraceMetadata) -> Self {
        let Some(root) = std::env::var_os(ORBITERX_ROLLOUT_TRACE_ROOT_ENV) else {
    ...
    /// Emits typed OrbiterX turn lifecycle events from protocol lifecycle events.
    pub fn record_orbiterx_turn_event(&self, default_turn_id: &str, event: &EventMsg) {
        let ThreadTraceContextState::Enabled(context) = &self.state else {
            return;
        };
        let Some(trace_event) =
            orbiterx_turn_trace_event(context.thread_id.clone(), default_turn_id, event)
        else {
            return;
        };
    ...
    /// what OrbiterX did while executing that boundary.
    pub fn record_tool_call_event(
        &self,
        orbiterx_turn_id: impl Into<OrbiterXTurnId>,
        event: &EventMsg,
    ) {
        let ThreadTraceContextState::Enabled(context) = &self.state else {
            return;
        ...
        context.append_with_context_best_effort(orbiterx_turn_id.into(), payload);
    }
    ...
    pub fn record_agent_result_interaction(
        &self,
        child_orbiterx_turn_id: impl Into<OrbiterXTurnId>,
    ...
    pub fn record_orbiterx_turn_started(&self, orbiterx_turn_id: impl Into<OrbiterXTurnId>) {
        let ThreadTraceContextState::Enabled(context) = &self.state else {
            return;
        };
        let orbiterx_turn_id = orbiterx_turn_id.into();
        context.append_with_context_best_effort(
            orbiterx_turn_id.clone(),
            RawTraceEventPayload::OrbiterXTurnStarted {
                orbiterx_turn_id,
                thread_id: context.thread_id.clone(),
            },
        );
    ...
    pub fn start_code_cell_trace(
        ...
        let context = self.code_cell_trace_context(orbiterx_turn_id, runtime_cell_id);
    ...
    /// Builds reusable inference trace context for one OrbiterX turn.
    ///
...
impl EnabledThreadTraceContext {
    ...
    fn append_with_context_best_effort(
        &self,
        orbiterx_turn_id: OrbiterXTurnId,
        payload: RawTraceEventPayload,
    ) {
        let event_context = RawTraceEventContext {
            thread_id: Some(self.thread_id.clone()),
            orbiterx_turn_id: Some(orbiterx_turn_id),
        };
