...
impl TraceReducer {
    ...
    /// Starts a OrbiterX turn inside an existing thread.
    pub(super) fn start_orbiterx_turn(
        &mut self,
        seq: RawEventSeq,
        wall_time_unix_ms: i64,
        ...
        if self.rollout.orbiterx_turns.contains_key(&orbiterx_turn_id) {
            bail!("duplicate orbiterx turn start for {orbiterx_turn_id}");
        }

        self.thread_mut(&thread_id)?;

        self.rollout.orbiterx_turns.insert(
    ...
    /// Marks a OrbiterX turn terminal and validates any thread id carried by the raw event.
    pub(super) fn end_orbiterx_turn(
        &mut self,
        seq: RawEventSeq,
        wall_time_unix_ms: i64,
        ...
        if let Some(event_thread_id) = thread_id.as_deref()
            && let Some(turn) = self.rollout.orbiterx_turns.get(&orbiterx_turn_id)
            && turn.thread_id != event_thread_id
        {
            bail!(
                "orbiterx turn end for {orbiterx_turn_id} used thread {event_thread_id}, \
                 but the turn belongs to {}",
                turn.thread_id
            );
        }

        let Some(turn) = self.rollout.orbiterx_turns.get_mut(&orbiterx_turn_id) else {
            bail!("orbiterx turn end referenced unknown turn {orbiterx_turn_id}");
        };
