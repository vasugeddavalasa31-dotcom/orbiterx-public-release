...
impl TraceReducer {
    ...
    pub(super) fn reduce_inference_request(
        ...
        // history, not the pre-compaction prompt. Any repeated developer/context prefix that OrbiterX
    ...
    pub(super) fn reduce_inference_response(
        ...
        let Some((thread_id, orbiterx_turn_id)) = self
            .rollout
            .inference_calls
            ...
            .map(|inference| {
                (
                    ...
                    inference.orbiterx_turn_id.clone(),
                )
            })
        else {
        ...
        let response_item_ids = self.reconcile_conversation_items(
            ...
            ReconcileItems {
                thread_id: &thread_id,
                orbiterx_turn_id: &orbiterx_turn_id,
                wall_time_unix_ms,
        ...
        for (offset, item) in items.into_iter().enumerate() {
            ...
            let item_id = if let Some(previous_item_id) = previous_snapshot.get(index) {
                if self.item_matches(previous_item_id, &item) {
                    ...
                    self.find_matching_snapshot_item(&previous_snapshot, &item_ids, &item)
                        .unwrap_or_else(|| {
                            self.create_conversation_item(
                                context.thread_id,
                                Some(context.orbiterx_turn_id.to_string()),
                    ...
                    let orbiterx_turn_id = context.orbiterx_turn_id;
                    let thread_id = context.thread_id;
                    bail!(
                        "model conversation mismatch while reducing turn {orbiterx_turn_id} for \
    ...
    pub(super) fn reduce_compaction_checkpoint(
        ...
        let marker_item_id = self.create_conversation_item(
            thread_id,
            Some(orbiterx_turn_id.to_string()),
            wall_time_unix_ms,
