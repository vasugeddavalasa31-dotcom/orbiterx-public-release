...
impl TraceReducer {
    ...
    pub(super) fn start_inference_call(
        ...
        let orbiterx_turn_id = started.orbiterx_turn_id.clone();
        let request_payload = started.request_payload.clone();
        let Some(turn) = self.rollout.orbiterx_turns.get(&orbiterx_turn_id) else {
            bail!(
                "inference start {inference_call_id} referenced unknown orbiterx turn {orbiterx_turn_id}"
            );
        };
        if turn.thread_id != thread_id {
            bail!(
                "inference start {inference_call_id} used thread {thread_id}, \
                 but orbiterx turn {orbiterx_turn_id} belongs to {}",
                turn.thread_id
            );
        }
    ...
    /// If a call is still `Running`, OrbiterX stopped observing that provider stream
    ...
    pub(super) fn close_running_inference_calls_for_turn_end(
        ...
        for inference in self.rollout.inference_calls.values_mut() {
            if inference.orbiterx_turn_id == orbiterx_turn_id
