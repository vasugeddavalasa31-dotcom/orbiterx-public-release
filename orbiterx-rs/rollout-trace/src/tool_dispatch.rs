...
use orbiterx_protocol::models::SearchToolCallParams;
use serde::Serialize;
...
/// Core-facing request data for the canonical OrbiterX tool boundary.
pub struct ToolDispatchInvocation {
    pub thread_id: AgentThreadId,
    pub orbiterx_turn_id: OrbiterXTurnId,
...
/// Raw invocation payload for the canonical OrbiterX tool boundary.
#[derive(Serialize)]
...
impl ToolDispatchTraceContext {
    ...
    pub(crate) fn start(writer: Arc<TraceWriter>, invocation: ToolDispatchInvocation) -> Self {
        ...
        let context = EnabledToolDispatchTraceContext {
            ...
            orbiterx_turn_id: invocation.orbiterx_turn_id.clone(),
...
fn suppresses_tool_dispatch_trace(invocation: &ToolDispatchInvocation) -> bool {
    matches!(invocation.payload, ToolDispatchPayload::Custom { .. })
        ...
        && invocation.tool_name == orbiterx_code_mode::PUBLIC_TOOL_NAME
}
