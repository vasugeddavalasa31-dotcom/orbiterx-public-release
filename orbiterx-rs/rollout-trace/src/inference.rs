...
use crate::model::OrbiterXTurnId;
use crate::payload::RawPayloadKind;
use crate::raw_event::RawTraceEventContext;
use crate::raw_event::RawTraceEventPayload;
use crate::writer::TraceWriter;

const INFERENCE_CALL_ID_HEADER: &str = "x-orbiterx-inference-call-id";
...
///
/// A OrbiterX turn can create multiple attempts when auth recovery retries the
...
impl InferenceTraceContext {
    ...
    /// Builds an enabled context for all upstream attempts made by one OrbiterX turn.
    pub fn enabled(
        writer: Arc<TraceWriter>,
        thread_id: AgentThreadId,
        orbiterx_turn_id: OrbiterXTurnId,
...
impl InferenceTraceAttempt {
    ...
    pub fn record_started(&self, request: &impl Serialize) {
        ...
        append_with_context_best_effort(
            ...
            RawTraceEventPayload::InferenceStarted {
                ...
                orbiterx_turn_id: attempt.context.orbiterx_turn_id.clone(),
    ...
    /// trace-specific serialization rules. That keeps orbiterx-core focused on
    ...
    /// Records a provider stream that OrbiterX intentionally stopped consuming.
    ///
...
/// OrbiterX received it, so this helper restores that content in the raw payload.
...
fn append_with_context_best_effort(
    ...
    let event_context = RawTraceEventContext {
        ...
        orbiterx_turn_id: Some(context.orbiterx_turn_id.clone()),
    };
...
mod tests {
    ...
    fn enabled_context_records_replayable_inference_attempt() -> anyhow::Result<()> {
        ...
        assert_eq!(inference.orbiterx_turn_id, "turn-1");
