//! Mapping from OrbiterX protocol events into raw rollout-trace events.
//!
...
//! `orbiterx-core`: this module translates the protocol surface into the smaller
//! trace vocabulary and keeps the mapping isolated inside `orbiterx-rollout-trace`.
//!
...
use orbiterx_protocol::protocol::PatchApplyStatus;
use orbiterx_protocol::protocol::SubAgentActivityEvent;
...
use crate::OrbiterXTurnId;
use crate::RawTraceEventPayload;

pub(crate) struct OrbiterXTurnTraceEvent {
    pub context_turn_id: OrbiterXTurnId,
    pub payload: RawTraceEventPayload,
}

pub(crate) fn orbiterx_turn_trace_event(
    thread_id: AgentThreadId,
    default_turn_id: &str,
    event: &EventMsg,
) -> Option<OrbiterXTurnTraceEvent> {
    match event {
        EventMsg::TurnStarted(event) => {
            let orbiterx_turn_id = event.turn_id.clone();
            Some(OrbiterXTurnTraceEvent {
                context_turn_id: orbiterx_turn_id.clone(),
                payload: RawTraceEventPayload::OrbiterXTurnStarted {
                    orbiterx_turn_id,
                    thread_id,
                },
            })
        ...
        EventMsg::TurnComplete(event) => {
            ...
            Some(OrbiterXTurnTraceEvent {
                context_turn_id: orbiterx_turn_id.clone(),
                payload: RawTraceEventPayload::OrbiterXTurnEnded {
                    orbiterx_turn_id,
...
pub(crate) enum ToolRuntimePayload<'a> {
    ...
    CollabAgentSpawnBegin(&'a orbiterx_protocol::protocol::CollabAgentSpawnBeginEvent),
    CollabAgentSpawnEnd(&'a orbiterx_protocol::protocol::CollabAgentSpawnEndEvent),
    CollabAgentInteractionBegin(&'a orbiterx_protocol::protocol::CollabAgentInteractionBeginEvent),
    CollabAgentInteractionEnd(&'a orbiterx_protocol::protocol::CollabAgentInteractionEndEvent),
    CollabWaitingBegin(&'a orbiterx_protocol::protocol::CollabWaitingBeginEvent),
    CollabWaitingEnd(&'a orbiterx_protocol::protocol::CollabWaitingEndEvent),
    CollabCloseBegin(&'a orbiterx_protocol::protocol::CollabCloseBeginEvent),
    CollabCloseEnd(&'a orbiterx_protocol::protocol::CollabCloseEndEvent),
...
struct ExecCommandBeginTracePayload<'a> {
    ...
    command: &'a [String],
    cwd: String,
    parsed_cmd: &'a [orbiterx_protocol::parse_command::ParsedCommand],
