...
use orbiterx_connectors::ConnectorMetadata;
use orbiterx_connectors::ConnectorMetadataStore;
use orbiterx_connectors::ConnectorToolSummary;
use orbiterx_connectors::DirectoryListResponse;
use orbiterx_connectors::merge::merge_connectors;
use orbiterx_connectors::merge::merge_plugin_connectors;
use orbiterx_core::config::Config;
pub use orbiterx_core::connectors::list_accessible_connectors_from_mcp_tools;
pub use orbiterx_core::connectors::list_accessible_connectors_from_mcp_tools_with_environment_manager;
pub use orbiterx_core::connectors::list_accessible_connectors_from_mcp_tools_with_mcp_manager;
pub use orbiterx_core::connectors::list_accessible_connectors_from_mcp_tools_with_options;
pub use orbiterx_core::connectors::list_accessible_connectors_from_mcp_tools_with_options_and_status;
pub use orbiterx_core::connectors::list_cached_accessible_connectors_from_mcp_tools;
pub use orbiterx_core::connectors::with_app_enabled_state;
...
const DEFAULT_APPS_PRODUCT_SKU: &str = "orbiterx";
...
async fn connector_auth(config: &Config) -> anyhow::Result<OrbiterXAuth> {
    let auth_manager =
        AuthManager::shared_from_config(config, /*enable_orbiterx_api_key_env*/ false).await;
    ...
    anyhow::ensure!(
        auth.uses_orbiterx_backend(),
        "ChatGPT connectors require OrbiterX backend auth"
    );
    Ok(auth)
}
...
pub async fn list_cached_all_connectors(
    ...
    let connectors = orbiterx_connectors::cached_directory_connectors(&cache_context)?;
...
pub async fn list_all_connectors_with_options(
    ...
    let cache_context = connector_directory_cache_context(config, &auth);
    let connectors = orbiterx_connectors::list_all_connectors_with_options(
        cache_context,
...
pub async fn read_connector_metadata(
    ...
    anyhow::ensure!(
        auth.uses_orbiterx_backend(),
        "ChatGPT backend requests require OrbiterX backend auth"
    );
    anyhow::ensure!(
        auth.get_account_id().is_some(),
        "ChatGPT account ID not available, please re-run orbiterx login"
    );
...
mod tests {
    use super::*;
    use orbiterx_connectors::metadata::connector_install_url;
    use orbiterx_plugin::AppConnectorId;
