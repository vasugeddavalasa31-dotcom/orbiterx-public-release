...
async fn end_to_end_login_flow_persists_auth_json() -> Result<()> {
    ...
    let server_home = orbiterx_home.clone();

    let opts = ServerOptions {
        orbiterx_home: server_home,
        cli_auth_credentials_store_mode: AuthCredentialsStoreMode::File,
        auth_route_config: None,
        client_id: orbiterx_login::CLIENT_ID.to_string(),
        issuer,
        port: 0,
        open_browser: false,
    ...
    // Validate auth.json
    let auth_path = orbiterx_home.join("auth.json");
...
async fn hosted_login_redirects_to_configured_open_app_url() -> Result<()> {
    ...
    let server = run_login_server(ServerOptions {
        ...
        login_success_page: LoginSuccessPage::Hosted {
            url: Url::parse("http://localhost:3000/orbiterx/open-app?source=old")?,
...
#[tokio::test]
async fn creates_missing_orbiterx_home_dir() -> Result<()> {
    skip_if_no_network!(Ok(()));
    ...
    let tmp = tempdir()?;
    let orbiterx_home = tmp.path().join("missing-subdir"); // does not exist
...
async fn oauth_access_denied_missing_entitlement_blocks_login_with_clear_error() -> Result<()> {
    ...
    let client = reqwest::Client::new();
    let url = format!(
        "http://127.0.0.1:{login_port}/auth/callback?state={state}&error=access_denied&error_description=missing_orbiterx_entitlement"
    );
    let resp = client.get(&url).send().await?;
    assert!(resp.status().is_success());
    let body = resp.text().await?;
    assert!(
        body.contains("You do not have access to OrbiterX"),
        "error body should clearly explain the OrbiterX access denial"
    );
    assert!(
    ...
    assert!(
        !body.contains("missing_orbiterx_entitlement"),
...
async fn oauth_access_denied_unknown_reason_uses_generic_error_page() -> Result<()> {
    ...
    assert!(
        body.contains("Return to OrbiterX to retry"),
    ...
    assert!(
        !body.contains("get access to OrbiterX"),
...
async fn falls_back_to_registered_fallback_port_when_default_port_is_in_use() -> Result<()> {
    ...
    let default_port_server_handle = {
        ...
        thread::spawn(move || {
            while let Ok(req) = server.recv() {
                let _ = req.respond(tiny_http::Response::from_string("not orbiterx"));
            }
        })
    };
    ...
    let mut opts = ServerOptions::new(
        ...
        orbiterx_login::CLIENT_ID.to_string(),
        /*forced_chatgpt_workspace_id*/ None,
...
async fn cancels_previous_login_server_when_port_is_in_use() -> Result<()> {
    ...
    let first_orbiterx_home = first_tmp.path().to_path_buf();

    let first_opts = ServerOptions {
        orbiterx_home: first_orbiterx_home,
        cli_auth_credentials_store_mode: AuthCredentialsStoreMode::File,
        auth_route_config: None,
        client_id: orbiterx_login::CLIENT_ID.to_string(),
        issuer: issuer.clone(),
    ...
    let second_orbiterx_home = second_tmp.path().to_path_buf();

    let second_opts = ServerOptions {
        orbiterx_home: second_orbiterx_home,
        cli_auth_credentials_store_mode: AuthCredentialsStoreMode::File,
        auth_route_config: None,
        client_id: orbiterx_login::CLIENT_ID.to_string(),
        issuer,
