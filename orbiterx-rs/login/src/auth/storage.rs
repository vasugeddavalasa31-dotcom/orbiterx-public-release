...
use orbiterx_agent_identity::AgentIdentityJwtClaims;
use orbiterx_agent_identity::decode_agent_identity_jwt;
use orbiterx_config::types::AuthCredentialsStoreMode;
pub use orbiterx_config::types::AuthKeyringBackendKind;
use orbiterx_keyring_store::DefaultKeyringStore;
...
use orbiterx_secrets::LocalSecretsNamespace;
use orbiterx_secrets::SecretName;
use orbiterx_secrets::SecretScope;
use orbiterx_secrets::SecretsBackendKind;
use orbiterx_secrets::SecretsManager;

/// Expected structure for $ORBITERX_HOME/auth.json.
...
pub(super) fn get_auth_file(orbiterx_home: &Path) -> PathBuf {
    orbiterx_home.join("auth.json")
}

pub(super) fn delete_file_if_exists(orbiterx_home: &Path) -> std::io::Result<bool> {
    let auth_file = get_auth_file(orbiterx_home);
...
impl FileAuthStorage {
    pub(super) fn new(orbiterx_home: PathBuf) -> Self {
        Self { orbiterx_home }
    }

    /// Attempt to read and parse the `auth.json` file in the given `ORBITERX_HOME` directory.
...
impl AuthStorageBackend for FileAuthStorage {
    fn load(&self) -> std::io::Result<Option<AuthDotJson>> {
        let auth_file = get_auth_file(&self.orbiterx_home);
    ...
    fn delete(&self) -> std::io::Result<bool> {
        delete_file_if_exists(&self.orbiterx_home)
    }
}

static ORBITERX_AUTH_SECRET_NAME: Lazy<SecretName> =
    Lazy::new(|| match SecretName::new("ORBITERX_AUTH") {
        Ok(name) => name,
        Err(err) => unreachable!("ORBITERX_AUTH should be a valid secret name: {err}"),
    });
const KEYRING_SERVICE: &str = "OrbiterX Auth";

// turns orbiterx_home path into a stable, short key string
fn compute_store_key(orbiterx_home: &Path) -> std::io::Result<String> {
    let canonical = orbiterx_home
        .canonicalize()
        .unwrap_or_else(|_| orbiterx_home.to_path_buf());
...
impl DirectKeyringAuthStorage {
    fn new(orbiterx_home: PathBuf, keyring_store: Arc<dyn KeyringStore>) -> Self {
        Self {
            orbiterx_home,
            keyring_store,
        }
...
impl AuthStorageBackend for DirectKeyringAuthStorage {
    fn load(&self) -> std::io::Result<Option<AuthDotJson>> {
        let key = compute_store_key(&self.orbiterx_home)?;
    ...
    fn save(&self, auth: &AuthDotJson) -> std::io::Result<()> {
        ...
        if let Err(err) = delete_file_if_exists(&self.orbiterx_home) {
    ...
    fn delete(&self) -> std::io::Result<bool> {
        ...
        let file_removed = delete_file_if_exists(&self.orbiterx_home)?;
...
impl SecretsKeyringAuthStorage {
    fn new(orbiterx_home: PathBuf, keyring_store: Arc<dyn KeyringStore>) -> Self {
        let direct_storage =
            DirectKeyringAuthStorage::new(orbiterx_home.clone(), Arc::clone(&keyring_store));
        let secrets_manager = SecretsManager::new_with_keyring_store_and_namespace(
            orbiterx_home.clone(),
            SecretsBackendKind::Local,
            keyring_store,
            LocalSecretsNamespace::OrbiterXAuth,
        );
        Self {
            orbiterx_home,
            direct_storage,
...
impl AuthStorageBackend for SecretsKeyringAuthStorage {
    fn load(&self) -> std::io::Result<Option<AuthDotJson>> {
        match self
            .secrets_manager
            .get(&SecretScope::Global, &ORBITERX_AUTH_SECRET_NAME)
            .map_err(|err| {
                std::io::Error::other(format!(
    ...
    fn delete(&self) -> std::io::Result<bool> {
        let keyring_removed = self
            .secrets_manager
            .delete(&SecretScope::Global, &ORBITERX_AUTH_SECRET_NAME)
            .map_err(|err| {
                std::io::Error::other(format!(
...
impl AutoAuthStorage {
    fn new(
        ...
        Self {
            ...
            file_storage: Arc::new(FileAuthStorage::new(orbiterx_home)),
        }
    }
}
...
// A global in-memory store for mapping orbiterx_home -> AuthDotJson.
...
impl EphemeralAuthStorage {
    fn new(orbiterx_home: PathBuf) -> Self {
        Self { orbiterx_home }
    }
...
pub(super) fn create_auth_storage(
    ...
    create_auth_storage_with_store(orbiterx_home, mode, keyring_store, keyring_backend_kind)
}
...
fn create_auth_storage_with_store(
    ...
    match mode {
        AuthCredentialsStoreMode::File => Arc::new(FileAuthStorage::new(orbiterx_home)),
        AuthCredentialsStoreMode::Keyring => {
            create_keyring_auth_storage(orbiterx_home, keyring_store, keyring_backend_kind)
        }
        ...
        AuthCredentialsStoreMode::Ephemeral => Arc::new(EphemeralAuthStorage::new(orbiterx_home)),
    }
}
...
fn create_keyring_auth_storage(
    ...
    match keyring_backend_kind {
        AuthKeyringBackendKind::Direct => {
            Arc::new(DirectKeyringAuthStorage::new(orbiterx_home, keyring_store))
        }
        AuthKeyringBackendKind::Secrets => {
            Arc::new(SecretsKeyringAuthStorage::new(orbiterx_home, keyring_store))
        }
    }
}
