//! Default OrbiterX HTTP client: shared `User-Agent`, `originator`, optional residency header, and
//! reqwest/`HttpClient` construction.
//!
//! Use [`crate::default_client`] or [`orbiterx_login::default_client`] from other crates in this
//! workspace.

use orbiterx_http_client::BuildCustomCaTransportError;
use orbiterx_http_client::BuildRouteAwareHttpClientError;
...
pub use orbiterx_http_client::RequestBuilder as OrbiterXRequestBuilder;
...
/// See https://github.com/openai/orbiterx/pull/3388/files for an example of what that would look like.
...
/// Parenthesis will be added by OrbiterX. This should only specify what goes inside of the parenthesis.
pub static USER_AGENT_SUFFIX: LazyLock<Mutex<Option<String>>> = LazyLock::new(|| Mutex::new(None));
pub const DEFAULT_ORIGINATOR: &str = "orbiterx_cli_rs";
pub const ORBITERX_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR: &str =
    "ORBITERX_INTERNAL_ORIGINATOR_OVERRIDE";
pub const RESIDENCY_HEADER_NAME: &str = "x-openai-internal-orbiterx-residency";

pub use orbiterx_config::ResidencyRequirement;
...
fn get_originator_value(provided: Option<String>) -> Originator {
    let value = std::env::var(ORBITERX_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR)
        .ok()
        .or(provided)
...
pub fn originator() -> Originator {
    ...
    if std::env::var(ORBITERX_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR).is_ok() {
...
pub fn is_first_party_originator(originator_value: &str) -> bool {
    originator_value == DEFAULT_ORIGINATOR
        || originator_value == "orbiterx-tui"
        || originator_value == "orbiterx_vscode"
        || originator_value.starts_with("OrbiterX ")
}

pub fn is_first_party_chat_originator(originator_value: &str) -> bool {
    originator_value == "orbiterx_atlas" || originator_value == "orbiterx_chatgpt_desktop"
}

pub fn get_orbiterx_user_agent() -> String {
...
fn sanitize_user_agent(candidate: String, fallback: &str) -> String {
    ...
    if !sanitized.is_empty() && HeaderValue::from_str(sanitized.as_str()).is_ok() {
        tracing::warn!(
            "Sanitized OrbiterX user agent because provided suffix contained invalid header characters"
        );
        sanitized
    } else if HeaderValue::from_str(fallback).is_ok() {
        tracing::warn!(
            "Falling back to base OrbiterX user agent because provided suffix could not be sanitized"
        );
        fallback.to_string()
    } else {
        tracing::warn!(
            "Falling back to default OrbiterX originator because base user agent string is invalid"
        );
        originator().value
    }
}
...
/// OrbiterX's route-aware system/PAC resolution.
pub fn create_client() -> HttpClient {
    let inner = build_reqwest_client();
    HttpClient::new(inner)
}

/// Builds the default reqwest client used for ordinary OrbiterX HTTP traffic.
///
/// This starts from the standard OrbiterX user agent, default headers, and sandbox-specific proxy
/// policy, then layers in shared custom CA handling from `ORBITERX_CA_CERTIFICATE` /
...
///
/// This supported default path preserves reqwest's existing proxy behavior and does not opt into
/// OrbiterX's route-aware system/PAC resolution. Auth callers with route settings must use
...
/// Tries to build the default reqwest client used for ordinary OrbiterX HTTP traffic.
///
...
/// Builds the default OrbiterX reqwest client for a concrete outbound route.
///
/// When route-aware proxy handling is disabled, or the client is running inside the OrbiterX
...
/// Builds the default OrbiterX reqwest client for a concrete outbound route without blocking the
...
/// Builds an HTTP client for an auth endpoint without OrbiterX default headers.
...
/// Builds the default OrbiterX reqwest client for an auth endpoint.
...
/// Builds the default OrbiterX HTTP client wrapper for an auth endpoint.
...
pub fn default_headers() -> HeaderMap {
    ...
    if let Ok(user_agent) = HeaderValue::from_str(&get_orbiterx_user_agent()) {
...
fn is_sandboxed() -> bool {
    std::env::var("ORBITERX_SANDBOX").as_deref() == Ok("seatbelt")
}
