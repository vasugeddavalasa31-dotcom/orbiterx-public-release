...
use orbiterx_utils_template::Template;
use rand::RngCore;
...
pub struct ServerOptions {
    ...
    pub orbiterx_streamlined_login: bool,
    ...
    match path.as_str() {
            ...
            .await
            {
                Ok(tokens) => {
                    ...
                    let redirect = compose_success_url(
                        ...
                        &tokens.id_token,
                        &tokens.access_token,
                        opts.orbiterx_streamlined_login,
                    ...
                    match tiny_http::Header::from_bytes(&b"Location"[..], url.as_bytes()) {
                        ...
                        Err(_) => login_error_response(
                            "Sign-in completed but redirecting back to OrbiterX failed.",
                            io::ErrorKind::Other,
        ...
        "/success" => {
            let use_streamlined_success = parsed_url
                .query_pairs()
                .any(|(key, value)| key == "orbiterx_streamlined_login" && value == "true");
...
pub(crate) async fn persist_tokens_async(
    ...
    let orbiterx_home = orbiterx_home.to_path_buf();
...
/// Returns true when the OAuth callback represents a missing OrbiterX entitlement.
fn is_missing_orbiterx_entitlement_error(
    error_code: &str,
    error_description: Option<&str>,
) -> bool {
    error_code == "access_denied"
        && error_description.is_some_and(|description| {
            description
                .to_ascii_lowercase()
                .contains("missing_orbiterx_entitlement")
        })
}
...
fn oauth_callback_error_message(error_code: &str, error_description: Option<&str>) -> String {
    if is_missing_orbiterx_entitlement_error(error_code, error_description) {
        return "OrbiterX is not enabled for your workspace. Contact your workspace administrator to request access to OrbiterX.".to_string();
    }
...
fn render_login_error_page(
    ...
    let (title, display_message, display_description, help_text) =
        if is_missing_orbiterx_entitlement_error(code, error_description) {
            (
                "You do not have access to OrbiterX".to_string(),
                "This account is not currently authorized to use OrbiterX in this workspace."
                    .to_string(),
                "Contact your workspace administrator to request access to OrbiterX.".to_string(),
                "Contact your workspace administrator to get access to OrbiterX, then return to OrbiterX and try again."
                    .to_string(),
            )
        } else {
            (
                ...
                "Return to OrbiterX to retry, switch accounts, or contact your workspace admin if access is restricted."
                    .to_string(),
            )
        };
...
mod tests {
    ...
    use super::is_missing_orbiterx_entitlement_error;
    ...
    fn render_login_error_page_uses_entitlement_copy() {
        let error_description = Some("missing_orbiterx_entitlement");
        assert!(is_missing_orbiterx_entitlement_error(
            "access_denied",
            error_description
        ));
        ...
        assert!(body.contains("You do not have access to OrbiterX"));
        assert!(body.contains("Contact your workspace administrator"));
        assert!(!body.contains("missing_orbiterx_entitlement"));
    }
}
