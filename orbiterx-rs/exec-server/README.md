...
`orbiterx-exec-server` is the library backing `orbiterx exec-server`, a small
JSON-RPC server for spawning and controlling subprocesses through
`orbiterx-utils-pty`.

It provides:

- a CLI entrypoint: `orbiterx exec-server`
- a Rust client: `ExecServerClient`
- a small protocol module with shared request/response types

This crate owns the transport, protocol, and filesystem/process handlers. The
top-level `orbiterx` binary owns hidden helper dispatch for sandboxed
filesystem operations and `orbiterx-linux-sandbox`.

## Transport

The server speaks the exec-specific `orbiterx-exec-server-protocol` message
...
must support it.
It uses the standard OrbiterX ChatGPT sign-in state; run `orbiterx login` first when
remote registration needs authentication. Containerized callers that receive an
Agent Identity JWT in `ORBITERX_ACCESS_TOKEN` can opt into that auth path with
`--use-agent-identity-auth`; OrbiterX then registers an Agent task and sends the
derived AgentAssertion headers on the registry request.

Alternatively, API users can instead use `ORBITERX_API_KEY`;
OrbiterX sends it as a bearer token on the registration request. For example:

```sh
ORBITERX_API_KEY="$OPENAI_API_KEY" \
orbiterx exec-server \
  --remote ... \
...
`orbiterx.exec_server.relay.v1.RelayMessageFrame`; the checked-in schema is in
`src/proto/orbiterx.exec_server.relay.v1.proto`. The relay frame carries stream
...
- `arg0`: optional argv0 override forwarded to `orbiterx-utils-pty`.
...
hidden helper process launched from the top-level `orbiterx` executable and
...
`orbiterx exec-server` command builds these paths from the `orbiterx` arg0 dispatch
state. `RemoteEnvironmentConfig::new(...)` also takes the auth provider that
remote registration should use; the CLI builds that provider from OrbiterX auth
