...
pub(crate) fn environment_provider_from_orbiterx_home(
    orbiterx_home: &Path,
) -> Result<Box<dyn EnvironmentProvider>, ExecServerError> {
    let path = orbiterx_home.join(ENVIRONMENTS_TOML_FILE);
...
mod tests {
        ...
        let provider = TomlEnvironmentProvider::new(EnvironmentsToml {
            ...
            environments: vec![
                ...
                EnvironmentToml {
                    ...
                    args: Some(vec![
                        "dev".to_string(),
                        "orbiterx exec-server --listen stdio".to_string(),
                    ]),
                    env: Some(HashMap::from([(
                        "ORBITERX_LOG".to_string(),
                        "debug".to_string(),
                    )])),
                    ..Default::default()
                },
        ...
        let cases = [
            ...
            (
                EnvironmentToml {
                    id: "devbox".to_string(),
                    url: Some("ws://127.0.0.1:8765".to_string()),
                    program: Some("orbiterx".to_string()),
                    ..Default::default()
                },
    ...
    fn load_environments_toml_reads_root_environment_list() {
        ...
        let path = orbiterx_home.path().join(ENVIRONMENTS_TOML_FILE);
        std::fs::write(
            &path,
            r#"
default = "ssh-dev"
...
id = "ssh-dev"
program = "ssh"
args = ["dev", "orbiterx exec-server --listen stdio"]
cwd = "/tmp"
[environments.env]
ORBITERX_LOG = "debug"
"#,
...
"#,
    ...
    #[tokio::test]
    async fn environment_provider_from_orbiterx_home_uses_present_environments_file() {
        let orbiterx_home = tempdir().expect("tempdir");
        std::fs::write(
            orbiterx_home.path().join(ENVIRONMENTS_TOML_FILE),
            r#"
default = "none"
include_local = false
"#,
        ...
        let provider = environment_provider_from_orbiterx_home(orbiterx_home.path())
    ...
    #[tokio::test]
    async fn environment_provider_from_orbiterx_home_falls_back_when_file_is_missing() {
        let orbiterx_home = tempdir().expect("tempdir");
