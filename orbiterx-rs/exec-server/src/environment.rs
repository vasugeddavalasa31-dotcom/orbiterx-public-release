...
use crate::environment_toml::environment_provider_from_orbiterx_home;
...
pub const ORBITERX_EXEC_SERVER_URL_ENV_VAR: &str = "ORBITERX_EXEC_SERVER_URL";
pub const ORBITERX_EXEC_SERVER_NOISE_REGISTRY_URL_ENV_VAR: &str =
    "ORBITERX_EXEC_SERVER_NOISE_REGISTRY_URL";
pub const ORBITERX_EXEC_SERVER_NOISE_ENVIRONMENT_ID_ENV_VAR: &str =
    "ORBITERX_EXEC_SERVER_NOISE_ENVIRONMENT_ID";
pub const ORBITERX_EXEC_SERVER_NOISE_AUTH_TOKEN_ENV_VAR: &str =
    "ORBITERX_EXEC_SERVER_NOISE_AUTH_TOKEN";
pub const ORBITERX_EXEC_SERVER_NOISE_CHATGPT_ACCOUNT_ID_ENV_VAR: &str =
    "ORBITERX_EXEC_SERVER_NOISE_CHATGPT_ACCOUNT_ID";
...
/// Owns the execution/filesystem environments available to the OrbiterX runtime.
///
/// `EnvironmentManager` is a shared registry for concrete environments. Its
/// default constructor preserves the legacy `ORBITERX_EXEC_SERVER_URL` behavior
/// while configured construction accepts a provider-supplied snapshot.
///
/// Setting `ORBITERX_EXEC_SERVER_URL=none` disables environment access by leaving
...
impl EnvironmentManager {
    ...
    /// Builds a manager from `ORBITERX_HOME` and local runtime paths used when
    /// creating local filesystem helpers.
    ///
    /// If `ORBITERX_HOME/environments.toml` is present, it defines the configured
    /// environments. Otherwise this preserves the legacy
    /// `ORBITERX_EXEC_SERVER_URL` behavior.
    pub async fn from_orbiterx_home(
        orbiterx_home: impl AsRef<std::path::Path>,
        local_runtime_paths: Option<ExecServerRuntimePaths>,
        ...
        let provider = environment_provider_from_orbiterx_home(orbiterx_home.as_ref())?;
    ...
    /// reading user config files from `ORBITERX_HOME`.
...
-> Result<Option<NoiseRendezvousEnvironmentConfig>, ExecServerError> {
    noise_environment_config_from_values(
        optional_environment_value(ORBITERX_EXEC_SERVER_NOISE_REGISTRY_URL_ENV_VAR),
        optional_environment_value(ORBITERX_EXEC_SERVER_NOISE_ENVIRONMENT_ID_ENV_VAR),
        optional_environment_value(ORBITERX_EXEC_SERVER_NOISE_AUTH_TOKEN_ENV_VAR),
        optional_environment_value(ORBITERX_EXEC_SERVER_NOISE_CHATGPT_ACCOUNT_ID_ENV_VAR),
    )
}
        ...
        match (registry_url, environment_id, auth_token) {
            ...
            _ => {
                return Err(ExecServerError::EnvironmentRegistryConfig(format!(
                    "Noise environment requires {ORBITERX_EXEC_SERVER_NOISE_REGISTRY_URL_ENV_VAR}, \
{ORBITERX_EXEC_SERVER_NOISE_ENVIRONMENT_ID_ENV_VAR}, and \
{ORBITERX_EXEC_SERVER_NOISE_AUTH_TOKEN_ENV_VAR}"
                )));
            }
        };
...
impl Environment {
    /// Builds an environment from the raw `ORBITERX_EXEC_SERVER_URL` value.
    pub fn create(
        exec_server_url: Option<String>,
        local_runtime_paths: ExecServerRuntimePaths,
    ) -> Result<Self, ExecServerError> {
    ...
    /// Builds an environment from the raw `ORBITERX_EXEC_SERVER_URL` value and
...
mod tests {
        ...
        let environment = Environment::remote_with_transport(
            ExecServerTransportParams::StdioCommand {
                command: StdioExecServerCommand {
                    program: "orbiterx-missing-exec-server-for-test".to_string(),
                    args: Vec::new(),
                    env: HashMap::new(),
                    cwd: None,
                },
    ...
    async fn local_environment_passes_runtime_paths_to_exec_backend() {
        ...
        let sandbox = crate::FileSystemSandboxContext::from_permission_profile_with_cwd(
            orbiterx_protocol::models::PermissionProfile::workspace_write(),
            sandbox_cwd.clone(),
        );
    ...
    async fn test_environment_rejects_sandboxed_filesystem_without_runtime_paths() {
        let environment = Environment::default_for_tests();
        let path = orbiterx_utils_absolute_path::AbsolutePathBuf::from_absolute_path(
            std::env::current_exe().expect("current exe").as_path(),
        )
        .expect("absolute current exe");
        let path = orbiterx_utils_path_uri::PathUri::from_abs_path(&path);
        let sandbox = crate::FileSystemSandboxContext::from_permission_profile(
            orbiterx_protocol::models::PermissionProfile::from_runtime_permissions(
                &orbiterx_protocol::permissions::FileSystemSandboxPolicy::restricted(Vec::new()),
                orbiterx_protocol::permissions::NetworkSandboxPolicy::Restricted,
            ),
        );
