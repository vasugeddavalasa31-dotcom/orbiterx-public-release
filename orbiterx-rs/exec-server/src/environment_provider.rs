...
use crate::environment::ORBITERX_EXEC_SERVER_URL_ENV_VAR;
use crate::environment::REMOTE_ENVIRONMENT_ID;

/// Lists the concrete environments available to OrbiterX.
///
...
/// Default provider backed by `ORBITERX_EXEC_SERVER_URL`.
#[derive(Clone, Debug)]
...
impl DefaultEnvironmentProvider {
    /// Builds a provider from an already-read raw `ORBITERX_EXEC_SERVER_URL` value.
    pub fn new(exec_server_url: Option<String>) -> Self {
        Self { exec_server_url }
    }

    /// Builds a provider by reading `ORBITERX_EXEC_SERVER_URL`.
    pub fn from_env() -> Self {
        Self::new(std::env::var(ORBITERX_EXEC_SERVER_URL_ENV_VAR).ok())
    }
