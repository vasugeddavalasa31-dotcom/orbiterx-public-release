    ...
    while let Some(event) = incoming_rx.recv().await {
        ...
        match event {
            JsonRpcConnectionEvent::MalformedMessage { reason } => {
                ...
                if outgoing_tx
                    .send(RpcServerOutboundMessage::Error {
                        request_id: orbiterx_exec_server_protocol::RequestId::Integer(-1),
            ...
            JsonRpcConnectionEvent::Message(message) => match message {
                orbiterx_exec_server_protocol::JSONRPCMessage::Request(request) => {
                ...
                orbiterx_exec_server_protocol::JSONRPCMessage::Notification(notification) => {
                ...
                orbiterx_exec_server_protocol::JSONRPCMessage::Response(response) => {
                    warn!(
                ...
                orbiterx_exec_server_protocol::JSONRPCMessage::Error(error) => {
                    warn!(
...
fn request_span(
    span_name: &str,
    request: &orbiterx_exec_server_protocol::JSONRPCRequest,
) -> tracing::Span {
    let method = request.method.as_str();
    let span = tracing::info_span!(
        "orbiterx.exec_server.request",
        otel.kind = "server",
        otel.name = span_name,
        method,
    ...
    if let Some(trace) = &request.trace
        && !orbiterx_otel::set_parent_from_w3c_trace_context(&span, trace)
    {
...
mod tests {
    ...
    use orbiterx_exec_server_protocol::JSONRPCNotification;
    use orbiterx_exec_server_protocol::JSONRPCRequest;
    ...
    fn request_span_uses_bounded_name_wire_method_and_inbound_trace_parent() {
        ...
        let subscriber = tracing_subscriber::registry().with(
            tracing_opentelemetry::layer()
                .with_tracer(tracer)
                .with_filter(filter_fn(orbiterx_otel::OtelProvider::trace_export_filter)),
        );
        ...
        let trace = orbiterx_protocol::protocol::W3cTraceContext {
