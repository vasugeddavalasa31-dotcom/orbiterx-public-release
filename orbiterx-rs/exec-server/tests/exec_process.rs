...
use orbiterx_exec_server::ExecBackend;
use orbiterx_exec_server::ExecOutputStream;
use orbiterx_exec_server::ExecParams;
use orbiterx_exec_server::ExecProcess;
use orbiterx_exec_server::ExecProcessEvent;
#[cfg(unix)]
...
async fn remote_process_keeps_sandbox_helper_visible_with_restricted_reads() -> Result<()> {
    if let Some(warning) =
        orbiterx_sandboxing::system_bwrap_warning(&PermissionProfile::read_only())
    {
...
async fn remote_tty_process_uses_configured_sandbox_helper_with_hostile_path() -> Result<()> {
    ...
    let file = workspace.path().join("allowed.txt");
    std::fs::write(&file, b"allowed")?;
    let hostile_helper = workspace.path().join("orbiterx-linux-sandbox");
