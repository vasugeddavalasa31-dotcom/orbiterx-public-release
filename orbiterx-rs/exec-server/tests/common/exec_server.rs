...
pub(crate) struct ExecServerHarness {
    ...
    _helper_paths: TestOrbiterXHelperPaths,
    child: Child,
...
pub(crate) struct TestOrbiterXHelperPaths {
    pub(crate) orbiterx_exe: PathBuf,
    pub(crate) orbiterx_linux_sandbox_exe: Option<PathBuf>,
}
...
pub(crate) fn test_orbiterx_helper_paths() -> anyhow::Result<TestOrbiterXHelperPaths> {
    let (helper_binary, orbiterx_linux_sandbox_exe) = super::current_test_binary_helper_paths()?;
    Ok(TestOrbiterXHelperPaths {
        orbiterx_exe: helper_binary,
...
{
    let helper_paths = test_orbiterx_helper_paths()?;
    let orbiterx_home = TempDir::new()?;
    let mut child = Command::new(&helper_paths.orbiterx_exe);
    child.args(["exec-server", "--listen", "ws://127.0.0.1:0"]);
    child.stdin(Stdio::null());
    child.stdout(Stdio::piped());
    child.stderr(Stdio::inherit());
    child.kill_on_drop(true);
    child.env("ORBITERX_HOME", orbiterx_home.path());
    child.envs(env);
