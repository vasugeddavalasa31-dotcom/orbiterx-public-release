...
use orbiterx_exec_server::CAPABILITY_ROOTS_DISCOVER_METHOD;
use orbiterx_exec_server::CapabilityRootDiscovery;
use orbiterx_exec_server::CapabilityRootsDiscoverParams;
use orbiterx_exec_server::CapabilityRootsDiscoverResponse;
...
async fn discovers_a_complete_capability_bundle_in_one_request() -> anyhow::Result<()> {
    ...
    write_file(
        &root.path().join(".orbiterx-plugin/plugin.json"),
        r#"{
  "name": "demo",
  ...
  "apps": "./config/apps.json"
    ...
    assert_eq!(
        ...
        root_uri.join(".orbiterx-plugin/plugin.json")?
    );
