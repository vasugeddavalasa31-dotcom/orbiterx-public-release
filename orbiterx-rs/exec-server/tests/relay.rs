...
#[path = "../src/proto/orbiterx.exec_server.relay.v1.rs"]
mod relay_proto;
...
use orbiterx_exec_server::EnvironmentConnectionState;
use orbiterx_exec_server::EnvironmentManager;
use orbiterx_exec_server::EnvironmentReadyInfo;
use orbiterx_exec_server::ExecParams;
use orbiterx_exec_server::ExecResponse;
use orbiterx_exec_server::ExecServerClient;
use orbiterx_exec_server::ExecServerError;
use orbiterx_exec_server::ExecServerRuntimePaths;
use orbiterx_exec_server::FsReadFileParams;
use orbiterx_exec_server::NoiseChannelIdentity;
use orbiterx_exec_server::NoiseChannelPublicKey;
use orbiterx_exec_server::NoiseRendezvousConnectArgs;
use orbiterx_exec_server::NoiseRendezvousConnectBundle;
use orbiterx_exec_server::NoiseRendezvousConnectProvider;
use orbiterx_exec_server::ProcessId;
use orbiterx_exec_server::RemoteEnvironmentConfig;
...
fn static_registry_auth_provider() -> orbiterx_api::SharedAuthProvider {
...
async fn deferred_noise_environment_connects_and_reconnects_with_fresh_bundle() -> Result<()> {
    ...
    let (orbiterx_exe, orbiterx_linux_sandbox_exe) = common::current_test_binary_helper_paths()?;
    let runtime_paths = ExecServerRuntimePaths::new(orbiterx_exe, orbiterx_linux_sandbox_exe)?;
    let config = RemoteEnvironmentConfig::new(
        registry.uri(),
        ENVIRONMENT_ID.to_string(),
        static_registry_auth_provider(),
    )?;
    let remote_environment = tokio::spawn(orbiterx_exec_server::run_remote_environment(
        config,
        runtime_paths,
    ));
