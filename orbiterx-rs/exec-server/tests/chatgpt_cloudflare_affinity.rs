...
async fn exec_server_replays_only_chatgpt_cloudflare_cookies() -> anyhow::Result<()> {
    ...
    let env = vec![
        (
            OsString::from("ORBITERX_CA_CERTIFICATE"),
...
fn generate_tls_material() -> anyhow::Result<TlsMaterial> {
    ...
    ca_distinguished_name.push(DnType::CommonName, "OrbiterX affinity test CA");
    ...
    T: DeserializeOwned,
{
    let message = server
        .wait_for_event(|event| match event {
            ...
            | JSONRPCMessage::Error(orbiterx_exec_server_protocol::JSONRPCError { id, .. }) => {
                id == &request_id
            }
            _ => false,
        })
