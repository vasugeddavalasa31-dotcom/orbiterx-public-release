...
use orbiterx_exec_server_protocol::JSONRPCError;
...
async fn exec_server_http_request_buffers_response_body() -> anyhow::Result<()> {
        ...
        .send_request(
            ...
            serde_json::to_value(HttpRequestParams {
                ...
                headers: vec![HttpHeader {
                    name: "x-orbiterx-test".to_string(),
    ...
    assert_eq!(
        (
            ...
            captured.headers.get("x-orbiterx-test").map(String::as_str),
...
async fn wait_for_error_response(
    server: &mut ExecServerHarness,
    request_id: RequestId,
) -> anyhow::Result<orbiterx_exec_server_protocol::JSONRPCErrorError> {
    let response = server
        .wait_for_event(|event| {
            matches!(
                event,
