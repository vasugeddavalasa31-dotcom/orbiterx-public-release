...
async fn detect_home_lists_config_skills_and_agents_md() {
    ...
    let items = service_for_paths(external_agent_home.clone(), orbiterx_home.clone())
    ...
    let expected = vec![
        ExternalAgentConfigMigrationItem {
            ...
            description: format!(
                ...
                orbiterx_home.join("config.toml").display()
            ),
            cwd: None,
            details: None,
        },
        ...
        ExternalAgentConfigMigrationItem {
            ...
            description: format!(
                ...
                orbiterx_home.join("AGENTS.md").display()
            ),
            cwd: None,
            details: None,
        },
...
async fn detect_home_lists_recent_sessions() {
    let (root, external_agent_home, orbiterx_home) = fixture_paths();
...
async fn detect_repo_still_reports_non_plugin_items_when_home_config_is_invalid() {
    ...
    .expect("write invalid orbiterx config");
    fs::write(
    ...
    let items = service_for_paths(root.path().join(EXTERNAL_AGENT_DIR), orbiterx_home)
        .detect(ExternalAgentConfigDetectOptions {
            include_home: false,
            include_memory: false,
        ...
        vec![
            ...
            ExternalAgentConfigMigrationItem {
                ...
                description: format!(
                    ...
                    repo_root.join(".orbiterx").join("hooks.json").display()
                ),
                cwd: Some(repo_root.clone()),
                details: Some(MigrationDetails {
            ...
            ExternalAgentConfigMigrationItem {
                ...
                description: format!(
                    ...
                    repo_root.join(".orbiterx").join("agents").display()
                ),
                cwd: Some(repo_root),
                details: Some(MigrationDetails {
...
"#,
    ...
    let _supported_mcp_config: std::collections::HashMap<
        String,
        orbiterx_config::types::McpServerConfig,
    > = mcp_servers
        .try_into()
        .expect("migrated MCP config should be supported");

    let hooks: JsonValue = serde_json::from_str(
        &fs::read_to_string(repo_root.join(".orbiterx").join("hooks.json")).expect("read hooks"),
    )
    .expect("parse hooks");
    let _supported_hooks: orbiterx_config::HooksFile =
...
Research with OrbiterX carefully."""
"#,
    )
