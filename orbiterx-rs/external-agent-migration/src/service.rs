...
const EXTERNAL_AGENT_CONFIG_IMPORT_METRIC: &str = "orbiterx.external_agent_config.import";

#[derive(Clone)]
pub struct ExternalAgentConfigService {
    pub(super) orbiterx_home: PathBuf,
...
impl ExternalAgentConfigService {
    ...
    #[cfg(test)]
    fn new_for_test(orbiterx_home: PathBuf, external_agent_home: PathBuf) -> Self {
        let source = ExternalAgentSource::default();
        let connector_metadata_roots = source.connector_metadata_roots(&external_agent_home);
        Self {
            orbiterx_home,
            ...
            let import_result = match migration_item.item_type {
                ...
                ExternalAgentConfigMigrationItemType::Memory if self.source.supports_memory() => {
                    async {
                        ...
                        let target_path = memory_import::resources_root(&self.orbiterx_home);
    ...
    fn import_config(&self, cwd: Option<&Path>) -> io::Result<Option<(String, String)>> {
        ...
        let target_config = match &scope {
            MigrationScope::Home => self.orbiterx_home.join("config.toml"),
            MigrationScope::Repository { root } => root.join(".orbiterx").join("config.toml"),
        };
    ...
    fn import_hooks(&self, cwd: Option<&Path>) -> io::Result<Vec<String>> {
        ...
        let target_hooks = match &scope {
            MigrationScope::Home => self.orbiterx_home.join("hooks.json"),
            MigrationScope::Repository { root } => root.join(".orbiterx").join("hooks.json"),
        };
    ...
    fn import_agents_md(&self, cwd: Option<&Path>) -> io::Result<Option<(String, String)>> {
        ...
        let (source_agents_md, target_agents_md) = match scope {
            ...
            MigrationScope::Home => {
                ...
                (source_agents_md, self.orbiterx_home.join("AGENTS.md"))
            }
        };
            ...
            marketplace
                ...
                .filter(|plugin| {
                    plugin.policy.products.as_deref().is_none_or(|products| {
                        Product::OrbiterX.matches_product_restriction(products)
                    })
                })
