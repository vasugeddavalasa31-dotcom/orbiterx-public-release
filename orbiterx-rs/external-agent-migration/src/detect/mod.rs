...
const EXTERNAL_AGENT_CONFIG_DETECT_METRIC: &str = "orbiterx.external_agent_config.detect";
...
impl ExternalAgentConfigService {
    pub async fn detect(
        ...
        if params.include_home
            ...
            && let Some(item) = memory::detect(&self.orbiterx_home, &self.external_agent_home)?
        {
            items.push(item);
    ...
    async fn detect_migrations(
        ...
        let target_config = repo_root.map_or_else(
            || self.orbiterx_home.join("config.toml"),
            |repo_root| repo_root.join(".orbiterx").join("config.toml"),
        );
        ...
        let target_hooks = repo_root.map_or_else(
            || self.orbiterx_home.join("hooks.json"),
            |repo_root| repo_root.join(".orbiterx").join("hooks.json"),
        );
        ...
        let target_subagents = repo_root.map_or_else(
            ...
            |repo_root| repo_root.join(".orbiterx").join("agents"),
        );
        ...
        let instruction_source_groups = if let Some(repo_root) = repo_root {
            ...
            (!sources.is_empty())
                .then(|| InstructionSourceGroup {
                    scope: self.orbiterx_home.clone(),
                    sources,
                })
                .into_iter()
                .collect()
        ...
        if self.source.supports_plugin_migration(settings.as_ref()) {
            match ConfigBuilder::default()
                ...
                .fallback_cwd(Some(self.orbiterx_home.clone()))
                .build()
                .await
            {
                Ok(config) => {
                    ...
                    let configured_marketplace_plugins = configured_marketplace_plugins(
                        &config,
                        &PluginsManager::new(self.orbiterx_home.clone()),
                    )?;
        ...
        if scope.is_home() {
            let sessions = self
                .source
                .recent_sessions(&self.external_agent_home, &self.orbiterx_home)?;
