...
pub fn has_current_session_been_imported(
    ...
    load_import_ledger(orbiterx_home)?.contains_current_source(source_path)
}
...
pub fn record_completed_session_imports(
    ...
    let mut ledger = load_import_ledger(orbiterx_home)?;
    ...
    save_import_ledger(orbiterx_home, &ledger)
}

pub fn read_imported_connector_candidates(
    orbiterx_home: &Path,
) -> io::Result<Vec<ImportedConnectorCandidate>> {
    let ledger = load_import_ledger(orbiterx_home)?;
...
pub(crate) fn load_import_ledger(
    ...
    let path = import_ledger_path(orbiterx_home);
...
fn import_ledger_path(orbiterx_home: &Path) -> PathBuf {
    orbiterx_home.join(SESSION_IMPORT_LEDGER_FILE)
}
