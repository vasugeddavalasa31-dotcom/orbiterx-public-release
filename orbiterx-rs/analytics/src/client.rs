...
use orbiterx_login::default_client::create_client;
...
impl AnalyticsEventsDestination {
    ...
    fn from_base_url_and_capture_file(base_url: String, capture_file: Option<PathBuf>) -> Self {
        ...
        Self::Http {
            url: format!("{base_url}/orbiterx/analytics-events/events"),
        }
    }
}
...
impl AnalyticsEventsClient {
    ...
    pub fn track_compaction(&self, event: crate::facts::OrbiterXCompactionEvent) {
        self.record_fact(AnalyticsFact::Custom(CustomAnalyticsFact::Compaction(
            Box::new(event),
        )));
    }

    pub fn track_goal_event(&self, event: OrbiterXGoalEvent) {
    ...
    pub fn track_turn_orbiterx_error(&self, fact: TurnOrbiterXErrorFact) {
        self.record_fact(AnalyticsFact::Custom(
            CustomAnalyticsFact::TurnOrbiterXError(Box::new(fact)),
        ));
    }
...
async fn send_track_events(
    ...
    } else if !auth.uses_orbiterx_backend() {
        return;
    }
    if events.is_empty() {
        return;
