...
use crate::events::OrbiterXCollabAgentToolCallEventParams;
use crate::events::OrbiterXCollabAgentToolCallEventRequest;
...
use crate::events::OrbiterXDynamicToolCallEventParams;
use crate::events::OrbiterXDynamicToolCallEventRequest;
use crate::events::OrbiterXFileChangeEventParams;
use crate::events::OrbiterXFileChangeEventRequest;
use crate::events::OrbiterXGoalEventRequest;
use crate::events::OrbiterXHookRunEventRequest;
use crate::events::OrbiterXImageGenerationEventParams;
use crate::events::OrbiterXImageGenerationEventRequest;
use crate::events::OrbiterXMcpToolCallEventParams;
use crate::events::OrbiterXMcpToolCallEventRequest;
use crate::events::OrbiterXOnboardingExternalAgentImportCompleteEventRequest;
use crate::events::OrbiterXOnboardingExternalAgentImportCompleteMetadata;
...
use crate::events::OrbiterXPluginInstallRequestedEventRequest;
...
use crate::events::OrbiterXTurnEventParams;
use crate::events::OrbiterXTurnEventRequest;
use crate::events::OrbiterXTurnSteerEventParams;
use crate::events::OrbiterXTurnSteerEventRequest;
use crate::events::OrbiterXWebSearchEventParams;
use crate::events::OrbiterXWebSearchEventRequest;
...
use crate::events::orbiterx_compaction_event_params;
use crate::events::orbiterx_goal_event_params;
use crate::events::orbiterx_hook_run_metadata;
use crate::events::orbiterx_plugin_install_requested_metadata;
...
use crate::facts::OrbiterXGoalEvent;
...
use crate::facts::TurnOrbiterXError;
...
use orbiterx_app_server_protocol::ClientResponse;
use orbiterx_app_server_protocol::CollabAgentStatus;
...
use orbiterx_app_server_protocol::DynamicToolCallOutputContentItem;
...
use orbiterx_app_server_protocol::NetworkPolicyRuleAction;
use orbiterx_app_server_protocol::OrbiterXErrorInfo;
use orbiterx_app_server_protocol::PatchApplyStatus;
use orbiterx_app_server_protocol::PatchChangeKind;
...
use orbiterx_app_server_protocol::WebSearchAction;
use orbiterx_git_utils::collect_git_info;
...
impl<'a> AnalyticsDropSite<'a> {
    ...
    fn compaction(input: &'a OrbiterXCompactionEvent) -> Self {
        Self {
    ...
    fn goal(input: &'a OrbiterXGoalEvent) -> Self {
        Self {
            event_name: "goal",
            thread_id: &input.thread_id,
    ...
    fn tool_item(
        notification: &'a orbiterx_app_server_protocol::ItemCompletedNotification,
        item_id: &'a str,
    ) -> Self {
        Self {
...
struct CompletedTurnState {
    ...
    turn_error: Option<OrbiterXErrorInfo>,
    completed_at: u64,
...
struct TurnState {
    ...
    orbiterx_error: Option<TurnOrbiterXError>,
...
impl AnalyticsReducer {
    pub(crate) async fn ingest(&mut self, input: AnalyticsFact, out: &mut Vec<TrackEventRequest>) {
        match input {
            ...
            AnalyticsFact::Custom(input) => match input {
                ...
                CustomAnalyticsFact::TurnOrbiterXError(input) => {
                    self.ingest_turn_orbiterx_error(*input);
                }
    ...
    fn ingest_guardian_review(
        ...
        out.push(TrackEventRequest::GuardianReview(Box::new(
            GuardianReviewEventRequest {
                event_type: "orbiterx_guardian_review",
    ...
    fn ingest_turn_orbiterx_error(&mut self, input: TurnOrbiterXErrorFact) {
        let TurnOrbiterXErrorFact {
            turn_id,
            thread_id,
            error,
        } = input;
        ...
        turn_state.orbiterx_error = Some(error);
    }
    ...
    fn ingest_app_mentioned(&mut self, input: AppMentionedInput, out: &mut Vec<TrackEventRequest>) {
        ...
        out.extend(mentions.into_iter().map(|mention| {
            let event_params = orbiterx_app_metadata(&tracking, mention);
            TrackEventRequest::AppMentioned(OrbiterXAppMentionedEventRequest {
    ...
    fn ingest_app_used(&mut self, input: AppUsedInput, out: &mut Vec<TrackEventRequest>) {
        ...
        let event_params = orbiterx_app_metadata(&tracking, app);
        out.push(TrackEventRequest::AppUsed(OrbiterXAppUsedEventRequest {
    ...
    fn ingest_hook_run(&mut self, input: HookRunInput, out: &mut Vec<TrackEventRequest>) {
        ...
        out.push(TrackEventRequest::HookRun(OrbiterXHookRunEventRequest {
            event_type: "orbiterx_hook_run",
            event_params: orbiterx_hook_run_metadata(&tracking, hook),
        }));
    }
    ...
    fn ingest_plugin_used(&mut self, input: PluginUsedInput, out: &mut Vec<TrackEventRequest>) {
        ...
        out.push(TrackEventRequest::PluginUsed(
            OrbiterXPluginUsedEventRequest {
                event_type: "orbiterx_plugin_used",
                event_params: orbiterx_plugin_used_metadata(&tracking, plugin),
            },
        ));
    }
    ...
    fn ingest_plugin_install_requested(
        ...
        out.push(TrackEventRequest::PluginInstallRequested(
            OrbiterXPluginInstallRequestedEventRequest {
                event_type: "orbiterx_plugin_install_requested",
                event_params: orbiterx_plugin_install_requested_metadata(&tracking, request),
            },
        ));
    }
    ...
    fn ingest_plugin_state_changed(
        ...
        let event = OrbiterXPluginEventRequest {
    ...
    fn ingest_external_agent_config_import_completed(
        ...
        out.push(TrackEventRequest::ExternalAgentConfigImportCompleted(
            OrbiterXOnboardingExternalAgentImportCompleteEventRequest {
                event_type: "orbiterx_onboarding_external_agent_import_complete",
                event_params: OrbiterXOnboardingExternalAgentImportCompleteMetadata {
            ...
            ServerNotification::TurnCompleted(notification) => {
                ...
                turn_state.completed = Some(CompletedTurnState {
                    ...
                    turn_error: notification
                        .turn
                        .error
                        .and_then(|error| error.orbiterx_error_info),
    ...
    fn emit_thread_initialized(
        &mut self,
        connection_id: u64,
        thread: orbiterx_app_server_protocol::Thread,
        model: String,
        initialization_mode: ThreadInitializationMode,
        thread_originator: Option<String>,
        out: &mut Vec<TrackEventRequest>,
    ...
    fn ingest_compaction(
        ...
        out.push(TrackEventRequest::Compaction(Box::new(
            OrbiterXCompactionEventRequest {
                event_type: "orbiterx_compaction_event",
                event_params: orbiterx_compaction_event_params(
                    input,
    ...
    fn ingest_goal(&mut self, input: OrbiterXGoalEvent, out: &mut Vec<TrackEventRequest>) {
        let Some((connection_state, thread_state, thread_metadata)) =
        ...
        out.push(TrackEventRequest::Goal(Box::new(
            OrbiterXGoalEventRequest {
                event_type: "orbiterx_goal_event",
                event_params: orbiterx_goal_event_params(
                    input,
                    thread_metadata.session_id.clone(),
                    thread_state.app_server_client(connection_state),
                    connection_state.runtime.clone(),
    ...
    fn ingest_guardian_review_completed(
        &mut self,
        notification: orbiterx_app_server_protocol::ItemGuardianApprovalReviewCompletedNotification,
        out: &mut Vec<TrackEventRequest>,
    ) {
    ...
    fn emit_turn_steer_event(
        ...
        out.push(TrackEventRequest::TurnSteer(
            OrbiterXTurnSteerEventRequest {
                event_type: "orbiterx_turn_steer_event",
                event_params: OrbiterXTurnSteerEventParams {
    ...
    fn emit_review_event(
        ...
        out.push(TrackEventRequest::ReviewEvent(OrbiterXReviewEventRequest {
    ...
    async fn maybe_emit_turn_event(&mut self, turn_id: &str, out: &mut Vec<TrackEventRequest>) {
        ...
        let turn_event = TrackEventRequest::TurnEvent(Box::new(OrbiterXTurnEventRequest {
            event_type: "orbiterx_turn_event",
            event_params: orbiterx_turn_event_params(
                thread_state.app_server_client(connection_state),
                connection_state.runtime.clone(),
                turn_id.to_string(),
                turn_state,
    ...
    match item {
        ThreadItem::CommandExecution {
            ...
            Some(TrackEventRequest::CommandExecution(
                OrbiterXCommandExecutionEventRequest {
        ...
        ThreadItem::FileChange {
            ...
            Some(TrackEventRequest::FileChange(
                OrbiterXFileChangeEventRequest {
                    event_type: "orbiterx_file_change_event",
                    event_params: OrbiterXFileChangeEventParams {
                        base,
        ...
        ThreadItem::McpToolCall {
            ...
            Some(TrackEventRequest::McpToolCall(
                OrbiterXMcpToolCallEventRequest {
                    event_type: "orbiterx_mcp_tool_call_event",
                    event_params: OrbiterXMcpToolCallEventParams {
                        base,
        ...
        ThreadItem::DynamicToolCall {
            ...
            Some(TrackEventRequest::DynamicToolCall(
                OrbiterXDynamicToolCallEventRequest {
                    event_type: "orbiterx_dynamic_tool_call_event",
                    event_params: OrbiterXDynamicToolCallEventParams {
                        base,
        ...
        ThreadItem::CollabAgentToolCall {
            ...
            Some(TrackEventRequest::CollabAgentToolCall(
                OrbiterXCollabAgentToolCallEventRequest {
                    event_type: "orbiterx_collab_agent_tool_call_event",
                    event_params: OrbiterXCollabAgentToolCallEventParams {
                        base,
        ...
        ThreadItem::WebSearch(item) => {
            ...
            Some(TrackEventRequest::WebSearch(
                OrbiterXWebSearchEventRequest {
                    event_type: "orbiterx_web_search_event",
                    event_params: OrbiterXWebSearchEventParams {
                        base,
        ...
        ThreadItem::ImageGeneration(item) => {
            ...
            Some(TrackEventRequest::ImageGeneration(
                OrbiterXImageGenerationEventRequest {
                    event_type: "orbiterx_image_generation_event",
                    event_params: OrbiterXImageGenerationEventParams {
                        base,
...
fn file_change_counts(
    changes: &[orbiterx_app_server_protocol::FileUpdateChange],
) -> FileChangeCounts {
...
fn orbiterx_turn_event_params(
    app_server_client: OrbiterXAppServerClientMetadata,
    runtime: OrbiterXRuntimeMetadata,
    turn_id: String,
    turn_state: &TurnState,
    thread_metadata: &ThreadMetadataState,
) -> OrbiterXTurnEventParams {
    let (
        Some(thread_id),
    ...
    let orbiterx_error = turn_state.orbiterx_error.as_ref();
    OrbiterXTurnEventParams {
        thread_id,
        session_id: thread_metadata.session_id.clone(),
        turn_id,
        ...
        orbiterx_error_kind: orbiterx_error.map(|error| error.kind),
        orbiterx_error_http_status_code: orbiterx_error.and_then(|error| error.http_status_code),
...
fn analytics_turn_status(status: orbiterx_app_server_protocol::TurnStatus) -> Option<TurnStatus> {
    match status {
        orbiterx_app_server_protocol::TurnStatus::Completed => Some(TurnStatus::Completed),
        orbiterx_app_server_protocol::TurnStatus::Failed => Some(TurnStatus::Failed),
        orbiterx_app_server_protocol::TurnStatus::Interrupted => Some(TurnStatus::Interrupted),
        orbiterx_app_server_protocol::TurnStatus::InProgress => None,
    }
}
