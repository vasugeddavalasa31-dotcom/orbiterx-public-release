...
use crate::events::OrbiterXAcceptedLineFingerprintsEventParams;
use crate::events::OrbiterXAcceptedLineFingerprintsEventRequest;
use crate::events::OrbiterXAppMentionedEventRequest;
use crate::events::OrbiterXAppServerClientMetadata;
use crate::events::OrbiterXAppUsedEventRequest;
use crate::events::OrbiterXCommandExecutionEventParams;
use crate::events::OrbiterXCommandExecutionEventRequest;
use crate::events::OrbiterXCompactionEventRequest;
use crate::events::OrbiterXHookRunEventRequest;
use crate::events::OrbiterXOnboardingExternalAgentImportFailureEventRequest;
use crate::events::OrbiterXOnboardingExternalAgentImportFailureMetadata;
use crate::events::OrbiterXPluginEventRequest;
use crate::events::OrbiterXPluginInstallFailedEventRequest;
use crate::events::OrbiterXPluginInstallFailedMetadata;
use crate::events::OrbiterXPluginUsedEventRequest;
use crate::events::OrbiterXReviewEventParams;
use crate::events::OrbiterXReviewEventRequest;
use crate::events::OrbiterXRuntimeMetadata;
use crate::events::OrbiterXToolItemEventBase;
use crate::events::OrbiterXTurnEventRequest;
...
use crate::events::orbiterx_app_metadata;
use crate::events::orbiterx_hook_run_metadata;
use crate::events::orbiterx_plugin_metadata;
use crate::events::orbiterx_plugin_used_metadata;
...
use crate::facts::OrbiterXCompactionEvent;
use crate::facts::OrbiterXErrKind;
...
use crate::facts::TurnOrbiterXErrorFact;
...
use orbiterx_app_server_protocol::ApprovalsReviewer as AppServerApprovalsReviewer;
use orbiterx_app_server_protocol::AskForApproval as AppServerAskForApproval;
...
use orbiterx_app_server_protocol::CollabAgentToolCallStatus;
use orbiterx_app_server_protocol::CommandAction;
use orbiterx_app_server_protocol::CommandExecutionApprovalDecision;
use orbiterx_app_server_protocol::CommandExecutionRequestApprovalParams;
use orbiterx_app_server_protocol::CommandExecutionRequestApprovalResponse;
use orbiterx_app_server_protocol::CommandExecutionSource;
use orbiterx_app_server_protocol::CommandExecutionStatus;
use orbiterx_app_server_protocol::DynamicToolCallStatus;
use orbiterx_app_server_protocol::GuardianApprovalReview;
use orbiterx_app_server_protocol::GuardianApprovalReviewAction;
use orbiterx_app_server_protocol::GuardianApprovalReviewStatus;
use orbiterx_app_server_protocol::GuardianCommandSource as AppServerGuardianCommandSource;
use orbiterx_app_server_protocol::ImageGenerationItem;
use orbiterx_app_server_protocol::InitializeCapabilities;
use orbiterx_app_server_protocol::InitializeParams;
use orbiterx_app_server_protocol::ItemCompletedNotification;
use orbiterx_app_server_protocol::ItemGuardianApprovalReviewCompletedNotification;
use orbiterx_app_server_protocol::ItemStartedNotification;
use orbiterx_app_server_protocol::JSONRPCErrorError;
use orbiterx_app_server_protocol::McpToolCallAppContext;
use orbiterx_app_server_protocol::McpToolCallStatus;
use orbiterx_app_server_protocol::NonSteerableTurnKind;
use orbiterx_app_server_protocol::OrbiterXErrorInfo;
use orbiterx_app_server_protocol::PatchApplyStatus;
use orbiterx_app_server_protocol::PermissionsRequestApprovalParams;
use orbiterx_app_server_protocol::RequestId;
use orbiterx_app_server_protocol::RequestPermissionProfile;
use orbiterx_app_server_protocol::SandboxPolicy as AppServerSandboxPolicy;
...
use orbiterx_app_server_protocol::SessionSource as AppServerSessionSource;
use orbiterx_app_server_protocol::SubAgentActivityKind;
...
use orbiterx_app_server_protocol::ThreadSource as AppServerThreadSource;
use orbiterx_app_server_protocol::ThreadStartResponse;
use orbiterx_app_server_protocol::ThreadStatus as AppServerThreadStatus;
use orbiterx_app_server_protocol::Turn;
use orbiterx_app_server_protocol::TurnCompletedNotification;
use orbiterx_app_server_protocol::TurnDiffUpdatedNotification;
use orbiterx_app_server_protocol::TurnError as AppServerTurnError;
use orbiterx_app_server_protocol::TurnStartParams;
use orbiterx_app_server_protocol::TurnStartedNotification;
use orbiterx_app_server_protocol::TurnStatus as AppServerTurnStatus;
use orbiterx_app_server_protocol::TurnSteerParams;
use orbiterx_app_server_protocol::TurnSteerResponse;
use orbiterx_app_server_protocol::UserInput;
use orbiterx_app_server_protocol::WebSearchItem;
use orbiterx_login::default_client::DEFAULT_ORIGINATOR;
use orbiterx_login::default_client::originator;
use orbiterx_plugin::AppConnectorId;
use orbiterx_plugin::PluginCapabilitySummary;
use orbiterx_plugin::PluginId;
use orbiterx_plugin::PluginTelemetryMetadata;
use orbiterx_protocol::approvals::NetworkApprovalProtocol;
...
use orbiterx_protocol::models::NetworkPermissions as CoreNetworkPermissions;
use orbiterx_protocol::models::PermissionProfile as CorePermissionProfile;
...
use orbiterx_protocol::request_permissions::PermissionGrantScope as CorePermissionGrantScope;
use orbiterx_protocol::request_permissions::RequestPermissionProfile as CoreRequestPermissionProfile;
use orbiterx_protocol::request_permissions::RequestPermissionsResponse as CoreRequestPermissionsResponse;
...
const TEST_PRODUCT_CLIENT_ID: &str = "orbiterx_work_desktop";
...
fn sample_app_server_client_metadata() -> OrbiterXAppServerClientMetadata {
    OrbiterXAppServerClientMetadata {
...
fn sample_runtime_metadata() -> OrbiterXRuntimeMetadata {
    OrbiterXRuntimeMetadata {
        orbiterx_rs_version: "0.1.0".to_string(),
...
fn sample_turn_start_response(turn_id: &str) -> ClientResponsePayload {
    ClientResponsePayload::TurnStart(orbiterx_app_server_protocol::TurnStartResponse {
        turn: Turn {
...
fn sample_turn_completed_notification(
    ...
    orbiterx_error_info: Option<orbiterx_app_server_protocol::OrbiterXErrorInfo>,
) -> ServerNotification {
    ServerNotification::TurnCompleted(TurnCompletedNotification {
        ...
        turn: Turn {
            ...
            items: vec![],
            status,
            error: orbiterx_error_info.map(|orbiterx_error_info| AppServerTurnError {
                message: "turn failed".to_string(),
                orbiterx_error_info: Some(orbiterx_error_info),
    ...
    JSONRPCErrorError {
        ...
        data: Some(
            serde_json::to_value(AppServerTurnError {
                message: "cannot steer a review turn".to_string(),
                orbiterx_error_info: Some(OrbiterXErrorInfo::ActiveTurnNotSteerable {
            ...
            AnalyticsFact::Initialize {
                ...
                params: InitializeParams {
                    client_info: ClientInfo {
                        name: "orbiterx-web".to_string(),
                        title: None,
                        version: "1.0.0".to_string(),
                    },
                    capabilities: None,
                },
                product_client_id: "orbiterx-web".to_string(),
    ...
    reducer
        .ingest(
            AnalyticsFact::Initialize {
                ...
                params: InitializeParams {
                    client_info: ClientInfo {
                        name: "orbiterx-tui".to_string(),
                        title: None,
                        version: "1.0.0".to_string(),
                    },
                    capabilities: None,
                },
                product_client_id: "orbiterx-tui".to_string(),
                runtime: sample_runtime_metadata(),
                rpc_transport: AppServerRpcTransport::Stdio,
            },
            out,
...
fn sample_initialize_fact(connection_id: u64) -> AnalyticsFact {
    AnalyticsFact::Initialize {
        ...
        runtime: OrbiterXRuntimeMetadata {
            orbiterx_rs_version: "0.99.0".to_string(),
    ...
    ServerRequest::PermissionsRequestApproval {
        ...
        params: PermissionsRequestApprovalParams {
            ...
            permissions: RequestPermissionProfile {
                network: Some(orbiterx_app_server_protocol::AdditionalNetworkPermissions {
                    enabled: Some(true),
                }),
                file_system: None,
            },
...
fn sample_guardian_review_completed(
    ...
    ServerNotification::ItemGuardianApprovalReviewCompleted(
        ItemGuardianApprovalReviewCompletedNotification {
            ...
            decision_source: orbiterx_app_server_protocol::AutoReviewDecisionSource::Agent,
...
fn normalize_path_for_skill_id_repo_scoped_uses_relative_path() {
    ...
    let skill_path = PathBuf::from("/repo/root/.orbiterx/skills/doc/SKILL.md");
    ...
    assert_eq!(path, ".orbiterx/skills/doc/SKILL.md");
}

#[test]
fn normalize_path_for_skill_id_user_scoped_uses_absolute_path() {
    let skill_path = PathBuf::from("/Users/abc/.orbiterx/skills/doc/SKILL.md");
...
fn normalize_path_for_skill_id_admin_scoped_uses_absolute_path() {
    let skill_path = PathBuf::from("/etc/orbiterx/skills/doc/SKILL.md");
...
fn normalize_path_for_skill_id_repo_root_not_in_skill_path_uses_absolute_path() {
    let repo_root = PathBuf::from("/repo/root");
    let skill_path = PathBuf::from("/other/path/.orbiterx/skills/doc/SKILL.md");
...
fn app_mentioned_event_serializes_expected_shape() {
    ...
    let event = TrackEventRequest::AppMentioned(OrbiterXAppMentionedEventRequest {
        event_type: "orbiterx_app_mentioned",
        event_params: orbiterx_app_metadata(
            &tracking,
            AppInvocation {
...
fn app_used_event_serializes_expected_shape() {
    ...
    let event = TrackEventRequest::AppUsed(OrbiterXAppUsedEventRequest {
        event_type: "orbiterx_app_used",
        event_params: orbiterx_app_metadata(
            &tracking,
            AppInvocation {
...
fn accepted_line_fingerprints_event_serializes_expected_shape() {
    let event = TrackEventRequest::AcceptedLineFingerprints(Box::new(
        OrbiterXAcceptedLineFingerprintsEventRequest {
            event_type: "orbiterx_accepted_line_fingerprints",
            event_params: OrbiterXAcceptedLineFingerprintsEventParams {
                event_type: "orbiterx.accepted_line_fingerprints",
                turn_id: "turn-1".to_string(),
                thread_id: "thread-1".to_string(),
                product_surface: Some("orbiterx".to_string()),
                model_slug: Some("gpt-5.1-orbiterx".to_string()),
    ...
    assert_eq!(
        ...
        json!({
            ...
            "event_params": {
                ...
                "turn_id": "turn-1",
                "thread_id": "thread-1",
                "product_surface": "orbiterx",
                "model_slug": "gpt-5.1-orbiterx",
                "completed_at": 1710000000,
...
fn compaction_event_serializes_expected_shape() {
    let event = TrackEventRequest::Compaction(Box::new(OrbiterXCompactionEventRequest {
        event_type: "orbiterx_compaction_event",
        event_params: crate::events::orbiterx_compaction_event_params(
            OrbiterXCompactionEvent {
                thread_id: "thread-1".to_string(),
                turn_id: "turn-1".to_string(),
                ...
                orbiterx_error_kind: None,
                orbiterx_error_http_status_code: None,
        ...
        json!({
            ...
            "event_params": {
                ...
                "app_server_client": {
                    ...
                    "client_name": "orbiterx-tui",
                    "client_version": "1.0.0",
                    "rpc_transport": "stdio",
                    "experimental_api_enabled": true
                },
                "runtime": {
                    "orbiterx_rs_version": "0.1.0",
                    "runtime_os": "macos",
                ...
                "strategy": "memento",
                "status": "completed",
                "orbiterx_error_kind": null,
                "orbiterx_error_http_status_code": null,
...
fn thread_initialized_event_serializes_expected_shape() {
    let event = TrackEventRequest::ThreadInitialized(ThreadInitializedEvent {
        event_type: "orbiterx_thread_initialized",
        event_params: ThreadInitializedEventParams {
            thread_id: "thread-0".to_string(),
            session_id: "session-thread-0".to_string(),
            app_server_client: OrbiterXAppServerClientMetadata {
                product_client_id: DEFAULT_ORIGINATOR.to_string(),
                client_name: Some("orbiterx-tui".to_string()),
                client_version: Some("1.0.0".to_string()),
                rpc_transport: AppServerRpcTransport::Stdio,
...
fn command_execution_event_serializes_expected_shape() {
    let event = TrackEventRequest::CommandExecution(OrbiterXCommandExecutionEventRequest {
        event_type: "orbiterx_command_execution_event",
        event_params: OrbiterXCommandExecutionEventParams {
            base: OrbiterXToolItemEventBase {
                thread_id: "thread-1".to_string(),
                session_id: "session-thread-1".to_string(),
                turn_id: "turn-1".to_string(),
                item_id: "item-1".to_string(),
                app_server_client: OrbiterXAppServerClientMetadata {
                    product_client_id: "orbiterx_tui".to_string(),
                    client_name: Some("orbiterx-tui".to_string()),
    ...
    assert_eq!(
        ...
        json!({
            ...
            "event_params": {
                ...
                "item_id": "item-1",
                "app_server_client": {
                    "product_client_id": "orbiterx_tui",
                    "client_name": "orbiterx-tui",
...
fn review_event_serializes_expected_shape() {
    let event = TrackEventRequest::ReviewEvent(OrbiterXReviewEventRequest {
        event_type: "orbiterx_review_event",
        event_params: OrbiterXReviewEventParams {
            thread_id: "thread-1".to_string(),
            turn_id: "turn-1".to_string(),
            item_id: None,
...
async fn initialize_caches_client_and_thread_lifecycle_publishes_once_initialized() {
    ...
    assert_eq!(payload[0]["event_type"], "orbiterx_thread_initialized");
    ...
    assert_eq!(
        payload[0]["event_params"]["runtime"]["orbiterx_rs_version"],
        "0.99.0"
    );
...
async fn thread_originator_overrides_shared_connection_across_thread_events() {
    ...
    assert_eq!(
        ...
        vec![
            ...
            json!({
                "event_type": "orbiterx_turn_event",
                "product_client_id": TEST_PRODUCT_CLIENT_ID,
            }),
            json!({
                "event_type": "orbiterx_compaction_event",
...
async fn compaction_event_ingests_custom_fact() {
    ...
    let parent_thread_id =
        orbiterx_protocol::ThreadId::from_string("22222222-2222-2222-2222-222222222222")
        ...
        .ingest(
            AnalyticsFact::Custom(CustomAnalyticsFact::Compaction(Box::new(
                OrbiterXCompactionEvent {
                    ...
                    orbiterx_error_kind: Some(OrbiterXErrKind::ContextWindowExceeded),
                    orbiterx_error_http_status_code: None,
                    active_context_tokens_before: 131_000,
    ...
    let payload = serde_json::to_value(&events).expect("serialize events");
    assert_eq!(payload.as_array().expect("events array").len(), 1);
    assert_eq!(payload[0]["event_type"], "orbiterx_compaction_event");
    assert_eq!(payload[0]["event_params"]["session_id"], "session-thread-1");
    assert_eq!(payload[0]["event_params"]["thread_id"], "thread-1");
    assert_eq!(payload[0]["event_params"]["turn_id"], "turn-compact");
    assert_eq!(
        payload[0]["event_params"]["orbiterx_error_kind"],
        json!("context_window_exceeded")
    );
    assert_eq!(
        payload[0]["event_params"]["orbiterx_error_http_status_code"],
        json!(null)
    );
    assert_eq!(
        payload[0]["event_params"]["app_server_client"]["product_client_id"],
...
async fn guardian_review_event_ingests_custom_fact_with_optional_target_item() {
        ...
        .ingest(
            AnalyticsFact::Custom(CustomAnalyticsFact::GuardianReview(Box::new(
                GuardianReviewEventParams {
                    ...
                    guardian_default_review_model_id: Some("orbiterx-auto-review".to_string()),
    ...
    let payload = serde_json::to_value(&events).expect("serialize events");
    assert_eq!(payload.as_array().expect("events array").len(), 1);
    assert_eq!(payload[0]["event_type"], "orbiterx_guardian_review");
    assert_eq!(
...
async fn item_lifecycle_notifications_publish_command_execution_event() {
    ...
    let payload = serde_json::to_value(&events).expect("serialize events");
    assert_eq!(payload.as_array().expect("events array").len(), 1);
    assert_eq!(payload[0]["event_type"], "orbiterx_command_execution_event");
    assert_eq!(payload[0]["event_params"]["thread_id"], "thread-1");
    assert_eq!(payload[0]["event_params"]["session_id"], "session-thread-1");
...
async fn command_execution_approval_response_publishes_user_review_event() {
    ...
    let payload = serde_json::to_value(&events).expect("serialize events");
    assert_eq!(payload.as_array().expect("events array").len(), 1);
    assert_eq!(payload[0]["event_type"], "orbiterx_review_event");
    assert_eq!(payload[0]["event_params"]["thread_id"], "thread-1");
    assert_eq!(payload[0]["event_params"]["turn_id"], "turn-1");
    assert_eq!(payload[0]["event_params"]["item_id"], "item-1");
...
async fn guardian_completed_notification_publishes_review_event_with_thread_metadata() {
    ...
    assert_eq!(payload["event_type"], "orbiterx_review_event");
...
fn subagent_thread_started_review_serializes_expected_shape() {
    let event = TrackEventRequest::ThreadInitialized(subagent_thread_started_event_request(
        SubAgentThreadStartedInput {
            ...
            forked_from_thread_id: None,
            product_client_id: "orbiterx-tui".to_string(),
            client_name: "orbiterx-tui".to_string(),
...
fn subagent_thread_started_thread_spawn_serializes_thread_lineage() {
    let parent_thread_id =
        orbiterx_protocol::ThreadId::from_string("11111111-1111-1111-1111-111111111111")
            .expect("valid thread id");
    let forked_from_thread_id =
        orbiterx_protocol::ThreadId::from_string("22222222-2222-4222-8222-222222222222")
            .expect("valid thread id");
    let event = TrackEventRequest::ThreadInitialized(subagent_thread_started_event_request(
        SubAgentThreadStartedInput {
            session_id: "session-root".to_string(),
...
fn subagent_thread_started_other_serializes_explicit_parent_thread_id() {
    let parent_thread_id =
        orbiterx_protocol::ThreadId::from_string("33333333-3333-4333-8333-333333333333")
            .expect("valid thread id");
    let event = TrackEventRequest::ThreadInitialized(subagent_thread_started_event_request(
        SubAgentThreadStartedInput {
            session_id: "session-root".to_string(),
...
async fn subagent_events_keep_thread_originator_with_explicit_turn_connection() {
    ...
    let mut events = Vec::new();
    let parent_thread_id =
        orbiterx_protocol::ThreadId::from_string("44444444-4444-4444-4444-444444444444")
            .expect("valid parent thread id");
    ...
    assert_eq!(params.runtime.orbiterx_rs_version, "0.1.0");
...
fn plugin_used_event_serializes_expected_shape() {
    ...
    let event = TrackEventRequest::PluginUsed(OrbiterXPluginUsedEventRequest {
        event_type: "orbiterx_plugin_used",
        event_params: orbiterx_plugin_used_metadata(&tracking, sample_plugin_metadata()),
    });
...
fn plugin_management_event_serializes_expected_shape() {
    let event = TrackEventRequest::PluginInstalled(OrbiterXPluginEventRequest {
        event_type: "orbiterx_plugin_installed",
        event_params: orbiterx_plugin_metadata(sample_plugin_metadata()),
    });
...
fn plugin_install_failed_event_serializes_expected_shape() {
    let event = TrackEventRequest::PluginInstallFailed(OrbiterXPluginInstallFailedEventRequest {
        event_type: "orbiterx_plugin_install_failed",
        event_params: OrbiterXPluginInstallFailedMetadata {
            plugin: orbiterx_plugin_metadata(sample_plugin_metadata()),
...
fn plugin_management_event_keeps_plugin_id_local_when_remote_id_exists() {
    ...
    let event = TrackEventRequest::PluginInstalled(OrbiterXPluginEventRequest {
        event_type: "orbiterx_plugin_installed",
        event_params: orbiterx_plugin_metadata(plugin),
    });
...
fn hook_run_event_serializes_expected_shape() {
    let tracking = test_tracking_context("thread-3", "turn-3");
    let event = TrackEventRequest::HookRun(OrbiterXHookRunEventRequest {
        event_type: "orbiterx_hook_run",
        event_params: orbiterx_hook_run_metadata(
            &tracking,
            HookRunFact {
...
fn hook_run_metadata_maps_sources_and_statuses() {
    ...
    let system = serde_json::to_value(orbiterx_hook_run_metadata(
        &tracking,
        HookRunFact {
    ...
    let project = serde_json::to_value(orbiterx_hook_run_metadata(
        &tracking,
        HookRunFact {
    ...
    let cloud_requirements = serde_json::to_value(orbiterx_hook_run_metadata(
        &tracking,
        HookRunFact {
            event_name: HookEventName::Stop,
    ...
    let unknown = serde_json::to_value(orbiterx_hook_run_metadata(
        &tracking,
        HookRunFact {
...
fn hook_run_metadata_maps_stopped_status() {
    ...
    let stopped = serde_json::to_value(orbiterx_hook_run_metadata(
        &tracking,
        HookRunFact {
            event_name: HookEventName::Stop,
            hook_source: HookSource::User,
        ...
        .ingest(
            AnalyticsFact::Custom(CustomAnalyticsFact::SkillInvoked(SkillInvokedInput {
                ...
                invocations: vec![SkillInvocation {
                    ...
                    skill_scope: orbiterx_protocol::protocol::SkillScope::User,
                    skill_path,
                    plugin_id: None,
...
async fn reducer_includes_plugin_id_for_plugin_skill_invocations() {
    ...
    let tracking = test_tracking_context("thread-1", "turn-1");
    let skill_path =
        PathBuf::from("/Users/abc/.orbiterx/plugins/cache/test/sample/skills/doc/SKILL.md");
...
async fn reducer_ingests_hook_run_fact() {
    ...
    let payload = serde_json::to_value(&events).expect("serialize events");
    assert_eq!(payload.as_array().expect("events array").len(), 1);
    assert_eq!(payload[0]["event_type"], "orbiterx_hook_run");
...
async fn reducer_ingests_app_and_plugin_facts() {
    ...
    assert_eq!(payload[0]["event_type"], "orbiterx_app_mentioned");
    assert_eq!(payload[1]["event_type"], "orbiterx_app_used");
    assert_eq!(payload[2]["event_type"], "orbiterx_plugin_used");
    assert_eq!(
...
async fn reducer_ingests_plugin_state_changed_fact() {
    ...
    assert_eq!(
        payload,
        json!([{
            "event_type": "orbiterx_plugin_disabled",
            "event_params": {
                "plugin_id": "sample@test",
                "remote_plugin_id": null,
                "plugin_name": "sample",
...
async fn reducer_ingests_plugin_install_requested_fact() {
    ...
    assert_eq!(
        ...
        json!([{
            "event_type": "orbiterx_plugin_install_requested",
            "event_params": {
...
async fn reducer_ingests_external_agent_config_import_completed_fact() {
    ...
    assert_eq!(
        payload,
        json!([{
            "event_type": "orbiterx_onboarding_external_agent_import_complete",
            "event_params": {
                "import_id": "import-1",
                "source": "app_server",
                "type": "PLUGINS",
...
fn external_agent_config_import_failure_event_serializes_expected_shape() {
    let event = TrackEventRequest::ExternalAgentConfigImportFailure(
        OrbiterXOnboardingExternalAgentImportFailureEventRequest {
            event_type: "orbiterx_onboarding_external_agent_import_failure",
            event_params: OrbiterXOnboardingExternalAgentImportFailureMetadata {
                import_id: "import-1".to_string(),
                source: "app_server".to_string(),
                item_type: "PLUGINS".to_string(),
...
fn turn_event_serializes_expected_shape() {
    let event = TrackEventRequest::TurnEvent(Box::new(OrbiterXTurnEventRequest {
        event_type: "orbiterx_turn_event",
        event_params: crate::events::OrbiterXTurnEventParams {
            thread_id: "thread-2".to_string(),
    ...
    let expected = serde_json::from_str::<serde_json::Value>(
        r#"{
            ...
            "event_params": {
                ...
                "app_server_client": {
                    "product_client_id": "orbiterx_cli_rs",
                    "client_name": "orbiterx-tui",
                    "client_version": "1.0.0",
                    "rpc_transport": "stdio",
                    "experimental_api_enabled": true
...
async fn accepted_turn_steer_emits_expected_event() {
    ...
    assert_eq!(payload["event_type"], json!("orbiterx_turn_steer_event"));
    ...
    assert_eq!(
        payload["event_params"]["runtime"]["orbiterx_rs_version"],
        json!("0.1.0")
    );
...
async fn turn_lifecycle_emits_turn_event() {
    ...
    assert_eq!(payload["event_type"], json!("orbiterx_turn_event"));
    assert_eq!(payload["event_params"]["thread_id"], json!("thread-2"));
    assert_eq!(
        payload["event_params"]["session_id"],
        json!("session-thread-2")
    ...
    assert_eq!(
        ...
        json!({
            "product_client_id": "orbiterx-tui",
            "client_name": "orbiterx-tui",
            "client_version": "1.0.0",
            "rpc_transport": "stdio",
...
async fn turn_event_counts_completed_tool_items() {
    ...
    assert_eq!(
        emitted_tool_events,
        vec![
            ("orbiterx_command_execution_event", "session-thread-2"),
            ("orbiterx_file_change_event", "session-thread-2"),
            ("orbiterx_mcp_tool_call_event", "session-thread-2"),
            ("orbiterx_dynamic_tool_call_event", "session-thread-2"),
            ("orbiterx_collab_agent_tool_call_event", "session-thread-2"),
            ("orbiterx_web_search_event", "session-thread-2"),
            ("orbiterx_image_generation_event", "session-thread-2"),
        ]
    );
...
async fn turn_lifecycle_emits_failed_turn_event() {
    ...
    reducer
        .ingest(
            AnalyticsFact::Custom(CustomAnalyticsFact::TurnOrbiterXError(Box::new(
                TurnOrbiterXErrorFact::from_orbiterx_err(
                    "thread-2".to_string(),
                    "turn-2".to_string(),
                    &OrbiterXErr::InvalidRequest("unknown turn environment id `env-2`".to_string()),
                ),
            ))),
            &mut out,
        )
    ...
    reducer
        .ingest(
            AnalyticsFact::Notification(Box::new(sample_turn_completed_notification(
                ...
                Some(orbiterx_app_server_protocol::OrbiterXErrorInfo::BadRequest),
            ))),
            &mut out,
        )
        .await;
    ...
    assert_eq!(
        payload["event_params"]["orbiterx_error_kind"],
        json!("invalid_request")
    );
    assert_eq!(
        payload["event_params"]["orbiterx_error_http_status_code"],
        json!(null)
    );
}
...
async fn turn_lifecycle_emits_interrupted_turn_event_without_error() {
    ...
    assert_eq!(payload["event_params"]["orbiterx_error_kind"], json!(null));
}
