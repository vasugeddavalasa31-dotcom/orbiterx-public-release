...
use orbiterx_api::ResponsesClient;
use orbiterx_api::ResponsesOptions;
use orbiterx_client::HttpTransport;
use orbiterx_client::Request;
use orbiterx_client::RequestBody;
...
fn provider(name: &str) -> Provider {
    Provider {
        ...
        query_params: None,
        headers: HeaderMap::new(),
        retry: orbiterx_api::RetryConfig {
            max_attempts: 1,
...
struct FlakyTransportState {
    attempts: i64,
    requests: Vec<(RequestBody, HeaderMap, orbiterx_client::RequestCompression)>,
}
...
impl FlakyTransport {
    ...
    fn requests(&self) -> Vec<(RequestBody, HeaderMap, orbiterx_client::RequestCompression)> {
        self.state
            .lock()
            .expect("flaky transport state mutex should not be poisoned")
            .requests
...
async fn streaming_client_retries_on_transport_error() -> Result<()> {
    ...
    assert_eq!(requests[0].2, orbiterx_client::RequestCompression::None);
    Ok(())
}
