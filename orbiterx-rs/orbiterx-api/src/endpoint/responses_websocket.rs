...
use orbiterx_websocket_client::WebSocketConnection;
use orbiterx_websocket_client::WebSocketConnector;
...
const X_ORBITERX_TURN_STATE_HEADER: &str = "x-orbiterx-turn-state";
...
const RESPONSES_WEBSOCKET_TIMING_EVENT_TARGET: &str = "orbiterx_api::responses_websocket_timing";
...
async fn connect_websocket(
    ...
    if let Some(turn_state) = turn_state
        && let Some(header_value) = response
            .headers()
            .get(X_ORBITERX_TURN_STATE_HEADER)
            .and_then(|value| value.to_str().ok())
    {
...
fn emit_responses_websocket_timing_event(
    ...
    // `RUST_LOG='orbiterx_api::responses_websocket_timing=trace'`.
    tracing::event!(
...
mod tests {
    ...
    fn parse_wrapped_websocket_error_event_maps_to_transport_http() {
        ...
        assert_eq!(
            headers
                .get("x-orbiterx-primary-used-percent")
                .and_then(|value| value.to_str().ok()),
            Some("100.0")
        );
        assert_eq!(
            headers
                .get("x-orbiterx-primary-window-minutes")
                .and_then(|value| value.to_str().ok()),
            Some("15")
        );
    ...
    fn websocket_safety_buffering_uses_event_before_header_fallback() {
        let metadata: ResponsesStreamEvent = serde_json::from_value(json!({
            "type": "orbiterx.response.metadata",
            "headers": {
                "x-orbiterx-safety-buffering-enabled": "true",
                "x-orbiterx-safety-buffering-faster-model": "gpt-fast-header"
            }
        }))
    ...
    fn websocket_safety_buffering_event_controls_visibility_when_header_disables_it() {
        let metadata: ResponsesStreamEvent = serde_json::from_value(json!({
            "type": "orbiterx.response.metadata",
            "headers": {
                "x-orbiterx-safety-buffering-enabled": "false",
                "x-orbiterx-safety-buffering-faster-model": "gpt-fast-header"
            }
        }))
        .expect("deserialize treatment metadata");
