...
use chrono::Utc;
use http::HeaderMap;
use orbiterx_protocol::auth::PlanType;
use orbiterx_protocol::error::OrbiterXErr;
use orbiterx_protocol::error::RetryLimitReachedError;
use orbiterx_protocol::error::UnexpectedResponseError;
use orbiterx_protocol::error::UsageLimitReachedError;
use serde::Deserialize;
use serde_json::Value;

pub fn map_api_error(err: ApiError) -> OrbiterXErr {
    match err {
        ApiError::ContextWindowExceeded => OrbiterXErr::ContextWindowExceeded,
        ApiError::QuotaExceeded => OrbiterXErr::QuotaExceeded,
        ApiError::UsageNotIncluded => OrbiterXErr::UsageNotIncluded,
        ApiError::Retryable { message, delay } => OrbiterXErr::Stream(message, delay),
        ApiError::Stream(msg) => OrbiterXErr::Stream(msg, None),
        ApiError::ServerOverloaded => OrbiterXErr::ServerOverloaded,
        ...
        ApiError::InvalidRequest { message } => OrbiterXErr::InvalidRequest(message),
        ApiError::CyberPolicy { message } => OrbiterXErr::CyberPolicy { message },
        ApiError::Transport(transport) => match transport {
            TransportError::Http {
                ...
                {
                    return OrbiterXErr::ServerOverloaded;
                }
                ...
                if status == http::StatusCode::BAD_REQUEST {
                            ...
                            == Some(CYBER_POLICY_ERROR_CODE)
                    {
                        ...
                        OrbiterXErr::CyberPolicy { message }
                    } else if body_text
                        .contains("The image data you provided does not represent a valid image")
                    {
                        OrbiterXErr::InvalidImageRequest()
                    } else {
                        OrbiterXErr::InvalidRequest(body_text)
                    }
                    ...
                    if let Ok(err) = serde_json::from_str::<UsageErrorResponse>(&body_text) {
                        if err.error.error_type.as_deref() == Some("usage_limit_reached") {
                            ...
                            return OrbiterXErr::UsageLimitReached(UsageLimitReachedError {
                            ...
                            return OrbiterXErr::UsageNotIncluded;
                        }
                    }

                    OrbiterXErr::RetryLimit(RetryLimitReachedError {
                        status,
            ...
            TransportError::RetryLimit => OrbiterXErr::RetryLimit(RetryLimitReachedError {
                status: http::StatusCode::INTERNAL_SERVER_ERROR,
                request_id: None,
            }),
            TransportError::Timeout => OrbiterXErr::RequestTimeout,
            TransportError::Network(msg) | TransportError::Build(msg) => {
                OrbiterXErr::Stream(msg, None)
            }
        },
        ApiError::RateLimit(msg) => OrbiterXErr::Stream(msg, None),
    }
}

const ACTIVE_LIMIT_HEADER: &str = "x-orbiterx-active-limit";
