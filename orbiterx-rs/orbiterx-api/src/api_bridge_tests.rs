...
fn map_api_error_maps_server_overloaded() {
    ...
    assert!(matches!(err, OrbiterXErr::ServerOverloaded));
}
...
fn map_api_error_maps_cloudflare_blocked_response_to_user_message() {
    ...
    let OrbiterXErr::UnexpectedStatus(err) = err else {
        panic!("expected OrbiterXErr::UnexpectedStatus, got {err:?}");
    };
    assert_eq!(
...
fn map_api_error_maps_cyber_policy_from_400_body() {
    ...
    let OrbiterXErr::CyberPolicy { message } = err else {
        panic!("expected OrbiterXErr::CyberPolicy, got {err:?}");
    };
    assert_eq!(
        message,
...
fn map_api_error_keeps_unknown_400_errors_generic() {
    ...
    let OrbiterXErr::InvalidRequest(message) = err else {
        panic!("expected OrbiterXErr::InvalidRequest, got {err:?}");
    };
...
fn map_api_error_maps_usage_limit_limit_name_header() {
    ...
    headers.insert(
        ACTIVE_LIMIT_HEADER,
        http::HeaderValue::from_static("orbiterx_other"),
    );
    headers.insert(
        "x-orbiterx-other-limit-name",
        http::HeaderValue::from_static("orbiterx_other"),
    );
    let body = serde_json::json!({
        "error": {
    ...
    let OrbiterXErr::UsageLimitReached(usage_limit) = err else {
        panic!("expected OrbiterXErr::UsageLimitReached, got {err:?}");
    };
    assert_eq!(
        usage_limit
            .rate_limits
...
fn map_api_error_copies_rate_limit_reached_type_to_usage_limit_snapshot() {
    for (active_limit, expected_limit_id) in [
        (None, "orbiterx"),
        (Some("orbiterx_other"), "orbiterx_other"),
    ] {
        let mut headers = HeaderMap::new();
...
fn map_api_error_ignores_unparseable_rate_limit_reached_type_headers() {
    ...
    for value in values {
        let mut headers = HeaderMap::new();
        headers.insert("x-orbiterx-rate-limit-reached-type", value);
        let body = serde_json::json!({
            "error": {
                "type": "usage_limit_reached",
                "plan_type": "pro",
