...
Runtime orchestration for Phase 1 and Phase 2 still lives in `orbiterx-core` under
`orbiterx-rs/core/src/memories/`.

## Crates

- `orbiterx-rs/memories/read` (`orbiterx-memories-read`) owns the read path:
  memory developer-instruction injection, memory citation parsing, and
  read-usage telemetry classification.
- `orbiterx-rs/memories/write` (`orbiterx-memories-write`) owns the write path:
...
- In `orbiterx`, edit those undated template files in place.
...
- keeps the memories root itself as a git-baseline directory, initialized under
  `~/.orbiterx/memories/.git` by `orbiterx-git-utils`
