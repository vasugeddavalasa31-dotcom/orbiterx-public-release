...
use orbiterx_git_utils::diff_since_latest_init;
use orbiterx_git_utils::reset_git_repository;
use orbiterx_login::AuthManager;
use orbiterx_login::OrbiterXAuth;
use orbiterx_model_provider::ModelProvider;
use orbiterx_model_provider::ModelProviderFuture;
use orbiterx_model_provider::ProviderAccountResult;
...
use orbiterx_state::Phase2JobClaimOutcome;
...
async fn memories_startup_phase2_tracks_workspace_diff_across_runs() -> anyhow::Result<()> {
    ...
    let test = build_test_orbiterx(&server, home.clone()).await?;
    trigger_memories_startup(&test).await;
...
async fn phase2_retries_when_clean_workspace_is_missing_artifacts() -> anyhow::Result<()> {
    ...
    assert_eq!(
        ...
        "# Memory Workspace Diff\n\nGenerated by OrbiterX before Phase 2 memory consolidation. Read this file first and do not edit it.\n\n## Status\n- none\n"
    );
...
async fn run_memory_phase_one_model_request_test(
    ...
    let test =
        build_test_orbiterx_with_memories_config(server, Arc::clone(&home), memories).await?;
...
async fn run_memory_phase_two_model_request_test(
    ...
    let test = build_test_orbiterx_with_memories_config(server, home.clone(), memories).await?;
    let provider = Arc::new(MockMemoryModelProvider::new(
        test.config.model_provider.clone(),
        Some(test.thread_manager.auth_manager()),
    ));
    ...
    let (context, config) = memory_startup_context_with_provider(&test, provider).await;
    let root = memory_root(&config.orbiterx_home);
...
async fn build_test_orbiterx(
    server: &wiremock::MockServer,
    home: Arc<TempDir>,
) -> anyhow::Result<TestOrbiterX> {
    build_test_orbiterx_with_memories_config(server, home, startup_test_memories_config()).await
}

async fn build_test_orbiterx_with_memories_config(
    server: &wiremock::MockServer,
    home: Arc<TempDir>,
    memories: MemoriesConfig,
) -> anyhow::Result<TestOrbiterX> {
...
async fn init_state_db(home: &Arc<TempDir>) -> anyhow::Result<Arc<orbiterx_state::StateRuntime>> {
    let db = orbiterx_state::StateRuntime::init(home.path().to_path_buf(), "test-provider".into())
        .await?;
    db.mark_backfill_complete(/*last_watermark*/ None).await?;
    Ok(db)
}

async fn trigger_memories_startup(test: &TestOrbiterX) {
    let config_snapshot = test.orbiterx.config_snapshot().await;
...
async fn memory_startup_context_with_provider(
    ...
    Arc<orbiterx_core::config::Config>,
) {
    let config_snapshot = test.orbiterx.config_snapshot().await;
    let mut config = test.config.clone();
    config
...
impl ModelProvider for MockMemoryModelProvider {
    ...
    ) -> orbiterx_models_manager::manager::SharedModelsManager {
        self.delegate
            .models_manager(orbiterx_home, config_model_catalog)
    }
}

async fn seed_stage1_output(
    db: &orbiterx_state::StateRuntime,
    orbiterx_home: &Path,
    ...
    let mut metadata_builder = orbiterx_state::ThreadMetadataBuilder::new(
        thread_id,
        orbiterx_home.join(format!("rollout-{thread_id}.jsonl")),
        updated_at,
        SessionSource::Cli,
    );
    metadata_builder.cwd = orbiterx_home.join(format!("workspace-{rollout_slug}"));
...
async fn seed_stage1_candidate(
    ...
    let thread_id = ThreadId::new();
    let rollout_path = orbiterx_home.join(format!("rollout-{thread_id}.jsonl"));
...
) -> anyhow::Result<orbiterx_core::ThreadConfigSnapshot> {
    let deadline = Instant::now() + Duration::from_secs(10);
    loop {
        let config_snapshot = test.orbiterx.config_snapshot().await;
...
async fn seed_stage1_output_for_existing_thread(
    ...
    let ownership_token = match claim {
        orbiterx_state::Stage1JobClaimOutcome::Claimed { ownership_token } => ownership_token,
...
async fn shutdown_test_orbiterx(test: &TestOrbiterX) -> anyhow::Result<()> {
    test.orbiterx.submit(Op::Shutdown {}).await?;
    wait_for_event(&test.orbiterx, |ev| {
