...
use orbiterx_file_system::FindUpErrorPolicy;
use orbiterx_file_system::find_nearest_native_ancestor_with_markers;
...
/// directory. If you need OrbiterX to work from such a checkout simply pass the
...
mod tests {
    ...
    fn canonicalize_git_remote_url_normalizes_github_variants() {
        for remote in [
            "git@github.com:OpenAI/OrbiterX.git",
            "ssh://git@github.com/openai/orbiterx.git",
            "ssh://git@github.com:22/OpenAI/OrbiterX.git",
            "https://github.com/openai/orbiterx.git",
            "https://github.com:443/openai/orbiterx.git",
            "https://token@github.com/openai/orbiterx/",
            "github.com/OpenAI/OrbiterX.git",
        ] {
            assert_eq!(
                canonicalize_git_remote_url(remote),
                Some("github.com/openai/orbiterx".to_string())
            );
        }
    }
    ...
    async fn local_git_branches_excludes_detached_head_entry() {
        ...
        run_git(&[
            "-c",
            "user.name=OrbiterX Tests",
            "-c",
            "user.email=orbiterx-tests@example.com",
            "commit",
            "--allow-empty",
            "-q",
            "-m",
