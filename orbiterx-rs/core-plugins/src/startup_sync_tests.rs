...
fn write_curated_plugin_sha(orbiterx_home: &Path) {
    write_file(
...
fn has_plugins_clone_dirs(orbiterx_home: &Path) -> bool {
    let Ok(entries) = std::fs::read_dir(orbiterx_home.join(".tmp")) else {
        return false;
    };
...
async fn run_http_sync(
    ...
    let api_base_url = api_base_url.into();
    tokio::task::spawn_blocking(move || {
        sync_openai_plugins_repo_via_http(orbiterx_home.as_path(), &api_base_url)
    })
    .await
    .expect("sync task should join")
}
...
fn assert_curated_gmail_repo(repo_path: &Path) {
    ...
    assert!(
        repo_path
            .join("plugins/gmail/.orbiterx-plugin/plugin.json")
            .is_file()
    );
}

#[test]
fn curated_plugins_repo_path_uses_orbiterx_home_tmp_dir() {
...
fn sync_openai_plugins_repo_via_git_succeeds_with_local_rewritten_remote() {
    ...
    std::fs::create_dir_all(work_repo.join("plugins/gmail/.orbiterx-plugin"))
    ...
    std::fs::write(
        work_repo.join("plugins/gmail/.orbiterx-plugin/plugin.json"),
        r#"{"name":"gmail"}"#,
    )
    ...
    run_git(
        ...
        &[
            "-c",
            "user.name=OrbiterX Test",
            "-c",
            "user.email=orbiterx@example.com",
            "commit",
            "-m",
            "init",
        ],
    ...
    assert!(
        curated_plugins_repo_path(tmp.path())
            .join("plugins/linear/.orbiterx-plugin/plugin.json")
            .is_file()
    );
    assert_eq!(
        read_curated_plugins_sha(tmp.path()).as_deref(),
...
if [ "$1" = "-C" ] && [ "$3" = "reset" ]; then
  mkdir -p "$2/plugins/linear/.orbiterx-plugin"
  printf '%s\n' '{{"name":"linear"}}' > "$2/plugins/linear/.orbiterx-plugin/plugin.json"
  exit 0
fi
if [ "$1" = "-C" ] && [ "$3" = "clean" ]; then
  exit 0
...
async fn sync_openai_plugins_repo_skips_export_archive_when_snapshot_exists() {
    ...
    let plugin_manifest_path = curated_root.join("plugins/linear/.orbiterx-plugin/plugin.json");
...
fn curated_repo_zipball_bytes(sha: &str) -> Vec<u8> {
  ...
  "plugins": [
    ...
    writer
        .start_file(
            format!("{root}/plugins/gmail/.orbiterx-plugin/plugin.json"),
            options,
        )
...
fn curated_repo_backup_archive_zip_bytes(sha: &str) -> Vec<u8> {
  ...
  "plugins": [
    ...
    writer
        .start_file(
            "plugins/plugins/gmail/.orbiterx-plugin/plugin.json",
            options,
        )
        .expect("start plugin manifest entry");
    writer
