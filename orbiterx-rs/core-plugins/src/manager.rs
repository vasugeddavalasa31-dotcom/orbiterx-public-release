...
use crate::loader::configured_curated_plugin_ids_from_orbiterx_home;
...
use orbiterx_config::clear_user_plugin;
use orbiterx_config::set_user_plugin_enabled;
use orbiterx_config::types::PluginConfig;
...
use orbiterx_core_skills::SkillMetadata;
use orbiterx_core_skills::config_rules::SkillConfigRules;
use orbiterx_core_skills::config_rules::skill_config_rules_from_stack;
use orbiterx_core_skills::loader::MAX_CONCURRENT_ROOT_SCANS;
use orbiterx_hooks::plugin_hook_declarations;
...
use orbiterx_plugin::PluginIdError;
use orbiterx_plugin::PluginTelemetryMetadata;
use orbiterx_plugin::app_connector_ids_from_declarations;
use orbiterx_plugin::prompt_safe_plugin_description;
use orbiterx_protocol::auth::AuthMode;
use orbiterx_protocol::protocol::HookEventName;
use orbiterx_protocol::protocol::Product;
use orbiterx_tools::DiscoverablePluginInfo;
use orbiterx_tools::DiscoverableTool;
...
use orbiterx_utils_plugins::PluginSkillRoot;
...
pub struct RecommendedPluginCandidatesInput<'a> {
    ...
    pub auth: Option<&'a OrbiterXAuth>,
...
fn featured_plugin_ids_cache_key(
    ...
    FeaturedPluginIdsCacheKey {
        chatgpt_base_url: config.chatgpt_base_url.clone(),
        account_id: auth.and_then(OrbiterXAuth::get_account_id),
        chatgpt_user_id: auth.and_then(OrbiterXAuth::get_chatgpt_user_id),
        is_workspace_account: auth.is_some_and(OrbiterXAuth::is_workspace_account),
    }
}
...
impl PluginsManager {
    pub fn new(orbiterx_home: PathBuf) -> Self {
    ...
    pub fn new_with_options(
        ...
        // Product restrictions are enforced at marketplace admission time for a given ORBITERX_HOME:
        ...
        // contents of that ORBITERX_HOME and does not re-filter configured plugins by product, so
        // already-admitted plugins may continue exposing MCP servers/tools from shared local state.
        //
        // This assumes a single ORBITERX_HOME is only used by one product.
        Self {
            orbiterx_home: orbiterx_home.clone(),
            store: PluginStore::new(orbiterx_home),
    ...
    pub fn cached_global_remote_discoverable_plugins_for_config(
        ...
        let Some(auth) = auth.filter(|auth| auth.uses_orbiterx_backend()) else {
            return Vec::new();
        };
    ...
    async fn install_resolved_plugin(
        ...
        let plugin_version =
            if is_openai_curated_marketplace_name(&resolved.plugin_id.marketplace_name) {
                let curated_plugin_version = read_curated_plugins_sha(self.orbiterx_home.as_path())
                    .ok_or_else(|| {
        ...
        let orbiterx_home = self.orbiterx_home.clone();
        ...
        let result: StorePluginInstallResult = tokio::task::spawn_blocking(move || {
            let materialized =
                materialize_marketplace_plugin_source(orbiterx_home.as_path(), &resolved.source)
    ...
    async fn uninstall_plugin_id(&self, plugin_id: PluginId) -> Result<(), PluginUninstallError> {
        ...
        clear_user_plugin(&self.orbiterx_home, plugin_id.as_key())
            .await
            .map_err(anyhow::Error::from)?;
    ...
    pub async fn read_plugin_detail_for_marketplace_plugin(
        ...
        let source_path = if plugin.source.is_install_materialized() && plugin.installed {
            ...
            let materialized = tokio::task::spawn_blocking(move || {
                materialize_marketplace_plugin_source(orbiterx_home.as_path(), &source)
            })
            .await
            .map_err(|err| {
                MarketplaceError::InvalidPlugin(format!(
        ...
        let manifest =
            if orbiterx_utils_plugins::find_plugin_manifest_path(source_path.as_path()).is_some() {
        ...
        let resolved_skills = load_plugin_skills(
            ...
            &manifest,
            self.restriction_product,
            &orbiterx_core_skills::config_rules::skill_config_rules_from_stack(
                &config.config_layer_stack,
            ),
            /*plugin_skill_snapshots*/ None,
            Arc::clone(&self.skill_root_scan_slots),
    ...
    pub fn maybe_start_plugin_startup_tasks_for_config(
        ...
        if config.plugins_enabled {
            let use_remote_global_catalog =
                config.remote_plugin_enabled && auth_manager.current_auth_uses_orbiterx_backend();
    ...
    fn schedule_non_curated_plugin_cache_refresh(
        ...
        let mut configured_plugin_keys =
            configured_plugins_from_stack(&config.config_layer_stack, self.orbiterx_home.as_path())
                .into_keys()
                .collect::<Vec<_>>();
        configured_plugin_keys.sort_unstable();
    ...
    fn start_curated_repo_sync(self: &Arc<Self>) {
        ...
        if let Err(err) = std::thread::Builder::new()
            ...
            .spawn(
                move || match sync_openai_plugins_repo(orbiterx_home.as_path()) {
                    Ok(curated_plugin_version) => {
                        let configured_curated_plugin_ids =
                            configured_curated_plugin_ids_from_orbiterx_home(
        ...
        let curated_marketplace_path = if include_openai_curated {
            if matches!(
                ...
                let api_marketplace_path =
                    curated_plugins_api_marketplace_path(self.orbiterx_home.as_path());
                api_marketplace_path
                    .is_file()
                    .then_some(api_marketplace_path)
            } else {
                let curated_repo_root = curated_plugins_repo_path(self.orbiterx_home.as_path());
