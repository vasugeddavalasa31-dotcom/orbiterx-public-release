...
use orbiterx_utils_plugins::find_plugin_manifest_path;
...
pub type PluginManifest = orbiterx_plugin::manifest::PluginManifest<AbsolutePathBuf>;
pub type PluginManifestHooks = orbiterx_plugin::manifest::PluginManifestHooks<AbsolutePathBuf>;
pub type PluginManifestInterface =
    orbiterx_plugin::manifest::PluginManifestInterface<AbsolutePathBuf>;
pub type PluginManifestMcpServers =
    orbiterx_plugin::manifest::PluginManifestMcpServers<AbsolutePathBuf>;
pub type PluginManifestPaths = orbiterx_plugin::manifest::PluginManifestPaths<AbsolutePathBuf>;

pub type UriPluginManifest = orbiterx_plugin::manifest::PluginManifest<PathUri>;
...
pub fn parse_plugin_manifest_uri(
    ...
    let interface = interface.and_then(|interface| {
        ...
        let interface = orbiterx_plugin::manifest::PluginManifestInterface {
            display_name,
            short_description,
            long_description,
            developer_name,
    ...
    Ok(orbiterx_plugin::manifest::PluginManifest {
        name,
        version,
        description,
        keywords,
        paths: orbiterx_plugin::manifest::PluginManifestPaths {
...
fn resolve_manifest_hooks(
    plugin_root: &PathUri,
    hooks: Option<RawPluginManifestHooks>,
) -> Option<orbiterx_plugin::manifest::PluginManifestHooks<PathUri>> {
    match hooks? {
        RawPluginManifestHooks::Path(path) => {
            resolve_manifest_path(plugin_root, "hooks", Some(&path))
                .map(|path| orbiterx_plugin::manifest::PluginManifestHooks::Paths(vec![path]))
        }
        RawPluginManifestHooks::Paths(paths) => {
            let hooks = paths
                ...
                .collect::<Vec<_>>();
            (!hooks.is_empty())
                .then_some(orbiterx_plugin::manifest::PluginManifestHooks::Paths(hooks))
        }
        RawPluginManifestHooks::Inline(hooks) => Some(
            orbiterx_plugin::manifest::PluginManifestHooks::Inline(vec![*hooks]),
        ),
        RawPluginManifestHooks::InlineList(hooks) => (!hooks.is_empty()).then_some(
            orbiterx_plugin::manifest::PluginManifestHooks::Inline(hooks),
        ),
...
fn resolve_manifest_mcp_servers(
    plugin_root: &PathUri,
    mcp_servers: Option<RawPluginManifestMcpServers>,
) -> Option<orbiterx_plugin::manifest::PluginManifestMcpServers<PathUri>> {
    match mcp_servers? {
        RawPluginManifestMcpServers::Path(path) => {
            resolve_manifest_path(plugin_root, "mcpServers", Some(&path))
                .map(orbiterx_plugin::manifest::PluginManifestMcpServers::Path)
        }
        RawPluginManifestMcpServers::Object(servers) => match serde_json::to_string(&servers) {
            Ok(servers) => Some(orbiterx_plugin::manifest::PluginManifestMcpServers::Object(
                servers,
            )),
            Err(err) => {
...
mod tests {
    ...
    use orbiterx_plugin::PluginProvider;
    use orbiterx_plugin::ResolvedPlugin;
    use orbiterx_plugin::manifest::PluginManifest as GenericPluginManifest;
    use orbiterx_plugin::manifest::PluginManifestHooks;
    use orbiterx_plugin::manifest::PluginManifestInterface;
    use orbiterx_plugin::manifest::PluginManifestMcpServers;
    use orbiterx_plugin::manifest::PluginManifestPaths;
    ...
    fn write_manifest(plugin_root: &Path, version: Option<&str>, interface: &str) {
        fs::create_dir_all(plugin_root.join(".orbiterx-plugin")).expect("create manifest dir");
        let version = version
  ...
  "interface": {{
    ...
    "composerIcon": "./assets/icon.svg"
        ...
        let manifest_contents =
            fs::read_to_string(plugin_root.join(".orbiterx-plugin/plugin.json")).expect("manifest");
