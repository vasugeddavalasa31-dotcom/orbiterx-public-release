...
use orbiterx_app_server_protocol::SkillInterface;
...
const ORBITERX_PRODUCT_SKU: &str = "orbiterx";
    ...
    for source in sources {
        match source {
            RemoteMarketplaceSource::Global => {
                ...
                if let Some(orbiterx_home) = global_catalog_cache_path
                ...
                if let (Some(orbiterx_home), Some(directory_plugins)) =
...
pub async fn fetch_and_cache_global_remote_plugin_catalog(
    ...
    catalog_cache::write_cached_global_directory_plugins(orbiterx_home, config, auth, &plugins);
    Ok(())
}
...
pub fn has_cached_global_remote_plugin_catalog(
    ...
    catalog_cache::load_cached_global_directory_plugins(orbiterx_home, config, auth).is_some()
}
...
pub fn cached_global_remote_discoverable_plugins(
    ...
    catalog_cache::load_cached_global_directory_plugins(orbiterx_home, config, auth)
        .unwrap_or_default()
        .into_iter()
...
fn remove_remote_plugin_cache(
    ...
    let store = PluginStore::try_new(orbiterx_home.clone())
    ...
    let legacy_remote_plugin_cache_root = orbiterx_home
...
fn ensure_chatgpt_auth(
    auth: Option<&OrbiterXAuth>,
) -> Result<&OrbiterXAuth, RemotePluginCatalogError> {
...
fn authenticated_request(
    ...
    Ok(request
        ...
        .headers(orbiterx_model_provider::auth_provider_from_auth(auth).to_auth_headers())
        .header(OAI_PRODUCT_SKU_HEADER, ORBITERX_PRODUCT_SKU))
}
