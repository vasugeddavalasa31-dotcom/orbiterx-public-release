...
pub(crate) async fn load_plugins_config(orbiterx_home: &Path, cwd: &Path) -> PluginsConfigInput {
    let orbiterx_home =
        AbsolutePathBuf::try_from(orbiterx_home).expect("orbiterx home should be absolute");
