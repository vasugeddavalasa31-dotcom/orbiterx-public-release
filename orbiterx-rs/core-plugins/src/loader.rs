...
use orbiterx_config::types::PluginMcpServerConfig;
use orbiterx_connectors::parse_plugin_app_config;
use orbiterx_connectors::parse_plugin_app_config_value;
...
use orbiterx_core_skills::config_rules::resolve_disabled_skill_paths;
use orbiterx_core_skills::config_rules::skill_config_rules_from_stack;
use orbiterx_core_skills::loader::SkillRoot;
use orbiterx_core_skills::loader::load_skills_from_roots;
use orbiterx_exec_server::LOCAL_FS;
use orbiterx_mcp::parse_plugin_mcp_config;
use orbiterx_plugin::AppDeclaration;
use orbiterx_plugin::LoadedPlugin;
...
async fn load_plugins_from_layer_stack_with_scope(
    ...
    let configured_plugins = merge_configured_plugins_with_remote_installed(
        configured_plugins_from_stack(config_layer_stack, store.orbiterx_home().as_path()),
        extra_plugins,
        store,
        remote_global_catalog_active,
    );
...
pub fn refresh_curated_plugin_cache(
    ...
    let store = PluginStore::try_new(orbiterx_home.to_path_buf()).map_err(|err| err.to_string())?;
    let curated_marketplace_paths = curated_marketplace_paths_for_cache_refresh(orbiterx_home)?;
...
fn curated_marketplace_paths_for_cache_refresh(
    ...
    let api_marketplace_path = orbiterx_home
        .join(".tmp/plugins")
...
fn refresh_non_curated_plugin_cache_with_mode(
    ...
    for plugin_id in configured_non_curated_plugin_ids {
        ...
        let refresh_result = (|| -> Result<bool, String> {
            let materialized = materialize_marketplace_plugin_source(orbiterx_home, &source)
                .map_err(|err| {
...
fn configured_plugins_from_orbiterx_home(
    orbiterx_home: &Path,
...
pub fn configured_curated_plugin_ids_from_orbiterx_home(orbiterx_home: &Path) -> Vec<PluginId> {
    curated_plugin_ids_from_config_keys(configured_plugins_from_orbiterx_home(
        orbiterx_home,
    ...
    match &manifest_paths.hooks {
        ...
        Some(PluginManifestHooks::Inline(hooks_files)) => {
            let manifest_path = find_plugin_manifest_path(plugin_root.as_path())
                ...
                .unwrap_or_else(|| plugin_root.join(".orbiterx-plugin/plugin.json"));
...
pub fn materialize_marketplace_plugin_source(
    ...
    match source {
        ...
        MarketplacePluginSource::Git {
            ...
            let staging_root = orbiterx_home.join("plugins/.marketplace-plugin-source-staging");
