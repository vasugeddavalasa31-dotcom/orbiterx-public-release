...
#[tokio::test]
async fn returns_fallback_plugins_when_remote_disabled_for_orbiterx_auth() {
    let orbiterx_home = tempdir().expect("tempdir should succeed");
    write_file(
...
"#,
    ...
    let curated_root = curated_plugins_repo_path(orbiterx_home.path());
    write_openai_curated_marketplace(&curated_root, &["sample", "slack", "openai-developers"]);

    let plugins = load_plugins_config(orbiterx_home.path(), orbiterx_home.path()).await;
...
async fn returns_api_curated_fallback_plugins_for_direct_provider_auth() {
    ...
    let auth = OrbiterXAuth::from_api_key("test-api-key");
    let discoverable_plugins = list_discoverable_plugins(
        &plugins_manager,
        discovery_input(plugins, &[], &[], &[]),
        Some(&auth),
...
async fn returns_microsoft_fallback_plugins() {
    ...
    install_marketplace_plugin(orbiterx_home.path(), curated_root.as_path(), "teams").await;
...
#[tokio::test]
async fn omits_openai_curated_but_keeps_configured_marketplaces_for_remote_orbiterx_auth() {
    let orbiterx_home = tempdir().expect("tempdir should succeed");
    let curated_root = curated_plugins_repo_path(orbiterx_home.path());
    write_openai_curated_marketplace(&curated_root, &["slack"]);

    let bundled_marketplace_name = OPENAI_BUNDLED_MARKETPLACE_NAME;
    let bundled_marketplace_root = orbiterx_home
        .path()
...
async fn clear_cache_invalidates_cached_tool_suggest_metadata() {
    ...
    let curated_root = curated_plugins_repo_path(orbiterx_home.path());
    write_openai_curated_marketplace(&curated_root, &["slack"]);
    let plugin_manifest = curated_root.join("plugins/slack/.orbiterx-plugin/plugin.json");
    write_file(
        &plugin_manifest,
        r#"{
  "name": "slack",
...
"#
    ...
    install_marketplace_plugin(orbiterx_home.path(), curated_root.as_path(), "installed").await;
...
async fn normalizes_description() {
    ...
    write_openai_curated_marketplace(&curated_root, &["installed", "slack"]);
    write_file(
        &curated_root.join("plugins/slack/.orbiterx-plugin/plugin.json"),
        r#"{
  "name": "slack",
  "description": "  Plugin\n   with   extra   spacing  "
}"#,
...
async fn omits_installed_curated_plugins() {
    ...
    let curated_root = curated_plugins_repo_path(orbiterx_home.path());
    write_openai_curated_marketplace(&curated_root, &["slack"]);
    install_marketplace_plugin(orbiterx_home.path(), curated_root.as_path(), "slack").await;
...
async fn does_not_reload_marketplace_per_plugin() {
    ...
    for plugin_name in ["gmail", "openai-developers"] {
        write_file(
            &curated_root.join(format!(
                "plugins/{plugin_name}/.orbiterx-plugin/plugin.json"
            )),
            &format!(
                r#"{{
  "name": "{plugin_name}",
  ...
  "interface": {{
    ...
    assert_eq!(
        logs.matches("gmail/.orbiterx-plugin/plugin.json").count(),
        4
    );
    assert_eq!(
        logs.matches("openai-developers/.orbiterx-plugin/plugin.json")
            .count(),
        4
    );
}
...
async fn does_not_expand_local_sales_apps() {
    ...
    let sales_marketplace_root = orbiterx_home
        .path()
...
"#,
    ...
    let auth = OrbiterXAuth::create_dummy_chatgpt_auth_for_testing();
    let mut plugins = load_plugins_config(orbiterx_home.path(), orbiterx_home.path()).await;
...
async fn install_marketplace_plugin(
    ...
    let config = load_plugins_config(orbiterx_home, marketplace_root).await;
    PluginsManager::new(orbiterx_home.to_path_buf())
        .install_plugin(
