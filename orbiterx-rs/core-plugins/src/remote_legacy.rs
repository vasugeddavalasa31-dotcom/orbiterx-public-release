...
pub async fn fetch_remote_featured_plugin_ids(
    ...
    let mut request = client
        ...
        .query(&[(
            "platform",
            product.unwrap_or(Product::OrbiterX).to_app_platform(),
        )])
        .timeout(REMOTE_FEATURED_PLUGIN_FETCH_TIMEOUT);

    if let Some(auth) = auth.filter(|auth| auth.uses_orbiterx_backend()) {
        request = request
...
fn ensure_orbiterx_backend_auth(
    auth: Option<&OrbiterXAuth>,
) -> Result<&OrbiterXAuth, RemotePluginMutationError> {
...
async fn post_remote_plugin_mutation(
    ...
    let auth = ensure_orbiterx_backend_auth(auth)?;
