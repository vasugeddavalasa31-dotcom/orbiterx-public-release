...
pub(crate) fn maybe_start_remote_installed_plugin_bundle_sync(
    ...
    let key = RemoteInstalledPluginBundleSyncKey {
        plugin_cache_root: remote_plugin_cache_root(&orbiterx_home),
    };
    ...
    tokio::spawn(async move {
        let result =
            sync_remote_installed_plugin_bundles_once(orbiterx_home, &config, Some(&auth)).await;
        match result {
            Ok(outcome) => {
                info!(
...
fn remove_stale_remote_plugin_caches(
    ...
    for marketplace_name in [
        ...
        let marketplace_root = orbiterx_home.join(PLUGINS_CACHE_DIR).join(marketplace_name);
...
fn remote_plugin_cache_root(orbiterx_home: &Path) -> PathBuf {
    orbiterx_home.join(PLUGINS_CACHE_DIR)
}
...
mod tests {
    ...
    fn remote_installed_plugin_sync_in_flight_dedupes_by_cache_root() {
        ...
        let key = RemoteInstalledPluginBundleSyncKey {
            plugin_cache_root: remote_plugin_cache_root(orbiterx_home.path()),
        };
    ...
    async fn sync_same_version_backfills_metadata_without_materialization() {
        ...
        let orbiterx_home = tempfile::tempdir().expect("create orbiterx home");
        let cached_manifest = orbiterx_home
            .path()
        ...
        let metadata_path = PluginStore::new(orbiterx_home.path().to_path_buf())
            .plugin_base_root(&plugin_id)
            .join(".orbiterx-remote-plugin-install.json");
        assert_eq!(
    ...
    fn stale_remote_plugin_cleanup_removes_stale_marketplace_caches_and_keeps_canonical_cache() {
        let orbiterx_home = tempfile::tempdir().expect("create orbiterx home");
        let created_by_me_cached_manifest = orbiterx_home
            .path()
            .join(PLUGINS_CACHE_DIR)
        ...
        std::fs::write(&cached_manifest, r#"{"name":"private-plugin"}"#)
            .expect("write cached plugin manifest");
        let canonical_cached_manifest = orbiterx_home
            .path()
            .join(PLUGINS_CACHE_DIR)
