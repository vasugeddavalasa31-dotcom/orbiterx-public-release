...
use orbiterx_config::MarketplaceAllowedSourceKind;
use orbiterx_config::MarketplaceAllowedSourceToml;
use orbiterx_config::RequirementSource;
use orbiterx_config::types::MarketplaceConfig;
use orbiterx_config::types::MarketplaceSourceType;
use orbiterx_config::types::PluginConfig;
use orbiterx_plugin::PluginId;
use orbiterx_utils_absolute_path::AbsolutePathBuf;
use orbiterx_utils_path::paths_match_after_normalization;
use regex::Regex;
...
impl MarketplacePolicy {
    ...
    pub(crate) fn validate_install(
        ...
        if let Some(expected_name) =
            managed_marketplace_name(orbiterx_home, marketplace_path, &root)
        {
        ...
        let configured_root = resolve_configured_marketplace_root(
            marketplace_name,
            marketplace,
            &marketplace_install_root(orbiterx_home),
        )
        .ok_or_else(|| {
...
pub(crate) fn project_effective_user_config(
    ...
    let allowed_marketplace_names =
        allowed_configured_marketplace_names_with_policy(&user_config, &policy, orbiterx_home);
...
fn allowed_configured_marketplace_names_with_policy(
    ...
    marketplaces
        ...
        .filter_map(|(marketplace_name, marketplace)| {
            let allowed = match managed_marketplace_config_name(orbiterx_home, marketplace) {
...
pub(crate) fn configured_plugins_from_stack(
    ...
    let Some(user_config) = project_effective_user_config(config_layer_stack, orbiterx_home) else {
        return HashMap::new();
    };
...
pub(crate) fn validate_marketplace_source_for_add(
    ...
    if let MarketplaceSource::Local { path } = source
        && let Some(expected_name) = managed_local_marketplace_name(orbiterx_home, path)
    {
...
fn managed_marketplace_name(
    ...
    if paths_match_after_normalization(
        ...
        curated_plugins_api_marketplace_path(orbiterx_home),
    ) {
        return Some(OPENAI_API_CURATED_MARKETPLACE_NAME);
    }
    if paths_match_after_normalization(root.as_path(), curated_plugins_repo_path(orbiterx_home)) {
        return Some(OPENAI_CURATED_MARKETPLACE_NAME);
    }
    managed_local_marketplace_name(orbiterx_home, root.as_path())
}
...
fn managed_marketplace_config_name(
    ...
    managed_local_marketplace_name(orbiterx_home, path)
}

fn managed_local_marketplace_name(orbiterx_home: &Path, root: &Path) -> Option<&'static str> {
    for marketplace_name in [
        OPENAI_BUNDLED_MARKETPLACE_NAME,
        OPENAI_BUNDLED_ALPHA_MARKETPLACE_NAME,
    ] {
        let expected_root = orbiterx_home
    ...
    let runtime_root = dirs::cache_dir()?
        .join("orbiterx-runtimes/orbiterx-primary-runtime/plugins")
