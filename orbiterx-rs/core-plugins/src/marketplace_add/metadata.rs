...
pub(super) fn record_added_marketplace_entry(
    ...
    record_user_marketplace(orbiterx_home, marketplace_name, &update).map_err(|err| {
...
mod tests {
    ...
    fn installed_marketplace_root_for_source_propagates_config_read_errors() {
        ...
        let install_root = orbiterx_home.path().join("marketplaces");
    ...
    fn installed_marketplace_root_for_source_uses_local_source_root() {
        let orbiterx_home = TempDir::new().unwrap();
        let install_root = orbiterx_home.path().join("marketplaces");
        let source_root = orbiterx_home.path().join("source");
        ...
        let install_metadata = MarketplaceInstallMetadata::from_source(&source, &[]);
        record_added_marketplace_entry(orbiterx_home.path(), "debug", &install_metadata).unwrap();
