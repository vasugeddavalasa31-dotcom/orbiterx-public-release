...
#[ignore = "requires tmux and a locally built orbiterx binary; run with --ignored for manual resize smoke"]
async fn tmux_split_preserves_fresh_session_composer_row_after_resize_reflow() -> Result<()> {
    ...
    let orbiterx = orbiterx_binary(&repo_root)?;
    ...
    write_config(orbiterx_home.path(), &repo_root)?;
    write_auth(orbiterx_home.path())?;

    let session_name = format!("orbiterx-resize-reflow-smoke-{}", std::process::id());
    ...
    let start_output = checked_output(
        Command::new("tmux")
            ...
            .arg("--")
            .arg("env")
            .arg(format!("ORBITERX_HOME={}", orbiterx_home.path().display()))
    ...
    let orbiterx_pane = stdout_text(&start_output).trim().to_string();
    anyhow::ensure!(!orbiterx_pane.is_empty(), "tmux did not report a pane id");
    ...
    let baseline_capture =
        wait_for_capture_contains(&orbiterx_pane, draft, Duration::from_secs(/*secs*/ 15))?;
    ...
    let final_capture = capture_pane(&orbiterx_pane)?;
    let final_row =
...
async fn tmux_width_resize_restore_keeps_visible_content_anchored() -> Result<()> {
    ...
    let session_name = format!("orbiterx-resize-width-{}", std::process::id());
    let _session = TmuxSession {
        name: session_name.clone(),
    };
    ...
    sleep(Duration::from_millis(/*millis*/ 1_000));
    let restored_capture = capture_pane(&orbiterx_pane)?;
    let restored_row =
...
async fn run_repeated_resize_smoke() -> Result<()> {
    ...
    let session_name = format!("orbiterx-resize-repeat-{}", std::process::id());
    let _session = TmuxSession {
        name: session_name.clone(),
    };
...
fn orbiterx_binary(repo_root: &Path) -> Result<PathBuf> {
    if let Ok(path) = orbiterx_utils_cargo_bin::cargo_bin("orbiterx") {
        return Ok(path);
    }

    let fallback = repo_root.join("orbiterx-rs/target/debug/orbiterx");
    anyhow::ensure!(
        fallback.is_file(),
        "orbiterx binary is unavailable; run `cargo build -p orbiterx-cli` first"
    );
    Ok(fallback)
}

fn write_config(orbiterx_home: &Path, repo_root: &Path) -> Result<()> {
...
"#
    ...
    std::fs::write(orbiterx_home.join("config.toml"), config)?;
    Ok(())
}

fn write_auth(orbiterx_home: &Path) -> Result<()> {
    std::fs::write(
