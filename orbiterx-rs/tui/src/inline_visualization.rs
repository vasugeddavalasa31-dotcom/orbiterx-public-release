...
pub(crate) const DIRECTIVE_PREFIX: &str = "::orbiterx-inline-vis{";
...
impl InlineVisualizationContext {
    pub(crate) fn new(orbiterx_home: &Path, thread_id: ThreadId) -> Option<Self> {
        Self::new_with_writable_roots(orbiterx_home, thread_id, std::iter::empty())
    }
    ...
    fn new_with_writable_roots<'a>(
        ...
        let visualizations_dir = orbiterx_home.join("visualizations");
...
fn link_placeholder() -> String {
    ...
    format!("https://orbiterx.invalid/inline-visualization/{token}")
}
