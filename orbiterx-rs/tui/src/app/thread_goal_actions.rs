...
const EPHEMERAL_THREAD_GOAL_ERROR_MESSAGE: &str = concat!(
    ...
    "Run `orbiterx` to start a saved session, or `orbiterx resume` / `/resume` to reopen one.",
);
...
impl App {
    ...
    pub(super) async fn open_thread_goal_editor(
        ...
        let orbiterx_home = app_server.orbiterx_home_path(&self.config.orbiterx_home);
    ...
    pub(super) async fn set_thread_goal_draft(
        ...
        let (objective, output_dir) =
            match goal_files::materialize_goal_draft(app_server, orbiterx_home.as_ref(), draft)
                .await
            {
