...
use dirs::home_dir;
use orbiterx_utils_string::normalize_markdown_hash_location_suffix;
...
///
/// This accepts the path shapes OrbiterX emits today: `file://` URLs, absolute and relative paths,
...
mod tests {
    ...
    fn column_classification_token_heavy_for_local_path_lists() {
        ...
        let rows = vec![
            vec![make_cell(
                "orbiterx-rs/core/src/next_prompt_suggestion.rs:1, orbiterx-rs/core/src/next_prompt_suggestion_tests.rs:1",
            )],
            vec![make_cell(
                "orbiterx-rs/core/src/context/next_prompt_suggestion.rs:1, orbiterx-rs/core/src/context/contextual_user_message_tests.rs:1",
            )],
        ];
        let metrics = W::collect_table_column_metrics(&header, &rows, /*column_count*/ 1);
        assert_eq!(metrics[0].kind, TableColumnKind::TokenHeavy);
