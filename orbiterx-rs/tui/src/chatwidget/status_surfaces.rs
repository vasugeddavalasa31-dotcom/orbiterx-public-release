...
impl ChatWidget {
    ...
    /// Clears the terminal title OrbiterX most recently wrote, if any.
    ///
    ...
    fn status_line_workspace_headline_should_fetch(&self, now: Instant) -> bool {
        if self
            ...
            || !self.has_orbiterx_backend_auth
        {
            return false;
        }
    ...
    pub(super) fn status_line_value_for_item(&mut self, item: StatusLineItem) -> Option<String> {
        match item {
            ...
            StatusLineItem::OrbiterXVersion => Some(ORBITERX_CLI_VERSION.to_string()),
    ...
    pub(super) fn status_surface_preview_value_for_item(
        ...
        let status_line_item = match item {
            StatusSurfacePreviewItem::AppName => return Some("orbiterx".to_string()),
            ...
            StatusSurfacePreviewItem::OrbiterXVersion => StatusLineItem::OrbiterXVersion,
    ...
    pub(super) fn terminal_title_value_for_item(
        ...
        match item {
            TerminalTitleItem::AppName => Some("orbiterx".to_string()),
            ...
            TerminalTitleItem::WeeklyLimit => self
                ...
                .map(|value| Self::truncate_terminal_title_part(value, /*max_chars*/ 32)),
            TerminalTitleItem::OrbiterXVersion => self
                .status_line_value_for_item(StatusLineItem::OrbiterXVersion)
                .map(|value| Self::truncate_terminal_title_part(value, /*max_chars*/ 32)),
...
fn five_hour_status_window(
    ...
    find_primary_orbiterx_window(snapshot, "5h")
...
fn weekly_status_window(
    ...
    find_orbiterx_window(snapshot, "weekly")
        .or_else(|| snapshot.secondary.as_ref().map(|window| (window, true)))
}

fn find_orbiterx_window<'a>(
...
fn find_primary_orbiterx_window<'a>(
    snapshot: &'a RateLimitSnapshotDisplay,
    label: &str,
) -> Option<(&'a RateLimitWindowDisplay, bool)> {
