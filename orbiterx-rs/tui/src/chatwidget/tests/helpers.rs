...
pub(super) async fn test_config() -> Config {
    ...
    let orbiterx_home = tempfile::Builder::new()
        .prefix("chatwidget-tests-")
        .tempdir()
        .expect("tempdir")
        .keep();
    let mut config = Config::load_default_with_cli_overrides_for_orbiterx_home(
        orbiterx_home.clone(),
        Vec::new(),
    )
    .await
    .expect("config");
    config.orbiterx_home = orbiterx_home.abs();
    config.sqlite_home = orbiterx_home.clone();
    config.log_dir = orbiterx_home.join("log");
...
pub(super) async fn make_chatwidget_manual_with_auth(
    ...
    let mut widget = ChatWidget::new_with_op_target(common, super::OrbiterXOpTarget::Direct(op_tx));
    widget.transcript.active_cell = None;
    widget.transcript.active_cell_revision = 0;
    widget.normal_placeholder_text = "Ask OrbiterX to do anything".to_string();
...
pub(crate) fn set_chatgpt_auth(chat: &mut ChatWidget) {
    ...
    chat.has_orbiterx_backend_auth = true;
...
fn token_usage_breakdown(usage: TokenUsage) -> orbiterx_app_server_protocol::TokenUsageBreakdown {
    orbiterx_app_server_protocol::TokenUsageBreakdown {
            ...
            chat.handle_server_notification(
                ServerNotification::ThreadTokenUsageUpdated(
                    orbiterx_app_server_protocol::ThreadTokenUsageUpdatedNotification {
                        ...
                        token_usage: orbiterx_app_server_protocol::ThreadTokenUsage {
...
pub(super) fn handle_error(
    ...
    orbiterx_error_info: Option<OrbiterXErrorInfo>,
) {
    chat.handle_server_notification(
...
pub(super) fn begin_exec_with_source(
    ...
    let command_actions = orbiterx_shell_command::parse_command::parse_command(&command)
        .into_iter()
...
pub(super) fn terminal_interaction(
    ...
    chat.handle_server_notification(
        ServerNotification::TerminalInteraction(
            orbiterx_app_server_protocol::TerminalInteractionNotification {
                thread_id: thread_id(chat),
                turn_id: chat
                    .turn_lifecycle
                    .last_turn_id
...
pub(super) fn plugins_test_absolute_path(path: &str) -> AbsolutePathBuf {
    std::env::temp_dir()
        .join("orbiterx-plugin-menu-tests")
        .join(path)
        .abs()
}
...
pub(super) fn plugins_test_detail(
    ...
    description: Option<&str>,
    skills: &[&str],
    hooks: &[(orbiterx_app_server_protocol::HookEventName, usize)],
    apps: &[&str],
    mcp_servers: &[&str],
) -> PluginDetail {
    PluginDetail {
        ...
        hooks: hooks
            ...
            .flat_map(|(event_index, (event_name, handler_count))| {
                (0..*handler_count).map(move |handler_index| {
                    orbiterx_app_server_protocol::PluginHookSummary {
        ...
        hook_run(
            ...
            vec![
                orbiterx_app_server_protocol::HookOutputEntry {
                    kind: orbiterx_app_server_protocol::HookOutputEntryKind::Warning,
...
fn hook_event_label(event_name: orbiterx_app_server_protocol::HookEventName) -> &'static str {
    match event_name {
        orbiterx_app_server_protocol::HookEventName::PreToolUse => "PreToolUse",
        orbiterx_app_server_protocol::HookEventName::PermissionRequest => "PermissionRequest",
        orbiterx_app_server_protocol::HookEventName::PostToolUse => "PostToolUse",
        orbiterx_app_server_protocol::HookEventName::PreCompact => "PreCompact",
        orbiterx_app_server_protocol::HookEventName::PostCompact => "PostCompact",
        orbiterx_app_server_protocol::HookEventName::SessionStart => "SessionStart",
        orbiterx_app_server_protocol::HookEventName::SessionEnd => "SessionEnd",
        orbiterx_app_server_protocol::HookEventName::UserPromptSubmit => "UserPromptSubmit",
        orbiterx_app_server_protocol::HookEventName::SubagentStart => "SubagentStart",
        orbiterx_app_server_protocol::HookEventName::SubagentStop => "SubagentStop",
        orbiterx_app_server_protocol::HookEventName::Stop => "Stop",
    }
}
