...
const GOAL_FILE_PREFIX: &str = "Read the OrbiterX goal objective file at ";
...
pub(crate) async fn materialize_goal_draft(
    ...
    orbiterx_home: Option<&GoalFilePath>,
    draft: GoalDraft,
    ...
    for (placeholder, text) in draft.pending_pastes.iter() {
        ...
        let path = ensure_goal_output_dir(app_server, orbiterx_home, &mut output_dir)
            .await?
...
pub(crate) async fn objective_text_for_edit(
    ...
    let Some(path) = objective_file_path(objective, orbiterx_home) else {
...
async fn ensure_goal_output_dir(
    ...
    let orbiterx_home = orbiterx_home
        .context("App server did not report $ORBITERX_HOME; cannot materialize goal files")?;
