...
use orbiterx_utils_path as path_utils;
...
async fn run_resume_picker_with_launch_context(
    ...
    run_session_picker_with_loader(
        ...
        spawn_app_server_page_loader(
            ...
            (!uses_remote_workspace).then(|| config.orbiterx_home.to_path_buf()),
            bg_tx,
        ),
        bg_rx,
    )
...
async fn load_transcript_preview(
    ...
    let inline_visualization_context = orbiterx_home.and_then(|orbiterx_home| {
        ThreadId::from_string(&thread.id)
            .ok()
            .and_then(|thread_id| InlineVisualizationContext::new(orbiterx_home, thread_id))
    });
            ...
            ThreadItem::UserMessage { content, .. } => Some(TranscriptPreviewLine {
                ...
                text: content
                    ...
                    .filter_map(|input| match input {
                        orbiterx_app_server_protocol::UserInput::Text { text, .. } => {
                            Some(text.as_str())
                        }
                        _ => None,
                    })
...
impl PickerState {
    ...
    async fn persist_density(&self) -> Result<()> {
        ...
        ConfigEditsBuilder::new(&persistence.orbiterx_home)
...
mod tests {
    ...
    fn row_search_matches_metadata_fields() {
        ...
        let row = Row {
            ...
            created_at: None,
            updated_at: None,
            cwd: Some(PathBuf::from("/tmp/orbiterx-session-picker")),
    ...
    fn expanded_session_details_include_metadata() {
        ...
        let row = Row {
            ...
            cwd: Some(PathBuf::from("/Users/felipe.coury/code/orbiterx")),
            git_branch: Some(String::from("orbiterx/raw-scrollback-mode")),
        };
        ...
        assert!(rendered.contains("Branch:      orbiterx/raw-scrollback-mode"));
    ...
    fn footer_prioritizes_active_sort_timestamp() {
        ...
        assert_metadata_order(&updated[0], "⌁ tmp/orbiterx", " main");
        assert_metadata_order(&created[0], "⌁ tmp/orbiterx", " main");
    }
    ...
    fn footer_marks_missing_branch() {
        ...
        assert!(rendered.contains("⌁ /tmp/orbiterx"));
        assert!(rendered.contains(" no branch"));
        assert_metadata_order(&footer[0], "⌁ /tmp/orbiterx", " no branch");
    }
    ...
    fn footer_branch_expands_when_line_has_room() {
        ...
        let footer = render_footer_lines(
            ...
            "4h ago",
            Some(branch),
            Some("~/code/orbiterx.etraut-animations-false-improvements/orbiterx-rs"),
            /*show_cwd*/ true,
            /*width*/ 140,
        );
    ...
    fn footer_cwd_truncates_to_responsive_column() {
        let cwd =
            "~/code/orbiterx.owner-extremely-long-worktree-name-that-needs-truncating/orbiterx-rs";
        ...
        assert!(footer.contains("⌁ ~/code/orbiterx."));
    ...
    fn footer_omits_cwd_when_hidden() {
        let footer = render_footer_lines(
            ...
            "4h ago",
            Some("owner/branch"),
            Some("~/code/orbiterx.owner-worktree/orbiterx-rs"),
            /*show_cwd*/ false,
            /*width*/ 80,
        );
...
"#
    ...
    async fn ctrl_o_keeps_toggled_density_when_persistence_fails() {
        let tmp = tempdir().expect("tmpdir");
        let orbiterx_home_file = tmp.path().join("orbiterx-home-file");
        std::fs::write(&orbiterx_home_file, "not a directory").expect("write orbiterx home file");
        let loader = page_only_loader(|_| {});
        let mut state = PickerState::new(
            FrameRequester::test_dummy(),
            loader,
        ...
        state.view_persistence = Some(SessionPickerViewPersistence {
            orbiterx_home: orbiterx_home_file,
        });
    ...
    fn dense_snapshot_row() -> Row {
        Row {
            ...
            cwd: Some(PathBuf::from(
                "/Users/felipe.coury/code/orbiterx.fcoury-session-picker/orbiterx-rs",
            )),
            git_branch: Some(String::from("fcoury/session-picker")),
        }
    }
    ...
    fn expanded_session_snapshot() {
        ...
        let row = Row {
            ...
            created_at: parse_timestamp_str("2026-04-28T16:30:00Z"),
            updated_at: parse_timestamp_str("2026-04-28T17:45:00Z"),
            cwd: Some(PathBuf::from("/tmp/orbiterx")),
            git_branch: Some(String::from("fcoury/session-picker")),
        };
        let mut state = PickerState::new(
            FrameRequester::test_dummy(),
    ...
    fn thread_to_transcript_cells_renders_core_message_types() {
        ...
        let thread = Thread {
            ...
            git_info: None,
            name: None,
            turns: vec![orbiterx_app_server_protocol::Turn {
