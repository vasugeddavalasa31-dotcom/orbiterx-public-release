...
impl AppEventSender {
    ...
    pub(crate) fn send(&self, event: AppEvent) {
        ...
        if !matches!(event, AppEvent::OrbiterXOp(_)) {
    ...
    pub(crate) fn interrupt(&self) {
        self.send(AppEvent::OrbiterXOp(AppCommand::interrupt()));
    }

    pub(crate) fn compact(&self) {
        self.send(AppEvent::OrbiterXOp(AppCommand::compact()));
    }

    pub(crate) fn set_thread_name(&self, name: String) {
        self.send(AppEvent::OrbiterXOp(AppCommand::set_thread_name(name)));
    }

    pub(crate) fn review(&self, target: ReviewTarget) {
        self.send(AppEvent::OrbiterXOp(AppCommand::review(target)));
    }

    pub(crate) fn list_skills(&self, cwds: Vec<PathBuf>, force_reload: bool) {
        self.send(AppEvent::OrbiterXOp(AppCommand::list_skills(
            cwds,
            force_reload,
        )));
    }
    ...
    pub(crate) fn user_input_answer(&self, id: String, response: ToolRequestUserInputResponse) {
        self.send(AppEvent::OrbiterXOp(AppCommand::user_input_answer(
            id, response,
        )));
    }
