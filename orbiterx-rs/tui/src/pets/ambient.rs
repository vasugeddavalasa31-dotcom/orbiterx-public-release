//! Ambient terminal rendering for the OrbiterX companion.
//!
...
impl AmbientPet {
    ...
    /// ORBITERX_HOME cache, and records the terminal protocol support snapshot used
    ...
    pub(crate) fn load(
        ...
        let pet = Pet::load_with_orbiterx_home(
            ...
            /*orbiterx_home*/ Some(orbiterx_home),
        )
        .with_context(|| "load ambient pet")?;
        let cache_dir = orbiterx_home
            .join("cache")
            .join("tui-pets")
            .join("frame-cache")
            .join(&pet.id)
...
mod tests {
    ...
    #[test]
    fn notification_labels_match_orbiterx_app_vocabulary() {
