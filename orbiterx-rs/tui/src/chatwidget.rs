//! The main OrbiterX TUI chat surface.
//!
...
use crate::version::ORBITERX_CLI_VERSION;
...
use orbiterx_app_server_protocol::CommandExecutionSource as ExecCommandSource;
use orbiterx_app_server_protocol::CreditsSnapshot;
...
use orbiterx_app_server_protocol::ModelVerification as AppServerModelVerification;
use orbiterx_app_server_protocol::OrbiterXErrorInfo as AppServerOrbiterXErrorInfo;
use orbiterx_app_server_protocol::RateLimitReachedType;
...
use orbiterx_app_server_protocol::SkillMetadata as ProtocolSkillMetadata;
use orbiterx_app_server_protocol::SkillsListResponse;
use orbiterx_app_server_protocol::ThreadGoal as AppThreadGoal;
use orbiterx_app_server_protocol::ThreadGoalStatus as AppThreadGoalStatus;
...
use orbiterx_config::Constrained;
use orbiterx_config::ConstraintResult;
...
use orbiterx_core_skills::model::SkillMetadata;
use orbiterx_features::FEATURES;
use orbiterx_features::Feature;
#[cfg(test)]
use orbiterx_git_utils::CommitLogEntry;
use orbiterx_git_utils::current_branch_name;
use orbiterx_git_utils::get_git_repo_root;
use orbiterx_git_utils::local_git_branches;
use orbiterx_git_utils::recent_commits;
use orbiterx_otel::RuntimeMetricsSummary;
use orbiterx_otel::SessionTelemetry;
use orbiterx_plugin::PluginCapabilitySummary;
use orbiterx_protocol::ThreadId;
use orbiterx_protocol::account::PlanType;
use orbiterx_protocol::approvals::GuardianAssessmentAction;
use orbiterx_protocol::approvals::GuardianAssessmentDecisionSource;
use orbiterx_protocol::approvals::GuardianAssessmentEvent;
use orbiterx_protocol::approvals::GuardianAssessmentStatus;
...
use orbiterx_protocol::items::AgentMessageContent;
use orbiterx_protocol::items::AgentMessageItem;
use orbiterx_protocol::models::MessagePhase;
use orbiterx_protocol::plan_tool::PlanItemArg as UpdatePlanItemArg;
use orbiterx_protocol::plan_tool::StepStatus as UpdatePlanItemStatus;
...
use orbiterx_utils_cli::resume_hint;
...
const MEMORIES_DOC_URL: &str = "https://developers.openai.com/orbiterx/memories";
...
use orbiterx_file_search::FileMatch;
...
use orbiterx_utils_approval_presets::ApprovalPreset;
use orbiterx_utils_approval_presets::builtin_approval_presets;
...
pub(crate) struct ChatWidgetInit {
    ...
    pub(crate) has_orbiterx_backend_auth: bool,
    pub(crate) model_catalog: Arc<ModelCatalog>,
    pub(crate) feedback: orbiterx_feedback::OrbiterXFeedback,
...
/// and by sending requests back to orbiterx-core.
///
...
pub(crate) struct ChatWidget {
    ...
    orbiterx_op_target: OrbiterXOpTarget,
    ...
    has_orbiterx_backend_auth: bool,
    ...
    orbiterx_rate_limit_reached_type: Option<RateLimitReachedType>,
    orbiterx_spend_control_reached: Option<bool>,
...
fn request_permissions_from_params(
    params: orbiterx_app_server_protocol::PermissionsRequestApprovalParams,
...
impl ChatWidget {
    ...
    pub(crate) fn open_feedback_consent(&mut self, category: crate::app_event::FeedbackCategory) {
        ...
        let include_windows_sandbox_log =
            orbiterx_windows_sandbox::current_log_file_path_for_orbiterx_home(
                &self.config.orbiterx_home,
            )
            .is_file();
    ...
    /// Forward a command directly to orbiterx.
        ...
        T: Into<AppCommand>,
    {
        ...
        match &self.orbiterx_op_target {
            OrbiterXOpTarget::Direct(orbiterx_op_tx) => {
                crate::session_log::log_outbound_op(&op);
                if let Err(e) = orbiterx_op_tx.send(op) {
                    tracing::error!("failed to submit op: {e}");
                    return false;
                }
            }
            OrbiterXOpTarget::AppEvent => {
                self.app_event_tx.send(AppEvent::OrbiterXOp(op));
            }
        }
        true
    }
