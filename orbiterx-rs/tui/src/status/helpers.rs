...
mod tests {
    ...
    async fn test_config(orbiterx_home: &TempDir, cwd: &TempDir) -> Config {
    ...
    async fn compose_agents_summary_includes_global_agents_path() {
        let orbiterx_home = TempDir::new().expect("temp orbiterx home");
        let cwd = TempDir::new().expect("temp cwd");
        let global_agents_path = orbiterx_home.path().join("global.md");
        let config = test_config(&orbiterx_home, &cwd).await;
    ...
    async fn compose_agents_summary_names_global_agents_override() {
        let orbiterx_home = TempDir::new().expect("temp orbiterx home");
        let cwd = TempDir::new().expect("temp cwd");
        let override_path = orbiterx_home.path().join("override.md");
        let config = test_config(&orbiterx_home, &cwd).await;
