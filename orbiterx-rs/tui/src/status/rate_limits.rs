...
use chrono::Local;
use chrono::Utc;
use orbiterx_app_server_protocol::CreditsSnapshot as CoreCreditsSnapshot;
use orbiterx_app_server_protocol::RateLimitSnapshot;
use orbiterx_app_server_protocol::RateLimitWindow;
use orbiterx_app_server_protocol::SpendControlLimitSnapshot as CoreSpendControlLimitSnapshot;
use orbiterx_protocol::num_format::format_with_separators;
...
pub(crate) struct RateLimitSnapshotDisplay {
    /// Canonical limit identifier (for example: `orbiterx` or `orbiterx_other`).
...
pub(crate) fn rate_limit_snapshot_display(
    ...
    rate_limit_snapshot_display_for_limit(snapshot, "orbiterx".to_string(), captured_at)
}
...
pub(crate) fn compose_rate_limit_data_many(
    ...
    for snapshot in snapshots {
        ...
        let show_limit_prefix = !limit_bucket_label.eq_ignore_ascii_case("orbiterx");
        ...
        let combine_non_orbiterx_single_limit = show_limit_prefix && window_count == 1;

        if show_limit_prefix && !combine_non_orbiterx_single_limit {
        ...
        if let Some(primary) = snapshot.primary.as_ref() {
            let label = if combine_non_orbiterx_single_limit {
                format!(
                    "{} {} limit",
                    limit_bucket_label,
...
mod tests {
    ...
    #[test]
    fn non_orbiterx_single_limit_renders_combined_row() {
        let now = Local::now();
        let orbiterx = RateLimitSnapshotDisplay {
        ...
        let other = RateLimitSnapshotDisplay {
            limit_name: "orbiterx-other".to_string(),
            captured_at: now,
        ...
        let rows = match compose_rate_limit_data_many(&[orbiterx, other], now) {
        ...
        assert_eq!(
            ...
            vec![
                "5h limit".to_string(),
                "Credits".to_string(),
                "orbiterx-other 5h limit".to_string(),
                "Credits".to_string(),
            ]
        );
    ...
    #[test]
    fn non_orbiterx_multi_limit_keeps_group_row() {
        let now = Local::now();
        let other = RateLimitSnapshotDisplay {
            limit_name: "orbiterx-other".to_string(),
            captured_at: now,
        ...
        assert_eq!(
            labels,
            vec![
                "orbiterx-other limit".to_string(),
