...
use clap::Parser;
use orbiterx_utils_cli::ApprovalModeCliArg;
...
pub struct Cli {
    ...
    // Internal controls set by the top-level `orbiterx resume` subcommand.
    // These are not exposed as user flags on the base `orbiterx` command.
    #[clap(skip)]
    ...
    /// top-level `orbiterx resume <SESSION_ID>` wrapper; not exposed as a public flag.
    #[clap(skip)]
    ...
    // Internal controls set by the top-level `orbiterx fork` subcommand.
    // These are not exposed as user flags on the base `orbiterx` command.
    #[clap(skip)]
    ...
    /// top-level `orbiterx fork <SESSION_ID>` wrapper; not exposed as a public flag.
    #[clap(skip)]
