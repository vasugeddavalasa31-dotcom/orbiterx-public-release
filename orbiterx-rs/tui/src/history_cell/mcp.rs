...
pub(crate) struct McpToolCallCell {
    ...
    result: Option<Result<orbiterx_protocol::mcp::CallToolResult, String>>,
...
impl McpToolCallCell {
    ...
    pub(crate) fn complete(
        &mut self,
        duration: Duration,
        result: Result<orbiterx_protocol::mcp::CallToolResult, String>,
...
impl HistoryCell for McpToolCallCell {
    fn display_lines(&self, width: u16) -> Vec<Line<'static>> {
        ...
        if let Some(result) = &self.result {
            match result {
                Ok(orbiterx_protocol::mcp::CallToolResult { content, .. }) => {
                    if !content.is_empty() {
...
///
/// Manual testing tip:
/// - Run the rmcp stdio test server (`orbiterx-rs/rmcp-client/src/bin/test_stdio_server.rs`) and
///   register it as an MCP server via `orbiterx mcp add`.
    ...
    let lines: Vec<Line<'static>> = vec![
        ...
        Line::from(vec![
            ...
            crate::terminal_hyperlinks::osc8_hyperlink(
                "https://developers.openai.com/orbiterx/mcp",
                "MCP docs",
            )
            .underlined(),
...
pub(crate) fn new_mcp_tools_output(
    config: &Config,
    tools: HashMap<String, orbiterx_protocol::mcp::Tool>,
...
pub(crate) fn new_mcp_tools_output_from_statuses(
    ...
    for status in statuses {
        ...
        let auth_status = match status.auth_status {
            orbiterx_app_server_protocol::McpAuthStatus::Unsupported => McpAuthStatus::Unsupported,
            orbiterx_app_server_protocol::McpAuthStatus::NotLoggedIn => McpAuthStatus::NotLoggedIn,
            orbiterx_app_server_protocol::McpAuthStatus::BearerToken => McpAuthStatus::BearerToken,
