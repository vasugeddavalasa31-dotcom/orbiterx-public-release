...
use orbiterx_ansi_escape::ansi_escape_line;
...
use orbiterx_app_server_protocol::ConfigReadResponse;
...
use orbiterx_app_server_protocol::SandboxMode as AppServerSandboxMode;
...
use orbiterx_app_server_protocol::SkillErrorInfo;
...
use orbiterx_app_server_protocol::WriteStatus;
...
use orbiterx_models_manager::model_presets::HIDE_GPT_5_1_ORBITERX_MAX_MIGRATION_PROMPT_CONFIG;
use orbiterx_models_manager::model_presets::HIDE_GPT5_1_MIGRATION_PROMPT_CONFIG;
...
#[cfg(target_os = "windows")]
use orbiterx_protocol::permissions::FileSystemSandboxKind;
...
use orbiterx_utils_approval_presets::builtin_permission_profile_for_active_permission_profile;
        ...
        ServerNotification::ItemCompleted(notification) => match &notification.item {
            ThreadItem::CollabAgentToolCall { agents_states, .. } => {
                agents_states.get(receiver_thread_id).is_some_and(|state| {
                    matches!(
                        &state.status,
                        orbiterx_app_server_protocol::CollabAgentStatus::NotFound
                    )
                })
            }
            _ => false,
...
fn default_exec_approval_decisions(
    network_approval_context: Option<&orbiterx_app_server_protocol::NetworkApprovalContext>,
    proposed_execpolicy_amendment: Option<&orbiterx_app_server_protocol::ExecPolicyAmendment>,
    proposed_network_policy_amendments: Option<
        &[orbiterx_app_server_protocol::NetworkPolicyAmendment],
    >,
    additional_permissions: Option<&orbiterx_app_server_protocol::AdditionalPermissionProfile>,
) -> Vec<orbiterx_app_server_protocol::CommandExecutionApprovalDecision> {
...
fn resume_hint_for_resumable_thread(
    ...
    orbiterx_utils_cli::resume_hint(thread.thread_name.as_deref(), Some(thread.thread_id))
}
...
fn active_turn_not_steerable_turn_error(error: &TypedRequestError) -> Option<AppServerTurnError> {
    ...
    matches!(
        turn_error.orbiterx_error_info,
        Some(AppServerOrbiterXErrorInfo::ActiveTurnNotSteerable { .. })
    )
...
fn archived_session_guidance(err: &color_eyre::eyre::Report) -> Option<String> {
    ...
    if !message.contains(" is archived. Run `orbiterx unarchive ") {
        return None;
    }
    let message = message
        .split_once(" (code ")
...
impl App {
    pub fn chatwidget_init_for_forked_or_resumed_thread(
        ...
        crate::chatwidget::ChatWidgetInit {
            ...
            has_orbiterx_backend_auth: self.chat_widget.has_orbiterx_backend_auth(),
    ...
    pub async fn run(
        ...
        let has_orbiterx_backend_auth = matches!(auth_mode, Some(TelemetryAuthMode::Chatgpt));
        ...
        let session_telemetry = SessionTelemetry::new(
            ...
            auth_mode,
            orbiterx_login::default_client::originator().value,
        ...
        {
            session_telemetry.counter("orbiterx.status_line", /*inc*/ 1, &[]);
        }
...
See the OrbiterX keymap documentation for supported actions and examples."
            )
        })?;
        #[cfg(not(debug_assertions))]
        ...
        #[cfg(target_os = "windows")]
        {
            ...
            if should_check {
                ...
                let logs_base_dir = app.config.orbiterx_home.clone();
