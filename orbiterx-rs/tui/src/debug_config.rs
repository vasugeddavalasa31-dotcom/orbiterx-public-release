...
use orbiterx_config::NetworkUnixSocketPermissionToml;
use orbiterx_config::RequirementSource;
use orbiterx_config::ResidencyRequirement;
use orbiterx_config::SandboxModeRequirement;
use orbiterx_config::WebSearchModeRequirement;
use orbiterx_config::format_config_layer_source;
...
mod tests {
    ...
    use orbiterx_config::ConstraintError;
    use orbiterx_config::FeatureRequirementsToml;
    use orbiterx_config::FilesystemConstraints;
    use orbiterx_config::HookEventsToml;
    use orbiterx_config::HookHandlerConfig;
    use orbiterx_config::LoaderOverrides;
    use orbiterx_config::ManagedHooksRequirementsToml;
    use orbiterx_config::MatcherGroup;
    ...
    use orbiterx_config::NetworkUnixSocketPermissionToml;
    use orbiterx_config::NetworkUnixSocketPermissionsToml;
    use orbiterx_config::RequirementSource;
    use orbiterx_config::ResidencyRequirement;
    use orbiterx_config::SandboxModeRequirement;
...
"#,
    ...
    fn debug_config_output_lists_all_layers_including_disabled() {
        let system_file = if cfg!(windows) {
            absolute_path("C:\\etc\\orbiterx\\config.toml")
        } else {
            absolute_path("/etc/orbiterx/config.toml")
        };
        let project_folder = if cfg!(windows) {
            absolute_path("C:\\repo\\.orbiterx")
        } else {
            absolute_path("/repo/.orbiterx")
        };
        ...
        let layers = vec![
            ...
            ConfigLayerEntry::new_disabled(
                ConfigLayerSource::Project {
                    dot_orbiterx_folder: project_folder,
                },
                empty_toml_table(),
    ...
    fn debug_config_output_lists_requirement_sources() {
        let requirements_file = if cfg!(windows) {
            absolute_path("C:\\ProgramData\\OpenAI\\OrbiterX\\requirements.toml")
        } else {
            absolute_path("/etc/orbiterx/requirements.toml")
        };
                ...
                BTreeMap::from([(
                    ...
                    McpServerRequirement::Identity {
                        identity: McpServerIdentity::Command {
                            command: "orbiterx-mcp".to_string(),
                        },
                    },
                )]),
        ...
        let user_file = if cfg!(windows) {
            absolute_path("C:\\users\\alice\\.orbiterx\\config.toml")
        } else {
            absolute_path("/home/alice/.orbiterx/config.toml")
        };
        let stack = ConfigLayerStack::new(
    ...
    fn debug_config_output_formats_unix_socket_permissions() {
        ...
        let requirements_source = (RequirementSource::LegacyManagedConfigTomlFromMdm).to_string();
        assert!(rendered.contains(&format!(
            "experimental_network: unix_sockets={{/tmp/blocked.sock=deny, /tmp/orbiterx.sock=allow}} (source: {requirements_source})"
        )));
    }
...
"#;
        ...
        let mdm_base_dir = if cfg!(windows) {
            absolute_path("C:\\orbiterx")
        } else {
            absolute_path("/var/lib/orbiterx")
        };
