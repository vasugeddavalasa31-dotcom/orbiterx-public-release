...
use orbiterx_config::types::KeybindingsSpec;
use orbiterx_config::types::MAX_FUNCTION_KEY;
...
Set unique keys in `~/.orbiterx/config.toml` and retry. \
...
Set a different key in `~/.orbiterx/config.toml` and retry. \
See the OrbiterX keymap documentation for supported actions and examples."
                ));
            }
        }
...
mod tests {
    use super::*;
    use orbiterx_config::types::KeybindingSpec;
