...
async fn append_entry_trims_history_when_beyond_max_bytes() {
    ...
    let mut config = HistoryConfig::new(orbiterx_home.path(), &history);
    let conversation_id = "conversation-id";

    let entry_one = "a".repeat(200);
    let entry_two = "b".repeat(200);

    let history_path = orbiterx_home.path().join("history.jsonl");
    ...
    config = HistoryConfig::new(orbiterx_home.path(), &history);
