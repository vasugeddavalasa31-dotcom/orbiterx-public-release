...
//!
//! The history is stored at `~/.orbiterx/history.jsonl` with **one JSON object per
...
/// Filename that stores the message history inside `~/.orbiterx`.
...
impl HistoryConfig {
    pub fn new(orbiterx_home: impl Into<PathBuf>, history: &History) -> Self {
        Self {
            orbiterx_home: orbiterx_home.into(),
...
fn history_filepath(config: &HistoryConfig) -> PathBuf {
    config.orbiterx_home.join(HISTORY_FILENAME)
}
...
pub async fn append_entry(
    ...
    // Resolve `~/.orbiterx/history.jsonl` and ensure the parent directory exists.
