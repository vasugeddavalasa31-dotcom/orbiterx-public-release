...
async fn selected_plugin_servers_use_managed_requirements_for_the_selected_root_id() -> TestResult {
    ...
    std::fs::create_dir_all(plugin_root.path().join(".orbiterx-plugin"))?;
    std::fs::write(
        plugin_root.path().join(".orbiterx-plugin/plugin.json"),
...
async fn raw_selected_plugin_contributions(
    ...
    orbiterx_mcp_extension::install_executor_plugins(
        &mut builder,
