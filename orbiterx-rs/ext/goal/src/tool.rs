...
use orbiterx_extension_api::JsonToolOutput;
use orbiterx_extension_api::ToolCall;
use orbiterx_extension_api::ToolExecutor;
use orbiterx_extension_api::ToolName;
use orbiterx_extension_api::ToolOutput;
use orbiterx_extension_api::ToolSpec;
use orbiterx_protocol::ThreadId;
use orbiterx_protocol::protocol::ThreadGoal;
use orbiterx_protocol::protocol::ThreadGoalStatus;
use orbiterx_protocol::protocol::validate_thread_goal_objective;
...
pub(crate) struct GoalToolExecutor {
    kind: GoalToolKind,
    thread_id: ThreadId,
    state_db: Arc<orbiterx_state::StateRuntime>,
...
impl ToolExecutor<ToolCall> for GoalToolExecutor {
    ...
    fn handle(&self, invocation: ToolCall) -> orbiterx_extension_api::ToolExecutorFuture<'_> {
        Box::pin(async move {
            match self.kind {
...
impl GoalToolExecutor {
    ...
    async fn handle_update(
        ...
        self.account_active_goal_progress(
            match args.status {
                ThreadGoalStatus::Complete => orbiterx_state::GoalAccountingMode::ActiveOrComplete,
                ThreadGoalStatus::Blocked => orbiterx_state::GoalAccountingMode::ActiveOrStopped,
    ...
    async fn account_active_goal_progress(
        &self,
        mode: orbiterx_state::GoalAccountingMode,
        event_id: &str,
        ...
        Ok(match outcome {
            orbiterx_state::GoalAccountingOutcome::Updated(goal) => {
                self.metrics
                    .record_terminal_if_status_changed(previous_status, &goal);
                self.analytics
            ...
            orbiterx_state::GoalAccountingOutcome::Unchanged(_) => None,
        })
    }
    ...
    ) -> Result<Option<orbiterx_state::ThreadGoalStatus>, FunctionCallError> {
        let goal = self
            .state_db
            .thread_goals()
            .get_thread_goal(self.thread_id)
...
pub(crate) async fn fill_empty_thread_preview_if_possible(
    state_db: &orbiterx_state::StateRuntime,
    thread_id: ThreadId,
    goal: &orbiterx_state::ThreadGoal,
) {
...
pub(crate) fn protocol_goal_from_state(goal: orbiterx_state::ThreadGoal) -> ThreadGoal {
    ThreadGoal {
...
fn protocol_status_from_state(status: orbiterx_state::ThreadGoalStatus) -> ThreadGoalStatus {
    match status {
...
pub(crate) fn state_status_from_protocol(
    status: ThreadGoalStatus,
) -> orbiterx_state::ThreadGoalStatus {
    match status {
        ThreadGoalStatus::Active => orbiterx_state::ThreadGoalStatus::Active,
        ThreadGoalStatus::Paused => orbiterx_state::ThreadGoalStatus::Paused,
        ThreadGoalStatus::Blocked => orbiterx_state::ThreadGoalStatus::Blocked,
        ThreadGoalStatus::UsageLimited => orbiterx_state::ThreadGoalStatus::UsageLimited,
        ThreadGoalStatus::BudgetLimited => orbiterx_state::ThreadGoalStatus::BudgetLimited,
        ThreadGoalStatus::Complete => orbiterx_state::ThreadGoalStatus::Complete,
    }
}
