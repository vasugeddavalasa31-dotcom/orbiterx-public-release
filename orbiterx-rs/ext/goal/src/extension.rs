...
use orbiterx_extension_api::ThreadIdleInput;
...
use orbiterx_extension_api::TokenUsageContributor;
...
use orbiterx_extension_api::ToolLifecycleContributor;
use orbiterx_extension_api::ToolLifecycleFuture;
use orbiterx_extension_api::TurnAbortInput;
use orbiterx_extension_api::TurnErrorInput;
use orbiterx_extension_api::TurnLifecycleContributor;
    ...
    C: Send + Sync + 'static,
{
    fn on_turn_start<'a>(&'a self, input: TurnStartInput<'a>) -> ExtensionFuture<'a, ()> {
        Box::pin(async move {
            ...
            if matches!(
                input.collaboration_mode.mode,
                orbiterx_protocol::config_types::ModeKind::Plan
            ) {
    ...
    fn on_turn_error<'a>(&'a self, input: TurnErrorInput<'a>) -> ExtensionFuture<'a, ()> {
        Box::pin(async move {
            ...
            let reason = match input.error {
                OrbiterXErrorInfo::UsageLimitExceeded => ActiveGoalStopReason::UsageLimit,
