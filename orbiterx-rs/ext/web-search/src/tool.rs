...
use orbiterx_api::SearchClient;
use orbiterx_api::SearchCommands;
use orbiterx_api::SearchQuery;
use orbiterx_api::SearchRequest;
use orbiterx_api::SearchSettings;
use orbiterx_core::X_ORBITERX_TURN_METADATA_HEADER;
use orbiterx_core::web_search_action_detail;
use orbiterx_extension_api::ExtensionTurnItem;
use orbiterx_extension_api::FunctionCallError;
use orbiterx_extension_api::ResponsesApiTool;
...
use orbiterx_extension_api::parse_tool_input_schema_without_compaction;
use orbiterx_extension_items::ExtensionItem;
use orbiterx_extension_items::web_search::WebSearchAction;
...
const RESULTS_PAYLOAD_BYTES_METRIC: &str = "orbiterx.web_search.results.payload_bytes";
...
impl ToolExecutor<ToolCall> for WebSearchTool {
    ...
    fn handle(&self, call: ToolCall) -> orbiterx_extension_api::ToolExecutorFuture<'_> {
...
impl WebSearchTool {
    async fn handle_call(&self, call: ToolCall) -> Result<Box<dyn ToolOutput>, FunctionCallError> {
        ...
        let extra_headers = search_request_headers(
            ...
            call.orbiterx_turn_metadata.as_deref(),
        );
        ...
        if let Some(results) = results.as_ref()
            && let Some(metrics) = orbiterx_otel::global()
...
fn search_request_headers(originator: Option<&str>, turn_metadata: Option<&str>) -> HeaderMap {
    ...
    {
        headers.insert(X_ORBITERX_TURN_METADATA_HEADER, header_value);
    }
