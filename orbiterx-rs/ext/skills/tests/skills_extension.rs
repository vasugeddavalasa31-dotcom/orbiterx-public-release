...
use orbiterx_core_skills::SKILLS_INTRO_WITH_ABSOLUTE_PATHS;
use orbiterx_core_skills::SkillLoadOutcome;
use orbiterx_core_skills::SkillMetadata;
use orbiterx_core_skills::injection::InjectedHostSkillPrompts;
use orbiterx_extension_api::ConversationHistory;
...
use orbiterx_extension_api::PreviousWorldStateSection;
...
use orbiterx_extension_api::WorldStateContributionInput;
...
use orbiterx_protocol::protocol::SKILLS_INSTRUCTIONS_CLOSE_TAG;
use orbiterx_protocol::protocol::SKILLS_INSTRUCTIONS_OPEN_TAG;
...
use orbiterx_skills_extension::SkillProviders;
use orbiterx_skills_extension::SkillsExtensionConfig;
use orbiterx_skills_extension::catalog::SkillAuthority;
use orbiterx_skills_extension::catalog::SkillCatalog;
use orbiterx_skills_extension::catalog::SkillCatalogEntry;
use orbiterx_skills_extension::catalog::SkillPackageId;
use orbiterx_skills_extension::catalog::SkillProviderError;
use orbiterx_skills_extension::catalog::SkillReadResult;
use orbiterx_skills_extension::catalog::SkillResourceId;
use orbiterx_skills_extension::catalog::SkillSearchResult;
use orbiterx_skills_extension::catalog::SkillSourceKind;
use orbiterx_skills_extension::install;
use orbiterx_skills_extension::install_with_providers;
use orbiterx_skills_extension::provider::SkillListQuery;
use orbiterx_skills_extension::provider::SkillProvider;
use orbiterx_skills_extension::provider::SkillProviderFuture;
use orbiterx_skills_extension::provider::SkillReadRequest;
use orbiterx_skills_extension::provider::SkillSearchRequest;
...
async fn installed_extension_uses_host_service_snapshot() -> TestResult {
    ...
    let skill_path = orbiterx_home.join("skills").join("demo").join("SKILL.md");
    ...
    std::fs::remove_dir_all(orbiterx_home)?;
    Ok(())
}
...
async fn selected_executor_catalog_follows_step_availability_and_reuses_its_cache() -> TestResult {
    ...
    let available_sections = registry.context_contributors()[0]
        .contribute_world_state(WorldStateContributionInput {
            thread_id: orbiterx_protocol::ThreadId::new(),
            turn_id: "turn-1",
...
fn test_orbiterx_home() -> PathBuf {
    ...
    std::env::temp_dir().join(format!(
        "orbiterx-skills-extension-test-{}-{id}",
        std::process::id(),
    ))
}
