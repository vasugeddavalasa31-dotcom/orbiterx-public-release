...
/// Filename used for the redacted `orbiterx doctor --json` feedback attachment.
pub const DOCTOR_REPORT_ATTACHMENT_FILENAME: &str = "orbiterx-doctor-report.json";
/// Filename used for the raw OrbiterX Apps MCP tools cache feedback attachment.
pub const ORBITERX_APPS_TOOLS_CACHE_ATTACHMENT_FILENAME: &str = "orbiterx-apps-tools-cache.json";
/// Filename used for the raw connector directory cache feedback attachment.
pub const ORBITERX_APP_DIRECTORY_CACHE_ATTACHMENT_FILENAME: &str =
    "orbiterx-app-directory-cache.json";
...
pub fn emit_feedback_request_tags_with_auth_env(
    ...
    tracing::info!(
        ...
        auth_env_orbiterx_api_key_present = tracing::field::debug(auth_env.orbiterx_api_key_env_present),
        auth_env_orbiterx_api_key_enabled = tracing::field::debug(auth_env.orbiterx_api_key_env_enabled),
...
#[derive(Clone)]
pub struct OrbiterXFeedback {
    inner: Arc<FeedbackInner>,
}

impl Default for OrbiterXFeedback {
    fn default() -> Self {
        Self::new()
    }
}

impl OrbiterXFeedback {
    pub fn new() -> Self {
        ...
        tracing_subscriber::fmt::layer()
            ...
            .with_filter(
                Targets::new()
                    ...
                    .with_target("orbiterx_api::responses_websocket_timing", LevelFilter::OFF),
            )
    }
...
pub struct FeedbackUploadOptions<'a> {
    ...
    ///
    /// These are included after `orbiterx-logs.log` and before path-backed rollout
...
impl FeedbackSnapshot {
    ...
    pub fn save_to_temp_file(&self) -> io::Result<PathBuf> {
        ...
        let filename = format!("orbiterx-feedback-{}.log", self.thread_id);
    ...
    fn feedback_attachments(
        ...
        if include_logs {
            attachments.push(Attachment {
                ...
                filename: String::from("orbiterx-logs.log"),
...
mod tests {
    ...
    fn ring_buffer_drops_front_when_full() {
        let fb = OrbiterXFeedback::with_capacity(/*max_bytes*/ 8);
        {
    ...
    fn logger_layer_excludes_responses_websocket_timing_payloads() {
        let fb = OrbiterXFeedback::new();
        let _guard = tracing_subscriber::registry()
            .with(fb.logger_layer())
            .set_default();

        tracing::trace!(target: "orbiterx_api::responses_websocket_timing", payload = "secret");
        tracing::trace!(target: "orbiterx_feedback_test", "retained");
    ...
    fn feedback_attachments_gate_connectivity_diagnostics() {
        let extra_filename = format!("orbiterx-feedback-extra-{}.jsonl", ThreadId::new());
        ...
        let snapshot_with_diagnostics = OrbiterXFeedback::new()
        ...
        let attachments_without_diagnostics = OrbiterXFeedback::new()
            .snapshot(/*session_id*/ None)
    ...
    fn path_backed_attachments_use_binary_content_types() {
        ...
        let gzip_filename = format!("orbiterx-desktop-app-logs-{suffix}.tar.gz");
        let unknown_filename = format!("orbiterx-feedback-extra-{suffix}.binunknown");
        ...
        let attachments = OrbiterXFeedback::new()
            .snapshot(/*session_id*/ None)
