...
async fn list_and_get_render_expected_output() -> Result<()> {
    ...
    let mut add = orbiterx_command(orbiterx_home.path())?;
    add.args([
        "mcp",
        "add",
        "docs",
    ...
    let mut servers = load_global_mcp_servers(orbiterx_home.path()).await?;
    ...
    let mut list_json_cmd = orbiterx_command(orbiterx_home.path())?;
    ...
    let mut get_cmd = orbiterx_command(orbiterx_home.path())?;
    ...
    assert!(stdout.contains("remove: orbiterx mcp remove docs"));

    let mut get_json_cmd = orbiterx_command(orbiterx_home.path())?;
    get_json_cmd
