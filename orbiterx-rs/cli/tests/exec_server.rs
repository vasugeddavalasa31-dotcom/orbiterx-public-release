...
"#
    ...
    let orbiterx_bin = orbiterx_utils_cargo_bin::cargo_bin("orbiterx")?;
    let orbiterx_home = orbiterx_home.path().to_path_buf();
    let subprocess = async move {
        let mut command = tokio::process::Command::new(orbiterx_bin);
        command
...
fn local_exec_server_exits_successfully_on_sigterm() -> Result<()> {
    let orbiterx_home = TempDir::new()?;
    let mut child = std::process::Command::new(orbiterx_utils_cargo_bin::cargo_bin("orbiterx")?)
