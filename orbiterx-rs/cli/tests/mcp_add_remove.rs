...
async fn add_and_remove_server_updates_global_config() -> Result<()> {
    ...
    let mut add_cmd = orbiterx_command(orbiterx_home.path())?;
    add_cmd
    ...
    let mut remove_cmd = orbiterx_command(orbiterx_home.path())?;
    remove_cmd
    ...
    let mut remove_again_cmd = orbiterx_command(orbiterx_home.path())?;
    remove_again_cmd
        .args(["mcp", "remove", "docs"])
        .assert()
        .success()
...
"#,
    ...
    let mut list_cmd = orbiterx_command(orbiterx_home.path())?;
    list_cmd
...
async fn add_streamable_http_without_manual_token() -> Result<()> {
    ...
    assert!(!orbiterx_home.path().join(".credentials.json").exists());
    assert!(!orbiterx_home.path().join(".env").exists());
