...
use orbiterx_core_plugins::ConfiguredMarketplace;
use orbiterx_core_plugins::OPENAI_BUNDLED_MARKETPLACE_NAME;
use orbiterx_core_plugins::PluginInstallOutcome;
...
use orbiterx_core_plugins::allowed_configured_marketplace_names;
use orbiterx_core_plugins::installed_marketplaces::marketplace_install_root;
use orbiterx_core_plugins::installed_marketplaces::resolve_configured_marketplace_root;
use orbiterx_core_plugins::marketplace::MarketplaceListError;
use orbiterx_core_plugins::marketplace::MarketplacePluginAuthPolicy;
use orbiterx_core_plugins::marketplace::MarketplacePluginInstallPolicy;
use orbiterx_core_plugins::marketplace::MarketplacePluginSource;
use orbiterx_core_plugins::marketplace::find_marketplace_manifest_path;
use orbiterx_login::OrbiterXAuth;
use orbiterx_login::auth::read_orbiterx_api_key_from_env;
use orbiterx_plugin::PluginId;
use orbiterx_plugin::validate_plugin_segment;
...
#[derive(Debug, Parser)]
#[command(bin_name = "orbiterx plugin")]
...
#[derive(Debug, Parser)]
#[command(
    bin_name = "orbiterx plugin add",
    after_help = "Examples:\n  orbiterx plugin add sample@debug\n  orbiterx plugin add sample --marketplace debug"
)]
...
#[derive(Debug, Parser)]
#[command(
    bin_name = "orbiterx plugin list",
    after_help = "Examples:\n  orbiterx plugin list\n  orbiterx plugin list --marketplace debug\n  orbiterx plugin list --json\n  orbiterx plugin list --available --json"
)]
...
#[derive(Debug, Parser)]
#[command(
    bin_name = "orbiterx plugin remove",
    after_help = "Examples:\n  orbiterx plugin remove sample@debug\n  orbiterx plugin remove sample --marketplace debug"
)]
...
pub async fn run_plugin_list(
    ...
    let marketplace_sources =
        configured_marketplace_sources(&plugins_input, orbiterx_home.as_path());
        ...
        for (index, marketplace) in marketplaces.into_iter().enumerate() {
            ...
            for plugin in &marketplace.plugins {
                ...
                let path = match &plugin.source {
                    orbiterx_core_plugins::marketplace::MarketplacePluginSource::Local { path } => {
                        path.as_path().display().to_string()
                    }
                    orbiterx_core_plugins::marketplace::MarketplacePluginSource::Git {
                        url,
                        path,
                        ref_name,
                        sha,
                    ...
                    orbiterx_core_plugins::marketplace::MarketplacePluginSource::Npm {
                        package,
                        version,
                        registry,
                    } => {
...
impl JsonPluginListOutput {
    fn from_marketplaces(
        marketplaces: Vec<orbiterx_core_plugins::ConfiguredMarketplace>,
...
pub(crate) fn configured_marketplace_sources(
    ...
    let allowed_marketplace_names =
        allowed_configured_marketplace_names(&plugins_input.config_layer_stack, orbiterx_home);
...
async fn load_plugin_command_context(
    ...
    let manager = PluginsManager::new(orbiterx_home.to_path_buf());
...
pub(crate) async fn load_cli_auth_mode(config: &Config) -> Option<AuthMode> {
    if let Some(api_key) = read_orbiterx_api_key_from_env() {
        return Some(OrbiterXAuth::from_api_key(&api_key).api_auth_mode());
    }

    let auth_route_config = config.auth_route_config();
    OrbiterXAuth::from_auth_storage(
...
pub(crate) fn configured_marketplace_snapshot_issues(
    ...
    let default_install_root = marketplace_install_root(orbiterx_home);
    ...
    for (configured_name, marketplace) in configured_marketplaces {
        ...
        match find_marketplace_manifest_path(&root) {
            ...
            None => {
                if is_implicit_system_marketplace_root(configured_name, orbiterx_home, &root) {
                    continue;
                }
                issues.push(ConfiguredMarketplaceSnapshotIssue {
                    marketplace_name: configured_name.clone(),
