...
use orbiterx_app_server_daemon::LifecycleOutput as AppServerLifecycleOutput;
use orbiterx_app_server_daemon::LifecycleStatus as AppServerLifecycleStatus;
use orbiterx_app_server_daemon::RemoteControlReadyOutput as AppServerRemoteControlReadyOutput;
use orbiterx_app_server_daemon::RemoteControlReadyStatus as AppServerRemoteControlReadyStatus;
use orbiterx_app_server_daemon::RemoteControlStartOutput as AppServerRemoteControlStartOutput;
...
pub(crate) async fn run(
    ...
    match command.subcommand {
        ...
        Some(RemoteControlSubcommand::Start) => {
            ...
            let output = orbiterx_app_server_daemon::ensure_remote_control_ready().await?;
        ...
        Some(RemoteControlSubcommand::Stop) => {
            ...
            let output = orbiterx_app_server_daemon::run(AppServerLifecycleCommand::Stop).await?;
            print_remote_control_stop_output(&output, command.json)?;
        }
        Some(RemoteControlSubcommand::Pair) => {
            let output = orbiterx_app_server_daemon::start_remote_control_pairing().await?;
...
async fn run_foreground_remote_control(
    ...
    let mut app_server_task = tokio::spawn(orbiterx_app_server::run_main_with_transport_options(
        arg0_paths,
...
async fn wait_for_foreground_remote_control_ready(
    ...
    orbiterx_app_server_daemon::enable_remote_control_on_socket(
        socket_path.as_path(),
...
fn daemon_app_server_human_lines(output: &AppServerRemoteControlStartOutput) -> Vec<String> {
    let (managed_orbiterx_path, managed_orbiterx_version) = daemon_app_server_identity(output);
    vec![
        "Daemon used app-server:".to_string(),
        format!("  path: {}", managed_orbiterx_path.display()),
        format!(
            "  version: {}",
            managed_orbiterx_version.unwrap_or("unknown")
        ),
    ]
}
...
fn daemon_app_server_identity(
    ...
    match output {
        AppServerRemoteControlStartOutput::Bootstrap(output) => (
            &output.managed_orbiterx_path,
            output.managed_orbiterx_version.as_deref(),
        ),
...
mod tests {
    ...
    fn daemon_ready_output(
        ...
        AppServerRemoteControlReadyOutput {
            daemon: AppServerRemoteControlStartOutput::Start(AppServerLifecycleOutput {
                ...
                backend: None,
                pid: Some(42),
                managed_orbiterx_path: PathBuf::from("/opt/orbiterx/bin/orbiterx"),
                managed_orbiterx_version: Some("1.0.0".to_string()),
    ...
    fn daemon_app_server_human_lines_include_path_and_version() {
        assert_eq!(
            ...
            vec![
                "Daemon used app-server:".to_string(),
                "  path: /opt/orbiterx/bin/orbiterx".to_string(),
        ...
        assert_eq!(
            ...
            json!({
                ...
                "daemon": {
                    "status": "started",
                    "pid": 42,
                    "managedOrbiterXPath": "/opt/orbiterx/bin/orbiterx",
                    "managedOrbiterXVersion": "1.0.0",
