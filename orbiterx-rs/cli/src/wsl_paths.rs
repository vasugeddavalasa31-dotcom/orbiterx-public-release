...
pub use orbiterx_utils_path::is_wsl;
...
mod tests {
    ...
    fn win_to_wsl_basic() {
        assert_eq!(
            win_path_to_wsl(r"C:\Temp\orbiterx.zip").as_deref(),
            Some("/mnt/c/Temp/orbiterx.zip")
        );
        assert_eq!(
            win_path_to_wsl("D:/Work/orbiterx.tgz").as_deref(),
            Some("/mnt/d/Work/orbiterx.tgz")
        );
        assert!(win_path_to_wsl("/home/user/orbiterx").is_none());
    }
