...
use orbiterx_core_plugins::PluginMarketplaceUpgradeOutcome;
...
use orbiterx_core_plugins::marketplace::marketplace_root_dir;
use orbiterx_core_plugins::marketplace_add::MarketplaceAddOutcome;
use orbiterx_core_plugins::marketplace_add::MarketplaceAddRequest;
use orbiterx_core_plugins::marketplace_add::add_marketplace;
use orbiterx_core_plugins::marketplace_remove::MarketplaceRemoveOutcome;
use orbiterx_core_plugins::marketplace_remove::MarketplaceRemoveRequest;
...
#[derive(Debug, Parser)]
#[command(bin_name = "orbiterx plugin marketplace")]
...
enum MarketplaceSubcommand {
    ...
    /// List plugin marketplaces OrbiterX is currently considering and their roots.
...
#[derive(Debug, Parser)]
#[command(
    bin_name = "orbiterx plugin marketplace add",
    after_help = "Examples:\n  orbiterx plugin marketplace add ./path/to/marketplace\n  orbiterx plugin marketplace add owner/repo --ref main\n  orbiterx plugin marketplace add https://github.com/owner/repo --sparse plugins/foo"
)]
...
#[derive(Debug, Parser)]
#[command(bin_name = "orbiterx plugin marketplace list")]
...
#[derive(Debug, Parser)]
#[command(
    bin_name = "orbiterx plugin marketplace upgrade",
    after_help = "Examples:\n  orbiterx plugin marketplace upgrade\n  orbiterx plugin marketplace upgrade debug"
)]
...
#[derive(Debug, Parser)]
#[command(
    bin_name = "orbiterx plugin marketplace remove",
    after_help = "Example:\n  orbiterx plugin marketplace remove debug"
)]
...
async fn run_list(overrides: Vec<(String, toml::Value)>, args: ListMarketplaceArgs) -> Result<()> {
    let config = Config::load_with_cli_overrides(overrides)
        .await
        .context("failed to load configuration")?;
    let manager = PluginsManager::new(config.orbiterx_home.to_path_buf());
    ...
    if args.json {
        let marketplace_sources =
            configured_marketplace_sources_by_root(config.orbiterx_home.as_path(), &plugins_input);
        let output =
...
impl JsonMarketplaceListOutput {
    fn from_marketplaces(
        marketplaces: Vec<orbiterx_core_plugins::marketplace::Marketplace>,
...
fn configured_marketplace_sources_by_root(
    ...
    let marketplace_sources = configured_marketplace_sources(plugins_input, orbiterx_home);
