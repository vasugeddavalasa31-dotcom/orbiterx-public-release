//! Diagnoses whether OrbiterX update paths target the running installation.
//!
...
const HOMEBREW_CASK_API_URL: &str = "https://formulae.brew.sh/api/cask/orbiterx.json";
...
pub(super) fn updates_check(config: &Config) -> DoctorCheck {
    ...
    let version_file = config.orbiterx_home.join(VERSION_FILE_NAME);
...
fn update_action_label(context: &InstallContext) -> &'static str {
    match &context.method {
        InstallMethod::Npm => "npm install -g @openai/orbiterx",
        InstallMethod::Bun => "bun install -g @openai/orbiterx",
        InstallMethod::Pnpm => "pnpm add -g @openai/orbiterx",
        InstallMethod::Brew => "brew upgrade --cask orbiterx",
