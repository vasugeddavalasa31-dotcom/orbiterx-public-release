...
use orbiterx_mcp::ResolvedMcpOAuthScopes;
use orbiterx_mcp::compute_auth_statuses;
use orbiterx_mcp::discover_supported_scopes;
...
use orbiterx_rmcp_client::delete_oauth_tokens;
use orbiterx_rmcp_client::perform_oauth_login;
use orbiterx_utils_cli::CliConfigOverrides;
use orbiterx_utils_cli::format_env_display;

/// Subcommands:
/// - `list`   — list configured servers (with `--json`)
/// - `get`    — show a single server (with `--json`)
/// - `add`    — add a server launcher entry to `~/.orbiterx/config.toml`
...
#[derive(Debug, clap::Parser)]
#[command(override_usage = "orbiterx mcp add [OPTIONS] <NAME> (--url <URL> | -- <COMMAND>...)")]
pub struct AddArgs {
...
async fn perform_oauth_login_retry_without_scopes(
    name: &str,
    url: &str,
    store_mode: orbiterx_config::types::OAuthCredentialsStoreMode,
    keyring_backend_kind: orbiterx_config::types::AuthKeyringBackendKind,
...
async fn run_add(config_overrides: &CliConfigOverrides, add_args: AddArgs) -> Result<()> {
    ...
    let mut servers = load_global_mcp_servers(&orbiterx_home)
        .await
        .with_context(|| {
            format!(
    ...
    ConfigEditsBuilder::new(&orbiterx_home)
        ...
        .apply()
        .await
        .with_context(|| format!("failed to write MCP servers to {}", orbiterx_home.display()))?;
    ...
    match oauth_login_support(&transport).await {
        ...
        McpOAuthLoginSupport::Unknown(_) => println!(
            "MCP server may or may not require login. Run `orbiterx mcp login {name}` to login."
        ),
    }
...
async fn run_remove(config_overrides: &CliConfigOverrides, remove_args: RemoveArgs) -> Result<()> {
    ...
    if removed {
        ConfigEditsBuilder::new(&orbiterx_home)
            ...
            .await
            .with_context(|| {
                format!("failed to write MCP servers to {}", orbiterx_home.display())
            })?;
    }
...
async fn run_list(config_overrides: &CliConfigOverrides, list_args: ListArgs) -> Result<()> {
    ...
    if entries.is_empty() {
        println!("No MCP servers configured yet. Try `orbiterx mcp add my-tool -- my-command`.");
        return Ok(());
    }
...
async fn run_get(config_overrides: &CliConfigOverrides, get_args: GetArgs) -> Result<()> {
    ...
    println!("  remove: orbiterx mcp remove {}", get_args.name);
