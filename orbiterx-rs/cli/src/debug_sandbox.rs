...
#[cfg(target_os = "macos")]
use orbiterx_core::spawn::ORBITERX_SANDBOX_ENV_VAR;
...
#[cfg(target_os = "macos")]
use orbiterx_sandboxing::seatbelt::CreateSeatbeltCommandArgsParams;
#[cfg(target_os = "macos")]
use orbiterx_sandboxing::seatbelt::create_seatbelt_command_args;
use orbiterx_sandboxing::with_managed_mitm_ca_readable_root;
...
async fn run_command_under_sandbox(
    ...
    let sandbox_state = config_options
        ...
        .sandbox_state_json
        .as_deref()
        .map(serde_json::from_str::<orbiterx_mcp::SandboxState>)
        .transpose()
    ...
    let orbiterx_linux_sandbox_exe = match sandbox_state.as_ref() {
        Some(state) => {
            ...
            state
                .orbiterx_linux_sandbox_exe
                .clone()
                .or(orbiterx_linux_sandbox_exe)
        }
        None => orbiterx_linux_sandbox_exe,
    };
    ...
    let network = network_proxy
        .as_ref()
        .map(orbiterx_core::config::StartedNetworkProxy::proxy);
    ...
    let mut child = match sandbox_type {
        ...
        SandboxType::Seatbelt => {
            ...
            spawn_debug_sandbox_child(
                ...
                env,
                |env_map| {
                    env_map.insert(ORBITERX_SANDBOX_ENV_VAR.to_string(), "seatbelt".to_string());
        ...
        SandboxType::Landlock => {
            ...
            let orbiterx_linux_sandbox_exe = config
                .orbiterx_linux_sandbox_exe
                .expect("orbiterx-linux-sandbox executable not found");
...
async fn run_command_under_windows_session(
    ...
    use orbiterx_windows_sandbox::WindowsSandboxProxySettingsMode;
    use orbiterx_windows_sandbox::WindowsSandboxSessionRequest;
    use orbiterx_windows_sandbox::spawn_windows_sandbox_session_for_level;

    let empty_paths: &[AbsolutePathBuf] = &[];
    let spawned = spawn_windows_sandbox_session_for_level(WindowsSandboxSessionRequest {
        permission_profile,
        workspace_roots: workspace_roots.as_slice(),
        orbiterx_home: config.orbiterx_home.as_path(),
        command,
        cwd: cwd.as_path(),
        env_map: env,
    ...
    let exit_code = orbiterx_windows_sandbox::forward_sandbox_session_stdio(spawned).await;
...
async fn spawn_debug_sandbox_child(
    ...
    if !network_sandbox_policy.is_enabled() {
        cmd.env(ORBITERX_SANDBOX_NETWORK_DISABLED_ENV_VAR, "1");
    }
...
async fn load_debug_sandbox_config(
    ...
    load_debug_sandbox_config_with_orbiterx_home(
        cli_overrides,
        orbiterx_linux_sandbox_exe,
        options,
        /*orbiterx_home*/ None,
...
async fn load_debug_sandbox_config_with_orbiterx_home(
    cli_overrides: Vec<(String, TomlValue)>,
    orbiterx_linux_sandbox_exe: Option<PathBuf>,
    options: DebugSandboxConfigOptions,
    ...
    // For legacy configs, `orbiterx sandbox` historically defaulted to read-only
    ...
    let config = build_debug_sandbox_config_with_loader_overrides(
        ...
        ConfigOverrides {
            cwd: cwd.clone(),
            orbiterx_linux_sandbox_exe: orbiterx_linux_sandbox_exe.clone(),
            ..Default::default()
        },
        orbiterx_home.clone(),
        managed_requirements_mode,
...
async fn build_debug_sandbox_config_with_loader_overrides(
    ...
    if let Some(orbiterx_home) = orbiterx_home {
        builder = builder
            .orbiterx_home(orbiterx_home.clone())
            .fallback_cwd(Some(orbiterx_home));
    }
    builder.build().await
}
...
mod tests {
    ...
    async fn debug_sandbox_honors_active_permission_profiles() -> anyhow::Result<()> {
        ...
        let config = load_debug_sandbox_config_with_orbiterx_home(
            Vec::new(),
    ...
    async fn debug_sandbox_honors_config_profile_loader_overrides() -> anyhow::Result<()> {
        ...
        let docs = sandbox_paths.path().join("docs");
        let private = docs.join("private");
        let profile_path = orbiterx_home.path().join("work.config.toml");
    ...
    async fn debug_sandbox_honors_explicit_builtin_permission_profile() -> anyhow::Result<()> {
        ...
        let config = load_debug_sandbox_config_with_orbiterx_home(
            ...
            Some(orbiterx_home.path().to_path_buf()),
            /*strict_config*/ false,
        )
        .await?;
        ...
        let actual = config
            ...
            .permission_profile()
            .file_system_sandbox_policy();
        let expected = orbiterx_protocol::models::PermissionProfile::workspace_write()
            .file_system_sandbox_policy();
        assert!(
            expected
                .entries
