//! Implements the `orbiterx doctor` diagnostic report.
//!
...
use orbiterx_api::ResponsesWebsocketClient;
use orbiterx_api::is_azure_responses_provider;
...
use orbiterx_install_context::InstallMethod;
use orbiterx_install_context::OrbiterXPackageLayout;
use orbiterx_install_context::StandalonePlatform;
use orbiterx_login::AuthDotJson;
use orbiterx_login::AuthManager;
use orbiterx_login::OPENAI_API_KEY_ENV_VAR;
use orbiterx_login::ORBITERX_ACCESS_TOKEN_ENV_VAR;
...
use orbiterx_login::default_client::default_headers;
use orbiterx_login::load_auth_dot_json;
...
use orbiterx_terminal_detection::Multiplexer;
use orbiterx_terminal_detection::TerminalInfo;
use orbiterx_terminal_detection::TerminalName;
use orbiterx_terminal_detection::terminal_info;
...
/// Options for building a local OrbiterX diagnostic report.
///
...
///
/// This is the CLI entry point for orbiterx doctor. It does not repair issues;
...
async fn build_report(
            ...
            let (
                ...
                async {
                    run_sync_check("config", progress.clone(), || {
                        ...
                        .remediation("Fix the reported config error, then rerun orbiterx doctor.")
                    })
                },
                async { run_sync_check("network", progress.clone(), network_check) },
                async {
    ...
    DoctorReport {
        ...
        overall_status,
        orbiterx_version: env!("CARGO_PKG_VERSION").to_string(),
        checks,
    }
}
...
async fn load_config(
    ...
    ConfigBuilder::default()
        ...
        .build()
        .await
        .context("failed to load OrbiterX config")
}
...
/// JSON support report emitted by `orbiterx doctor --json`.
///
...
fn redacted_json_report(report: &DoctorReport) -> JsonDoctorReport {
    ...
    JsonDoctorReport {
        ...
        orbiterx_version: report.orbiterx_version.clone(),
        checks,
    }
}
...
fn installation_check(show_details: bool) -> DoctorCheck {
    ...
    details.push(format!(
        "managed by bun: {}",
        env::var_os("ORBITERX_MANAGED_BY_BUN").is_some()
    ));
    details.push(format!(
        "managed by pnpm: {}",
        env::var_os("ORBITERX_MANAGED_BY_PNPM").is_some()
    ));
    push_env_path_detail(
        &mut details,
        "managed package root",
        "ORBITERX_MANAGED_PACKAGE_ROOT",
    );

    let path_entries = orbiterx_path_entries();
    let mut status = CheckStatus::Ok;
    let mut summary = "installation looks consistent".to_string();
    let mut remediation = None;

    if path_entries.len() > 1 {
        details.push(format!("PATH orbiterx entries: {}", path_entries.len()));
    }
    if show_details || path_entries.len() > 1 {
        details.extend(
            path_entries
                .iter()
                .enumerate()
                .map(|(index, path)| format!("PATH orbiterx #{}: {path}", index + 1)),
        );
    }
    ...
    if doctor_managed_by_npm(current_exe.as_deref()) {
        match npm_global_root_check() {
            ...
            NpmRootCheck::Mismatch {
                ...
                summary =
                    "npm install -g @openai/orbiterx would update a different install".to_string();
            ...
            NpmRootCheck::MissingPackageRoot => {
                ...
                remediation = Some(
                    "Reinstall or update OrbiterX so the JS shim provides ORBITERX_MANAGED_PACKAGE_ROOT."
                        .to_string(),
                );
            }
...
fn doctor_managed_by_npm(current_exe: Option<&Path>) -> bool {
    env::var_os("ORBITERX_MANAGED_BY_NPM").is_some()
        && !inherited_managed_env_for_cargo_binary(current_exe)
}

fn inherited_managed_env_for_cargo_binary(current_exe: Option<&Path>) -> bool {
    if env::var_os("ORBITERX_MANAGED_BY_NPM").is_none()
        && env::var_os("ORBITERX_MANAGED_BY_BUN").is_none()
        && env::var_os("ORBITERX_MANAGED_BY_PNPM").is_none()
    {
        return false;
    }
...
fn describe_method_with_package_layout(
    method: &str,
    package_layout: Option<&OrbiterXPackageLayout>,
) -> String {
...
fn npm_global_root_check() -> NpmRootCheck {
    let Some(running_package_root) =
        env::var_os("ORBITERX_MANAGED_PACKAGE_ROOT").map(PathBuf::from)
    else {
...
fn compare_npm_package_roots(running_package_root: &Path, npm_root: &Path) -> NpmRootCheck {
    let npm_package_root = npm_root.join("@openai").join("orbiterx");
...
fn orbiterx_path_entries() -> Vec<String> {
    #[cfg(windows)]
    let result = run_command("where", ["orbiterx"]);
    #[cfg(not(windows))]
    let result = run_command("which", ["-a", "orbiterx"]);
...
fn config_check(config: &Config) -> DoctorCheck {
    let mut details = Vec::new();
    details.push(format!("ORBITERX_HOME: {}", config.orbiterx_home.display()));
...
fn config_toml_details(config: &Config, details: &mut Vec<String>) {
    let config_path = config.orbiterx_home.join(orbiterx_config::CONFIG_TOML_FILE);
...
fn auth_check(config: &Config) -> DoctorCheck {
    let mut details = Vec::new();
    let auth_path = config.orbiterx_home.join("auth.json");
    details.push(format!(
        "auth storage mode: {:?}",
    ...
    let env_auth_vars = [
        ...
        ORBITERX_API_KEY_ENV_VAR,
        ORBITERX_ACCESS_TOKEN_ENV_VAR,
    ]
    .into_iter()
    ...
    match load_auth_dot_json(
        ...
        Ok(Some(auth)) => {
            ...
            if status == CheckStatus::Fail {
                check = check
                    .remediation("Run orbiterx login again or provide a supported auth env var.");
            }
            check
        }
        ...
        Ok(None) => DoctorCheck::new(
            ...
            "auth",
            CheckStatus::Fail,
            "no OrbiterX credentials were found",
        )
        .details(details)
        .remediation("Run orbiterx login or provide an API key through a supported auth env var."),
        ...
        .detail(err.to_string())
        .remediation("Fix auth storage access or run orbiterx login again."),
    }
}
...
fn stored_auth_mode(auth: &orbiterx_login::AuthDotJson) -> &'static str {
    ...
    match stored_auth_mode_value(auth) {
        AuthMode::ApiKey => {
            ...
            let env_key_present = env_var_present(OPENAI_API_KEY_ENV_VAR)
                || env_var_present(ORBITERX_API_KEY_ENV_VAR);
...
fn network_check() -> DoctorCheck {
    ...
    for name in ["ORBITERX_CA_CERTIFICATE", "SSL_CERT_FILE"] {
...
fn sandbox_check(config: &Config, arg0_paths: &Arg0DispatchPaths) -> DoctorCheck {
    ...
    push_path_detail(
        &mut details,
        "orbiterx-linux-sandbox helper",
        arg0_paths.orbiterx_linux_sandbox_exe.as_deref(),
    );
    push_path_detail(
        &mut details,
    ...
    if let Some(helper) = arg0_paths.orbiterx_linux_sandbox_exe.as_deref()
        && !helper.exists()
    {
        status = CheckStatus::Warning;
...
async fn state_check(config: &Config) -> DoctorCheck {
    let mut details = Vec::new();
    path_readiness(&mut details, "ORBITERX_HOME", &config.orbiterx_home);
    ...
    for db in orbiterx_state::runtime_db_paths(&config.sqlite_home) {
    ...
    rollout_stats_details(&mut details, &config.orbiterx_home);
...
async fn sqlite_integrity_detail(
    ...
    match orbiterx_state::sqlite_integrity_check(path).await {
...
fn rollout_stats_details(details: &mut Vec<String>, orbiterx_home: &Path) {
    let active = collect_rollout_stats(&orbiterx_home.join("sessions"));
    let archived = collect_rollout_stats(&orbiterx_home.join("archived_sessions"));
...
fn auth_mode_name(auth: &OrbiterXAuth) -> &'static str {
...
fn fallback_state_check() -> DoctorCheck {
    ...
    match orbiterx_home {
        Ok(path) => DoctorCheck::new(
            ...
            "state",
            CheckStatus::Ok,
            "ORBITERX_HOME was resolved without config",
        )
        .detail(format!("ORBITERX_HOME: {}", path.display())),
        Err(err) => DoctorCheck::new(
            "state.paths",
            "state",
            CheckStatus::Warning,
            "ORBITERX_HOME could not be resolved",
        )
        .detail(err.to_string()),
    }
}
...
fn provider_auth_reachability_mode_from_auth(
    ...
    if env_var_present(OPENAI_API_KEY_ENV_VAR) || env_var_present(ORBITERX_API_KEY_ENV_VAR) {
        return ProviderAuthReachabilityMode::ApiKey;
    }
    if env_var_present(ORBITERX_ACCESS_TOKEN_ENV_VAR) {
...
mod tests {
    ...
    fn compare_npm_package_roots_detects_match() {
        let running = PathBuf::from("/prefix/lib/node_modules/@openai/orbiterx");
        let npm_root = PathBuf::from("/prefix/lib/node_modules");
        assert_eq!(
            compare_npm_package_roots(&running, &npm_root),
            NpmRootCheck::Match {
                package_root: npm_root.join("@openai").join("orbiterx")
            }
        );
    }
    ...
    fn compare_npm_package_roots_detects_mismatch() {
        let running = PathBuf::from("/old/lib/node_modules/@openai/orbiterx");
        let npm_root = PathBuf::from("/new/lib/node_modules");
        assert_eq!(
            compare_npm_package_roots(&running, &npm_root),
            NpmRootCheck::Mismatch {
                running_package_root: running,
                npm_package_root: npm_root.join("@openai").join("orbiterx"),
            }
        );
    }
    ...
    fn startup_warning_counts_group_known_sources() {
        let warnings = vec![
            ...
            "[features].orbiterx_hooks is deprecated. Use [features].hooks instead.".to_string(),
    ...
    fn config_overrides_from_interactive_preserves_global_options() {
        ...
        let arg0_paths = Arg0DispatchPaths {
            orbiterx_self_exe: Some(PathBuf::from("/bin/orbiterx")),
            orbiterx_linux_sandbox_exe: Some(PathBuf::from("/bin/orbiterx-linux-sandbox")),
            main_execve_wrapper_exe: Some(PathBuf::from("/bin/orbiterx-execve-wrapper")),
        };
        ...
        assert_eq!(overrides.orbiterx_self_exe, arg0_paths.orbiterx_self_exe);
        assert_eq!(
            overrides.orbiterx_linux_sandbox_exe,
            arg0_paths.orbiterx_linux_sandbox_exe
        );
        assert_eq!(
    ...
    fn redacted_json_report_structures_and_sanitizes_details() {
        let report = DoctorReport {
            ...
            orbiterx_version: "0.0.0".to_string(),
            checks: vec![
                DoctorCheck::new(
                    "system.environment",
                    "system",
    ...
    async fn mcp_check_ignores_disabled_servers() {
        let disabled_server: McpServerConfig = toml::from_str(
            r#"
                ...
                enabled = false
                required = true
                bearer_token_env_var = "ORBITERX_DOCTOR_DISABLED_MCP_TOKEN"
            "#,
        )
        ...
        assert!(
            check
                .details
                .iter()
                .all(|detail| !detail.contains("ORBITERX_DOCTOR_DISABLED_MCP_TOKEN"))
        );
        assert!(
            check
                .details
    ...
    fn provider_specific_auth_fails_when_provider_env_key_is_missing() {
        let check = provider_specific_auth_check(
            ...
            Some("Set PROVIDER_API_KEY before running OrbiterX."),
            Vec::new(),
            |_| false,
        )
        .expect("non-OpenAI provider should produce a provider-specific check");
        ...
        assert_eq!(
            check.remediation,
            Some("Set PROVIDER_API_KEY before running OrbiterX.".to_string())
        );
    }
    ...
    async fn mcp_check_fails_required_missing_stdio_command() {
        let required_server: McpServerConfig = toml::from_str(
            r#"
                command = "definitely-missing-orbiterx-doctor-mcp"
                required = true
            "#,
        )
        .expect("should deserialize required MCP config");
        ...
        assert!(check.details.iter().any(|detail| {
            detail.contains(
                "required: stdio command \"definitely-missing-orbiterx-doctor-mcp\" is not resolvable",
            )
        }));
    }
