...
use orbiterx_config::HookStateToml;
...
use orbiterx_config::version_for_toml;
...
fn config_toml_source_path(layer: &ConfigLayerEntry) -> AbsolutePathBuf {
    match &layer.name {
        ...
        ConfigLayerSource::Project {
            ...
            .hooks_config_folder()
            .unwrap_or_else(|| dot_orbiterx_folder.clone())
...
fn append_matcher_groups(
    ...
    display_order: &mut i64,
    source: &HookHandlerSource<'_>,
    event_name: orbiterx_protocol::protocol::HookEventName,
        ...
        for (handler_index, handler) in group.hooks.iter().cloned().enumerate() {
            match handler {
                HookHandlerConfig::Command {
                    ...
                    if r#async
                        && event_name != orbiterx_protocol::protocol::HookEventName::SessionEnd
                    {
                        warnings.push(format!(
...
fn normalize_command_hook(
    ...
    if event_name != orbiterx_protocol::protocol::HookEventName::SessionEnd {
...
mod tests {
        ...
        let source = RequirementSource::Composite {
            sources: vec![
                RequirementSource::SystemRequirementsToml {
                    file: test_path_buf("/etc/orbiterx/requirements.toml").abs(),
                },
    ...
    fn hook_metadata_for_config_layer_source_discards_source_details() {
        let config_file = test_path_buf("/tmp/.orbiterx/config.toml").abs();
        let dot_orbiterx_folder = test_path_buf("/tmp/worktree/.orbiterx").abs();
