...
use orbiterx_utils_path_uri::PathUriParseError;
pub use parser::Hunk;
...
/// Special argv[1] flag used when the OrbiterX executable self-invokes to run the
/// internal `apply_patch` path.
///
/// Although this constant lives in `orbiterx-apply-patch` (to avoid forcing
/// `orbiterx-arg0` to depend on `orbiterx-core`), it remains part of the "orbiterx core"
/// process-invocation contract for the standalone `apply_patch` command
/// surface.
pub const ORBITERX_CORE_APPLY_PATCH_ARG1: &str = "--orbiterx-run-as-apply-patch";
