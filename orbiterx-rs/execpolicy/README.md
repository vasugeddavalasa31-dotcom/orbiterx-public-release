...
- From the OrbiterX CLI, run `orbiterx execpolicy check` subcommand with one or more policy files (for example `src/default.rules`) to check a command:

```bash
orbiterx execpolicy check --rules path/to/policy.rules git status
```

- To opt into basename fallback for absolute program paths, pass `--resolve-host-executables`:

```bash
orbiterx execpolicy check \
...
```bash
cargo run -p orbiterx-execpolicy -- check --rules path/to/policy.rules git status
```
