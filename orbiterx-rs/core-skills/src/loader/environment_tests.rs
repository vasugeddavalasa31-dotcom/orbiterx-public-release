...
async fn loads_plugin_namespace_dependencies_and_policy() {
    ...
    fs::create_dir_all(root.path().join(".orbiterx-plugin")).expect("manifest dir");
...
"#,
    ...
    let outcome =
        load_environment_skills_from_root(LOCAL_FS.as_ref(), &root_uri, Some(Product::OrbiterX))
            .await;
