...
use orbiterx_backend_client::ConfigBundleResponse;
use orbiterx_backend_client::DeliveredTomlFragment;
use orbiterx_config::AbsolutePathBuf;
use orbiterx_config::CloudConfigFragment;
use orbiterx_config::CloudConfigTomlBundle;
use orbiterx_config::CloudRequirementsFragment;
use orbiterx_config::CloudRequirementsTomlBundle;
...
fn write_auth_json(orbiterx_home: &Path, value: serde_json::Value) -> std::io::Result<()> {
    std::fs::write(
...
fn create_test_cache(orbiterx_home: &Path) -> CloudConfigBundleCache {
...
async fn auth_manager_with_agent_identity_business_plan() -> Arc<AuthManager> {
    ...
    AuthManager::from_auth_for_testing(OrbiterXAuth::AgentIdentity(
...
struct TestExternalChatgptAuth {
    current: RwLock<OrbiterXAuth>,
    refreshed: OrbiterXAuth,
...
async fn get_bundle_ignores_invalid_cache_and_refetches() {
    let orbiterx_home = tempdir().expect("tempdir");
    let cache = create_test_cache(orbiterx_home.path());
    cache
        .save(
...
async fn get_bundle_refreshes_external_auth_after_unauthorized() {
    ...
    let initial_auth = OrbiterXAuth::from_external_chatgpt_tokens(
    ...
    let refreshed_auth = OrbiterXAuth::from_external_chatgpt_tokens(
        &refreshed_token,
        "account-12345",
        Some("enterprise"),
    )
...
fn bundle_response_conversion_preserves_fragment_order() {
    let response = ConfigBundleResponse {
        config_toml: Some(Some(Box::new(
            orbiterx_backend_client::DeliveredConfigToml {
        ...
        requirements_toml: Some(Some(Box::new(
            orbiterx_backend_client::DeliveredRequirementsToml {
