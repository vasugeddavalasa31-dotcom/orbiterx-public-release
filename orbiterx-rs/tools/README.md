...
`orbiterx-tools` is the shared support crate for building, adapting, and executing
model-visible tools outside `orbiterx-core`.
...
compatibility-sensitive orchestration in `orbiterx-core` until the surrounding
...
- code-mode compatibility shims that do not depend on `orbiterx-core`
- other narrowly scoped host utilities that multiple crates need

The corresponding non-goals are just as important:

- do not move `orbiterx-core` orchestration here prematurely
...
1. Keep extension-owned executable-tool authoring in `orbiterx-extension-api`.
2. Move host-side planning/adaptation helpers here when they no longer need to
   stay coupled to `orbiterx-core`.
3. Leave compatibility-sensitive adapters in `orbiterx-core` while downstream
...
`orbiterx-core`, that is a sign to revisit the extraction boundary before adding
more here.
