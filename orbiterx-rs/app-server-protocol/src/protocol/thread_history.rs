...
use orbiterx_protocol::protocol::ContextCompactedEvent;
use orbiterx_protocol::protocol::DynamicToolCallResponseEvent;
use orbiterx_protocol::protocol::ErrorEvent;
use orbiterx_protocol::protocol::EventMsg;
use orbiterx_protocol::protocol::ExecCommandBeginEvent;
use orbiterx_protocol::protocol::ExecCommandEndEvent;
use orbiterx_protocol::protocol::GuardianAssessmentEvent;
use orbiterx_protocol::protocol::GuardianAssessmentStatus;
use orbiterx_protocol::protocol::ImageGenerationBeginEvent;
use orbiterx_protocol::protocol::ImageGenerationEndEvent;
use orbiterx_protocol::protocol::ItemCompletedEvent;
use orbiterx_protocol::protocol::ItemStartedEvent;
use orbiterx_protocol::protocol::McpToolCallBeginEvent;
use orbiterx_protocol::protocol::McpToolCallEndEvent;
use orbiterx_protocol::protocol::PatchApplyBeginEvent;
use orbiterx_protocol::protocol::PatchApplyEndEvent;
...
use orbiterx_protocol::protocol::ViewImageToolCallEvent;
use orbiterx_protocol::protocol::WebSearchBeginEvent;
...
#[cfg(test)]
use orbiterx_protocol::protocol::ExecCommandStatus as CoreExecCommandStatus;
#[cfg(test)]
use orbiterx_protocol::protocol::PatchApplyStatus as CorePatchApplyStatus;
...
impl ThreadHistoryBuilder {
    ...
    /// See `should_persist_event_msg` in `orbiterx-rs/core/rollout/policy.rs`.
    ...
    fn handle_materialized_item_lifecycle(
        &mut self,
        turn_id: &str,
        item: &orbiterx_protocol::items::TurnItem,
    ) {
        let is_review_mode_item = matches!(
            item,
            orbiterx_protocol::items::TurnItem::EnteredReviewMode(_)
                | orbiterx_protocol::items::TurnItem::ExitedReviewMode(_)
        );
        let should_upsert = match item {
            orbiterx_protocol::items::TurnItem::Plan(plan) => !plan.text.is_empty(),
            orbiterx_protocol::items::TurnItem::HookPrompt(_)
            | orbiterx_protocol::items::TurnItem::CommandExecution(_)
            | orbiterx_protocol::items::TurnItem::DynamicToolCall(_)
            | orbiterx_protocol::items::TurnItem::CollabAgentToolCall(_)
            | orbiterx_protocol::items::TurnItem::SubAgentActivity(_)
            | orbiterx_protocol::items::TurnItem::Extension(_)
            | orbiterx_protocol::items::TurnItem::EnteredReviewMode(_)
            | orbiterx_protocol::items::TurnItem::ExitedReviewMode(_) => true,
            orbiterx_protocol::items::TurnItem::UserMessage(_)
            | orbiterx_protocol::items::TurnItem::AgentMessage(_)
            | orbiterx_protocol::items::TurnItem::Reasoning(_)
            | orbiterx_protocol::items::TurnItem::WebSearch(_)
            | orbiterx_protocol::items::TurnItem::ImageView(_)
            | orbiterx_protocol::items::TurnItem::ImageGeneration(_)
            | orbiterx_protocol::items::TurnItem::FileChange(_)
            | orbiterx_protocol::items::TurnItem::McpToolCall(_)
            | orbiterx_protocol::items::TurnItem::ContextCompaction(_) => false,
        };
    ...
    fn handle_dynamic_tool_call_request(
        &mut self,
        payload: &orbiterx_protocol::dynamic_tools::DynamicToolCallRequest,
    ) {
    ...
    fn handle_collab_agent_spawn_begin(
        &mut self,
        payload: &orbiterx_protocol::protocol::CollabAgentSpawnBeginEvent,
    ) {
    ...
    fn handle_collab_agent_spawn_end(
        &mut self,
        payload: &orbiterx_protocol::protocol::CollabAgentSpawnEndEvent,
    ) {
    ...
    fn handle_collab_agent_interaction_begin(
        &mut self,
        payload: &orbiterx_protocol::protocol::CollabAgentInteractionBeginEvent,
    ) {
        let item = ThreadItem::CollabAgentToolCall {
            id: payload.call_id.clone(),
    ...
    fn handle_collab_agent_interaction_end(
        &mut self,
        payload: &orbiterx_protocol::protocol::CollabAgentInteractionEndEvent,
    ) {
        let status = match &payload.status {
            AgentStatus::Errored(_) | AgentStatus::NotFound => CollabAgentToolCallStatus::Failed,
    ...
    fn handle_sub_agent_activity(
        &mut self,
        payload: &orbiterx_protocol::protocol::SubAgentActivityEvent,
    ) {
    ...
    fn handle_collab_waiting_begin(
        &mut self,
        payload: &orbiterx_protocol::protocol::CollabWaitingBeginEvent,
    ) {
        let item = ThreadItem::CollabAgentToolCall {
            id: payload.call_id.clone(),
    ...
    fn handle_collab_waiting_end(
        &mut self,
        payload: &orbiterx_protocol::protocol::CollabWaitingEndEvent,
    ) {
    ...
    fn handle_collab_close_begin(
        &mut self,
        payload: &orbiterx_protocol::protocol::CollabCloseBeginEvent,
    ) {
        let item = ThreadItem::CollabAgentToolCall {
            id: payload.call_id.clone(),
    ...
    fn handle_collab_close_end(
        &mut self,
        payload: &orbiterx_protocol::protocol::CollabCloseEndEvent,
    ) {
        let status = match &payload.status {
            AgentStatus::Errored(_) | AgentStatus::NotFound => CollabAgentToolCallStatus::Failed,
            _ => CollabAgentToolCallStatus::Completed,
    ...
    fn handle_collab_resume_begin(
        &mut self,
        payload: &orbiterx_protocol::protocol::CollabResumeBeginEvent,
    ) {
        let item = ThreadItem::CollabAgentToolCall {
            id: payload.call_id.clone(),
    ...
    fn handle_collab_resume_end(
        &mut self,
        payload: &orbiterx_protocol::protocol::CollabResumeEndEvent,
    ) {
        let status = match &payload.status {
            AgentStatus::Errored(_) | AgentStatus::NotFound => CollabAgentToolCallStatus::Failed,
            _ => CollabAgentToolCallStatus::Completed,
    ...
    fn handle_entered_review_mode(
        &mut self,
        payload: &orbiterx_protocol::protocol::EnteredReviewModeEvent,
    ) {
        let review = payload
            .user_facing_hint
            .clone()
    ...
    fn handle_exited_review_mode(
        &mut self,
        payload: &orbiterx_protocol::protocol::ExitedReviewModeEvent,
    ) {
    ...
    fn handle_error(&mut self, payload: &ErrorEvent) {
        ...
        let changed_turn = if let Some(turn) = self.current_turn.as_mut() {
            ...
            turn.error = Some(V2TurnError {
                ...
                orbiterx_error_info: payload.orbiterx_error_info.clone().map(Into::into),
...
fn convert_dynamic_tool_content_items(
    items: &[orbiterx_protocol::dynamic_tools::DynamicToolCallOutputContentItem],
) -> Vec<DynamicToolCallOutputContentItem> {
    items
        .iter()
        .cloned()
        .map(|item| match item {
            orbiterx_protocol::dynamic_tools::DynamicToolCallOutputContentItem::InputText {
                text,
            } => DynamicToolCallOutputContentItem::InputText { text },
            orbiterx_protocol::dynamic_tools::DynamicToolCallOutputContentItem::InputImage {
                image_url,
...
mod tests {
    ...
    use orbiterx_extension_items::ExtensionItem as CoreExtensionItem;
    use orbiterx_extension_items::sleep::SleepItem as CoreSleepItem;
    use orbiterx_protocol::ThreadId;
    use orbiterx_protocol::dynamic_tools::DynamicToolCallOutputContentItem as CoreDynamicToolCallOutputContentItem;
    use orbiterx_protocol::items::CommandExecutionItem as CoreCommandExecutionItem;
    use orbiterx_protocol::items::CommandExecutionStatus as CoreCommandExecutionStatus;
    use orbiterx_protocol::items::EnteredReviewModeItem as CoreEnteredReviewModeItem;
    use orbiterx_protocol::items::ExitedReviewModeItem as CoreExitedReviewModeItem;
    use orbiterx_protocol::items::HookPromptFragment as CoreHookPromptFragment;
    use orbiterx_protocol::items::TurnItem as CoreTurnItem;
    use orbiterx_protocol::items::UserMessageItem as CoreUserMessageItem;
    use orbiterx_protocol::items::build_hook_prompt_message;
    use orbiterx_protocol::mcp::CallToolResult;
    use orbiterx_protocol::models::ImageDetail;
    use orbiterx_protocol::models::MessagePhase as CoreMessagePhase;
    use orbiterx_protocol::models::WebSearchAction as CoreWebSearchAction;
    ...
    use orbiterx_protocol::protocol::CompactedItem;
    use orbiterx_protocol::protocol::DynamicToolCallResponseEvent;
    use orbiterx_protocol::protocol::EnteredReviewModeEvent;
    use orbiterx_protocol::protocol::ExecCommandEndEvent;
    use orbiterx_protocol::protocol::ExecCommandSource;
    use orbiterx_protocol::protocol::ExitedReviewModeEvent;
    use orbiterx_protocol::protocol::ItemStartedEvent;
    use orbiterx_protocol::protocol::McpInvocation;
    use orbiterx_protocol::protocol::McpToolCallEndEvent;
    use orbiterx_protocol::protocol::OrbiterXErrorInfo;
    use orbiterx_protocol::protocol::PatchApplyBeginEvent;
    use orbiterx_protocol::protocol::ReviewTarget;
    use orbiterx_protocol::protocol::ThreadRolledBackEvent;
        ...
        let events = vec![
            ...
            EventMsg::ItemStarted(ItemStartedEvent {
                ...
                item: CoreTurnItem::UserMessage(CoreUserMessageItem {
                    ...
                    content: vec![orbiterx_protocol::user_input::UserInput::Text {
                        text: "hello".into(),
                        text_elements: Vec::new(),
                    }],
                }),
        ...
        let events = vec![
            ...
            EventMsg::WebSearchEnd(WebSearchEndEvent {
                ...
                action: CoreWebSearchAction::Search {
                    query: Some("orbiterx".into()),
                    queries: None,
                },
                results: Some(vec![serde_json::json!({
                    "type": "text_result",
                    "ref_id": "turn0search0",
                    "url": "https://example.com/orbiterx",
                })]),
            }),
    ...
    fn reconstructs_dynamic_tool_items_from_request_and_response_events() {
        let events = vec![
            ...
            EventMsg::DynamicToolCallRequest(
                orbiterx_protocol::dynamic_tools::DynamicToolCallRequest {
                    call_id: "dyn-1".into(),
                    turn_id: "turn-1".into(),
                    started_at_ms: 0,
                    namespace: Some("orbiterx_app".into()),
        ...
        let events = vec![
            ...
            EventMsg::PatchApplyEnd(PatchApplyEndEvent {
                ...
                changes: [(
                    ...
                    orbiterx_protocol::protocol::FileChange::Add {
                        content: "hello\n".into(),
                    },
                )]
                .into_iter()
    ...
    fn reconstructs_collab_resume_end_item() {
        let events = vec![
            ...
            EventMsg::CollabResumeEnd(orbiterx_protocol::protocol::CollabResumeEndEvent {
    ...
    fn reconstructs_collab_spawn_end_item_with_model_metadata() {
        ...
        let events = vec![
            ...
            EventMsg::CollabAgentSpawnEnd(orbiterx_protocol::protocol::CollabAgentSpawnEndEvent {
                ...
                reasoning_effort: orbiterx_protocol::openai_models::ReasoningEffort::Medium,
        ...
        assert_eq!(
            ...
            ThreadItem::CollabAgentToolCall {
                ...
                reasoning_effort: Some(orbiterx_protocol::openai_models::ReasoningEffort::Medium),
                agents_states: [(
                    "00000000-0000-0000-0000-000000000002".into(),
                    CollabAgentState {
    ...
    fn reconstructs_interrupted_send_input_as_completed_collab_call() {
        ...
        let events = vec![
            ...
            EventMsg::CollabAgentInteractionBegin(
                orbiterx_protocol::protocol::CollabAgentInteractionBeginEvent {
                    call_id: "send-1".into(),
                    started_at_ms: 0,
            ...
            EventMsg::CollabAgentInteractionEnd(
                orbiterx_protocol::protocol::CollabAgentInteractionEndEvent {
                    call_id: "send-1".into(),
                    completed_at_ms: 0,
                    sender_thread_id: sender,
                    receiver_thread_id: receiver,
    ...
    fn rollback_failed_error_does_not_mark_turn_failed() {
        let events = vec![
            ...
            EventMsg::Error(ErrorEvent {
                ...
                orbiterx_error_info: Some(OrbiterXErrorInfo::ThreadRollbackFailed),
            }),
        ];
    ...
    fn error_then_turn_complete_preserves_failed_status() {
        let events = vec![
            ...
            EventMsg::Error(ErrorEvent {
                ...
                orbiterx_error_info: Some(OrbiterXErrorInfo::ResponseStreamDisconnected {
        ...
        assert_eq!(
            ...
            Some(TurnError {
                message: "stream failure".into(),
                orbiterx_error_info: Some(
                    crate::protocol::v2::OrbiterXErrorInfo::ResponseStreamDisconnected {
                        http_status_code: Some(502),
                    }
                ),
                additional_details: None,
    ...
    fn canonical_hook_prompt_completion_updates_turn_history() {
        let hook_prompt = CoreTurnItem::HookPrompt(orbiterx_protocol::items::HookPromptItem {
    ...
    fn ignores_plain_user_response_items_in_rollout_replay() {
        let items = vec![
            ...
            RolloutItem::ResponseItem(orbiterx_protocol::models::ResponseItem::Message {
                id: Some(orbiterx_protocol::ResponseItemId::with_suffix("msg", "1")),
                role: "user".into(),
