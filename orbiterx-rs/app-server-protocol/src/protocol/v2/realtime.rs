...
use orbiterx_protocol::protocol::RealtimeAudioFrame as CoreRealtimeAudioFrame;
...
pub struct ThreadRealtimeStartParams {
    ...
    /// Leaves OrbiterX response handoffs to the client's explicit append calls instead of forwarding
    /// them automatically. Defaults to false.
    #[ts(optional = nullable)]
    pub client_managed_handoffs: Option<bool>,
    /// Routes any transcript tail remaining at session end through OrbiterX. Defaults to false.
    ...
    #[ts(optional = nullable)]
    pub orbiterx_responses_as_items: Option<bool>,
    // TODO: Remove this experiment-only prefix with `orbiterx_responses_as_items`.
    /// Optional prefix added to automatic OrbiterX response items when `orbiterxResponsesAsItems` is true.
    #[ts(optional = nullable)]
    pub orbiterx_response_item_prefix: Option<String>,
    /// Selects how automatic OrbiterX responses are routed in Frameless Bidi sessions. Omitted values
    /// default to `thinking`. Realtime V1 and V2 ignore this setting.
    #[ts(optional = nullable)]
    pub orbiterx_response_handoff_mode: Option<OrbiterXResponseHandoffMode>,
    ...
    /// Set to false to start without OrbiterX's startup context. Omitted or null includes it.
    #[ts(optional = nullable)]
