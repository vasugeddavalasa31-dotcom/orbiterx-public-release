...
use orbiterx_protocol::config_types::ApprovalsReviewer as CoreApprovalsReviewer;
use orbiterx_protocol::config_types::SandboxMode as CoreSandboxMode;
...
use orbiterx_protocol::protocol::NonSteerableTurnKind as CoreNonSteerableTurnKind;
...
/// This translation layer make sure that we expose orbiterx error code in camel case.
///
/// When an upstream HTTP status is available (for example, from the Responses API or a provider),
/// it is forwarded in `httpStatusCode` on the relevant `orbiterxErrorInfo` variant.
...
impl From<CoreOrbiterXErrorInfo> for OrbiterXErrorInfo {
    fn from(value: CoreOrbiterXErrorInfo) -> Self {
        match value {
            CoreOrbiterXErrorInfo::ContextWindowExceeded => {
                OrbiterXErrorInfo::ContextWindowExceeded
            }
            CoreOrbiterXErrorInfo::SessionBudgetExceeded => {
                OrbiterXErrorInfo::SessionBudgetExceeded
            }
            CoreOrbiterXErrorInfo::UsageLimitExceeded => OrbiterXErrorInfo::UsageLimitExceeded,
            CoreOrbiterXErrorInfo::ServerOverloaded => OrbiterXErrorInfo::ServerOverloaded,
            CoreOrbiterXErrorInfo::CyberPolicy => OrbiterXErrorInfo::CyberPolicy,
            CoreOrbiterXErrorInfo::HttpConnectionFailed { http_status_code } => {
                OrbiterXErrorInfo::HttpConnectionFailed { http_status_code }
            }
            CoreOrbiterXErrorInfo::ResponseStreamConnectionFailed { http_status_code } => {
                OrbiterXErrorInfo::ResponseStreamConnectionFailed { http_status_code }
            }
            CoreOrbiterXErrorInfo::InternalServerError => OrbiterXErrorInfo::InternalServerError,
            CoreOrbiterXErrorInfo::Unauthorized => OrbiterXErrorInfo::Unauthorized,
            CoreOrbiterXErrorInfo::BadRequest => OrbiterXErrorInfo::BadRequest,
            CoreOrbiterXErrorInfo::ThreadRollbackFailed => OrbiterXErrorInfo::ThreadRollbackFailed,
            CoreOrbiterXErrorInfo::SandboxError => OrbiterXErrorInfo::SandboxError,
            CoreOrbiterXErrorInfo::ResponseStreamDisconnected { http_status_code } => {
                OrbiterXErrorInfo::ResponseStreamDisconnected { http_status_code }
            }
            CoreOrbiterXErrorInfo::ResponseTooManyFailedAttempts { http_status_code } => {
                OrbiterXErrorInfo::ResponseTooManyFailedAttempts { http_status_code }
            }
            CoreOrbiterXErrorInfo::ActiveTurnNotSteerable { turn_kind } => {
                OrbiterXErrorInfo::ActiveTurnNotSteerable {
                    turn_kind: turn_kind.into(),
                }
            }
            CoreOrbiterXErrorInfo::Other => OrbiterXErrorInfo::Other,
        }
    }
}
