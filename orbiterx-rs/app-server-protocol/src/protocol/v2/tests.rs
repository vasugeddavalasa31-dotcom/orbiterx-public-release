...
use orbiterx_protocol::approvals::ElicitationRequest as CoreElicitationRequest;
use orbiterx_protocol::config_types::MultiAgentMode;
...
use orbiterx_protocol::items::CollabAgentToolCallStatus as CoreCollabAgentToolCallStatus;
use orbiterx_protocol::items::CommandExecutionItem;
...
use orbiterx_protocol::items::FileChangeItem;
use orbiterx_protocol::items::ImageViewItem;
use orbiterx_protocol::items::McpToolCallItem;
use orbiterx_protocol::items::McpToolCallStatus as CoreMcpToolCallStatus;
use orbiterx_protocol::items::ReasoningItem;
...
use orbiterx_protocol::items::WebSearchItem as CoreWebSearchItem;
use orbiterx_protocol::mcp::CallToolResult;
use orbiterx_protocol::mcp::McpServerInfo;
use orbiterx_protocol::memory_citation::MemoryCitation as CoreMemoryCitation;
use orbiterx_protocol::memory_citation::MemoryCitationEntry as CoreMemoryCitationEntry;
...
use orbiterx_protocol::permissions::FileSystemAccessMode as CoreFileSystemAccessMode;
use orbiterx_protocol::permissions::FileSystemPath as CoreFileSystemPath;
use orbiterx_protocol::permissions::FileSystemSandboxEntry as CoreFileSystemSandboxEntry;
use orbiterx_protocol::permissions::FileSystemSpecialPath as CoreFileSystemSpecialPath;
use orbiterx_protocol::protocol::AgentStatus as CoreAgentStatus;
use orbiterx_protocol::protocol::AskForApproval as CoreAskForApproval;
use orbiterx_protocol::protocol::ConversationTextRole;
use orbiterx_protocol::protocol::ExecCommandSource as CoreExecCommandSource;
use orbiterx_protocol::protocol::GranularApprovalConfig as CoreGranularApprovalConfig;
use orbiterx_protocol::protocol::NetworkAccess as CoreNetworkAccess;
use orbiterx_protocol::protocol::SubAgentActivityKind as CoreSubAgentActivityKind;
use orbiterx_protocol::request_permissions::RequestPermissionProfile as CoreRequestPermissionProfile;
use orbiterx_protocol::user_input::UserInput as CoreUserInput;
...
fn thread_sources_round_trip_as_scalar_labels() {
    for (source, label) in [
        ...
        let core_source: orbiterx_protocol::protocol::ThreadSource = source.clone().into();
...
fn sandbox_policy_round_trips_external_sandbox_network_access() {
    ...
    assert_eq!(
        core_policy,
        orbiterx_protocol::protocol::SandboxPolicy::ExternalSandbox {
...
fn sandbox_policy_round_trips_read_only_network_access() {
    ...
    assert_eq!(
        core_policy,
        orbiterx_protocol::protocol::SandboxPolicy::ReadOnly {
            network_access: true,
        }
    );
...
fn sandbox_policy_round_trips_workspace_write_access() {
    ...
    assert_eq!(
        core_policy,
        orbiterx_protocol::protocol::SandboxPolicy::WorkspaceWrite {
            writable_roots: vec![],
            network_access: true,
            exclude_tmpdir_env_var: false,
            exclude_slash_tmp: false,
...
fn core_turn_item_into_thread_item_converts_supported_variants() {
    let user_item = TurnItem::UserMessage(UserMessageItem {
        ...
        content: vec![
            ...
            CoreUserInput::Skill {
                ...
                path: PathBuf::from("/repo/.orbiterx/skills/skill-creator/SKILL.md"),
            },
    ...
    let command_item = TurnItem::CommandExecution(CommandExecutionItem {
        ...
        parsed_cmd: vec![orbiterx_protocol::parse_command::ParsedCommand::Unknown {
    ...
    let sender_thread_id = orbiterx_protocol::ThreadId::default();
    let receiver_thread_id = orbiterx_protocol::ThreadId::default();
    ...
    let sub_agent_activity_item = TurnItem::SubAgentActivity(SubAgentActivityItem {
        ...
        agent_path: orbiterx_protocol::AgentPath::root()
            .join("worker")
            .expect("worker path"),
    });
    ...
    assert_eq!(
        ThreadItem::from(TurnItem::Extension(
            orbiterx_extension_items::ExtensionItem::WebSearch(expected_search_item.clone()),
        )),
    ...
    let file_change_item = TurnItem::FileChange(FileChangeItem {
        ...
        .into_iter()
        .collect(),
        status: Some(orbiterx_protocol::protocol::PatchApplyStatus::Completed),
        auto_approved: None,
...
fn mcp_tool_call_app_context_serializes_connector_id() {
    let item = ThreadItem::McpToolCall {
        id: "mcp-1".to_string(),
        server: "orbiterx_apps".to_string(),
...
fn plugin_share_params_and_response_serialization_use_camel_case_fields() {
    ...
    assert_eq!(
        serde_json::to_value(PluginShareCheckoutResponse {
            remote_plugin_id: "plugins~Plugin_00000000000000000000000000000000".to_string(),
            plugin_id: "gmail@orbiterx-curated".to_string(),
            plugin_name: "gmail".to_string(),
            plugin_path,
            marketplace_name: "orbiterx-curated".to_string(),
            marketplace_path,
        ...
        json!({
            "remotePluginId": "plugins~Plugin_00000000000000000000000000000000",
            "pluginId": "gmail@orbiterx-curated",
            "pluginName": "gmail",
            "pluginPath": plugin_path_json,
            "marketplaceName": "orbiterx-curated",
            "marketplacePath": marketplace_path_json,
            "remoteVersion": "1.2.3",
        }),
    );
...
#[test]
fn orbiterx_error_info_serializes_http_status_code_in_camel_case() {
    let value = OrbiterXErrorInfo::ResponseTooManyFailedAttempts {
...
#[test]
fn orbiterx_error_info_serializes_cyber_policy_in_camel_case() {
    assert_eq!(
        serde_json::to_value(OrbiterXErrorInfo::CyberPolicy).unwrap(),
        json!("cyberPolicy")
    );
}

#[test]
fn orbiterx_error_info_serializes_active_turn_not_steerable_turn_kind_in_camel_case() {
    let value = OrbiterXErrorInfo::ActiveTurnNotSteerable {
...
fn turn_start_params_round_trip_multi_agent_mode() {
    ...
    assert_eq!(
        ...
        Some(orbiterx_protocol::config_types::MultiAgentMode::Proactive)
    );
    assert_eq!(
...
fn thread_settings_update_params_preserve_field_level_experimental_gates() {
    ...
    let collaboration_mode = ThreadSettingsUpdateParams {
        ...
        collaboration_mode: Some(orbiterx_protocol::config_types::CollaborationMode {
            mode: orbiterx_protocol::config_types::ModeKind::Plan,
