...
pub use event_processor_with_jsonl_output::OrbiterXStatus;
...
use orbiterx_app_server_client::EnvironmentManager;
use orbiterx_app_server_client::ExecServerRuntimePaths;
use orbiterx_app_server_client::InProcessAppServerClient;
use orbiterx_app_server_client::InProcessClientStartArgs;
use orbiterx_app_server_client::InProcessServerEvent;
...
use orbiterx_app_server_protocol::ThreadItem as AppServerThreadItem;
...
use orbiterx_core::StateDbHandle;
use orbiterx_core::check_execpolicy_for_warnings;
use orbiterx_core::config::Config;
use orbiterx_core::config::ConfigBuilder;
use orbiterx_core::config::ConfigOverrides;
use orbiterx_core::config::ConfigTomlLoadResult;
use orbiterx_core::config::find_orbiterx_home;
use orbiterx_core::config::load_config_toml_with_layer_stack;
use orbiterx_core::config::resolve_bootstrap_auth_keyring_backend_kind;
use orbiterx_core::config::resolve_bootstrap_auth_route_config;
use orbiterx_core::config::resolve_oss_provider;
use orbiterx_core::config::resolve_profile_v2_config_path;
use orbiterx_core::find_thread_meta_by_name_str;
use orbiterx_core::format_exec_policy_error_with_source;
...
use orbiterx_login::default_client::set_default_originator;
...
use orbiterx_otel::set_parent_from_context;
use orbiterx_otel::traceparent_context_from_env;
...
enum StdinPromptBehavior {
    ...
    /// `orbiterx exec` behavior for `orbiterx exec` with piped input.
    RequiredIfPiped,
    /// Always treat stdin as the prompt, used for the explicit `orbiterx exec -`
...
struct ExecRunArgs {
    ...
    config: Config,
    resume_approvals_reviewer_override: Option<orbiterx_app_server_protocol::ApprovalsReviewer>,
...
pub async fn run_main(cli: Cli, arg0_paths: Arg0DispatchPaths) -> anyhow::Result<()> {
    ...
    if let Err(err) = set_default_originator("orbiterx_exec".to_string()) {
        tracing::warn!(
            ?err,
            "Failed to set orbiterx exec originator override {err:?}"
        );
    }
    ...
    let user_config_path = config_profile_v2
        .as_ref()
        .map(|profile_v2| resolve_profile_v2_config_path(&orbiterx_home, profile_v2));
    ...
    let otel = match std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        orbiterx_core::otel_init::build_provider(
            &config,
    ...
    orbiterx_core::otel_init::record_process_start(otel.as_ref(), "orbiterx_exec");
    orbiterx_core::otel_init::install_sqlite_telemetry(otel.as_ref(), "orbiterx_exec");
    ...
    let state_db = orbiterx_core::init_state_db(&config).await;
    let environment_manager = if run_loader_overrides.ignore_user_config {
        ...
        EnvironmentManager::from_orbiterx_home(
    ...
    let in_process_start_args = InProcessClientStartArgs {
        ...
        client_name: "orbiterx_exec".to_string(),
...
async fn run_exec_session(args: ExecRunArgs) -> anyhow::Result<()> {
    ...
    let (initial_operation, prompt_summary) = match (command.as_ref(), prompt, images) {
        (Some(ExecCommand::Review(review_cli)), _, _) => {
            ...
            let summary = orbiterx_core::review_prompts::user_facing_hint(&review_request.target);
    ...
    // Print the effective configuration and initial request so users can see what OrbiterX
    // is using.
    event_processor.print_config_summary(&config, &prompt_summary, &session_configured);
    if !json_mode
        && let Some(message) =
            orbiterx_core::config::system_bwrap_warning(config.permissions.permission_profile())
    {
        event_processor.process_warning(message);
    }

    info!("OrbiterX initialized with event: {session_configured:?}");
        ...
        match server_event {
            ...
            InProcessServerEvent::ServerNotification(mut notification) => {
                if let ServerNotification::Error(payload) = &notification {
                    ...
                    && matches!(
                        payload.turn.status,
                        orbiterx_app_server_protocol::TurnStatus::Failed
                            | orbiterx_app_server_protocol::TurnStatus::Interrupted
                    )
                {
                    error_seen = true;
                }
                ...
                if should_process_notification(
                    ...
                    match event_processor.process_server_notification(notification) {
                        OrbiterXStatus::Running => {}
                        OrbiterXStatus::InitiateShutdown => {
...
fn thread_resume_params_from_config(
    config: &Config,
    thread_id: String,
    approvals_reviewer_override: Option<orbiterx_app_server_protocol::ApprovalsReviewer>,
) -> ThreadResumeParams {
    let permissions = permissions_selection_from_config(config);
    let sandbox = permissions.is_none().then(|| {
        sandbox_mode_from_permission_profile(
...
fn session_configured_from_thread_response(
    ...
    thread_source: Option<orbiterx_protocol::protocol::ThreadSource>,
    ...
    permission_profile: PermissionProfile,
    active_permission_profile: Option<orbiterx_protocol::models::ActivePermissionProfile>,
    cwd: AbsolutePathBuf,
...
async fn resolve_resume_thread_id(
    ...
    if let Some(state_db) = state_db {
        ...
        if let Some((_, session_meta)) =
            find_thread_meta_by_name_str(&config.orbiterx_home, session_id, Some(state_db.as_ref()))
                .await?
