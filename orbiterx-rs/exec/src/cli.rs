...
#[command(
    version,
    override_usage = "orbiterx exec [OPTIONS] [PROMPT]\n       orbiterx exec [OPTIONS] <COMMAND> [ARGS]"
)]
pub struct Cli {
    ...
    /// Allow running OrbiterX outside a Git repository.
    ...
    /// Do not load `$ORBITERX_HOME/config.toml`; auth still uses `ORBITERX_HOME`.
...
struct ResumeArgsRaw {
    ...
    // so "orbiterx resume --last <prompt>" treats the positional as a prompt, not a session id.
