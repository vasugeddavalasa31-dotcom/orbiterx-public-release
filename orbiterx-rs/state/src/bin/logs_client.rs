...
use clap::ValueEnum;
use dirs::home_dir;
use orbiterx_state::LogQuery;
use orbiterx_state::LogRow;
use orbiterx_state::StateRuntime;
use owo_colors::OwoColorize;

#[derive(Debug, Parser)]
#[command(name = "orbiterx-state-logs")]
#[command(about = "Tail OrbiterX logs from the dedicated logs SQLite DB with simple filters")]
struct Args {
    /// Path to ORBITERX_HOME. Defaults to $ORBITERX_HOME or ~/.orbiterx.
    #[arg(long, env = "ORBITERX_HOME")]
    orbiterx_home: Option<PathBuf>,

    /// Direct path to the logs SQLite database. Overrides --orbiterx-home.
    #[arg(long)]
    db: Option<PathBuf>,
...
async fn main() -> anyhow::Result<()> {
    ...
    let orbiterx_home = db_path
        .parent()
        .map(ToOwned::to_owned)
        .unwrap_or_else(|| PathBuf::from("."));
    let runtime = StateRuntime::init(orbiterx_home, "logs-client".to_string()).await?;
...
fn resolve_db_path(args: &Args) -> anyhow::Result<PathBuf> {
    ...
    let orbiterx_home = args
        .orbiterx_home
        .clone()
        .unwrap_or_else(default_orbiterx_home);
    Ok(orbiterx_state::logs_db_path(orbiterx_home.as_path()))
}

fn default_orbiterx_home() -> PathBuf {
    if let Some(home) = home_dir() {
        return home.join(".orbiterx");
    }
...
mod tests {
    ...
    fn log_level_rejects_aliases_and_unknown_values() {
        assert!(Args::try_parse_from(["orbiterx-state-logs", "--level", "warning"]).is_err());
        assert!(Args::try_parse_from(["orbiterx-state-logs", "--level", "err"]).is_err());
        assert!(Args::try_parse_from(["orbiterx-state-logs", "--level", "warn,error"]).is_err());
    }

    #[test]
    fn log_level_accepts_canonical_values_case_insensitively() {
        let args = Args::try_parse_from(["orbiterx-state-logs", "--level", "WARN"])
