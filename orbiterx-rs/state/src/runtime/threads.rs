...
ON CONFLICT(id) DO NOTHING
        ...
        .bind(
            metadata
                .thread_source
                .as_ref()
                .map(orbiterx_protocol::protocol::ThreadSource::as_str),
        )
...
mod tests {
    ...
    use orbiterx_protocol::protocol::GitInfo;
    ...
    async fn upsert_thread_keeps_creation_memory_mode_for_existing_rows() {
        let orbiterx_home = unique_temp_dir();
        let runtime = StateRuntime::init(orbiterx_home.clone(), "test-provider".to_string())
            .await
            .expect("state db should initialize");
        let thread_id =
            ThreadId::from_string("00000000-0000-0000-0000-000000000123").expect("valid thread id");
        let mut metadata = test_thread_metadata(&orbiterx_home, thread_id, orbiterx_home.clone());
    ...
    async fn delete_thread_cleans_associated_state() -> Result<()> {
        let orbiterx_home = unique_temp_dir();
        let runtime =
            StateRuntime::init(orbiterx_home.clone(), "test-provider".to_string()).await?;
    ...
    async fn list_threads_updated_after_returns_oldest_changes_first() {
        ...
        for (thread_id, updated_at) in [
            ...
            let mut metadata =
                test_thread_metadata(&orbiterx_home, thread_id, orbiterx_home.clone());
            metadata.updated_at = updated_at;
    ...
    async fn list_threads_filters_by_cwd() {
        ...
        let first_cwd = orbiterx_home.join("first");
        let second_cwd = orbiterx_home.join("second");
        let other_cwd = orbiterx_home.join("other");
        ...
        for (thread_id, cwd, updated_at) in [
            ...
            let mut metadata = test_thread_metadata(&orbiterx_home, thread_id, cwd);
            metadata.updated_at =
    ...
    async fn list_threads_uses_indexes_matching_cwd_filters() {
        let orbiterx_home = unique_temp_dir();
        let runtime = StateRuntime::init(orbiterx_home, "test-provider".to_string())
            .await
            .expect("state db should initialize");
    ...
    async fn apply_rollout_items_restores_memory_mode_from_session_meta() {
        ...
        let metadata = test_thread_metadata(&orbiterx_home, thread_id, orbiterx_home.clone());
    ...
    async fn apply_rollout_items_preserves_existing_git_branch_and_fills_missing_git_fields() {
        ...
        let items = vec![RolloutItem::SessionMeta(SessionMetaLine {
            ...
            git: Some(GitInfo {
                commit_hash: Some(orbiterx_git_utils::GitSha::new("rollout-sha")),
                branch: Some("rollout-branch".to_string()),
                repository_url: Some("git@example.com:openai/orbiterx.git".to_string()),
            }),
        })];
        ...
        assert_eq!(
            ...
            Some("git@example.com:openai/orbiterx.git")
        );
    }
    ...
    async fn upsert_thread_preserves_existing_git_fields_atomically() {
        ...
        metadata.git_branch = Some("sqlite-branch".to_string());
        metadata.git_origin_url = Some("git@example.com:openai/orbiterx.git".to_string());
    ...
    async fn update_thread_git_info_preserves_newer_non_git_metadata() {
        ...
        let updated = runtime
            .update_thread_git_info(
                ...
                Some(Some("git@example.com:openai/orbiterx.git")),
            )
            .await
    ...
    async fn insert_thread_if_absent_preserves_existing_metadata() {
        ...
        let mut existing = test_thread_metadata(&orbiterx_home, thread_id, orbiterx_home.clone());
        ...
        let mut fallback = test_thread_metadata(&orbiterx_home, thread_id, orbiterx_home.clone());
    ...
    async fn thread_updated_at_uses_unique_epoch_millis_and_reads_legacy_seconds() {
        ...
        let mut first = test_thread_metadata(&orbiterx_home, first_id, orbiterx_home.clone());
        first.updated_at = updated_at;
        first.recency_at = updated_at;
        let mut second = test_thread_metadata(&orbiterx_home, second_id, orbiterx_home.clone());
        ...
        let mut older = test_thread_metadata(&orbiterx_home, older_id, orbiterx_home.clone());
    ...
    async fn apply_rollout_items_uses_override_updated_at_when_provided() {
        ...
        let items = vec![RolloutItem::EventMsg(EventMsg::TokenCount(
            orbiterx_protocol::protocol::TokenCountEvent {
                info: Some(orbiterx_protocol::protocol::TokenUsageInfo {
                    total_token_usage: orbiterx_protocol::protocol::TokenUsage {
                        input_tokens: 0,
                    ...
                    last_token_usage: orbiterx_protocol::protocol::TokenUsage::default(),
