//! Backup-and-rebuild support for OrbiterX runtime SQLite databases.
//!
//! OrbiterX keeps several independent runtime SQLite databases under one SQLite
...
/// Move one OrbiterX runtime SQLite database out of the way so that database can
    ...
    match tokio::fs::metadata(sqlite_home).await {
        ...
        Err(err) if err.kind() == std::io::ErrorKind::NotFound => {
            ...
            Err(std::io::Error::other(format!(
                "no OrbiterX runtime database files were found to back up for {}",
                db_path.display()
            )))
        }
        Err(err) => Err(err),
...
async fn backup_sqlite_paths(
    ...
    if backups.is_empty() {
        ...
        return Err(std::io::Error::other(
            "no OrbiterX runtime database files were found to back up",
        ));
    }
