...
//! orchestration and rollout scanning live in `orbiterx-core`.
...
pub const SQLITE_HOME_ENV: &str = "ORBITERX_SQLITE_HOME";
...
pub const DB_ERROR_METRIC: &str = "orbiterx.db.error";
/// Metrics on backfill process. Tags: [status]
pub const DB_METRIC_BACKFILL: &str = "orbiterx.db.backfill";
/// Metrics on backfill duration. Tags: [status]
pub const DB_METRIC_BACKFILL_DURATION_MS: &str = "orbiterx.db.backfill.duration_ms";
/// SQLite initialization attempts. Tags: [status, phase, db, error]
pub const DB_INIT_METRIC: &str = "orbiterx.sqlite.init.count";
/// SQLite initialization latency. Tags: [status, phase, db, error]
pub const DB_INIT_DURATION_METRIC: &str = "orbiterx.sqlite.init.duration_ms";
/// Rollout fallback attempts. Tags: [caller, reason]
pub const DB_FALLBACK_METRIC: &str = "orbiterx.sqlite.fallback.count";
