...
impl RuntimeDbSpec {
    fn path(self, orbiterx_home: &Path) -> PathBuf {
        orbiterx_home.join(self.filename)
    }
}
...
impl StateRuntime {
    /// Initialize the state runtime using the provided OrbiterX home and default provider.
    ///
    /// This opens (and migrates) the SQLite databases under `orbiterx_home`.
    ...
    pub(crate) async fn init_with_telemetry_for_tests(
        ...
        Self::init_inner(orbiterx_home, default_provider, Some(telemetry_override)).await
    }
    ...
    async fn init_inner(
        ...
        let sqlite =
            SqliteConfig::from_sqlite_home(AbsolutePathBuf::try_from(orbiterx_home.clone())?);
        ...
        let state_path = STATE_DB.path(orbiterx_home.as_path());
        let logs_path = LOGS_DB.path(orbiterx_home.as_path());
        let goals_path = GOALS_DB.path(orbiterx_home.as_path());
        let memories_path = MEMORIES_DB.path(orbiterx_home.as_path());
    ...
    /// Return the configured OrbiterX home directory for this runtime.
...
pub fn state_db_path(orbiterx_home: &Path) -> PathBuf {
    STATE_DB.path(orbiterx_home)
}
...
pub fn logs_db_path(orbiterx_home: &Path) -> PathBuf {
    LOGS_DB.path(orbiterx_home)
}
...
pub fn goals_db_path(orbiterx_home: &Path) -> PathBuf {
    GOALS_DB.path(orbiterx_home)
}
...
pub fn memories_db_path(orbiterx_home: &Path) -> PathBuf {
    MEMORIES_DB.path(orbiterx_home)
}
...
pub fn thread_history_db_path(orbiterx_home: &Path) -> PathBuf {
    THREAD_HISTORY_DB.path(orbiterx_home)
}

pub fn runtime_db_paths(orbiterx_home: &Path) -> Vec<RuntimeDbPath> {
    RUNTIME_DBS
        .iter()
        .map(|spec| RuntimeDbPath {
            label: spec.label,
            path: spec.path(orbiterx_home),
        })
        .collect()
}
...
mod tests {
    ...
    async fn sqlite_integrity_check_reports_ok_for_valid_db() {
        ...
        let path = state_db_path(orbiterx_home.as_path());
        let pool = crate::SqliteConfig::new_for_testing(orbiterx_home.clone())
            .open_read_write_pool(&path)
            .await
    ...
    async fn open_state_sqlite_tolerates_newer_applied_migrations() {
        ...
        tokio::fs::create_dir_all(&orbiterx_home)
            .await
            .expect("create orbiterx home");
        let state_path = state_db_path(orbiterx_home.as_path());
        let pool = crate::SqliteConfig::new_for_testing(orbiterx_home.clone())
        ...
        let tolerant_pool = open_state_sqlite(
            &crate::SqliteConfig::new_for_testing(orbiterx_home.clone()),
            state_path.as_path(),
            &tolerant_migrator,
            /*telemetry_override*/ None,
        )
