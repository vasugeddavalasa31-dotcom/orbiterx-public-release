...
mod tests {
    ...
    fn parser_process_rejects_stop_parsing_forms() {
        ...
        let parsed = parser
            .parse("git log --% HEAD --output=orbiterx_poc.txt")
            .unwrap();
    ...
    fn parser_process_rejects_named_blocks() {
        ...
        let parsed = parser
            .parse("begin { Set-Content orbiterx_poc.txt pwned } end { Get-Content Cargo.toml }")
            .unwrap();
        assert_eq!(parsed, PowershellParseOutcome::Unsupported);
    }
    ...
    fn parser_process_rejects_using_statements() {
        ...
        let parsed = parser
            .parse("using module ./orbiterx_poc.psm1\nGet-Content Cargo.toml")
            .unwrap();
        assert_eq!(parsed, PowershellParseOutcome::Unsupported);
    }
