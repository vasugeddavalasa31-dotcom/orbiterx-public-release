...
const ORBITERX_AUTH_SECRETS_FILENAME: &str = "orbiterx_auth.age";
...
pub enum LocalSecretsNamespace {
    ...
    #[default]
    ManagedSecrets,
    /// OrbiterX authentication credentials used by the CLI, TUI, app server, and other clients.
    OrbiterXAuth,
...
impl LocalSecretsBackend {
    pub fn new(orbiterx_home: PathBuf, keyring_store: Arc<dyn KeyringStore>) -> Self {
    ...
    fn load_or_create_passphrase(&self) -> Result<SecretString> {
        let account = compute_keyring_account(&self.orbiterx_home);
        let loaded = self
            .keyring_store
...
mod tests {
    ...
    fn load_file_rejects_newer_schema_versions() -> Result<()> {
        ...
        let backend = LocalSecretsBackend::new(orbiterx_home.path().to_path_buf(), keyring);
    ...
    fn set_fails_when_keyring_is_unavailable() -> Result<()> {
        let orbiterx_home = tempfile::tempdir().expect("tempdir");
        let keyring = Arc::new(MockKeyringStore::default());
        let account = compute_keyring_account(orbiterx_home.path());
        keyring.set_error(
            &account,
    ...
    fn local_namespaces_write_separate_files() -> Result<()> {
        let orbiterx_home = tempfile::tempdir().expect("tempdir");
        let keyring = Arc::new(MockKeyringStore::default());
        let orbiterx_auth_backend = LocalSecretsBackend::new_with_namespace(
        ...
        orbiterx_auth_backend.set(&scope, &name, "orbiterx-auth-value")?;
        mcp_backend.set(&scope, &name, "mcp-value")?;

        assert_eq!(
            orbiterx_auth_backend.get(&scope, &name)?,
            Some("orbiterx-auth-value".to_string())
        );
        assert_eq!(
