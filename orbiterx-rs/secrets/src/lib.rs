...
const KEYRING_SERVICE: &str = "orbiterx";
...
impl SecretsManager {
    pub fn new(orbiterx_home: PathBuf, backend_kind: SecretsBackendKind) -> Self {
        let backend: Arc<dyn SecretsBackend> = match backend_kind {
            SecretsBackendKind::Local => {
                let keyring_store: Arc<dyn KeyringStore> = Arc::new(DefaultKeyringStore);
                Arc::new(LocalSecretsBackend::new(orbiterx_home, keyring_store))
            }
        };
        Self { backend }
    }
...
pub fn compute_keyring_account(orbiterx_home: &Path) -> String {
