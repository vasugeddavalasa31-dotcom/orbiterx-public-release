# Bazel in orbiterx-rs

This repository uses Bazel to build the Rust workspace under `orbiterx-rs`.
...
- `rules_rs` imports third-party crates from `orbiterx-rs/Cargo.toml` and
  `orbiterx-rs/Cargo.lock` via `crate.from_cargo(...)` and exposes them under
  `@crates`.
- `../defs.bzl` provides `orbiterx_rust_crate`, which wraps `rust_library`,
...
- Each crate in `orbiterx-rs/*/BUILD.bazel` typically uses `orbiterx_rust_crate` and
...
OrbiterX uses BuildBuddy for a shared Bazel cache and remoted builds and tests. To use it
...
| --- | --- | --- |
| Push to `main` in `openai/orbiterx` | Yes | Yes |
| `workflow_dispatch` in `openai/orbiterx` | Yes | Yes |
| Same-repository pull request in `openai/orbiterx` | Yes | Yes |
| Fork pull request into `openai/orbiterx` | No | No; local |
...
```bash
BUILDBUDDY_API_KEY=... GITHUB_REPOSITORY=my-fork/orbiterx \
...
run in `openai/orbiterx`. A missing or malformed pull request event
...
2. Create a `BUILD.bazel` that calls `orbiterx_rust_crate` (see nearby crates for
   examples).
3. If a dependency needs special handling (compile/runtime data, additional binaries
   ...
   `orbiterx_rust_crate` to configure it.
