...
#[test]
fn orbiterx_hooks_is_legacy_alias_for_hooks() {
    assert_eq!(feature_for_key("hooks"), Some(Feature::OrbiterXHooks));
    assert_eq!(
        feature_for_key("orbiterx_hooks"),
        Some(Feature::OrbiterXHooks)
    );
}
