...
const MEMO_URI: &str = "memo://orbiterx/example-note";
const MEMO_CONTENT: &str = "This is a sample MCP resource served by the rmcp test server.";
const SANDBOX_STATE_META_CAPABILITY: &str = "orbiterx/sandbox-state-meta";
...
impl TestToolServer {
    ...
    /// Tool intended for manual testing of OrbiterX TUI rendering for MCP image tool results.
    ///
    ...
    ///
    /// Manual testing approach (OrbiterX TUI):
    /// - Build this binary: `cargo build -p orbiterx-rmcp-client --bin test_stdio_server`
    /// - Register it:
    ///   - `orbiterx mcp add mcpimg -- /abs/path/to/test_stdio_server`
    /// - Then in OrbiterX TUI, ask it to call:
    ...
    fn memo_template() -> ResourceTemplate {
        let raw = RawResourceTemplate {
            uri_template: "memo://orbiterx/{slug}".to_string(),
            name: "orbiterx-memo".to_string(),
            title: Some("OrbiterX Memo".to_string()),
            description: Some(
                "Template for memo://orbiterx/{slug} resources used in tests.".to_string(),
            ),
            mime_type: Some("text/plain".to_string()),
            icons: None,
        };
...
/// Scenarios for `image_scenario`, intended to exercise OrbiterX TUI handling of MCP image outputs.
///
    ...
    fn get_info(&self) -> ServerInfo {
        ...
        match dynamic_server_process_label() {
            Some(process_label) => server_info
                .with_server_info(
                    Implementation::new("orbiterx-rmcp-test-server", env!("CARGO_PKG_VERSION"))
    ...
    async fn call_tool(
        ...
        match request.name.as_ref() {
            ...
            "encrypted_output" => {
                ...
                meta.insert("orbiterx/encryptedContent".to_string(), json!(true));
    ...
    fn image_scenario_result(args: ImageScenarioArgs) -> Result<CallToolResult, McpError> {
        ...
        match args.scenario {
            ...
            ImageScenario::ImageOnlyOriginalDetail => {
                ...
                meta.insert(
                    "orbiterx/imageDetail".to_string(),
