...
const LOCK_CONTENTION_EVENT_TARGET: &str = "orbiterx_rmcp_client::oauth::store_lock::contention";
...
impl OAuthStoreLock {
    pub(super) fn acquire(store: OAuthStore) -> Result<Self, OAuthStoreLockFailure> {
        ...
        // authority. Those stores are ORBITERX_HOME-backed today: if ORBITERX_HOME is unset they use
        // the default home (`~/.orbiterx`), and if an embedder has no local home/filesystem authority
        ...
        let orbiterx_home = find_orbiterx_home()
            .map_err(|source| OAuthStoreLockFailure::OrbiterXHome { store, source })?;
        Self::acquire_in(&orbiterx_home, store, STORE_LOCK_ACQUIRE_TIMEOUT)
    }
    ...
    pub(super) fn acquire_in(
        ...
        let path = oauth_store_lock_path(orbiterx_home, store);
...
pub(super) enum OAuthStoreLockFailure {
    #[error("failed to resolve ORBITERX_HOME for MCP OAuth {store} aggregate-store lock")]
    OrbiterXHome {
        store: OAuthStore,
        #[source]
        source: io::Error,
...
fn oauth_store_lock_path(orbiterx_home: &Path, store: OAuthStore) -> PathBuf {
    orbiterx_home
        .join(OAUTH_LOCK_DIR)
