...
fn send_builds_payload_with_tags_and_histograms() -> Result<()> {
    let (metrics, exporter) =
        build_metrics_with_defaults(&[("service", "orbiterx-cli"), ("env", "prod")])?;

    metrics.counter_with_description(
        "orbiterx.turns",
        "Total number of OrbiterX turns.",
        /*inc*/ 1,
        &[("model", "gpt-5.1"), ("env", "dev")],
    )?;
    metrics.histogram(
        "orbiterx.tool_latency",
        /*value*/ 25,
        &[("tool", "shell")],
    )?;
    metrics.gauge_with_description(
        "orbiterx.active",
        "Number of active OrbiterX operations.",
        /*value*/ 2,
        &[("component", "test")],
    )?;
    metrics.shutdown()?;
    ...
    let counter = find_metric(&resource_metrics, "orbiterx.turns").expect("counter metric missing");
    assert_eq!(counter.description(), "Total number of OrbiterX turns.");
    ...
    let expected_counter_attributes = BTreeMap::from([
        ("service".to_string(), "orbiterx-cli".to_string()),
    ...
    let (bounds, bucket_counts, sum, count) =
        histogram_data(&resource_metrics, "orbiterx.tool_latency");
    ...
    let histogram_attrs = attributes_to_map(
        find_metric(&resource_metrics, "orbiterx.tool_latency")
            ...
            .expect("orbiterx.tool_latency histogram attributes should exist"),
    );
    ...
    let gauge = find_metric(&resource_metrics, "orbiterx.active").expect("gauge metric missing");
    assert_eq!(gauge.description(), "Number of active OrbiterX operations.");
...
fn send_merges_default_tags_per_line() -> Result<()> {
    ...
    let resource_metrics = latest_metrics(&exporter);
    let alpha_metric =
        find_metric(&resource_metrics, "orbiterx.alpha").expect("orbiterx.alpha metric missing");
    ...
    let beta_metric =
        find_metric(&resource_metrics, "orbiterx.beta").expect("orbiterx.beta metric missing");
...
fn client_sends_enqueued_metric() -> Result<()> {
    ...
    metrics.counter("orbiterx.turns", /*inc*/ 1, &[("model", "gpt-5.1")])?;
    metrics.shutdown()?;
...
fn shutdown_flushes_in_memory_exporter() -> Result<()> {
    ...
    metrics.counter("orbiterx.turns", /*inc*/ 1, &[])?;
    metrics.shutdown()?;
