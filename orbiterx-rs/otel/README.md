...
`orbiterx-otel` is the OpenTelemetry integration crate for OrbiterX. It provides:

- Provider wiring for log/trace/metric exporters (`orbiterx_otel::OtelProvider`
  and `orbiterx_otel::provider`).
- Session-scoped business event emission via `orbiterx_otel::SessionTelemetry`.
- Low-level metrics APIs via `orbiterx_otel::metrics`.
- Trace-context helpers via `orbiterx_otel::trace_context` and crate-root re-exports.
...
```rust
use orbiterx_otel::config::OtelExporter;
use orbiterx_otel::config::OtelHttpProtocol;
use orbiterx_otel::config::OtelSettings;
use orbiterx_otel::OtelProvider;
use tracing_subscriber::prelude::*;

let settings = OtelSettings {
    environment: "dev".to_string(),
    service_name: "orbiterx-cli".to_string(),
    service_version: env!("CARGO_PKG_VERSION").to_string(),
    orbiterx_home: std::path::PathBuf::from("/tmp"),
...
that member. If propagated trace context already has the named member, OrbiterX
...
OrbiterX-specific session events. Rich session/business events should go through
...
`orbiterx-otel` also provides `OtelExporter::Statsig`, a shorthand for exporting OTLP/HTTP JSON metrics
to Statsig using OrbiterX-internal defaults.

Statsig ingestion (OTLP/HTTP JSON) example:

```rust
use orbiterx_otel::config::{OtelExporter, OtelHttpProtocol};
...
metrics.counter("orbiterx.session_started", 1, &[("source", "tui")])?;
metrics.histogram("orbiterx.request_latency", 83, &[("route", "chat")])?;
```
...
metrics.counter("orbiterx.turns", 1, &[("model", "gpt-5.1")])?;
