...
use orbiterx_login::default_client::USER_AGENT_SUFFIX;
...
use crate::orbiterx_tool_config::OrbiterXToolCallParam;
use crate::orbiterx_tool_config::OrbiterXToolCallReplyParam;
use crate::orbiterx_tool_config::create_tool_for_orbiterx_tool_call_param;
use crate::orbiterx_tool_config::create_tool_for_orbiterx_tool_call_reply_param;
...
pub(crate) struct MessageProcessor {
    ...
    running_requests_id_to_orbiterx_uuid: Arc<Mutex<HashMap<RequestId, ThreadId>>>,
}
...
impl MessageProcessor {
    ...
    pub(crate) async fn new(
        ...
        orbiterx_image_generation_extension::install(
            &mut extensions,
            auth_manager.clone(),
            |config: &Config| Some(config.orbiterx_home.clone()),
        );
        let thread_manager = Arc::new(ThreadManager::new(
            config.as_ref(),
            Arc::clone(&auth_manager),
            orbiterx_core::build_models_manager(config.as_ref(), auth_manager),
            ...
            orbiterx_core::thread_store_from_config(config.as_ref(), state_db.clone()),
        ...
        Self {
            ...
            arg0_paths,
            thread_manager,
            running_requests_id_to_orbiterx_uuid: Arc::new(Mutex::new(HashMap::new())),
        }
    }
    ...
    async fn handle_initialize(
        ...
        let server_info = Implementation::new("orbiterx-mcp-server", env!("CARGO_PKG_VERSION"))
            .with_title("OrbiterX");

        // Preserve OrbiterX's existing non-spec `serverInfo.user_agent` field.
        ...
        if let serde_json::Value::Object(ref mut obj) = server_info_value {
            obj.insert("user_agent".to_string(), json!(get_orbiterx_user_agent()));
        }
    ...
    async fn handle_list_tools(
        ...
        let result = rmcp::model::ListToolsResult {
            meta: None,
            tools: vec![
                create_tool_for_orbiterx_tool_call_param(),
                create_tool_for_orbiterx_tool_call_reply_param(),
            ],
            next_cursor: None,
        };
    ...
    async fn handle_call_tool(&self, id: RequestId, params: CallToolRequestParams) {
        ...
        match name.as_ref() {
            "orbiterx" => self.handle_tool_call_orbiterx(id, arguments).await,
            "orbiterx-reply" => {
                self.handle_tool_call_orbiterx_session_reply(id, arguments)
                    .await
            }
            _ => {
    ...
    async fn handle_tool_call_orbiterx(
        &self,
        id: RequestId,
        ...
        let (initial_prompt, config): (String, Config) = match arguments {
            Some(json_val) => match serde_json::from_value::<OrbiterXToolCallParam>(json_val) {
                Ok(tool_cfg) => match tool_cfg.into_config(self.arg0_paths.clone()).await {
                    ...
                    Err(e) => {
                        let result = CallToolResult::error(vec![rmcp::model::Content::text(
                            format!("Failed to load OrbiterX configuration from overrides: {e}"),
                        )]);
                        self.outgoing.send_response(id, result).await;
                        return;
                    }
                ...
                Err(e) => {
                    let result = CallToolResult::error(vec![rmcp::model::Content::text(format!(
                        "Failed to parse configuration for OrbiterX tool: {e}"
                    ))]);
                    self.outgoing.send_response(id, result).await;
                    return;
                }
            ...
            None => {
                let result = CallToolResult::error(vec![rmcp::model::Content::text(
                    "Missing arguments for orbiterx tool-call; the `prompt` field is required.",
                )]);
                self.outgoing.send_response(id, result).await;
                return;
            }
        ...
        let running_requests_id_to_orbiterx_uuid =
            self.running_requests_id_to_orbiterx_uuid.clone();

        // Spawn an async task to handle the OrbiterX session so that we do not
        // block the synchronous message-processing loop.
        task::spawn(async move {
            // Run the OrbiterX session and stream events back to the client.
            crate::orbiterx_tool_runner::run_orbiterx_tool_session(
                id,
                initial_prompt,
                config,
                outgoing,
                thread_manager,
                running_requests_id_to_orbiterx_uuid,
            )
            .await;
        });
    }

    async fn handle_tool_call_orbiterx_session_reply(
        &self,
        ...
        // parse arguments
        let orbiterx_tool_call_reply_param: OrbiterXToolCallReplyParam = match arguments {
            Some(json_val) => {
                match serde_json::from_value::<OrbiterXToolCallReplyParam>(json_val) {
                    Ok(params) => params,
                    Err(e) => {
                        tracing::error!("Failed to parse OrbiterX tool call reply parameters: {e}");
                        let result = CallToolResult::error(vec![rmcp::model::Content::text(
                            format!("Failed to parse configuration for OrbiterX tool: {e}"),
                        )]);
            ...
            None => {
                tracing::error!(
                    "Missing arguments for orbiterx-reply tool-call; the `thread_id` and `prompt` fields are required."
                );
                let result = CallToolResult::error(vec![rmcp::model::Content::text(
                    "Missing arguments for orbiterx-reply tool-call; the `thread_id` and `prompt` fields are required.",
                )]);
        ...
        let thread_id = match orbiterx_tool_call_reply_param.get_thread_id() {
            Ok(id) => id,
            Err(e) => {
        ...
        let orbiterx = match self.thread_manager.get_thread(thread_id).await {
            Ok(c) => c,
            Err(_) => {
                tracing::warn!("Session not found for thread_id: {thread_id}");
                let result = crate::orbiterx_tool_runner::create_call_tool_result_with_thread_id(
                    thread_id,
        ...
        let prompt = orbiterx_tool_call_reply_param.prompt.clone();
        tokio::spawn({
            let outgoing = outgoing.clone();
            let running_requests_id_to_orbiterx_uuid = running_requests_id_to_orbiterx_uuid.clone();

            async move {
                crate::orbiterx_tool_runner::run_orbiterx_tool_session_reply(
                    thread_id,
                    orbiterx,
                    outgoing,
                    request_id,
    ...
    async fn handle_cancelled_notification(&self, params: rmcp::model::CancelledNotificationParam) {
        ...
        let thread_id = {
            let map_guard = self.running_requests_id_to_orbiterx_uuid.lock().await;
        ...
        // Obtain the OrbiterX thread from the server.
        let orbiterx_arc = match self.thread_manager.get_thread(thread_id).await {
            Ok(c) => c,
            Err(_) => {
                tracing::warn!("Session not found for thread_id: {thread_id}");
                return;
        ...
        // Submit interrupt to OrbiterX.
        if let Err(e) = orbiterx_arc
            .submit_with_id(Submission {
                id: request_id_string,
                op: orbiterx_protocol::protocol::Op::Interrupt,
                client_user_message_id: None,
                trace: None,
            })
            .await
        {
            tracing::error!("Failed to submit interrupt to OrbiterX: {e}");
            return;
        }
        // unregister the id so we don't keep it in the map
        self.running_requests_id_to_orbiterx_uuid
            .lock()
            .await
            .remove(&request_id);
    }
