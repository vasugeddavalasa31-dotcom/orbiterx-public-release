//! Asynchronous worker that executes a **OrbiterX** tool-call inside a spawned
...
use orbiterx_core::config::Config as OrbiterXConfig;
...
/// To adhere to MCP `tools/call` response format, include the OrbiterX
...
/// Run a complete OrbiterX session and stream events back to the client.
///
/// On completion (success or error) the function sends the appropriate
/// `tools/call` response so the LLM can continue the conversation.
pub async fn run_orbiterx_tool_session(
    id: RequestId,
    initial_prompt: String,
    config: OrbiterXConfig,
    ...
    let NewThread {
        ...
        Err(e) => {
            let result = CallToolResult::error(vec![Content::text(format!(
                "Failed to start OrbiterX session: {e}"
            ))]);
    ...
    // Use the original MCP request ID as the `sub_id` for the OrbiterX submission so that
    ...
    run_orbiterx_tool_session_inner(
        thread_id,
        thread,
        outgoing,
        id,
...
pub async fn run_orbiterx_tool_session_reply(
    thread_id: ThreadId,
...
async fn run_orbiterx_tool_session_inner(
    thread_id: ThreadId,
    thread: Arc<OrbiterXThread>,
    outgoing: Arc<OutgoingMessageSender>,
    request_id: RequestId,
    ...
    loop {
        match thread.next_event().await {
            Ok(event) => {
                ...
                match event.msg {
                    ...
                    | EventMsg::DeprecationNotice(_) => {
                        ...
                        // events. Note that
                        // send(orbiterx_event_to_notification(&event)) above has
            ...
            Err(e) => {
                let result = create_call_tool_result_with_thread_id(
                    thread_id,
                    format!("OrbiterX runtime error: {e}"),
                    Some(true),
                );
                outgoing.send_response(request_id.clone(), result).await;
                break;
