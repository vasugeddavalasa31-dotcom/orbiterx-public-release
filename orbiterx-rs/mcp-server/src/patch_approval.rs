...
pub struct PatchApprovalElicitRequestParams {
    ...
    pub orbiterx_reason: Option<String>,
    #[serde(skip_serializing_if = "Option::is_none")]
    pub orbiterx_grant_root: Option<PathBuf>,
    pub orbiterx_changes: HashMap<PathBuf, FileChange>,
}
...
pub(crate) async fn handle_patch_approval_request(
    ...
    // Listen for the response on a separate task so we don't block the main agent loop.
    {
        ...
        tokio::spawn(async move {
            on_patch_approval_response(approval_id, on_response, orbiterx).await;
        });
    }
}
...
pub(crate) async fn on_patch_approval_response(
    ...
    let value = match response {
        ...
        Err(err) => {
            ...
            if let Err(submit_err) = orbiterx
