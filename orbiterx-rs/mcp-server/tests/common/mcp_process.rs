...
impl McpProcess {
    pub async fn new(orbiterx_home: &Path) -> anyhow::Result<Self> {
        Self::new_with_env(orbiterx_home, &[]).await
    }
    ...
    pub async fn new_with_env(
        ...
        let program = orbiterx_utils_cargo_bin::cargo_bin("orbiterx-mcp-server")
            .context("should find binary for orbiterx-mcp-server")?;
    ...
    pub async fn initialize(&mut self) -> anyhow::Result<()> {
        ...
        let originator = orbiterx_login::default_client::originator().value;
    ...
    pub async fn send_orbiterx_tool_call(
        &mut self,
        params: OrbiterXToolCallParam,
    ) -> anyhow::Result<i64> {
        let orbiterx_tool_call_params = CallToolRequestParams::new("orbiterx").with_arguments(
        ...
        self.send_request(
            "tools/call",
            Some(serde_json::to_value(orbiterx_tool_call_params)?),
        )
        .await
    }
    ...
    /// Method "orbiterx/event" with params.msg.type == "task_complete".
        ...
        loop {
            ...
            match message {
                JsonRpcMessage::Notification(notification) => {
                    let is_match = if notification.notification.method == "orbiterx/event" {
...
impl Drop for McpProcess {
    fn drop(&mut self) {
        // These tests spawn a `orbiterx-mcp-server` child process.
        //
