...
use orbiterx_core::spawn::ORBITERX_SANDBOX_NETWORK_DISABLED_ENV_VAR;
use orbiterx_mcp_server::ExecApprovalElicitRequestParams;
use orbiterx_mcp_server::ExecApprovalResponse;
use orbiterx_mcp_server::OrbiterXToolCallParam;
use orbiterx_mcp_server::PatchApprovalElicitRequestParams;
use orbiterx_mcp_server::PatchApprovalResponse;
use orbiterx_protocol::protocol::FileChange;
use orbiterx_protocol::protocol::ReviewDecision;
use orbiterx_shell_command::parse_command;
...
async fn test_shell_command_approval_triggers_elicitation() {
    if env::var(ORBITERX_SANDBOX_NETWORK_DISABLED_ENV_VAR).is_ok() {
        println!(
            "Skipping test because it cannot execute when network is disabled in a OrbiterX sandbox."
        );
        return;
    }
...
async fn shell_command_approval_triggers_elicitation() -> anyhow::Result<()> {
    ...
    // Send a "orbiterx" tool request, which should hit the responses endpoint.
    // In turn, it should reply with a tool call, which the MCP should forward
    // as an elicitation.
    let orbiterx_request_id = mcp_process
        .send_orbiterx_tool_call(OrbiterXToolCallParam {
    ...
    assert_eq!(
        ...
        Some(create_expected_elicitation_request_params(
            ...
            orbiterx_request_id.to_string(),
            params.orbiterx_event_id.clone(),
            params.thread_id,
        )?)
    );
    ...
    // Verify the original `orbiterx` tool call completes and that the file was created.
    let orbiterx_response = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp_process.read_stream_until_response_message(RequestId::Number(orbiterx_request_id)),
    )
    .await??;
    assert_eq!(
        JsonRpcResponse {
            jsonrpc: JsonRpcVersion2_0,
            id: RequestId::Number(orbiterx_request_id),
            result: json!({
                "content": [
                    {
                        "text": "File created!",
...
fn create_expected_elicitation_request_params(
    command: Vec<String>,
    workdir: &Path,
    orbiterx_mcp_tool_call_id: String,
    orbiterx_event_id: String,
    thread_id: orbiterx_protocol::ThreadId,
) -> anyhow::Result<serde_json::Value> {
    let expected_message = format!(
        "Allow OrbiterX to run `{}` in `{}`?",
        shlex::try_join(command.iter().map(std::convert::AsRef::as_ref))?,
        workdir.to_string_lossy()
    );
    let orbiterx_parsed_cmd = parse_command::parse_command(&command);
    let params_json = serde_json::to_value(ExecApprovalElicitRequestParams {
        ...
        orbiterx_mcp_tool_call_id,
        orbiterx_event_id,
        orbiterx_command: command,
        orbiterx_cwd: workdir.to_path_buf(),
        orbiterx_call_id: "call1234".to_string(),
        orbiterx_parsed_cmd,
    })?;
    Ok(params_json)
}
...
async fn patch_approval_triggers_elicitation() -> anyhow::Result<()> {
    ...
    // Send a "orbiterx" tool request that will trigger the apply_patch command
    let orbiterx_request_id = mcp_process
        .send_orbiterx_tool_call(OrbiterXToolCallParam {
    ...
    // Verify the original `orbiterx` tool call completes
    let orbiterx_response = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp_process.read_stream_until_response_message(RequestId::Number(orbiterx_request_id)),
    )
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_orbiterx_tool_passes_base_instructions() {
    skip_if_no_network!();

    // Apparently `#[tokio::test]` must return `()`, so we create a helper
    // function that returns `Result` so we can use `?` in favor of `unwrap`.
    orbiterx_tool_passes_base_instructions()
        .await
        .expect("orbiterx tool should pass base instructions");
}

async fn orbiterx_tool_passes_base_instructions() -> anyhow::Result<()> {
    ...
    // Run `orbiterx mcp` with a specific config.toml.
    let orbiterx_home = TempDir::new()?;
    create_config_toml(orbiterx_home.path(), &server.uri())?;
    let mut mcp_process = McpProcess::new(orbiterx_home.path()).await?;
    ...
    .await??;
    assert_eq!(orbiterx_response.jsonrpc, JsonRpcVersion2_0);
    assert_eq!(orbiterx_response.id, RequestId::Number(orbiterx_request_id));
    assert_eq!(
        orbiterx_response.result,
        json!({
            "content": [
                {
                    "text": "Enjoy!",
            ...
            "structuredContent": {
                "threadId": orbiterx_response
                    .result
                    .get("structuredContent")
                    .and_then(|v| v.get("threadId"))
                    .and_then(serde_json::Value::as_str)
                    .expect("orbiterx tool response should include structuredContent.threadId"),
                "content": "Enjoy!"
            }
        })
    );
...
fn create_expected_patch_approval_elicitation_request_params(
    ...
    message_lines.push("Allow OrbiterX to apply proposed code changes?".to_string());
    let params_json = serde_json::to_value(PatchApprovalElicitRequestParams {
        message: message_lines.join("\n"),
        requested_schema: json!({"type":"object","properties":{}}),
        thread_id,
        orbiterx_elicitation: "patch-approval".to_string(),
        orbiterx_mcp_tool_call_id,
        orbiterx_event_id,
        orbiterx_reason: reason,
        orbiterx_grant_root: grant_root,
        orbiterx_changes: changes,
        orbiterx_call_id: "call1234".to_string(),
    })?;
...
/// Create a OrbiterX config that uses the mock server as the model provider.
