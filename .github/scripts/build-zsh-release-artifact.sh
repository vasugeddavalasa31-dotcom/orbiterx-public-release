...
work_root="$(mktemp -d "${temp_root%/}/orbiterx-zsh-release.XXXXXX")"
trap 'rm -rf "$work_root"' EXIT

source_root="${work_root}/zsh"
package_root="${work_root}/orbiterx-zsh"
...
#!/usr/bin/env bash
set -euo pipefail
: "${ORBITERX_WRAPPER_LOG:?missing ORBITERX_WRAPPER_LOG}"
printf '%s\n' "$@" > "$ORBITERX_WRAPPER_LOG"
file="$1"
shift
if [[ "$#" -eq 0 ]]; then
  exec "$file"
...
ORBITERX_WRAPPER_LOG="$wrapper_log_path" \
...
(cd "$work_root" && tar -czf "${workspace}/${archive_path}" orbiterx-zsh)
