...
usage() {
  cat <<'EOF'
Usage: build-orbiterx-package-archive.sh \
  --target <rust-target> \
  ...
  [--zsh-manifest <path>] \
  [--orbiterx-command-runner-bin <path>] \
  [--orbiterx-windows-sandbox-setup-bin <path>] \
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --orbiterx-command-runner-bin)
      resource_args+=(
        --orbiterx-command-runner-bin
        "${2:?--orbiterx-command-runner-bin requires a value}"
      )
    ...
    --orbiterx-windows-sandbox-setup-bin)
      resource_args+=(
        --orbiterx-windows-sandbox-setup-bin
        "${2:?--orbiterx-windows-sandbox-setup-bin requires a value}"
      )
...
case "$bundle" in
  primary)
    variant="orbiterx"
    entrypoint="orbiterx"
    archive_stem="orbiterx-package"
    ;;
  app-server)
    variant="orbiterx-app-server"
    entrypoint="orbiterx-app-server"
    archive_stem="orbiterx-app-server-package"
    ;;
  *)
    echo "No OrbiterX package variant for bundle: $bundle" >&2
    exit 1
    ;;
esac
...
code_mode_host_bin="${entrypoint_dir%/}/orbiterx-code-mode-host${exe_suffix}"
...
case "$target" in
  *linux*)
    ...
    ;;
  *windows*)
    command_runner_bin="${entrypoint_dir%/}/orbiterx-command-runner.exe"
    sandbox_setup_bin="${entrypoint_dir%/}/orbiterx-windows-sandbox-setup.exe"
    if [[ "$command_runner_bin_provided" == "false" && -f "$command_runner_bin" ]]; then
      resource_args+=(--orbiterx-command-runner-bin "$command_runner_bin")
    fi
    if [[ "$sandbox_setup_bin_provided" == "false" && -f "$sandbox_setup_bin" ]]; then
      resource_args+=(--orbiterx-windows-sandbox-setup-bin "$sandbox_setup_bin")
    fi
    ;;
esac
...
python_args=(
  "${repo_root}/scripts/build_orbiterx_package.py"
  --target "$target"
  --variant "$variant"
